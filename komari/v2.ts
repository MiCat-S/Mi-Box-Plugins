...
export default function createKomari(){return definePlugin({renderHelp: help, apiVersion: STRUCTURED_PLUGIN_API_VERSION,id:"komari",description:"查询 Komari 服务与节点状态",commands:{komari: command},settings:c=>({id:"komari",title:"Komari",category:"插件配置",icon:"📡",getSchema:()=>[{key:"url",label:"服务地址",type:"string"}],getValues:async()=>({url:(await store(c).read()).url}),async setValues(p){if(typeof p.url!=="string")throw new Error("invalid URL");const url=p.url;base(url);await store(c).update(v=>({...v,url:url.replace(/\/+$/,"")}));}}),setup:migrate});}
