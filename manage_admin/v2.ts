...
const inGroup = (message: MessageEnvelope): boolean => {
  ...
  return raw?.isPrivate !== true && (message.chatId.startsWith("-") || raw?.isGroup === true || raw?.isChannel === true);
};
...
].filter(Boolean).map(escape).concat(`<a href="tg://user?id=${escape(id)}">${escape(id)}</a>`).join(" ");

async function deliver(context: PluginContext, message: MessageEnvelope, html: string, signal: AbortSignal): Promise<void> {
  ...
  signal.throwIfAborted();
  try { await context.telegram.reply(message, ui.interruptedNotice(delivery)); }
  catch { context.log.error("manage_admin_delivery_notice_failed"); }
}

async function run(message: MessageEnvelope, args: readonly string[], prefix: string, context: PluginContext): Promise<void> {
  if (!inGroup(message)) {
    await context.telegram.edit(message, `请在群组/频道对话中使用 <code>${escape(prefix)}manage_admin</code> 命令`, {parseMode: "html"});
    return;
  }
  ...
  await context.telegram.withClient(async (client: any, signal) => {
    ...
    const resolveTarget = async (): Promise<{entity: any; input: any; id: string} | undefined> => {
      ...
      try {
        ...
        for (let page = 0; page < 5; page += 1) {
          signal.throwIfAborted();
          const result = await invoke(new Api.channels.GetParticipants({
            channel, filter: new Api.ChannelParticipantsRecent(), offset, limit: 200, hash: 0 as any,
          }));
          const participants = result?.participants ?? [];
          const users = result?.users ?? [];
          const participant = participants.find((value: any) => String(value.userId ?? value.peer?.userId) === targetToken);
    ...
    if (["ls", "list"].includes(subcommand)) {
      ...
      try {
        const result = await invoke(new Api.channels.GetParticipants({
          channel, filter: new Api.ChannelParticipantsAdmins(), offset: 0, limit: 200, hash: 0 as any,
        }));
        ...
        await deliver(context, message, lines.length ? `当前管理员列表：\n${lines.join("\n")}` : "当前对话没有管理员或无法获取", signal);
      } catch {
        signal.throwIfAborted();
    ...
    if (chat.className === "Channel") {
      try {
        const permission = await invoke(new Api.channels.GetParticipant({channel, participant: new Api.InputPeerSelf()}));
        const self = permission?.participant;
        allowed = self?.className === "ChannelParticipantCreator" ||
          (self?.className === "ChannelParticipantAdmin" && self.adminRights?.addAdmins === true);
      } catch { signal.throwIfAborted(); allowed = false; }
    }
    if (!allowed) {
    ...
    const title = Array.from((reply ? args.slice(1) : args.slice(2)).join(" ")).slice(0, 16).join("");
    let applied = false;
    try {
      if (chat.className === "Channel") {
        await invoke(new Api.channels.EditAdmin({
          channel, userId: target.input,
          adminRights: new Api.ChatAdminRights(adding ? {banUsers: true} : {}), rank: adding ? title : "",
        }));
      } else {
      ...
      if (adding && chat.className === "Channel") {
        ...
        try {
          const self = (await invoke(new Api.channels.GetParticipant({channel, participant: new Api.InputPeerSelf()}))).participant;
          selfIsCreator = self?.className === "ChannelParticipantCreator";
          const refreshed = (await invoke(new Api.channels.GetParticipant({channel, participant: target.input}))).participant;
          if (["ChannelParticipantAdmin", "ChannelParticipantCreator"].includes(refreshed?.className)) appliedRank = refreshed.rank ?? "";
        } catch { signal.throwIfAborted(); }
      }
      const rankText = adding && title ? appliedRank === title
        ? `，头衔：<code>${escape(title)}</code>`
        : `，但头衔未更新。${selfIsCreator ? "可能原因：非超级群或系统暂未同步。" : "可能原因：仅群主可设置头衔；或非超级群；或系统暂未同步。"}` : "";
      await context.telegram.edit(message, `${adding ? "已设置" : "已移除"}管理员: ${display(target.entity, target.id)}${rankText}`, {parseMode: "html"});
    } catch {
      signal.throwIfAborted();
      if (applied) {
      ...
      await context.telegram.edit(message, `${adding ? "设置" : "移除"}管理员失败，请确认目标属于当前对话且账号拥有所需权限`);
    }
  });
}
...
export default function createManageAdmin() {
  return definePlugin({
    ...
    commands: {manage_admin: {
      helpArgs: ["help", "h"], helpOnEmpty: true, description: "管理管理员", ignoreEdited: true,
      async handle({message, args, prefix}, context) { await run(message, args, prefix, context); },
    }},
  });
}
