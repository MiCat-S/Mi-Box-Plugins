...
async function writeAll(handle: FileHandle, chunk: Uint8Array, signal: AbortSignal): Promise<void> {
  let offset = 0;
...
type DownloadResult = { ok: true; file: string; size: number; extension: string } | { ok: false; reason: string };
...
function hasAudioSignature(value: Uint8Array): boolean {
  const ascii = (start: number, text: string) =>
    text.split("").every((character, offset) => value[start + offset] === character.charCodeAt(0));
  return (
    ascii(0, "ID3") ||
    ascii(0, "fLaC") ||
    ascii(0, "OggS") ||
...
function safeDownloadUrl(value: unknown): URL {
  ...
  if (url.protocol !== "https:" || url.username || url.password)
    throw new MusicHubUserError("播放链接不是安全的 HTTPS 地址");
  const host = url.hostname.toLowerCase();
  if (host === "localhost" || host.endsWith(".localhost")) throw new MusicHubUserError("播放链接主机不可用");
  const literal = host.replace(/^\[|\]$/g, "");
  const family = isIP(literal);
  if (family && blocked.check(literal, family === 4 ? "ipv4" : "ipv6"))
    throw new MusicHubUserError("播放链接主机不可用");
  return url;
}
...
async function responseJson(response: Response, signal: AbortSignal): Promise<unknown> {
  ...
  const cancel = () => (cancelPromise ??= reader.cancel());
  const onAbort = () => {
  ...
  try {
    ...
    if (!done) await cancel().catch(() => undefined);
    reader.releaseLock();
  }
  try {
...
async function api(
  ...
  const value = await context.http.withResponse(
    ...
    responseJson,
    {
      ...
      redirects: { allowedHosts: [API.hostname], maxRedirects: 2 },
    },
  );
...
export async function searchMusic(
  ...
  throw new MusicHubUserError("没有搜索结果或音源不可用");
}
  ...
  return context.http.withResponse(
    ...
    async (response, signal): Promise<DownloadResult> => {
      if (response.status < 200 || response.status >= 300 || !response.body)
        return { ok: false, reason: `音频下载失败（HTTP ${response.status}）` };
      ...
      if (Number.isFinite(announced) && announced > config.maxUploadBytes)
        return { ok: false, reason: "文件超过上传大小限制" };
      ...
      try {
        for (;;) {
          ...
          if (signatureChecked) await writeAll(handle, chunk.value, signal);
          else {
            pending.push(chunk.value);
            pendingBytes += chunk.value.byteLength;
            if (pendingBytes >= 12) {
              if (!hasAudioSignature(Buffer.concat(pending, pendingBytes).subarray(0, 12))) {
                unsupported = true;
                break;
              }
              signatureChecked = true;
              for (const value of pending) await writeAll(handle, value, signal);
              pending.length = 0;
            }
          }
        }
        if (!signatureChecked && !oversized && !unsupported) {
          ...
          else for (const value of pending) await writeAll(handle, value, signal);
        }
      } finally {
        signal.removeEventListener("abort", onAbort);
        try {
    ...
    },
    {
      ...
      denyPrivateAddresses: true,
      redirects: { allowedHosts: [host], maxRedirects: 3 },
    },
  );
}
