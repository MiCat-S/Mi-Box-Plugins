...
import {
  autoSourceOrder, MusicHubUserError, normalizeSearchResults, type ApiSong, type MusicHubConfig,
...
function safeDownloadUrl(value: unknown): URL {
  ...
  if (url.protocol !== "https:" || url.username || url.password) throw new MusicHubUserError("播放链接不是安全的 HTTPS 地址");
  const host = url.hostname.toLowerCase();
  if (host === "localhost" || host.endsWith(".localhost")) throw new MusicHubUserError("播放链接主机不可用");
  const literal = host.replace(/^\[|\]$/g, "");
  const family = isIP(literal);
  if (family && blocked.check(literal, family === 4 ? "ipv4" : "ipv6")) throw new MusicHubUserError("播放链接主机不可用");
  return url;
}
...
async function responseJson(response: Response, signal: AbortSignal): Promise<unknown> {
  ...
  let done=false,cancelPromise:Promise<void>|undefined;
...
export async function downloadSong(
  ...
  return context.http.withResponse(info.url, {
    ...
    try {
      ...
      signal.removeEventListener("abort",onAbort);
      try{if(!done)await cancel().catch(()=>undefined);}
