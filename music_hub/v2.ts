...
function searchPage(session: SearchSession, prefix: string): string {
  ...
  const selected =
    session.requestedSource === "auto"
      ? `${sourceLabel("auto")} → ${sourceLabel(session.resolvedSource)}`
...
async function sendFile(
  ...
  song: ApiSong,
  info: Awaited<ReturnType<typeof songUrl>>,
  config: MusicHubConfig,
): Promise<void> {
  let sent = false;
  try {
    await context.files.withTemp(async (directory, tempSignal) => {
      const signal = AbortSignal.any([context.signal, tempSignal]);
      signal.throwIfAborted();
      const downloaded = await downloadSong({ ...context, signal } as PluginContext, song, info, config, directory);
      signal.throwIfAborted();
      if (!downloaded.ok) throw new MusicHubUserError(downloaded.reason);
      await context.telegram.edit(message, `📤 正在上传 ${escape(song.name)}（${formatBytes(downloaded.size)}）`, {
        parseMode: "html",
      });
      signal.throwIfAborted();
      await context.telegram.withClient(async (client, clientSignal) => {
        const uploadSignal = AbortSignal.any([signal, clientSignal]);
        const { Api } = await import("teleproto");
        const [{ CustomFile }, { returnBigInt }] = await Promise.all([
        ...
        const peer =
          raw?.peerId ??
          (await client.getInputEntity(/^-?\d+$/.test(message.chatId) ? returnBigInt(message.chatId) : message.chatId));
        uploadSignal.throwIfAborted();
        await client.sendFile(peer, {
          ...
          attributes: [
            new Api.DocumentAttributeAudio({
              duration: 0,
              title: bounded(song.name, 128),
              performer: bounded(formatArtists(song.artist), 128),
            }),
          ],
        });
        sent = true;
    ...
    if (sent && !context.signal.aborted) {
      context.log.info("music_hub_temp_cleanup_failed");
      return;
    }
    throw error;
  }
...
export default function createMusicHub() {
  ...
  const configuration = async (context: PluginContext): Promise<MusicHubConfig> =>
    normalizeConfig(await store(context).read());
  ...
  const fail = async (invocation: CommandInvocation, context: PluginContext, error: unknown): Promise<void> => {
    ...
    await context.telegram.edit(
      invocation.message,
      `❌ <b>Music Hub 操作失败</b>\n${escape(error instanceof MusicHubUserError ? error.feedback : "请稍后重试")}`,
      { parseMode: "html", linkPreview: false },
    );
  };
  const guarded =
    (
      operation: (invocation: CommandInvocation, context: PluginContext) => Promise<void>,
    ): CommandDefinition["handle"] =>
    async (invocation, context) => {
      try {
        await operation(invocation, context);
      } catch (error) {
        await fail(invocation, context, error);
      }
    };
  ...
  const runSearch = async (
    ...
    query = query.trim();
    if (!query) throw new MusicHubUserError("请提供搜索关键词");
    if (query.length > 200) throw new MusicHubUserError("搜索关键词最多 200 字符");
    const key = keyFor(invocation.message),
      generation = ++searchSequence;
    searches.set(key, generation);
    try {
      await context.telegram.edit(invocation.message, `🔍 正在通过 ${escape(sourceLabel(mode))} 搜索…`, {
        parseMode: "html",
      });
      const session = await searchMusic(context, await configuration(context), mode, query);
      if (searches.get(key) !== generation) return;
      saveSession(invocation.message, session);
      await context.telegram.edit(invocation.message, searchPage(session, invocation.prefix), {
        parseMode: "html",
        linkPreview: false,
      });
    } catch (error) {
      ...
      if (searches.get(key) === generation) searches.delete(key);
    }
  };
  ...
  const sourceSearch = (source: SourceMode): SubcommandDefinition => {
    ...
    return {
      ...
      args: "关键词",
      arguments: [{ name: "关键词", required: true }],
  ...
  const setDefault = guarded(async (invocation, context) => {
    ...
    if (!source) throw new MusicHubUserError("不支持该音乐源");
  ...
  const defaultSource: SubcommandDefinition = {
    description: "查看或设置默认音乐源",
    aliases: ["set"],
    args: "[音乐源]",
    examples: [{ args: "default auto" }, { args: "default netease" }],
    handle: setDefault,
  };
  const sourceCompatibility: SubcommandDefinition = {
    description: "查看音乐源；提供参数时设置默认源",
    args: "[音乐源]",
    examples: [{ args: "source" }, { args: "source kuwo" }],
    handle: guarded(async (invocation, context) => {
  ...
  const bitrate: SubcommandDefinition = {
    description: "查看或设置默认码率",
    aliases: ["quality"],
    args: "[low|medium|high|数值]",
    examples: [{ args: "br medium" }],
    handle: guarded(async (invocation, context) => {
      ...
      if (!br) throw new MusicHubUserError("码率须为 low、medium、high 或 2 至 4 位数字");
  ...
  const navigate = (delta: number): SubcommandDefinition => ({
    ...
    aliases: [delta > 0 ? "n" : "p"],
    args: "",
    examples: [{ args: delta > 0 ? "next" : "prev" }],
    handle: guarded(async (invocation, context) => {
      const session = getSession(invocation.message);
      if (!session) throw new MusicHubUserError("没有可翻页的搜索结果，请先搜索");
  ...
  const page: SubcommandDefinition = {
    description: "跳转到指定结果页",
    args: "页码",
    arguments: [{ name: "页码", required: true }],
    examples: [{ args: "page 2" }],
    handle: guarded(async (invocation, context) => {
      const session = getSession(invocation.message);
      if (!session) throw new MusicHubUserError("没有可翻页的搜索结果，请先搜索");
      const selected = parsePositiveIndex(invocation.args[0]);
      if (!selected) throw new MusicHubUserError("页码必须是正整数");
  ...
  const select = guarded(async (invocation, context) => {
    ...
    if (!index) throw new MusicHubUserError("请提供歌曲序号");
    const session = getSession(invocation.message);
    if (!session) throw new MusicHubUserError("没有可选择的搜索结果，请先搜索");
    const key = keyFor(invocation.message);
    if (transfers.has(key)) throw new MusicHubUserError("当前搜索结果正在传输，请勿重复选择");
    const song = session.results[index - 1];
    if (!song) throw new MusicHubUserError(`歌曲序号须在 1 至 ${session.results.length} 之间`);
    transfers.add(key);
    try {
      const config = await configuration(context);
      await context.telegram.edit(invocation.message, `🔗 正在获取 ${escape(bounded(song.name, 240))} 的播放链接…`, {
        parseMode: "html",
      });
      ...
      try {
        ...
        context.log.info("music_hub_completion_feedback_failed");
      }
    } finally {
  ...
  const play: SubcommandDefinition = {
    description: "下载并发送指定序号的歌曲",
    aliases: ["download", "get"],
    args: "序号",
    arguments: [{ name: "序号", required: true }],
    examples: [{ args: "play 1" }],
    handle: select,
  };
  const search: SubcommandDefinition = {
    description: "搜索音乐，可选首个参数指定音乐源",
    aliases: ["s"],
    args: "[音乐源] 关键词",
  ...
  const sourceCommands = Object.fromEntries(
    MUSIC_SOURCES.map(source => [source.key, sourceSearch(source.key)]),
  ) as Record<SourceKey, SubcommandDefinition>;
  const command: CommandDefinition = {
    description: "搜索并下载多个来源的音乐",
    helpArgs: ["help", "h"],
    subcommandsCaseSensitive: false,
    args: "[音乐源|子命令] 关键词",
    examples: [{ args: "周杰伦" }, { args: "netease 周杰伦" }, { args: "1" }],
    subcommands: {
      sources: showSources,
    ...
    help: [
      ...
      {
        heading: "会话与传输：",
        body: "搜索会话按聊天与发送者隔离，30 分钟过期，每页 5 首；下载仅接受安全 HTTPS 音频，同主机重定向与上传字节受限。",
      },
    ],
    handle: guarded(async (invocation, context) => {
  ...
  const help = (prefix: string) =>
    `${renderCommandHelp("mh", command, { prefix, title: "🎵 Music Hub" })}\n\n兼容入口：${code(`${prefix}music_hub`)}，参数与 mh 相同。`;
  ...
  return definePlugin({
    ...
    settings: context => ({
      ...
      getSchema: () => [
        {
          ...
          options: [
            { value: "auto", label: "自动" },
            ...MUSIC_SOURCES.map(source => ({ value: source.key, label: source.name })),
          ],
        },
        {
          key: "br",
      ...
      setValues: async patch => {
        ...
        for (const [name, minimum, maximum] of [
          ["maxResults", PAGE_SIZE, 100],
          ["maxUploadBytes", 1024 * 1024, 2 * 1024 * 1024 * 1024],
        ] as const) {
    ...
    async setup(context) {
      await store(context).update(normalizeConfig);
    },
    cleanup() {
      sessions.clear();
      transfers.clear();
