...
import {
  ...
  MusicHubUserError, parsePositiveIndex, sessionKey, sourceLabel,
...
async function sendFile(
  ...
  let sent=false;
  try{await context.files.withTemp(async (directory,tempSignal) => {
    ...
    await context.telegram.withClient(async (client,clientSignal) => {
      ...
      const [{CustomFile},{returnBigInt}]=await Promise.all([import("teleproto/client/uploads.js"),import("teleproto/Helpers.js")]);
      uploadSignal.throwIfAborted();
      const raw = message.raw as ApiTypes.Message | undefined;
      const peer = raw?.peerId ?? await client.getInputEntity(/^-?\d+$/.test(message.chatId)?returnBigInt(message.chatId):message.chatId);
      uploadSignal.throwIfAborted();
  ...
  });}catch(error){if(sent&&!context.signal.aborted){context.log.info("music_hub_temp_cleanup_failed");return;}throw error;}
}

export default function createMusicHub() {
  const sessions = new Map<string, SearchSession>();
  const transfers = new Set<string>();
  const searches=new Map<string,number>();let searchSequence=0;
  ...
  const runSearch = async (invocation: CommandInvocation, context: PluginContext, mode: SourceMode, query: string): Promise<void> => {
    ...
    const key=keyFor(invocation.message),generation=++searchSequence;searches.set(key,generation);
    try{await context.telegram.edit(invocation.message,`🔍 正在通过 ${escape(sourceLabel(mode))} 搜索…`,{parseMode:"html"});const session=await searchMusic(context,await configuration(context),mode,query);if(searches.get(key)!==generation)return;saveSession(invocation.message,session);await context.telegram.edit(invocation.message,searchPage(session,invocation.prefix),{parseMode:"html",linkPreview:false});}
    catch(error){if(searches.get(key)!==generation)return;throw error;}
    finally{if(searches.get(key)===generation)searches.delete(key);}
  };
  ...
  const clear: SubcommandDefinition = {
    ...
    async handle(invocation, context) {
      const key=keyFor(invocation.message);sessions.delete(key);searches.delete(key);
  ...
  const select = guarded(async (invocation, context) => {
    ...
    try {
      ...
      try{await context.telegram.edit(invocation.message,"音乐已发送");}catch{context.log.info("music_hub_completion_feedback_failed");}
  ...
  return definePlugin({
    ...
    cleanup() {sessions.clear();transfers.clear();searches.clear();},
  });
}
