...
const ROLES: Record<string, string> = {
  ...
  王琨: "4f201abba2574feeae11e5ebf737859e",
  麦辣鸡腿堡: "c293697468924f3089cd9b90520dbc16",
  猪八戒: "4313e3ec56f14eb3946630dbdad01059",
  "夏(中配) 蔚蓝档案": "c5fca4f670214e3cb7fbb9d595552e6e",
  蔚蓝档案阿洛娜: "6ec8168d8392467c82358a780b35c5ca",
...
function profile(state: State, user: string): Profile | undefined {
  return state.users[user] ?? state.users.panel;
}
...
async function state(context: PluginContext) {
  ...
  const value = await store.update(current => ({
    ...
    covers: {
      薯薯: INITIAL.covers["薯薯"],
      ...(current.covers && typeof current.covers === "object" ? current.covers : {}),
    },
  }));
...
async function ffmpeg(
  ...
  for (const command of FFMPEG) {
    try {
      signal.throwIfAborted();
      await context.processes.run(command, args, {
        cwd: directory,
        env: {},
        signal,
        timeoutMs: 180_000,
...
async function downloadCover(
  ...
  try {
    ...
    await context.http.withResponse(
      parsed,
      { credentials: "omit", signal: callerSignal },
      (response, signal) => stream(response, target, AbortSignal.any([callerSignal, signal]), 5 * 1024 * 1024),
      {
        timeoutMs: 20_000,
        denyPrivateAddresses: true,
        redirects: { allowedHosts: [parsed.hostname], maxRedirects: 2 },
      },
    );
    return true;
  } catch {
...
async function send(
  ...
  await context.telegram.withClient(async (client, clientSignal) => {
    const signal = AbortSignal.any([callerSignal, clientSignal]);
    signal.throwIfAborted();
    await client.sendFile(raw.peerId, { file, ...options });
    delivered();
    signal.throwIfAborted();
  });
}
...
async function deleteCommand(context: PluginContext, invocation: any): Promise<void> {
  ...
  try {
    ...
    if ((raw as any).isPrivate === true) await raw.delete({ revoke: true });
    else await raw.delete();
    context.signal.throwIfAborted();
  } catch {
    context.signal.throwIfAborted();
    context.log.error("t_command_cleanup_failed");
  }
}

function userId(invocation: any): string {
  return String(invocation.message.senderId ?? "");
}

export default function createT() {
  const commandT = {
    description: "使用 Fish Audio 合成语音或音乐",
    async handle(invocation: any, context: PluginContext) {
      const id = userId(invocation);
      if (!id) return;
      const loaded = await state(context);
      const config = profile(loaded.value, id);
      if (!config?.apiKey) {
        await context.telegram.edit(invocation.message, `请先在收藏夹使用 ${invocation.prefix}tk APIKey`);
        return;
      }
      if (invocation.args[0]?.toLowerCase() === "fm" && invocation.args[1]) {
        let parsed: URL;
        try {
          parsed = new URL(invocation.args[1]);
        } catch {
          await context.telegram.edit(invocation.message, "封面必须是 HTTPS URL");
          return;
        }
        ...
        await loaded.store.update(value => ({
          ...value,
          covers: { ...value.covers, [config.defaultRole]: parsed.toString() },
        }));
      ...
      try {
        ...
        await context.files.withTemp(async (directory, signal) => {
          ...
          try {
            ...
            const title = music ? invocation.args[0] : "",
              artist = music ? invocation.args[1] : "";
            const album = music && invocation.args.length >= 4 ? invocation.args[2] : config.defaultRole;
            const sourceText = music
              ? invocation.args.slice(invocation.args.length >= 4 ? 3 : 2).join(" ")
              : invocation.args.join(" ") || reply?.text || "";
            const text = clean(sourceText);
            if (!text || text.length > 5000) throw new Error("Invalid text");
            const raw = path.join(directory, "speech.mp3");
            await synthesize(context, config, text, raw, active);
            active.throwIfAborted();
            if (music) {
              const output = path.join(directory, "music.mp3");
              const coverFile = path.join(directory, "cover.jpg");
              const cover = loaded.value.covers?.[config.defaultRole];
              const hasCover = cover ? await downloadCover(context, cover, coverFile, active) : false;
              active.throwIfAborted();
              ...
              await ffmpeg(context, directory, active, args);
              ...
              await send(
                ...
                output,
                {
                  ...
                  replyTo: reply?.id ?? invocation.message.id,
              ...
              await ffmpeg(context, directory, active, [
                "-nostdin",
                "-y",
                "-protocol_whitelist",
                "file",
            ...
            if (active.aborted) cancelled = true;
            throw error;
          }
        });
        ...
        if (delivered) {
          context.log.error("t_temp_cleanup_failed");
          await deleteCommand(context, invocation);
          return;
        }
  ...
  const commandTs = {
    description: "查看、切换或增加语音角色",
    async handle(invocation: any, context: PluginContext) {
      const id = userId(invocation);
      if (!id) return;
      const loaded = await state(context);
      const names = Object.keys(loaded.value.roles);
      const page =
        invocation.args.length === 1 && /^\d+$/.test(invocation.args[0])
          ? Math.max(1, Number(invocation.args[0]))
          : undefined;
      if (!invocation.args.length || page) {
        const pages = Math.max(1, Math.ceil(names.length / 20));
        const selected = Math.min(page ?? 1, pages);
        const start = (selected - 1) * 20;
        await context.telegram.edit(
          invocation.message,
          `可用角色（${names.length}） | 第 ${selected}/${pages} 页\n当前：${profile(loaded.value, id)?.defaultRole ?? "未设置"}\n\n` +
            names
              .slice(start, start + 20)
              .map((name, index) => `${start + index + 1}. ${name}`)
              .join("\n") +
            `\n\n用法：\n• ${invocation.prefix}ts 角色名（切换）\n• ${invocation.prefix}ts 角色名 角色ID（新增/更新并切换）\n• ${invocation.prefix}ts 2（查看第 2 页）`,
        );
        return;
      }
      const name = invocation.args[0],
        roleId = invocation.args[1];
      if (!roleId && !loaded.value.roles[name]) {
        await context.telegram.edit(invocation.message, `无效的角色名：${name}`);
        return;
      }
      await loaded.store.update(value => ({
        ...value,
        roles: roleId ? { ...value.roles, [name]: roleId } : value.roles,
        users: {
          ...value.users,
          [id]: {
            ...(profile(value as State, id) ?? { apiKey: "" }),
            defaultRole: name,
      ...
      await context.telegram.edit(
        invocation.message,
        roleId ? `已新增/更新角色：${name}，并切换为默认` : `默认角色已切换为：${name}`,
      );
    },
  };
  const commandTk = {
    description: "在收藏夹设置 Fish Audio API Key",
    async handle(invocation: any, context: PluginContext) {
      if (!invocation.message.saved) {
        await context.telegram.edit(invocation.message, "请仅在收藏夹中设置 API Key");
        return;
      }
      const id = userId(invocation);
      const key = invocation.args[0] ?? "";
      if (!id || !key) {
        await context.telegram.edit(invocation.message, "请提供 API Key");
        return;
      }
      const loaded = await state(context);
      const existing = profile(loaded.value, id) ?? {
        apiKey: "",
        defaultRole: "雷军",
        defaultRoleId: loaded.value.roles["雷军"],
      };
  ...
  return definePlugin({
    ...
    apiVersion: 1,
    id: "t",
    description: "Fish Audio 文字转语音与音乐",
    ...
    settings: context => ({
      ...
      icon: "🔊",
      getSchema: () => [
        { key: "apiKey", label: "API 密钥", type: "password", secret: true },
        { key: "defaultRole", label: "默认角色", type: "string" },
        { key: "defaultRoleId", label: "默认角色 ID", type: "string" },
      ],
      async getValues() {
        const value = (await state(context)).value.users.panel ?? {
          apiKey: "",
          defaultRole: "雷军",
          defaultRoleId: ROLES["雷军"],
        };
        return value;
      },
      async setValues(patch) {
        const loaded = await state(context);
        const current = loaded.value.users.panel ?? { apiKey: "", defaultRole: "雷军", defaultRoleId: ROLES["雷军"] };
