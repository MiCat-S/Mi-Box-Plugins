...
function safeName(value:string,fallback:string){return value.normalize("NFC").replace(/[\\/:*?"<>|\x00-\x1f]/g,"_").replace(/^\.+/,"").slice(0,120)||fallback;}
function userId(message:MessageEnvelope){return message.senderId||"self";}
async function migrateLegacyConfig(ctx:PluginContext){const current=await database(ctx).read();if(Object.keys(current.users||{}).length||typeof ctx.files.dataPath!=="function")return;const legacy=path.resolve(ctx.files.dataPath(),"..","prometheus","config.json");try{const info=await lstat(legacy);if(!info.isFile()||info.isSymbolicLink())return;const parsed=JSON.parse(await readFile(legacy,"utf8"));if(!parsed?.users||typeof parsed.users!=="object")return;const users:Record<string,UserConfig>={};for(const[id,value]of Object.entries(parsed.users as Record<string,any>)){if(value&&typeof value.target==="string")users[String(id)]={target:value.target,showSource:value.showSource===true};}if(Object.keys(users).length)await database(ctx).update(data=>({...data,schemaVersion:1,users:{...users,...data.users}}));}catch(error){if(!(error instanceof Error&&"code" in error&&error.code==="ENOENT"))ctx.log.error("save.legacy_config_migration_failed",{message:error instanceof Error?error.message:String(error)});}}
async function config(ctx:PluginContext,id:string){await migrateLegacyConfig(ctx);const data=await database(ctx).read();return data.users[id]||{target:"me",showSource:false};}
async function setConfig(ctx:PluginContext,id:string,patch:Partial<UserConfig>){await migrateLegacyConfig(ctx);await database(ctx).update(data=>({...data,schemaVersion:1,users:{...data.users,[id]:{target:data.users[id]?.target||"me",showSource:data.users[id]?.showSource??false,...patch}}}));}
async function sourceMessage(ctx:PluginContext,value:Link){return ctx.telegram.withClient(async(client:any)=>{const peer=await client.getInputEntity(/^-?\d+$/.test(value.chatId)?returnBigInt(value.chatId):value.chatId);const messages:any[]=await client.getMessages(peer,{ids:[value.messageId]});return messages[0]?{message:messages[0],peer}:undefined;});}
function mediaName(message:any){const document=message?.media?.document;const named=document?.attributes?.find((item:any)=>item?.className==="DocumentAttributeFilename")?.fileName;const mime=String(document?.mimeType||"");const ext=named?path.extname(named):mime.includes("jpeg")?".jpg":mime.includes("png")?".png":mime.includes("webp")?".webp":mime.includes("mp4")?".mp4":mime.includes("ogg")?".ogg":mime.includes("mpeg")?".mp3":".bin";return safeName(named||`message_${message.id}${ext}`,`message_${message.id}.bin`);}
const maxMediaBytes=2*1024*1024*1024;
async function writeAll(file:Awaited<ReturnType<typeof open>>,chunk:Uint8Array){let offset=0;while(offset<chunk.length){const result=await file.write(chunk,offset,chunk.length-offset);if(result.bytesWritten<=0)throw new Error("媒体写入失败");offset+=result.bytesWritten;}}
async function downloadTo(ctx:PluginContext,message:any,target:string):Promise<number>{return ctx.telegram.withClient(async(client:any,signal)=>{const file=await open(target,"wx");let size=0;try{for await(const chunk of client.iterDownload(message.media,{})){signal.throwIfAborted();size+=chunk.length;if(size>maxMediaBytes)throw new Error("媒体超过 2 GiB 上限");await writeAll(file,chunk);}if(!size)throw new Error("下载媒体失败");return size;}finally{await file.close();}});}
async function copyAcross(source:string,target:string){try{await rename(source,target);}catch{await copyFile(source,target);}}

async function saveLocal(ctx:PluginContext,source:any,info:Link){if(!source.media)return{ok:false,skipped:true};const root=await ctx.files.dataDirectory("saved");const chat=path.join(root,safeName(info.chatId,"chat"));await mkdir(chat,{recursive:true});const base=mediaName(source);let target=path.join(chat,`msg_${info.messageId}_${base}`);for(let index=1;;index++){try{await stat(target);target=path.join(chat,`msg_${info.messageId}_${index}_${base}`);}catch{break;}}let size=0;await ctx.files.withTemp(async directory=>{const temporary=path.join(directory,base);size=await downloadTo(ctx,source,temporary);await copyAcross(temporary,target);});const metadata=`${target}.json`;await writeFile(metadata,JSON.stringify({schemaVersion:1,savedAt:new Date().toISOString(),source:{chatId:info.chatId,messageId:info.messageId,link:linkFor(info.chatId,info.messageId)},media:{fileName:path.basename(target),mimeType:source.media?.document?.mimeType||null,fileSize:size}},null,2),"utf8");return{ok:true,file:path.relative(root,target),metadata:path.relative(root,metadata)};}
...
async function sourceNotice(ctx:PluginContext,target:any,last:any,sources:Link[]){
  ...
  const lines=unique.map(item=>`• <a href="${escape(linkFor(item.chatId,item.messageId))}">${escape(item.chatId)} / ${item.messageId}</a>`);
  const pages:string[]=[];let page="🔗 <b>消息来源</b>";
  for(const line of lines){if(page.length+line.length+1>3500){pages.push(page);page="🔗 <b>消息来源（续）</b>";}page+=`\n${line}`;}
  if(page)pages.push(page);
  await ctx.telegram.withClient(async(client:any)=>{let replyTo=last.id;for(const value of pages){ctx.signal.throwIfAborted();const sent=await client.sendMessage(target,{message:value,parseMode:"html",replyTo});if(sent?.id)replyTo=sent.id;}});
}
...
const command: CommandDefinition = {
  ...
  handle: guarded(async ({message, args, prefix}, ctx) => {
    ...
    const id = userId(message);
      ...
      let noticeFailed=false;if(!local&&current.showSource)try{await sourceNotice(ctx,target,last,sources);}catch{if(ctx.signal.aborted)return;noticeFailed=true;ctx.log.error("save_source_notice_failed");}await ctx.telegram.edit(message,local?`✅ 本地保存完成\n已保存: ${succeeded}\n跳过: ${skipped}\n失败: ${failed}\n目录: <code>saved/</code>`:`✅ 处理完成\n成功: ${succeeded}/${links.length}\n失败: ${failed}${noticeFailed?"\n来源通知发送失败":""}`,{parseMode:"html"});
