...
function normalize(input: unknown): State {
  ...
  const rules: Rule[] = rawRules.flatMap(item => {
    ...
    return [{id: value.id ? String(value.id) : "", command, delay, ...(parameters?.length ? {parameters} : {}),
  ...
  if (legacyV0) {
    const ruleKey = (rule: Rule) => `${rule.command}:${rule.parameters?.join(",") ?? ""}:${rule.exactMatch ? "exact" : "normal"}`;
  ...
  return {...source, ...(legacyV0 ? {configVersion: 1} : {}), schemaVersion: 2, enabled: source.enabled === true, rules, pending};
}
...
function match(rules: Rule[], command: string, args: readonly string[]): Rule | undefined {
  return rules.find(rule => rule.command === command && rule.parameters?.includes(args[0] ?? ""))
    ?? (args.length === 0 ? rules.find(rule => rule.command === command && !rule.parameters?.length && rule.exactMatch) : undefined)
...
export default function createPlugin() {
  ...
  commands: {autodelcmd: {helpArgs: ["help", "h"], helpOnEmpty: true, description: "管理命令自动删除规则", async handle({message, args, prefix}, context) {
    ...
    if (action === "reset") {
      ...
      await context.telegram.edit(message, `✅ 已重置为默认配置，共 ${defaultRules().length} 条规则`); return;
    }
    if (action === "add") {
      ...
      await database(context).update(value => {
        const rules = value.rules.map(rule => ({...rule, ...(rule.parameters ? {parameters: [...rule.parameters]} : {})}));
        const conflict = rules.find(rule => rule.command === command && !!rule.exactMatch === exact &&
          (!parameters.length ? !rule.parameters?.length : parameters.some(parameter => rule.parameters?.includes(parameter))));
        ...
        if (merge && parameters.length) merge.parameters = [...new Set([...(merge.parameters ?? []), ...parameters])];
          ...
          const retained = parameters.length ? rules : rules.filter(rule =>
            !(rule.command === command && !rule.parameters?.length && !!rule.exactMatch === exact));
          retained.push({id: nextId(retained), command, delay, ...(parameters.length ? {parameters} : {}), ...(response ? {deleteResponse: true} : {}), ...(exact ? {exactMatch: true} : {})});
          return {...value, rules: retained};
        }
        return {...value, rules};
      });
      if (conflictId) { await context.telegram.edit(message, `❌ 规则冲突，请先删除 ID ${escape(conflictId)}`, {parseMode: "html"}); return; }
      await context.telegram.edit(message, "✅ 已保存自动删除规则"); return;
    }
    if (["del", "remove"].includes(action)) {
      const target = args[1];
      if (!target) { await context.telegram.edit(message, `❌ 用法：<code>${prefix}autodelcmd del [规则ID或命令名]</code>`, {parseMode: "html"}); return; }
      let removed = false;
      await database(context).update(value => {
      ...
      if (removed) { await context.telegram.edit(message, "✅ 已删除自动删除规则"); return; }
      const matches = (await database(context).read()).rules.filter(rule => rule.command === target.toLowerCase());
      if (!matches.length) { await context.telegram.edit(message, `❌ 未找到规则 ID 或命令 ${escape(target)}`, {parseMode: "html"}); return; }
      const body = matches.map((rule, index) => `${index + 1}. <code>${escape(rule.command)}${rule.parameters?.length ? ` [${rule.parameters.map(escape).join(", ")}]` : ""}</code> → ${rule.delay}秒${rule.deleteResponse ? " 🔄" : ""}${rule.exactMatch ? " 🎯" : ""} <code>[ID: ${escape(rule.id)}]</code>`).join("\n");
      await context.telegram.edit(message, `📋 <b>命令 “${escape(target)}” 的自动删除规则</b>\n\n${body}`, {parseMode: "html"}); return;
    }
  ...
  listeners: [{edited: false, ignoreCommands: false, async handle(message, context) {
    ...
    const route = context.commands.parse(message.text); if (!route) return;
