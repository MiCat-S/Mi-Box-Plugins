...
async function resultPages(values: readonly string[]): Promise<readonly string[]> {
  ...
  for (const value of values) {
    ...
    for (const character of value) {
      ...
      if (chunk && chunk.length + encoded.length > 3300) {
  ...
  for (const block of blocks) {
    if (page.length + block.length + 2 > 3480) {
      pages.push(page);
      page = "";
    }
    page += `${page ? "\n\n" : ""}${block}`;
...
async function download(context: PluginContext, source: Api.Message): Promise<Buffer> {
  ...
  return context.telegram.withClient(async (client: any, signal) => {
    ...
    if (typeof client.iterDownload !== "function") {
      const value = await client.downloadMedia(source.media!, {
        outputFile: Buffer.alloc(0),
        signal: active,
        progressCallback(received: any) {
          active.throwIfAborted();
          if (typeof received?.greater === "function" && received.greater(MAX_IMAGE_BYTES))
...
async function sendQr(context: PluginContext, invocation: any, input: string): Promise<void> {
  ...
  await context.telegram.withClient(async (client, signal) => {
    ...
    await client.sendFile(raw.peerId, {
      file: new CustomFile("qrcode.png", image.length, "", image),
      caption: "二维码生成完成",
    ...
    if (typeof raw.delete === "function") {
      try {
        ...
        active.throwIfAborted();
        context.log.error("qr_command_cleanup_failed");
      }
    }
  });
}
...
export default function createQr() {
  const command: CommandDefinition = {
    ...
    examples: [
      { args: "Hello World" },
      { args: "", description: "回复文本生成二维码；回复图片识别二维码，也可读取命令消息中的图片" },
    ],
    help: [
      {
        heading: "功能与限制：",
        body: "生成 PNG 二维码，输入最多 4000 字节；识别图片上限 20 MiB，一次最多返回 20 条内容。生成成功后删除命令消息。",
      },
      {
        heading: "系统依赖：",
        body: "需要 qrencode 与 zbarimg。macOS：<code>brew install qrencode zbar</code>；Ubuntu/Debian：<code>sudo apt-get install qrencode zbar-tools</code>；CentOS/RHEL：<code>sudo yum install qrencode zbar</code>。",
      },
    ],
    ...
    async handle(invocation, context) {
      ...
      try {
        if (input) {
          await sendQr(context, invocation, input);
          return;
        }
        const reply =
          invocation.message.replyToId === undefined ? undefined : await context.telegram.getReply(invocation.message);
        const source = (media(invocation.message.raw as Api.Message | undefined) ? invocation.message : reply)?.raw as
        ...
        if (media(source)) {
          ...
          if (!values.length) await context.telegram.edit(invocation.message, "未在图片中识别到二维码");
            ...
            const pages = await resultPages(values),
              delivery = await ui.deliverPages(pages, context.signal, (page, index) =>
                index
            ...
            if (delivery.interrupted) {
              context.log.info("qr_result_delivery_interrupted", {
        ...
        if (reply?.text) {
          await sendQr(context, invocation, reply.text);
          return;
        }
        ...
        await context.telegram.edit(
          invocation.message,
          "二维码操作失败，请确认输入有效且服务器已安装 qrencode 与 zbarimg",
        );
      }
    },
  };
  ...
  return definePlugin({
    ...
    id: "qr",
    description: "生成或识别二维码",
    resources: { processes: { concurrency: 1, queueCapacity: 4, timeoutMs: 30_000, maxOutputBytes: 2 * 1024 * 1024 } },
    commands: {
      qr: command,
    },
  });
