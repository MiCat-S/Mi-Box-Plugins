...
import {
  DEL_RE_MAX_LIMIT, TASK_TYPES, buildCopyCommand, createDefaults, ensureCron, escapeHtml, formatDate,
  getRemarkFromMsg, hasSixCronFields, nextRunTime, parseBoolFlag, parseRegex, parseRegexInput, renderTaskList, splitTarget,
  validateCronExpr, type State, type Task, type TaskType,
} from "./v2/tasks";
...
function formatEntityDisplay(entity: unknown, fallback: string): string {
  const value = entity as {title?: unknown; firstName?: unknown; lastName?: unknown; username?: unknown; id?: unknown} | undefined;
  const parts: string[] = [];
  ...
  if (id) {
    parts.push(entity instanceof Api.User
...
export default function createAcron() {
  ...
  const execute = async (context: PluginContext, id: string) => serial(async () => {
    const store = context.storage.json<State>("acron_config.json", createDefaults()); const state = await store.read(); const task = state.tasks.find(item => item.id === id); if (!task || task.disabled) return;
    ...
    try {
      ...
      await context.telegram.withClient(async client => {
        ...
        if (task.type === "send") {
          ...
          await client.sendMessage(target, {message: task.message ?? "", ...(entities ? {formattingEntities: entities} : {}), ...(replyTo ? {replyTo} : {})});
          result = "已发送 1 条消息";
        } else if (task.type === "cmd") {
          const sent = await client.sendMessage(target, {message: task.message ?? "", ...(replyTo ? {replyTo} : {})});
          const outcome = await context.commands.dispatch(sent);
          result = outcome.status === "dispatched"
            ? "已执行命令"
            : `已发送命令（未执行: ${outcome.reason ?? "unknown"}）`;
          ...
          await client.sendMessage(target, {message: source, formattingEntities: source.entities, ...(replyTo ? {replyTo} : {})});
          result = "已复制发送 1 条消息";
        } else if (task.type === "forward") {
          await client.invoke(new Api.messages.ForwardMessages({fromPeer: peer(task.fromChatId)!, id: [numeric(task.fromMsgId)!], toPeer: target, ...(replyTo ? {topMsgId: replyTo} : {})}));
          result = "已转发 1 条消息";
          ...
          if (limit === undefined || !Number.isInteger(limit) || limit < 1 || limit > DEL_RE_MAX_LIMIT) throw new Error("DEL_RE_LIMIT");
      ...
      await store.update(current => {const found = current.tasks.find(item => item.id === id); if (found) {found.lastRunAt = String(Date.now()); found.lastResult = result; delete found.lastError; found.delivery = "sent";} return current;});
    } catch (error) { await store.update(current => {const found = current.tasks.find(item => item.id === id); if (found) {found.lastRunAt = String(Date.now()); found.lastError = error instanceof Error && /^(DEL_RE_|INVALID_)/.test(error.message) ? error.message : "任务执行失败"; found.delivery = "pending";} return current;}); }
  });
  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "acron", description: "定时发送、复制、转发、删除及置顶消息",
    commands: {acron: {helpOnEmpty: true, description: "管理 Cron 定时任务", async handle(invocation, context) {
      ...
      if (["list", "ls", "la"].includes(sub)) {
        ...
        if (sub === "la") { typeArg = scope; scope = "all"; }
        const all = scope === "all";
        ...
        try {
          await context.telegram.withClient(async client => {
            for (const task of tasks) {
              ...
              try { displayMap.set(key, formatEntityDisplay(await client.getEntity(peer(task.chatId) ?? task.chat), task.chat)); } catch {}
            }
          });
        } catch {}
        const text = renderTaskList({tasks, all, typeFilter, prefix: invocation.prefix,
      ...
      if (["rm", "remove", "del_task", "disable", "off", "enable", "on"].includes(sub)) {
        ...
        if (!id) return context.telegram.edit(invocation.message, `请提供定时任务ID: <code>${invocation.prefix}acron ${sub} ID</code>`, {parseMode: "html"});
        const state = await store.read(); const task = state.tasks.find(item => item.id === id);
        if (!task) return context.telegram.edit(invocation.message, `未找到任务: <code>${escape(id)}</code>`, {parseMode: "html"});
        if (["rm", "remove", "del_task"].includes(sub)) {await disposers.get(id)?.(); disposers.delete(id); await store.update(current => ({...current, tasks: current.tasks.filter(item => item.id !== id)})); return context.telegram.edit(invocation.message, `✅ 已删除任务 <code>${escape(id)}</code>`, {parseMode: "html"});}
        const disabled = sub === "disable" || sub === "off";
        if (disabled) {
          await disposers.get(id)?.(); disposers.delete(id);
          await store.update(current => {const found = current.tasks.find(item => item.id === id); if (found) found.disabled = true; return current;});
          return context.telegram.edit(invocation.message, `⏸️ 已禁用任务 <code>${escape(id)}</code>`, {parseMode: "html"});
        }
        ...
        if (!validateCronExpr(task.cron)) return context.telegram.edit(invocation.message, `任务 <code>${escape(id)}</code> 的 Cron 表达式无效，无法启用`, {parseMode: "html"});
        try { await register(context, {...task, disabled: false}); }
        catch { return context.telegram.edit(invocation.message, `任务 <code>${escape(id)}</code> 启用失败`, {parseMode: "html"}); }
        await store.update(current => {const found = current.tasks.find(item => item.id === id); if (found) found.disabled = false; return current;});
        const next = nextRunTime(task.cron);
        const tip = [`▶️ 已启用任务 <code>${escape(id)}</code>`, next ? `下次执行: ${escape(formatDate(next))}` : "",
          `复制: ${buildCopyCommand({...task, disabled: false}, invocation.prefix)}`].filter(Boolean).join("\n");
        return context.telegram.edit(invocation.message, tip, {parseMode: "html"});
      }

      if (!TASK_TYPES.includes(sub as TaskType)) return context.telegram.edit(invocation.message, help(invocation.prefix), {parseMode: "html"});
      const type = sub as TaskType;
      const cronExpr = invocation.args.slice(1, 7).join(" ");
      if (!hasSixCronFields(cronExpr)) return context.telegram.edit(invocation.message, "Cron 表达式必须为 6 段", {});
      ...
      let task: Task = {id: "", type, cron: cronExpr, chat, ...(resolved !== undefined ? {chatId: resolved, resolvedPeer: true} : {}),
        createdAt: String(Date.now()), delivery: "pending", display, ...(replyTo ? {replyTo} : {})};

      if (type === "send") {
        const reply = await context.telegram.getReply(invocation.message);
        const raw = reply?.raw as Api.Message | undefined;
        if (raw?.media || raw?.replyMarkup) return context.telegram.edit(invocation.message, "不支持带多媒体或 replyMarkup 的消息 可考虑使用本插件的定时复制/转发功能", {});
        ...
        if (!reply.id || !reply.chatId) return context.telegram.edit(invocation.message, "无法识别源消息ID或会话ID", {});
        const remark = getRemarkFromMsg(firstLine, 8);
        task = {...task, fromChatId: String(reply.chatId), fromMsgId: String(reply.id), ...(remark ? {remark} : {})};
      } else if (type === "del") {
        const msgId = rest[0];
        ...
        if (!rest[0] || !Number.isInteger(limit) || limit < 1 || limit > DEL_RE_MAX_LIMIT) return context.telegram.edit(invocation.message, `请提供 1-${DEL_RE_MAX_LIMIT} 之间的整数条数限制`, {});
        ...
        try { parseRegex(regex); } catch (error) { return context.telegram.edit(invocation.message, `无效的正则表达式: ${error instanceof Error ? error.message : String(error)}`, {}); }
        ...
        if (!notifyRaw || !pmOneSideRaw) return context.telegram.edit(invocation.message, "请提供是否发通知与是否仅对自己置顶参数，如: 1 0 或 true false", {});
        const remark = getRemarkFromMsg(firstLine, 11);
        task = {...task, msgId, notify: parseBoolFlag(notifyRaw), pmOneSide: parseBoolFlag(pmOneSideRaw), ...(remark ? {remark} : {})};
      } else {
        const msgId = rest[0];
        if (!msgId) return context.telegram.edit(invocation.message, "请提供消息 ID", {});
        if (numeric(msgId) === undefined) return context.telegram.edit(invocation.message, "无效的消息ID", {});
      ...
      try { await register(context, stored); }
      catch { await store.update(current => ({...current, tasks: current.tasks.filter(item => item.id !== stored.id)})); return context.telegram.edit(invocation.message, "无效的 Cron 表达式", {}); }

      const next = nextRunTime(cronExpr);
      const tip = [CREATION_TITLES[type], `ID: <code>${escape(stored.id)}</code>`, `对话: ${display}`,
        ...(stored.replyTo ? [`回复: ${escape(stored.replyTo)}`] : []),
        ...(stored.msgId ? [`消息ID: <code>${escape(stored.msgId)}</code>`] : []),
        ...(type === "del_re" ? [`最近条数: <code>${escape(stored.limit ?? "")}</code>`, `匹配: <code>${escape(stored.regex ?? "")}</code>`] : []),
        ...(type === "pin" ? [`通知: <code>${stored.notify ? "1" : "0"}</code>`, `仅自己置顶: <code>${stored.pmOneSide ? "1" : "0"}</code>`] : []),
    ...
    async setup(context) {
      await ensureCron();
      const state = await context.storage.json<State>("acron_config.json", createDefaults()).update(current => ({...current, schemaVersion: 1, seq: String(current.seq || "0"), tasks: (current.tasks || []).map(task => ({...task, id: String(task.id), ...(task.chatId === undefined ? {} : {chatId: String(task.chatId)}), delivery: task.delivery || "pending"}))}));
      for (const task of state.tasks) {
        if (task.disabled) continue;
        if (!validateCronExpr(task.cron)) { context.log.error("acron_task_invalid_cron", {taskId: task.id}); continue; }
        try { await register(context, task); }
        catch { context.log.error("acron_task_register_failed", {taskId: task.id}); }
      }
    },
