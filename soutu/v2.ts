    ...
    commands: {
      soutu: {
        ...
        async handle(invocation, context) {
          if (
            ["help", "h"].includes(invocation.args[0]?.toLowerCase() ?? "") ||
          ...
          try {
            ...
            const document =
              (raw as unknown as { document?: { mimeType?: string }; media?: { document?: { mimeType?: string } } })
                ?.document ?? (raw as unknown as { media?: { document?: { mimeType?: string } } })?.media?.document;
            if (
              !raw ||
              (!raw.photo && !document?.mimeType?.toLowerCase().startsWith("image/")) ||
              typeof raw.downloadMedia !== "function"
            ) {
            ...
            try {
              uploaded = await context.files.withTemp(async (directory, signal) => {
                ...
                await context.telegram.withClient(async (_client, clientSignal) => {
                  ...
                  const result = await raw.downloadMedia({
                    outputFile: target,
                    signal: clientSignal,
                ...
                const result = await context.http.text(
                  "https://0x0.st",
                  {
                    method: "POST",
                    body: form,
            ...
            const yandex = escape(
              `https://yandex.ru/images/search?url=${encodeURIComponent(image.href)}&rpt=imageview`,
            );
