...
async function localBinary(ctx: PluginContext, signal: AbortSignal = ctx.signal): Promise<string> {
  for (const candidate of speedtestCandidates) {
    try { await ctx.processes.run(candidate, ["--version"], {env:{},timeoutMs: 5_000, maxOutputBytes: 32 * 1024, signal}); signal.throwIfAborted();return candidate; }
...
async function pinnedKnownHost(ctx: PluginContext, server: Server, signal: AbortSignal): Promise<string> {
  const scan = await ctx.processes.run("/usr/bin/ssh-keyscan", ["-T", "10", "-p", String(server.port), server.host],
    {env:{},timeoutMs: 15_000, maxOutputBytes: 256 * 1024, signal});
...
export async function runLocal(ctx: PluginContext, timeoutSeconds: number, signal: AbortSignal = ctx.signal): Promise<SpeedResult> {
  ...
  const result=await ctx.processes.run(executable, fixedArgs, {env:{},timeoutMs, maxOutputBytes: 2 * 1024 * 1024, signal});signal.throwIfAborted();return parse(result.stdout);
}
...
export async function runRemote(ctx: PluginContext, server: Server, timeoutSeconds: number,
  ...
  let completed:SpeedResult|undefined;try{return await ctx.files.withTemp(async (directory, tempSignal) => {
    ...
    signal.throwIfAborted();
    signal.throwIfAborted();completed=parse(result.stdout);return completed;
  });}catch(error){callerSignal.throwIfAborted();if(error instanceof DOMException&&error.name==="AbortError")throw error;if(completed){ctx.log.error("speedlink_remote_cleanup_failed");return completed;}throw error;}
}
...
export async function dependencies(ctx: PluginContext): Promise<{local: boolean; ssh: boolean; keyscan: boolean; keygen: boolean}> {
  ...
  const probe = async (file: string, args: string[]) => {
    try { await ctx.processes.run(file, args, {env:{},signal,timeoutMs: 5_000, maxOutputBytes: 32 * 1024});signal.throwIfAborted();return true; }
    catch (error) { signal.throwIfAborted();return (error as {code?: string})?.code === "EXIT_FAILED"; }
  };
  const local = await localBinary(ctx,signal).then(() => true, error => {signal.throwIfAborted();return false;});
