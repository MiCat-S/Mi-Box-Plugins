...
export function parseTimeout(value: unknown): number | undefined {
  ...
  return Number.isInteger(number) && number >= MIN_TIMEOUT_SECONDS && number <= MAX_TIMEOUT_SECONDS
    ? number
    : undefined;
}
...
export function parseConnection(value: string): { username: string; host: string; port: number } {
  ...
  if (matched[1]!.startsWith("-")) throw new Error("连接格式应为 user@host:port 或 user@[IPv6]:port");
  const host = matched[2] ?? matched[3]!;
  const port = Number(matched[4] ?? 22);
...
export function normalizeServer(value: any): Server | undefined {
  ...
  if (
    ...
    port < 1 ||
    port > 65535 ||
    !/^SHA256:[A-Za-z0-9+/]{20,100}={0,2}$/u.test(fingerprint)
  )
    return;
...
export function normalizeState(value: any): State {
  ...
  return {
    ...source,
    schemaVersion: 1,
    timeoutSeconds: normalizeTimeout(source.timeoutSeconds ?? source.timeout),
    servers,
    legacyDatabaseDetected: source.legacyDatabaseDetected === true,
    legacyNoticeShown: source.legacyNoticeShown === true,
  };
}
...
export function selectServer(servers: readonly Server[], value: string): Server | undefined {
  ...
  return servers.find(server => server.name.toLowerCase() === name);
}
