...
const escape = (value: unknown): string => String(value ?? "").replace(/[&<>"']/g, character =>
  ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;"})[character]!);
const PUBLIC_ERRORS=new Set(["请提供别名、连接地址和 SHA256 主机指纹","服务器配置无效","服务器别名已存在","请提供序号或别名","未找到服务器","请提供有效的序号/旧别名和新别名","新别名已存在","连接格式应为 user@host:port 或 user@[IPv6]:port","主机或端口无效","恢复只允许在收藏夹执行","请回复 SpeedLink JSON 备份","备份文件过大","备份文件无效","备份不是有效 JSON","备份包含无效或重复服务器","测速超时必须在 10 到 180 秒之间","远端主机指纹与保存值不一致","当前 V2 远程测速只支持提供 OpenSSH 的类 Unix 主机","未找到官方 Ookla speedtest CLI，请由管理员安装"]);
const publicError=(error:unknown):string=>{let message="";try{const value=Object.getOwnPropertyDescriptor(error as object,"message")?.value;if(typeof value==="string")message=value;}catch{}return PUBLIC_ERRORS.has(message)?message:"请检查参数、配置与本机工具";};
...
async function receipt(ctx:PluginContext,message:CommandInvocation["message"],text:string):Promise<void>{try{await ctx.telegram.edit(message,text);}catch{ctx.signal.throwIfAborted();ctx.log.error("speedlink_receipt_failed");}}
...
export async function downloadImage(ctx: PluginContext, resultUrl: string, target: string): Promise<void> {
  ...
  await ctx.http.withResponse(source, {method: "GET", credentials: "omit", headers: {Accept: "image/png"}}, async (response, signal) => {
    ...
    const reader=response.body.getReader();let handle:Awaited<ReturnType<typeof open>>|undefined,total=0,done=false,cancelPromise:Promise<void>|undefined;const cancel=()=>cancelPromise??=reader.cancel().catch(()=>undefined),onAbort=()=>{void cancel();};signal.addEventListener("abort",onAbort,{once:true});
    try {
      handle = await open(target, "wx", 0o600);
      for (;;) { signal.throwIfAborted(); const part = await reader.read();signal.throwIfAborted();if (part.done){done=true;break;}
        signal.throwIfAborted();total += part.value.byteLength; if (total > 8 * 1024 * 1024) throw new Error("测速结果图片过大"); await writeAll(handle, part.value,signal); }
    } finally {
      signal.removeEventListener("abort",onAbort);try{if(!done)await cancel();}finally{try{reader.releaseLock();}finally{await handle?.close();}}
    }
  ...
  if (result.resultUrl) {
    ...
    try {
      await ctx.files.withTemp(async (directory, signal) => {
        ...
        await ctx.telegram.withClient(async(client,clientSignal)=>{const combined=AbortSignal.any([signal,clientSignal]);combined.throwIfAborted();await client.sendFile(raw.peerId, {file: image, caption: text, parseMode: "html",
          replyTo: invocation.message.replyToId ?? invocation.message.id,topMsgId:invocation.message.topicId});delivered=true;combined.throwIfAborted();});
      });
      try{await ctx.telegram.edit(invocation.message, `✅ ${label} 测速完成`);}catch{ctx.signal.throwIfAborted();ctx.log.error("speedlink_receipt_failed");}return;
    } catch(error) { if (ctx.signal.aborted||aborted(error)) return;if(delivered){ctx.log.error("speedlink_image_cleanup_failed");try{await ctx.telegram.edit(invocation.message, `✅ ${label} 测速完成`);}catch{ctx.signal.throwIfAborted();ctx.log.error("speedlink_receipt_failed");}return;}ctx.log.error("speedlink_image_failed"); }
  }
...
async function readBackup(ctx: PluginContext, invocation: CommandInvocation): Promise<State> {
  ...
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    await ctx.telegram.withClient(async(client,clientSignal)=>{const combined=AbortSignal.any([signal,clientSignal]),file=await open(target,"wx",0o600);let total=0;try{for await(const chunk of client.iterDownload(raw.media??raw,{signal:combined})){combined.throwIfAborted();total+=chunk.length;if(total>256*1024)throw new Error("备份文件过大");await writeAll(file,chunk,combined);}if(!total)throw new Error("备份文件无效");}finally{await file.close();}combined.throwIfAborted();});
    signal.throwIfAborted();const info = await stat(target);signal.throwIfAborted();if (!info.isFile() || !info.size || info.size > 256 * 1024) throw new Error("备份文件无效");
    let decoded: any; try { decoded = JSON.parse(await readFile(target, "utf8"));signal.throwIfAborted(); } catch(error) {signal.throwIfAborted();throw new Error("备份不是有效 JSON"); }
...
export default function createSpeedlink() {
  ...
  const runBatch=async(invocation:CommandInvocation,ctx:PluginContext,current:State,targets:readonly Server[])=>{const lines:string[]=[],deadline=new AbortController(),timer=setTimeout(()=>deadline.abort(new DOMException("SpeedLink batch exceeded 30 minutes","TimeoutError")),ALL_TIMEOUT_MS),signal=AbortSignal.any([ctx.signal,deadline.signal]);let timedOut=false;try{for(const server of targets){if(signal.aborted){timedOut=deadline.signal.aborted;break;}await ctx.telegram.edit(invocation.message,`正在测试 ${server.name}…`);signal.throwIfAborted();try{lines.push(report(await runRemote(ctx,server,current.timeoutSeconds,signal),server.name));}catch{if(ctx.signal.aborted)return;if(deadline.signal.aborted){timedOut=true;break;}lines.push(`<b>${escape(server.name)}</b>：失败`);}}}finally{clearTimeout(timer);}if(timedOut)lines.push("<b>批量测速已达到 30 分钟总时限，剩余服务器未执行。</b>");await deliverPages(ctx,invocation,lines.join("\n\n")||"没有需要测试的服务器");};
  ...
  const all: SubcommandDefinition = {description: "顺序测试全部远端服务器，可排除序号或别名", args: "[no 序号|别名...]", examples: [{args: "all"}, {args: "all no 2 backup"}],
    handle: guarded(async (invocation, ctx) => {
      ...
      const targets = current.servers.filter(server => !excluded.has(server.name));await runBatch(invocation,ctx,current,targets);
    })};
  ...
  const command: CommandDefinition = {description: "使用官方 Ookla CLI 测试本机或 SSH 远端速度", args: "[序号|别名]", helpArgs: ["help", "h"],
    ...
    subcommands: {
      ...
      timeout: {description: `查看或设置 ${MIN_TIMEOUT_SECONDS}–${MAX_TIMEOUT_SECONDS} 秒的持久超时`, args: "[秒]", handle: guarded(async (invocation, ctx) => {
        ...
        await update(ctx, value => ({...value, timeoutSeconds: seconds})); await receipt(ctx,invocation.message,`超时已设置为 ${seconds} 秒`);
      })},
      ...
      backup: {description: "把无密码、无私钥路径的 JSON 备份发送到收藏夹", args: "", handle: guarded(async (invocation, ctx) => {
        ...
        let sent=false;try{await ctx.files.withTemp(async (directory, signal) => {
          ...
          await ctx.telegram.withClient(async (client,clientSignal) => {const combined=AbortSignal.any([signal,clientSignal]); const {Api} = await import("teleproto");combined.throwIfAborted();
            await client.sendFile(new Api.InputPeerSelf(), {file: target, forceDocument: true,
              attributes: [new Api.DocumentAttributeFilename({fileName: "speedlink-backup.json"})], caption: "SpeedLink V2 脱敏备份"});sent=true;combined.throwIfAborted(); });
        });}catch(error){ctx.signal.throwIfAborted();if(aborted(error))return;if(sent)ctx.log.error("speedlink_backup_cleanup_failed");else throw error;}
        try{await ctx.telegram.edit(invocation.message, "备份已发送到收藏夹");}catch{ctx.signal.throwIfAborted();ctx.log.error("speedlink_receipt_failed");}
      })},
    ...
    handle: guarded(async (invocation, ctx) => {
      ...
      const current=await state(ctx),selected=invocation.args.map(value=>selectServer(current.servers,value));if(selected.some(value=>!value))throw new Error("未找到服务器");
      const unique=[...new Map((selected as Server[]).map(server=>[server.name.toLowerCase(),server])).values()];if(unique.length===1)await run(invocation,ctx,unique[0]);else await runBatch(invocation,ctx,current,unique);
    })};
  ...
  return definePlugin({apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "speedlink", description: "使用固定 SSH 与 Ookla CLI 测试本机和远端速度",
    ...
    async setup(ctx) {
      const legacy = await ctx.tasks.run("speedlink:legacy-probe", async signal => {
        signal.throwIfAborted(); try { await access(ctx.files.dataPath("servers.db"));signal.throwIfAborted();return true; } catch {signal.throwIfAborted();return false; }
      });
    ...
    settings: ctx => ({id: "speedlink", title: "SpeedLink", description: "远端测速超时配置", category: "插件配置", icon: "🌐",
      ...
      async getValues(signal) {signal.throwIfAborted();const value=normalizeState(await store(ctx).read(signal)).timeoutSeconds;signal.throwIfAborted();return {timeoutSeconds:value}; },
      async setValues(patch,signal) {signal.throwIfAborted();const timeoutSeconds = parseTimeout(patch.timeoutSeconds); if (timeoutSeconds === undefined) throw new Error("timeout must be 10-180 integer seconds");
