...
import {
  buildSortText, buildTailText, buildTargetDisplay, cachedUserDisplay, escapeHtml, formatAvgPerDay,
  formatDaysAgo, userDisplay, type Counts, type Stat, type TargetDisplay,
} from "./v2/render";
import {
  errorText, getTextAfterTokens, isPotentialUserIdentifier, parseSeatActionArgs, parseTailArgs,
  parseTrimArgs,
} from "./v2/parse";
...
async function resolveTarget(ctx: PluginContext, message: MessageEnvelope, value?: string): Promise<Target> {
  return ctx.telegram.withClient(async (client: any, signal) => {
    ...
    return {entity, title: entity.title || "未命名对话",
      username: entity.className === "Channel" && entity.username ? `@${entity.username}` : null,
      channel: entity.className === "Channel", key: String(entity.id)};
  });
}
...
async function getAdminCollection(ctx: PluginContext, target: Target): Promise<{admins: any[]; counts: Counts}> {
  ...
  const allAdmins = (users || []).filter((user: any) => user?.className === "User" &&
  ...
  return {admins: visibleAdmins, counts: {totalCount: allAdmins.length, botCount: allAdmins.filter((user: any) => !!user.bot).length, nonBotCount: visibleAdmins.length}};
}

async function collectStats(ctx: PluginContext, message: MessageEnvelope, target: Target): Promise<{stats: Stat[]; counts: Counts}> {
  await ctx.telegram.edit(message, `🔍 正在获取 <b>${escapeHtml(target.title)}</b> 的管理员列表...`, {parseMode: "html", linkPreview: false});
    ...
    for (let index = 0; index < admins.length; index++) {
      ...
      if (avg < 0) {
        try {
          ...
          signal.throwIfAborted();
          const found: any = await client.invoke(new Api.messages.Search({peer: target.entity, q: "", filter: new Api.InputMessagesFilterEmpty(),
            minDate: Math.floor(Date.now() / 1000) - WEEK, offsetId: 0, addOffset: 0, limit: 1, maxId: 0, minId: 0, hash: 0 as any, fromId: from}));
          signal.throwIfAborted();
          avg = Number(found.count ?? found.messages?.length ?? 0) / 7;
        } catch (error) { if (signal.aborted || ctx.signal.aborted) throw error; }
      }
      signal.throwIfAborted();
      let last = 0;
      try {
      ...
      result.push({user, id, name: userName(user), username: user.username ? String(user.username) : null,
        rank: participant?.rank?.trim() || "无", avg, avgText: avg < 0 ? "N/A" : formatAvgPerDay(avg),
        last, lastText: last ? formatDaysAgo(new Date(last)) : "无记录",
        locked: lockSet.has(id), creator: ["ChatParticipantCreator", "ChannelParticipantCreator"].includes(participant?.className)});
      await cache(ctx).update(data => ({...data, values: {...data.values,
        [key]: {updatedAt: Date.now(), ...(avg >= 0 ? {avgPerDay: avg} : {}), name: userName(user), username: user.username || null}}}));
    }
...
async function resolveUserForSeatAction(ctx: PluginContext, target: Target, identifier: string): Promise<any | undefined> {
...
async function findUserInChatParticipants(client: any, target: Target, identifier: string, exact: string, signal: AbortSignal): Promise<any | undefined> {
  const username = /^-?\d+$/.test(identifier) ? "" : identifier.replace(/^@/, "").toLowerCase();
  signal.throwIfAborted();
  const participants: any[] = await client.getParticipants(target.entity, {showTotal: false, ...(username ? {search: username} : {})});
  signal.throwIfAborted();
  return participants.find((user: any) => user?.className === "User" &&
    (username ? (user.username || "").toLowerCase() === username : String(user.id) === exact));
}

async function findUserInChannelParticipants(client: any, target: Target, identifier: string, exact: string, signal: AbortSignal): Promise<any | undefined> {
  const username = /^-?\d+$/.test(identifier) ? "" : identifier.replace(/^@/, "").toLowerCase();
  if (username) {
    try {
      signal.throwIfAborted();
      ...
      signal.throwIfAborted();
      const matched = users.find((user: any) => user?.className === "User" && (user.username || "").toLowerCase() === username);
  ...
  for (let index = 0; index < 5; index++) {
    const result: any = await client.invoke(new Api.channels.GetParticipants({channel: inputChannel,
      filter: new Api.ChannelParticipantsRecent(), offset, limit, hash: 0 as any}));
    signal.throwIfAborted();
...
async function demote(ctx: PluginContext, target: Target, user: any): Promise<void> {
  await ctx.telegram.withClient(async (client: any, signal) => {
    ...
    if (target.channel) {
      const inputChannel = await client.getInputEntity(target.entity);
      signal.throwIfAborted();
      await client.invoke(new Api.channels.EditAdmin({channel: inputChannel, userId: inputUser, adminRights: new Api.ChatAdminRights({}), rank: ""}));
    } else {
      await client.invoke(new Api.messages.EditChatAdmin({chatId: target.entity.id, userId: inputUser, isAdmin: false}));
    }
    signal.throwIfAborted();
  });
}

async function getCachedUserInfo(ctx: PluginContext, target: Target, userId: string): Promise<{name?: string; username?: string | null} | undefined> {
...
async function handleSeatAction(ctx: PluginContext, message: MessageEnvelope, action: "lock" | "unlock", rawText: string, commandName: string): Promise<void> {
  const remainder = getTextAfterTokens(rawText, 2);
  const {identifiers, targetArg} = parseSeatActionArgs(remainder);
  if (identifiers.length === 0) {
    await ctx.telegram.edit(message, `❌ 参数不足\n\n用法:\n<code>${escapeHtml(commandName)} ${action} 用户1,用户2 [对话id/@username]</code>`, {parseMode: "html", linkPreview: false});
    return;
  }
  const target = await resolveTarget(ctx, message, targetArg);
  await ctx.telegram.edit(message, `🔍 正在解析要${action === "lock" ? "锁定" : "取消锁定"}席位的用户...\n${buildTargetDisplay(target)}`, {parseMode: "html", linkPreview: false});
  ...
  const lines = [`${action === "lock" ? "🔒" : "🔓"} <b>席位${action === "lock" ? "锁定" : "取消锁定"}完成</b>`,
    buildTargetDisplay(target), `成功: <code>${storedUserIds.length}</code>`, `失败: <code>${failures.length}</code>`];
  if (storedUserIds.length > 0) {
    lines.push("", "<b>成功用户</b>");
...
async function handleClear(ctx: PluginContext, message: MessageEnvelope, rawText: string): Promise<void> {
  ...
  await ctx.telegram.edit(message, [`🧹 <b>缓存已清理</b>`, buildTargetDisplay(target), `清理条目: <code>${count}</code>`,
    `说明: <code>已清理周日均和用户信息缓存，席位锁定数据不受影响</code>`].join("\n"), {parseMode: "html", linkPreview: false});
}

async function handleTrimAction(ctx: PluginContext, message: MessageEnvelope, rawText: string, commandName: string): Promise<void> {
  const remainder = getTextAfterTokens(rawText, 2);
  const {limit, targetArg} = parseTrimArgs(remainder);
  if (!limit) {
    await ctx.telegram.edit(message, `❌ 参数不足\n\n<code>rm</code> 的人数参数是必填正整数。\n\n用法:\n<code>${escapeHtml(commandName)} rm 正整数 [对话id/@username]</code>`, {parseMode: "html", linkPreview: false});
    return;
  }
  const target = await resolveTarget(ctx, message, targetArg);
  ...
  if (selected.length === 0) {
    await ctx.telegram.edit(message, [`✂️ <b>无需处理</b>`, buildTargetDisplay(target), `说明: <code>没有可下掉的未锁定席位管理员</code>`].join("\n"), {parseMode: "html", linkPreview: false});
    return;
  }
  ...
  for (let index = 0; index < selected.length; index++) {
    ...
    await ctx.telegram.edit(message, `✂️ 正在下掉倒数管理员...\n${buildTargetDisplay(target)}\n进度: <code>${index + 1}/${selected.length}</code>`, {parseMode: "html", linkPreview: false});
    try {
  ...
  const lines = [`✂️ <b>尾部管理员清理完成</b>`, buildTargetDisplay(target), `目标人数: <code>${limit}</code>`,
    `实际候选: <code>${selected.length}</code>`, `成功: <code>${success.length}</code>`, `失败: <code>${failures.length}</code>`];
  if (success.length > 0) {
    lines.push("", "<b>已下掉</b>");
...
async function handleTail(ctx: PluginContext, message: MessageEnvelope, rawText: string): Promise<void> {
  ...
  if (firstToken && /^\d+$/.test(firstToken) && !/^[1-9]\d*$/.test(firstToken)) {
    await ctx.telegram.edit(message, `❌ 参数错误\n\n<code>tail</code> 的人数参数必须是正整数`, {parseMode: "html", linkPreview: false});
    return;
  }
...
export default function createAdminBoard(): ReturnType<typeof definePlugin> {
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "admin_board",
    description: "管理员活跃度排行、席位锁定和尾部管理员清理",
    commands: {admin_board: {helpArgs: ["help", "h"], helpOnEmpty: true, description: "管理员席位管理",
      async handle(invocation, ctx) {
        ...
        try {
          if (!action || action === "help" || action === "h") {
            await ctx.telegram.edit(message, renderPluginHelp(invocation.prefix), {parseMode: "html", linkPreview: false});
            return;
          }
          if (action === "ls") { await handleList(ctx, message, rawText); return; }
          if (action === "tail") { await handleTail(ctx, message, rawText); return; }
          if (action === "rm") { await handleTrimAction(ctx, message, rawText, commandName); return; }
          if (action === "lock" || action === "unlock") { await handleSeatAction(ctx, message, action, rawText, commandName); return; }
          if (action === "clear") { await handleClear(ctx, message, rawText); return; }
          await ctx.telegram.edit(message, `❌ 不支持的动作: <code>${escapeHtml(action)}</code>\n\n${renderPluginHelp(invocation.prefix)}`, {parseMode: "html", linkPreview: false});
        } catch (error) {
          if (!ctx.signal.aborted) await ctx.telegram.edit(message, `❌ <b>执行失败</b>\n\n${escapeHtml(errorText(error))}`, {parseMode: "html", linkPreview: false});
        }
      }}},
  });
}
