      ...
      try {
        ...
        await context.telegram.withClient(async (client, signal) => {
          ...
          while (deleted < maximum) {
            ...
            try {
              history = await client.invoke(new Api.messages.GetHistory({
                peer, offsetId, offsetDate: 0, addOffset: 0, limit: 100,
            ...
            if (selected.length) {
              ...
              if (deletionSucceeded) {
                try { await context.telegram.edit(invocation.message, `正在清理贴纸消息… ${deleted}/${maximum}`); }
                catch { signal.throwIfAborted(); context.log.error("clear_sticker_progress_edit_failed", {deleted}); }
              }
            }
          ...
          signal.throwIfAborted();
          try { await context.telegram.edit(invocation.message, deleted ? `清理完成，共删除 ${deleted} 条贴纸消息` : "未找到贴纸消息"); }
          catch { signal.throwIfAborted(); context.log.error("clear_sticker_result_edit_failed", {deleted}); }
          signal.throwIfAborted();
          if (deleted) {
