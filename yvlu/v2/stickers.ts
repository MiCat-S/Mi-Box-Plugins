...
export async function saveSticker(ctx: PluginContext, message: MessageEnvelope, shortName: string): Promise<boolean> {
  ...
  try {
    const result = await native(ctx, client =>
      client.invoke(new Api.messages.GetStickerSet({ stickerset: set, hash: 0 })),
    );
  ...
  if (sticker && doc.id && doc.accessHash) {
    ...
    const png = await sharp(buffer, { limitInputPixels: 16 * 1024 * 1024 })
      .rotate()
    ...
    const uploaded = await native(ctx, client =>
      client.uploadFile({ file: new CustomFile("sticker.png", png.length, "", png), workers: 1 }),
    );
    // UploadMedia turns InputFile into the InputDocument required by sticker RPCs.
    const result: any = await native(ctx, client =>
      client.invoke(
