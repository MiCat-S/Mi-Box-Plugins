...
export async function downloadMediaBuffer(ctx: PluginContext, target: any): Promise<Buffer> {
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    const result = await native(ctx, client =>
      client.downloadMedia(target, { outputFile: output, signal, progressCallback }),
    );
    signal.throwIfAborted();
...
async function executable(ctx: PluginContext, name: string): Promise<string> {
  ...
  for (const dir of (process.env.PATH || "").split(delimiter).filter(isAbsolute)) {
    ...
    try {
      ...
      if (!(error instanceof Error && "code" in error && ["ENOENT", "EACCES", "ENOTDIR"].includes(String(error.code))))
        throw error;
    }
  }
...
export async function convertVideo(ctx: PluginContext, buffer: Buffer, tgs: boolean): Promise<Buffer> {
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    const gif = join(directory, "input.gif"),
      output = join(directory, "output.webm");
    ...
    if (tgs) {
      ...
      try {
        await ctx.processes.run(
          ...
          [
            "-c",
            "import sys\nfrom rlottie_python import LottieAnimation\nLottieAnimation.from_tgs(sys.argv[1]).save_animation(sys.argv[2])",
            input,
            gif,
          ],
...
export async function avatar(ctx: PluginContext, entity: any): Promise<{ url: string } | undefined> {
  ...
  for (const big of [false, true]) {
    try {
      const result = await native(ctx, client =>
        client.downloadProfilePhoto(entity, { isBig: big, signal: ctx.signal }),
      );
      if (Buffer.isBuffer(result) && result.length) {
        buffer = bounded(result);
        break;
      }
    } catch {
      ctx.signal.throwIfAborted();
      ctx.log.info("yvlu.avatar.download.failed", { big });
    }
  }
  if (!buffer) return undefined;
  const sharp = (await import("sharp")).default;
  ctx.signal.throwIfAborted();
  const png = await sharp(buffer, { limitInputPixels: MAX_IMAGE_PIXELS })
...
export async function generateQuote(
  ctx: PluginContext,
  data: unknown,
): Promise<{ buffer: Buffer; ext: "webp" | "png" | "webm" }> {
  const result = await ctx.http.withResponse(
    "https://quote-api-enhanced.zhetengsha.eu.org/generate.webp",
    {
      method: "POST",
      redirect: "error",
      headers: { "Content-Type": "application/json", "User-Agent": "TeleBox/0.2.1" },
...
export async function sendQuote(
  ...
  peer: any,
  replyTo: number,
  result: Awaited<ReturnType<typeof generateQuote>>,
): Promise<void> {
