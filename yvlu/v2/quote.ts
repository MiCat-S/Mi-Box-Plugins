...
export function convertEntities(entities: readonly any[] = []): any[] {
  ...
  return entities.map(entity => {
    const name = (entity.className || entity._ || entity.constructor?.name || "").replace(
...
async function sender(ctx: PluginContext, message: any): Promise<any> {
  ...
  const resolved =
    typeof message.getSender === "function"
      ? await optional(ctx, "sender", () => native(ctx, () => message.getSender()))
      : undefined;
...
async function replyBlock(ctx: PluginContext, message: any, parent: MessageEnvelope): Promise<any> {
  ...
  return {
    name: author
      ? `${author.firstName || author.title || ""} ${author.lastName || ""}`.trim() || author.username || "unknown"
      : "unknown",
    text,
    entities: convertEntities((quote ? header.quoteEntities : replied?.entities) || []),
...
function advancedMedia(message: any): Record<string, unknown> {
  ...
  if (audio)
    return {
      audio: {
        title: audio.title || audio.fileName || "Audio",
        ...(audio.performer ? { performer: audio.performer } : {}),
        ...(audio.duration ? { duration: Number(audio.duration) } : {}),
      },
    };
  if (find("Animated") || video || (media.className || "").includes("Dice")) {
    return {
      mediaType: find("Animated") || (media.className || "").includes("Dice") ? "gif" : "video",
...
export async function quoteData(
  ...
  if (options.count > 1) {
    messages = await native(ctx, client =>
      client.getMessages(replied.peerId || invocation.chatId, {
  ...
  for (const [i, message] of messages.entries()) {
    ...
    const emoji =
      status?.documentId ??
      status?.document_id ??
      status?.customEmojiId ??
      status?.custom_emoji_id ??
      status?.id ??
    ...
    const item: any = {
      from: {
        ...
        first_name: show ? first || undefined : undefined,
        last_name: show ? last || undefined : undefined,
        username: show && photo ? author.username || undefined : undefined,
        photo,
        emoji_status: show && emoji ? String(emoji) : undefined,
      },
      text: fake?.text ?? (partial || message.message || ""),
      entities: convertEntities(fake?.entities ?? (partial ? header.quoteEntities : message.entities) ?? []),
      avatar: show,
    };
    ...
    if (author.accessHash) {
      ...
      const result: any = await optional(ctx, "rank", () =>
        native(ctx, client =>
          client.invoke(
  ...
  return {
    type: options.format === "webp" ? "quote" : options.format,
    format: options.format === "webp" ? "webp" : "png",
    backgroundColor: "#1b1429",
    width: options.format === "stories" ? 360 : 512,
    height: options.format === "stories" ? 640 : 768,
    scale: 2,
    emojiBrand: "apple",
    messages: items,
  };
