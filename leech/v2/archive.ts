...
export async function ensureDatabase(context: PluginContext, recover = false): Promise<void> {
  await db(context).transaction(connection => {
    ...
    if (recover)
      connection
        .prepare("UPDATE leech_jobs SET status='failed',error_kind='INTERRUPTED',finished_at=? WHERE status='running'")
...
async function deliver(
  ...
  if (result.published > 0) {
    try {
      await context.telegram.reply(message, ui.interruptedNotice(result));
    } catch {
      if (!context.signal.aborted) context.log.error(`${event}_notice`);
    }
  }
  return false;
}
...
function identity(entity: any) {
  ...
  const id =
    entity?.className === "Channel"
      ? source.startsWith("-100")
        ? source
        : `-100${source}`
      : entity?.className?.startsWith("Chat")
        ? source.startsWith("-")
          ? source
          : `-${source}`
        : source;
  const title = String(
    entity?.title ||
      ...
      (entity?.username ? `@${entity.username}` : entity?.className || "unknown"),
  ).slice(0, 4_000);
  const type =
    entity?.className === "Channel"
      ? entity.broadcast
...
function snapshot(message: any): string {
  return JSON.stringify({
    ...
    senderId: exact(message?.senderId),
    chatId: exact(message?.chatId),
    peerId: exact(message?.peerId),
    mediaClassName: message?.media?.className ?? null,
    groupedId: exact(message?.groupedId),
...
async function markFailed(
  ...
  await db(context)
    .transaction(connection => {
      ...
      connection
        .prepare(
          "UPDATE leech_jobs SET status='failed',saved_count=?,scanned_count=?,finished_at=?,error_kind=? WHERE id=?",
        )
...
export async function archive(
  ...
  try {
    await context.telegram.withClient(async (client: any, signal) => {
      ...
      const value =
        input.target === "here" ? (raw?.peerId ?? invocation.message.chatId) : normalizedTarget(input.target);
      ...
      job = await db(context).transaction(connection => {
        ...
        return Number(
          connection
            .prepare(
              "INSERT INTO leech_jobs(target,chat_id,chat_title,chat_type,from_ts,to_ts,status,requested_limit,batch_size,started_at) VALUES(?,?,?,?,?,?,'running',?,?,?)",
            )
            .run(
              input.target,
              chat!.id,
      ...
      for (;;) {
        ...
        for (const message of messages) {
          ...
          rows.push({
            ...
            name: sender?.title || [sender?.firstName, sender?.lastName].filter(Boolean).join(" ") || null,
            text: typeof message.message === "string" ? message.message : null,
            media: message.media?.className ?? null,
            reply: Number(message.replyTo?.replyToMsgId ?? message.replyToMsgId) || null,
            grouped: exact(message.groupedId),
            views: Number(message.views) || null,
            forwards: Number(message.forwards) || null,
            outgoing: message.out ? 1 : 0,
            json: snapshot(message),
            savedAt: new Date().toISOString(),
          });
          if (input.limit && saved + rows.length >= input.limit) {
            stopped = "limit_reached";
            break;
          }
        }
        ...
        try {
          ...
          if (!signal.aborted) context.log.error("leech_progress_delivery_failed");
        }
      }
    });
    ...
    await db(context).transaction(connection => {
      ...
      connection
        .prepare(
          "UPDATE leech_jobs SET status='completed',saved_count=?,scanned_count=?,finished_at=?,error_kind=NULL WHERE id=?",
        )
...
export async function session(invocation: CommandInvocation, context: PluginContext): Promise<void> {
  try {
    ...
    if (!context.signal.aborted) {
      context.log.error("leech_session_failed");
      await context.telegram.edit(invocation.message, "❌ Leech 会话检查失败");
    }
  }
}
...
export async function jobs(invocation: CommandInvocation, context: PluginContext): Promise<void> {
  ...
  const rows = await db(context).transaction(connection => {
    schema(connection);
    return connection
      .prepare("SELECT id,status,chat_title,target,saved_count,from_ts,to_ts FROM leech_jobs ORDER BY id DESC LIMIT ?")
      .all(limit) as any[];
  }, context.signal);
  const result = rows.length
    ? `<b>Recent Leech Jobs</b>\n${rows.map(row => `#${row.id} | ${ui.text(row.status)} | ${ui.text(row.chat_title || row.target)} | saved=${row.saved_count} | range=${row.from_ts}-${row.to_ts}`).join("\n")}`
    : "📭 No leech jobs yet.";
...
export async function stats(invocation: CommandInvocation, context: PluginContext): Promise<void> {
  const value = await db(context).transaction(connection => {
    schema(connection);
    return {
      messages: (connection.prepare("SELECT COUNT(*) n FROM leech_messages").get() as any).n,
      jobs: (connection.prepare("SELECT COUNT(*) n FROM leech_jobs").get() as any).n,
    };
  }, context.signal);
  await deliver(
    context,
