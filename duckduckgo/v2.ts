...
function bounded(value:string,maximum:number){let output="";for(const character of value){const encoded=esc(character);if(output.length+encoded.length>maximum)break;output+=encoded;}return output;}
...
function target(raw: string) { try { const u=new URL(raw, "https://duckduckgo.com"); const encoded=u.searchParams.get("uddg"); const parsed=new URL(encoded || u.toString()); const value=parsed.toString();return ["http:","https:"].includes(parsed.protocol)&&esc(value).length<=1500 ? value : ""; } catch { return ""; } }
...
async function fallback(ctx: PluginContext, query: string, limit: number): Promise<Result[]> { const data=await ctx.http.json<any>(`https://${FIRECRAWL}/v2/search`, {method:"POST",headers:{"content-type":"application/json","user-agent":"MiBot/2"},body:JSON.stringify({query,limit})}, {timeoutMs:15000,redirects:{allowedHosts:[FIRECRAWL],maxRedirects:1}}); const items=Array.isArray(data?.data?.web)?data.data.web:Array.isArray(data?.data)?data.data:[]; return items.slice(0,limit).flatMap((x:any)=>{try{const u=new URL(String(x?.url??"")); const url=u.toString();if(!["http:","https:"].includes(u.protocol)||esc(url).length>1500)return[]; return [{title:String(x?.title||u.hostname).slice(0,200),url,snippet:String(x?.description||"").slice(0,600),source:"Firecrawl"}];}catch{return[];}}); }
function pages(query:string, items:Result[]) { const header=`🔍 <b>DuckDuckGo</b> · <code>${bounded(query,1200)}</code> · ${items.length} 条`;const footer=`🌐 <a href="https://duckduckgo.com/?q=${encodeURIComponent(query)}">在 DuckDuckGo 打开</a>`; if(!items.length)return[`${header}\n\n⚠️ 没有找到结果。可换关键词重试。\n\n${footer}`]; const blocks=[...items.map((r,i)=>`<b>${i+1}.</b> <a href="${esc(r.url)}">${bounded(r.title,500)}</a>\n<blockquote expandable>🏷 ${r.source}${r.snippet?`\n\n${bounded(r.snippet,1000)}`:""}</blockquote>`),footer]; const out:string[]=[]; let current=header; for(const block of blocks){if(`${current}\n${block}`.length>3600){out.push(current);current=`🔍 <b>DuckDuckGo</b> · 续\n${block}`;}else current+=`\n${block}`;} out.push(current); return out; }
export default function createDuckDuckGo(){return definePlugin({renderHelp: renderPluginHelp, apiVersion:1,id:"duckduckgo",description:"DuckDuckGo 网页搜索",commands:{duckduckgo:{description:"搜索网页",handle:handle},ddg:{description:"搜索网页",handle:handle}}});}
async function handle(invocation:any,ctx:PluginContext){const {query,limit}=parseArgs(invocation.args);if(!query||query.length>200){await ctx.telegram.edit(invocation.message,query?"❌ 关键词过长（最多 200 字符）":HELP,{parseMode:"html"});return;}await ctx.telegram.edit(invocation.message,`⏳ 正在搜索 <code>${esc(query)}</code>…`,{parseMode:"html"});try{let items:Result[]=[];try{items=await ddg(ctx,query,limit);}catch(error){ctx.signal.throwIfAborted();if(["REDIRECT_BLOCKED","TOO_MANY_REDIRECTS","ADDRESS_BLOCKED"].includes(String((error as any)?.code??"")))throw error;ctx.log.info("duckduckgo_primary_unavailable");}if(items.length<limit){
  try {
