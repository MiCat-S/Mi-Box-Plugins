...
class PortballError extends Error{constructor(readonly kind:Failure){super(kind);}}
export function category(error:unknown){const value=error instanceof PortballError?error.kind:error&&typeof error==="object"&&"errorMessage" in error?(error as {errorMessage?:unknown}).errorMessage:undefined;return value==="ADMIN_REQUIRED"||value==="CHAT_ADMIN_REQUIRED"?"需要管理员权限":value==="USER_ADMIN_INVALID"?"无法禁言管理员":value==="CHANNEL_PRIVATE"?"无法在私有频道操作":value==="USER_NOT_PARTICIPANT"?"用户不在群组中":"请确认目标、群组类型和管理员权限";}
async function errorReceipt(context:PluginContext,message:MessageEnvelope,text:string){await context.telegram.edit(message,text,{parseMode:"html"});const raw=message.raw as {delete?:(value:unknown)=>Promise<unknown>}|undefined;if(!raw?.delete)return;void context.tasks.run("portball:error-cleanup",async(signal:AbortSignal)=>{try{await delay(5000,undefined,{signal});await raw.delete!({revoke:true});}catch{if(!signal.aborted)context.log.error("portball_error_cleanup_failed");}}).catch(()=>{});}
  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "portball", description: "回复消息临时禁言群组成员",
    commands: {portball: {description: "回复消息临时禁言群组成员", async handle(invocation, context) {
      ...
      if (!seconds || invocation.message.replyToId === undefined) {
        await errorReceipt(context,invocation.message,
      ...
      try {
        ...
        await context.telegram.withClient(async (client, signal) => {
          ...
          const ownMembership = await client.invoke(new Api.channels.GetParticipant({
            channel: chat, participant: new Api.InputPeerSelf(),
          }));
          signal.throwIfAborted();
          if (!(ownMembership.participant instanceof Api.ChannelParticipantCreator) &&
              (!(ownMembership.participant instanceof Api.ChannelParticipantAdmin) || !ownMembership.participant.adminRights?.banUsers)) {
            throw new PortballError("ADMIN_REQUIRED");
          }
          const inputTarget=await client.getInputEntity(target);signal.throwIfAborted();
          const targetMembership = await client.invoke(new Api.channels.GetParticipant({
            channel: chat, participant: inputTarget,
          }));
          signal.throwIfAborted();
          if (targetMembership.participant instanceof Api.ChannelParticipantCreator ||
              targetMembership.participant instanceof Api.ChannelParticipantAdmin) throw new PortballError("USER_ADMIN_INVALID");
          signal.throwIfAborted();
          ...
          try{await client.sendMessage(raw.peerId, {message:
            `<b>禁言成功</b>\n用户：${escape(name)}\n时长：${seconds} 秒${reason ? `\n理由：${escape(reason)}` : ""}\n到期自动解除`, parseMode: "html"});}catch{context.log.error("portball_success_receipt_failed");return;}
          signal.throwIfAborted();
