...
const schemeName = (value: unknown) => {
  ...
  return (
    ({ amex: "AMERICAN EXPRESS", mastercard: "MASTERCARD", unionpay: "UNIONPAY" } as Record<string, string>)[name] ??
...
class BinlistStatusError extends Error {
  constructor(readonly status: number) {
    super(`Binlist response status ${status}`);
  }
}
...
async function bincheck(ctx: PluginContext, bin: string) {
  try {
    const html = await ctx.http.withResponse(
      `https://bincheck.io/details/${bin.slice(0, 6)}`,
    ...
    const match =
      description.match(
        /valid BIN number\s+\d+\s+(?:is\s+)?(?:a\s+)?valid BIN number\s+([A-Z ]+)\s+issued by\s+(.+?)\s+in\s+(.+)/i,
      ) ?? description.match(/valid BIN number\s+([A-Z ]+)\s+issued by\s+(.+?)\s+in\s+(.+)/i);
    return match
      ? { scheme: match[1].trim().toLowerCase().replace(/\s+/g, ""), bank: match[2].trim(), country: match[3].trim() }
      : {};
  } catch {
    return {};
  }
  ...
  async function exchange(ctx: PluginContext): Promise<Rates | undefined> {
    ...
    pendingRates = (async () => {
      try {
        ...
        cachedRates = {
          at: Date.now(),
          updated: Number.isFinite(time.getTime()) ? time.toISOString().slice(0, 10) : "",
          rates,
        };
        return cachedRates;
      } catch {
      ...
      bin: {
        ...
        async handle(invocation, ctx: PluginContext) {
          ...
          try {
            ...
            const countryName = (data?.country?.name || checked.country || "未知")
            ...
            const level =
              brand
                .toUpperCase()
                .match(/WORLD ELITE|BUSINESS|CORPORATE|PLATINUM|GOLD|CLASSIC|SIGNATURE|INFINITE|WORLD|PREMIUM/)?.[0] ??
              "未知";
            ...
            const business =
              typeof data?.commercial === "boolean"
                ? data.commercial
                : /BUSINESS|CORPORATE|COMMERCIAL/i.test(brand)
                  ? true
                  : undefined;
            const type = field(data?.type).toUpperCase();
            const numberDetails = [
              ...(Number.isSafeInteger(data?.number?.length) && data.number.length > 0
                ? [`卡号长度  ${data.number.length} 位`]
                : []),
              ...(typeof data?.number?.luhn === "boolean" ? [`Luhn ${yesNo(data.number.luhn)}`] : []),
            ];
            const quote = (rate: number) =>
              rate.toLocaleString("en-US", {
                minimumFractionDigits: 2,
                maximumFractionDigits: rate < 0.01 ? 6 : 4,
                useGrouping: false,
              });
            const lines = [
              `<b>💳 BIN · 卡片档案</b>`,
              ...
              ...(rates
                    ...
                    ...(currency && currency !== "USD"
                      ? [
                          rates.rates[currency]
                            ? `<code>1 ${currency} = ${quote(rates.rates.CNY / rates.rates[currency])} CNY</code>`
                            : `${esc(currency)} 汇率暂不可用`,
                        ]
                      : []),
                ...
                : ["汇率暂不可用"]),
              ...(numberDetails.length
                ? [``, `<blockquote expandable>${esc(numberDetails.join(" · "))}</blockquote>`]
                : []),
            ];
            const pages = await ui.renderRichText(lines.join("\n"));
            for (const [index, page] of pages.entries()) {
              ctx.signal.throwIfAborted();
              await ctx.telegram[index ? "reply" : "edit"](invocation.message, page, {
                parseMode: "html",
                linkPreview: false,
              });
            }
            ...
            if (details.status === 404)
              await ctx.telegram.edit(invocation.message, `❌ 未找到: <code>${value}</code>`, { parseMode: "html" });
            else if (details.status === 429)
              await ctx.telegram.edit(invocation.message, "⏳ 频率受限，请稍后重试", { parseMode: "html" });
            else if (
              details.code === "TIMEOUT" ||
              (typeof details.message === "string" && details.message.includes("timeout"))
            ) {
