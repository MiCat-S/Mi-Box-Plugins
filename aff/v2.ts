  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "aff", description: "管理并发送 Aff 信息",
    ...
    aff: {helpArgs: ["help","h"], description: "管理并发送 Aff 信息", async handle(invocation, ctx) {
      ...
      if (!args.length || args[0] === "help" || args[0] === "h") {
        if (!args.length) {
          ...
          if (data.affs.length === 0) {
            await ctx.telegram.edit(message, `❌ <b>暂无Aff信息</b>\n\n💡 请回复一条消息使用 <code>${esc(prefix)}aff save</code> 保存`, {parseMode: "html"});
            return;
          }
          if (data.affs.length === 1) return output(ctx, message, data.affs[0]);
          return list(ctx, message, data.affs, "1", prefix);
        }
        await ctx.telegram.edit(message, renderPluginHelp(prefix), {parseMode: "html"}); return;
      }
      ...
      if (sub === "save") {
        ...
        if (!reply || !text.trim()) { await ctx.telegram.edit(message, "❌ <b>请回复一条消息以保存新的Aff信息</b>", {parseMode: "html"}); return; }
        if (text.length > 4000) {
          await ctx.telegram.edit(message, "❌ <b>文本超过 4000 字符，未保存，请缩短后重试</b>", {parseMode: "html"}); return;
        }
        let index = -1;
        let full = false;
        await db.update(current => {
          ...
          return {...current, affs: [...current.affs, {text, webPage: /https?:\/\/[^\s]+/.test(text), format: "html", created_at: Date.now()}]};
        });
        if (full) {
          await ctx.telegram.edit(message, "❌ <b>已保存 32 条，请先删除不需要的记录再保存</b>", {parseMode: "html"}); return;
        }
        await ctx.telegram.edit(message, `✅ <b>Aff信息保存成功！</b>\n🆔 当前序号：${index}`, {parseMode: "html"});
        return;
      }
      ...
      if (["remove", "rm", "del"].includes(sub)) {
        const raw = args[1];
        if (!raw) {
          await ctx.telegram.edit(message, `❌ <b>请指定要删除的序号</b>\n💡 例如：<code>${esc(prefix)}aff remove 1</code>`, {parseMode: "html"}); return;
        }
        if (!/^[1-9]\d*$/.test(raw)) { await ctx.telegram.edit(message, "❌ <b>无效的序号</b>", {parseMode: "html"}); return; }
        ...
        if (removed) await ctx.telegram.edit(message, `✅ <b>已删除序号 ${Number(raw)} 的Aff信息</b>`, {parseMode: "html"});
        else await ctx.telegram.edit(message, `❌ <b>删除失败：找不到序号 ${Number(raw)}</b>`, {parseMode: "html"});
        return;
      }

      if (/^-?\d+$/.test(sub)) {
        const real = Number(sub) - 1;
        if (Number.isSafeInteger(real) && real >= 0 && real < data.affs.length) return output(ctx, message, data.affs[real]);
        await ctx.telegram.edit(message, `❌ <b>找不到序号为 ${esc(sub)} 的Aff信息</b>`, {parseMode: "html"}); return;
      }

      await ctx.telegram.edit(message, `❌ <b>无效的参数</b>\n\n${renderPluginHelp(prefix)}`, {parseMode: "html"});
    }},
  }});
}
