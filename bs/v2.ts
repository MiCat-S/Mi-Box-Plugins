...
function floodWait(error: unknown): number | undefined {
  ...
  const match = text.match(/(?:^|\b)FLOOD_WAIT_(\d+)(?:\b|$)/);
  if (!match) return;
...
function forwarded(result: any): any[] {
  return Array.isArray(result?.updates)
    ? result.updates.map((value: any) => value?.message).filter((value: any) => value?.className === "Message")
    : [];
}
...
async function forward(invocation: any, context: PluginContext, count: number): Promise<void> {
  ...
  if (!reply) {
    await context.telegram.edit(invocation.message, "你需要回复一条消息");
    return;
  }
  if (!Number.isSafeInteger(count) || count < 1) {
    await context.telegram.edit(
      invocation.message,
      `❌ <b>消息数必须是正整数</b>\n示例：<code>${escape(invocation.prefix)}bs 3</code>`,
      { parseMode: "html" },
    );
    return;
  }
  ...
  try {
    await context.telegram.withClient(async (client, signal) => {
      ...
      for (let id = replied.id; values.length < count && id < replied.id + limit; id++) {
        ...
        try {
          ...
          signal.throwIfAborted();
          context.log.info("bs_source_message_skipped");
        }
      }
      ...
      for (const channel of channels) {
        ...
        try {
          ...
          for (let attempt = 0; attempt < 2; attempt++) {
            ...
            try {
              result = await client.invoke(
                new Api.messages.ForwardMessages({ fromPeer: sourcePeer, id: values, toPeer: input }),
              );
              break;
            } catch (error) {
              if (restrictedError(error)) throw new ForwardRestrictedError();
              const delay = floodWait(error);
              if (delay === undefined) throw error;
              if (delay > MAX_FLOOD_WAIT_MS || attempt === 1) throw new FloodRetryError();
          ...
          signal.throwIfAborted();
          if (error instanceof ForwardRestrictedError || error instanceof FloodRetryError) throw error;
          context.log.info("bs_target_skipped", { channel });
        }
      }
      if (!messages.length || !targetEntity || !targetChannel) {
        await context.telegram.edit(invocation.message, "没有找到有发送权限的频道");
        return;
      }
      // 回执消息
      ...
      await context.telegram.edit(
        invocation.message,
        `亲爱的被观察者 您的 ${forwardedCount} 条消息已被保送到频道 ${targetHtml}`,
    ...
    await context.telegram.edit(
      invocation.message,
      error instanceof ForwardRestrictedError
        ? "该消息不允许被转发"
        : error instanceof FloodRetryError
          ? "操作频繁，请稍后重试"
          : error instanceof SourceMissingError
            ? "无法获取被回复的消息"
            : error instanceof NoMessagesError
              ? "未找到可转发的消息\n请确认消息未被删除"
              : "保送失败，请检查目标权限或稍后重试",
    );
  }
...
export default function createBs() {
  return definePlugin({
    ...
    apiVersion: 1,
    id: "bs",
    description: `保送. 将回复的消息转发至指定频道 支持数字参数 表示要转发的消息条数(会自动跳过已删除的消息). 从左往右获取到第一个有权限发送的频道: ${channels.join(", ")}`,
    commands: {
      bs: {
        description: "回复消息后转发到配置的频道",
