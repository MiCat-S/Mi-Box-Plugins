...
export default function createEncode() {
  return definePlugin({
    ...
    apiVersion: 1,
    id: "encode",
    description: "Base64 与 URL 编码解码工具",
    commands: {
      encode: {
        helpOnEmpty: true,
...
async function handle(
  invocation: { message: MessageEnvelope; args: readonly string[] },
  ctx: PluginContext,
  operation: string,
  label: string,
): Promise<void> {
  ...
  if (!input) {
    await ctx.telegram.edit(
      invocation.message,
      `<b>缺少文本内容</b>\n请提供要${operation.endsWith("decode") ? "解码" : "编码"}的文本，或回复一条消息`,
      { parseMode: "html" },
    );
    return;
  }
  if (input.length > MAX_INPUT) {
    await ctx.telegram.edit(invocation.message, `<b>处理失败</b>\n输入不能超过 <code>${MAX_INPUT}</code> 个字符`, {
      parseMode: "html",
    });
    return;
  }
  ...
  try {
    ...
    await ctx.telegram.edit(
      invocation.message,
      `<b>${label}失败</b>\n输入不是有效的${operation.startsWith("b64") ? " Base64 或 UTF-8 文本" : " URL 编码或文本"}`,
      { parseMode: "html" },
    );
    return;
  }
