...
export async function writeAll(handle:FileHandle,chunk:Uint8Array,signal:AbortSignal):Promise<void>{let offset=0;while(offset<chunk.byteLength){
  signal.throwIfAborted();const{bytesWritten}=await handle.write(chunk,offset,chunk.byteLength-offset,null);signal.throwIfAborted();
  if(bytesWritten<=0)throw new GifError("媒体写入失败");offset+=bytesWritten;
}}
  ...
  await context.telegram.withClient(async(client,clientSignal)=>{
    ...
    try{
      for await(const chunk of client.iterDownload(raw.media,{requestSize:512*1024,signal:active})){
        active.throwIfAborted();total+=chunk.length;if(total>maximum)throw new GifError("文件超过配置上限");await writeAll(handle,chunk,active);active.throwIfAborted();
      }
...
const ffmpegArgs=(input:string,output:string,config:Config,low:boolean)=>["-nostdin","-y","-i",input,"-t",String(config.maxDuration),
  ...
  "-an","-c:v","libvpx-vp9","-b:v","0","-crf",String(low?Math.min(31,config.quality+10):config.quality),"-pix_fmt","yuva420p","-auto-alt-ref","0","-f","webm","-fs",String(MAX_WORK_OUTPUT_BYTES),output];
...
async function convert(context:PluginContext,raw:any,config:Config,peer:any,replyTo:number|undefined):Promise<void>{
  await context.files.withTemp(async(directory,signal)=>{
    ...
    const {Api}=await import("teleproto");signal.throwIfAborted();context.signal.throwIfAborted();
    await context.telegram.withClient(async(client,clientSignal)=>{const active=AbortSignal.any([context.signal,signal,clientSignal]);active.throwIfAborted();
  ...
  const command:CommandDefinition={description:"将回复的 GIF 或短视频转换为动态贴纸",helpArgs:["help","h"],args:"",arguments:[{name:"回复媒体",required:true,description:"回复 GIF 或视频后运行"}],examples:[{args:"",description:"回复 GIF 或视频"},{args:"clear"}],subcommandsCaseSensitive:false,subcommands:{clear},
    ...
    async handle(invocation,context){
      ...
      try{
        ...
        await context.telegram.edit(invocation.message,"正在下载并转换动态贴纸…");await convert(context,source,config,raw.peerId,reply?.id);context.signal.throwIfAborted();
