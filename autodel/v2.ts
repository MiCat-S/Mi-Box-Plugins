...
async function migrate(context: PluginContext): Promise<void> {
  ...
  await store(context).update(value => ({...value, schemaVersion: 1,
    // Existing V2 settings are newer than the one-time legacy import.
    settings: {...imported, ...value.settings}, importedLegacy: true}));
}
...
export default function createPlugin() {
  ...
  const definition = definePlugin({renderHelp: renderPluginHelp,
  apiVersion: 1,
  id: "autodel",
