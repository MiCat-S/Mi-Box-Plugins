...
function visibleConfig(cfg: Config): ui.Html {
  const providers: ui.Html[] = Object.keys(cfg.configs)
    .sort()
    .map(tag => {
      ...
      return ui.concat(
        ...
        ui.code(tag),
        ui.text(
          ` · ${provider.type ?? "auto"}${models ? ` · ${models}` : ""} · stream=${provider.stream ? "on" : "off"} · responses=${provider.responses ? "on" : "off"}`,
        ),
      );
    });
  ...
  return ui.concat(
    ...
    joinLines(providers),
    ui.text("\n\n"),
    ui.field("聊天", `${cfg.currentChatTag || "-"} / ${cfg.currentChatModel || "-"}`),
    ui.text("\n"),
    ui.field("搜索", `${cfg.currentSearchTag || "-"} / ${cfg.currentSearchModel || "-"}`),
    ui.text("\n"),
    ui.field("图片", `${cfg.currentImageTag || "-"} / ${cfg.currentImageModel || "-"}`),
    ui.text("\n"),
    ui.field("视频", `${cfg.currentVideoTag || "-"} / ${cfg.currentVideoModel || "-"}`),
    ui.text("\n"),
...
function telegraphStatus(cfg: Config): string {
  ...
  if (cfg.telegraph.list.length) {
    status +=
      ...
      cfg.telegraph.list
        .slice(0, 100)
        .map((item, index) => `${index + 1}. <a href="${escape(item.url)}">🔗 ${escape(item.title)}</a>`)
        .join("\n");
  }
  return status;
}
async function setModel(invocation: CommandInvocation, ctx: PluginContext): Promise<void> {
  ...
  requireInput(
    ["chat", "search", "image", "video"].includes(mode) && tag && model,
    "用法：ai model chat|search|image|video tag model",
  );
  assertAllowedModel(model);
  await updateConfig(ctx, (raw, cfg) => {
    const provider = cfg.configs[tag];
    requireInput(provider, "API 配置不存在");
    raw[modeKey(mode, "Tag")] = tag;
    raw[modeKey(mode, "Model")] = model;
    raw.configs = {
      ...record(raw.configs),
      [tag]: { ...provider, models: { ...record(provider.models), [mode]: model } },
    };
  });
...
async function setEnum(
  ...
  await updateConfig(ctx, raw => {
    raw[modeKey(mode, kind === "reasoning" ? "ReasoningEffort" : "ServiceTier")] = value;
  });
  await ctx.telegram.edit(invocation.message, feedback("success", `${kind} 已设置为 ${value}`), htmlOptions);
}
async function askImages(
  ctx: PluginContext,
  replied: MessageEnvelope | undefined,
  own: MessageEnvelope,
): Promise<{ images: ChatImage[]; dropped: boolean }> {
  ...
  return {
    images: merged.images.map(item => ({ data: item.data, mimeType: item.mimeType.toLowerCase() })),
...
function normalizeVideoMode(
  requested: VideoImageMode,
  images: readonly MediaInput[],
  hasPrompt: boolean,
): { mode: VideoImageMode; images: MediaInput[] } {
  let mode = requested;
  let parts = [...images];
...
async function ask(invocation: CommandInvocation, ctx: PluginContext, search: boolean): Promise<void> {
  ...
  requireInput(
    ...
    search ? "请输入搜索问题或回复一条文字消息" : "请输入问题或回复一条文字消息",
  );
  ...
  if (images.dropped)
    await ctx.telegram.reply(invocation.message, "⚠️ 部分图片因数量或体积限制被忽略", { parseMode: "html" });
  ...
  if (search) {
    ...
    if (cfg.telegraph.enabled && formatted.length > 4050) {
      ...
      await deliverTelegraphAnswer(
        ctx,
        invocation.message,
        { question, url, tag: cfg.currentSearchTag, collapse: cfg.collapse, replyToId },
        ctx.signal,
      );
    } else {
      await deliverAnswer(
  ...
  if (cfg.telegraph.enabled && formatted.length > 4050) {
    ...
    await deliverTelegraphAnswer(
      ctx,
      invocation.message,
      { question, url, tag: cfg.currentChatTag, collapse: cfg.collapse, replyToId },
      ctx.signal,
    );
  } else {
    await deliverAnswer(
      ctx,
      invocation.message,
      { question, answer: answerText, tag: cfg.currentChatTag, collapse: cfg.collapse, replyToId },
      ctx.signal,
    );
  }
}
async function media(invocation: CommandInvocation, ctx: PluginContext, kind: "image" | "video"): Promise<void> {
  ...
  const requested: VideoImageMode =
    kind === "video" && ["first", "firstlast", "reference"].includes(action) ? (action as VideoImageMode) : "auto";
  ...
  const replyText = replied?.text.trim() ?? "";
  const prompt =
    ownPrompt && replyText && replyCollect.images.length === 0
      ? `${replyText}\n\n${ownPrompt}`
      : ownPrompt || replyText;
  requireInput(
    kind === "image" ? prompt : prompt || inputs.length,
    kind === "image" ? "至少需要一条文字提示" : "至少需要文字提示或参考图",
  );
  if (merged.dropped)
    await ctx.telegram.reply(invocation.message, "⚠️ 部分图片因数量或体积限制被忽略", { parseMode: "html" });
  if (kind === "image") {
  ...
  const result = input.map(item => {
    ...
    requireInput(
      value.data instanceof Uint8Array &&
        ...
        /^image\/(?:jpeg|png|gif|webp)$/i.test(value.mimeType),
      "图片输入无效",
    );
...
function serviceText(input: unknown): ServiceTextInput {
  ...
  requireInput(
    value.model === undefined || (typeof value.model === "string" && value.model.trim().length > 0),
    "模型无效",
  );
  requireInput(value.tag === undefined || (typeof value.tag === "string" && value.tag.trim().length > 0), "提供商无效");
  requireInput(
    value.reasoningEffort === undefined || reasoningValues.includes(value.reasoningEffort as ReasoningEffort),
    "思考强度无效",
  );
  requireInput(
  ...
  requireInput(
    value.maxOutputTokens === undefined ||
      (Number.isSafeInteger(value.maxOutputTokens) &&
        ...
        Number(value.maxOutputTokens) <= 32768),
    "输出上限无效",
  );
  requireInput(
    value.fallbackToChatCompletions === undefined || typeof value.fallbackToChatCompletions === "boolean",
    "兼容回退选项无效",
  );
  ...
  return {
    ...
    ...(typeof value.fallbackToChatCompletions === "boolean"
      ? { fallbackToChatCompletions: value.fallbackToChatCompletions }
      : {}),
    ...(images ? { images } : {}),
  };
}
...
interface ProviderImport {
  ...
  stream: boolean;
  responses: boolean;
  models: Partial<Record<ProviderMode, string>>;
  select: ProviderMode[];
}
function importedProvider(input: unknown): ProviderImport {
  ...
  requireInput(
    /^[A-Za-z0-9._-]{1,64}$/.test(tag) && !["__proto__", "constructor", "prototype"].includes(tag),
    "导入标签无效",
  );
  ...
  const models: Partial<Record<ProviderMode, string>> = {};
  for (const mode of ["chat", "search", "image", "video"] as const)
    if (typeof source[mode] === "string" && source[mode].trim()) {
      const model = source[mode].trim();
      assertAllowedModel(model);
      models[mode] = model;
    }
  const select = Array.isArray(value.select)
    ? value.select.filter(
        (mode): mode is ProviderMode => typeof mode === "string" && ["chat", "search", "image", "video"].includes(mode),
      )
    : [];
  return {
    tag,
...
function sameImportedProvider(current: Config["configs"][string], value: ProviderImport): boolean {
  return (
    ...
    current.stream === value.stream &&
    current.responses === value.responses
  );
}

function availableImportTag(
  configs: Config["configs"],
  value: ProviderImport,
): { tag: string; existing?: Config["configs"][string] } {
  const requested = configs[value.tag];
  if (!requested || sameImportedProvider(requested, value))
    return { tag: value.tag, ...(requested ? { existing: requested } : {}) };
...
export default function createAi() {
  const guard =
    (run: (invocation: CommandInvocation, ctx: PluginContext) => Promise<void>) =>
    async (invocation: CommandInvocation, ctx: PluginContext): Promise<void> => {
      try {
        await run(invocation, ctx);
      } catch (error) {
        if (!ctx.signal.aborted)
          await ctx.telegram.edit(
            invocation.message,
            feedback("error", "AI 操作失败", safeMessage(error), "检查配置、API 可用性和网络后重试"),
            htmlOptions,
          );
      }
    };
  const providerType = (value: string | undefined): void => {
    if (value) requireInput(providerTypes.includes(value as (typeof providerTypes)[number]), "无效 API 类型");
  };
  const configList = guard(async (invocation, ctx) => {
    await sendPaged(ctx, invocation.message, visibleConfig(await readConfig(ctx)));
  });
  const configAdd = guard(async (invocation, ctx) => {
    ...
    await updateConfig(ctx, (raw, cfg) => {
      const configs =
        raw.configs && typeof raw.configs === "object" && !Array.isArray(raw.configs)
          ? (raw.configs as Record<string, unknown>)
          : {};
      configs[tag] = {
        tag,
        url,
  ...
  const configDel = guard(async (invocation, ctx) => {
    ...
    await updateConfig(ctx, raw => {
      ...
      for (const mode of modes)
        if (raw[`current${mode}Tag`] === tag) {
          raw[`current${mode}Tag`] = "";
          raw[`current${mode}Model`] = "";
        }
    });
    await ctx.telegram.edit(invocation.message, feedback("success", "AI 配置已更新"), htmlOptions);
  });
  const configToggle = (action: "type" | "stream" | "responses") =>
    guard(async (invocation, ctx) => {
      const [tag = "", value = ""] = invocation.args;
      requireInput(tag && value, `用法：ai config ${action} tag value`);
      await updateConfig(ctx, (raw, cfg) => {
        const current = cfg.configs[tag];
        requireInput(current, "API 配置不存在");
  ...
  const setCollapse = guard(async (invocation, ctx) => {
    ...
    await updateConfig(ctx, raw => {
      raw.collapse = bool(invocation.args[0] ?? "");
    });
    await ctx.telegram.edit(invocation.message, feedback("success", "AI 输出设置已更新"), htmlOptions);
  });
  const setTimeoutValue = guard(async (invocation, ctx) => {
    ...
    requireInput(Number.isSafeInteger(seconds) && seconds >= 1 && seconds <= 600, "超时范围为 1-600 秒");
    await updateConfig(ctx, raw => {
      raw.timeout = seconds;
    });
    await ctx.telegram.edit(invocation.message, feedback("success", "AI 输出设置已更新"), htmlOptions);
  });
  const telegraphToggle = (enabled: boolean) =>
    guard(async (invocation, ctx) => {
      await updateConfig(ctx, raw => {
        const current =
          raw.telegraph && typeof raw.telegraph === "object" && !Array.isArray(raw.telegraph)
            ? (raw.telegraph as Record<string, unknown>)
            : {};
  ...
  const telegraphLimit = guard(async (invocation, ctx) => {
    ...
    requireInput(Number.isSafeInteger(limit) && limit >= 1 && limit <= 100, "记录容量范围为 1-100");
    await updateConfig(ctx, raw => {
      const current =
        raw.telegraph && typeof raw.telegraph === "object" && !Array.isArray(raw.telegraph)
          ? (raw.telegraph as Record<string, unknown>)
  ...
  const telegraphDel = guard(async (invocation, ctx) => {
    ...
    if (index >= count) {
      await ctx.telegram.edit(invocation.message, `序号超出范围 (1-${count})`, htmlOptions);
      return;
    }
    if (!removed) {
      await ctx.telegram.edit(invocation.message, "该记录已不存在，请刷新状态后重试", htmlOptions);
      return;
    }
    await ctx.telegram.edit(invocation.message, feedback("success", `已删除第 ${index + 1} 项`), htmlOptions);
  });
  const mediaPreview = (kind: "image" | "video") =>
    guard(async (invocation, ctx) => {
      const cfg = await readConfig(ctx);
      ...
      if (!invocation.args.length) {
        await sendPaged(
          ctx,
          invocation.message,
          `${kind === "image" ? "🖼️" : "🎬"} <b>${kind === "image" ? "图片" : "视频"}预览状态:</b>\n\n📄 当前状态: ${current ? "开启" : "关闭"}`,
        );
        return;
      }
      ...
      await updateConfig(ctx, raw => {
        raw[kind === "image" ? "imagePreview" : "videoPreview"] = value === "on";
      });
      await ctx.telegram.edit(
        invocation.message,
        feedback("success", `${kind} preview 已设置为 ${value}`),
        htmlOptions,
      );
    });
    ...
    subcommands: {
      help: {
        ...
        handle: async (invocation, ctx) => {
          await ctx.telegram.edit(invocation.message, renderAiHelp(invocation.prefix), htmlOptions);
        },
      },
      config: {
        group: "⚙️ API 配置:",
        description: "管理 API 配置",
        args: "add|del|list|type|stream|responses ...",
        defaultSubcommand: "list",
        subcommandsCaseSensitive: true,
        subcommands: {
          add: {
            description: "添加 API 配置",
            args: "tag url key [type]",
            arguments: [
              { name: "tag", required: true },
              { name: "url", required: true },
              { name: "key", required: true, description: "仅收藏夹中配置" },
              { name: "type", description: providerTypes.join("/") },
            ],
            examples: [{ args: "add main https://api.openai.com secret openai-compatible" }],
            handle: configAdd,
          },
          ...
          type: {
            description: "设置 API 类型；不设置时按 URL 特征自动识别",
            args: "tag type",
            arguments: [{ name: "type", required: true, description: providerTypes.join("/") }],
            examples: [{ args: "type main gemini" }],
            handle: configToggle("type"),
          },
          stream: {
            description: "设置 API 流式传输",
            args: "tag on|off",
            examples: [{ args: "stream main on" }],
            handle: configToggle("stream"),
          },
          responses: {
            description: "设置 chat/search 的 Responses 模式",
            args: "tag on|off",
            examples: [{ args: "responses main on" }],
            handle: configToggle("responses"),
          },
        },
        handle: guard(async () => {
          throw new InputError("未知 config 子命令");
        }),
        help: [
          { heading: "API 类型：", body: `可选值：${providerTypes.join(" / ")}。若不设置，按 URL 特征自动识别。` },
        ],
      },
      model: {
        group: "🧠 模型设置:",
        description: "设置聊天/搜索/图片/视频模型",
        args: "chat|search|image|video tag model",
        examples: [{ args: "model chat main gpt-4o" }],
        handle: guard(setModel),
      },
      reasoning: {
        group: "🧠 模型设置:",
        description: "设置思考强度",
        args: "chat|search auto|none|minimal|low|medium|high|xhigh",
        examples: [{ args: "reasoning chat medium" }],
        handle: guard((invocation, ctx) => setEnum(invocation, ctx, "reasoning")),
      },
      service: {
        group: "🧠 模型设置:",
        description: "设置服务等级",
        args: "chat|search auto|default|priority|fast|flex",
        examples: [{ args: "service chat auto" }],
        handle: guard((invocation, ctx) => setEnum(invocation, ctx, "service")),
      },
      image: {
        group: "💬 提问:",
        description: "文生/编辑图片",
        ...
        subcommands: {
          preview: {
            description: "开/关图片预览",
            args: "on|off",
            examples: [{ args: "preview on" }],
            handle: mediaPreview("image"),
          },
        },
      ...
      video: {
        group: "💬 提问:",
        description: "文生/参考图生成视频",
        args: "[first|firstlast] 提示词",
        subcommands: {
          preview: {
            description: "开/关视频预览",
            args: "on|off",
            examples: [{ args: "preview on" }],
            handle: mediaPreview("video"),
          },
          audio: { description: "开/关视频音频", args: "on|off", examples: [{ args: "audio on" }], handle: videoAudio },
          duration: {
            description: "视频输出时长（5-20 秒）",
            args: "秒数",
            examples: [{ args: "duration 10" }],
      ...
      collapse: {
        ...
        args: "on|off",
        caseSensitive: true,
        examples: [{ args: "collapse on" }],
        handle: setCollapse,
      },
      timeout: {
        group: "🧩 消息设置:",
        description: "设置超时时间（1-600 秒）",
        args: "秒数",
        caseSensitive: true,
        examples: [{ args: "timeout 120" }],
        handle: setTimeoutValue,
      },
      telegraph: {
        group: "📰 Telegraph:",
        description: "管理 Telegraph 长文",
        args: "on|off|limit 数量|del all",
        caseSensitive: true,
        subcommands: {
          ...
          limit: {
            description: "设置记录容量（1-100）",
            args: "数量",
            examples: [{ args: "limit 50" }],
            handle: telegraphLimit,
          },
          del: {
            description: "删除一条记录或全部（del 序号|all）",
            args: "序号|all",
            examples: [{ args: "del 1" }, { args: "del all" }],
            handle: telegraphDel,
          },
        },
        handle: guard(async (invocation, ctx) => {
      ...
      search: {
        ...
        description: "联网搜索并回答",
        args: "问题",
        examples: [{ args: "search 今天有什么新闻" }],
        handle: guard(async (invocation, ctx) => ask(invocation, ctx, true)),
      },
    },
    help: [
      {
        heading: "使用说明：",
        body: "• <code>{prefix}ai 问题</code> 直接提问；回复一条文字消息后发送 <code>{prefix}ai</code> 会带上该文字\n• 不回复且没有问题时提示缺少输入\n• <code>{prefix}ai search 问题</code> 联网搜索；<code>{prefix}ai image/video 提示词</code> 生成媒体",
      },
      { heading: "密钥配置：", body: "涉及 API Key、Token 或其他登录凭据的设置命令请在收藏夹中执行。" },
    ],
    handle: guard(async (invocation, ctx) => ask(invocation, ctx, false)),
  };
  const renderAiHelp = (prefix: string): string =>
    renderCommandHelp("ai", aiCommand, { prefix, title: "🤖 智能 AI 助手" });
  return definePlugin({
    apiVersion: STRUCTURED_PLUGIN_API_VERSION,
    id: "ai",
    description: "AI 对话、搜索、媒体生成与配置",
    renderHelp: renderAiHelp,
    settings,
    commands: { ai: aiCommand },
    services: {
      chat: {
        description: "使用当前聊天模型生成文字，可指定 model/tag/reasoningEffort",
        async handle(input, ctx, signal) {
          const value = serviceText(input);
          ...
          try {
            return await chatText(
              ...
              value.text,
              signal,
              value.systemPrompt ?? cfg.prompt,
              {},
              options,
            );
          } catch (error) {
            if (
              !value.fallbackToChatCompletions ||
              !provider.responses ||
            ...
            return chatText(
              { ...selected, configs: { ...selected.configs, [tag]: fallback } },
              ctx.http,
              value.text,
              signal,
              value.systemPrompt ?? cfg.prompt,
      ...
      search: {
        description: "使用当前搜索模型联网检索并返回文字与来源",
        async handle(input, ctx, signal) {
          const value = serviceText(input);
          requireInput(!value.images, "搜索服务不接受图片");
          const cfg = await readConfig(ctx, signal);
          const tag = value.tag ?? cfg.currentSearchTag;
          ...
          return searchText(
            {
              ...
              prompt: value.systemPrompt ?? cfg.prompt,
            },
            ctx,
            value.text,
            signal,
          ...
          requireInput(
            value.timeoutMs === undefined ||
              (Number.isSafeInteger(value.timeoutMs) &&
                ...
                Number(value.timeoutMs) <= 1_800_000),
            "超时无效",
          );
          const media = value.input === undefined ? undefined : serviceImages([value.input])?.[0];
          const cfg = await readConfig(ctx, signal);
          const tag = typeof value.tag === "string" ? value.tag.trim() : cfg.currentImageTag;
          requireInput(Boolean(cfg.configs[tag]), "未找到指定的 AI 提供商");
          const model =
            typeof value.model === "string"
              ? value.model.trim()
              : selectedModel(cfg, tag, "image", cfg.currentImageModel);
          const selected = {
            ...cfg,
            currentImageTag: tag,
          ...
          const tag =
            typeof input === "string"
              ...
              : input && typeof input === "object"
                ? String((input as Record<string, unknown>).tag ?? "").trim()
                : cfg.currentChatTag;
      ...
      diagnostics: {
        description: "返回不含凭据和原始错误的 API 诊断与基准结果",
        async handle(input, ctx, signal) {
          requireInput(input !== null && typeof input === "object" && !Array.isArray(input), "诊断输入无效");
          const value = input as Record<string, unknown>,
            action = value.action;
          requireInput(
            (action === "full" || action === "benchmark") && typeof value.tag === "string" && value.tag.trim(),
            "诊断输入无效",
          );
          if (action === "benchmark" && value.models !== undefined)
            requireInput(
              Array.isArray(value.models) &&
                value.models.length >= 1 &&
                value.models.length <= 3 &&
                value.models.every(model => typeof model === "string" && model.trim()),
              "诊断模型无效",
            );
          const cfg = await readConfig(ctx, signal);
          return diagnostics(
            ...
            cfg,
            {
              action,
              tag: value.tag.trim(),
              ...(action === "benchmark" && Array.isArray(value.models)
                ? { models: (value.models as string[]).map(model => model.trim()) }
                : {}),
            } as DiagnosticsInput,
            signal,
          );
      ...
      import_provider: {
        description: "把已安装插件的既有 AI 配置一次性迁入统一配置",
        async handle(input, ctx, signal) {
          const value = importedProvider(input);
          let imported = false;
          let actualTag = value.tag;
          await updateConfig(
            ctx,
            (raw, cfg) => {
              const target = availableImportTag(cfg.configs, value);
              actualTag = target.tag;
              const models = { ...(target.existing?.models ?? {}) };
              for (const [mode, model] of Object.entries(value.models))
                if (model && !models[mode as ProviderMode]) models[mode as ProviderMode] = model;
              if (!target.existing) {
                raw.configs = {
                  ...record(raw.configs),
                  [actualTag]: {
              ...
              for (const mode of value.select) {
                ...
                if (!model) continue;
                const title = `${mode[0].toUpperCase()}${mode.slice(1)}`;
                const tagKey = `current${title}Tag`;
                const modelKey = `current${title}Model`;
                if (!raw[tagKey] || !raw[modelKey]) {
                  raw[tagKey] = actualTag;
                  raw[modelKey] = model;
                }
              }
            },
      ...
      selection: {
        description: "只读返回统一提供商及各模式的当前选择",
        async handle(_input, ctx, signal) {
          const cfg = await readConfig(ctx, signal);
          return {
            chat: {
              tag: cfg.currentChatTag,
              model: cfg.currentChatModel,
              reasoningEffort: cfg.currentChatReasoningEffort,
              serviceTier: cfg.currentChatServiceTier,
            },
            search: {
              tag: cfg.currentSearchTag,
              model: cfg.currentSearchModel,
              reasoningEffort: cfg.currentSearchReasoningEffort,
              serviceTier: cfg.currentSearchServiceTier,
            },
            image: { tag: cfg.currentImageTag, model: cfg.currentImageModel },
            video: { tag: cfg.currentVideoTag, model: cfg.currentVideoModel },
            providers: Object.keys(cfg.configs)
              .sort()
              .map(tag => ({ tag, type: cfg.configs[tag].type ?? "auto", models: { ...cfg.configs[tag].models } })),
          };
        },
      },
    },
