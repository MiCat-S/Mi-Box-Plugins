...
export async function searchText(
  ...
  let selected = {
    ...cfg,
    currentChatTag: cfg.currentSearchTag,
    currentChatModel: cfg.currentSearchModel,
    currentChatReasoningEffort: cfg.currentSearchReasoningEffort,
    currentChatServiceTier: cfg.currentSearchServiceTier,
  };
  ...
  if (type === "local-cliproxy" && selected.currentChatModel.includes("gemini")) {
    ...
    selected = {
      ...selected,
      configs: { ...selected.configs, [selected.currentChatTag]: { ...provider, type: "gemini", url: url.toString() } },
    };
  }
  ...
  if (req.format === "gemini") body.tools = [{ googleSearch: {} }];
    ...
    body.tools = [{ type: "web_search", web_search: { searchContextSize: "high" } }];
  ...
  for (const payload of (Array.isArray(parsed) ? parsed : [parsed]).map(record)) {
    if (req.format === "gemini") {
      ...
      for (const chunk of list(metadata.groundingChunks ?? metadata.grounding_chunks).map(record)) {
        const web = record(chunk.web ?? chunk.web_chunk);
        append(web.uri, web.title);
      }
    } else {
      ...
      for (const obj of [payload, choice, record(choice.message), record(choice.delta)]) {
        for (const entry of [...list(obj.citations), ...list(obj.annotations)].map(record)) {
          const citation = record(entry.url_citation ?? entry);
          append(citation.url, citation.title);
        }
      }
      ...
      for (const item of [payload.item, ...list(response.output)].map(record)) {
        for (const part of list(item.content).map(record))
          for (const annotation of list(part.annotations).map(record)) append(annotation.url, annotation.title);
        for (const action of (Array.isArray(item.action) ? item.action : [item.action]).map(record))
          for (const source of list(action.sources).map(record)) append(source.url, source.title);
      }
      for (const annotation of list(record(payload.part).annotations).map(record))
        append(annotation.url, annotation.title);
    }
  }
...
export async function sendText(
  ...
  for (const character of text) {
    ...
    if (page.length + escaped.length > 3600) {
      pages.push(page);
      page = "";
    }
    page += escaped;
...
export async function publish(
  ...
  if (typeof page.url !== "string" || !/^https:\/\/telegra\.ph\//.test(page.url))
