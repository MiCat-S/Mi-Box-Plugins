...
export interface Config extends Mutable<ChatConfigSnapshot>, Record<string, unknown> {
  ...
  telegraph: {
    enabled: boolean;
    limit: number;
    list: { url: string; title: string; createdAt: string }[];
    [key: string]: unknown;
  };
}
...
function providerUrl(provider: Record<string, unknown>): string {
  ...
  const type = resolveProviderType({
    url,
    ...(typeof provider.type === "string" ? { type: provider.type as ProviderConfig["type"] } : {}),
  });
...
export function snapshot(raw: Record<string, unknown>): Config {
  ...
  const cfg = {
    ...base,
    ...raw,
    configs: { ...record(raw.configs) },
    telegraph: { ...base.telegraph, ...record(raw.telegraph) },
  } as Config;
  for (const [tag, value] of Object.entries(cfg.configs)) {
    const p = record(value);
    const modelSource = record(p.models);
    const models = Object.fromEntries(
      ["chat", "search", "image", "video"].flatMap(mode =>
        typeof modelSource[mode] === "string" && String(modelSource[mode]).trim()
          ? [[mode, String(modelSource[mode]).trim()]]
          : [],
      ),
    );
    cfg.configs[tag] = {
      ...
      responses: providerResponses(p),
      models,
    } as Config["configs"][string];
  }
  for (const mode of modes)
    for (const suffix of ["Tag", "Model"] as const) {
  ...
  for (const mode of ["Chat", "Search"] as const) {
    const effort = String(cfg[`current${mode}ReasoningEffort`] ?? "")
      .trim()
      .toLowerCase();
    const tier = String(cfg[`current${mode}ServiceTier`] ?? "")
      .trim()
      .toLowerCase();
    cfg[`current${mode}ReasoningEffort`] = reasoningValues.includes(effort as ReasoningEffort)
      ? (effort as ReasoningEffort)
      : "auto";
    cfg[`current${mode}ServiceTier`] = tierValues.includes(tier as ServiceTier) ? (tier as ServiceTier) : "auto";
  }
  for (const key of ["imagePreview", "videoPreview", "videoAudio", "collapse"] as const)
    if (typeof cfg[key] !== "boolean") cfg[key] = base[key];
  ...
  cfg.telegraph.list = Array.isArray(cfg.telegraph.list)
    ? cfg.telegraph.list.filter(
        item =>
...
export async function updateConfig(
  ctx: PluginContext,
  mutate: (raw: Record<string, unknown>, view: Config) => void,
  signal = ctx.signal,
): Promise<void> {
  await store(ctx).update(raw => {
    signal.throwIfAborted();
    mutate(raw, snapshot(raw));
    return raw;
  }, signal);
}
...
const schema: Schema = [
  ...
  ...(["Chat", "Search"] as const).flatMap(mode => [
    {
      key: `current${mode}ReasoningEffort`,
      label: `${mode} 思考强度`,
      type: "select" as const,
      options: reasoningValues.map(value => ({ value, label: value })),
    },
    {
      key: `current${mode}ServiceTier`,
      label: `${mode} 服务等级`,
      type: "select" as const,
      options: tierValues.map(value => ({ value, label: value })),
    },
  ]),
  ...(["imagePreview", "videoPreview", "videoAudio", "collapse"] as const).map(key => ({
    key,
    label: key,
          ...
          if (key.startsWith("telegraph.")) raw.telegraph = { ...record(raw.telegraph), [key.slice(10)]: value };
            ...
            if (typeof parsed === "string") {
              try {
                ...
                throw new InputError("API 配置 JSON 无效");
              }
            }
            ...
            for (const [tag, provider] of Object.entries(record(parsed))) {
              requireInput(
                !["__proto__", "constructor", "prototype"].includes(tag) && Object.keys(record(provider)).length,
                "无效 API 配置",
              );
