...
export type ProviderType =
  "openai" | "openai-compatible" | "gemini" | "anthropic" | "codex" | "doubao" | "moonshot" | "local-cliproxy";
...
export const CODEX_USER_AGENT = "codex-tui/0.146.0 (Mac OS 26.5.0; arm64) iTerm.app/3.6.10 (codex-tui; 0.146.0)";
...
function safeError(error: unknown): ProviderError {
  ...
  switch (object(error).code) {
    ...
    case "RESPONSE_TOO_LARGE":
      return new ProviderError("RESPONSE_TOO_LARGE");
    case "INVALID_JSON":
      return new ProviderError("INVALID_RESPONSE");
    case "CLEANUP_FAILED":
      return new ProviderError("CLEANUP_FAILED");
    default:
      return new ProviderError("FAILED");
  }
}
...
const providerTypes: readonly string[] = [
  "openai",
  "openai-compatible",
  "gemini",
  "anthropic",
...
export function resolveProviderType(provider: Pick<ProviderConfig, "type" | "url">): ProviderType {
  ...
  try {
    return hostTypes[new URL(provider.url).hostname] ?? "openai";
  } catch {
    return "openai";
  }
}
...
export function buildChatRequest(
  ...
  try {
    ...
    if (
      ...
      images.some(
        image =>
          !image || !(image.data instanceof Uint8Array) || !/^image\/(?:jpeg|png|gif|webp)$/i.test(image.mimeType),
      )
    )
    ...
    if (
      ...
      (!Number.isSafeInteger(options.maxOutputTokens) || options.maxOutputTokens < 1 || options.maxOutputTokens > 32768)
    )
      throw new ProviderError("INPUT");
    ...
    if (!["https:", "http:"].includes(parsedUrl.protocol) || typeof provider.key !== "string")
      throw new ProviderError("CONFIG");
    ...
    let url = gemini
      ...
      : anthropic
        ? endpoint(normalizeOpenAIBaseUrl(base), "messages")
        : provider.responses
    ...
    if (gemini) {
      ...
      if (options.maxOutputTokens !== undefined)
        data.generationConfig = { ...object(data.generationConfig), maxOutputTokens: options.maxOutputTokens };
    } else if (anthropic) {
      ...
      if (provider.responses) {
        ...
        data = {
          model,
          input: encodedImages.length
            ? [{ role: "user", content }]
            : text.trim()
              ? [{ role: "user", content: [{ type: "input_text", text: text.trim() }] }]
              : text,
...
export function parseChatText(raw: string, format: ChatRequest["format"], limits: ProviderLimits = {}): string {
  ...
  if (format === "gemini") {
    ...
    text = list(object(candidate.content).parts)
      .map(value => string(object(value).text))
      .join("");
  } else if (format === "anthropic") {
    const payload = object(parseJson(raw));
    checkProviderFailure(payload);
    text = list(payload.content)
      .map(value => {
        const part = object(value);
    ...
    const responses = payloads.some(
      payload =>
        payload.object === "response" ||
        object(payload.response).object === "response" ||
    ...
    for (const payload of payloads) {
      ...
      if (responses) {
        ...
        if (payload.type === "response.output_text.done" && typeof payload.text === "string" && !deltas.length)
          fallback = checked(payload.text).trim();
        const response =
          object(payload.response).object === "response"
...
export async function readBody(response: Response, signal: AbortSignal, maxBytes: number): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      failure ??= new ProviderError("CLEANUP_FAILED");
    } finally {
      reader.releaseLock();
    }
  }
...
export async function chatText(
  ...
  try {
    ...
    const result = await http.withResponse(
      request.url,
      request.init,
  ...
  try {
    ...
    const base =
      type === "doubao"
        ...
        : type === "gemini"
          ? (() => {
              const current = new URL(provider.url);
              current.pathname = "/v1beta";
              current.search = "";
              current.hash = "";
    ...
    if (result.error) throw result.error;
    const payload = object(parseJson(result.text!));
    checkProviderFailure(payload);
