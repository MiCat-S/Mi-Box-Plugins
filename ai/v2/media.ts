...
function auth(
  ...
  if (type === "gemini" || type === "local-cliproxy") {
    const parsed = new URL(url);
    if (!parsed.searchParams.has("key")) parsed.searchParams.set("key", provider.key);
...
async function json(
  ...
  const result = await ctx.http.withResponse(
    request.url,
    ...
    async (response, active) => {
      ...
      try {
        return { text: await readBody(response, active, 32 * 1024 * 1024) };
      } catch (error) {
        return { error: error instanceof ProviderError ? error : new ProviderError("FAILED") };
      }
    },
...
async function raw(
  ...
  const result = await ctx.http.withResponse(
    ...
    async (response, active) => {
      if (!response.ok) return { error: new ProviderError("HTTP_STATUS", response.status) };
      try {
        return { text: await readBody(response, active, 8 * 1024 * 1024) };
      } catch (error) {
        return { error: error instanceof ProviderError ? error : new ProviderError("FAILED") };
      }
    },
...
async function codexStream(response: Response, signal: AbortSignal): Promise<CodexResult> {
  ...
  const consume = (block: string): void => {
    for (const line of block.split(/\r?\n/)) {
      ...
      try {
        visitCodex(JSON.parse(data), result);
      } catch {
  ...
  try {
    for (;;) {
      ...
      if (total > 48 * 1024 * 1024) throw new ProviderError("RESPONSE_TOO_LARGE");
...
async function codexImages(
  ...
  const content = input
    ? [
        ...
        { type: "input_image", image_url: `data:${input.mimeType};base64,${input.data.toString("base64")}` },
      ]
    : prompt;
  const body = {
    model,
  ...
  let result = await ctx.http.withResponse(
    endpointUrl,
    { method: "POST", redirect: "manual", credentials: "omit", headers, body: JSON.stringify(body) },
    codexStream,
    {
  ...
  while (!result.image && result.id && result.status === "in_progress" && Date.now() < deadline) {
    ...
    const response = await ctx.http.withResponse(
      pollUrl,
      { method: "GET", redirect: "manual", credentials: "omit", headers },
      ...
      {
        timeoutMs: Math.max(1000, Math.min(60_000, deadline - Date.now())),
        signal,
        redirects: { allowedHosts: [new URL(endpointUrl).hostname], maxRedirects: 0 },
      },
    );
    const next: CodexResult = {};
    visitCodex(JSON.parse(response), next);
    result = { ...result, ...next };
  }
...
function selected(cfg: Config, mode: "Image" | "Video"): { provider: ProviderConfig; model: string } {
  const tag = cfg[`current${mode}Tag`];
  const model = cfg[`current${mode}Model`];
...
function imageResults(payload: any): MediaResult[] {
  ...
  const result = rows.flatMap((item: any) => {
    ...
    if (typeof encoded === "string" && encoded)
      return [{ data: Buffer.from(encoded, "base64"), mimeType: item.mime_type ?? item.mimeType ?? "image/png" }];
    if (typeof item?.url === "string" && /^https?:\/\//i.test(item.url))
      return [{ url: item.url, mimeType: item.mime_type ?? item.mimeType ?? "image/png" }];
    return [];
  });
...
async function sharpFirstFrame(buffer: Buffer): Promise<Buffer | undefined> {
  ...
  try {
    return await sharp(buffer, { pages: 1, limitInputPixels: MAX_INPUT_PIXELS }).png().toBuffer();
  } catch {
    return undefined;
  }
}
...
export async function messageMedia(ctx: PluginContext, message?: MessageEnvelope): Promise<MediaInput | undefined> {
  ...
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    const result = await ctx.telegram.withClient(async (client, active) => {
      ...
      const value = await client.downloadMedia(rawMessage, {
        outputFile: output,
        progressCallback: (downloaded: unknown) => checkDownloadProgress(downloaded, combined),
      });
      combined.throwIfAborted();
      return value;
    });
...
function isAnimatedDocument(doc: any): boolean {
  ...
  return (
    ...
    mime === "application/x-tg-sticker" ||
    (doc?.attributes ?? []).some((attr: any) => attr?.className === "DocumentAttributeAnimated")
  );
}
...
async function fetchMedia(
  ...
  return ctx.files.withTemp(async (directory, tempSignal) => {
    ...
    const value = await ctx.telegram.withClient(async (client, active) => {
      const combined = AbortSignal.any([signal, tempSignal, active]);
      combined.throwIfAborted();
      const downloaded = await client.downloadMedia(raw, {
        outputFile: output,
        ...options,
...
async function imagePartFromMessage(
  ctx: PluginContext,
  raw: any,
  signal: AbortSignal,
...
export async function collectMessageImages(
  ...
  for (const candidate of raws) {
    ...
    if (!part) continue;
    if (total + part.data.byteLength > MAX_INPUT_BYTES || images.length >= 4) {
      dropped = true;
      continue;
    }
...
export function mergeMessageImages(sets: readonly CollectedMedia[]): CollectedMedia {
  ...
  for (const set of sets) {
    for (const image of set.images) {
      if (images.length >= 4 || total + image.data.byteLength > MAX_INPUT_BYTES) {
        dropped = true;
        continue;
      }
  ...
  if (type === "gemini" || (type === "local-cliproxy" && model.toLowerCase().includes("gemini"))) {
    ...
    if (model.toLowerCase().includes("imagen") && !input) {
      const payload = await json(
        ...
        { instances: [{ prompt }], parameters: { sampleCount: 1, outputOptions: { mimeType: "image/png" } } },
        signal,
        cfg.timeout,
      );
      const result = (payload.predictions ?? []).flatMap((item: any) =>
    ...
    const result = (root.candidates ?? [])
      .flatMap((candidate: any) => candidate?.content?.parts ?? [])
      .flatMap((part: any) => {
        const inline = part?.inlineData ?? part?.inline_data;
        return inline?.data
          ? [{ data: Buffer.from(inline.data, "base64"), mimeType: inline.mimeType ?? inline.mime_type ?? "image/png" }]
          : [];
      });
    if (!result.length) throw new ProviderError("EMPTY_OUTPUT");
    return result;
...
async function poll<T>(
  fetchJob: (signal: AbortSignal) => Promise<any>,
...
async function generateGeminiVideo(
  ...
  for (const input of inputs.slice(0, 4))
    parts.push({ inlineData: { data: input.data.toString("base64"), mimeType: input.mimeType } });
  const response = await json(
    ...
    {
      contents: [{ parts }],
      videoGenerationConfig: { numberOfVideos: 1, durationSeconds: cfg.videoDuration, enableAudio: cfg.videoAudio },
    },
    signal,
    cfg.timeout,
  );
  ...
  const operation = await poll<any>(
    ...
    data =>
      data?.done === true
        ? data?.error
          ? { status: "failed", error: geminiOperationError(data) }
...
async function generateDoubaoVideo(
  ...
  const response = await json(
    ...
    { model, content: doubaoContent(prompt, inputs, mode), generateAudio: cfg.videoAudio, duration: cfg.videoDuration },
    signal,
    cfg.timeout,
  );
...
export async function generateVideos(
  ...
  requireInput(
    type === "openai" || type === "openai-compatible" || type === "local-cliproxy",
    "当前提供商暂不支持视频生成",
  );
  const content: any[] = [];
  if (prompt.trim()) content.push({ type: "text", text: prompt.trim() });
  ...
  const response = await raw(
    ctx,
    provider,
    endpoint(normalizeOpenAIBaseUrl(provider.url), "chat/completions"),
    { model, messages: [{ role: "user", content: userContent }], stream: provider.stream },
    signal,
    cfg.timeout,
  );
    ...
    async (response, active) => {
      ...
      try {
        while (true) {
          ...
          if (total > 64 * 1024 * 1024) throw new ProviderError("RESPONSE_TOO_LARGE");
...
export async function sendMedia(
  ...
  await ctx.telegram.withClient(async (client, signal) => {
    ...
    for (const [index, source] of items.slice(0, 4).entries()) {
      ...
      const extension = item.mimeType.includes("webm")
        ? "webm"
      ...
      await client.sendFile(peer, {
        file: new CustomFile(`ai_${kind}_${Date.now()}_${index}.${extension}`, item.data.length, "", item.data),
        forceDocument: !preview,
        caption,
        parseMode: "html",
        ...(replyTo ? { replyTo } : {}),
      });
    ...
    try {
      ...
      ctx.log.error("ai:command-delete-failed");
    }
  });
}
