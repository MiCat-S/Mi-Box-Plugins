...
export type ChatProbe =
  | { ok: true; text: string; model: string; elapsedMs: number; usage?: Usage; rateLimits?: Record<string, string> }
...
export type DiagnosticResult = {
  ...
  chat?: ChatProbe;
  models?: { ok: true; names: string[] } | { ok: false; error: DiagnosticError };
  benchmarks?: readonly ({ model: string } & ChatProbe)[];
};
export type DiagnosticsInput =
  { action: "full"; tag: string } | { action: "benchmark"; tag: string; models?: readonly string[] };
...
const limits = (headers: Headers): Record<string, string> | undefined => {
  ...
  for (const name of HEADER_NAMES) {
    ...
    if (value !== null) result[name] = value.slice(0, 256);
  }
...
const classify = (code: unknown, status?: number): DiagnosticError =>
  status === 401
    ? "invalid"
    : status === 403
      ? "forbidden"
      : status === 429
        ...
        : code === "TIMEOUT"
          ? "timeout"
          : code === "INVALID_RESPONSE" || code === "INVALID_JSON"
            ? "invalid_response"
            : "unavailable";
function configured(config: ChatConfigSnapshot, tag: string): ProviderConfig {
  const provider = config.configs[tag];
  if (!tag || !provider) throw new Error("invalid tag");
  return provider;
}
function identity(tag: string, provider: ProviderConfig) {
  const type = resolveProviderType(provider),
    host = (() => {
      try {
        return new URL(provider.url).hostname.toLowerCase();
      } catch {
        return "";
      }
    })();
  const detected: [[string, string], ...[string, string][]] = [
...
async function request(ctx: PluginContext, signal: AbortSignal, url: string, init: RequestInit, timeoutMs: number) {
  ...
  try {
    return await ctx.http.withResponse(
      url,
      init,
...
function auth(provider: ProviderConfig, type: string): Record<string, string> {
  if (type === "anthropic")
    return { "x-api-key": provider.key, "anthropic-version": "2023-06-01", "content-type": "application/json" };
...
async function chat(
  ...
  const selected = {
    ...config,
    configs: { ...config.configs, [tag]: { ...provider, stream: false } },
    currentChatTag: tag,
    currentChatModel: model,
    currentChatReasoningEffort: "auto" as const,
    currentChatServiceTier: "auto" as const,
  };
  const built = buildChatRequest(selected, "ok", "", { maxOutputTokens: max, temperature: 0 }),
    started = Date.now();
  try {
    return await ctx.http.withResponse(
      built.url,
      ...
      async (response, signal) => {
        ...
        try {
          const raw = await readBody(response, signal, 2 * 1024 * 1024),
          ...
          return {
            ok: true,
            text: parseChatText(raw, built.format, { maxResponseBytes: 2 * 1024 * 1024, maxOutputChars: 1024 * 1024 }),
            model: typeof payload.model === "string" ? payload.model : model,
            elapsedMs: Date.now() - started,
            ...(total ? { usage: { prompt, completion, total } } : {}),
            ...(rateLimits ? { rateLimits } : {}),
          };
        } catch (error) {
          signal.throwIfAborted();
          return { ok: false, error: classify(object(error).code), elapsedMs: Date.now() - started } as ChatProbe;
        }
      },
      { signal, timeoutMs: built.timeoutMs },
    );
  } catch (error) {
    signal.throwIfAborted();
    return { ok: false, error: classify(object(error).code), elapsedMs: Date.now() - started };
  }
}
async function safeChat(
  ...
  try {
    return await chat(ctx, signal, config, tag, model);
  } catch (error) {
    signal.throwIfAborted();
    return { ok: false, error: classify(object(error).code), elapsedMs: 0 };
  }
}
type Balance = DiagnosticResult["balance"];
const responseStatus = (value: object): number | undefined =>
  "status" in value ? (value as { status?: number }).status : undefined;
const responseData = (value: object): Record<string, unknown> =>
  object("data" in value ? (value as { data?: unknown }).data : undefined);
...
async function openAiBalance(ctx: PluginContext, signal: AbortSignal, provider: ProviderConfig): Promise<Balance> {
  ...
  const headers = auth(provider, "openai"),
  ...
  const [subscription, usage, modelData] = await Promise.all([
    ...
    request(
      ctx,
      signal,
      new URL(`dashboard/billing/usage?start_date=${now - 90 * 86400}&end_date=${now}`, root).toString(),
      { headers },
      10_000,
    ),
  ...
  const data = responseData(subscription),
    plan = object(data.plan);
  ...
  if (data.access_until !== undefined)
    fields.push({ label: "有效期", value: new Date(number(data.access_until) * 1000).toLocaleDateString("zh-CN") });
  if (data.has_payment_method !== undefined)
    fields.push({ label: "支付方式", value: data.has_payment_method ? "是" : "否" });
  if (usage.ok)
    fields.push({ label: "近 90 天", value: `$${(number(responseData(usage).total_usage) / 100).toFixed(4)}` });
  if (modelData.ok) {
    const value = responseData(modelData),
      rows = Array.isArray(value.data) ? value.data : [],
      owners = new Set<string>();
    let tier = "";
    for (const row of rows.slice(0, 50)) {
      const item = object(row);
      if (typeof item.owned_by === "string") owners.add(item.owned_by);
      if (typeof item.max_tier === "string") tier = item.max_tier;
    }
...
async function anthropicBalance(
  ctx: PluginContext,
  signal: AbortSignal,
  ...
  const response = await request(
    ...
    {
      method: "POST",
      headers: auth(provider, "anthropic"),
  ...
  if (response.ok || status === 429)
    return {
      status: "ok",
      fields: [
        { label: "Key", value: `有效 (${status === 429 ? "限流" : "正常"})` },
        { label: "余额", value: "请前往官网查看" },
      ],
    };
...
async function balance(
  ...
  const base = normalizeOpenAIBaseUrl(provider.url),
    origin = (() => {
      try {
  ...
  const url =
    branch === "openrouter"
      ? `${origin}/api/v1/auth/key`
      : branch === "deepseek"
        ? `${origin}/user/balance`
        : branch === "gemini"
          ? `${origin}/v1beta/models?key=${encodeURIComponent(provider.key)}`
          : `${base.replace(/\/$/, "")}/models`;
  ...
  const data = responseData(response),
    root = object(data.data ?? data),
    fields: { label: string; value: string }[] = [];
  if (kind === "openrouter") {
    fields.push(
      { label: "标签", value: String(root.label ?? root.name ?? "?") },
      { label: "余额", value: String(root.credits ?? "?") },
      { label: "已用", value: String(root.usage ?? "?") },
      { label: "限额", value: String(root.limit ?? "?") },
    );
    const rate = object(root.rate_limit);
    if (Object.keys(rate).length)
      fields.push({ label: "速率", value: `${String(rate.requests ?? "?")} req / ${String(rate.interval ?? "?")}` });
    if (Array.isArray(root.disabled_providers) && root.disabled_providers.length)
      fields.push({ label: "禁用", value: String(root.disabled_providers.length) });
  } else if (kind === "deepseek") {
    fields.push({ label: "可用", value: data.is_available ? "是" : "否" });
    for (const row of Array.isArray(data.balance_infos) ? data.balance_infos : []) {
      const item = object(row);
      fields.push({ label: String(item.currency ?? "余额"), value: String(item.total_balance ?? "?") });
    }
  } else {
...
export async function diagnostics(
  ...
  const provider = configured(config, input.tag),
    providerInfo = identity(input.tag, provider);
  if (input.action === "benchmark") {
    let models = input.models ?? benchmarkModels[providerInfo.type];
    if (!models) {
      const configuredModel =
        provider.models?.chat || (config.currentChatTag === input.tag ? config.currentChatModel : "");
      models = configuredModel
        ? [configuredModel]
        : (await listProviderModels(config, ctx.http, input.tag, signal)).slice(0, 1);
    }
  ...
  const modelsTask = listProviderModels(config, ctx.http, input.tag, signal).then(
    ...
    error => {
      signal.throwIfAborted();
      return { ok: false as const, error: classify(object(error).code) };
    },
  );
  ...
  const model =
    ...
    (config.currentChatTag === input.tag ? config.currentChatModel : "") ||
    (models.ok ? (models.names[0] ?? "") : "");
  const result = {
    provider: providerInfo,
    balance: account,
    models,
    ...(model ? { chat: await safeChat(ctx, signal, config, input.tag, model) } : {}),
  };
  signal.throwIfAborted();
  return result;
}
