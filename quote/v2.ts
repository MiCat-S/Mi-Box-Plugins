...
import {STRUCTURED_PLUGIN_API_VERSION, definePlugin, renderCommandHelp, requireSdkFeatures,
  type CommandDefinition, type CommandInvocation, type MessageEnvelope, type PluginContext} from "telebox/sdk";
...
const isColor = (value: string) => value.toLowerCase() === "random" ||
  /^#(?:[\da-f]{3}|[\da-f]{6})(\/#(?:[\da-f]{3}|[\da-f]{6}))?$/i.test(value) ||
  /^\/\/#?(?:[\da-f]{3}|[\da-f]{6})$/i.test(value) ||
  /^(?:[\da-f]{3}|[\da-f]{6})(\/(?:[\da-f]{3}|[\da-f]{6}))?$/i.test(value);
function color(value: string): string {
  if (value.toLowerCase() === "random") return `#${Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, "0")}`;
  if (value.startsWith("#") || value.startsWith("//")) return value;
  return /^[\da-f]{3,6}(\/[\da-f]{3,6})?$/i.test(value)
    ? value.split("/").map(part => `#${part}`).join("/") : value;
}
export function parseOptions(args: readonly string[]): QuoteOptions {
  const result: QuoteOptions = {count: 1, replies: false, media: false, imagePreview: false, crop: false, hidden: false,
    scale: 2, format: "webp", background: "#231d2b/#372e44", emojiBrand: "apple", explicitCount: false};
  for (let index = 0; index < args.length; index++) {
    const raw = args[index]!, token = raw.toLowerCase();
    if (["r", "reply"].includes(token)) {result.replies = true; continue;}
    if (["png", "image", "img"].includes(token)) {result.imagePreview = true; if (result.format !== "story") result.format = "png"; continue;}
    if (["story", "stories"].includes(token)) {result.format = "story"; continue;}
    if (["webp", "quote"].includes(token)) {result.format = "webp"; continue;}
    if (["hidden", "hide", "anonymous"].includes(token)) {result.hidden = true; continue;}
    if (["media", "m"].includes(token)) {result.media = true; continue;}
    if (token === "crop") {result.crop = true; continue;}
    ...
    if (["scale", "s"].includes(token)) {
      ...
      if (Number.isFinite(next) && next > 0) {result.scale = Math.min(20, Math.max(1, next)); index++;}
      continue;
    }
    ...
    if (isColor(raw)) {result.background = color(raw); continue;}
    if (BRANDS.has(token)) {result.emojiBrand = token; continue;}
    const brand = token.match(/^(?:emoji|brand)[=:]([a-z]+)$/);
    if (brand && BRANDS.has(brand[1]!)) {result.emojiBrand = brand[1]!; continue;}
    if (/^[+-]?\d+$/.test(token)) {
...
export async function collectMessages(ctx: PluginContext, invocation: CommandInvocation, options: QuoteOptions) {
  ...
  try {reply = await ctx.telegram.getReply(invocation.message);}
  ...
  const query = reply
    ? options.count > 0 ? {offsetId: base.id - 1, limit, reverse: true} : {offsetId: base.id + 1, limit}
  ...
  try {
    const messages = await ctx.telegram.withClient(async (client, signal) => {
      ...
      signal.throwIfAborted();
      return (result ?? []).filter((value: any) => value && Number.isSafeInteger(value.id) &&
        (!value.className || value.className === "Message")).sort((a: any, b: any) => a.id - b.id);
    });
    return {sources: messages.length ? messages.slice(0, limit).map(raw => ({raw})) : [source], replyTo};
  } catch {ctx.signal.throwIfAborted(); return {sources: [source], replyTo};}
}
async function progress(ctx: PluginContext, message: MessageEnvelope, text: string) {
  try {await ctx.telegram.edit(message, text);}
  catch {ctx.signal.throwIfAborted(); ctx.log.error("quote_progress_failed");}
}
export default function createQuote() {
  const ensureAssets = createAssetLoader();
  let assetRoot: string | undefined;
  const command: CommandDefinition = {
    description: "按原版 glass 样式将消息渲染为语录", args: "[数量] [image|stories] [选项]",
    helpArgs: ["help", "h", "?", "帮助"],
    arguments: [
      {name: "数量", description: "最多 50 条；回复时正数向后、负数向前。无回复时正数取命令之前的消息。"},
      {name: "选项", description: "r/reply、hidden、media、crop、scale=1..20、bg=颜色、apple/google/twitter/joypixels/blob。"},
    ],
    examples: [{args: ""}, {args: "3 image bg=#aaa/#bbb"}, {args: "stories"}, {args: "r google"}],
    help: [{heading: "原版格式与资源：", body: "默认 WebP 贴纸、scale=2；image/png 为背景大图，stories 为 720×1280 PNG。首次使用从原固定来源下载字体、背景和表情素材到插件数据目录；之后复用。"},
      {heading: "资源边界：", body: "消息最多 50 条；媒体单项 12 MiB、合计 32 MiB。单画布最多 2000 万像素；高缩放或长内容超过预算会明确失败。代码和依赖由宿主提供。"}],
    async handle(invocation, ctx) {
      if (wantsHelp(invocation.args)) {await ctx.telegram.edit(invocation.message, help(invocation.prefix), {parseMode: "html"}); return;}
      let sent = false;
      try {
        ...
        if (!result.length || result.length > (webp ? 512 * 1024 : 20 * 1024 * 1024)) throw new Error("QUOTE_OUTPUT_BUDGET");
        ...
        await ctx.files.withTemp(async (directory, signal) => {
          ...
          await ctx.telegram.withClient(async (client, active) => {
            ...
            await client.sendFile(peer, {file, forceDocument: false, replyTo: collected.replyTo,
              topMsgId: invocation.message.topicId, ...(webp ? {attributes: [new Api.DocumentAttributeSticker({alt: "💜", stickerset: new Api.InputStickerSetEmpty()})]} : {})});
            sent = true;
            combined.throwIfAborted();
            try {
              ...
              combined.throwIfAborted();
            } catch {combined.throwIfAborted(); ctx.log.error("quote_command_cleanup_failed");}
          });
        });
      } catch {
        if (ctx.signal.aborted) return;
        if (sent) {ctx.log.error("quote_temp_cleanup_failed"); return;}
  ...
  return definePlugin({apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "quote", description: "本地 glass 语录渲染",
    cleanup() {if (assetRoot) clearResources(assetRoot);},
    renderHelp: help, commands: {q: command, quote: command}});
}
