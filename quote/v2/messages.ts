...
function decimalId(value: any): string {
  ...
  try {
    return returnBigInt(String(raw)).toString();
  } catch {
    return "0";
  }
}
...
function displayName(entity: any, fallback?: unknown): string {
  return (
    ...
    String(fallback ?? `ID ${decimalId(entity)}`)
  );
}

const documentOf = (raw: any) => raw?.document ?? raw?.media?.document;
const attributes = (raw: any): any[] => documentOf(raw)?.attributes ?? [];
const attribute = (raw: any, name: string) =>
  attributes(raw).find(value => String(value?.className ?? value?.constructor?.name ?? "").includes(name));
...
async function staticThumbnail(raw: any, signal: AbortSignal): Promise<any | undefined> {
  ...
  const location = new Api.InputDocumentFileLocation({
    id: returnBigInt(decimalId(document.id)),
    accessHash: returnBigInt(decimalId(document.accessHash)),
    fileReference: document.fileReference ?? document.file_reference ?? Buffer.alloc(0),
...
async function rasterPng(
  ...
  signal.throwIfAborted();
  const result = await sharp(buffer, { animated: false, pages: 1, limitInputPixels: MAX_PIXELS })
    .rotate()
    .resize({ width, height, fit: crop ? "cover" : "inside", withoutEnlargement: !crop })
    .png()
    .toBuffer();
  signal.throwIfAborted();
  return result;
...
async function videoPng(context: PluginContext, buffer: Buffer, signal: AbortSignal): Promise<Buffer> {
  return context.files.withTemp(async (directory, tempSignal) => {
    ...
    const args = [
      ...
      "1",
      "-vf",
      "scale=584:240:force_original_aspect_ratio=decrease",
      "-fs",
      String(MAX_ITEM_BYTES),
      output,
    ];
    let complete = false;
    for (const executable of FFMPEG) {
      try {
        await context.processes.run(executable, args, {
          cwd: directory,
          env: {},
          signal: active,
          timeoutMs: 15000,
...
async function senderEntity(client: any, signal: AbortSignal, raw: any): Promise<any> {
  ...
  if (typeof raw?.getSender === "function")
    try {
  ...
  if (raw?.senderId !== undefined && typeof client.getEntity === "function")
    try {
...
async function forwardSource(client: any, signal: AbortSignal, raw: any): Promise<any> {
  ...
  let entity;
  if (peer && typeof client.getEntity === "function")
    try {
...
function emojiStatus(entity: any): Record<string, any> | undefined {
  const value =
    entity?.emojiStatus?.documentId ??
    entity?.emoji_status?.document_id ??
    ...
    const result: any = await client.invoke(
      new Api.channels.GetParticipant({
        ...
        participant: new Api.InputUser({
          userId: returnBigInt(decimalId(entity)),
          accessHash: returnBigInt(decimalId(entity.accessHash)),
        }),
      }),
    );
    signal.throwIfAborted();
...
export async function toQuoteMessages(
  context: PluginContext,
  sources: Source[],
  options: Options,
  ...
  return context.telegram.withClient(async (client: any, signal) => {
    const convert = async (source: Source, includeReply: boolean): Promise<Record<string, any>> => {
      ...
      if (
        (isVisual(kind) ||
          ((options.media || options.imagePreview || options.format === "png") &&
            kind === "document" &&
        ...
        if (mediaBuffer)
          try {
            let png =
              isVideo(mediaBuffer) ||
              ((kind === "video" || kind === "round" || kind === "animation") && !isRaster(mediaBuffer))
                ? await videoPng(context, mediaBuffer, signal)
                : isRaster(mediaBuffer)
                  ? await mediaPng(mediaBuffer, signal)
                  : undefined;
            if (!png) {
              const thumbnail = await staticThumbnail(raw, signal);
              const thumbnailBuffer = thumbnail
                ? await download(client, signal, thumbnail, budget, 2 * 1024 * 1024)
                : undefined;
      ...
      const voice =
        kind === "voice" && wave.length
          ? { waveform: wave, duration: Number(audioAttribute?.duration) || undefined }
          : undefined;
      const attachment =
        kind === "document"
          ? {
              file_name: String(
                attribute(raw, "Filename")?.fileName ?? attribute(raw, "Filename")?.file_name ?? "file",
              ),
              file_size: Number(document?.size) || undefined,
            }
          : undefined;
      const audio =
        kind === "audio"
          ? {
              title: String(audioAttribute?.title ?? "Audio"),
              performer: audioAttribute?.performer ? String(audioAttribute.performer) : undefined,
              duration: Number(audioAttribute?.duration) || undefined,
            }
          : undefined;
      ...
      return {
        chatId: senderId,
        message_id: raw?.id ?? source.envelope?.id ?? 0,
        from: {
          id: senderId,
          name: options.hidden ? false : senderName,
          first_name: options.hidden ? false : senderName,
          photo: {},
          emoji_status: status,
        },
        name: options.hidden ? false : senderName,
        avatar: !options.hidden && !!avatar,
        avatarBuffer: avatar,
        avatarScale: options.scale,
        text,
        entities,
        caption: text,
        caption_entities: entities,
        replyMessage,
        forward: forwarded ? { label: forwarded.name } : undefined,
        mediaBuffer,
        mediaCanvas,
