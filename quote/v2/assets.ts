...
const FILES = [
  ...
  [
    ...
    2092162,
    "https://raw.githubusercontent.com/LyoSU/quote-api/master/assets/emoji/emoji-blob-image.json",
  ],
  [
...
async function valid(file: string) {
  try {
    const info = await lstat(file);
    return info.isFile() && info.size > 0 && info.size <= LIMIT;
  } catch (error) {
    if (error instanceof Error && "code" in error && error.code === "ENOENT") return false;
    throw error;
  }
}
async function fetchFile(context: PluginContext, relative: string, _referenceSize: number, url: string) {
  const target = await context.files.dataFile(relative);
  ...
  try {
    await context.http.withResponse(
      ...
      async (response, signal) => {
        if (!response.ok || !response.body) throw new Error("quote asset download failed");
        const header = response.headers.get("content-length");
        let announced: number | undefined;
        if (header !== null) {
          announced = Number(header);
          if (!Number.isSafeInteger(announced) || announced <= 0 || announced > LIMIT)
            throw new Error("quote asset size mismatch");
        }
        ...
        try {
          for (;;) {
            ...
            if (total > LIMIT) throw new Error("quote asset too large");
            let offset = 0;
            while (offset < part.value.byteLength) {
              const written = await handle.write(part.value, offset, part.value.byteLength - offset, null);
              signal.throwIfAborted();
              if (written.bytesWritten <= 0) throw new Error("quote asset short write");
              offset += written.bytesWritten;
            }
          }
          if (!total || (announced !== undefined && total !== announced)) throw new Error("quote asset short download");
        } finally {
      ...
      },
      {
        ...
        redirects: { allowedHosts: ["raw.githubusercontent.com"], maxRedirects: 0 },
      },
    );
...
export function createAssetLoader() {
  ...
  return (context: PluginContext): Promise<string> => {
    if (pending) return pending;
    const operation = (async () => {
      for (const [name, size, url] of FILES) {
        if (!url.startsWith(`${ROOT}/`)) throw new Error("quote asset host rejected");
        context.signal.throwIfAborted();
        await fetchFile(context, name, size, url);
      }
      return context.files.dataPath();
    })();
    pending = operation;
    void operation
      .finally(() => {
