...
const help=(p:string)=>`<b>抽奖</b>\n<code>${p}lottery create 标题 关键词 人数 中奖数 仓库</code>\n<code>${p}lottery status|list|draw|delete|winners</code>\n<code>${p}lottery prize create|add|list|clear</code>`;

const lotteryPlugin=definePlugin({renderHelp: renderPluginHelp, apiVersion:1,id:"lottery",description:"持久化群组抽奖与奖品仓库",commands:{lottery:{helpArgs: ["help"], helpOnEmpty: true, description:"创建和管理抽奖",ignoreEdited:true,async handle({message,args,prefix},ctx){try{const sub=args[0]?.toLowerCase();if(!sub||sub==="help"){await ctx.telegram.edit(message,help(prefix),{parseMode:"html"});return;}const state=await store(ctx).read(),active=current(state,message.chatId);
  if(sub==="prize"){if(!message.saved&&!(message.raw as any)?.isPrivate&&message.chatId!==message.senderId)throw new Error("奖品仓库管理只能在私聊或收藏夹中进行");const op=args[1]?.toLowerCase(),name=args[2]??"default";if(op==="create"){await store(ctx).update(v=>({...v,warehouses:{...v.warehouses,[name]:v.warehouses[name]??[]}}));await ctx.telegram.edit(message,`奖品仓库已就绪：<code>${esc(name)}</code>`,{parseMode:"html"});return;}if(op==="add"){const stock=Number(args.at(-1)),text=args.slice(3,-1).join(" ").replace(/^"|"$/g,"");if(!text||!Number.isSafeInteger(stock)||stock<1)throw new Error("用法：lottery prize add 仓库 奖品 数量");await store(ctx).update(v=>({...v,warehouses:{...v.warehouses,[name]:[...(v.warehouses[name]??[]),{text,stock,order:(v.warehouses[name]?.length??0)}]}}));await ctx.telegram.edit(message,"奖品已添加。");return;}if(op==="list"){const list=state.warehouses[name]??[];await ctx.telegram.edit(message,list.length?list.map((x,i)=>`${i+1}. ${esc(x.text)}（${x.stock}）`).join("\n"):"仓库为空",{parseMode:"html"});return;}if(op==="clear"){await store(ctx).update(v=>({...v,warehouses:args[2]==="all"?{}:{...v.warehouses,[name]:[]}}));await ctx.telegram.edit(message,"奖品仓库已清空。");return;}throw new Error("未知奖品命令");}
  if(sub==="create"){if(args[1]==="list"){const rows=Object.entries(state.warehouses).sort(([a],[b])=>a<b?-1:a>b?1:0);await ctx.telegram.edit(message,rows.length?rows.map(([name,items],i)=>`${i+1}. <code>${esc(name)}</code>（库存 ${items.reduce((n,x)=>n+x.stock,0)}）`).join("\n"):"没有可用奖品仓库",{parseMode:"html"});return;}if(active)throw new Error("当前聊天已有进行中的抽奖");const title=args[1],keyword=args[2],max=Number(args[3]),count=Number(args[4]),warehouse=getWarehouseByNameOrIndex(args[5]??"default",Object.keys(state.warehouses).sort());if(!warehouse)throw new Error("奖品仓库不存在，请使用仓库名称或序号");if(!title||!keyword||!Number.isSafeInteger(max)||!Number.isSafeInteger(count)||count<1||max<count||max<state.settings.minUsers||max>state.settings.maxUsers)throw new Error("创建参数无效");if(!(state.warehouses[warehouse]??[]).some(x=>x.stock>0))throw new Error("奖品仓库不存在或库存为空");const id=randomUUID(),a:Activity={id,chatId:message.chatId,title,keyword,maxParticipants:max,winnerCount:count,warehouse,creatorId:String(message.senderId??""),createdAt:Date.now(),status:"active",deleteDelay:5,claimTimeout:86400,requireAvatar:false,requireUsername:false,allowBots:false,participants:[],winners:[]};const sent=await send(ctx,(message.raw as any)?.peerId??returnBigInt(message.chatId),`🎉 <b>抽奖活动已创建</b>\n活动：${esc(title)}\n关键词：<code>${esc(keyword)}</code>\n名额：${count}/${max}`,message.id);a.messageId=sent.id;await store(ctx).update(v=>({...v,activities:{...v.activities,[id]:a}}));await ctx.telegram.withClient(async client=>client.pinMessage((message.raw as any)?.peerId??returnBigInt(message.chatId),sent.id,{notify:args.includes("notify")})).catch(()=>{});await ctx.telegram.edit(message,`抽奖已创建：<code>${esc(id)}</code>`,{parseMode:"html"});return;}
  if(sub==="winners"||sub==="claim"||sub==="expire"){const last=latest(state,message.chatId);if(!last)throw new Error("当前聊天没有抽奖记录");if(sub==="winners"){await ctx.telegram.edit(message,last.winners.length?last.winners.map(w=>`${display(w)} (${w.status})`).join("\n"):"暂无中奖记录",{parseMode:"html"});return;}if(last.creatorId&&last.creatorId!==message.senderId)throw new Error("只有创建者可以管理领奖状态");if(sub==="expire"){let changed=0;await store(ctx).update(v=>{const a=v.activities[last.id],now=Date.now(),winners=a.winners.map(w=>w.status==="prepared"&&w.expiresAt<=now?(changed++,{...w,status:"expired" as const}):w);return{...v,activities:{...v.activities,[last.id]:{...a,winners}}};});await ctx.telegram.edit(message,`已处理 ${changed} 个过期奖品。`);return;}const target=(args[1]??"").replace(/^@/,"");if(!target)throw new Error("请提供中奖用户 ID 或用户名");let changed=false;await store(ctx).update(v=>{const a=v.activities[last.id],winners=a.winners.map(w=>(w.userId===target||w.username===target)?(changed=true,{...w,status:"claimed" as const}):w);return{...v,activities:{...v.activities,[last.id]:{...a,winners}}};});if(!changed)throw new Error("未找到中奖用户");await ctx.telegram.edit(message,"已标记为已领奖。");return;}
  if(!active)throw new Error("当前聊天没有进行中的抽奖");if(sub==="status"){await ctx.telegram.edit(message,`<b>${esc(active.title)}</b>\n关键词：<code>${esc(active.keyword)}</code>\n进度：${active.participants.length}/${active.maxParticipants}`,{parseMode:"html"});return;}if(sub==="list"){await ctx.telegram.edit(message,active.participants.length?active.participants.map((p,i)=>`${i+1}. ${display(p)}`).join("\n"):"暂无参与用户",{parseMode:"html"});return;}if(sub==="winners"){await ctx.telegram.edit(message,active.winners.length?active.winners.map(w=>`${display(w)} (${w.status})`).join("\n"):"暂无中奖记录",{parseMode:"html"});return;}if(sub==="draw"){if(active.creatorId!==message.senderId&&!await ctx.telegram.withClient(client=>isUserAdmin(client,message.chatId,String(message.senderId))))throw new Error("只有抽奖创建者或群组管理员可以手动开奖");await ctx.telegram.edit(message,"正在开奖…");await draw(ctx,active.id);return;}if(sub==="delete"||sub==="cancel"){if(active.creatorId!==message.senderId&&!await ctx.telegram.withClient(client=>isUserAdmin(client,message.chatId,String(message.senderId))))throw new Error("只有抽奖创建者或群组管理员可以删除活动");await store(ctx).update(v=>({...v,activities:{...v.activities,[active.id]:{...active,status:"cancelled"}}}));await ctx.telegram.edit(message,"抽奖已取消。");return;}throw new Error("未知子命令");}catch(e){if(!ctx.signal.aborted)await ctx.telegram.edit(message,`操作失败：<code>${esc(error(e))}</code>`,{parseMode:"html"});}}}},listeners:[{edited:false,ignoreCommands:true,async handle(message,ctx){if(message.outgoing||!message.senderId||!message.text)return;const state=await store(ctx).read(),a=current(state,message.chatId);if(!a||message.text.trim()!==a.keyword)return;const sender:any=await eligible(ctx,message,a);if(!sender)return;let joined=false,count=0;await store(ctx).update(v=>{const live=v.activities[a.id];if(!live||live.status!=="active"||live.participants.some(p=>p.userId===message.senderId))return v;const p:Participant={userId:message.senderId!,...(sender.username?{username:String(sender.username)}:{}),...(sender.firstName?{firstName:String(sender.firstName)}:{}),...(sender.lastName?{lastName:String(sender.lastName)}:{}),joinedAt:Date.now()};const updated={...live,participants:[...live.participants,p]};joined=true;count=updated.participants.length;return{...v,activities:{...v.activities,[a.id]:updated}};});if(!joined)return;await ctx.telegram.reply(message,`参与成功：${count}/${a.maxParticipants}`);void deleteLater(ctx,message,[message.id],a.deleteDelay,`${a.id}:${message.senderId}`).catch(()=>{});if(count>=a.maxParticipants)await draw(ctx,a.id);}}],settings:ctx=>({id:"lottery",title:"抽奖",description:"抽奖人数与等待设置",category:"插件配置",icon:"🎰",getSchema:()=>[{key:"minUsers",label:"最少参与人数",type:"number",min:2,max:100},{key:"maxUsers",label:"最多参与人数",type:"number",min:2,max:1000},{key:"timeout",label:"等待时间（秒）",type:"number",min:10,max:600}],getValues:async()=>({...(await store(ctx).read()).settings}),setValues:async patch=>{await store(ctx).update(v=>({...v,settings:{minUsers:Number(patch.minUsers??v.settings.minUsers),maxUsers:Number(patch.maxUsers??v.settings.maxUsers),timeout:Number(patch.timeout??v.settings.timeout)}}));}}),async setup(ctx){await migrate(ctx);const state=await store(ctx).read();for(const a of Object.values(state.activities).filter(x=>x.status==="drawing"))await store(ctx).update(v=>({...v,activities:{...v.activities,[a.id]:{...a,status:"active"}}}));}});
