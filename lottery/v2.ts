...
const rows=Object.entries(state.warehouses).sort(([a],[b])=>a<b?-1:a>b?1:0);await editPages(ctx,message,rows.length?rows.map(([name,items],i)=>`${i+1}. <code>${esc(name)}</code>（库存 ${items.reduce((n,x)=>n+x.stock,0)}）`).join("\n"):"没有可用奖品仓库");return;
      ...
      handle: guarded(async ({message, args}, ctx) => {
        ...
        await store(ctx).update(state=>{if(ongoing(state,message.chatId))throw new Error("当前聊天已有进行中的抽奖");const warehouse=getWarehouseByNameOrIndex(args[4]??"default",Object.keys(state.warehouses).sort());if(!warehouse)throw new Error("奖品仓库不存在，请使用仓库名称或序号");if(!title||!keyword||!Number.isSafeInteger(max)||!Number.isSafeInteger(count)||count<1||max<count||max<state.settings.minUsers||max>state.settings.maxUsers)throw new Error("创建参数无效");if(!(state.warehouses[warehouse]??[]).some(x=>x.stock>0))throw new Error("奖品仓库不存在或库存为空");reserved={id,chatId:message.chatId,title,keyword,maxParticipants:max,winnerCount:count,warehouse,creatorId:String(message.senderId??""),createdAt:Date.now(),status:"active",deleteDelay:5,claimTimeout:86400,requireAvatar:false,requireUsername:false,allowBots:false,participants:[],winners:[]};return{...state,activities:{...state.activities,[id]:reserved}};});
        try{const peer=(message.raw as any)?.peerId??returnBigInt(message.chatId);const sent=await send(ctx,peer,`🎉 <b>抽奖活动已创建</b>\n活动：${esc(title)}\n关键词：<code>${esc(keyword)}</code>\n名额：${count}/${max}`,message.id);let attached=false;await store(ctx).update(v=>{const live=v.activities[id];if(live?.status!=="active")return v;attached=true;return{...v,activities:{...v.activities,[id]:{...live,messageId:sent.id}}};});if(!attached){await ctx.telegram.withClient(client=>client.deleteMessages(peer,[sent.id],{revoke:true})).catch(()=>{});throw new Error("抽奖状态已变化，请重试");}await ctx.telegram.withClient(async client=>client.pinMessage(peer,sent.id,{notify:args.includes("notify")})).catch(()=>{});await ctx.telegram.edit(message,`抽奖已创建：<code>${esc(id)}</code>`,{parseMode:"html"});}
        catch(e){await store(ctx).update(v=>{const live=v.activities[id];if(!live||live.status!=="active"||live.messageId!==undefined)return v;const activities={...v.activities};delete activities[id];return{...v,activities};});throw e;}return;
      })},
...
if(active.creatorId!==message.senderId&&!await ctx.telegram.withClient(client=>isUserAdmin(client,message.chatId,String(message.senderId))))throw new Error("只有抽奖创建者或群组管理员可以删除活动");let cancelled=false;await store(ctx).update(v=>{const live=v.activities[active.id];if(!live||live.status!=="active")return v;cancelled=true;return{...v,activities:{...v.activities,[active.id]:{...live,status:"cancelled"}}};});if(!cancelled)throw new Error("抽奖状态已变化，请重试");await ctx.telegram.edit(message,"抽奖已取消。");return;
    })},
