...
import {STRUCTURED_PLUGIN_API_VERSION, renderCommandHelp, type CommandDefinition, type CommandInvocation, definePlugin, type MessageEnvelope, type PluginContext, ui} from "telebox/sdk";
import {returnBigInt} from "teleproto/Helpers";

type Participant={userId:string;username?:string;firstName?:string;lastName?:string;joinedAt:number};
type Winner=Participant&{prize?:string;status:"prepared"|"sent"|"claimed"|"expired";assignedAt:number;expiresAt:number;messageId?:number;deliveredPages?:number;totalPages?:number};
...
const defaults:State={schemaVersion:1,activities:{},warehouses:{},settings:{minUsers:2,maxUsers:1000},importedLegacy:false};
...
const ongoing=(state:State,chatId:string)=>Object.values(state.activities).some(x=>x.chatId===chatId&&(x.status==="active"||x.status==="drawing"));
...
async function send(ctx:PluginContext,peer:any,text:string,replyTo?:number){return ctx.telegram.withClient(async (client,signal)=>{signal.throwIfAborted();const sent=await client.sendMessage(peer,{message:text,parseMode:"html",...(replyTo?{replyTo}:{})});signal.throwIfAborted();return sent;});}
async function editPages(ctx:PluginContext,message:MessageEnvelope,text:string){const pages=(await ui.renderRichText(text,ui.PAGE_LABEL_RESERVE)).map((page,index,all)=>page+ui.pageLabel(index,all.length));const delivered=await ui.deliverPages(pages,ctx.signal,(page,index)=>index?ctx.telegram.reply(message,page,{parseMode:"html"}):ctx.telegram.edit(message,page,{parseMode:"html"}));if(delivered.interrupted){ctx.log.info("lottery:pages-interrupted",{published:delivered.published,total:delivered.total,category:ui.deliveryErrorCategory(delivered.error)});if(!delivered.published)throw new Error("page delivery failed");await ctx.telegram.reply(message,ui.interruptedNotice(delivered)).catch(()=>{});}}
async function sendPages(ctx:PluginContext,peer:any,text:string,replyTo?:number){const pages=(await ui.renderRichText(text,ui.PAGE_LABEL_RESERVE)).map((page,index,all)=>page+ui.pageLabel(index,all.length));return ctx.telegram.withClient(async (client,signal)=>{let first:any;const delivery=await ui.deliverPages(pages,signal,async(page,index)=>{const sent=await client.sendMessage(peer,{message:page,parseMode:"html",...(index===0&&replyTo?{replyTo}:{})});first??=sent;});if(delivery.interrupted){ctx.log.info("lottery:send-pages-interrupted",{published:delivery.published,total:delivery.total,category:ui.deliveryErrorCategory(delivery.error)});if(!delivery.published)throw new Error("page delivery failed");}return{first,delivery};});}
async function deleteLater(ctx:PluginContext,message:MessageEnvelope,ids:number[],seconds:number,key:string){await ctx.tasks.run(`lottery:delete:${key}`,async signal=>{await sleep(Math.max(0,seconds)*1000,undefined,{signal});await ctx.telegram.withClient(async (client,clientSignal)=>{const active=AbortSignal.any([signal,clientSignal]);active.throwIfAborted();await client.deleteMessages(returnBigInt(message.chatId),ids,{revoke:true});active.throwIfAborted();});});}
function display(p:Participant){return esc(p.username?`@${p.username}`:[p.firstName,p.lastName].filter(Boolean).join(" ")||p.userId);}
function deliveryLabel(winner:Winner){if(winner.status==="sent")return"已私聊发放";if((winner.deliveredPages??0)>0&&winner.deliveredPages!<(winner.totalPages??0))return`部分发送 ${winner.deliveredPages}/${winner.totalPages} 页，需补发`;return"待领取";}
async function eligible(ctx:PluginContext,message:MessageEnvelope,a:Activity){return ctx.telegram.withClient(async (client,signal)=>{const call=async<T>(operation:()=>Promise<T>)=>{signal.throwIfAborted();const value=await operation();signal.throwIfAborted();return value;};const raw=message.raw as any;const sender:any=raw?.sender??await call(()=>client.getEntity(returnBigInt(message.senderId!)));if(sender?.bot&&!a.allowBots)return; if(a.requireUsername&&!sender?.username)return;if(a.requireAvatar&&(!sender?.photo||sender.photo.className==="UserProfilePhotoEmpty"))return;if(a.requiredChannel){try{const {Api}=await import("teleproto");signal.throwIfAborted();const channel=await call(()=>client.getInputEntity(a.requiredChannel!)),participant=await call(()=>client.getInputEntity(returnBigInt(message.senderId!)));await call(()=>client.invoke(new Api.channels.GetParticipant({channel,participant})));}catch{signal.throwIfAborted();return;}}return sender;});}
    ...
    if (chatEntity.className === 'Chat') {
      try {
        ...
        return participants.some((participant: any) => {
          ...
          return String(participant?.userId ?? participant?.user_id ?? "") === userId &&
...
async function draw(ctx:PluginContext,activityId:string){let activity:Activity|undefined;
  ...
  try{const shuffled=[...activity.participants];for(let i=shuffled.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[shuffled[i],shuffled[j]]=[shuffled[j],shuffled[i]];}const selected=shuffled.slice(0,Math.min(activity.winnerCount,shuffled.length));let winners:Winner[]=[];
    ...
    for(const winner of winners){ctx.signal.throwIfAborted();try{const sent=await sendPages(ctx,returnBigInt(winner.userId),`🎉 <b>恭喜中奖</b>\n活动：${esc(activity.title)}\n奖品：${esc(winner.prize??"请联系发奖者")}`);ctx.signal.throwIfAborted();await store(ctx).update(state=>{const a={...state.activities[activityId],winners:state.activities[activityId].winners.map(w=>w.userId===winner.userId?{...w,status:sent.delivery.interrupted?"prepared" as const:"sent" as const,messageId:sent.first.id,deliveredPages:sent.delivery.published,totalPages:sent.delivery.total}:w)};return{...state,activities:{...state.activities,[activityId]:a}};});}catch(e){ctx.signal.throwIfAborted();ctx.log.error("lottery:notify",{userId:winner.userId});}}
    if(activity.messageId)try{await ctx.telegram.withClient(async (client,signal)=>{signal.throwIfAborted();await client.deleteMessages(returnBigInt(activity!.chatId),[activity!.messageId!],{revoke:true});signal.throwIfAborted();});}catch{ctx.signal.throwIfAborted();}
    const published=(await store(ctx).read()).activities[activityId].winners;
    await sendPages(ctx,returnBigInt(activity.chatId),`🎊 <b>开奖结果</b>\n\n🏆 ${esc(activity.title)}\n${published.length?published.map(w=>`• ${display(w)} — ${deliveryLabel(w)}`).join("\n"):"没有用户参与抽奖"}`);return true;
...
const guarded = (operation: CommandDefinition["handle"]): CommandDefinition["handle"] => async (i, ctx) => {
  ...
  catch(e) { if (!ctx.signal.aborted) await ctx.telegram.edit(i.message, `操作失败：<code>${esc(e instanceof BusinessError?e.message:"操作失败，请稍后重试")}</code>`, {parseMode: "html"}); }
};
...
const withPrize = (operation: (i: CommandInvocation, ctx: PluginContext, state: State, name: string) => Promise<void>): CommandDefinition["handle"] => withState(async (i, ctx, state) => {
  ...
  if (!message.saved && !(message.raw as any)?.isPrivate && message.chatId !== message.senderId) throw new BusinessError("奖品仓库管理只能在私聊或收藏夹中进行");
...
const withLatest = (manage: boolean, operation: (i: CommandInvocation, ctx: PluginContext, last: Activity) => Promise<void>): CommandDefinition["handle"] => withState(async (i, ctx, state) => {
  ...
  if (manage && last.creatorId && last.creatorId !== i.message.senderId) throw new BusinessError("只有创建者可以管理领奖状态");
  ...
  subcommands: {
    prize: {description: "管理奖品仓库，仅限私聊或收藏夹", subcommands: {
      ...
      add: {description: "添加奖品与库存", args: "仓库名 奖品内容 数量", examples: [{args: 'add myprizes "iPhone 15 Pro" 1'}, {args: 'add myprizes "现金红包100元" 5'}, {args: 'add vip "VIP会员1个月" 10'}, {args: 'add vip "VIP会员1年" 1'}], handle: withPrize(async ({message, args}, ctx, state, name) => {
        const stock=Number(args.at(-1)),text=args.slice(1,-1).join(" ").replace(/^"|"$/g,"");if(!text||!Number.isSafeInteger(stock)||stock<1)throw new BusinessError("用法：lottery prize add 仓库 奖品 数量");await store(ctx).update(v=>({...v,warehouses:{...v.warehouses,[name]:[...(v.warehouses[name]??[]),{text,stock,order:(v.warehouses[name]?.length??0)}]}}));await ctx.telegram.edit(message,"奖品已添加。");return;
      })},
      list: {description: "查看仓库中的奖品及库存", args: "[仓库名]", examples: [{args: "list vip"}], handle: withPrize(async ({message, args}, ctx, state, name) => {
        const list=state.warehouses[name]??[];await editPages(ctx,message,list.length?list.map((x,i)=>`${i+1}. ${esc(x.text)}（${x.stock}）`).join("\n"):"仓库为空");return;
      })},
      ...
      handle: withPrize(async () => { throw new BusinessError("未知奖品命令"); })},
...
const rows=Object.entries(state.warehouses).sort(([a],[b])=>a<b?-1:a>b?1:0);await editPages(ctx,message,rows.length?rows.map(([name,items],i)=>`${i+1}. <code>${esc(name)}</code>（库存 ${items.reduce((n,x)=>n+x.stock,0)}）`).join("\n"):"没有可用奖品仓库");return;
      })}},
      handle: guarded(async ({message, args}, ctx) => {
        const title=args[0],keyword=args[1],max=Number(args[2]),count=Number(args[3]),id=randomUUID();let reserved:Activity|undefined;
        await store(ctx).update(state=>{if(ongoing(state,message.chatId))throw new BusinessError("当前聊天已有进行中的抽奖");const warehouse=getWarehouseByNameOrIndex(args[4]??"default",Object.keys(state.warehouses).sort());if(!warehouse)throw new BusinessError("奖品仓库不存在，请使用仓库名称或序号");if(!title||!keyword||!Number.isSafeInteger(max)||!Number.isSafeInteger(count)||count<1||max<count||max<state.settings.minUsers||max>state.settings.maxUsers)throw new BusinessError("创建参数无效");if(!(state.warehouses[warehouse]??[]).some(x=>x.stock>0))throw new BusinessError("奖品仓库不存在或库存为空");reserved={id,chatId:message.chatId,title,keyword,maxParticipants:max,winnerCount:count,warehouse,creatorId:String(message.senderId??""),createdAt:Date.now(),status:"active",deleteDelay:5,claimTimeout:86400,requireAvatar:false,requireUsername:false,allowBots:false,participants:[],winners:[]};return{...state,activities:{...state.activities,[id]:reserved}};});
        let peer:any,sent:any;try{peer=(message.raw as any)?.peerId??returnBigInt(message.chatId);sent=await send(ctx,peer,`🎉 <b>抽奖活动已创建</b>\n活动：${esc(title)}\n关键词：<code>${esc(keyword)}</code>\n名额：${count}/${max}`,message.id);let attached=false;await store(ctx).update(v=>{const live=v.activities[id];if(live?.status!=="active")return v;attached=true;return{...v,activities:{...v.activities,[id]:{...live,messageId:sent.id}}};});if(!attached){await ctx.telegram.withClient(client=>client.deleteMessages(peer,[sent.id],{revoke:true})).catch(()=>{});throw new BusinessError("抽奖状态已变化，请重试");}}
        catch(e){await store(ctx).update(v=>{const live=v.activities[id];if(!live||live.status!=="active"||live.messageId!==undefined)return v;const activities={...v.activities};delete activities[id];return{...v,activities};});throw e;}
        try{await ctx.telegram.withClient(async client=>client.pinMessage(peer,sent.id,{notify:args.includes("notify")}));}catch{ctx.signal.throwIfAborted();ctx.log.info("lottery:create-pin-failed");}
        try{await ctx.telegram.edit(message,`抽奖已创建：<code>${esc(id)}</code>`,{parseMode:"html"});}catch{ctx.signal.throwIfAborted();ctx.log.info("lottery:create-receipt-failed");}return;
      })},
...
await editPages(ctx,message,active.participants.length?active.participants.map((p,i)=>`${i+1}. ${display(p)}`).join("\n"):"暂无参与用户");return;
    })},
    draw: {description: "手动开奖（创建者或群组管理员）", args: "", handle: withActive(async ({message}, ctx, active) => {
if(active.creatorId!==message.senderId&&!await ctx.telegram.withClient(client=>isUserAdmin(client,message.chatId,String(message.senderId))))throw new BusinessError("只有抽奖创建者或群组管理员可以手动开奖");await ctx.telegram.edit(message,"正在开奖…");await draw(ctx,active.id);return;
    })},
    delete: {aliases: ["cancel"], description: "取消活动（创建者或群组管理员）", args: "", handle: withActive(async ({message}, ctx, active) => {
if(active.creatorId!==message.senderId&&!await ctx.telegram.withClient(client=>isUserAdmin(client,message.chatId,String(message.senderId))))throw new BusinessError("只有抽奖创建者或群组管理员可以删除活动");let cancelled=false;await store(ctx).update(v=>{const live=v.activities[active.id];if(!live||live.status!=="active")return v;cancelled=true;return{...v,activities:{...v.activities,[active.id]:{...live,status:"cancelled"}}};});if(!cancelled)throw new BusinessError("抽奖状态已变化，请重试");await ctx.telegram.edit(message,"抽奖已取消。");return;
    })},
    winners: {description: "查看中奖名单和领奖状态", args: "", handle: withLatest(false, async ({message, args}, ctx, last) => {
await editPages(ctx,message,last.winners.length?last.winners.map(w=>`${display(w)} (${w.status})`).join("\n"):"暂无中奖记录");return;
    })},
