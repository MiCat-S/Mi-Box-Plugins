...
export type ThemeFormat = "attheme" | "tdesktop-theme" | "tgx-theme" | "ios-theme";

export const API_MIME: Record<ThemeFormat, string> = {
...
export const FORMAT_LABELS: Record<ThemeFormat, string> = {
...
export const FORMAT_EXT: Record<ThemeFormat, string> = {
  attheme: ".attheme",
...
export const TARGET_ALIASES: Record<string, ThemeFormat> = {
...
export const TARGET_CLIENT_LABELS: Record<ThemeFormat, string> = {
  attheme: "📱 Android",
...
export const CLIENT_ENGINE_NOTE: Record<string, string> = {
...
export function normalizeWallpaper(raw: Buffer | null | undefined): Buffer | null {
...
function parseZip(buf: Buffer): Record<string, Buffer> {
  ...
  const maximumEntryBytes = 8 * 1024 * 1024;
  const maximumExpandedBytes = 16 * 1024 * 1024;
  let entries = 0;
  let expandedBytes = 0;

  const inflate = (method: number, data: Buffer, declaredSize: number): Buffer => {
    if (declaredSize > maximumEntryBytes || (data.length && declaredSize / data.length > 100)) {
  ...
  while (i + 30 <= buf.length) {
    ...
    storeEntry(name, Buffer.from(inflate(method, data, uncomp)));
  }
  ...
  if (eocd >= 0) {
    ...
    if (cdTotal > maximumEntries) throw new Error("Theme archive has too many entries");
    let c = cdOffset;
    for (let n = 0; n < cdTotal && c + 46 <= buf.length; n++) {
      ...
      const uncomp = buf.readUInt32LE(c + 24);
...
export function parseDesktopColorText(text: string): Record<string, string> {
...
export function colorsFromThemeSettings(settings: any): Record<string, string> {
  const colors: Record<string, string> = {};
...
export function genCloudThemeSettingsExport(
  colors: Record<string, string>,
  meta?: {
    title?: string;
    basedOn?: string | null;
...
export function parseCloudSettingsJson(buf: Buffer): ThemeDoc | null {
  try {
...
export function parseThemeBuffer(buf: Buffer, hint?: ThemeFormat | null): ThemeDoc | null {
...
export function parseAttheme(buf: Buffer): ThemeDoc | null {
  try {
    const colors: Record<string, string> = {};
...
export function renderDoc(doc: ThemeDoc, target: ThemeFormat, name = "TeleBox Theme"): Buffer | null {
  try {
