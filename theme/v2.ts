...
import {
  ...
  FORMAT_EXT,
  FORMAT_LABELS,
  colorsFromThemeSettings,
  detectFmt,
  genCloudThemeSettingsExport,
  normalizeWallpaper,
...
async function readReply(invocation: CommandInvocation, ctx: PluginContext): Promise<{ buffer: Buffer; name: string }> {
  ...
  if (document.size !== undefined && BigInt(String(document.size)) > BigInt(MAX_FILE_BYTES))
    throw new Error("主题文件超过 5 MiB");
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    await ctx.telegram.withClient(async client => {
      await client.downloadMedia(raw, {
        outputFile: target,
        signal,
    ...
    return { buffer: await readFile(target, { signal }), name: filenameOf(raw) };
  });
}
...
async function sendBuffer(
  ...
  try {
    await ctx.files.withTemp(async (directory, signal) => {
      ...
      await writeFile(target, buffer, { mode: 0o600, flag: "wx", signal });
      signal.throwIfAborted();
      await ctx.telegram.withClient(async client => {
    ...
    if (published) {
      ctx.log.error("theme_temp_cleanup_failed");
      return;
    }
    throw error;
  }
...
async function downloadDocument(ctx: PluginContext, client: any, document: any): Promise<Buffer> {
  ...
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    const location = new Api.InputDocumentFileLocation({
      id: returnBigInt(String(document.id)),
      accessHash: returnBigInt(String(document.accessHash)),
      fileReference: Buffer.from(document.fileReference ?? []),
      thumbSize: "",
    });
    await client.downloadFile(location, {
      outputFile: target,
      fileSize: document.size === undefined ? undefined : returnBigInt(String(document.size)),
      dcId: document.dcId === undefined ? undefined : Number(document.dcId),
      signal,
      progressCallback: (received: any) => {
        if (BigInt(String(received)) > BigInt(MAX_FILE_BYTES)) throw new Error("主题文件超过 5 MiB");
      },
...
async function uploadWallpaper(client: any, doc: ThemeDoc, signal: AbortSignal): Promise<ThemeDoc> {
...
async function downloadWallpaper(ctx: PluginContext, client: any, slug: string): Promise<Buffer> {
  ...
  ctx.signal.throwIfAborted();
  const result: any = await client.invoke(
    new Api.account.GetWallPaper({ wallpaper: new Api.InputWallPaperSlug({ slug }) }),
  );
  ctx.signal.throwIfAborted();
...
async function uploadThemeDocument(
  client: any,
  buffer: Buffer,
  format: ThemeFormat,
  signal: AbortSignal,
  ...
  const media: any = await client.invoke(
...
function clientList(): string {
  return `<b>支持的客户端 / 引擎</b>\n\n${Object.entries(CLIENT_ENGINE_NOTE)
    .map(([name, note]) => `• <code>${escape(name)}</code> — ${escape(note)}`)
    .join("\n")}`;
}
...
export default function createTheme() {
  ...
  const removeWallpaper = (slug: string) => {
    const current = wallpaperBySlug.get(slug);
    if (!current) return;
    wallpaperBySlug.delete(slug);
    wallpaperCacheBytes -= current.buffer.length;
  ...
  const cacheWallpaper = (slug: string, buffer: Buffer) => {
    ...
    const hash = wallpaperHash(buffer);
    const previousSlug = slugByHash.get(hash);
    if (previousSlug && previousSlug !== slug) removeWallpaper(previousSlug);
    wallpaperBySlug.set(slug, { buffer, hash });
    slugByHash.set(hash, slug);
    wallpaperCacheBytes += buffer.length;
    while (wallpaperBySlug.size > MAX_WALLPAPER_CACHE_ENTRIES || wallpaperCacheBytes > MAX_WALLPAPER_CACHE_BYTES) {
      const oldest = wallpaperBySlug.keys().next().value as string | undefined;
      if (!oldest) break;
      removeWallpaper(oldest);
    }
  };
  const cachedWallpaper = (slug: string): Buffer | undefined => {
    ...
    if (!current) return;
    wallpaperBySlug.delete(slug);
    wallpaperBySlug.set(slug, current);
  ...
  const ensureWallpaperSlug = async (client: any, doc: ThemeDoc, signal: AbortSignal): Promise<ThemeDoc> => {
    const wallpaper = normalizeWallpaper(doc.wallpaper);
    if (doc.wallpaperSlug || !wallpaper) return doc;
    const cached = slugByHash.get(wallpaperHash(wallpaper));
    if (cached) return { ...doc, wallpaperSlug: cached };
    const result = await uploadWallpaper(client, doc, signal);
    if (result.wallpaperSlug) cacheWallpaper(result.wallpaperSlug, wallpaper);
    return result;
  };

  const fetchTheme = (ctx: PluginContext, slug: string): Promise<ThemeBundle> =>
    ctx.telegram.withClient(async client => {
      const { Api } = await import("teleproto");
      const collected = new Map<ThemeFormat, Buffer>();
      let fallback: ThemeDoc | undefined;
      for (const format of formats) {
        ctx.signal.throwIfAborted();
        try {
          const result: any = await client.invoke(
            new Api.account.GetTheme({ format: apiFormats[format], theme: new Api.InputThemeSlug({ slug }) }),
          );
          ctx.signal.throwIfAborted();
          if (result?.document) {
            const buffer = await downloadDocument(ctx, client, result.document);
            collected.set(format, buffer);
            fallback ??= parseThemeBuffer(buffer, format) ?? undefined;
          }
  ...
  const convertTo =
    (target: ThemeFormat): SubcommandDefinition["handle"] =>
    async (invocation, ctx) => {
      try {
        ...
        if (
          doc.wallpaperSlug &&
          !normalizeWallpaper(doc.wallpaper) &&
          (target === "attheme" || target === "tdesktop-theme")
        ) {
          doc = await ctx.telegram.withClient(client => resolveWallpaper(ctx, client, doc));
        }
        if (
          (target === "ios-theme" || target === "tgx-theme") &&
          normalizeWallpaper(doc.wallpaper) &&
          !doc.wallpaperSlug
        ) {
          doc = await ctx.telegram.withClient((client, signal) => ensureWallpaperSlug(client, doc, signal));
        }
        ...
        try {
          ...
          ctx.log.error("theme_completion_receipt_failed");
        }
      } catch {
        if (!ctx.signal.aborted) {
          ctx.log.error("theme_convert_failed");
          try {
            await ctx.telegram.edit(invocation.message, "主题转换失败，请检查文件格式后重试");
          } catch {
            ctx.log.error("theme_failure_receipt_failed");
          }
        }
      }
    };

  const link: SubcommandDefinition = {
    description: "下载 t.me/addtheme 主题并补齐四端格式",
    args: "链接",
    examples: [{ args: "link https://t.me/addtheme/example" }],
    async handle(invocation, ctx) {
      const value = invocation.args[0] ?? "";
      const matched = /^(?:https?:\/\/)?t\.me\/addtheme\/([A-Za-z0-9_.-]{1,64})\/?$/u.exec(value);
      if (!matched) {
        await ctx.telegram.edit(invocation.message, "链接格式应为 https://t.me/addtheme/slug");
        return;
      }
      const slug = matched[1]!;
      const key = slug;
      let running = inflight.get(key);
      const reused = Boolean(running);
      if (!running) {
        const created = fetchTheme(ctx, slug);
        running = created.finally(() => {
      ...
      try {
        await ctx.telegram.edit(
          invocation.message,
          reused ? `正在复用 ${escape(slug)} 的主题数据…` : `正在读取云端主题 ${escape(slug)}…`,
        );
        ...
        if (!ctx.signal.aborted) {
          if (delivered) {
            ctx.log.error("theme_link_partial_delivery_failed");
            try {
              await ctx.telegram.edit(invocation.message, `云端主题已发送 ${delivered} 个格式，其余格式发送失败`);
            } catch {
              ctx.log.error("theme_failure_receipt_failed");
            }
          } else {
            ctx.log.error("theme_link_failed");
            try {
              await ctx.telegram.edit(invocation.message, "云端主题读取失败，请检查链接后重试");
            } catch {
              ctx.log.error("theme_failure_receipt_failed");
            }
          }
  ...
  const cloud: SubcommandDefinition = {
    description: "把回复的主题文件创建为 Telegram 云端主题",
    args: "",
    async handle(invocation, ctx) {
      try {
        const input = await readReply(invocation, ctx);
        ...
        const converted = parseCloudSettingsJson(input.buffer)
          ? renderDoc(parsed, "attheme")
          : renderDoc(parsed, format);
        if (!converted) throw new Error("主题规范化失败");
        const slug = randomSlug();
        const created: any = await ctx.telegram.withClient(async (client, signal) => {
          const { Api } = await import("teleproto");
          signal.throwIfAborted();
          const document = await uploadThemeDocument(client, converted, format, signal);
          signal.throwIfAborted();
          const created = await client.invoke(
            new Api.account.CreateTheme({ slug, title: `TeleBox Theme (${FORMAT_LABELS[format]})`, document }),
          );
          signal.throwIfAborted();
          return created;
        });
        ...
        try {
          await ctx.telegram.edit(
            invocation.message,
            `✅ 云端主题已创建\nhttps://t.me/addtheme/${escape(resultSlug)}`,
        ...
        if (!ctx.signal.aborted) {
          ctx.log.error("theme_cloud_failed");
          try {
            await ctx.telegram.edit(invocation.message, "云端主题创建失败，请稍后重试");
          } catch {
            ctx.log.error("theme_failure_receipt_failed");
          }
        }
  ...
  const cloudSettings: SubcommandDefinition = {
    description: "创建供 Unigram/Web 使用的云端配色主题",
    aliases: ["settings", "cloudsettings"],
    args: "",
    async handle(invocation, ctx) {
      try {
        const input = await readReply(invocation, ctx);
        let doc = parseInput(input.buffer);
        const result = await ctx.telegram.withClient(async (client, signal) => {
          const { Api } = await import("teleproto");
          signal.throwIfAborted();
          doc = await uploadWallpaper(client, doc, signal);
          signal.throwIfAborted();
          const exported = genCloudThemeSettingsExport(doc.colors, {
            title: "TeleBox Theme",
            basedOn: doc.basedOn,
          ...
          const inputSettings = new Api.InputThemeSettings({
            baseTheme: baseTheme(Api, settings.baseTheme?._ ?? settings.baseTheme),
            accentColor: settings.accentColor,
            outboxAccentColor: settings.outboxAccentColor,
            ...
            wallpaperSettings: doc.wallpaperSlug
              ? new Api.WallPaperSettings({
                  blur: Boolean(doc.wallpaperBlur),
          ...
          const created: any = await client.invoke(
            new Api.account.CreateTheme({ slug, title: "TeleBox Cloud Theme", settings: [inputSettings] }),
          );
          signal.throwIfAborted();
        ...
        try {
          await sendBuffer(
            ctx,
            invocation,
            Buffer.from(result.exported),
            `${result.slug}-cloud-settings.json`,
            "☁️ 云端 themeSettings 备份",
          );
        } catch (error) {
          if (ctx.signal.aborted) throw error;
          ctx.log.error("theme_cloud_settings_backup_failed");
          await ctx.telegram.edit(
            invocation.message,
            `✅ 云端配色主题已创建\nhttps://t.me/addtheme/${escape(result.slug)}\n⚠️ 配色备份发送失败`,
            { parseMode: "html", linkPreview: false },
          );
          return;
        }
        ...
        if (!ctx.signal.aborted) {
          ctx.log.error("theme_cloud_settings_failed");
          try {
            await ctx.telegram.edit(invocation.message, "云端配色创建失败，请稍后重试");
          } catch {
            ctx.log.error("theme_failure_receipt_failed");
          }
        }
  ...
  const targetCommands: Record<string, SubcommandDefinition> = {
    android: {
      description: "转换为 Android .attheme",
      aliases: ["official", "nekogram", "neko", "nicegram", "owlgram", "extera", "cherrygram", "materialgram"],
      args: "",
      handle: convertTo("attheme"),
    },
    desktop: {
      description: "转换为 Desktop .tdesktop-theme",
      aliases: ["tdesktop", "64gram", "kotatogram", "ayugram", "unigram", "web", "webk", "weba", "telegramweb"],
      args: "",
      handle: convertTo("tdesktop-theme"),
    },
    tgx: {
      description: "转换为 Telegram X/macOS .tgx-theme",
      aliases: ["macos", "mac"],
      args: "",
      handle: convertTo("tgx-theme"),
    },
    ios: { description: "转换为 iOS .tgios-theme", aliases: ["iphone"], args: "", handle: convertTo("ios-theme") },
  };
  const command: CommandDefinition = {
    description: "转换 Telegram 主题或创建云端主题",
    helpOnEmpty: true,
    helpArgs: ["help", "h"],
    args: "客户端",
    examples: [{ args: "android", description: "回复主题文件转换" }, { args: "clients" }],
    subcommands: {
      link,
      cloud,
      "cloud-settings": cloudSettings,
      clients: {
        description: "列出客户端与实际主题引擎",
        aliases: ["list"],
        args: "",
        async handle(invocation, ctx) {
    ...
    help: [
      {
        heading: "输入与限制：",
        body: "仅响应显式 theme 命令，须回复不超过 5 MiB 的 .attheme、.tdesktop-theme、.tgx-theme、.tgios-theme 或 cloud-settings.json。ZIP 限制 64 项、单项 8 MiB、总展开 16 MiB、压缩比 100。",
      },
      {
        heading: "云端操作：",
        body: "link 仅读取 t.me/addtheme；cloud 与 cloud-settings 会在当前 Telegram 账号创建新主题。把内嵌壁纸转换为 iOS/TGX 时会创建云壁纸，并在当前插件实例的有界缓存中复用。插件不会注册广域文件监听。",
      },
    ],
    async handle(invocation, ctx) {
  ...
  return definePlugin({
    ...
    id: "theme",
    description: "转换 Telegram 四端主题与云端配色",
    renderHelp: help,
    commands: { theme: command },
    cleanup() {
      inflight.clear();
      wallpaperBySlug.clear();
      slugByHash.clear();
      wallpaperCacheBytes = 0;
    },
  });
}
