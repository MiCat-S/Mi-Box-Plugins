...
async function readReply(invocation: CommandInvocation, ctx: PluginContext): Promise<{buffer: Buffer; name: string}> {
  ...
  return ctx.files.withTemp(async (directory, signal) => {
    ...
    const info = await stat(target);signal.throwIfAborted();
...
async function sendBuffer(ctx: PluginContext, invocation: CommandInvocation, buffer: Buffer, fileName: string,
  ...
  let published=false;try{await ctx.files.withTemp(async (directory, signal) => {
  ...
  });}catch(error){if(ctx.signal.aborted)throw ctx.signal.reason;if(published){ctx.log.error("theme_temp_cleanup_failed");return;}throw error;}
}
...
async function uploadThemeDocument(client: any, buffer: Buffer, format: ThemeFormat, signal:AbortSignal): Promise<any> {
  const {Api} = await import("teleproto");
  signal.throwIfAborted();
...
export default function createTheme() {
  ...
  const convertTo = (target: ThemeFormat): SubcommandDefinition["handle"] => async (invocation, ctx) => {
    try {
      ...
      try{await ctx.telegram.edit(invocation.message, `✅ 已完成 ${FORMAT_LABELS[target]} 转换`);}catch{ctx.log.error("theme_completion_receipt_failed");}
    } catch {
      if (!ctx.signal.aborted){ctx.log.error("theme_convert_failed");try{await ctx.telegram.edit(invocation.message,"主题转换失败，请检查文件格式后重试");}catch{ctx.log.error("theme_failure_receipt_failed");}}
    }
  };
  ...
  const link: SubcommandDefinition = {description: "下载 t.me/addtheme 主题并补齐四端格式", args: "链接",
    examples: [{args: "link https://t.me/addtheme/example"}], async handle(invocation, ctx) {
      ...
      const slug = matched[1]!; const key = slug;
      ...
      let delivered=0;
      try {
        await ctx.telegram.edit(invocation.message,reused?`正在复用 ${escape(slug)} 的主题数据…`:`正在读取云端主题 ${escape(slug)}…`);
        ...
        try{await ctx.telegram.edit(invocation.message, `✅ 云端主题已处理：${escape(slug)}`, {parseMode: "html"});}catch{ctx.log.error("theme_completion_receipt_failed");}
      } catch {
        if (!ctx.signal.aborted){if(delivered){ctx.log.error("theme_link_partial_delivery_failed");try{await ctx.telegram.edit(invocation.message,`云端主题已发送 ${delivered} 个格式，其余格式发送失败`);}catch{ctx.log.error("theme_failure_receipt_failed");}}else{ctx.log.error("theme_link_failed");try{await ctx.telegram.edit(invocation.message,"云端主题读取失败，请检查链接后重试");}catch{ctx.log.error("theme_failure_receipt_failed");}}}
      }
    }};
    ...
    async handle(invocation, ctx) {
      try {
        ...
        const created: any = await ctx.telegram.withClient(async (client,signal) => {
          const {Api} = await import("teleproto");
          signal.throwIfAborted();const document = await uploadThemeDocument(client, converted, format,signal);signal.throwIfAborted();
          const created=await client.invoke(new Api.account.CreateTheme({slug, title: `TeleBox Theme (${FORMAT_LABELS[format]})`, document}));signal.throwIfAborted();return created;
        });
        const resultSlug = String(created?.slug ?? slug);
        try{await ctx.telegram.edit(invocation.message, `✅ 云端主题已创建\nhttps://t.me/addtheme/${escape(resultSlug)}`, {parseMode: "html", linkPreview: false});}catch{ctx.log.error("theme_completion_receipt_failed");}
      } catch {
        if (!ctx.signal.aborted){ctx.log.error("theme_cloud_failed");try{await ctx.telegram.edit(invocation.message,"云端主题创建失败，请稍后重试");}catch{ctx.log.error("theme_failure_receipt_failed");}}
      }
    }};
    ...
    async handle(invocation, ctx) {
      try {
        ...
        const result = await ctx.telegram.withClient(async (client,signal) => {
          const {Api} = await import("teleproto");
          signal.throwIfAborted();doc = await uploadWallpaper(client, doc,signal);signal.throwIfAborted();
        ...
        try{await sendBuffer(ctx, invocation, Buffer.from(result.exported), `${result.slug}-cloud-settings.json`, "☁️ 云端 themeSettings 备份");}catch(error){if(ctx.signal.aborted)throw error;ctx.log.error("theme_cloud_settings_backup_failed");await ctx.telegram.edit(invocation.message,`✅ 云端配色主题已创建\nhttps://t.me/addtheme/${escape(result.slug)}\n⚠️ 配色备份发送失败`,{parseMode:"html",linkPreview:false});return;}
        try{await ctx.telegram.edit(invocation.message, `✅ 云端配色主题已创建\nhttps://t.me/addtheme/${escape(result.slug)}`, {parseMode: "html", linkPreview: false});}catch{ctx.log.error("theme_completion_receipt_failed");}
      } catch {
        if (!ctx.signal.aborted){ctx.log.error("theme_cloud_settings_failed");try{await ctx.telegram.edit(invocation.message,"云端配色创建失败，请稍后重试");}catch{ctx.log.error("theme_failure_receipt_failed");}}
      }
    }};
