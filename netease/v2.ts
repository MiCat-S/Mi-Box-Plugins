...
function botCommand(input: string): string {
  ...
  let netease = false;
  try { const hostname = new URL(input).hostname.toLowerCase(); netease = hostname === "music.163.com" || hostname.endsWith(".music.163.com"); }
  catch { /* ordinary search text */ }
...
class BotSession {
  ...
  dispose(): void {if (!this.controller.signal.aborted) this.controller.abort(new DOMException("Plugin disposed", "AbortError")); this.tail=Promise.resolve(); this.cursor=0;}
}
...
export default function createNetease() {
  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "netease", description: "通过 Music163bot 搜索和发送网易云音乐",
    commands: {netease: {helpArgs: ["help","h"], helpOnEmpty: true, description: "搜索和发送网易云音乐", async handle(invocation, context) {
      ...
      try {
        ...
        await context.telegram.withClient(async (client, clientSignal) => session.serial(AbortSignal.any([clientSignal,session.signal]), async () => {
          ...
          const cleanup = async (): Promise<void> => {
            ...
            catch { signal.throwIfAborted(); context.log.error("netease_cleanup_failed", {kind: "internal"}); }
          };
          try { await client.invoke(new Api.contacts.Unblock({id: BOT})); signal.throwIfAborted(); }
          catch { signal.throwIfAborted(); }
          try {
            const peer = await client.getInputEntity(BOT); signal.throwIfAborted();
            await client.invoke(new Api.account.UpdateNotifySettings({peer,settings:new Api.InputPeerNotifySettings({silent:true,muteUntil:2147483647})}));
            signal.throwIfAborted();
          } catch { signal.throwIfAborted(); }
          try { await client.invoke(new Api.messages.StartBot({bot: BOT, peer: BOT, startParam: ""})); signal.throwIfAborted(); }
          catch {
            signal.throwIfAborted();
            try { await client.sendMessage(BOT, {message: "/start"}); signal.throwIfAborted(); }
            catch { signal.throwIfAborted(); }
          }
          const before=await recent(client,signal);
          let boundary=session.boundary(before);
          const command = botCommand(keyword);
          try { const sent=await client.sendMessage(BOT, {message: command}); signal.throwIfAborted(); boundary=Math.max(boundary,session.boundary([sent])); }
          catch {
            signal.throwIfAborted();
            try { const sent=await client.sendMessage(BOT, {message: command.replace(/^\/(?:search|music)\s+/, "")}); signal.throwIfAborted(); boundary=Math.max(boundary,session.boundary([sent])); }
            catch { signal.throwIfAborted(); }
          }
          ...
          const markRead = async (): Promise<void> => {
            ...
            try {
              const peer = await client.getInputEntity(BOT); signal.throwIfAborted();
              await client.invoke(new Api.messages.ReadHistory({peer, maxId:lastMsgId})); signal.throwIfAborted();
            } catch { signal.throwIfAborted(); }
          };
          ...
          if(media)boundary=session.boundary([media]);if(buttons)boundary=session.boundary([buttons]);
          if (!media && buttons) {
            boundary=session.boundary(await recent(client,signal));
            try { await buttons.click({}); signal.throwIfAborted(); }
            catch {
              signal.throwIfAborted();
            ...
            for (let attempt = 0; attempt < 20 && !media; attempt++) {
              await delay(700, undefined, {signal});
              media = (await recent(client, signal)).slice().reverse().find(message =>
                !message.out && messageId(message)>boundary && Boolean(message.media));
