...
import {
  STRUCTURED_PLUGIN_API_VERSION, definePlugin, renderCommandHelp, requireSdkFeatures, ui,
...
const decimal = (value: unknown): string | undefined => {
  ...
  if (value && typeof value === "object" && typeof (value as {toString?: unknown}).toString === "function") {
    const text = String(value); if (/^-?\d+$/.test(text)) return helpers.returnBigInt(text).toString();
  }
  return undefined;
};
const entityLike = (value: string): string | ReturnType<typeof helpers.returnBigInt> => /^-?\d+$/.test(value) ? helpers.returnBigInt(value) : value;
...
function normalized(value: Partial<State>): State {
  ...
  if (value.backups && typeof value.backups === "object") for (const [id, item] of Object.entries(value.backups)) {
    const raw: any = item;
    const source = decimal(raw?.source ?? raw?.sourceId), target = decimal(raw?.target ?? raw?.targetId);
    if (!/^[a-z0-9-]{1,80}$/i.test(id) || !source || !target) continue;
    const startedAt = Date.parse(String(raw.startedAt ?? "")), completedAt = Date.parse(String(raw.completedAt ?? ""));
    backups[id] = {id, source, target, order: raw.order === "asc" || raw.reverse === true ? "asc" : "desc",
      status: ["pending", "running", "paused", "completed", "failed"].includes(raw.status) ? raw.status : "failed",
      processed: Math.max(0, Number(raw.processed ?? raw.processedMessages) || 0), failed: Math.max(0, Number(raw.failed ?? raw.failedMessages) || 0),
      createdAt: Number(raw.createdAt) || startedAt || Date.now(), ...(Number.isSafeInteger(Number(raw.topicId)) && Number(raw.topicId) > 0 ? {topicId: Number(raw.topicId)} : {}),
      ...(Number.isSafeInteger(Number(raw.cursor ?? raw.lastMessageId)) && Number(raw.cursor ?? raw.lastMessageId) > 0 ? {cursor: Number(raw.cursor ?? raw.lastMessageId)} : {}),
      ...(Number.isSafeInteger(Number(raw.upperBound)) && Number(raw.upperBound) > 0 ? {upperBound: Number(raw.upperBound)} : {}),
      ...(Number.isFinite(Number(raw.completedAt)) ? {completedAt: Number(raw.completedAt)} : completedAt ? {completedAt} : {}), ...(raw.error ? {error: String(raw.error).slice(0, 240)} : {})};
  }
...
function legacyStats(rules: ShiftRule[], raw: unknown): ShiftRule[] {
  ...
  return rules.map(rule => {
    let forwarded = rule.stats.forwarded, lastForwardedAt = rule.stats.lastForwardedAt;
    for (const [date, sources] of Object.entries(raw as Record<string, unknown>)) {
      ...
      const stamp = Date.parse(date); if (Number.isFinite(stamp) && (!lastForwardedAt || stamp > lastForwardedAt)) lastForwardedAt = stamp;
    }
...
async function sqliteLegacy(context: PluginContext): Promise<Record<string, unknown> | undefined> {
  try {
    return await context.storage.sqlite("shift.db", {mustExist: true}).read(db => {
      ...
      const rules: Record<string, unknown> = {}, stats: Record<string, Record<string, unknown>> = {};
      for (const row of rows) rules[String(row.source_id)] = {target_id: row.target_id, options: JSON.parse(row.options || "[]"),
        target_type: row.target_type, paused: row.paused === 1n, created_at: row.created_at, filters: JSON.parse(row.filters || "[]")};
      for (const row of statRows) { const parts = String(row.stats_key).split("."); if (parts.length < 4) continue;
...
async function stateDocument(context: PluginContext): Promise<Record<string, unknown> | undefined> {
  try { const value = JSON.parse(await readFile(context.files.dataPath("state.json"), {encoding: "utf8", signal: context.signal})); context.signal.throwIfAborted();
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Invalid shift state"); return value; }
  catch (error) { context.signal.throwIfAborted(); if (error instanceof Error && "code" in error && error.code === "ENOENT") return; throw new Error("SHIFT_STATE_INVALID"); }
}
...
async function deliverLines(context: PluginContext, message: MessageEnvelope, lines: readonly string[], html = false): Promise<void> {
  ...
  const result = await ui.deliverPages(pages, context.signal, (value, index) => index
    ? context.telegram.reply(message, value, {parseMode: "html"})
    : context.telegram.edit(message, value, {parseMode: "html"}));
  if (!result.interrupted) return;
  context.log.info("shift_pagination_interrupted", {published: result.published, total: result.total,
    category: ui.deliveryErrorCategory(result.error)});
  if (!result.published) throw result.error;
  try { await context.telegram.reply(message, ui.interruptedNotice(result), {parseMode: "html"}); } catch {}
}
...
async function admitsContent(rule: ShiftRule, textValue: string, context: PluginContext): Promise<boolean> {
  if (rule.whitelistEnabled) {
    ...
    let matched = false;
    for (const pattern of rule.whitelistPatterns) { const result = await context.regexp.test(pattern, textValue, {flags: "i"}, context.signal); context.signal.throwIfAborted(); if (result.matched && !result.timedOut) {matched = true; break;} }
...
async function onMessage(runtime: Runtime, message: MessageEnvelope, context: PluginContext): Promise<void> {
  ...
  if (!grouped) { if (await admitsContent(rule, message.text, context)) await forward(context, rule, [message.id]); return; }
  ...
  void context.tasks.run(`shift-album:${key}`, async signal => {
    try {
      ...
      let admitted = false; for (const item of current.messages) if (await admitsContent(latest, item.text, context)) {admitted = true; break;}
      if (!admitted) return;
  ...
  }).catch(() => { if (!context.signal.aborted) context.log.error("shift_album_failed"); });
}
...
async function runBackup(runtime: Runtime, context: PluginContext, id: string): Promise<void> {
  ...
  try {
    ...
    await context.telegram.withClient(async (client: any, signal) => {
      ...
      if (task.order === "asc") {
        ...
        while (collected.length < MAX_BACKUP_MESSAGES) {
          signal.throwIfAborted();
          const values: any[] = await client.getMessages(entityLike(task.source), {limit: 100, offsetId: pageCursor}); signal.throwIfAborted();
        ...
        for (let offset = Math.min(processed + failed, ordered.length); offset < ordered.length; offset += 100) {
          ...
          try { await client.forwardMessages(entityLike(task.target), {messages: ids, fromPeer: entityLike(task.source), ...(task.topicId ? {topMsgId: task.topicId} : {})}); signal.throwIfAborted(); processed += ids.length; }
          catch { signal.throwIfAborted(); failed += ids.length; }
        ...
        signal.throwIfAborted();
        const values: any[] = await client.getMessages(entityLike(task.source), {limit: Math.min(100, MAX_BACKUP_MESSAGES - processed - failed), offsetId: cursor}); signal.throwIfAborted();
        if (!values?.length) break;
    ...
    if (!context.signal.aborted) await update(context, value => ({...value, backups: {...value.backups, [id]: {...value.backups[id]!, status: "failed", error: "BACKUP_FAILED"}}}));
  } finally { runtime.backups.delete(id); }
}

function startBackup(runtime: Runtime, context: PluginContext, id: string): void {
  void context.tasks.run(`shift-backup:${id}`, () => runBackup(runtime, context, id)).catch(() => {
...
export default function createShift() {
  ...
  const command: CommandDefinition = {
    ...
    subcommands: {
      ...
      backup: {description: "启动历史消息备份", args: "[源] [目标|话题ID] [--asc]", subcommands: {
        status: {description: "查看备份任务", args: "[任务ID]", async handle(invocation, context) {
          ...
          const selected = invocation.args[0] ? [backups[invocation.args[0]]].filter(Boolean) : Object.values(backups);
          await deliverLines(context, invocation.message, selected.length ? selected.map(task => `${task!.id}: ${task!.status}，成功 ${task!.processed}，失败 ${task!.failed}`) : ["没有备份任务"]);
        }},
    ...
    help: [{heading: "规则与类型：", body: "每个源会话保留一条规则，支持文字及常见媒体类型、静默转发、编辑消息、目标话题和 send-as。创建与导入时拒绝直接或链式循环。相册会短暂聚合后整组转发。"},
      {heading: "过滤与生命周期：", body: "filter 按关键词包含匹配；whitelist 保留合法 JavaScript 正则语义，并由 SDK worker 按每个表达式最多 512 字符、输入最多 4096 字符、单次 50ms 的预算执行。动态相册和备份都由插件任务作用域跟踪，卸载时可取消；备份最多处理 10000 条。"},
  ...
  return definePlugin({apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "shift", description: "按规则转发会话消息并执行历史备份", renderHelp: help,
    ...
    async setup(context) {
      ...
      if (!current.legacyImported) {
        ...
        const imported = legacyStats(rawRules.map((item: any) => cleanRule(item)).filter((item: ShiftRule | undefined): item is ShiftRule => !!item).slice(0, MAX_RULES), (legacy as any).stats);
        await update(context, value => ({...legacy, ...value, legacyImported: true,
          rules: document && Object.hasOwn(document, "rules") ? value.rules : imported,
          backups: document && Object.hasOwn(document, "backups") ? value.backups : (legacy as any).backups ?? value.backups}));
      }
  ...
  function patternCommand(label: string, key: "filters" | "whitelistPatterns", whitelist: boolean): SubcommandDefinition {
    return {description: `管理${label}`, aliases: whitelist ? ["wl"] : ["f"], args: "[规则序号] [add|del|list|enable|disable] [内容]",
      ...
      async handle(invocation, context) {
        ...
        if (whitelist ? !validRegex(content) : Buffer.byteLength(content) > 240) throw new Error(whitelist ? "正则无效或超过 512 字符" : "关键词过长");
  ...
  function protect<T extends CommandDefinition | SubcommandDefinition>(definition: T): T {
    ...
    return {...definition, ...(children ? {subcommands: children} : {}), async handle(invocation: CommandInvocation, context: PluginContext) {
      try { await original(invocation, context); }
        ...
        const detail = branch === "set" ? "规则设置失败，请检查会话、选项和循环关系"
          : branch === "whitelist" ? "正则无效或白名单参数错误"
          : branch === "backup" ? "备份参数无效，源会话与目标会话不能相同"
          : branch === "import" ? "导入数据无效"
          : "请检查命令参数";
