...
async function prepareImage(image: Buffer, signal: AbortSignal): Promise<Buffer> {
  ...
  signal.throwIfAborted();
  if (!metadata.width || !metadata.height || metadata.width * metadata.height > MAX_IMAGE_PIXELS) throw new Error("图片尺寸无效");
...
export default function createJupai() {
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "jupai", description: "生成举牌小人图片", commands: {
    jupai: {description: "生成举牌小人图片", helpArgs: ["help", "h"], async handle(invocation, ctx: PluginContext) {
      let text = invocation.args.join(" ").trim();
      if (["help", "h"].includes(text.toLowerCase())) { await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode: "html"}); return; }
      if (!text) {
        const reply = await ctx.telegram.getReply(invocation.message);
        ctx.signal.throwIfAborted();
        text = reply?.text?.trim() ?? "";
      }
      if (!text) { await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode:"html"}); return; }
      if (text.length > MAX_TEXT_LENGTH) { await ctx.telegram.edit(invocation.message, "文本过长，最多支持 500 字符"); return; }
      try {
        await ctx.telegram.edit(invocation.message, "正在生成举牌小人…");
        ctx.signal.throwIfAborted();
        const response = await ctx.http.withResponse(`${api}?msg=${encodeURIComponent(text)}`, {credentials: "omit"}, responseBytes,
        ...
        await ctx.telegram.withClient(async (client, clientSignal) => {
          ...
          if (typeof raw?.delete === "function") {
            ...
            catch { ctx.log.info("jupai_receipt_cleanup_failed"); }
            signal.throwIfAborted();
          } else {
