...
function flood(error: unknown): number | undefined {
  if (!error || typeof error !== "object") return;
  const own = Object.getOwnPropertyDescriptor(error, "message")?.value;
  if (typeof own !== "string") return;
  const match = /FLOOD_WAIT[_ ]?(\d+)/.exec(own);
  if (!match) return;
...
export async function deleteBatch(
  ...
  for (let retry = 0; retry <= MAX_FLOOD_RETRIES; retry++) {
    ...
    try {
      ...
      signal.throwIfAborted();
      const seconds = flood(error);
      if (seconds === undefined || retry === MAX_FLOOD_RETRIES) break;
...
export async function cleanupReceipt(
  ctx: PluginContext,
  chat: any,
  id: number,
  signal: AbortSignal,
  ...
  signal.throwIfAborted();
  await ctx.telegram.withClient(async (c, clientSignal) => {
    const combined = AbortSignal.any([signal, clientSignal]);
    combined.throwIfAborted();
    await c.deleteMessages(chat, [id], { revoke: true });
    combined.throwIfAborted();
  });
}
...
async function run(message: MessageEnvelope, ctx: PluginContext) {
  ...
  if (raw?.isPrivate || !message.chatId.startsWith("-")) {
    await ctx.telegram.edit(message, "❌ 仅群组可用", { parseMode: "html" });
    return;
  }
  await ctx.telegram.withClient(async (client: any, clientSignal) => {
    const signal = AbortSignal.any([ctx.signal, clientSignal]);
    signal.throwIfAborted();
    const target = raw?.peerId ?? peer(message.chatId);
    const chat = await client.getEntity(target);
    signal.throwIfAborted();
    if (chat?.className !== "Channel") {
      await ctx.telegram.edit(message, "❌ 仅超级群组或频道可用", { parseMode: "html" });
      return;
    }
    let participant: any;
    try {
      ...
      signal.throwIfAborted();
      await ctx.telegram.edit(message, "❌ 无法确认管理员权限", { parseMode: "html" });
      return;
    }
    const rights = participant?.adminRights;
    if (
      participant?.className !== "ChannelParticipantCreator" &&
      (participant?.className !== "ChannelParticipantAdmin" || !rights?.banUsers || !rights?.deleteMessages)
    ) {
      await ctx.telegram.edit(message, "❌ 需要封禁成员和删除消息权限才能执行此操作", { parseMode: "html" });
      return;
    }
    await ctx.telegram.edit(message, "🚨 <b>一键跑路</b>\n\n正在处理中...", { parseMode: "html" });
    signal.throwIfAborted();
    let muted = false,
      deleted = 0,
      failed = 0;
    try {
      ...
      signal.throwIfAborted();
      ctx.log.error("paolu_mute_failed");
    }
    let batch: number[] = [];
    for await (const item of client.iterMessages(chat, { minId: 1, reverse: true })) {
      ...
      if (batch.length === BATCH_SIZE) {
        const count = await deleteBatch(client, chat, batch, signal);
        deleted += count;
    ...
    try {
      ...
      signal.throwIfAborted();
      failed++;
      ctx.log.error("paolu_command_cleanup_failed");
    }
    const sent = await client.sendMessage(chat, {
      message: `${muted ? "✅" : "⚠️"} <b>跑路完成</b>\n\n• 全员禁言: ${muted ? "成功" : "失败"}\n• 已删除 ${deleted} 条消息${failed ? `\n• 删除失败 ${failed} 条` : ""}\n\n此消息将在10秒后自动删除`,
      parseMode: "html",
    });
    signal.throwIfAborted();
    void ctx.tasks
      .run(`paolu:cleanup:${message.chatId}:${sent.id}`, scoped => cleanupReceipt(ctx, chat, sent.id, scoped))
      .catch(() => {
        if (!ctx.signal.aborted) ctx.log.error("paolu_receipt_cleanup_failed");
      });
  });
}
export default function createPaolu() {
  return definePlugin({
    renderHelp: renderPluginHelp,
    apiVersion: 1,
    id: "paolu",
    description: "删除群内消息并禁言所有成员",
    commands: {
      paolu: {
        description: "群组一键跑路",
        ignoreEdited: true,
        async handle({ message }, ctx) {
          try {
            ...
            if (!ctx.signal.aborted) {
              ctx.log.error("paolu_failed");
              await ctx.telegram.edit(message, "❌ 操作失败，请稍后重试", { parseMode: "html" });
            }
          }
        },
      },
