...
const points: readonly (readonly [number, number])[] = [
  [0, 1376438400],
  [50000000, 1400000000],
  [150000000, 1451606400],
  [350000000, 1483228800],
  [500000000, 1514764800],
  [900000000, 1559347200],
  [1100000000, 1585699200],
  [1450000000, 1609459200],
  [2150000000, 1640995200],
  [5100000000, 1654041600],
  [5600000000, 1672531200],
  [6800000000, 1704067200],
  [7800000000, 1735689600],
  [8500000000, 1767225600],
];
...
type Profile = Partial<
  Pick<Api.User, "id" | "username" | "firstName" | "lastName" | "bot" | "verified" | "premium" | "scam" | "fake">
> & {
  first_name?: string;
  last_name?: string;
};
...
function format(info: Info): string {
  ...
  const links = [
    `tg://user?id=${id}`,
    username ? `https://t.me/${username}` : `https://t.me/@id${id}`,
...
async function send(context: PluginContext, message: MessageEnvelope, text: string): Promise<void> {
  ...
  if (delivery.interrupted) {
    context.log.info("ids_delivery_interrupted", {
...
const idsCommand: CommandDefinition = {
  ...
  help: [
    {
      heading: "显示信息：",
      body: "用户名与显示名称、用户 ID、注册时间估算、DC、入群时间（仅群组）、共同群组数量、用户简介与三种跳转链接。",
    },
  ],
    ...
    try {
      ...
      if (target === "help" || target === "h") {
        await context.telegram.edit(
          message,
          renderCommandHelp("ids", idsCommand, { prefix, title: "🆔 用户信息查询插件" }),
          { parseMode: "html" },
        );
        return;
      }
      ...
      const info = await context.telegram.withClient(async (client, signal): Promise<Info | undefined> => {
        ...
        if (target.startsWith("@")) {
          ...
          user = entity as Profile;
          id = returnBigInt(entity.id);
        } else if (target) {
        ...
        const resolveUser = async () => {
          ...
          const resolved = user
            ? utils.getInputUser(user as any)
            ...
            const participant = await call(() =>
              client.invoke(
                new Api.channels.GetParticipant({
                  channel: utils.getInputChannel(channelEntity),
