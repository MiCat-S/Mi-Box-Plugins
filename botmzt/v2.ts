...
async function waitFor(
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    if (found) {
      runtime.cursor = Math.max(runtime.cursor, Number(found.id));
      return found;
    }
...
async function request(
  ...
  await context.telegram.edit(invocation.message, photo ? "🔄 正在获取图片..." : "📅 正在执行签到...", {
    parseMode: "html",
  });
  try {
    await context.telegram.withClient((client, signal) =>
      serial(runtime, async () => {
        signal.throwIfAborted();
        ...
        if (typeof raw.delete === "function") {
          try {
            ...
            context.log.error("botmzt_command_delete_failed");
          }
        }
      }),
    );
    ...
    if (flood) {
      ...
      await context.telegram.edit(invocation.message, `❌ <b>无法访问机器人</b>\n\n请先私聊 ${BOT} 并发送 /start`, {
        parseMode: "html",
      });
    } else {
      await context.telegram.edit(invocation.message, `❌ <b>${photo ? "获取图片" : "签到"}失败</b>，请稍后重试`, {
        parseMode: "html",
      });
    }
  }
...
export default function createBotmzt() {
  ...
  const commands: Record<string, any> = {
    ...
    qd: {
      description: "向图片机器人签到",
      handle: (invocation: any, context: PluginContext) => request(runtime, context, invocation, "checkin", false),
    },
  };
  for (const [name, command] of Object.entries(IMAGE_COMMANDS)) {
    commands[name] = {
      description: "从图片机器人获取剧透图片",
      handle: (invocation: any, context: PluginContext) => request(runtime, context, invocation, command, true),
    };
  }
  return definePlugin({
