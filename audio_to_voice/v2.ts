...
function audioDocument(raw: ApiTypes.Message | undefined): AudioDocument | undefined {
  ...
  const protocolDocument =
    ...
    (media.document as AudioDocument | undefined)?.className === "Document"
      ? (media.document as AudioDocument)
      : undefined;
...
function audioDuration(document: AudioDocument): number {
  const attribute = (document.attributes ?? []).find(
    candidate => (candidate as { className?: unknown }).className === "DocumentAttributeAudio",
...
export default function createAudioToVoice(dependencies: { readonly resolveFfmpeg?: () => Promise<string> } = {}) {
  ...
  return definePlugin({
    ...
    apiVersion: 1,
    id: "audio_to_voice",
    description: "使用 FFmpeg 将回复音频转换为 Telegram 语音",
            ...
            await context.files.withTemp(async (directory, signal) => {
              ...
              await context.telegram.withClient(async (client, clientSignal) => {
                ...
                await downloadBounded(
                  client,
                  source.message.media! as Parameters<TelegramClient["iterDownload"]>[0],
                  input,
                  activeSignal,
                );
              });
              ...
              if (
                ...
                outputStat.size === 0 ||
                outputStat.size > MAX_AUDIO_BYTES
              ) {
                ...
                await client.sendFile(raw.peerId, {
                  ...
                  attributes: [
                    new Api.DocumentAttributeAudio({
                      duration: audioDuration(source.document),
                      voice: true,
