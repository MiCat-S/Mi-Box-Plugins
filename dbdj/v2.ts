    ...
    commands: {dbdj: {helpOnEmpty: true, description: "从近期发言者中随机抽取用户", async handle(invocation, context) {
      ...
      if (!scanCount || !pickCount) {
        ...
        await context.telegram.withClient(async (_client, signal) => {
          ...
          if (typeof raw?.delete === "function") {
            ...
            catch { context.log.error("dbdj_delete_failed"); }
            signal.throwIfAborted();
          }
        });
        return;
      ...
      try {
        ...
        await context.telegram.withClient(async (client, signal) => {
          ...
          try {
            const result = await client.getMessages(peer, {limit: scanCount, offsetId: Math.max(0, invocation.message.id - 1)});
            signal.throwIfAborted();
          ...
          if (delivery.interrupted) {
            context.log.info("dbdj_delivery_interrupted", {published: delivery.published, total: delivery.total,
              category: ui.deliveryErrorCategory(delivery.error)});
            if (!delivery.published) throw delivery.error;
            try { await context.telegram.reply(invocation.message, ui.interruptedNotice(delivery), {parseMode: "html"}); }
            catch {}
            return;
          }
          signal.throwIfAborted();
