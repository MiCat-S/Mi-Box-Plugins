...
const promptChange = (adding: boolean): CommandDefinition["handle"] => guarded(async (invocation, context, state, edit) => {
  ...
  else if (!adding) delete state.prompts[name]; else throw new Error("用法：prompt add|del|list");
  await store(context).update(value => ({...normalize(value), prompts: state.prompts, aiMigrated: state.aiMigrated, legacyImported: true}));
  await edit("提示词已更新");
});
