...
type Provider = {name:string;baseUrl:string;apiKey:string;model:string;type:ProviderType;authMethod:AuthMethod};
type State = {schemaVersion:1;providers:Record<string,Provider>;defaultProvider:string|null;prompts:Record<string,string>;timeoutMs:number;collapse:boolean;legacyImported:boolean;[key:string]:unknown};
const DEFAULTS:State={schemaVersion:1,providers:{},defaultProvider:null,prompts:{},timeoutMs:120_000,collapse:true,legacyImported:false};
const BUILTIN:Readonly<Record<string,string>>={zj:"请总结以下消息的主要内容，提取关键信息，用简洁的中文回复：",fx:"请分析以下消息的观点、态度和倾向，用简洁的中文回复："};
const store=(c:PluginContext)=>c.storage.json<State>("v2-config.json",DEFAULTS);
const esc=(v:unknown)=>String(v??"").replace(/[&<>\"']/g,x=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#x27;"})[x]!);
async function responseJson(response:Response,signal:AbortSignal){if(response.status<200||response.status>=300)throw new Error("上游状态异常");const type=response.headers.get("content-type")??"";if(type&&!type.toLowerCase().includes("json"))throw new Error("上游格式异常");if(Number(response.headers.get("content-length"))>2*1024*1024)throw new Error("上游响应过大");if(!response.body)return{};const reader=response.body.getReader(),chunks:Uint8Array[]=[];let size=0;try{for(;;){signal.throwIfAborted();const part=await reader.read();if(part.done)break;size+=part.value.byteLength;if(size>2*1024*1024)throw new Error("上游响应过大");chunks.push(part.value);}return JSON.parse(Buffer.concat(chunks,size).toString("utf8"));}finally{await reader.cancel().catch(()=>{});reader.releaseLock();}}

function endpoint(value:string):URL {
  let url:URL; try{url=new URL(value);}catch{throw new Error("API 地址无效");}
  if(url.protocol!=="https:"||url.username||url.password||url.hash||url.search||!url.hostname||url.port)throw new Error("API 地址必须是无凭据的 HTTPS 地址");
  url.pathname=url.pathname.replace(/\/+$/u,"");
  return url;
}
function provider(value:unknown,name?:string):Provider|undefined{
  if(!value||typeof value!=="object")return;
  const x=value as Record<string,unknown>,type=x.type==="gemini"?"gemini":x.type==="openai"?"openai":undefined;
  const auth=x.authMethod??x.auth_method;
  if(!type||typeof (x.baseUrl??x.base_url)!=="string"||typeof (x.apiKey??x.api_key)!=="string"||typeof x.model!=="string")return;
  const authMethod:AuthMethod=auth==="api_key_header"||auth==="query_param"||auth==="bearer_token"?auth:(type==="gemini"?"query_param":"bearer_token");
  const result:Provider={name:String(x.name??name??"").slice(0,64),baseUrl:String(x.baseUrl??x.base_url),apiKey:String(x.apiKey??x.api_key),model:String(x.model).slice(0,128),type,authMethod};
  if(!result.name||!result.apiKey||!result.model)return; endpoint(result.baseUrl); return result;
}
function normalize(raw:unknown):State{
  const x=raw&&typeof raw==="object"?raw as Record<string,unknown>:{};const providers:Record<string,Provider>={};
  if(x.providers&&typeof x.providers==="object")for(const[name,value]of Object.entries(x.providers)){const p=provider(value,name);if(p)providers[p.name]=p;}
  const prompts:Record<string,string>={};if(x.prompts&&typeof x.prompts==="object")for(const[k,v]of Object.entries(x.prompts)){if(/^[\w-]{1,32}$/u.test(k)&&typeof v==="string"&&v.length<=10_000)prompts[k]=v;}
  const candidate=String(x.defaultProvider??x.default_provider??"");
  return {...x,schemaVersion:1,providers,defaultProvider:providers[candidate]?candidate:Object.keys(providers)[0]??null,prompts,
...
async function ai(c:PluginContext,p:Provider,prompt:string,content:string,timeoutMs:number){const base=endpoint(p.baseUrl),headers:Record<string,string>={"content-type":"application/json"};let target:URL,body:unknown;
  if(p.type==="gemini"){target=new URL(`${base.pathname}/v1beta/models/${encodeURIComponent(p.model)}:generateContent`,base.origin);if(p.authMethod==="api_key_header")headers["x-goog-api-key"]=p.apiKey;else target.searchParams.set("key",p.apiKey);body={contents:[{role:"user",parts:[{text:`${prompt}\n\n${content}`}]}]};}
  else{target=new URL(`${base.pathname}${/\/v\d+$/u.test(base.pathname)?"":"/v1"}/chat/completions`,base.origin);if(p.authMethod==="api_key_header")headers["x-api-key"]=p.apiKey;else if(p.authMethod==="query_param")target.searchParams.set("key",p.apiKey);else headers.authorization=`Bearer ${p.apiKey}`;body={model:p.model,messages:[{role:"user",content:`${prompt}\n\n${content}`}],max_tokens:4096};}
