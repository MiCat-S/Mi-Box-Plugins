...
function provider(value: unknown, name?: string): LegacyProvider | undefined {
  ...
  const result: LegacyProvider = {
    name: String(item.name ?? name ?? "").slice(0, 64),
    baseUrl,
    apiKey,
    model: item.model.slice(0, 200),
    type,
  };
...
function normalize(raw: unknown): State {
  ...
  if (value.providers && typeof value.providers === "object")
    for (const [name, item] of Object.entries(value.providers)) {
      const parsed = provider(item, name);
  ...
  return {
    ...
    schemaVersion: 2,
    providers,
    defaultProvider: providers[defaultName] ? defaultName : (Object.keys(providers)[0] ?? null),
    prompts,
    collapse: value.collapse !== false,
    legacyImported: value.legacyImported === true,
    aiMigrated: value.aiMigrated === true,
  };
}
async function readState(context: PluginContext): Promise<State> {
  ...
  try {
    const raw = JSON.parse(
      await context.tasks.run("uai:v2-read", signal =>
  ...
  if (!current.legacyImported) {
    ...
    try {
      legacyRaw = JSON.parse(
        await context.tasks.run("uai:legacy-read", signal =>
    ...
    if (legacyRaw !== undefined) {
      ...
      current = normalize({
        ...
        prompts: { ...legacy.prompts, ...current.prompts },
        collapse: Object.hasOwn(explicit, "collapse") ? current.collapse : legacy.collapse,
        defaultProvider:
          Object.hasOwn(explicit, "defaultProvider") || Object.hasOwn(explicit, "default_provider")
    ...
    current = await store(context).update(value => {
      ...
      return normalize({
        ...
        defaultProvider:
          Object.hasOwn(explicit, "defaultProvider") || Object.hasOwn(explicit, "default_provider") || defaultChanged
            ? latest.defaultProvider
            : imported.defaultProvider,
        legacyImported: true,
        aiMigrated: Object.keys(providers).length ? false : latest.aiMigrated,
      });
    });
  }
...
async function migrateAi(context: PluginContext): Promise<State> {
  ...
  if (!entries.length)
    return store(context).update(value => ({
      ...normalize(value),
      providers: {},
      defaultProvider: null,
      aiMigrated: true,
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    const tag = `uai-${index + 1}`;
    await context.services.call(
      "ai",
      "import_provider",
      {
        tag,
        url: providerUrl(item),
        key: item.apiKey,
        type: item.type === "gemini" ? "gemini" : "openai-compatible",
        models: { chat: item.model },
        select: name === current.defaultProvider ? ["chat"] : [],
      },
      context.signal,
    );
    context.signal.throwIfAborted();
  ...
  await context.storage
    .json<Record<string, unknown>>("config.json", {})
    .update(value => ({ ...value, providers: {}, default_provider: null, defaultProvider: null }), context.signal);
  context.signal.throwIfAborted();
  return store(context).update(value => ({
    ...normalize(value),
    providers: {},
...
const id = (value: any): string =>
  value == null
    ? ""
    : typeof value === "object"
      ? id(value.userId ?? value.channelId ?? value.chatId ?? value.value)
      : String(value);
function limit(args: readonly string[]) {
  let result: { kind: "count"; value: number } | { kind: "time"; value: number } | { kind: "today" } = {
    kind: "today",
  };
  for (const arg of args)
    if (/^\d+$/u.test(arg)) result = { kind: "count", value: Math.min(500, Math.max(1, Number(arg))) };
    else {
      const match = /^(\d+)([hm])$/iu.exec(arg);
      if (match)
        result = {
          kind: "time",
          value: Math.min(7 * 86400, Number(match[1]) * (match[2].toLowerCase() === "h" ? 3600 : 60)),
        };
    }
  return result;
}
async function collect(
  ...
  peer: any,
  sender: string | null,
  rule: ReturnType<typeof limit>,
): Promise<string[]> {
  return context.telegram.withClient(async client => {
    const now = Math.floor(Date.now() / 1000);
    const start =
      rule.kind === "today" ? new Date().setHours(0, 0, 0, 0) / 1000 : rule.kind === "time" ? now - rule.value : 0;
    const maximum = rule.kind === "count" ? rule.value : 500;
    const options: any = { limit: sender ? Math.min(maximum * 20, 3000) : maximum };
    if (sender)
      try {
    ...
    for await (const value of client.iterMessages(peer, options)) {
      ...
      if (typeof message.message !== "string" || !message.message || /^📊 (分析|总结)结果/u.test(message.message))
        continue;
      ...
      if (characters + line.length > 100_000) break;
...
const guarded =
  (
    operation: (
      ...
      context: PluginContext,
      state: State,
      edit: (text: string, html?: boolean) => Promise<void>,
    ) => Promise<void>,
  ...
  async (invocation, context) => {
    ...
    try {
      if (
        ...
        (!invocation.args[0] || invocation.args[0] === "help" || invocation.args[0] === "h")
      ) {
        await edit(help(invocation.prefix), true);
        return;
      }
      await operation(invocation, context, await migrateAi(context), edit);
    } catch {
      if (!context.signal.aborted) {
        context.log.error("uai_failed");
        await edit("UAI 执行失败，请检查引用消息、ai 配置和网络");
      }
    }
  };
async function deliverAnalysis(invocation: CommandInvocation, context: PluginContext, html: string): Promise<void> {
  ...
  const delivery = await ui.deliverPages(pages, context.signal, (page, index) => {
    ...
    return index
      ? context.telegram.reply(invocation.message, labelled, { parseMode: "html", linkPreview: false })
      : context.telegram.edit(invocation.message, labelled, { parseMode: "html", linkPreview: false });
  });
  if (!delivery.interrupted) return;
  context.log.error("uai_delivery_failed", {
    kind: ui.deliveryErrorCategory(delivery.error),
  ...
  try {
    ...
    context.signal.throwIfAborted();
    context.log.error("uai_delivery_notice_failed", { kind: ui.deliveryErrorCategory(error) });
  }
}
const analyze = (selectedPrompt?: string): CommandDefinition["handle"] =>
  guarded(async (invocation, context, state, edit) => {
    const reply = await context.telegram.getReply(invocation.message);
    if (!reply) throw new Error("请引用一条消息");
    const raw: any = invocation.message.raw,
      replyRaw: any = reply.raw;
    if (!raw?.peerId || !replyRaw) throw new Error("消息上下文不可用");
    let source: string | null = id(replyRaw.senderId),
      peer: any = raw.peerId,
      name = String(replyRaw.sender?.firstName ?? replyRaw.sender?.username ?? "用户");
    const forwarded = replyRaw.fwdFrom?.fromId;
    if (forwarded?.channelId) {
      source = null;
      peer = `-100${id(forwarded.channelId)}`;
      name = "频道";
    } else if (forwarded?.userId) source = id(forwarded.userId);
    if (!source && !forwarded?.channelId) throw new Error("无法确定消息来源");
    const key = selectedPrompt ?? invocation.args[0];
    const promptKey = Object.hasOwn(BUILTIN, key) || Object.hasOwn(state.prompts, key) ? key : "zj";
    const messages = await collect(context, peer, source, limit(invocation.args));
    if (!messages.length) throw new Error("没有找到消息");
    if (!context.services.available("ai", "chat")) throw new Error("请先安装 ai 插件");
    await edit("正在分析消息…");
    const result = await context.services.call<string>(
      "ai",
      "chat",
      {
        text: `${name}\n\n${messages.join("\n")}`,
        systemPrompt: state.prompts[promptKey] ?? BUILTIN[promptKey] ?? BUILTIN.zj,
        maxOutputTokens: 4096,
      },
      context.signal,
    );
    const content = esc(result);
    await deliverAnalysis(
      invocation,
      context,
      `📊 <b>${promptKey === "fx" ? "分析" : "总结"}结果</b>（${esc(name)}，${messages.length} 条）\n\n${state.collapse ? `<blockquote expandable>${content}</blockquote>` : content}`,
    );
  });
const promptChange = (adding: boolean): CommandDefinition["handle"] =>
  guarded(async (invocation, context, _state, edit) => {
    ...
    await store(context).update(value => {
      ...
      if (adding) prompts[name] = prompt.slice(0, 10_000);
      else delete prompts[name];
      return { ...current, prompts, legacyImported: true };
    });
    await edit("提示词已更新");
  });
const collapse = (enabled: boolean): CommandDefinition["handle"] =>
  guarded(async (_invocation, context, state, edit) => {
    await store(context).update(value => ({
      ...normalize(value),
      collapse: enabled,
      aiMigrated: state.aiMigrated,
      legacyImported: true,
    }));
    await edit("折叠设置已更新");
  });
const central: CommandDefinition["handle"] = guarded(async (invocation, _context, _state, edit) => {
  await edit(
    `供应商、密钥和模型由 ai 插件统一管理，请使用 ${invocation.prefix}ai config 与 ${invocation.prefix}ai model chat。`,
  );
});
const command: CommandDefinition = {
  description: "使用统一 AI 服务汇总或分析引用来源",
  helpArgs: ["help", "h"],
  helpOnEmpty: true,
  args: "自定义提示词名 [数量|时间]",
  ...
  subcommands: {
    zj: {
      description: "总结引用来源消息，提取关键信息",
      args: "[数量|时间]",
      examples: [{ args: "zj" }, { args: "zj 50" }],
      handle: analyze("zj"),
    },
    fx: {
      description: "分析引用来源消息的观点和态度",
      args: "[数量|时间]",
      examples: [{ args: "fx 100" }, { args: "fx 2h" }],
      handle: analyze("fx"),
    },
    add: { description: "查看统一 AI 配置方式", handle: central },
    del: { description: "查看统一 AI 配置方式", handle: central },
    set: { description: "查看统一 AI 配置方式", handle: central },
    model: { description: "查看统一 AI 配置方式", handle: central },
    list: { description: "查看统一 AI 配置方式", handle: central },
    collapse: {
      description: "设置回答折叠，默认开启",
      subcommands: {
        on: { description: "开启折叠", handle: collapse(true) },
        off: { description: "关闭折叠", handle: collapse(false) },
      },
      handle: guarded(async () => {
        throw new Error("用法：collapse on|off");
      }),
    },
    prompt: {
      description: "管理自定义提示词",
      subcommands: {
        add: { description: "添加或更新提示词", args: "名称 内容", handle: promptChange(true) },
        del: { description: "删除自定义提示词", args: "名称", handle: promptChange(false) },
        list: {
          description: "列出内置和自定义提示词",
          handle: guarded(async (_i, _c, state, edit) =>
            edit([...Object.keys(BUILTIN), ...Object.keys(state.prompts)].join("\n")),
          ),
        },
      },
      handle: guarded(async () => {
        throw new Error("用法：prompt add|del|list");
      }),
    },
  },
  help: [
    {
      heading: "引用与范围：",
      body: "先引用用户消息，再用 zj 总结或 fx 分析；默认取服务器本地当天消息，支持 50 等数量或 2h/30m 等时间参数；最多 500 条、7 天和 100000 输入字符。",
    },
    {
      heading: "AI 配置：",
      body: "供应商、密钥、聊天模型和超时由 ai 插件统一管理；UAI 仅保留提示词与折叠设置。长结果会完整分页发送。",
    },
  ],
  handle: analyze(),
};
...
export default function createUai() {
  return definePlugin({
    ...
    id: "uai",
    description: "引用消息并使用统一 AI 服务汇总或分析历史消息",
    commands: { uai: command },
    settings: context => ({
      id: "uai",
      title: "UAI",
      description: "消息分析提示词与显示",
      category: "插件配置",
      icon: "🤖",
      getSchema: () => [
        { key: "prompts", label: "提示词", type: "prompt-map" },
        { key: "collapse", label: "折叠输出", type: "boolean" },
      ],
      async getValues() {
        const state = await migrateAi(context);
        return { prompts: state.prompts, collapse: state.collapse };
      },
      async setValues(patch) {
        await store(context).update(value => {
          const current = normalize(value);
          const prompts = patch.prompts === undefined ? current.prompts : patch.prompts;
          ...
          return {
            ...
            legacyImported: current.legacyImported,
            aiMigrated: current.aiMigrated,
          };
        });
      },
    }),
