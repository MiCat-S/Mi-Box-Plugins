...
async function load(ctx: PluginContext): Promise<Data> {
  const result = await ctx.http.withResponse(
    ...
    { headers: { accept: "application/json", "user-agent": "TeleBox/1.0" } },
    async (response, signal) => {
      ...
      try {
        for (;;) {
          ...
          if (total > 2 * 1024 * 1024) throw new Error("News response too large");
      ...
      try {
        return JSON.parse(Buffer.concat(chunks, total).toString("utf8")) as { data?: Data };
      } catch {
...
function pages(data: Data): string[] {
  ...
  if (data.poem) {
    ...
    if (!Array.isArray(data.poem.content) || data.poem.content.some(line => typeof line !== "string"))
  ...
  for (const block of blocks) {
    if (page && page.length + block.length + 1 > 3500) {
      result.push(page);
      page = "";
    }
    page += (page ? "\n" : "") + block;
...
export default function createNews() {
  return definePlugin({
    ...
    commands: {
      news: {
        helpArgs: ["help", "h"],
        description: "获取每日新闻资讯",
        async handle(invocation, ctx) {
          if (invocation.args[0] === "help" || invocation.args[0] === "h") {
            await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), { parseMode: "html" });
            return;
          }
          if (invocation.args.length) {
            await ctx.telegram.edit(invocation.message, `未知参数，请使用 ${invocation.prefix}news help 查看帮助`);
            return;
          }
          try {
            ...
            const delivery = await ui.deliverPages(output, ctx.signal, (page, index) =>
              index === 0
            ...
            if (delivery.interrupted) {
              ctx.log.error("news_result_delivery_failed");
              if (!delivery.published) throw delivery.error;
              try {
                await ctx.telegram.reply(invocation.message, ui.interruptedNotice(delivery));
              } catch {
                if (!ctx.signal.aborted) ctx.log.error("news_interrupted_notice_failed");
              }
            }
          } catch {
            if (!ctx.signal.aborted) {
              ctx.log.error("news_request_failed");
              try {
                await ctx.telegram.edit(invocation.message, "今日资讯获取失败，请稍后重试");
              } catch {
                ctx.log.error("news_failure_receipt_failed");
              }
            }
          }
        },
