...
function isMissingDateCrash(error: unknown): boolean {
  return error instanceof Error && error.message.includes("Cannot read properties of undefined") && error.message.includes("reading 'date'");
}
...
async function handle(message: MessageEnvelope, args: readonly string[], prefix: string, ctx: PluginContext) {
  await ctx.telegram.withClient(async (client: any, signal) => {
    ...
    if (!message.replyToId) {
      ...
      const sent = await client.sendMessage(chat, {message: `⚠️ 请回复一条消息以确定删除范围，或使用 \`${prefix}bd <数字>\` 删除您最近的消息。\n💡 当前删除他人权限: ${configured ? "开启" : "关闭"} (${prefix}bd on/off 切换)`});
      signal.throwIfAborted();
    ...
    const selected = messages.filter(item => item.id >= startId && item.id <= message.id &&
...
export default function createBulkDelete() {
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "bulk_delete", description: "回复消息后批量删除范围消息；bd <数字> 删除自己的最近消息；bd on/off 控制是否删除他人消息", commands: {
    bd: {helpArgs: ["help", "h"], description: "批量删除消息", async handle({message, args, prefix}, ctx) { await handle(message, args, prefix, ctx); }},
  }});
}
