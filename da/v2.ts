...
const failure = (kind: "permission" | "delete" | "task") =>
  kind === "permission" ? "权限检查失败" : kind === "delete" ? "部分消息删除失败" : "删除任务执行失败";
const MAX_WAIT_SEGMENT_MS = 60_000;
function floodMilliseconds(raw: string): number | undefined {
  try {
    const value = BigInt(raw) * 1000n;
    if (value < 0n || value > BigInt(Number.MAX_SAFE_INTEGER - Date.now())) return;
    return Number(value);
  } catch {
    return;
  }
...
export default function createDa(options: { sleep?: (ms: number, signal: AbortSignal) => Promise<void> } = {}) {
  ...
  const database = (ctx: PluginContext) =>
    ctx.storage.json<{ tasks: DeleteTask[]; imported: boolean }>("database.json", { tasks: [], imported: false });
  const save = async (ctx: PluginContext, task: DeleteTask) => {
    ...
    await database(ctx).update(data => {
      ...
      if (index < 0) data.tasks.push(task);
  ...
  const progress = async (ctx: PluginContext, task: DeleteTask, status: string) => {
    ...
    const state =
      task.sleepUntil && task.sleepUntil > Date.now()
        ? `休眠中 (${Math.ceil((task.sleepUntil - Date.now()) / 1000)}秒)`
        : task.isRunning
          ? "运行中"
          : task.isPaused
            ? "已暂停"
    ...
    try {
      await ctx.telegram.withClient(async (client, signal) => {
        ...
        if (task.savedMessageId) {
          try {
            await client.editMessage("me", { message: task.savedMessageId, text, parseMode: "html" });
            return;
          } catch (error) {
            signal.throwIfAborted();
      ...
      da: {
        ...
        async handle({ message, args, prefix }, ctx) {
          ...
          if (raw?.isPrivate || !message.chatId.startsWith("-")) {
            await ctx.telegram.edit(message, "仅群组可用");
            return;
          }
          ...
          if (!["true", "stop", "status"].includes(sub)) {
            await ctx.telegram.edit(message, "未知命令");
            return;
          }
          const id = message.chatId;
          const removeCommand = () =>
            ctx.telegram.withClient(async (client, signal) => {
          ...
          if (sub !== "true") {
            ...
            if (task) {
              ...
              await progress(
                ctx,
                task,
                sub === "status" ? "状态查询" : slot ? "正在停止，等待当前请求结束" : "已手动停止",
              );
            }
            await removeCommand();
            return;
          ...
          active.set(id, slot);
          void ctx.tasks
            .run(`da:delete:${id}`, async scoped => {
              const signal = AbortSignal.any([scoped, slot.controller.signal]);
              const wait = async (ms: number) => {
                let remaining = ms;
                while (remaining > 0) {
                  signal.throwIfAborted();
                  const segment = Math.min(remaining, MAX_WAIT_SEGMENT_MS);
                  await pause(segment, signal);
                  remaining -= segment;
                }
              };
              let completed = false;
              try {
                ...
                await ctx.telegram.withClient(async client => {
                  ...
                  if (chat.className === "Channel") {
                    try {
                      ...
                      admin = ["ChannelParticipantAdmin", "ChannelParticipantCreator"].includes(
                        result.participant.className,
                      );
                    } catch (error) {
                      signal.throwIfAborted();
                      ctx.log.error("da:permission");
                      try {
                        const result = await call(() =>
                          client.invoke(
                        ...
                        signal.throwIfAborted();
                        ctx.log.error("da:permission-fallback");
                      }
                    }
                  }
                    ...
                    while (true) {
                      ...
                      try {
                        ...
                        if (flood) {
                          ...
                          if (ms === undefined) {
                            task.errors.push(failure("delete"));
                  ...
                  if (admin) {
                    ...
                    while (true) {
                      ...
                      if (!Number.isFinite(next) || (offsetId > 0 && next >= offsetId))
                        throw new Error("DA search cursor did not advance");
                      offsetId = next;
                      const ids = page
                        .filter(m => m.className === "Message" && m.senderId?.toString() === me.id.toString())
                        .map(m => m.id);
                ...
                try {
                  if (slot.task && !ctx.signal.aborted) {
                    ...
                    if (completed && !task.errors.length)
                      await database(ctx).update(data => {
                        data.tasks = data.tasks.filter(t => t.chatId !== id);
                        return data;
                      });
                  }
                } finally {
