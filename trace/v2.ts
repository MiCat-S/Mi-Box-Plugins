...
type Data = {
  users: Record<string, Stored[]>;
  keywords: Record<string, Stored[]>;
  config: { keepLog: boolean; big: boolean };
};
...
for (const { segment } of emojiSegments.segment(
  "👍👎❤️🔥🥰👏😁🤔🤯😱🤬😢🎉🤩🤮💩🙏👌🕊🤡🥱🥴😍🐳❤️‍🔥🌚🌭💯🤣⚡️🍌🏆💔🤨😐🍓🍾💋🖕😈😎😇😤",
))
  standard.add(segment);
function normalize(items: Stored[]): Reaction[] {
  return items.map(item =>
    typeof item === "string" ? (/^[1-9]\d*$/.test(item) ? { documentId: item } : { emoticon: item }) : item,
  );
}
async function parse(message: MessageEnvelope, text: string, ctx: PluginContext): Promise<Reaction[]> {
  ...
  const custom = (raw?.entities ?? []).filter(
    (entity): entity is Api.MessageEntityCustomEmoji =>
      entity instanceof Api.MessageEntityCustomEmoji &&
      entity.offset >= offset &&
      entity.offset + entity.length <= offset + text.length,
  );
  if (custom.length) {
  ...
  for (const { segment, index } of emojiSegments.segment(text)) {
    ...
    if (entity) found.push({ documentId: entity.documentId.toString() });
      ...
      !custom.some(item => offset + index >= item.offset && offset + index < item.offset + item.length) &&
      standard.has(segment)
    )
      found.push({ emoticon: segment });
  }
...
async function receipt(ctx: PluginContext, message: MessageEnvelope, text: string) {
  ...
  void ctx.tasks
    .run("trace:receipt", async signal => {
    ...
    .catch(() => {
      if (!ctx.signal.aborted) ctx.log.error("trace.receipt_delete_failed");
    });
}
...
async function showStatus(ctx: PluginContext, message: MessageEnvelope, data: Data): Promise<void> {
  const lines = [
    ...
    ...Object.entries(data.keywords).map(
      ([keyword, items]) => `<code>${ui.text(keyword)}</code>：${ui.text(display(items))}`,
    ),
        ...
        async handle(invocation, ctx) {
          ...
          try {
            ...
            if (sub === "clean" || sub === "reset") {
              ...
              await receipt(ctx, invocation.message, sub === "reset" ? "自动回应已重置" : "追踪已清空");
              return;
            }
            if (sub === "log" || sub === "big") {
              ...
              await db.update(data => ({
                ...data,
                config: { ...data.config, [sub === "log" ? "keepLog" : "big"]: action === "true" },
              }));
              await receipt(ctx, invocation.message, "自动回应配置已更新");
              return;
            }
            if (sub === "kw") {
              if (!keyword || !["add", "del"].includes(action)) throw new Error("请提供关键词及 add/del 操作");
              const items =
                action === "add" ? await parse(invocation.message, invocation.args.slice(3).join(" "), ctx) : [];
              if (action === "add" && !items.length) throw new Error("未找到有效表情");
              await db.update(data => {
                const keywords = { ...data.keywords };
                if (action === "add") keywords[keyword] = items;
                else delete keywords[keyword];
                return { ...data, keywords };
              });
              await receipt(ctx, invocation.message, "关键词追踪已更新");
              return;
            }
            ...
            if (!ctx.signal.aborted)
              await ctx.telegram.edit(invocation.message, "自动回应操作失败，请检查参数、会员状态或会话权限");
          }
        },
      },
    },
    listeners: [
      {
        async handle(message, ctx) {
          ...
          const items =
            data.users[message.senderId] ??
            Object.entries(data.keywords).find(([keyword]) => message.text.includes(keyword))?.[1];
          if (!items?.length) return;
          try {
            await sendReactions(ctx, message, normalize(items), data.config.big);
          } catch {
            if (!ctx.signal.aborted) ctx.log.error("trace.reaction_failed");
          }
        },
      },
    ],
