...
function addSyntax(args: readonly string[]): { target: string; interval: string; rest: readonly string[] } {
  ...
  const dayOfWeekToken = (value: string | undefined) => {
    if (
      !value ||
      !/^(?:[0-9*?,/#-]+|(?:SUN|MON|TUE|WED|THU|FRI|SAT)(?:[-,/#](?:SUN|MON|TUE|WED|THU|FRI|SAT|[0-7*]))*)$/i.test(
        value,
      )
    )
      return false;
...
function providerUrl(provider: CustomProvider, type: "gemini" | "anthropic" | "openai-compatible"): string {
  ...
  url.hash = "";
  if (type === "gemini") {
    if (!/\/v1beta$/i.test(pathname)) pathname = pathname.replace(/\/v1$/i, "") + "/v1beta";
  } else if (!/\/v1$/i.test(pathname)) pathname += "/v1";
  url.pathname = pathname;
  return url.toString();
}
function providerType(provider: CustomProvider): {
  type: "gemini" | "anthropic" | "openai-compatible";
  responses: boolean;
} {
...
async function migrateAi(ctx: PluginContext): Promise<void> {
  ...
  if (!entries.length) {
    await store(ctx).update(value => {
      value.aiConfig.providers = {};
      value.aiConfig.aiMigrated = true;
      scrubLegacyAiSettings(value.aiConfig);
      return value;
    }, ctx.signal);
    return;
  }
  ...
  for (let index = 0; index < entries.length; index++) {
    const [name, provider] = entries[index]!;
    const safe = name
    ...
    const imported = await ctx.services.call<{ tag?: string }>(
      "ai",
      "import_provider",
      {
        tag: requestedTag,
        url: providerUrl(provider, detected.type),
        key: provider.api_key,
        type: detected.type,
        responses: detected.responses,
        models: { chat: provider.model },
        select: name === db.aiConfig.default_provider ? ["chat"] : [],
      },
      ctx.signal,
    );
  ...
  await store(ctx).update(value => {
    value.tasks = value.tasks.map(task =>
      task.aiProvider && tags[task.aiProvider] ? { ...task, aiProvider: tags[task.aiProvider] } : task,
    );
    value.aiConfig.providers = {};
    value.aiConfig.aiMigrated = true;
    scrubLegacyAiSettings(value.aiConfig);
...
async function readMessages(
  ...
  caller: AbortSignal,
  timeRange?: number,
): Promise<{ data: MessageData[]; title: string }> {
  return ctx.telegram.withClient(async (client, signal) => {
    ...
    for await (const item of client.iterMessages(entity, options)) {
      ...
      const sender =
        senderEntity?.title ||
        [senderEntity?.firstName, senderEntity?.lastName].filter(Boolean).join(" ") ||
        item.senderId?.toString() ||
        "未知发送者";
      rows.push({
        text: `[${sender}] ${content}`,
        content,
        telegramLink: buildMessageLink(chatId, item.id, username),
...
async function summarize(
  ctx: PluginContext,
  task: Pick<SummaryTask, "chatId" | "messageCount" | "timeRange" | "aiProvider" | "aiPrompt" | "useSpoiler">,
  beforeId?: number,
  ...
  ctx.log.info("sum:request", {
    messages: messages.data.length,
    inputChars: messages.data.reduce((n, item) => n + item.content.length, 0),
  });
  let output = await ctx.services.call<string>(
    "ai",
    "chat",
    {
      text: formatMessagesForAI(messages.data),
      systemPrompt: task.aiPrompt || db.aiConfig.default_prompt || DEFAULT_PROMPT,
      maxOutputTokens: 2000,
      fallbackToChatCompletions: true,
      ...(task.aiProvider ? { tag: task.aiProvider } : {}),
    },
    signal,
  );
  ...
  if (task.useSpoiler === true && !output.includes("<blockquote expandable>"))
...
async function sendPages(ctx: PluginContext, message: MessageEnvelope, html: string): Promise<void> {
  ...
  if (delivery.interrupted) {
    ...
    try {
      ...
      if (!ctx.signal.aborted) ctx.log.error("sum:output-notice-failed");
    }
  }
}
...
async function pushSummary(ctx: PluginContext, task: SummaryTask, signal: AbortSignal): Promise<void> {
  ...
  await ctx.telegram.withClient(async (client, active) => {
    ...
    for (const page of htmlPages(result.html)) {
      active.throwIfAborted();
      await client.sendMessage(peer as never, {
        message: page,
        parseMode: "html",
        linkPreview: db.aiConfig.link_preview === true,
      });
      active.throwIfAborted();
    }
  });
...
export default function createSum() {
  ...
  const registerUnlocked = async (ctx: PluginContext, task: SummaryTask): Promise<void> => {
    ...
    const dispose = await ctx.jobs.register(
      `task-${task.id}`,
      { cron: task.cron, description: `群消息总结 ${task.id}` },
      async signal => {
      ...
      try {
        ...
        if (!ctx.signal.aborted)
          await ctx.telegram.edit(
            invocation.message,
            `❌ ${error instanceof UserError ? error.message : "摘要操作失败，请检查配置、权限和网络后重试"}`,
          );
      }
    };
  const configured = (
    operation: (i: CommandInvocation, ctx: PluginContext, db: SummaryDB) => Promise<void>,
  ): CommandDefinition["handle"] =>
    guarded(async (i, ctx) => {
      const db = await store(ctx).read(ctx.signal);
      await operation(i, ctx, db);
    });
  const confirmed = async (i: CommandInvocation, ctx: PluginContext) => {
    await ctx.telegram.edit(i.message, "✅ 摘要配置已更新");
  };
  const configList = configured(async (invocation, ctx, db) => {
    await migrateAi(ctx);
    let selected = "请先安装并配置 ai 插件";
    if (ctx.services.available("ai", "selection")) {
      const value = await ctx.services.call<{ chat?: { tag?: string; model?: string } }>(
        "ai",
        "selection",
        null,
        ctx.signal,
    ...
    await ctx.telegram.edit(
      invocation.message,
      `<b>摘要配置</b>\nAI：<code>${htmlEscape(selected)}</code>\n提示词：${db.aiConfig.default_prompt === DEFAULT_PROMPT || !db.aiConfig.default_prompt ? "内置" : "自定义"}\n链接预览：${db.aiConfig.link_preview ? "on" : "off"}`,
      { parseMode: "html" },
    );
    return;
  });
  const switchSetting = (field: "link_preview" | "default_spoiler", value: boolean): CommandDefinition["handle"] =>
    configured(async (i, ctx) => {
      await store(ctx).update(data => {
        data.aiConfig[field] = value;
        return data;
      }, ctx.signal);
  ...
  const switches = (field: "link_preview" | "default_spoiler"): SubcommandDefinition => ({
    ...
    subcommands: {
      on: { description: "开启", args: "", handle: switchSetting(field, true) },
      off: { description: "关闭", args: "", handle: switchSetting(field, false) },
    },
    handle: configured(async () => {
      throw new UserError("请输入 on 或 off");
    }),
  });
  const prompt = (reset: boolean): CommandDefinition["handle"] =>
    configured(async (i, ctx) => {
      const value = reset ? DEFAULT_PROMPT : i.args.join(" ").trim();
      requireValue(value, "提示词不能为空");
      await store(ctx).update(data => {
        data.aiConfig.default_prompt = value;
        return data;
      }, ctx.signal);
      await confirmed(i, ctx);
    });
  ...
  const manageTask = (sub: "run" | "del" | "disable" | "enable"): CommandDefinition["handle"] =>
    guarded(async (invocation, ctx) => {
        ...
        await withTaskLock(id, async () => {
          ...
          if (sub === "enable") {
            ...
            await store(ctx).update(data => {
              ...
              requireValue(index >= 0, "摘要任务不存在");
              if (sub === "del") data.tasks.splice(index, 1);
              else data.tasks[index]!.disabled = true;
              return data;
            }, ctx.signal);
        });
        try {
          await ctx.telegram.edit(invocation.message, "✅ 摘要任务已更新");
        } catch {
          if (!ctx.signal.aborted) ctx.log.error("sum:receipt-failed");
        }
        return;
      }
      const db = await store(ctx).read(ctx.signal);
      ...
      requireValue(task, "摘要任务不存在");
      if (sub === "run") {
        await ctx.telegram.edit(invocation.message, "📝 正在生成摘要...");
        await pushSummary(ctx, task, ctx.signal);
        try {
          await ctx.telegram.edit(invocation.message, "✅ 摘要已推送");
        } catch {
          if (!ctx.signal.aborted) ctx.log.error("sum:receipt-failed");
        }
        return;
  ...
  const command: CommandDefinition = {
    description: "群消息即时与定时摘要",
    helpArgs: ["help", "h", "?"],
    args: "[消息数] [--provider ai标签]",
    subcommandsCaseSensitive: false,
    examples: [
      { args: "", description: "总结当前群最近 100 条文字消息" },
      { args: "200" },
      { args: "100 --provider main" },
    ],
    subcommands: {
      list: {
        description: "查看定时摘要任务",
        aliases: ["ls"],
        args: "",
        handle: guarded(async (invocation, ctx) => {
          ...
          const text =
            db.tasks
              .map(
                task =>
                  `• <code>${htmlEscape(task.id)}</code> · ${htmlEscape(task.remark || task.chatDisplay || task.chatId)} · ${htmlEscape(task.interval)} · ${task.timeRange ? `过去 ${task.timeRange} 小时` : `${task.messageCount} 条`} · ${task.disabled ? "停用" : "启用"} · 推送 ${htmlEscape(task.pushTarget || db.defaultPushTarget || "me")}`,
              )
              .join("\n") || "暂无定时任务";
      ...
      add: {
        description: "创建定时摘要，推送到收藏夹",
        args: "here|群组 间隔 [消息数]",
        examples: [{ args: "add here 2h 100" }, { args: "add here 30m 200" }, { args: 'add here "30 */2 * * *" 100' }],
        help: [
          {
            heading: "间隔：",
            body: "分钟间隔为 1–59；小时间隔为 1–23；按天仅支持 1d。也支持五或六字段 Cron，表达式可加引号。消息数默认 100，范围 10–500。可用 --time、--provider、--spoiler 和备注。",
          },
        ],
        handle: guarded(async (invocation, ctx) => {
          ...
          for (let index = 0; index < syntax.rest.length; index++) {
            ...
            if (value === "--time" || value === "--provider") {
              const next = syntax.rest[++index];
              requireValue(next, `请提供 ${value} 的值`);
              if (value === "--time") {
                timeRange = Number(next);
                requireValue(
                  Number.isSafeInteger(timeRange) && timeRange > 0 && timeRange <= 720,
                  "时间范围须为 1-720 小时",
                );
              } else provider = next;
          ...
          await store(ctx).update(data => {
            ...
            task = {
              ...
              interval,
              messageCount: count,
              pushTarget: data.defaultPushTarget || "me",
              createdAt: new Date().toISOString(),
              ...(timeRange ? { timeRange } : {}),
              ...(provider ? { aiProvider: provider } : {}),
              ...(remark ? { remark } : {}),
              useSpoiler: spoiler ?? data.aiConfig.default_spoiler === true,
            };
          ...
          try {
            await withTaskLock(task.id, () => registerUnlocked(ctx, task!));
          } catch (error) {
            await store(ctx).update(data => {
          ...
          try {
            await ctx.telegram.edit(invocation.message, `✅ 已创建摘要任务 ${task.id}`);
          } catch {
            if (!ctx.signal.aborted) ctx.log.error("sum:receipt-failed");
          }
          return;
      ...
      config: {
        ...
        subcommands: {
          ...
          set: {
            description: "修改摘要显示，或查看统一 AI 配置方式",
            args: "preview|spoiler|prompt ...",
            subcommands: {
              default: { description: "查看统一 AI 配置方式", handle: centralConfig },
              preview: switches("link_preview"),
              spoiler: switches("default_spoiler"),
              push: {
                description: "设置定时摘要默认推送目标",
                args: "目标",
                handle: configured(async (i, ctx) => {
                  const value = i.args.join(" ").trim();
                  requireValue(value, "推送目标不能为空");
                  await store(ctx).update(data => {
                    data.defaultPushTarget = value;
                    return data;
                  }, ctx.signal);
                  await confirmed(i, ctx);
                }),
              ...
              prompt: {
                description: "设置默认摘要提示词",
                args: "内容",
                subcommands: {
                  show: {
                    description: "查看当前实际提示词",
                    args: "",
                    handle: configured(async (i, ctx, db) => {
                      await sendPages(
                        ctx,
                        i.message,
        ...
        handle: configured(async () => {
          throw new UserError("未知 config 子命令");
        }),
      },
    },
    help: [
      {
        heading: "即时与定时：",
        body: "即时总结默认取当前群命令之前的最近 100 条文字消息，范围 10–500；带媒体的文字说明会作为文字处理。可用 --provider 临时指定 ai 插件中的标签。定时任务默认发往收藏夹。长结果自动分段；即时结果关闭链接预览，定时结果按 preview 配置。",
      },
      {
        heading: "AI 配置：",
        body: "供应商、密钥、聊天模型、思考强度、服务等级和超时由 ai 插件统一管理；sum 仅保留摘要提示词与显示设置。",
      },
    ],
    handle: guarded(async (invocation, ctx) => {
  ...
  return definePlugin({
    ...
    async setup(ctx) {
      await store(ctx).update(data => {
        ...
        if (!value.aiConfig || typeof value.aiConfig !== "object" || Array.isArray(value.aiConfig))
          value.aiConfig = { ...defaults.aiConfig, providers: {} };
        if (
          !value.aiConfig.providers ||
          typeof value.aiConfig.providers !== "object" ||
          Array.isArray(value.aiConfig.providers)
        )
          value.aiConfig.providers = {};
        if (value.aiConfig.aiMigrated !== true && Object.keys(value.aiConfig.providers).length === 0)
          value.aiConfig.aiMigrated = true;
      ...
      for (const task of db.tasks) {
        try {
          ...
          ctx.log.error("sum:register-failed", { taskId: task.id });
        }
      }
    },
    async cleanup() {
