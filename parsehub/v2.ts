...
function textPages(source: string): string[] {
  const budget = ui.MAX_HTML_LENGTH - ui.PAGE_LABEL_RESERVE,
    result: string[] = [];
  let page = "";
  for (const character of source) {
    if (page && page.length + character.length > budget) {
      result.push(page);
      page = "";
    }
    page += character;
...
function botMessages(messages: unknown): any[] {
  return (Array.isArray(messages) ? messages : []).filter(
    message => message && (message as { className?: unknown }).className !== "MessageService" && messageId(message) > 0,
  );
}
...
async function latestBotMessageId(client: any, signal: AbortSignal, incomingOnly = false): Promise<number> {
  ...
  signal.throwIfAborted();
  return (incomingOnly ? incoming(history) : history).reduce(
    (latest, message) => Math.max(latest, messageId(message)),
    0,
  );
}
...
async function advanceBoundary(context: PluginContext, value: number, initialized?: boolean): Promise<void> {
  await store(context).update(source => {
    ...
    return {
      ...
      ignoredUpToId: Math.max(state.ignoredUpToId, value),
    };
  });
}
...
async function ensureBotReady(client: any, signal: AbortSignal): Promise<{ ready: boolean; boundary: number }> {
  ...
  try {
    ...
    signal.throwIfAborted();
    const id =
      entity?.id !== undefined && entity?.accessHash !== undefined
        ? new Api.InputUser({ userId: entity.id, accessHash: entity.accessHash })
        : BOT;
...
async function relay(
  ...
  try {
    requestBoundary = Math.max(requestBoundary, messageId(await client.sendMessage(BOT, { message: link })));
    signal.throwIfAborted();
  } catch {
    signal.throwIfAborted();
    return { lastId: baseline, forwarded: false, reason: "send_failed" };
  }
  ...
  while (Date.now() < deadline) {
    ...
    let messages: any[];
    try {
      messages = incoming(await client.getMessages(BOT, { limit: FETCH_LIMIT }));
    } catch {
      signal.throwIfAborted();
      return { lastId, forwarded: false, reason: "fetch_failed" };
    }
    signal.throwIfAborted();
  ...
  for (let offset = 0; offset < messages.length; offset += 100) {
    ...
    try {
      await client.forwardMessages(target.peer, {
        fromPeer: BOT,
        messages: chunk.map(messageId),
        dropAuthor: true,
        replyTo: target.replyTo,
        ...(target.topMsgId === undefined ? {} : { topMsgId: target.topMsgId }),
      });
      signal.throwIfAborted();
    } catch {
      signal.throwIfAborted();
      ...
      try {
        ...
        for (const [index, page] of pages.entries()) {
          await client.sendMessage(target.peer, {
            message: page + ui.pageLabel(index, pages.length),
            parseMode: false,
            replyTo: target.replyTo,
            ...(target.topMsgId === undefined ? {} : { topMsgId: target.topMsgId }),
          });
...
export default function createParsehub() {
  ...
  const command: CommandDefinition = {
    ...
    help: [
      {
        heading: "说明：",
        body: "链接会发送给第三方 Telegram 机器人 @ParseHubot；支持抖音、哔哩哔哩、YouTube、TikTok、小红书、Twitter、贴吧、Facebook、微博和 Instagram 等平台。存在多条链接时处理第一条。",
      },
      {
        heading: "会话：",
        body: "所有聊天共享一个机器人会话队列，避免并发解析结果串线；每条链接最长等待 10 分钟，进度仍活跃时最多延长到 30 分钟。",
      },
    ],
    async handle(invocation, context) {
      ...
      if (!links.length) {
        await context.telegram.edit(
          invocation.message,
          renderCommandHelp("parsehub", command, { prefix: invocation.prefix, title: "🔗 ParseHub 解析" }),
          { parseMode: "html" },
        );
        return;
      }
      ...
      try {
        completed = await serialized(context, () =>
          context.telegram.withClient(async (client, signal) => {
            ...
            for (const link of links) {
              ...
              if (outcome.forwarded) success += 1;
                ...
                await context.telegram.reply(
                  invocation.message,
                  `⚠️ <b>解析未完成</b>\n${escape(link)}\n${escape(reasonText(outcome.reason))}`,
      ...
      try {
        ...
        context.signal.throwIfAborted();
        context.log.error("parsehub_receipt_failed", { kind: "internal" });
      }
      context.signal.throwIfAborted();
      ...
      if (typeof raw?.delete === "function")
        try {
          ...
          context.signal.throwIfAborted();
          context.log.error("parsehub_cleanup_failed", { kind: "internal" });
        }
    },
  };
