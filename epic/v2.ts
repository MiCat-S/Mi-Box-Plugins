...
function readChunk(reader: ReadableStreamDefaultReader<Uint8Array>, signal: AbortSignal, cancel: () => Promise<void>): Promise<ReadResult> {
...
function games(data: unknown): Game[] {
  ...
  for (const value of elements) {
    ...
    result.push({
      ...
      originalPrice: typeof price?.fmtPrice?.originalPrice === "string" && price.fmtPrice.originalPrice ? price.fmtPrice.originalPrice : "免费",
...
function date(value: string): string {
  ...
  return Number.isNaN(parsed.valueOf()) ? "时间未知" : parsed.toLocaleString("zh-CN", {timeZone: "Asia/Shanghai", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit"});
}
...
async function render(items: readonly Game[]): Promise<readonly string[]> {
  ...
  for (const block of blocks) {
    if (page.length + block.length + 2 > limit) { pages.push(page); page = block; }
    else page += `\n\n${block}`;
  }
...
async function query(context: PluginContext): Promise<Game[]> {
  const data = await context.http.withResponse(ENDPOINT, {
    ...
    const cancel = () => cancelPromise ??= reader.cancel().then(() => undefined, () => undefined);
    ...
    commands: {epic: {helpArgs: ["help","h"], description: "查看 Epic Games 当前限免游戏", async handle(invocation, context) {
      ...
      try {
        ...
        if (delivery.interrupted) {
          context.log.info("pagination_delivery_interrupted", {plugin: "epic", published: delivery.published, total: delivery.total, category: ui.deliveryErrorCategory(delivery.error)});
