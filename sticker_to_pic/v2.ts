...
function options(args: readonly string[]): {format: "jpg" | "png"; transparent: boolean; document: boolean} | undefined {
  ...
  const values = args.map(value => value.toLowerCase()), sub = values[0];
  if (sub === "png" && values.every(value => value === "png" || value === "transparent")) return {format: "png", transparent: values.includes("transparent"), document: false};
...
async function download(context: PluginContext, media: unknown, target: string, signal: AbortSignal): Promise<void> {
  await context.telegram.withClient(async (client: any, clientSignal) => {
    const file = await open(target, "wx", 0o600); let size = 0;
    try {
      for await (const chunk of client.iterDownload(media, {signal: clientSignal})) {
        signal.throwIfAborted(); clientSignal.throwIfAborted(); size += chunk.length;
        if (size > MAX_MEDIA_BYTES) throw new Error("Sticker too large");
        let offset = 0; while (offset < chunk.length) { signal.throwIfAborted(); const result = await file.write(chunk, offset, chunk.length - offset); signal.throwIfAborted(); if (!result.bytesWritten) throw new Error("Sticker write failed"); offset += result.bytesWritten; }
      }
...
async function validateWebp(target: string, signal: AbortSignal): Promise<void> {
  const file = await open(target, "r"); const header = Buffer.alloc(12);
  try { const result = await file.read(header, 0, header.length, 0); signal.throwIfAborted(); if (result.bytesRead !== 12 || header.toString("ascii", 0, 4) !== "RIFF" || header.toString("ascii", 8, 12) !== "WEBP") throw new Error("Invalid WebP"); }
  finally { await file.close(); }
}

async function withBusinessTemp(context: PluginContext, use: (directory: string, signal: AbortSignal) => Promise<void>): Promise<void> {
  let completed = false;
  try { await context.files.withTemp(async (directory, signal) => { await use(directory, signal); completed = true; }); }
  catch (error) { if (context.signal.aborted || !completed) throw error; context.log.error("sticker_to_pic_temp_cleanup_failed"); }
}
...
export default function createStickerToPic() {
  const command = {description: "将回复的静态贴纸转换为图片", async handle(invocation: any, context: PluginContext) {
    ...
    if (sub === "help" || sub === "h") { await context.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode: "html"}); return; }
    if (sub === "check") {
      try {
    ...
    if (!selected) { await context.telegram.edit(invocation.message, `❌ <b>未知子命令:</b> <code>${escape(sub ?? "")}</code>\n\n请使用 <code>${escape(invocation.prefix)}stp help</code> 查看可用选项`, {parseMode: "html"}); return; }
    ...
    try {
      ...
      if (document.size != null) {
        let declared: bigint; try { declared = BigInt(document.size.toString()); } catch { await context.telegram.edit(invocation.message, "贴纸文件大小无效"); return; }
        if (declared <= 0n || declared > BigInt(MAX_MEDIA_BYTES)) { await context.telegram.edit(invocation.message, "贴纸文件不能超过 20 MiB"); return; }
      }
      ...
      await withBusinessTemp(context, async (directory, signal) => {
        ...
        await context.telegram.edit(invocation.message, `🔄 正在转换为${selected.format.toUpperCase()}格式...`, {parseMode: "html"});
        const flatten = selected.transparent ? [] : selected.format === "png"
          ? ["-background", "white", "-alpha", "remove"]
          : ["-background", "white", "-alpha", "remove", "-alpha", "off"];
        const limits = ["-limit", "width", "512", "-limit", "height", "512", "-limit", "memory", "64MiB", "-limit", "map", "128MiB", "-limit", "disk", "256MiB"];
        ...
        await context.telegram.withClient(async (client, clientSignal) => {
          ...
          await client.sendFile(raw.peerId, {file: output, replyTo: invocation.message.replyToId,
            forceDocument: selected.document, parseMode: "html", caption: `${selected.document ? "📄" : "🖼️"} <b>贴纸已转换为${selected.format.toUpperCase()}格式${selected.document ? "（原图）" : ""}</b>${selected.transparent ? "（透明背景）" : ""}`});
          signal.throwIfAborted(); clientSignal.throwIfAborted();
          try { await client.deleteMessages(raw.peerId, [invocation.message.id], {revoke: true}); }
          catch { signal.throwIfAborted(); clientSignal.throwIfAborted(); deleteFailed = true; context.log.error("sticker_to_pic_delete_failed"); }
        });
        if (deleteFailed) try { await context.telegram.edit(invocation.message, "图片已发送，命令消息删除失败"); }
        catch { context.log.error("sticker_to_pic_receipt_failed"); }
      });
    } catch {
