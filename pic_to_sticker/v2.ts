...
async function writeAll(file:Awaited<ReturnType<typeof open>>,chunk:Uint8Array,signal:AbortSignal){let offset=0;while(offset<chunk.length){signal.throwIfAborted();const result=await file.write(chunk,offset,chunk.length-offset);signal.throwIfAborted();if(result.bytesWritten<=0)throw new Error("Image write failed");offset+=result.bytesWritten;}}
...
function normalize(value: Record<string, unknown>): Config {
  ...
  return {...value, schemaVersion: 1,
    defaultEmoji: typeof value.defaultEmoji === "string" && value.defaultEmoji.trim() ? Array.from(value.defaultEmoji).slice(0, 32).join("") : defaults.defaultEmoji,
...
async function convert(context: PluginContext, source: ApiTypes.Message, config: Config, emoji: string,
  send: (output: string, signal: AbortSignal) => Promise<void>): Promise<void> {
  ...
  try { await context.files.withTemp(async (directory, signal) => {
    ...
    await operation.toFile(output); signal.throwIfAborted();
...
const convertReply = (batch: boolean): CommandDefinition["handle"] => async (invocation, context) => {
    if (batch && invocation.message.replyToId === undefined) { await context.telegram.edit(invocation.message, help(invocation.prefix), {parseMode: "html"}); return; }
    let delivered = 0;
    try {
      ...
      context.signal.throwIfAborted();
      const selected = batch ? config.defaultEmoji : Array.from(invocation.args[0] || config.defaultEmoji).slice(0, 32).join("");
      const candidates: ApiTypes.Message[] = [source];
      if (batch && (source as any).groupedId) {
        const groups = await context.telegram.withClient(async (client, clientSignal) => {const signal=AbortSignal.any([context.signal,clientSignal]);const result=await Promise.all([
      ...
      await context.telegram.edit(invocation.message,batch?"🔄 正在批量处理图片...":"🔍 正在分析图片...",{parseMode:"html"});
      for (const item of candidates.slice(0, 20)) {
        try {
          await convert(context, item, config, selected, async (output, tempSignal) => context.telegram.withClient(async (client, clientSignal) => {
            const signal=AbortSignal.any([context.signal,tempSignal,clientSignal]);signal.throwIfAborted();
            ...
            await client.sendFile(raw.peerId!, {file: output,
              attributes: [new Api.DocumentAttributeSticker({alt: selected, stickerset: new Api.InputStickerSetEmpty()})], replyTo: invocation.message.id, topMsgId: invocation.message.topicId});
            signal.throwIfAborted();
          }));
          completed++; delivered++;
        } catch (error) { context.signal.throwIfAborted(); if(error instanceof DeliveredCleanupError){completed++;delivered++;context.log.error("pic_to_sticker_temp_cleanup_failed");}else failed++; }
      }
      if (!completed) throw new Error("No image converted");
      if (batch) await context.telegram.edit(invocation.message, `✅ <b>批量转换完成</b>\n\n成功: ${completed} 张\n失败: ${failed} 张`, {parseMode: "html"});
      if (config.autoDelete && failed === 0 && typeof raw.delete === "function") {
        if(batch)await sleep(3000,undefined,{signal:context.signal});
        context.signal.throwIfAborted();
        try { await raw.delete({revoke: true}); context.signal.throwIfAborted(); }
        catch { context.signal.throwIfAborted(); context.log.error("pic_to_sticker_command_cleanup_failed"); }
      } else if(!batch)await context.telegram.edit(invocation.message, `✅ 贴纸已发送 ${escape(selected)}`, {parseMode: "html"});
    } catch {
      if (context.signal.aborted) return;
      if(delivered){context.log.error("pic_to_sticker_receipt_failed");return;}
...
const command: CommandDefinition = {
  ...
  subcommands: {
    ...
    config: {description: "查看或修改转换配置", args: "", subcommands: {
      emoji: {description: "设置默认表情", args: "表情", examples: [{args: "emoji 🔥"}], handle: configure(value => value ? {defaultEmoji: Array.from(value).slice(0,32).join("")} : undefined)},
...
export default function createPicToSticker() {
  return definePlugin({renderHelp: help, apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "pic_to_sticker", description: "将图片转换为贴纸", commands: {pic_to_sticker: command, pts: command},
    settings: context => ({title: "图片转贴纸", description: "贴纸转换配置", category: "插件配置", icon: "🖼️",
      ...
      async getValues(signal) { return (await configuration(context,signal)).current; },
      async setValues(patch, signal) { signal.throwIfAborted(); await (await configuration(context,signal)).store.update(value => normalize({...value, ...patch}),signal); }}),
  });
}
