...
const canonicalTime = (value:string):string => {const [hour,minute]=value.split(":");return `${hour!.padStart(2,"0")}:${minute}`;};
...
function normalize(value:any):State {
  ...
  return {...value,schemaVersion:SCHEMA_VERSION,runTime:time(value?.runTime)?canonicalTime(value.runTime):base.runTime,
    ...(time(value?.runTimeEnd)?{runTimeEnd:canonicalTime(value.runTimeEnd)}:{}),randomDelay:integer(value?.randomDelay,0,60,0),
...
function withoutExecution(state:State):State {const {execution: _execution,...rest}=state;return rest;}
...
export default function createCheckin(){
  ...
  const poll=async(ctx:PluginContext,client:any,peer:string,minDate:number,sentId:number,wanted:Target,button:boolean,signal:AbortSignal)=>{const deadline=Date.now()+POLL_TIMEOUT_MS;while(Date.now()<deadline){await abortableDelay(POLL_MS,signal);signal.throwIfAborted();const messages=await client.getMessages(peer,{limit:8});signal.throwIfAborted();for(const message of messages){signal.throwIfAborted();if(message.out||Number(message.date??0)<minDate||Number(message.id??0)<=sentId)continue;if(!button||callback(message,wanted))return message;}}};
  const single=async(ctx:PluginContext,wanted:Target,signal:AbortSignal):Promise<Result>=>ctx.telegram.withClient(async(client,clientSignal)=>{const active=AbortSignal.any([signal,clientSignal]);active.throwIfAborted();const sent=await client.sendMessage(wanted.target,{message:wanted.command});active.throwIfAborted();const id=Number(sent?.id??0),date=Math.floor(Date.now()/1000);const hasButton=!!(wanted.callbackData||wanted.buttonText);const first=await poll(ctx,client,wanted.target,date,id,wanted,hasButton,active);active.throwIfAborted();if(!first)return{success:false,message:"未收到签到结果"};if(!hasButton)return{success:true,message:String(first.message||"签到命令已发送")};const data=callback(first,wanted);if(!data)return{success:false,message:"未找到签到按钮"};const {Api}=await import("teleproto");active.throwIfAborted();await client.invoke(new Api.messages.GetBotCallbackAnswer({peer:wanted.target,msgId:first.id,data}));active.throwIfAborted();const second=await poll(ctx,client,wanted.target,Math.floor(Date.now()/1000),Number(first.id??0),wanted,false,active);active.throwIfAborted();return{success:true,message:String(second?.message||first.message||"已点击签到按钮")};});
  const pages=async(html:string):Promise<string[]> => (await ui.renderRichText(html,ui.PAGE_LABEL_RESERVE)).map((page,index,all)=>page+ui.pageLabel(index,all.length));
  const deliver=async(ctx:PluginContext,message:MessageEnvelope,html:string):Promise<void>=>{const output=await pages(html);const result=await ui.deliverPages(output,ctx.signal,(page,index)=>index?ctx.telegram.reply(message,page,{parseMode:"html"}):ctx.telegram.edit(message,page,{parseMode:"html"}));if(result.interrupted&&!result.published)throw result.error;};
  ...
  const all=async(ctx:PluginContext,source:string,fallback:string|undefined,signal:AbortSignal,scheduledDate?:string)=>{
    const state=normalize(await store(ctx).read()),enabled=state.targets.filter(x=>x.enabled);signal.throwIfAborted();if(!enabled.length)throw new Error("没有启用的签到目标");
    const results:Array<{target:Target;result:Result}>=[];
    for(const [index,item] of enabled.entries()){
      signal.throwIfAborted();
      if(scheduledDate){const saved=normalize(await store(ctx).read()).execution?.targets?.[item.id];signal.throwIfAborted();if(saved){results.push({target:item,result:saved.status==="sent"?{success:saved.success===true,message:saved.message||"已执行"}:{success:false,message:"上次执行状态未确认，已避免重复发送"}});continue;}await store(ctx).update(raw=>{const value=normalize(raw),execution=value.execution;if(!execution||execution.date!==scheduledDate)return value;return{...value,execution:{...execution,targets:{...execution.targets,[item.id]:{status:"prepared"}}}};});signal.throwIfAborted();}
      let result:Result;try{result=await single(ctx,item,signal);}catch(error){signal.throwIfAborted();if(isAbort(error))throw error;result={success:false,message:safeError(error)};}results.push({target:item,result});
      if(scheduledDate){await store(ctx).update(raw=>{const value=normalize(raw),execution=value.execution;if(!execution||execution.date!==scheduledDate)return value;return{...value,execution:{...execution,targets:{...execution.targets,[item.id]:{status:"sent",success:result.success,message:result.message.slice(0,500)}}}};});signal.throwIfAborted();}
      if(index+1<enabled.length)await abortableDelay(2_000,signal);
    }
    const ok=results.filter(x=>x.result.success).length,renderedTime=new Date().toLocaleString("zh-CN",{timeZone:TIME_ZONE});
  ...
  const command=async(message:MessageEnvelope,args:readonly string[],prefix:string,ctx:PluginContext)=>{
    ...
    if(!action){
      ...
      if(!state.targets.length){await ctx.telegram.edit(message,renderPluginHelp(prefix),{parseMode:"html",linkPreview:false});return;}
      try{await ctx.telegram.edit(message,"🚀 开始执行签到任务…");const result=await serial(()=>all(ctx,"手动触发",message.chatId,ctx.signal));await ctx.telegram.edit(message,`✅ 已执行 ${result.total} 个任务`);}catch(error){ctx.signal.throwIfAborted();if(isAbort(error))throw error;await ctx.telegram.edit(message,`❌ ${esc(error instanceof Error?error.message:"执行失败")}`,{parseMode:"html"});}return;
    }
    if(["help","h"].includes(action)){await ctx.telegram.edit(message,help(prefix),{parseMode:"html"});return;}
    if(action==="add"){
      const [id,name,peer]=args.slice(1,4);if(!id||!name||!peer){await ctx.telegram.edit(message,help(prefix),{parseMode:"html"});return;}
      const match=matcher(args.slice(4));
      const prompt=["📝 请<b>回复此消息</b>，发送签到命令（支持空格和特殊字符）","",`ID: ${esc(id)}`,`名称: ${esc(name)}`,`目标: ${esc(peer)}`,match.callbackData?`回调: ${esc(match.callbackData)}`:"",match.buttonText?`按钮: ${esc(match.buttonText)}`:""].filter(Boolean).join("\n");
      ...
      ctx.signal.throwIfAborted();
      await store(ctx).update(raw=>{const state=normalize(raw);return{...state,pending:{...state.pending,[message.chatId]:{promptId:Number(sent.id),id,name,target:peer,...match}}};});return;
    }
    if(action==="list"){
      const state=normalize(await store(ctx).read()),enabled=state.targets.filter(x=>x.enabled).length;
      const body=state.targets.map((x,i)=>{const detail=x.callbackData?`\n   回调: ${esc(x.callbackData)}`:x.buttonText?`\n   按钮: ${esc(x.buttonText)}`:"";return`${x.enabled?"🟢":"🔴"} <b>${i+1}. ${esc(x.name)}</b>\n   ID: ${esc(x.id)}\n   目标: ${esc(x.target)}\n   命令: ${esc(x.command)}${detail}`;}).join("\n\n");
      await deliver(ctx,message,state.targets.length?`📝 <b>签到目标列表</b> (${enabled}/${state.targets.length} 个启用)\n\n${body}`:"📝 当前没有配置签到目标");return;
    }
    if(["del","delete","toggle","test"].includes(action)){
      const id=args[1];if(!id){await ctx.telegram.edit(message,"请指定目标 ID");return;}const state=normalize(await store(ctx).read()),found=state.targets.find(x=>x.id===id);if(!found){await ctx.telegram.edit(message,"未找到签到目标");return;}
      if(action==="test"){await ctx.telegram.edit(message,`🚀 开始测试签到目标: ${esc(found.name)}...`,{parseMode:"html"});const result=await serial(()=>single(ctx,found,ctx.signal));await ctx.telegram.edit(message,result.success?`✅ <b>${esc(found.name)}</b> 测试成功\n\n结果: ${esc(result.message||"无")}`:`❌ <b>${esc(found.name)}</b> 测试失败\n\n错误: ${esc(result.message||"执行失败")}`,{parseMode:"html"});return;}
      await store(ctx).update(raw=>{const value=normalize(raw);return{...value,targets:action==="toggle"?value.targets.map(x=>x.id===id?{...x,enabled:!x.enabled}:x):value.targets.filter(x=>x.id!==id)};});await ctx.telegram.edit(message,"✅ 配置已更新");return;
    }
    if(action==="settings"||action==="info"){
      const state=normalize(await store(ctx).read());await ctx.telegram.edit(message,`⏰ ${state.runTime}${state.runTimeEnd?` ~ ${state.runTimeEnd}`:""}\n🎲 ${state.randomDelay} 分钟\n🤖 Bot: ${state.botToken?"已配置":"未配置"}\n📅 最近执行: ${esc(state.lastRunDate||"无")}\n🎯 ${state.targets.filter(x=>x.enabled).length}/${state.targets.length}`,{parseMode:"html"});return;
    }
    if(action==="reset"){await store(ctx).update(raw=>({...withoutExecution(normalize(raw)),lastRunDate:""}));await ctx.telegram.edit(message,"✅ 已重置每日运行状态");return;}
    if(action==="set"||action==="config"){
      const aliases:Record<string,string>={t:"time",r:"range",d:"delay",b:"bot",l:"log"};const rawField=(args[1]??"").toLowerCase(),field=aliases[rawField]??rawField,value=args[2];
      if(field==="time"&&time(value)){const canonical=canonicalTime(value);await store(ctx).update(raw=>({...withoutExecution(normalize(raw)),runTime:canonical}));await ctx.telegram.edit(message,`✅ 开始时间已设置为: ${canonical}`);return;}
      if(field==="range"){if(value&&!time(value)){await ctx.telegram.edit(message,"时间格式应为 HH:MM");return;}const canonical=value?canonicalTime(value):undefined;await store(ctx).update(raw=>{const state=withoutExecution(normalize(raw));if(canonical)return{...state,runTimeEnd:canonical};delete state.runTimeEnd;return state;});await ctx.telegram.edit(message,"✅ 时间范围已更新");return;}
      if(field==="delay"){const delay=Number(value);if(!Number.isInteger(delay)||delay<0||delay>60){await ctx.telegram.edit(message,"随机延迟应为 0-60 分钟");return;}await store(ctx).update(raw=>({...normalize(raw),randomDelay:delay}));await ctx.telegram.edit(message,"✅ 随机延迟已更新");return;}
      if(field==="bot"){if(!message.saved){await ctx.telegram.edit(message,"Bot Token 只能在收藏夹中设置");return;}if(!value||!args[3]){await ctx.telegram.edit(message,"请提供 Token 与 Chat ID");return;}await store(ctx).update(raw=>({...normalize(raw),botToken:value,pushChatId:args[3]!}));await ctx.telegram.edit(message,`✅ Bot 配置已更新：${esc(value.slice(0,5))}…`,{parseMode:"html"});return;}
      if(field==="log"&&value){await store(ctx).update(raw=>({...normalize(raw),logChat:value}));await ctx.telegram.edit(message,"✅ 日志对话已更新");return;}
    }
