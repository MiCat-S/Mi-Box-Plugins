...
export default function createWhois() {
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "whois", description: "查询域名注册信息",
    ...
    whois: {helpArgs: ["help","h"], description: "查询域名注册信息", async handle(invocation, ctx) {
      ...
      if (raw.toLowerCase() === "help" || raw.toLowerCase() === "h") { await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode:"html", linkPreview:false}); return; }
      if (!raw || !["batch", "history", "clear"].includes(raw.toLowerCase())) {
        const reply = await ctx.telegram.getReply(invocation.message);
        raw = reply?.text.match(/(?:https?:\/\/)?(?:www\.)?[a-z0-9][a-z0-9.-]*\.[a-z]{2,}(?:\/[^\s]*)?/i)?.[0] ?? raw;
        if (!raw) {await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode:"html", linkPreview:false}); return;}
      }
      const db = records(ctx);
      if (raw.toLowerCase() === "clear") {
        let counts: Awaited<ReturnType<typeof db.clear>>;
        try { counts = await db.clear(); }
        catch { if (!ctx.signal.aborted) await ctx.telegram.edit(invocation.message, "WHOIS 记录清除失败，请稍后重试"); return; }
        try { await ctx.telegram.edit(invocation.message, `已清除历史 ${counts.history} 条、缓存 ${counts.cache} 个域名`); }
        catch { if (!ctx.signal.aborted) ctx.log.error("whois:receipt-failed"); } return;
      }
      if (raw.toLowerCase() === "history") {
        try { const data = await db.history();
          const rows = data.rows.map((item, i) => ui.concat(ui.text(`${i + 1}. `), ui.code(item.domain), ui.text(` ${item.queryTime.slice(5, 16).replace("T", " ")}`)));
          await deliver(ctx, invocation.message, await ui.renderDocument({title:"WHOIS 查询历史",
            sections:[ui.section(rows.length ? rows : [ui.text("暂无查询历史")])], footer:`共 ${data.history} 条，缓存 ${data.cache} 个`})); }
        catch { if (!ctx.signal.aborted) await ctx.telegram.edit(invocation.message, "WHOIS 历史读取失败，请稍后重试"); } return;
      }
      const inputs = raw.toLowerCase() === "batch" ? invocation.args.slice(1) : invocation.args.slice(0, 10);
      if (raw.toLowerCase() === "batch") {
        if (!inputs.length) { await ctx.telegram.edit(invocation.message, renderPluginHelp(invocation.prefix), {parseMode:"html", linkPreview:false}); return; }
...
async function deliver(ctx: PluginContext, message: MessageEnvelope, pages: readonly string[]): Promise<void> {
  ...
  try { await ctx.telegram.reply(message, ui.interruptedNotice(result)); }
  catch { if (!ctx.signal.aborted) ctx.log.error("whois:output-notice-failed"); }
}
