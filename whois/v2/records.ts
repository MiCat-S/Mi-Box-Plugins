...
type Data = {
  history: RecordItem[];
  cache: Record<string, RecordItem>;
...
const encode = (value: unknown) =>
  JSON.stringify(value, (_key, item) => (typeof item === "bigint" ? json.rawJSON(item.toString()) : item));
const decode = <T>(value: string): T =>
  json.parse(value, (_key, item, context) =>
  ...
  return {
    async initialize() {
      ...
      const current = existing
        ? undefined
        : await ctx.storage.json<Data>("data.json", { history: [], cache: {} }).read();
      ...
      await store.transaction(db => {
        ...
        if (!meta) {
          importData(db, merge(current!, legacy));
          return;
        }
        ...
        const history = (db.prepare("SELECT value FROM history ORDER BY id DESC").all() as { value: string }[]).map(
          row => decode<RecordItem>(row.value),
        );
        const cache = Object.fromEntries(
          (db.prepare("SELECT domain, value FROM cache").all() as { domain: string; value: string }[]).map(row => [
            row.domain,
            decode<RecordItem>(row.value),
          ]),
        );
    ...
    history() {
      return store.read(db => ({
        ...counts(db),
        rows: db.prepare("SELECT domain, query_time AS queryTime FROM history ORDER BY id DESC LIMIT 20").all() as Pick<
          RecordItem,
          "domain" | "queryTime"
        >[],
      }));
    ...
    save(item: RecordItem) {
      return store.transaction(db => {
        ...
        db.prepare("INSERT INTO history (domain, query_time, value) VALUES (?, ?, ?)").run(
          item.domain,
          item.queryTime,
          encode(item),
        );
        db.prepare("DELETE FROM history WHERE id <= (SELECT id FROM history ORDER BY id DESC LIMIT 1 OFFSET ?)").run(
          Math.min(Math.floor(maxHistory), Number.MAX_SAFE_INTEGER),
        );
        db.prepare(
          "INSERT INTO cache (domain, value) VALUES (?, ?) ON CONFLICT(domain) DO UPDATE SET value = excluded.value",
