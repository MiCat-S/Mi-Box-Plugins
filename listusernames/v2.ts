...
class OutputDeliveryError extends Error {
  constructor() {
    super("LISTUSERNAMES_OUTPUT_DELIVERY_FAILED");
  }
}
...
async function sendParts(message: MessageEnvelope, context: PluginContext, parts: readonly string[]): Promise<void> {
  const send = async (client?: { sendMessage(peer: unknown, value: unknown): Promise<unknown> }) => {
    ...
    const delivery = await ui.deliverPages(parts, context.signal, (part, index) =>
      index
        ? client!
            .sendMessage(raw!.peerId, { message: part, parseMode: "html", replyTo: message.id })
            .then(() => undefined)
        : context.telegram.edit(message, part, { parseMode: "html" }),
    );
    if (delivery.interrupted) {
      context.log.info("listusernames_delivery_interrupted", {
    ...
    commands: {
      listusernames: {
        ...
        async handle(invocation, context) {
          ...
          await context.telegram.edit(invocation.message, "🔄 <b>正在获取公开群组/频道列表...</b>", {
            parseMode: "html",
          });
          try {
            const { Api } = await import("teleproto");
            const result = (await context.telegram.invoke(new Api.channels.GetAdminedPublicChannels({}))) as {
              chats?: unknown;
            };
