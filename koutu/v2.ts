...
async function key(ctx: PluginContext): Promise<string> {
  ...
  return typeof config.apiKey === "string" && config.apiKey.trim()
    ? config.apiKey.trim()
    : (process.env.PICUP_API_KEY ?? "").trim();
}
...
async function matting(ctx: PluginContext, apiKey: string, bytes: Buffer, mime: string, filename: string) {
  ...
  return ctx.http.withResponse(
    ...
    async (response, signal) => {
      ...
      const retry =
        failed && (/"code"\s*:\s*5013/.test(data.toString("utf8")) || data.includes(Buffer.from("文件类型不支持")));
...
async function validateImage(bytes: Buffer): Promise<void> {
  ...
  if (!info.width || !info.height || info.width * info.height > MAX_PIXELS)
    throw new UserError("图片尺寸无效或超过 16777216 像素");
}
...
export async function selectInput(
  ...
  if (mime === "application/x-tgsticker" || extension === ".tgs")
    throw new UserError("暂不支持 TGS 动态贴纸，请回复图片或视频贴纸");
  let bytes: Buffer = Buffer.alloc(0);
  const media = Boolean(source?.photo || doc);
  if (media) {
    if (!/^image\//.test(mime) && !["video/mp4", "video/webm"].includes(mime))
      throw new UserError("请回复图片、GIF、MP4 或 WebM");
  ...
  if (!bytes.length) {
    bytes = await ctx.telegram.withClient(async (client, clientSignal) => {
      ...
      const target = source?.senderId
        ? await source.getInputSender()
        : command?.fromId
          ? await client.getInputEntity(command.fromId)
  ...
  const animation =
    ["gif", "mp4", "webm"].find(value => filename.toLowerCase().endsWith(`.${value}`)) ??
...
export async function videoFrame(
  ...
  for (const bin of ["/usr/bin", "/usr/local/bin", "/opt/homebrew/bin"]) {
    ...
    let result;
    try {
      result = await ctx.processes.run(
        ...
        { cwd: directory, timeoutMs: 10000, maxOutputBytes: 65536 },
      );
    } catch (error) {
      signal.throwIfAborted();
    ...
    if (
      ...
      throw new UserError("视频尺寸无效或超过 16777216 像素");
    signal.throwIfAborted();
...
export default function createKoutu() {
  ...
  const command: CommandDefinition = {
    ...
    subcommands: {
      set: {
        ...
        subcommands: {
          key: {
            description: "仅在收藏夹设置 PicUP API Key",
            args: "<apikey>",
            async handle(i, ctx) {
              if (!i.message.saved) {
                await ctx.telegram.edit(i.message, "请仅在收藏夹中设置 API Key");
                return;
              }
              if (i.args.length !== 1 || !i.args[0].trim()) {
                await ctx.telegram.edit(i.message, `用法：${i.prefix}koutu set key <apikey>`);
                return;
              }
    ...
    help: [
      {
        heading: "使用方式",
        body: "回复图片或静态贴纸执行 <code>{prefix}koutu</code>，保留原图文件名、格式和内容上传；GIF、MP4、WebM 取原尺寸 PNG 首帧。回复文字取发送者头像，不回复取当前发送身份的头像；不支持 TGS。",
      },
      {
        heading: "抠图流程",
        body: "接口报 5013 或文件类型不支持时，才将输入转换为 PNG 重试一次。成功结果原样发送，保留透明度，由 Telegram 自动选择媒体展示方式。",
      },
      {
        heading: "服务与限制",
        body: "图片会上传至 picupapi.tukeli.net，使用你配置的 API Key，可能消耗服务额度。输入和结果上限 20 MiB，图片解码上限 16777216 像素；GIF、视频需 FFmpeg 和 FFprobe。同一时间处理一个任务。兼容原 config.json 的 apiKey 及 PICUP_API_KEY 环境变量。",
      },
    ],
    async handle(i, ctx) {
      if (i.args.length) {
        await ctx.telegram.edit(i.message, renderCommandHelp("koutu", command, { prefix: i.prefix }), {
          parseMode: "html",
        });
        return;
      }
      if (busy) {
        await ctx.telegram.edit(i.message, "已有抠图任务正在处理，请稍后重试");
        return;
      }
      busy = true;
      try {
        ...
        if (!apiKey) {
          await ctx.telegram.edit(i.message, `请先在收藏夹执行 ${i.prefix}koutu set key <apikey>`);
          return;
        }
        ...
        await ctx.files.withTemp(async (directory, signal) => {
          ...
          await ctx.telegram.withClient(async (client, clientSignal) => {
            ...
            await client.sendFile(raw.peerId!, {
              file: new CustomFile("koutu.webp", output.length, "", output),
              replyTo: selected.reply?.id,
        ...
        try {
          await (i.message.raw as Api.Message).delete({ revoke: true });
        } catch {
          ctx.log.info("koutu_receipt_cleanup_failed");
        }
      } catch (error) {
        ...
        await ctx.telegram.edit(
          i.message,
          error instanceof UserError ? error.message : "抠图失败，请确认图片格式、尺寸及服务器日志",
        );
      } finally {
        busy = false;
      }
  ...
  return definePlugin({
    ...
    id: "koutu",
    description: "PicUP 图片与头像一键抠图",
    commands: { koutu: command },
    renderHelp: prefix => renderCommandHelp("koutu", command, { prefix }),
    resources: { processes: { concurrency: 1, queueCapacity: 1, timeoutMs: 30_000, maxOutputBytes: 65536 } },
    settings: ctx => ({
      title: "一键抠图",
      description: "PicUP 服务配置",
      category: "插件配置",
      icon: "✂️",
      ...
      async setValues(patch) {
        ...
        if (typeof apiKey === "string") await store(ctx).update(value => ({ ...value, apiKey: apiKey.trim() }));
      },
    }),
  });
}
