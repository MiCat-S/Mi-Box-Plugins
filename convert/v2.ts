...
const MAX_MEDIA_BYTES=2n*1024n*1024n*1024n;
...
async function runHelper(context: PluginContext, candidates: readonly string[], args: readonly string[], timeoutMs: number,cwd?:string) {
  for (const command of candidates) {
    try { return await context.processes.run(command, args, {timeoutMs, maxOutputBytes: 256 * 1024,...(cwd?{cwd}:{})}); }
    catch (error) {
...
type Helpers={ffmpeg:readonly string[];ffprobe:readonly string[]};
const DEFAULT_HELPERS:Helpers={ffmpeg:FFMPEG,ffprobe:FFPROBE};
async function download(context:PluginContext,media:unknown,target:string,outer:AbortSignal):Promise<void>{
  await context.telegram.withClient(async(client,signal)=>{
    const progress=(downloaded:unknown)=>{signal.throwIfAborted();outer.throwIfAborted();if(BigInt(String(downloaded))>MAX_MEDIA_BYTES)throw new Error("Input too large");};
    await client.downloadMedia(media as any,{outputFile:target,progressCallback:progress,signal});
    signal.throwIfAborted();outer.throwIfAborted();
  });
}
async function runConvert(invocation: any, context: PluginContext, ai: boolean,helpers:Helpers,command:CommandDefinition) {
  const message = invocation.message;
  if (message.replyToId === undefined) {
    await context.telegram.edit(message, renderCommandHelp("convert", command, {prefix: invocation.prefix, title: "🎬 视频转音频 AI 助手"}), {parseMode: "html"});
    return;
  }
  try {
    ...
    const declared=(source.document as any)?.size??(source.video as any)?.size;
    if(declared!==undefined&&BigInt(String(declared))>MAX_MEDIA_BYTES)throw new Error("Input too large");
    await context.files.withTemp(async (directory, signal) => {
      ...
      await download(context,source.media!,input,signal);
      signal.throwIfAborted();
      const inputInfo=await stat(input);if(!inputInfo.isFile()||inputInfo.size===0||BigInt(inputInfo.size)>MAX_MEDIA_BYTES)throw new Error("Invalid input");
      await context.telegram.edit(message, "正在转换为 MP3…");
      await runHelper(context, helpers.ffmpeg, ["-nostdin", "-y", "-protocol_whitelist", "file,pipe", "-i", input, "-vn", "-c:a", "libmp3lame", "-q:a", "2", "-fs", String(MAX_MEDIA_BYTES), rawMp3], 180_000,directory);
      ...
      if (ai) {
        ...
        args.splice(2,0,"-protocol_whitelist","file,pipe");args.splice(args.length-1,0,"-fs",String(MAX_MEDIA_BYTES));
        await runHelper(context, helpers.ffmpeg, args, 120_000,directory); output = finalMp3;
      }
      const info = await stat(output);
      if (!info.isFile() || info.size === 0 || info.size > 2 * 1024 * 1024 * 1024) throw new Error("Invalid output");
      let duration = 0;
      try { duration = Math.max(0, Math.round(Number((await runHelper(context, helpers.ffprobe,
        ["-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", input], 30_000,directory)).stdout.toString("utf8").trim()) || 0)); } catch {signal.throwIfAborted();}
      await context.telegram.withClient(async (client,nativeSignal) => {
        const {Api} = await import("teleproto");
        const raw = message.raw as ApiTypes.Message | undefined;
        const peer=raw?.peerId??returnBigInt(message.chatId);
        nativeSignal.throwIfAborted();
        await client.sendFile(peer, {file: output, thumb: coverFound ? coverFile : undefined, forceDocument: false,
        ...
        nativeSignal.throwIfAborted();
        try{await client.deleteMessages(peer,[message.id],{revoke:true});}catch{context.log.error("convert_command_delete_failed");}
      });
    });
  } catch {
...
function createCommand(helpers:Helpers):CommandDefinition{
  let command:CommandDefinition;
  const apikey:SubcommandDefinition={description:"查看统一 AI 配置方式",args:"",examples:[{args:"apikey"}],async handle(invocation,context){
    await context.telegram.edit(invocation.message,`供应商、密钥与搜索模型由 ai 插件统一管理，请使用 ${invocation.prefix}ai config 与 ${invocation.prefix}ai model search。`);}};
  const u:SubcommandDefinition={description:"AI 智能识别并转换",args:"歌曲名",arguments:[{name:"歌曲名",required:true,description:"用于 AI 查找元数据与封面的歌曲名"}],examples:[{args:"u 稻香"}],
    async handle(invocation,context){await runConvert(invocation,context,true,helpers,command);}};
  const clear:SubcommandDefinition={description:"清理临时文件",args:"",examples:[{args:"clear"}],async handle(invocation,context){await context.telegram.edit(invocation.message,"临时文件由 V2 作用域自动清理");}};
  command={description:"将回复视频转换为 MP3",helpArgs:["help","h"],args:"[文件名]",
    arguments:[{name:"文件名",description:"可选，自定义输出 MP3 文件名；AI 模式使用 u 子命令"}],
    examples:[{args:"",description:"回复视频后按原名转换"},{args:"周杰伦-稻香-演唱会版",description:"自定义输出文件名"}],subcommandsCaseSensitive:false,subcommands:{u,apikey,clear},
    help:[{heading:"说明：",body:"回复视频后发送本命令即可转换为 MP3；不使用 <code>u</code> 时可直接指定输出文件名，不提供文件名则使用视频原名。AI 智能识别、自定义文件名、高质量音轨转 MP3，以及元数据嵌入（歌曲名、歌手、专辑和封面）。"},
      {heading:"AI 配置：",body:"智能识别使用 ai 插件当前搜索供应商和模型；请先通过 <code>{prefix}ai config</code> 与 <code>{prefix}ai model search</code> 完成统一配置。"}],
    async handle(invocation,context){const first=invocation.args[0]?.toLowerCase();if(first==="help"||first==="h"||(!invocation.args.length&&invocation.message.replyToId===undefined)){
      await context.telegram.edit(invocation.message,renderCommandHelp("convert",command,{prefix:invocation.prefix,title:"🎬 视频转音频 AI 助手"}),{parseMode:"html"});return;}
      await runConvert(invocation,context,false,helpers,command);}};
  return command;
}

export default function createConvert(options:Partial<Helpers>={}) {
  const helpers:Helpers={ffmpeg:options.ffmpeg??FFMPEG,ffprobe:options.ffprobe??FFPROBE};
  const command=createCommand(helpers);
  return definePlugin({apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "convert", description: "将回复视频流式转换为 MP3，可用统一 AI 服务补充元数据",
    ...
    renderHelp: prefix => renderCommandHelp("convert", command, {prefix, title: "🎬 视频转音频 AI 助手"}),
    commands: {convert: command}, setup: migrateAi,
  });
}
