  ...
  const cximg: CommandDefinition = {
    ...
    async handle({message,args,prefix}, context) {
      ...
      try {
        ...
        await context.telegram.edit(message, ref ? "🖼️ 已检测到参考图，正在生成图片..." : "🎨 正在根据提示词生成图片...");
        ...
        const heartbeat = (async () => {
          try {while (true) {
            ...
            await context.telegram.edit(message, `⏳ 正在等待 AI 返回结果...\n⏱️ 已耗时：${duration(Date.now()-startedAt)}`);
          }} catch {if (!heartbeatSignal.aborted) context.log.error("codex_image_progress_failed");}
        })();
        let results: ImageResult[];
        try {results = await context.services.call<ImageResult[]>("ai", "image", {prompt, timeoutMs:state.maxWaitMs,
          ...(ref ? {input:{data:ref.data,mimeType:ref.mimeType}} : {})}, context.signal);}
        finally {heartbeatController.abort(); await heartbeat;}
        ...
        const caption = fullCaption.length <= 1024 ? fullCaption : `<b>图片生成完成</b>\n<b>耗时:</b> ${duration(Date.now()-startedAt)}`;
        await context.telegram.withClient(async (client, signal) => {
          const {CustomFile} = await import("teleproto/client/uploads.js");
          signal.throwIfAborted();
          await client.sendFile(raw?.peerId ?? returnBigInt(message.chatId), {file:new CustomFile(`ai-image-${Date.now()}.png`, image.length, "", image),
            caption,
        ...
        if (caption !== fullCaption) {
          try {
            ...
            const delivery = await ui.deliverPages(pages, context.signal, page => context.telegram.reply(message, page, {parseMode:"html"}));
        ...
        if (deleteFailed) {
          try {await context.telegram.edit(message, "✅ 图片生成完成");}
          catch {context.signal.throwIfAborted(); context.log.error("codex_image_completion_edit_failed");}
        }
