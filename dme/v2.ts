...
const normalizeConfig = (value: Partial<Config>): Config => ({
  ...
  searchLimit: Number.isInteger(value.searchLimit)
    ? Math.max(10, Math.min(500, value.searchLimit!))
    : defaults.searchLimit,
  retryAttempts: Number.isInteger(value.retryAttempts)
    ? Math.max(0, Math.min(10, value.retryAttempts!))
...
function topic(message: any): number | undefined {
  ...
  return [reply?.replyToTopId, reply?.topMsgId, reply?.replyToMsgId, reply?.replyToMsg?.id].find(
...
async function execute(
  ...
  let chat: any;
  try {
    chat = await retry(() => client.getEntity(raw?.peerId ?? returnBigInt(message.chatId)), 2);
  } catch (error) {
    signal.throwIfAborted();
    log("entity");
    ...
    for (const folder of [0, 1]) {
      try {
        for await (const dialog of client.iterDialogs({ folder })) {
          signal.throwIfAborted();
          if (dialog.id?.toString() === message.chatId && dialog.entity) {
            chat = dialog.entity;
            break;
          }
        }
  ...
  const saved =
    message.saved === true ||
    chat.className === "InputPeerSelf" ||
    chat.className === "PeerSelf" ||
  ...
  if (anti && chat.className === "Channel" && chat.broadcast === true) {
    try {
      const result = await call(() =>
        client.invoke(new Api.channels.GetParticipant({ channel: chat, participant: new Api.InputPeerSelf() })),
      );
  ...
  if (!direct && chat.className === "Channel") {
    try {
      ...
      for (const item of result.peers) {
        const key = typedPeer(item.peer);
  ...
  const mine = (m: any) =>
    ...
    m.out === true ||
    [m.fromId, m.senderId].some(peer => {
      const key = typedPeer(peer);
      return key !== undefined && identities.has(key);
    });
  const eligible = (m: any) =>
    ...
    m.id > 0 &&
    m.id < message.id &&
    (topicId === undefined || topic(m) === topicId) &&
    (direct || mine(m));

  try {
    await retry(() => client.deleteMessages(chat, [message.id], { revoke: true }));
  } catch {
    signal.throwIfAborted();
    log("command-delete");
  }
  ...
  const validImage = (value: Buffer) =>
    value.length >= 8 && value.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]));
    ...
    try {
      image = await ctx.tasks.run("dme:image", async scoped => {
        ...
        try {
          ...
          if (details.size <= 0 || details.size > MAX_IMAGE_BYTES) {
            await unlink(file);
            combined.throwIfAborted();
          } else {
            const cached = await readFile(file, { signal: combined });
            combined.throwIfAborted();
        ...
        const buffer = await ctx.http.withResponse(
          ...
          async (response, responseSignal) => {
            ...
            const active = AbortSignal.any([combined, responseSignal]),
        ...
        try {
          await writeFile(temporary, buffer, { signal: combined });
          combined.throwIfAborted();
  ...
  const edit = async (m: any) => {
    ...
    try {
      if (!m.media && typeof m.message === "string") {
        if (m.message !== "占位符")
          await call(() => client.invoke(new Api.messages.EditMessage({ peer: chat, id: m.id, message: "占位符" })));
        edited++;
        return;
      }
      if (
        ...
        m.media.document?.attributes?.some((a: any) => a.className === "DocumentAttributeSticker")
      )
        return;
      ...
      const file = await call(() =>
        client.uploadFile({ file: new CustomFile("dme_troll.png", buffer.length, "", buffer), workers: 1 }),
      );
      await call(() =>
        client.invoke(
  ...
  const deleteBatch = async (ids: number[]) => {
    ...
    for (let offset = 0; offset < ids.length;) {
      ...
      try {
        ...
        try {
          await call(() => client.invoke(new Api.updates.GetState()));
        } catch {
          signal.throwIfAborted();
          log("sync");
        }
        ...
        if (batch.length === 1) {
          failed++;
          offset++;
        } else size = Math.max(1, Math.floor(batch.length / 2));
      }
      await wait(200);
    }
  };
  ...
  while (matched < target) {
    ...
    offsetId = next;
    const batch = page
      .filter(m => eligible(m) && !seen.has(m.id))
      .sort((a, b) => b.id - a.id)
      .slice(0, target - matched);
    if (!batch.length) {
      empty++;
      const emptyLimit = chat.className === "Channel" ? 300 : 3;
      if (history && !direct && empty >= emptyLimit) {
        ctx.log.info("dme:scan-limit", { emptyBatches: empty });
        break;
      }
    } else {
      empty = 0;
      ...
      async setValues(patch, signal) {
        ...
        for (const [key, value] of Object.entries(patch)) {
          ...
          if (!range || typeof value !== "number" || !Number.isInteger(value) || value < range[0] || value > range[1])
            throw new Error("Invalid DME setting");
        }
        signal.throwIfAborted();
    ...
    commands: {
      dme: {
        ...
        async handle({ message, args, prefix }, ctx) {
          ...
          const anti = sub === "-f",
            token = anti ? args[1] : args[0];
          if (anti && !token) {
            await ctx.telegram.edit(
              message,
              `❌ <b>参数错误:</b> 请指定删除数量\n\n💡 使用 <code>${escape(prefix)}dme -f [数量]</code>`,
              { parseMode: "html" },
            );
            return;
          }
          ...
          if (!token || !/^\d+$/.test(token) || !Number.isSafeInteger(count) || count <= 0) {
            await ctx.telegram.edit(
              message,
              `❌ <b>参数错误:</b> 删除数量必须为正整数\n\n💡 使用 <code>${escape(prefix)}dme [数量]</code> 或 <code>${escape(prefix)}dme -f [数量]</code>`,
              { parseMode: "html" },
            );
            return;
          }
          if (active.has(message.chatId)) {
            await ctx.telegram.edit(message, "当前会话已有 DME 删除任务正在执行，请等待任务完成");
            return;
          }
          ...
          try {
            await ctx.tasks.run(`dme:delete:${message.chatId}`, async scoped => {
              ...
              await ctx.telegram.withClient((client, signal) =>
                execute(ctx, client, AbortSignal.any([signal, scoped]), message, count, anti, config),
              );
            });
          } catch (error) {
            if (!ctx.signal.aborted) {
              ...
              await ctx.telegram.withClient(async (client, signal) => {
                ...
                await client.sendMessage("me", {
                  message: `DME ${message.chatId} 操作失败，请检查权限、网络和 Telegram 限制后重试。`,
                });
              });
            }
          } finally {
            active.delete(message.chatId);
          }
        },
      },
    },
