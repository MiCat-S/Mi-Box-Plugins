...
const TZ_SUFFIX = new RegExp(
  String.raw`[\s|·•\-–—]*[（(\[【]?\s*(?:${TIME_PART}\s*${TZ_PART}|${TZ_PART}|${TIME_PART})\s*[）)\]】]?\s*$`,
  "u",
);
...
function nameOf(message: MessageEnvelope | undefined): string {
  if (!message) return "";
  const sender = (
    message.raw as { sender?: { firstName?: string; lastName?: string; title?: string; username?: string } } | undefined
  )?.sender;
  if (!sender) return "";
...
export default function createDiss() {
  ...
  const doUnlock = async (invocation: CommandInvocation, ctx: PluginContext): Promise<void> => {
    ...
    await mutate(ctx, current => {
      ...
      if (Object.keys(map).length) chats[chat] = map;
      else delete chats[chat];
      return chats;
    });
    await ctx.telegram.edit(invocation.message, `🔓 已解锁 <b>${escape(target.name)}</b>，放过 TA 了。`, {
      parseMode: "html",
    });
  };
  ...
  const doList = async (invocation: CommandInvocation, ctx: PluginContext): Promise<void> => {
    ...
    const lines = entries.map(
      ([id, info]) => `• <b>${escape(info.name)}</b> <code>${escape(id)}</code> · 已喷 ${info.hits} 次`,
    );
    await ctx.telegram.edit(invocation.message, `🔫 本会话已锁定 ${entries.length} 人：\n${lines.join("\n")}`, {
      parseMode: "html",
    });
  };
  ...
  const buildInsult = async (ctx: PluginContext, signal: AbortSignal, text: string, name: string): Promise<string> => {
    ...
    if (ctx.services.available("ai", "chat")) {
      try {
        ...
        signal.throwIfAborted(); /* fall through to the local list */
      }
    }
  ...
  const autoReply = (message: MessageEnvelope, info: TargetInfo, ctx: PluginContext, content: string): void => {
    ...
    void ctx.tasks.run("diss:reply", async signal => {
      try {
        ...
        /* cancelled or delivery failed; keep listening */
      } finally {
        inFlight.delete(key);
      }
    });
  ...
  const guarded =
    (operation: (invocation: CommandInvocation, ctx: PluginContext) => Promise<void>) =>
    async (invocation: CommandInvocation, ctx: PluginContext): Promise<void> => {
      try {
        await operation(invocation, ctx);
      } catch {
        if (ctx.signal.aborted) return;
        ctx.log.error("diss_command_failed");
        if (!ctx.signal.aborted) {
          try {
            await ctx.telegram.edit(invocation.message, "❌ 操作失败，请稍后重试");
          } catch {
            /* delivery failed too */
          }
        }
  ...
  const aiShow = async (invocation: CommandInvocation, ctx: PluginContext): Promise<void> => {
    ...
    await ctx.telegram.edit(
      invocation.message,
      dynamic + renderCommandHelp("dissai", dissaiCommand, { prefix: invocation.prefix, title: "" }),
      { parseMode: "html" },
    );
  };
  const aiSet = (field: keyof DissConfig, label: string): SubcommandDefinition => ({
    description:
      field === "model"
        ...
        : field === "tag"
          ? "指定 ai 配置里的提供商"
          : "思考强度，可选 auto | none | minimal | low | medium | high | xhigh",
    args: "[值|reset]",
    examples: [
      { args: field === "model" ? "model gpt-4o" : field === "tag" ? "provider openai" : "reasoning high" },
      { args: field === "model" ? "model reset" : field === "tag" ? "provider reset" : "reasoning reset" },
    ],
    arguments: [{ name: "值", description: "reset 恢复跟随 ai 插件" }],
    handle: guarded(async (invocation, ctx) => {
      const [action, ...extra] = invocation.args;
      if (!action) {
        await aiShow(invocation, ctx);
        return;
      }
      if (extra.length) {
      ...
      if (field === "reasoningEffort" && !REASONING_VALUES.has(action)) {
        await ctx.telegram.edit(
          invocation.message,
          `思考强度必须是：<code>${[...REASONING_VALUES].join(" | ")}</code>`,
          { parseMode: "html" },
        );
        return;
      }
      await configStore(ctx).update(current => ({ ...current, [field]: action }));
      await ctx.telegram.edit(invocation.message, `已设置 Diss ${label}：<code>${escape(action)}</code>`, {
        parseMode: "html",
      });
    }),
  });
  ...
  const dissCommand: CommandDefinition = {
    ...
    args: "[语录|@对方]",
    arguments: [
      { name: "语录", description: "发送语录获取一条祖安语录" },
      { name: "@对方", description: "艾特锁定，或回复对方消息后发送本命令" },
    ],
    examples: [{ args: "" }, { args: "@对方" }, { args: "语录" }],
    help: [
      {
        heading: "说明：",
        body: "锁定后对方一说话就会自动回怼；对方只发表情包、图片、语音等（无文字）也会回怼。昵称里的花体时区（如 <code>𝟚𝟙:𝟜𝟙 𝔾𝕄𝕋+𝟠</code>）会自动去掉，只留昵称。可能产生调用费用；AI 不可用时使用本地模板。",
      },
    ],
    handle: guarded(async (invocation, ctx) => {
      const first = invocation.args[0]?.toLowerCase();
      if (first && QUOTE_ARGS.has(first)) {
        await doQuote(invocation, ctx);
        return;
      }
  ...
  const undissCommand: CommandDefinition = {
    description: "解锁目标（也可回复对方消息后发送本命令）",
    args: "[@对方]",
  ...
  const dissaiCommand: CommandDefinition = {
    ...
    arguments: [
      { name: "字段", description: "model / provider / reasoning" },
      { name: "值", description: "reset 恢复跟随 ai 插件" },
    ],
    examples: [
      { args: "" },
      { args: "model gpt-4o" },
    ...
    help: [
      {
        heading: "说明：",
        body: "自动回怼默认复用 ai 插件的聊天提供商，可单独覆盖模型 / 提供商 / 思考强度；只设模型不设 provider 时沿用 ai 当前聊天提供商。修改会在下次自动回怼时生效。可能产生调用费用；AI 不可用时使用本地模板。",
      },
    ],
    handle: guarded(async (invocation, ctx) => {
      const scope = invocation.args[0];
      if (!scope || ["help", "h", "?"].includes(scope.toLowerCase())) {
        await aiShow(invocation, ctx);
        return;
      }
      await ctx.telegram.edit(
        invocation.message,
        `用法：<code>${escape(invocation.prefix)}dissai model|provider|reasoning [值|reset]</code>`,
        { parseMode: "html" },
      );
    }),
  };
  ...
  const commands = {
    diss: dissCommand,
    undiss: undissCommand,
    dislist: dislistCommand,
    dissclear: dissclearCommand,
    dissai: dissaiCommand,
  };
  const renderGuide = (prefix: string): string =>
    Object.entries(commands)
      .map(([name, command], index) =>
        renderCommandHelp(name, command, {
          prefix,
          ...(index === 0 ? { title: "🔫 Diss · 嘴臭对线机" } : { title: "" }),
        }),
      )
      .join("\n\n");
  const dishelpCommand: CommandDefinition = {
    description: "查看嘴臭对线机帮助",
    args: "",
    examples: [{ args: "" }],
    handle: guarded(async (invocation, ctx) => {
      const pages = await ui.renderRichText(renderGuide(invocation.prefix));
      ...
      if (delivery.interrupted)
        ctx.log.error("diss_help_delivery_failed", { published: delivery.published, total: delivery.total });
    }),
  };
    ...
    listeners: [
      {
        ...
        async handle(message, ctx) {
          ...
          const text = raw?.message ?? message.text;
          const me = await selfInfo(ctx);
          const entities = [...(raw?.entities ?? [])];
          const botMentions = entities.filter(entity =>
            entity.className === "MessageEntityMentionName"
              ? String(entity.userId) === me.id
          ...
          if (botMentions.length) {
            const botMentionSet = new Set(botMentions);
            const removed = [...botMentions]
              .sort((left, right) => right.offset - left.offset)
              .reduce(
                (value, entity) =>
                  `${value.slice(0, entity.offset)}${" ".repeat(entity.length)}${value.slice(entity.offset + entity.length)}`,
                text,
              );
            const cleaned = removed.trim();
            const match = cleaned.match(/^(diss|undiss)(?:\s|$)/i);
            if (match) {
              const command = match[1]!.toLowerCase();
              const filtered = entities.filter(entity => !botMentionSet.has(entity));
              const incoming = { ...message, raw: raw ? { ...raw, entities: filtered } : undefined };
              const invocation = { message: incoming, command, prefix: ".", args: [] };
              const incomingContext = {
                ...
                telegram: {
                  ...
                  edit: (
                    ...
                    value: string,
                    options?: { parseMode?: "html" | "markdown"; linkPreview?: boolean },
                  ) => ctx.telegram.reply(message, value, options),
                },
              } as PluginContext;
              if (command === "diss") await doLock(invocation, incomingContext);
              else await doUnlock(invocation, incomingContext);
              try {
                await ctx.telegram.withClient(async (client, signal) => {
                  const { returnBigInt } = await import("teleproto/Helpers.js");
                  signal.throwIfAborted();
                  const peer = raw?.inputChat ?? raw?.peerId ?? returnBigInt(message.chatId);
                  await client.deleteMessages(peer, [message.id], { revoke: false });
                  signal.throwIfAborted();
                });
              } catch {
                ctx.signal.throwIfAborted();
                ctx.log.error("diss_command_delete_failed");
              }
              return;
            }
          }
