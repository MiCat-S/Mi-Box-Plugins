import {STRUCTURED_PLUGIN_API_VERSION, definePlugin, renderCommandHelp, ui, type CommandDefinition, type CommandInvocation, type MessageEnvelope, type PluginContext, type SubcommandDefinition} from "telebox/sdk";
...
export default function createDiss() {
  ...
  const buildInsult = async (ctx: PluginContext, signal: AbortSignal, text: string, name: string): Promise<string> => {
    ...
    if (ctx.services.available("ai", "chat")) {
      ...
      } catch { signal.throwIfAborted(); /* fall through to the local list */ }
    }
  ...
  const dishelpCommand: CommandDefinition = {
    ...
    handle: guarded(async (invocation, ctx) => {
      ...
      if (delivery.interrupted) ctx.log.error("diss_help_delivery_failed", {published: delivery.published, total: delivery.total});
    }),
  };
  ...
  return definePlugin({
    ...
    listeners: [{
      ...
      async handle(message, ctx) {
        ...
        const botMentions = entities.filter(entity => entity.className === "MessageEntityMentionName"
          ? String(entity.userId) === me.id
          : entity.className === "MessageEntityMention" && !!me.username &&
            text.slice(entity.offset, entity.offset + entity.length).replace(/^@/, "").toLowerCase() === me.username.toLowerCase());
        if (botMentions.length) {
          const botMentionSet = new Set(botMentions);
          const removed = [...botMentions].sort((left, right) => right.offset - left.offset)
            .reduce((value, entity) => `${value.slice(0, entity.offset)}${" ".repeat(entity.length)}${value.slice(entity.offset + entity.length)}`, text);
          ...
          if (match) {
            ...
            const incomingContext = {...ctx, telegram: {...ctx.telegram,
              edit: (_message: MessageEnvelope, value: string, options?: {parseMode?: "html" | "markdown"; linkPreview?: boolean}) =>
            ...
            } catch { ctx.signal.throwIfAborted(); ctx.log.error("diss_command_delete_failed"); }
            return;
          }
        }
