...
async function deletedMembers(message:MessageEnvelope,remove:boolean,ctx:PluginContext){if(!group(message)){await ctx.telegram.edit(message,"❌ <b>错误:</b> 此命令仅在群组中可用",{parseMode:"html"});return;}await ctx.telegram.withClient(async(client:any,signal)=>{const {Api}=await import("teleproto");const raw:any=message.raw,chat=await client.getEntity(raw?.peerId??message.chatId),input=await client.getInputEntity(chat);if(remove&&!await permission(client,chat,Api)){await ctx.telegram.edit(message,"❌ <b>错误:</b> 没有封禁用户权限，无法执行清理",{parseMode:"html"});return;}await ctx.telegram.edit(message,remove?"🔍 正在扫描并清理群组已注销账号...":"🔍 正在扫描群组已注销账号...");let found=0,ok=0,failed=0;const ids:string[]=[];for await(const user of client.iterParticipants(chat)){signal.throwIfAborted();if(user.className!=="User"||!user.deleted)continue;found++;ids.push(String(user.id));if(remove)try{const target=await client.getInputEntity(user);if(chat.className==="Chat")await client.invoke(new Api.messages.DeleteChatUser({chatId:chat.id,userId:target,revokeHistory:false}));else{await client.invoke(new Api.channels.EditBanned({channel:input,participant:target,bannedRights:new Api.ChatBannedRights({viewMessages:true,untilDate:0})}));await client.invoke(new Api.channels.EditBanned({channel:input,participant:target,bannedRights:new Api.ChatBannedRights({untilDate:0})}));}ok++;}catch(e){failed++;if(await wait(e,signal))ctx.log.info("clean:flood-resumed");}await sleep(150,undefined,{signal});}
