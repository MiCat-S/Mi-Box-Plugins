...
async function output(context: PluginContext, message: MessageEnvelope, source: string): Promise<void> {
  ...
  const result = await ui.deliverPages(pages, context.signal, (page, index) => index
    ? context.telegram.reply(message, page, {parseMode: "html"})
    : context.telegram.edit(message, page, {parseMode: "html"}));
  if (!result.interrupted) return;
  context.log.info("oxost_pagination_interrupted", {published: result.published, total: result.total,
    ...
    commands: {"0x0": {helpArgs: ["help","h"], description: "上传回复中的媒体到 0x0.st", async handle(invocation, context) {
      ...
      try {
        ...
        try { await context.files.withTemp(async (directory, tempSignal) => {
          ...
          try { await handle.read(header, 0, header.length, 0); tempSignal.throwIfAborted(); } finally { await handle.close(); }
          tempSignal.throwIfAborted();
          ...
          }, {timeoutMs: 60_000, signal: AbortSignal.any([context.signal, tempSignal]), redirects:{allowedHosts:["0x0.st"],maxRedirects:2}});
          tempSignal.throwIfAborted();
