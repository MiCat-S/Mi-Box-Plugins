...
import { access, chmod, copyFile, lstat, mkdir, open, readFile, realpath, rename, rm } from "node:fs/promises";
...
export function parseSpeedtestResult(text: string): SpeedtestResult {
  ...
  try {
    ...
    throw new SpeedtestError("Speedtest CLI 未返回有效 JSON，请运行 diagnose 检查安装");
  }
  ...
  return {
    ...
    ...(latency === undefined
      ? {}
      : { ping: { latency, ...(finite(rawPing?.jitter) === undefined ? {} : { jitter: finite(rawPing?.jitter) }) } }),
    ...(download ? { download } : {}),
    ...(upload ? { upload } : {}),
...
function output(error: unknown, name: "stdout" | "stderr"): string {
  ...
  return descriptor && "value" in descriptor && Buffer.isBuffer(descriptor.value)
    ? descriptor.value.toString("utf8")
    : "";
}
...
function processFailure(error: unknown): never {
  ...
  if (kind === "SPAWN_FAILED")
    throw new SpeedtestError(
      "Speedtest CLI 无法启动；请运行 diagnose，托管模式可运行 fix，系统模式请安装官方 Ookla CLI",
    );
  const diagnostic = output(error, "stderr");
  if (/NoServersException|Server not found/i.test(diagnostic))
    throw new SpeedtestError("指定服务器不可用，请运行 list 选择其他服务器");
  if (/timeout|Cannot read|socket/i.test(diagnostic))
    throw new SpeedtestError("Speedtest CLI 网络连接失败或超时，请检查网络后重试");
...
async function runResult(
  ...
  try {
    const result = await context.processes.run(executable, args, {
      timeoutMs,
      maxOutputBytes: 2 * 1024 * 1024,
      env: await cliEnvironment(context),
    });
    ...
    if (partial.trim()) {
      try {
        return parseSpeedtestResult(partial);
      } catch {
        /* Report the bounded process failure below. */
      }
    }
...
async function version(context: PluginContext, executable: string): Promise<string> {
  try {
    const result = await context.processes.run(executable, ["--version"], {
      timeoutMs: 10_000,
...
export async function resolveCli(
  ...
  if (system) {
    ...
    if (!executable)
      throw new SpeedtestError(
        "系统未安装官方 Ookla Speedtest CLI；请按 speedtest.net/apps/cli 安装，或移除 --system 使用托管版本",
      );
...
export async function diagnose(
  context: PluginContext,
  system: boolean,
): Promise<{ ok: boolean; path?: string; version?: string; message: string }> {
  try {
    ...
    if (!executable || !(await ordinaryFile(executable))) {
      return {
        ok: false,
        message: system
          ? "未找到官方系统 CLI；请按 speedtest.net/apps/cli 安装"
          : "托管 CLI 不存在；请运行 speedtest fix 安装",
      };
    }
...
function packageInfo(): PackageInfo {
  if (process.platform === "linux") {
    ...
    return {
      fileName: `ookla-speedtest-${SPEEDTEST_VERSION}-linux-${architecture}.tgz`,
      executableName: "speedtest",
      archive: "tgz",
    };
  }
  if (process.platform === "darwin") {
    if (process.arch !== "x64" && process.arch !== "arm64")
      throw new SpeedtestError(`官方 Speedtest CLI 1.2.0 不支持当前 macOS 架构 ${process.arch}`);
    return {
  ...
  if (process.platform === "win32") {
    if (process.arch !== "x64")
      throw new SpeedtestError(`官方 Speedtest CLI 1.2.0 不支持当前 Windows 架构 ${process.arch}`);
    return {
...
async function downloadArchive(context: PluginContext, url: URL, destination: string): Promise<void> {
  await context.http.withResponse(
    ...
    async (response, signal) => {
      ...
      if (
        expected !== undefined &&
        (!Number.isSafeInteger(expected) || expected <= 0 || expected > MAX_ARCHIVE_BYTES)
      ) {
      ...
      try {
        ...
        for (;;) {
          ...
          if (total > MAX_ARCHIVE_BYTES || (expected !== undefined && total > expected))
            throw new Error("download size rejected");
          let offset = 0;
...
async function replaceManaged(context: PluginContext, source: string, target: string): Promise<void> {
  ...
  try {
    ...
    if (backedUp && !(await ordinaryFile(target)) && (await ordinaryFile(backup)))
...
export async function installManaged(context: PluginContext): Promise<string> {
  ...
  if (current && (current.isSymbolicLink() || !current.isFile()))
    throw new SpeedtestError("托管 CLI 路径不是普通文件，请管理员检查 assets/speedtest");
  try {
    await context.files.withTemp(async (directory, signal) => {
      ...
      signal.throwIfAborted();
      if (!(await ordinaryFile(archive, MAX_ARCHIVE_BYTES)))
        throw new SpeedtestError("官方 CLI 安装包不是有效普通文件");
...
export async function listServers(context: PluginContext, executable: string): Promise<ServerInfo[]> {
  try {
    const result = await context.processes.run(
      executable,
