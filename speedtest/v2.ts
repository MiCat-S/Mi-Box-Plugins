...
import { DEFAULT_ORDER, migrateConfig, normalizeType, readConfig, updateConfig, type MessageType } from "./v2/config";
import {
  BEST_CANDIDATES,
  SPEEDTEST_VERSION,
  SpeedtestError,
...
async function deliverHtml(context: PluginContext, invocation: CommandInvocation, html: string): Promise<void> {
  ...
  const result = await ui.deliverPages(pages, context.signal, (page, index) => {
    ...
    return index
      ? context.telegram.reply(invocation.message, value, { parseMode: "html", linkPreview: false })
      : context.telegram.edit(invocation.message, value, { parseMode: "html", linkPreview: false });
  });
  if (result.interrupted) {
    context.log.error("speedtest_delivery_failed", {
      kind: "internal",
      published: result.published,
      total: result.total,
    });
    if (!result.published) throw new SpeedtestError("Speedtest 结果发送失败，请稍后重试");
    try {
      await context.telegram.reply(invocation.message, ui.interruptedNotice(result));
    } catch {
      context.signal.throwIfAborted();
      context.log.error("speedtest_delivery_notice_failed", { kind: "internal" });
    }
  }
}
...
async function networkCheck(context: PluginContext): Promise<{ ok: boolean; message: string }> {
  try {
    const status = await context.http.withResponse(
    ...
    return status >= 200 && status < 400
      ? { ok: true, message: `网络连接正常（HTTP ${status}）` }
      : { ok: false, message: `Speedtest 官网返回 HTTP ${status}` };
  } catch (error) {
    context.signal.throwIfAborted();
...
function brief(result: SpeedtestResult): string {
  const rate = (bandwidth: number | undefined) =>
    bandwidth === undefined ? "失败" : `${Math.round((bandwidth * 8) / 10_000) / 100} Mbps`;
  return (
...
export default function createSpeedtest() {
  ...
  const command = {
    ...
    async handle(invocation: CommandInvocation, context: PluginContext) {
      ...
      try {
        ...
        if (sub === "diagnose") {
          ...
          await edit(
            ...
            `${header}\n<code>${result.ok ? "正常" : "异常"}：${escape(result.message)}</code>\n` +
              ...
              (result.version ? `<code>版本：${escape(result.version)}</code>` : ""),
            true,
          );
          return;
        }
        ...
        if (sub === "best") {
          ...
          const results = await serial(context, async () => {
            ...
            for (const candidate of candidates) {
              context.signal.throwIfAborted();
              try {
                measured.push(await probeServer(context, executable, candidate.id));
              } catch {
                context.signal.throwIfAborted();
              }
            }
            return measured.sort(
              (left, right) =>
                (left.ping?.latency ?? Number.POSITIVE_INFINITY) - (right.ping?.latency ?? Number.POSITIVE_INFINITY),
            );
          });
          ...
          await edit(
            ...
            `${header}\n<b>实测推荐服务器</b>\n${results
              .map(
                (result, index) =>
                  `${index + 1}. <code>${result.server.id}</code> <code>${escape(result.server.name)}</code> <code>${result.ping?.latency ?? "延迟失败"} ms</code>`,
              )
              .join("\n")}\n` + `<code>已实际测速 ${results.length}/${BEST_CANDIDATES} 个有界候选</code>`,
            true,
          );
          return;
        }
        ...
        if ((sub && directId === undefined) || args.length > (sub ? 1 : 0))
          throw new SpeedtestError(help(invocation.prefix));
        ...
        await edit(
          context,
          invocation,
          message.includes("<b>") ? message : `❌ <b>Speedtest 操作失败</b>\n<code>${escape(message)}</code>`,
          true,
        );
      }
    },
  ...
  return definePlugin({
    ...
    settings: context => ({
      ...
      async getValues() {
        const value = await readConfig(context);
        return { default_server_id: value.default_server_id, preferred_type: value.preferred_type };
      },
      async setValues(patch) {
        ...
        if (Object.hasOwn(patch, "default_server_id")) {
          if (
            patch.default_server_id !== null &&
            (typeof patch.default_server_id !== "number" ||
              !Number.isSafeInteger(patch.default_server_id) ||
              patch.default_server_id <= 0)
          )
