...
function userOf(value:unknown):Api.User|undefined{if(value instanceof Api.User)return value;if(value instanceof Api.ChannelParticipant){const user=(value as Api.ChannelParticipant&{user?:unknown}).user;return user instanceof Api.User?user:undefined;}return;}
function failure(error:unknown):string{if(!error||typeof error!=="object")return"请确认当前会话是可访问成员列表的群组";const values=[Object.getOwnPropertyDescriptor(error,"errorMessage")?.value,Object.getOwnPropertyDescriptor(error,"message")?.value].filter(v=>typeof v==="string") as string[];const text=values.join(" ");if(text.includes("CHAT_ADMIN_REQUIRED"))return"需要管理员权限才能查看群组成员列表";if(text.includes("CHANNEL_PRIVATE"))return"无法访问该群组，请确认当前账号仍是群组成员";if(text.includes("AUTH_KEY_UNREGISTERED"))return"会话未注册，请重新登录";const match=/FLOOD_WAIT[_ ]?(\d+)/.exec(text);if(match){const seconds=Number(match[1]);if(Number.isSafeInteger(seconds)&&seconds>=0&&seconds<=86400)return`请求过于频繁，请等待 ${seconds} 秒后重试`;}return"请确认当前会话是可访问成员列表的群组";}
    ...
    commands: {premium: {helpArgs: ["help","h"], description: "统计群组 Telegram Premium 用户比例", async handle(invocation, context) {
      ...
      try {
        ...
        await context.telegram.withClient(async (client, clientSignal) => {
          const signal=AbortSignal.any([context.signal,clientSignal]);signal.throwIfAborted();
          const raw = invocation.message.raw as Api.Message | undefined;
          const chat = await client.getEntity(raw?.peerId??peer(invocation.message.chatId));signal.throwIfAborted();
          ...
          if (chat instanceof Api.Channel) {
            try {
              ...
              signal.throwIfAborted();const count=(full.fullChat as {participantsCount?:unknown}).participantsCount;if(typeof count==="number"&&Number.isSafeInteger(count)&&count>=0)participantCount=count;
            } catch { signal.throwIfAborted();context.log.error("premium_count_failed"); }
          }
          ...
          for await (const participant of client.iterParticipants(chat, {limit: 10_000})) {
            signal.throwIfAborted();
            processed++;
            if (processed % 100 === 0) {await context.telegram.edit(invocation.message, `正在统计群组成员… 已处理 ${processed} 个成员`,{parseMode:"html"});signal.throwIfAborted();}
            const user = userOf(participant);if(!user)continue;
            if (user.bot) bots++;
