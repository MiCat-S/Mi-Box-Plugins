...
const DEFAULT_FILTERS = ["广告","推广","赞助","合作","代理","招商","加盟","投资","理财","贷款","借钱","网贷","信用卡","pos机","刷单","兼职","副业","微商","代购","淘宝","拼多多","京东","直播带货","优惠券","返利","红包","现金","提现","充值","游戏币","点卡","彩票","博彩","赌博","六合彩","时时彩","北京赛车","股票","期货","外汇","数字货币","比特币","挖矿","保险","医疗","整容","减肥","丰胸","壮阳","药品","假货","高仿","A货","精仿","原单","尾单","办证","刻章","发票","学历","文凭","证书","黑客","破解","外挂","木马","病毒","盗号","vpn","翻墙","代理ip","科学上网","梯子"];
...
async function writeAll(file:Awaited<ReturnType<typeof open>>,chunk:Uint8Array,signal:AbortSignal){let offset=0;while(offset<chunk.length){signal.throwIfAborted();const result=await file.write(chunk,offset,chunk.length-offset);signal.throwIfAborted();if(result.bytesWritten<=0)throw new Error("video write failed");offset+=result.bytesWritten;}}
...
async function editLines(ctx:PluginContext,message:MessageEnvelope,title:string,lines:readonly string[]){const pages=(await ui.renderDocument({title,sections:[ui.section(lines.map(ui.text))]},ui.PAGE_LABEL_RESERVE)).map((page,index,all)=>page+ui.pageLabel(index,all.length));const delivery=await ui.deliverPages(pages,ctx.signal,(page,index)=>index?ctx.telegram.reply(message,page,{parseMode:"html"}):ctx.telegram.edit(message,page,{parseMode:"html"}));if(delivery.interrupted){ctx.log.error("search_page_delivery_failed",{category:"DELIVERY_FAILED",published:delivery.published,total:delivery.total});if(!delivery.published)throw new Error("delivery failed");}}
...
async function add(ctx: PluginContext, message: MessageEnvelope, raw: string) {
  ...
  await ctx.telegram.withClient(async (client: any,signal) => {
    const call=async<T>(operation:()=>Promise<T>)=>{signal.throwIfAborted();const value=await operation();signal.throwIfAborted();return value;};
    ...
    for (const value of raw.split("\\").map(item => item.trim()).filter(Boolean)) {
      ...
      try {
        ...
        if (entity.className === "Channel" && entity.broadcast && !entity.megagroup) {
          try { const full: any = await call(()=>client.invoke(new Api.channels.GetFullChannel({channel: entity}))); const id = full?.fullChat?.linkedChatId; if (id) { const linked: any = await call(()=>client.getEntity(id)); linkedGroup = linked?.username ? `@${linked.username}` : undefined; } } catch {signal.throwIfAborted();}
        }
        added.push({title: entity.title || value, handle: value, ...(linkedGroup ? {linkedGroup} : {})});
      } catch { signal.throwIfAborted(); failures.push(`${value}: 无法访问`); }
    }
  });
  let applied = 0;
  ...
  await editLines(ctx,message,`✅ 成功添加 ${applied} 个频道。`,failures.length?["⚠️ 以下频道无法添加：",...failures]:[]);
}
...
async function search(ctx: PluginContext, message: MessageEnvelope, originalArgs: readonly string[], forcedType?: "kkp") {
  ...
  const config = await database(ctx).read(); if (!config.channelList.length) throw new BusinessError("请至少使用 so add 添加一个搜索频道。");
  ...
  await ctx.telegram.withClient(async (client: any, signal) => {
    ...
    for (const [index,handle] of order.entries()) {
      ...
      if(!info)continue;
      try {
        signal.throwIfAborted();const entity=await client.getEntity(handle);signal.throwIfAborted();
    ...
    if (!spoiler) {
      try { signal.throwIfAborted();await client.forwardMessages(peer, {messages:[selected.id], fromPeer:selected.peerId, ...(message.topicId ? {topMsgId: message.topicId} : {})});signal.throwIfAborted();if(message.outgoing&&typeof (message.raw as any)?.delete==="function")try{await (message.raw as any).delete();signal.throwIfAborted();}catch{signal.throwIfAborted();ctx.log.error("search_command_cleanup_failed");}return; } catch {signal.throwIfAborted();}
    }
    let fallbackSent=false;try{await ctx.files.withTemp(async (directory, scoped) => {
      ...
      try { for await (const chunk of client.iterDownload(selected.media, {signal:active})) { active.throwIfAborted(); total += chunk.length; if (total > MAX_MEDIA_BYTES) throw new BusinessError("视频超过 2 GiB 上限"); await writeAll(file,chunk,active);active.throwIfAborted(); } }
      finally { await file.close(); }
      if (!total) throw new BusinessError("视频下载为空"); scoped.throwIfAborted();
      const {Api}=await import("teleproto");
      active.throwIfAborted();
      const attribute=selected.video?.attributes.find((item:any)=>item.className==="DocumentAttributeVideo");
      await client.sendFile(peer, {file:target,caption:query||messageText(selected),spoiler,forceDocument:false,replyTo:message.id,...(message.topicId?{topMsgId:message.topicId}:{}),
      ...
      active.throwIfAborted();fallbackSent=true;
    });}catch(error){signal.throwIfAborted();if(!fallbackSent)throw error;ctx.log.error("search_temp_cleanup_failed");}
    signal.throwIfAborted();if(message.outgoing&&typeof (message.raw as any)?.delete==="function")try{await (message.raw as any).delete();signal.throwIfAborted();}catch{signal.throwIfAborted();ctx.log.error("search_command_cleanup_failed");}
  });
}
...
const command: CommandDefinition = {
  ...
  subcommands: {
    ...
    await database(ctx).update(current => {
      if (value && !current.channelList.some(item => item.handle === value)) throw new BusinessError("请先使用 so add 添加此频道。");
    ...
    list: {description: "列出频道源及默认项", args: "", handle: guarded(async ({message, args}, ctx) => {
      const raw = args.join(" ");
 const config = await database(ctx).read(); await editLines(ctx,message,config.channelList.length?"当前搜索频道列表":"没有添加任何搜索频道。",config.channelList.map((item,i)=>`${i+1}. ${item.title}${item.handle===config.defaultChannel?" (默认)":""}`));return;
    })},
    ...
    await ctx.telegram.withClient(async(client,signal)=>{signal.throwIfAborted();await client.sendFile((message.raw as any)?.peerId??returnBigInt(message.chatId),{file:Buffer.from(config.channelList.map(item=>item.handle).join("\n")),caption:"✅ 您的频道源已导出。",replyTo:message.id});signal.throwIfAborted();});return;

    })},
    import: {description: "回复备份文件导入频道源", args: "", handle: guarded(async ({message, args}, ctx) => {
      const raw = args.join(" ");

    const reply = await ctx.telegram.getReply(message); if (!reply) throw new BusinessError("❌ 请回复备份文件。");
    const text = await ctx.telegram.withClient(async (client: any, signal) => { const raw: any = reply.raw; if (!raw?.media || Number(raw.document?.size ?? 0) > MAX_IMPORT_BYTES) throw new BusinessError("备份文件超过 256 KiB"); const chunks:Buffer[]=[];let total=0;for await(const chunk of client.iterDownload(raw.media,{signal})){signal.throwIfAborted();total+=chunk.length;if(total>MAX_IMPORT_BYTES)throw new BusinessError("备份文件超过 256 KiB");chunks.push(Buffer.from(chunk));signal.throwIfAborted();}return Buffer.concat(chunks,total).toString("utf8"); });
    const handles = text.split(/\r?\n/).map(item => item.trim()).filter(Boolean); if (!handles.length) throw new BusinessError("备份文件无效。"); await add(ctx, message, handles.join("\\")); return;
    ...
    ad: {description: "管理广告过滤词", subcommands: {
      ...
      list: {description: "查看过滤词", args: "", handle: guarded(async (i, ctx) => {
        ...
        await editLines(ctx,i.message,config.adFilters.length?"当前广告过滤词":"当前没有广告过滤词。",config.adFilters);
      })},
    }, handle: guarded(async () => { throw new BusinessError("用法: so ad <add|del|list> [关键词]"); })},
