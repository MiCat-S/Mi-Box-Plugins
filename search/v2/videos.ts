...
export const normalize = (text: string) =>
  text
    ...
    .replace(/\s+/g, " ")
    .trim();
export const fileName = (message: any): string =>
  message?.video?.attributes?.find((a: any) => a.className === "DocumentAttributeFilename")?.fileName || "";
export const duration = (message: any): number =>
  Number(message?.video?.attributes?.find((a: any) => a.className === "DocumentAttributeVideo")?.duration || 0);
...
export function matches(message: any, query: string) {
  ...
  return [message.text, message.message, fileName(message)].some(
    source => source && fuzzyMatch(normalize(source), normalizedQuery),
  );
}
export function score(message: any, query: string) {
  const normalizedQuery = normalize(query);
  return (
    (fileName(message) && normalize(fileName(message)).includes(normalizedQuery) ? 100 : 0) +
...
export async function channelVideos(
  ...
  const pureVideo = (message: any) =>
    message.video && message.media?.className !== "MessageMediaWebPage" && !isAd(message);
  const videos: any[] = [];
  if (type === "kkp") {
    const messages = await read(entity, {
      limit: entity.className === "Channel" && entity.megagroup === true ? 200 : 100,
