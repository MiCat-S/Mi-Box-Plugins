...
async function readLegacyConfig(context: PluginContext): Promise<LegacyConfig | undefined> {
  let file: string;
  try {
    file = context.files.dataPath("aitc_config.db");
  } catch {
  ...
  try {
    ...
    throw new Error("LEGACY_STAT");
  }
  if (!info.isFile()) throw new Error("LEGACY_NOT_FILE");
  let rows: { key: string; value: string }[];
  try {
    rows = await context.storage
      .sqlite("aitc_config.db", { mustExist: true })
      .read(
        connection => connection.prepare("SELECT key, value FROM config").all() as { key: string; value: string }[],
      );
  } catch (error) {
  ...
  const map = new Map(
    rows
      .filter(row => row && typeof row.key === "string" && typeof row.value === "string")
      .map(row => [row.key, row.value] as const),
  );
  const result: LegacyConfig = {};
  const key = map.get("aitc_api_key");
  if (typeof key === "string" && key.length) result.apiKey = key;
  const url = map.get("aitc_api_url");
  if (typeof url === "string" && url.length) result.apiUrl = url;
  const model = map.get("aitc_model");
  if (typeof model === "string" && model.length) result.model = model;
  const prompt = map.get("aitc_prompt");
  if (typeof prompt === "string" && prompt.length) result.prompt = prompt;
  ...
  if (typeof promptsRaw === "string" && promptsRaw) {
    try {
      ...
      /* ignore malformed legacy prompt map */
    }
  }
...
): Promise<{ present: Set<string>; values: Record<string, unknown> } | undefined> {
  let file: string;
  try {
    file = context.files.dataPath("config.json");
  } catch {
    return undefined;
  }
  try {
...
async function migrateLegacy(context: PluginContext): Promise<void> {
  ...
  await store(context).update(value => {
    ...
    if (legacy) {
      ...
      if (!present.has("temperature") && value.temperature === initialTemperature && legacy.temperature !== undefined)
        patch.temperature = legacy.temperature;
    }
  ...
  const jsonSource = jsonKey
    ? { key: jsonKey, url: current.apiUrl || DEFAULT_API_URL, model: current.model || DEFAULT_MODEL }
    : undefined;
  const legacySource = legacyKey
    ? { key: legacyKey, url: legacy?.apiUrl || DEFAULT_API_URL, model: legacy?.model || DEFAULT_MODEL }
    : undefined;
  if (!jsonSource && !legacySource) {
    await store(context).update(value => (value.providerMigrated ? value : { ...value, providerMigrated: true }));
    return;
  }
  if (!context.services.available("ai", "import_provider")) return; // retry once ai is installed
  const importProvider = async (
    source: { key: string; url: string; model: string },
    tag: string,
    select: string[],
  ): Promise<void> => {
    ...
    await context.services.call(
      "ai",
      "import_provider",
      { tag, url, key: source.key, type: "openai-compatible", models: { chat: model }, select },
      context.signal,
    );
  };
  try {
    ...
    else if (legacySource && jsonSource && !sameProvider(jsonSource, legacySource))
      await importProvider(legacySource, "aitc-legacy", []);
  } catch (error) {
    if (context.signal.aborted) throw error;
...
function sameProvider(
  a: { key: string; url: string; model: string },
  b: { key: string; url: string; model: string },
): boolean {
  try {
    return (
      a.key === b.key &&
      (a.model.trim() || DEFAULT_MODEL) === (b.model.trim() || DEFAULT_MODEL) &&
      migrateBase(a.url) === migrateBase(b.url)
    );
  } catch {
    return false;
  }
...
function validateConfig(current: Config, patch: Record<string, unknown>): Config {
  ...
  if (patch.model !== undefined) {
    if (typeof patch.model !== "string" || !patch.model.trim() || patch.model.length > 200)
      throw new Error("Invalid model");
    next.model = patch.model.trim();
  }
  if (patch.prompt !== undefined) {
    if (typeof patch.prompt !== "string" || !patch.prompt.trim() || patch.prompt.length > 20_000)
  ...
  if (patch.prompts !== undefined) {
    if (!patch.prompts || typeof patch.prompts !== "object" || Array.isArray(patch.prompts))
...
export default function createAitc() {
  ...
  const guard =
    (run: (invocation: CommandInvocation, context: PluginContext) => Promise<void>) =>
    async (invocation: CommandInvocation, context: PluginContext): Promise<void> => {
      try {
        await run(invocation, context);
      } catch {
  ...
  const setString =
    (field: "prompt", missing: string, success: string) =>
    async (invocation: CommandInvocation, context: PluginContext): Promise<void> => {
      const value = stripTokens(invocation.message.text ?? "", 2);
      if (!value.trim()) {
        await edit(context, invocation, missing, false);
        return;
      }
  ...
  const setTemperature = async (invocation: CommandInvocation, context: PluginContext): Promise<void> => {
    ...
    if (!/^-?(?:\d+(?:\.\d+)?|\.\d+)$/.test(raw)) {
      await edit(context, invocation, "无效的温度值，请输入数字", false);
      return;
    }
    const value = Number(raw);
    if (value < 0 || value > 2) {
      await edit(context, invocation, "温度范围需在 0-2 之间", false);
      return;
    }
  ...
  const setPreset = async (invocation: CommandInvocation, context: PluginContext): Promise<void> => {
    ...
    if (!/^[a-z0-9_-]{1,32}$/.test(name) || RESERVED.has(name) || !value) {
      await edit(context, invocation, "Prompt 预设需要有效名称和内容", false);
      return;
    }
  ...
  const info = async (invocation: CommandInvocation, context: PluginContext): Promise<void> => {
    ...
    if (context.services.available("ai", "selection")) {
      const selected = await context.services.call<{ chat?: { tag?: string; model?: string } }>(
        "ai",
        "selection",
        null,
        context.signal,
      ...
      selection =
        selected.chat?.tag && selected.chat.model
          ? `${selected.chat.tag} / ${selected.chat.model}`
          : "请在 ai 插件中设置聊天模型";
    }
    await sendPaged(
  ...
  const aitc: CommandDefinition = {
    ...
    args: "[预设] [文本]",
    arguments: [
      { name: "预设", description: "已保存的 Prompt 预设名；省略时使用默认 Prompt" },
      { name: "文本", description: "要转写的文本；也可回复一条文字消息" },
    ],
    examples: [{ args: "文本" }, { args: "预设 文本" }, { args: "info" }],
    help: [
      {
        heading: "说明：",
        body: "使用 ai 插件当前聊天供应商与模型，温度默认 0.2，默认 Prompt 转写为英文。检索 Prompt 预设名后跟文本即使用该预设，否则使用默认 Prompt。",
      },
      {
        heading: "AI 配置：",
        body: "先安装 ai 插件，再用 <code>{prefix}ai config</code> 与 <code>{prefix}ai model chat</code> 统一管理供应商、密钥和模型。",
      },
    ],
    subcommandsCaseSensitive: false,
    subcommands: {
      key: {
        aliases: ["apikey", "_set_key"],
        description: "查看统一 AI 配置方式",
        args: "",
        handle: guard(centralConfig),
      },
      url: {
        aliases: ["api", "_set_url", "_set_api"],
        description: "查看统一 AI 配置方式",
        args: "",
        handle: guard(centralConfig),
      },
      ...
      prompt: {
        aliases: ["_set_prompt"],
        description: "设置默认 Prompt",
        args: "Prompt 文本",
        examples: [{ args: "prompt 翻译为英文" }],
        handle: guard(setString("prompt", "请提供 Prompt 文本", "默认 Prompt 已更新")),
      },
      temp: {
        aliases: ["temperature", "_set_temperature"],
        description: "设置模型温度",
        args: "0-2",
        examples: [{ args: "temp 0.2" }],
        handle: guard(setTemperature),
      },
      spn: {
        description: "保存或更新 Prompt 预设",
        args: "名称 Prompt",
        arguments: [
          { name: "名称", required: true, description: "字母、数字、下划线或连字符，1-32 位" },
          { name: "Prompt", required: true, description: "预设内容" },
        ],
        examples: [{ args: "spn en Translate to English" }],
        handle: guard(setPreset),
      },
      info: {
        aliases: ["_info"],
        description: "查看当前配置",
    ...
    async handle(invocation, context) {
      ...
      if (firstToken === "help" || firstToken === "h" || (!rest.trim() && invocation.message.replyToId === undefined)) {
        await edit(context, invocation, renderCommandHelp("aitc", aitc, { prefix: invocation.prefix }), true);
        return;
      }
      try {
        if (firstToken.startsWith("_")) {
          await edit(context, invocation, "未知配置命令", false);
          return;
        }
        const config = await store(context).read();
        ...
        if (!input.trim() && invocation.message.replyToId !== undefined)
          input = ((await context.telegram.getReply(invocation.message))?.text ?? "").trim();
        if (!input.trim()) {
          await edit(context, invocation, "请提供文本或回复一条文字消息", false);
          return;
        }
        if (input.length > 50_000) {
          await edit(context, invocation, "输入文本过长", false);
          return;
        }
        await migrateLegacy(context);
        if (!context.services.available("ai", "chat")) {
          await edit(context, invocation, "请先安装并配置 ai 插件", false);
          return;
        }
        await edit(context, invocation, "正在请求…", false);
        const content = await context.services.call<string>(
          "ai",
          "chat",
          { text: input, systemPrompt: prompt, temperature: config.temperature },
          context.signal,
        );
  ...
  return definePlugin({
    ...
    settings: context => ({
      ...
      async getValues() {
        ...
        return { temperature: value.temperature, prompt: value.prompt, prompts: value.prompts };
      },
      setValues: async patch => {
        await store(context).update(current => validateConfig(current, patch));
      },
    }),
    setup: migrateLegacy,
  });
