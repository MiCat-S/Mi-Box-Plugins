...
async function readLocation(ctx: PluginContext, url: string): Promise<Record<string, unknown>> {
  ...
  return ctx.http.withResponse(
    url,
    { headers: { "User-Agent": "Mi-Box-DNS/2" } },
...
function label(data: Record<string, unknown>): string {
  ...
  const asn =
    typeof data.asn === "number" && Number.isSafeInteger(data.asn) && data.asn > 0
      ? `AS${data.asn}`
      : /^(?:AS)?\d+$/.test(text(data.asn))
        ? `AS${text(data.asn).replace(/^AS/, "")}`
        : (text(data.org).match(/\bAS\d+\b/)?.[0] ?? "");
  ...
  for (const line of output.split("\n")) {
    ...
    if (address && isIP(address) && !locations.has(address)) {
      ...
      for (const url of [
        ...
        `https://ipinfo.io/${encodeURIComponent(address)}/json`,
      ]) {
        ctx.signal.throwIfAborted();
        try {
          location = label(await readLocation(ctx, url));
        } catch {
          ctx.signal.throwIfAborted();
        }
        if (location) break;
