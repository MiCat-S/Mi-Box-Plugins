...
const validStyle = (value: unknown): value is TextStyle =>
  typeof value === "string" && STYLES.includes(value as TextStyle);
const time = (zone: string, format: "12" | "24" = "24"): string =>
  new Intl.DateTimeFormat(format === "12" ? "en-US" : "zh-CN", {
    timeZone: zone,
    hour: "2-digit",
    minute: "2-digit",
...
const clock = (zone: string): string => {
  const hour =
    Number(new Intl.DateTimeFormat("en", { timeZone: zone, hour: "numeric", hourCycle: "h23" }).format(new Date())) %
    12;
...
function normalizeUser(userId: string, value: unknown): UserSettings {
  ...
  const originalLast =
    typeof source.original_last_name === "string" && source.original_last_name ? source.original_last_name : null;
  const components = Array.isArray(source.displayComponents)
    ? [
        ...new Set(
          source.displayComponents.filter(
            (entry): entry is string =>
              typeof entry === "string" && COMPONENTS.includes(entry as (typeof COMPONENTS)[number]),
          ),
        ),
      ]
    : undefined;
...
async function weather(
  ...
  try {
    ...
    return {
      text: `${weatherEmoji(current.weather_code)} ${Math.round(current.temperature_2m)}°C`,
      fetchedAt: Date.now(),
    };
  } catch {
...
async function updateUser(context: PluginContext, userId: string, force = false): Promise<boolean> {
  ...
  const selected =
    Array.isArray(user.displayComponents) && user.displayComponents.length
  ...
  const requested = (user.display_order || "name,time")
    .split(",")
    .map(value => value.trim())
    .filter(Boolean);
  const pieces = [...new Set([...requested, ...COMPONENTS])]
    .map(key => {
...
function protectSubcommand(definition: SubcommandDefinition): SubcommandDefinition {
  const subcommands = definition.subcommands
    ? Object.fromEntries(
        Object.entries(definition.subcommands).map(([name, child]) => [name, protectSubcommand(child)]),
      )
    : undefined;
  return {
    ...definition,
...
export default function createAutoChangeName(): PluginDefinition {
  ...
  const mutate = async (
    context: PluginContext,
    userId: string,
    transform: (user: UserSettings) => void,
  ...
  const updateOrderComponent = (user: UserSettings, component: string, enabled: boolean): void => {
    const order = (user.display_order || "name,time")
      .split(",")
      .map(value => value.trim())
      .filter(Boolean);
  ...
  const setTimezoneDisplay = async (
    invocation: CommandInvocation,
    context: PluginContext,
    enabled: boolean,
  ): Promise<void> => {
  ...
  const rawCommand: CommandDefinition = {
    ...
    arguments: [
      {
        name: "子命令",
        description: "save/on/off/mode/tz/text/show/time/emoji/order/style/weather/config/update/reset/status",
      },
    ],
      ...
      mode: {
        ...
        async handle(invocation, context) {
          await needUser(invocation, context, async (_state, _user, userId) => {
            ...
            const updated = await mutate(context, userId, target => {
              target.mode = next[target.mode];
            });
            await refreshIfEnabled(context, userId, updated);
            await edit(
              context,
              invocation,
              `✅ <b>显示模式已切换</b>\n\n📝 当前模式: <code>${updated?.mode || ""}</code>`,
              true,
            );
          });
        },
      ...
      tz: {
        ...
        subcommands: {
          ...
          on: {
            description: "开启时区显示",
            async handle(invocation, context) {
              await setTimezoneDisplay(invocation, context, true);
            },
          },
          off: {
            description: "关闭时区显示",
            async handle(invocation, context) {
              await setTimezoneDisplay(invocation, context, false);
            },
          },
          format: {
            description: "设置或查看时区格式",
            args: "[GMT|UTC|simp|offset|custom:文字]",
            async handle(invocation, context) {
              await needUser(invocation, context, async (_state, user, userId) => {
                ...
                const updated = await mutate(context, userId, target => {
                  target.timezone_format = format.toLowerCase().startsWith("custom:")
                ...
                await edit(
                  context,
                  invocation,
                  `✅ <b>时区格式已更新为:</b> <code>${escape(updated?.timezone_format || "")}</code>`,
                  true,
                );
              });
            },
            ...
            async handle(invocation, context) {
              await needUser(invocation, context, async state => {
                ...
                for (const candidate of candidates) {
                  ...
                  else if (state.random_texts.includes(candidate) || valid.includes(candidate))
                ...
                await store(context).update(current => ({
                  ...current,
                  random_texts: [...current.random_texts, ...accepted],
                }));
                ...
                if (invalid.length || valid.length > accepted.length)
                  response += `❌ 跳过 ${invalid.length + valid.length - accepted.length} 条超长或超出上限\n`;
          ...
          del: {
            ...
            async handle(invocation, context) {
              await needUser(invocation, context, async state => {
                ...
                await store(context).update(current => ({
                  ...current,
                  random_texts: current.random_texts.filter((_value, at) => at !== index),
                }));
                await edit(
                  context,
                  invocation,
                  `✅ <b>文本已删除</b>\n📊 剩余数量: ${state.random_texts.length - 1}`,
                  true,
                );
              });
            },
            ...
            async handle(invocation, context) {
              await needUser(invocation, context, async (_state, user, userId) => {
                const updated = await mutate(context, userId, target => {
                  ...
                  if (target.displayComponents && !target.displayComponents.includes("text"))
                    target.displayComponents.push("text");
                ...
                await edit(
                  context,
                  invocation,
                  `✅ <b>随机文案已开启</b>\n模式: <code>${updated?.mode || user.mode}</code>`,
                  true,
                );
              });
            },
          ...
          off: {
            ...
            async handle(invocation, context) {
              await needUser(invocation, context, async (_state, _user, userId) => {
                const updated = await mutate(context, userId, target => {
                  target.mode = "time";
                  if (target.displayComponents)
                    target.displayComponents = target.displayComponents.filter(component => component !== "text");
        ...
        async handle(invocation, context) {
          await needUser(invocation, context, async (_state, user, userId) => {
            ...
            if (format === "12" || format === "24") {
              const updated = await mutate(context, userId, target => {
                target.hour_format = format;
              });
              await refreshIfEnabled(context, userId, updated);
              await edit(
                context,
                invocation,
                `✅ 已切换为 ${format} 小时制${format === "12" ? "（如 02:32 AM）" : "（如 14:32）"}`,
              );
              return;
            }
            if (action !== "on" && action !== "off") {
              await edit(
                context,
                invocation,
                `请使用 ${invocation.prefix}acn time on/off 或 ${invocation.prefix}acn time 12/24`,
              );
              return;
            }
          ...
          await needUser(invocation, context, async (_state, user, userId) => {
            ...
            if (!action) {
              await edit(
                context,
                invocation,
                `<b>时钟Emoji</b>\n当前: <code>${user.show_clock_emoji ? "开启" : "关闭"}</code>\n使用 <code>${usage(invocation, "emoji on/off")}</code> 切换`,
                true,
              );
              return;
            }
          ...
          await needUser(invocation, context, async (_state, user, userId) => {
            ...
            if (!values.length) {
              await edit(
                context,
                invocation,
                `📋 <b>当前显示顺序</b>\n<code>${escape(user.display_order || "默认")}</code>\n使用 <code>${usage(invocation, "order time,name,weather...")}</code> 调整`,
                true,
              );
              return;
            }
          ...
          await needUser(invocation, context, async (_state, user, userId) => {
            ...
            if (action === "reset") {
              ...
              await edit(
                context,
                invocation,
                `✅ <b>已重置为默认值</b>\n\n当前模式默认组件: <code>${updated?.displayComponents?.join(", ") || ""}</code>`,
                true,
              );
              return;
            }
            ...
            if (target !== "on" && target !== "off") {
              await edit(
                context,
                invocation,
                `❌ <b>请指定 on 或 off</b>\n使用: <code>${usage(invocation, `show ${action} on/off`)}</code>`,
                true,
              );
              return;
            }
            if (action === "weather" && target === "on" && !user.weather_location?.trim()) {
              await edit(
                context,
                invocation,
                `❌ <b>请先设置天气地点</b>\n使用 <code>${usage(invocation, "weather set 北京")}</code>`,
                true,
              );
              return;
            }
            ...
            await edit(
              context,
              invocation,
              `✅ <b>组件已${target === "on" ? "开启" : "关闭"}</b>\n当前组件: <code>${updated?.displayComponents?.join(", ") || ""}</code>`,
              true,
            );
          });
        },
      ...
      weather: {
        ...
        subcommands: {
          on: {
            description: "开启天气",
            async handle(invocation, context) {
              await setWeather(invocation, context, "on");
            },
          },
          off: {
            description: "关闭天气",
            async handle(invocation, context) {
              await setWeather(invocation, context, "off");
            },
          },
          set: {
            description: "设置地点",
          ...
          await needUser(invocation, context, async (state, user) => {
            ...
            await edit(
              ...
              `<b>🔧 您的配置状态</b>\n${Object.entries(fields)
                .map(([label, value]) => `${label}: <code>${escape(String(value ?? ""))}</code>`)
                .join("\n")}`,
              true,
            );
          });
    ...
    help: [
      { heading: "命令说明：", body: "使用 <code>{prefix}acn</code> 查看完整帮助；所有设置保存在插件命名空间内。" },
    ],
    async handle(invocation, context) {
      ...
      await needUser(invocation, context, async () => {
        await edit(
          context,
          invocation,
          `❌ <b>未知命令</b>\n\n未知的子命令: <code>${escape(subcommand)}</code>\n\n输入 <code>${usage(invocation)}</code> 查看帮助。`,
          true,
        );
      });
    },
