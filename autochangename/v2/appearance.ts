...
export class NameAppearance {
  ...
  applyTextStyle(text: string, style?: TextStyleMode): string {
    ...
    return Array.from(text)
      .map(char => this.stylizeChar(char, finalStyle))
      .join("");
  }
  timezoneLabel(timezone: string, format = "GMT"): string {
    ...
    switch (format.toUpperCase()) {
      ...
      case "OFFSET":
        return this.getOffsetKey(sign, hours, minutes);
      case "UTC":
        return hours || minutes ? (minutes ? offset.replace("GMT", "UTC") : `UTC${sign}${hours}`) : "UTC";
      default:
        return hours || minutes ? offset.replace(/([+-])0(\d):00$/, "$1$2").replace(/:00$/, "") : "GMT";
    }
  }
}
