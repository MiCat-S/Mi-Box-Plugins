...
function normalize(source: Config): Config {
  ...
  try {
    ...
    if (url.protocol !== "https:" || url.username || url.password) base = defaults().git_api_base_url;
    else base = url.href.replace(/\/$/, "");
  } catch {
    base = defaults().git_api_base_url;
  }
  return {
    ...source,
    schemaVersion: 1,
    git_email: String(source.git_email ?? ""),
    git_username: String(source.git_username ?? ""),
    git_token: String(source.git_token ?? ""),
    git_api_base_url: base,
  };
}
...
async function api(
  ...
  const base = new URL(config.git_api_base_url);
  const url = new URL(endpoint.replace(/^\//, ""), `${base.href.replace(/\/$/, "")}/`);
  if (url.origin !== base.origin) throw new BusinessError("无效 API 路径");
  const result = await context.http.withResponse(
    url,
    {
      method,
      credentials: "omit",
    ...
    async (response, signal) => {
      ...
      try {
        if (reader)
          for (;;) {
            ...
            if (size > 2 * 1024 * 1024) return { error: "API 返回内容过大" };
      ...
      const text = parts.join("") + decoder.decode();
      signal.throwIfAborted();
...
export default function createGitPr() {
  return definePlugin({
    ...
    apiVersion: 1,
    id: "git_PR",
    description: "通过 Git API 管理 Pull Request",
    async setup(context) {
      await store(context).update(normalize);
    },
    commands: {
      git: {
        helpArgs: ["help", "h"],
          ...
          try {
            if (sub === "login") {
              if (!invocation.message.saved) {
                await context.telegram.edit(invocation.message, "Git Token 仅限在收藏夹中设置");
                return;
              }
              const [email, username, token] = invocation.args.slice(1);
              if (!email || !username || !token || token.length > 500)
                throw new BusinessError("格式：git login 邮箱 用户名 Token");
            ...
            if (sub === "repos") {
              ...
              const names = values
                .filter(value => value?.permissions?.push || value?.permissions?.admin || value?.permissions?.maintain)
                .map(value => (typeof value?.full_name === "string" ? value.full_name : ""))
                .filter(Boolean)
                .slice(0, 100);
              await output(
                invocation,
            ...
            if (sub === "prs") {
              ...
              const values = await api(
                ...
                config,
                "GET",
                `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/pulls?state=open&per_page=50`,
              );
              ...
              for (const value of values.slice(0, 50)) {
                ...
                const status =
                  mergeable === true
                    ? "✅ 可合并"
                    : mergeable === false
                      ? `⛔ 不可合并（${escape(state || "unknown")}）`
                      : "❓ 未知";
                lines.push(
                  `• <b>#${Number.isSafeInteger(number) ? number : 0}</b> ${escape(String(value?.title ?? "").slice(0, 500))}\n  作者：<code>${escape(value?.user?.login ?? "")}</code> | 状态：${status}`,
                );
              }
              await output(
                invocation,
                context,
                lines.length ? `<b>待处理的 PR</b>\n\n${lines.join("\n\n")}` : "没有待处理的 PR",
              );
              return;
            }
            if (sub === "merge") {
              const [owner, name] = repo(invocation.args[1] ?? "");
              const number = Number(invocation.args[2]);
            ...
            if (sub === "mergeall") {
              ...
              const values = await api(
                ...
                config,
                "GET",
                `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/pulls?state=open&per_page=100`,
              );
              if (!Array.isArray(values)) throw new BusinessError("PR 列表格式无效");
              const mergeable: any[] = [];
              for (const item of values) {
                ...
                try {
                  ...
                  if (detail?.mergeable === true) mergeable.push(item);
                } catch {
                  context.signal.throwIfAborted();
                }
              }
              if (!mergeable.length) {
                await context.telegram.edit(
                  invocation.message,
                  `仓库 <code>${escape(`${owner}/${name}`)}</code> 中没有可自动合并的 PR`,
                  { parseMode: "html" },
                );
                return;
              }
              ...
              for (const item of mergeable.sort((a, b) => Number(a?.number) - Number(b?.number))) {
                ...
                try {
                  ...
                  if (result?.merged !== true) throw new Error("not merged");
                  success++;
                } catch {
            ...
            const message =
              error instanceof BusinessError ? `操作失败：${escape(error.message)}` : "操作失败，请稍后重试";
            try {
              await context.telegram.edit(invocation.message, message, { parseMode: "html" });
            } catch {}
          }
        },
      },
    ...
    settings: context => ({
      id: "git_PR",
      title: "Git PR 管理",
      description: "Git API 与访问令牌",
      category: "插件配置",
      icon: "🔀",
      getSchema: () => [
        { key: "git_token", label: "Access Token", type: "password", secret: true },
        { key: "git_api_base_url", label: "Git API 地址", type: "string", required: true },
      ],
      getValues: async () => {
        const config = normalize(await store(context).read());
        return { git_token: config.git_token ? MASK : "", git_api_base_url: config.git_api_base_url };
      },
      setValues: async patch => {
        await store(context).update(source =>
          normalize({
            ...source,
            ...(typeof patch.git_token === "string" && patch.git_token && patch.git_token !== MASK
              ? { git_token: patch.git_token }
              : {}),
            ...(typeof patch.git_api_base_url === "string" ? { git_api_base_url: patch.git_api_base_url } : {}),
          } as Config),
        );
      },
    }),
