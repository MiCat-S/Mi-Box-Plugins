...
async function accountStats(context: PluginContext): Promise<{ chats: ChatStats; blocked: number; user: any }> {
  return context.telegram.withClient(async (client, signal) => {
    ...
    try {
      ...
      signal.throwIfAborted();
      blocked = Number.isFinite(blockedResult?.count)
        ? Number(blockedResult.count)
        : Array.isArray(blockedResult?.users)
          ? blockedResult.users.length
          : 0;
    } catch {
      signal.throwIfAborted();
...
async function hitokoto(context: PluginContext): Promise<string> {
  try {
    const data = (await context.http.json<unknown>(
      ...
      { timeoutMs: 10_000, signal: context.signal, redirects: { allowedHosts: ["v1.hitokoto.cn"], maxRedirects: 2 } },
    )) as { hitokoto?: unknown; from_who?: unknown; from?: unknown };
        ...
        async handle(invocation, context) {
          ...
          try {
            ...
            const name = user?.username
              ? `@${user.username}`
              : [user?.firstName, user?.lastName].filter(Boolean).join(" ") || "Telegram 用户";
