...
function root(value:string){let u:URL;try{u=new URL(value);}catch{throw new BusinessError("面板地址无效");}if(!/^https?:$/.test(u.protocol)||u.username||u.password)throw new BusinessError("面板地址无效");u.pathname=u.pathname.replace(/\/+$/,"");u.search="";u.hash="";return u;}
...
async function get(c:PluginContext,config:Config,endpoint:string){const u=root(config.url);u.pathname=`${u.pathname}${endpoint}`;return c.http.withResponse(u,{headers:{cookie:`nz-jwt=${jwt(config.secret)}`,"user-agent":"MiBot-Nezha/2",accept:"application/json"}},async(r,s)=>{const active=AbortSignal.any([c.signal,s]),reader=r.body?.getReader();if(!reader)throw new Error("empty response");const chunks:Buffer[]=[];let total=0,cancelling:Promise<void>|undefined;const cancel=()=>{cancelling??=reader.cancel().catch(()=>{});};const abort=()=>cancel();active.addEventListener("abort",abort,{once:true});if(active.aborted)cancel();try{for(;;){active.throwIfAborted();const x=await reader.read();active.throwIfAborted();if(x.done)break;total+=x.value.length;if(total>2*1024*1024)throw new Error("response too large");chunks.push(Buffer.from(x.value));}}finally{active.removeEventListener("abort",abort);cancel();await cancelling;reader.releaseLock();}let data:any;try{data=JSON.parse(Buffer.concat(chunks).toString("utf8"));}catch{throw new Error("invalid response");}if(!r.ok||data?.success!==true&&!Array.isArray(data))throw new Error("invalid response");return Array.isArray(data)?data:data.data;},{timeoutMs:15_000,redirects:{allowedHosts:[u.hostname],maxRedirects:2}});}
...
async function services(c:PluginContext,config:Config,id:number){const result=new Map<string,number>();try{const data=await get(c,config,`/api/v1/service/${id}`);if(Array.isArray(data))for(const x of data){const delay=Array.isArray(x?.avg_delay)?Number(x.avg_delay.at(-1)):NaN;if(typeof x?.monitor_name==="string"&&Number.isFinite(delay))result.set(x.monitor_name,delay);}}catch{c.signal.throwIfAborted();}return result;}
...
async function chart(c:PluginContext,m:MessageEnvelope,config:Config,target:string){
  ...
  const query=target.toLowerCase(),server=(all as Server[]).find(s=>String(s.id)===target||s.name.toLowerCase().includes(query));
  ...
  await c.files.withTemp(async(dir,signal)=>{
    ...
    await c.http.withResponse(u,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)},async(r,s)=>{
      ...
      const active=AbortSignal.any([c.signal,signal,s]),reader=r.body?.getReader();
      if(!reader)throw new Error("图表响应为空");
      let handle:FileHandle|undefined,total=0,cancelling:Promise<void>|undefined;
      const cancel=()=>{cancelling??=reader.cancel().catch(()=>{});},abort=()=>cancel();
      active.addEventListener("abort",abort,{once:true});if(active.aborted)cancel();
      try{
        ...
        active.removeEventListener("abort",abort);cancel();await cancelling;
        try{await handle?.close();}finally{reader.releaseLock();}
      }
    },{timeoutMs:30_000,redirects:{allowedHosts:["quickchart.io"],maxRedirects:1}});
    signal.throwIfAborted();
    await c.telegram.withClient(async(client,clientSignal)=>{const active=AbortSignal.any([c.signal,signal,clientSignal]);active.throwIfAborted();await client.sendFile((m.raw as any)?.peerId??m.chatId,{file,caption:`${server.name} 服务延迟`,replyTo:m.id});active.throwIfAborted();});
  });
}
async function migrate(c:PluginContext){const {readFile}=await import("node:fs/promises"),read=async(name:string)=>{c.signal.throwIfAborted();try{const value=JSON.parse(await readFile(c.files.dataPath(name),{encoding:"utf8",signal:c.signal}));c.signal.throwIfAborted();if(value===null||typeof value!=="object"||Array.isArray(value))throw new Error("Invalid Nezha configuration root");return value;}catch(e){c.signal.throwIfAborted();if((e as NodeJS.ErrnoException).code==="ENOENT")return{};throw e;}};const persisted=await read("config-v2.json");if(persisted.legacyImported===true)return;const legacy=await read("config.json"),has=(key:string)=>Object.hasOwn(persisted,key);await store(c).update(v=>({...v,url:has("url")?String(persisted.url??""):String(legacy.url??v.url),secret:has("secret")?String(persisted.secret??""):String(legacy.secret??v.secret),serviceMonitor:has("serviceMonitor")?typeof persisted.serviceMonitor==="boolean"?persisted.serviceMonitor:v.serviceMonitor:typeof legacy.serviceMonitor==="boolean"?legacy.serviceMonitor:v.serviceMonitor,legacyImported:true}));}
const guarded = (operation: CommandDefinition["handle"]): CommandDefinition["handle"] => async (i, c) => {
  try { await operation(i, c); }
  catch(e) { if (!c.signal.aborted) await c.telegram.edit(i.message, `❌ ${esc(e instanceof BusinessError ? e.message : "哪吒请求失败，请稍后重试")}`, {parseMode: "html"}); }
};
...
const command: CommandDefinition = {
  ...
  subcommands: {
    ...
    service: {description: "设置列表中的服务延迟显示，默认开启", subcommandsCaseSensitive: true,
      ...
      handle: configured(async (i,c,config) => { await c.telegram.edit(i.message,`📶 服务监控当前状态: <b>${config.serviceMonitor?"开启":"关闭"}</b>\n\n用法: <code>${esc(i.prefix)}nezha service on/off</code>`,{parseMode:"html"}); })},
