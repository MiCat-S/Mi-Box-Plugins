...
function reverseLayout(text: string): TextLayout {
  ...
  while (lineStart <= text.length) {
    ...
    const lineEnd = hasLineFeed
      ? lineFeed > lineStart && text[lineFeed - 1] === "\r"
        ? lineFeed - 1
        : lineFeed
      : text.length;
    ...
    for (const value of graphemes) {
      ...
      units.push({
        sourceStart,
        sourceEnd: sourceStart + value.segment.length,
        outputStart: lineStart + line.length - value.index - value.segment.length,
        outputEnd: lineStart + line.length - value.index,
        text: value.segment,
      });
    }
    ...
    units.push({
      sourceStart: lineEnd,
      sourceEnd: newlineEnd,
      outputStart: lineEnd,
      outputEnd: newlineEnd,
      text: newline,
    });
    output.push(newline);
...
function reversedEntities(layout: TextLayout, sourceText: string, entities: readonly any[]): any[] {
  ...
  entities.forEach((entity, sourceIndex) => {
    ...
    if (
      ...
      length > sourceText.length - offset
    )
      return;
    ...
    const candidates = merged.flatMap((range, pieceIndex): EntityRange[] => {
      ...
      for (
        let tail = outputUnitByEnd.get(rangeEnd);
        tail && tail.outputStart >= range.start && tail.text.trim().length === 0;
        tail = outputUnitByEnd.get(rangeEnd)
      )
        rangeEnd = tail.outputStart;
      if (rangeEnd <= range.start) return [];
      return [
        {
          entity: Object.assign(Object.create(Object.getPrototypeOf(entity)), entity, {
            offset: range.start,
            length: rangeEnd - range.start,
          }),
          offset: range.start,
          length: rangeEnd - range.start,
          sourceIndex,
...
function mediaInfo(
  raw: ApiTypes.Message | undefined,
): { extension: string; gif: boolean; webm: boolean; webp: boolean } | undefined {
  if (!raw?.media) return;
  ...
  const filename =
    (document?.attributes ?? []).map((value: any) => value?.fileName).find((value: any) => typeof value === "string") ??
    "";
...
async function editReversedReply(context: PluginContext, invocation: any, reply: any): Promise<void> {
  ...
  if (!entities.length || !raw?.peerId) {
    await context.telegram.edit(invocation.message, layout.text);
    return;
  }
  await context.telegram.withClient(async (client, clientSignal) => {
    ...
    signal.throwIfAborted();
    const peer = await client.getInputEntity(raw.peerId!);
    signal.throwIfAborted();
    await client.invoke(
      new Api.messages.EditMessage({ peer, id: invocation.message.id, message: layout.text, entities }),
    );
    signal.throwIfAborted();
  });
}
...
export default function createRev() {
  return definePlugin({
    ...
    apiVersion: 1,
    id: "rev",
    description: "反转文字或翻转回复的媒体",
    resources: { processes: { concurrency: 1, queueCapacity: 2, timeoutMs: 180_000, maxOutputBytes: 512 * 1024 } },
    commands: {
      rev: {
        description: "反转文字或翻转回复的媒体",
        async handle(invocation, context) {
          ...
          if (selected.text) {
            await context.telegram.edit(invocation.message, reverse(selected.text));
            return;
          }
          const reply =
          ...
          if (!info && reply?.text) {
            await editReversedReply(context, invocation, reply);
            return;
          }
          if (!info) {
          ...
          try {
            ...
            try {
              await context.files.withTemp(async (directory, signal) => {
                ...
                await context.telegram.withClient(async (client, clientSignal) => {
                  ...
                  try {
                    for await (const chunk of client.iterDownload(source.media! as any, { signal: combined })) {
                      ...
                      total += chunk.length;
                      if (total > MAX_MEDIA_BYTES) throw new Error("Input too large");
                      let offset = 0;
                      while (offset < chunk.length) {
                        combined.throwIfAborted();
                        const result = await file.write(chunk, offset, chunk.length - offset);
                        combined.throwIfAborted();
                        if (result.bytesWritten <= 0) throw new Error("Write failed");
                        offset += result.bytesWritten;
                      }
                    }
                    if (!total) throw new Error("Empty media");
                  } finally {
                    await file.close();
                  }
                  combined.throwIfAborted();
                ...
                await runFfmpeg(
                  context,
                  ffmpegArgs(input, output, selected.flip, selected.invert, info.gif, info.webm),
                  directory,
                  signal,
                );
                const result = await stat(output);
                signal.throwIfAborted();
                if (!result.isFile() || !result.size || result.size > MAX_MEDIA_BYTES)
                ...
                await context.telegram.withClient(async (client, clientSignal) => {
                  ...
                  if (info.webm || info.webp) {
                    ...
                    combined.throwIfAborted();
                    options.attributes = [
                      new Api.DocumentAttributeSticker({ alt: "rev", stickerset: new Api.InputStickerSetEmpty() }),
                    ];
                  }
                  combined.throwIfAborted();
              ...
              if (sent) {
                context.log.error("rev_temp_cleanup_failed");
              } else throw error;
            }
            const raw = invocation.message.raw as ApiTypes.Message | undefined;
            if (typeof raw?.delete === "function") {
              ...
              try {
                ...
                context.signal.throwIfAborted();
                context.log.error("rev_command_cleanup_failed");
              }
            }
            try {
              await context.telegram.edit(invocation.message, "✅ 媒体已处理完成");
            } catch {
              context.signal.throwIfAborted();
              context.log.error("rev_receipt_failed");
            }
          } catch (error) {
