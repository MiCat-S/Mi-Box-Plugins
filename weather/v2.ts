...
async function get(ctx: PluginContext, url: string, params: Record<string, string | number>): Promise<any> {
  ...
  return ctx.http.json(
    target.toString(),
    {},
    { timeoutMs: 10000, redirects: { allowedHosts: [target.hostname], maxRedirects: 2 } },
  );
}
...
async function cityQuery(ctx: PluginContext, city: string): Promise<string> {
  ...
  try {
    const translated = await get(ctx, "https://translate.googleapis.com/translate_a/single", {
      client: "gtx",
      sl: "auto",
      tl: "en",
      dt: "t",
...
function report(name: string, data: any): string {
  ...
  if ([...required, ...optional].some(value => typeof value !== "number" || !Number.isFinite(value)))
      ...
      weather: {
        ...
        async handle(invocation, ctx) {
          ...
          await ctx.telegram.edit(
            ...
            name === city
              ? `🌍 <b>正在搜索 ${esc(name)}...</b>`
              : `🌍 <b>正在搜索...</b>\n<i>${esc(city)} → ${esc(name)}</i>`,
            { parseMode: "html" },
          );
          let geo: any;
          try {
            geo = await get(ctx, "https://geocoding-api.open-meteo.com/v1/search", {
              name,
              count: 10,
              language: "zh",
              format: "json",
            ...
            if (ctx.signal.aborted) throw error;
            await ctx.telegram.edit(invocation.message, "天气查询失败，请稍后重试");
            return;
          }
          const place = geo?.results?.[0];
          if (!place) {
            await ctx.telegram.edit(invocation.message, "未找到该城市");
            return;
          }
          ...
          const label = [place.name, place.admin1 !== place.name ? place.admin1 : undefined, place.country]
            .filter(value => value && value !== "undefined")
            .join(", ");
          await ctx.telegram.edit(invocation.message, `🌡️ <b>正在获取 ${esc(label)} 的天气...</b>`, {
            parseMode: "html",
          });
          let output: string;
          try {
            const data = await get(ctx, "https://api.open-meteo.com/v1/forecast", {
