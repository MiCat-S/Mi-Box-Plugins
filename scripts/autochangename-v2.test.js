...
test.before(async () => {
  const built = buildPlugin({
    id: "autochangename",
    packageRoot: path.resolve(__dirname, "../autochangename"),
    entry: "v2.ts",
  });
...
async function fixture(t, fetch = async () => assert.fail("Unexpected HTTP")) {
  ...
  const client = {
    async getMe() {
      return { id: 7, firstName: "Alice", lastName: "User" };
    },
...
test("acn accepts multiline texts and reports the full user configuration", async t => {
  ...
  for (const value of [
    ...
    assert.ok(f.edits.at(-1).includes(value));
  await f.run(".acn off");
...
async function parityFixture(t, options = {}) {
  ...
  const client = {
    async getMe() {
      return options.getProfile ? options.getProfile() : { id: 7, firstName: "Alice", lastName: "User" };
    },
    async invoke(request) {
  ...
  const host = new PluginHost({
    ...
    http: { fetch: options.fetch ?? (async () => assert.fail("Unexpected HTTP")) },
    telegram: {
    ...
    initial: {
      users: {
        7: {
          user_id: 7,
          timezone: "Asia/Tokyo",
          original_first_name: "Old",
          original_last_name: "Name",
          is_enabled: false,
          mode: "both",
          last_update: "2025-01-01T00:00:00.000Z",
          text_index: 2,
          text_style: "mono",
          display_order: "name,text,time",
          marker: "keep",
        },
      },
      random_texts: ["focus"],
...
test("command failures classify flood waits and never expose unknown details", async t => {
  ...
  const secrets = [
    ...
    "sk-live-private-key",
    "token-123",
    "https://private.example.test/?key=hidden",
  ];
  ...
  assert.deepEqual(
    ...
    ["autochangename_command_failed", "autochangename_command_failed"],
  );
  assert.equal(
    errors.every(entry => entry.fields === undefined),
    true,
  );
});
...
test("managed job cancels between users and registers exactly once after reload", async t => {
  ...
  const f = await parityFixture(t, {
    initial: { users: { 7: user("7"), 8: user("8") }, random_texts: [] },
    onInvoke: async (_request, count) => {
      if (count === 1) {
        entered();
        await gate;
      }
