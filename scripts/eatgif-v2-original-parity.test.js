...
test("accepts every shipped animation while rejecting loose numeric and excessive pixel budgets", async () => {
  ...
  assert.throws(
    () => moduleUnderTest.validateDetail({ width: "2", height: 2, res: [{ url: "frame.png" }] }),
    /Invalid animation/,
  );
  assert.throws(
    () =>
      moduleUnderTest.validateDetail({
        width: 512,
        height: 512,
        res: Array.from({ length: 65 }, () => ({ url: "frame.png" })),
      }),
    /Invalid animation/,
  );
});
  ...
  const actual = await moduleUnderTest.downloadAvatarDirect(
    {
      ...
      async downloadProfilePhoto() {
        throw new Error("scheduler path must not run");
      },
...
test("actively cancels a blocked HTTP reader and stops avatar RPC chains after cancellation", async () => {
  ...
  const pending = moduleUnderTest.responseBytes(
    {
      http: {
        withResponse: async (_url, _options, use) =>
          use(
            {
              ok: true,
              body: {
    ...
    new URL("https://github.com/frame.png"),
    10,
  );
  controller.abort();
  ...
  let invokes = 0;
  let releaseEntity;
  const entityGate = new Promise(resolve => {
    releaseEntity = resolve;
  });
  const avatarPending = moduleUnderTest.downloadAvatarDirect(
    {
      async getEntity() {
        await entityGate;
        return {};
  ...
  let deletes = 0;
  const uploadPending = moduleUnderTest.uploadResult(
    {
      signal: uploadController.signal,
      log: { info() {}, error() {} },
      telegram: {
        withClient: async use =>
          use(
            {
              async sendFile() {
                await uploadGate;
...
test("scope unload waits for the first reader cancellation and retains its task directory until release", async t => {
  ...
  const work = files.withTemp(async (directory, signal) => {
    temporary = directory;
    return moduleUnderTest.responseBytes(
      { http: { withResponse: (_url, _options, use) => use({ ok: true, body: { getReader: () => reader } }, signal) } },
      new URL("https://github.com/frame.png"),
      10,
    );
  });
...
test("routes list through the real Host and uses the active prefix", async t => {
  ...
  const catalog = Object.fromEntries(
    Array.from({ length: 240 }, (_, index) => [`item${index}`, { url: "demo.json", desc: `<Demo ${index}>` }]),
  );
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async reply(_message, text, options) {
        replies.push({ text, options });
      },
      async invoke() {},
      async getReply() {},
      async withClient() {
        assert.fail("list must not acquire Telegram client");
      },
    },
  });
...
test("keeps HTTP bodies owned, clears in-flight cache atomically, and falls back to GIF with fixed FFmpeg argv", async t => {
  ...
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  const frame = await sharp({ create: { width: 2, height: 2, channels: 4, background: "#ff0000ff" } })
    .png()
    .toBuffer();
  const avatar = new Api.User({
    id: 1,
    accessHash: 2,
    firstName: "Avatar",
  ...
  const context = {
    ...
    files: {
      dataPath(relative) {
        return path.join(root, "data", relative);
      },
      async dataFile(relative) {
        const target = path.join(root, "data", relative);
        await fs.mkdir(path.dirname(target), { recursive: true });
        if (holdStaging && relative.endsWith(".part")) {
          stagingEntered += 1;
          await stagingGate;
        }
        return target;
      ...
      async withTemp(use) {
        ...
        try {
          return await use(directory, context.signal);
        } finally {
    ...
    processes: {
      async run(command, args, options) {
        ...
        throw Object.assign(new Error("hidden-secret"), { code: "SPAWN_FAILED" });
      },
    },
    telegram: {
      ...
      async getReply() {
        return { raw: { sender: avatar, senderId: avatar.id } };
      },
        ...
        return use(
          {
            ...
            async invoke() {
              return new Api.upload.File({ type: new Api.storage.FileJpeg(), mtime: 0, bytes: frame });
            },
  ...
  const concurrentRun = plugin.commands.eatgif.handle(
    {
      command: "eatgif",
      prefix: ".",
      args: ["demo"],
      message: { ...message, id: 2, raw: { ...message.raw, async delete() {} } },
    },
    context,
  );
  ...
  await plugin.commands.eatgif.handle(
    { command: "eatgif", prefix: ".", args: ["clear"], message: { ...message, replyToId: undefined } },
    context,
  );
  releaseFrame();
  ...
  holdStaging = true;
  stagingGate = new Promise(resolve => {
    releaseStaging = resolve;
  });
  const publishing = plugin.commands.eatgif.handle(
    { command: "eatgif", prefix: ".", args: ["demo"], message },
    context,
  );
  while (stagingEntered === 0) await new Promise(resolve => setImmediate(resolve));
  let clearFinished = false;
  const clearing = plugin.commands.eatgif
    .handle({ command: "eatgif", prefix: ".", args: ["clear"], message: { ...message, replyToId: undefined } }, context)
    .then(() => {
      clearFinished = true;
    });
  await new Promise(resolve => setImmediate(resolve));
