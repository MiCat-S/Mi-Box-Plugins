...
async function fixture(t, id, client, message = {}) {
  ...
  const host = new PluginHost({
    ...
    logger: { info() {}, error() {} },
    telegram: {
      async edit(envelope, text, options) {
        edits.push({ envelope, text, options });
      },
      async reply(envelope, text, options) {
        replies.push({ envelope, text, options });
      },
...
test("dbdj samples unique valid recent speakers and deletes its command", async t => {
    ...
    async getMessages() {
      return [1, 2, 1, 3].map(
        id =>
          new Api.Message({
            id,
            peerId: new Api.PeerUser({ userId: id }),
            fromId: new Api.PeerUser({ userId: id }),
            message: "x",
          }),
      );
    },
  ...
  const f = await fixture(t, "dbdj", client, {
    raw: {
      peerId: {},
      async delete() {
        deleted++;
...
test("netease sends a direct media response returned by Music163bot", async t => {
  ...
  const client = {
    ...
    async getMessages() {
      return sent.some(item => item.value.message === "/music 12345")
  ...
  const f = await fixture(t, "netease", client, {
    raw: {
      peerId: {},
      async delete() {
        deleted++;
  ...
  assert.equal(
    sent.some(item => item.peer === "Music163bot" && item.value.message === "/music 12345"),
    true,
  );
...
test("clear_sticker deletes only sticker documents from history", async t => {
  ...
  const client = {
    async getInputEntity() {
      return new Api.InputPeerChannel({ channelId: 1, accessHash: 2 });
    },
    async invoke() {
      return { messages: [sticker, plain] };
    },
...
test("restore_pin restores unique unpinned messages from the admin log", async t => {
  ...
  const eventMessage = new Api.Message({
    id: 55,
    peerId: new Api.PeerChannel({ channelId: 1 }),
    message: "",
    pinned: false,
  ...
  const client = {
    ...
    async getMe() {
      return new Api.User({ id: 9, firstName: "Owner" });
    },
    async invoke(request) {
      if (request instanceof Api.channels.GetParticipant)
        return { participant: new Api.ChannelParticipantCreator({ userId: 9 }) };
      if (request instanceof Api.channels.GetAdminLog)
        return { events: [{ action: new Api.ChannelAdminLogEventActionUpdatePinned({ message: eventMessage }) }] };
      if (request instanceof Api.messages.UpdatePinnedMessage) {
        pinned.push(request.id);
        return {};
      }
