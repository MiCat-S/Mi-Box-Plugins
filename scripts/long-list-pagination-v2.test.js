...
async function fixture(t, id, { files = {}, http, replyFails = false } = {}) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async reply(_m, text) {
        if (replyFails) throw new Error("PAGE_DELIVERY_FAILED");
        replies.push(text);
      },
      async invoke() {
  ...
  return {
    ...
    replies,
    run: text =>
      host.dispatchPrimary({ id: 9, chatId: "123", senderId: "123", outgoing: true, text, raw: { peerId: "123" } }),
...
test("komari paginates a long report instead of one oversized edit", async t => {
  ...
  const f = await fixture(t, "komari", {
    ...
    http: async target => {
      if (target.pathname.endsWith("/api/public"))
        return Response.json({ status: "success", data: { sitename: siteName } });
      if (target.pathname.endsWith("/api/version"))
        return Response.json({ status: "success", data: { version: "1.0", hash: "abc" } });
