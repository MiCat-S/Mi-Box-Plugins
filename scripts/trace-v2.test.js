...
async function fixture(t, { initial, premium = false, prefixes } = {}) {
  ...
  let failReply = false;
  let deletion;
  const deletionDone = new Promise(resolve => {
    deletion = resolve;
  });
  let reply;
  const host = new PluginHost({
    ...
    telegram: {
      async edit(_, text) {
        if (failEdit) throw new Error("transport secret");
        edits.push(text);
      },
      async reply(_, text) {
        if (failReply) throw new Error("reply transport secret");
        replies.push(text);
      },
      async invoke() {},
      async getReply() {
  ...
  return {
    ...
    failEdits(value = true) {
      failEdit = value;
    },
    failReplies(value = true) {
      failReply = value;
    },
    run: (text, extra = {}) => host.dispatchPrimary({ ...message, text, ...extra }),
    listen: (text, extra = {}) =>
      host.dispatchListeners({ ...message, senderId: "77", outgoing: false, text, ...extra }),
  };
}
...
test("trace status preserves tracked details and paginates within Telegram limits", async t => {
  ...
  const f = await fixture(t, {
    initial: { users: { "9007199254740993": ["🔥"] }, keywords, config: { big: true, keepLog: true } },
  });
...
test("trace keyword listener preserves graphemes, config and own-message filtering", async t => {
  ...
  assert.deepEqual(
    f.reactions[0].reaction.map(r => r.emoticon),
    ["❤️‍🔥", "👍"],
  );
  await f.run(".trace big false");
  await f.listen("hello");
...
test("trace reads legacy string reaction IDs and preserves unrelated stored metadata", async t => {
  const f = await fixture(t, {
    initial: {
      users: { 77: ["👍", "9007199254740993"] },
      keywords: {},
      config: { big: false, keepLog: true, marker: "keep" },
      marker: "keep",
    },
  });
  await f.listen("hello");
      ...
      raw: {
        entities: [
          new Api.MessageEntityCustomEmoji({
            offset: text.indexOf("👍"),
            length: 2,
            documentId: bigInt("9007199254740993"),
          }),
        ],
      },
    });
