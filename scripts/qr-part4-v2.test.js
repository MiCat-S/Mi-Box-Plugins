...
const create = require(
  path.join(
    buildPlugin({ id: "qr", packageRoot: path.resolve(__dirname, "../qr"), entry: "v2.ts" }).artifactDir,
    "index.cjs",
  ),
).default;
  ...
  const context = {
    ...
    files: {
      async withTemp(operation) {
        const directory = await fs.mkdtemp(path.join(os.tmpdir(), "qr-part4-"));
        try {
          return await operation(directory, signal);
        } finally {
    ...
    processes: {
      async run(command) {
        return {
          stdout: Buffer.from(command.includes("zbarimg") ? (options.decoded ?? "") : "PNG"),
    ...
    create().commands.qr.handle(
      {
        ...
        message: {
          id: 1,
          chatId: "1",
          replyToId: options.reply ? 2 : undefined,
          text: `.qr ${args.join(" ")}`,
          outgoing: true,
          raw,
        },
