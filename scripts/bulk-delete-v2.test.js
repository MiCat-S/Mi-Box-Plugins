...
const { artifactDir } = buildPlugin({
  id: "bulk_delete",
  packageRoot: path.resolve(__dirname, "../bulk_delete"),
  entry: "v2.ts",
});
...
function fixture(options = {}) {
  ...
  const client = {
    ...
    async deleteMessages(chat, ids, settings) {
      deleted.push({ chat, ids, settings });
    },
    async sendMessage(chat, value) {
  ...
  return {
    ...
    run(
      args,
      message = { id: 10, chatId: "7", text: `.bd ${args.join(" ")}`, outgoing: true, raw: { chatId: "7" } },
      prefix = ".",
    ) {
...
test("bd exposes only a fixed collection error when native history lookup fails", async () => {
  ...
  const f = fixture({
    client: {
      async getMessages(_chat, params) {
        if (params.ids) return [{ id: 5, senderId: 2n }];
        const error = new Error(`BOT_TOKEN=${secret}`);
        error.name = secret;
        throw error;
      },
    },
  ...
  const f = fixture({
    client: {
      async getMessages(chat) {
        chats.push(chat);
        return [{ id: 9, senderId: 1n }];
      },
      async deleteMessages(chat, ids) {
        chats.push(chat);
        f.deleted.push({ chat, ids });
      },
      async sendMessage(chat, value) {
        chats.push(chat);
        f.sent.push({ chat, value });
        return { id: 50 };
      },
    },
  });
...
test("bd permission hint uses the active command prefix", async () => {
  const f = fixture({
    initial: { schemaVersion: 1, userDeleteMode: { 1: false } },
    client: {
      async getMessages(_chat, params) {
        if (params.ids) return [{ id: 5, senderId: 2n }];
        return [
...
test("real host resolves and serializes the self-permission RPC before deleting a range", async t => {
  ...
  const client = {
    ...
    async getMessages(_chat, params) {
      ...
      return [
        { id: 5, senderId: integer(2) },
        { id: 10, senderId: integer(1) },
      ];
    },
    async getEntity() {
      return { className: "Channel" };
    },
      ...
      await request.resolve(
        {
          async getInputEntity(value) {
            ...
            if (value instanceof Api.PeerChannel)
    ...
    async sendMessage() {
      assert.fail("successful range deletion must be silent");
    },
  };
...
test("bd delayed cleanup releases abort listeners on completion and cancellation", async () => {
  ...
  const pending = sleep(60000, controller.signal),
...
test("bd delayed cleanup reports only a fixed safe failure event", async () => {
  ...
  candidate.paths = Module._nodeModulePaths(path.dirname(filename));
  candidate._compile(
    fsSync.readFileSync(filename, "utf8") + "\nmodule.exports.removeLaterForTest=removeLater;",
    filename,
  );
