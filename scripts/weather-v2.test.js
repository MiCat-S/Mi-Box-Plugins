...
async function fixture(t, options = {}) {
  ...
  const fetch = options.fetch || (async (url, init) => {
    requests.push({url: String(url), init});
    const parsed = new URL(url);
    if (parsed.hostname.includes('translate')) return new Response(JSON.stringify([[['Test City', '测试城']]]), {status: 200});
    ...
    return new Response(JSON.stringify(options.weather || {current: {temperature_2m: 20, apparent_temperature: 19, relative_humidity_2m: 50, precipitation: 0, rain: 0, snowfall: 0, weather_code: 0, cloud_cover: 25, pressure_msl: 1012, wind_speed_10m: 8, wind_direction_10m: 0, wind_gusts_10m: 0}, daily: {temperature_2m_max: [25], temperature_2m_min: [12], sunrise: ['2026-09-06T05:30'], sunset: ['2026-09-06T18:30']}}), {status: 200});
  });
  const host = new PluginHost({storageRoot: root, ...(options.prefixes ? {prefixes: options.prefixes} : {}), logger: {info() {}, error() {}}, http: {fetch}, telegram: {
  ...
  return {edits, requests, shutdown: () => host.shutdown(1000), run: text => host.dispatchPrimary({id: 1, chatId: '1', senderId: '1', outgoing: true, text})};
}
...
test('weather accepts escaped punctuation and renders legacy fog and snow warnings', async t => {
  const f = await fixture(t, {weather: {current: {temperature_2m: 1, apparent_temperature: -1, relative_humidity_2m: 80, precipitation: 0, rain: 0, snowfall: 1, weather_code: 71, cloud_cover: 90, pressure_msl: 1000, wind_speed_10m: 5, wind_direction_10m: 360, wind_gusts_10m: 0}, daily: {temperature_2m_max: [2], temperature_2m_min: [-2], sunrise: ['2026-09-06T05:30'], sunset: ['2026-09-06T18:30']}}});
...
test('weather cancellation aborts a hanging managed request without a late failure receipt', async t => {
  let aborted = false;
  const f = await fixture(t, {fetch: async (_url, init) => new Promise((_, reject) => {
