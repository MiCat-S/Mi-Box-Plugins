...
async function fixture(t, initial) {
  ...
  return {
    edits,
    read: async () => JSON.parse(await fs.readFile(path.join(root, "aff/data.json"), "utf8")),
    setReply(value) {
      reply = value;
    },
...
test("aff reads legacy entries and migrates singleton once while preserving metadata", async t => {
  const f = await fixture(t, {
    affs: [{ text: "<b>old</b>", web_page: true, created_at: 123 }],
    aff: { text: "singleton", web_page: false },
    marker: "preserved",
  });
  await f.run(".aff 1");
...
test("aff paginates imported lists with bounded escaped Unicode previews", async t => {
  const f = await fixture(t, {
    affs: Array.from({ length: 35 }, (_, i) => ({
      text: `entry${i + 1} <&😀`.repeat(50),
      web_page: false,
    })),
  });
