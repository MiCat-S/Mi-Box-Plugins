  ...
  return {
    json() {
      return {
        ...
        async update(fn) {
          value = await fn(structuredClone(value));
          return structuredClone(value);
        },
      };
    },
...
async function fixture(t, options = {}) {
  ...
  const ctx = {
    ...
    files: {
      ...
      async withTemp(use) {
        ...
        let value;
        try {
          value = await use(dir, controller.signal);
        } finally {
    ...
    http: {
      async withResponse(url, init, consume, limits) {
    ...
    telegram: {
      async edit(message, text, editOptions) {
        ...
        await options.edit?.(edits.length, text);
      },
      async getReply() {
  ...
  return {
    ...
    logs,
    controller,
    command: async (args, extra = {}) =>
      create(options.dependencies).commands.op.handle(
        {
          message: {
            id: 1,
...
test("candidate exposes one structured command tree, complete dynamic help and bounded resources", () => {
  ...
  for (const item of [
    ...
    assert.match(help, new RegExp(item));
...
test("credential-bearing commands alone require Saved Messages", async t => {
  ...
  for (const command of [
    root.subcommands.login,
    root.subcommands.admin.subcommands.setpass,
    root.subcommands.admin.subcommands.random,
  ]) {
...
test("login and settings preserve spaced secrets without echoing them", async t => {
  ...
  assert.equal(
    f.edits.some(edit => edit.text.includes("s e c r e t")),
    false,
  );
  ...
  assert.deepEqual(await settings.getValues(), {
    username: "alice",
    password: "new secret",
    defaultPath: "/media/uploads",
  });
});
...
test("setup imports legacy credentials once and initializes the managed port", async t => {
  ...
  await fs.writeFile(
    path.join(f.root, "credentials.json"),
    JSON.stringify({ username: "legacy-user", password: "legacy-secret", defaultPath: "/legacy" }),
  );
  await create().setup(f.ctx);
  assert.deepEqual(f.store.value(), {
    schemaVersion: 1,
  ...
  await fs.writeFile(
    path.join(f.root, "credentials.json"),
    JSON.stringify({ username: "replace", password: "replace" }),
  );
  await create().setup(f.ctx);
...
test("admin mutations use fixed argv, synchronize credentials and never echo passwords", async t => {
  const f = await fixture(t, {
    initial: { username: "old-user", password: "old-pass" },
    process(call) {
      if (call.command === "/opt/openlist/openlist" && call.args.join(" ") === "admin random") {
        return {
          stdout: Buffer.from("username: random-user\npassword: random-secret\n"),
  ...
  assert.deepEqual(
    f.processes.filter(call => call.command === "/opt/openlist/openlist").map(call => call.args),
    [
      ["admin", "setuser", "new-user"],
      ["admin", "set", "new secret"],
      ["admin", "random"],
    ],
  );
  assert.equal(
    f.edits.some(edit => /new secret|random-secret/.test(edit.text)),
    false,
  );
});
...
test("a successful operation followed by receipt failure is not relabeled as an operation failure", async t => {
  const f = await fixture(t, {
    edit: async () => {
      throw new Error("private telegram transport");
    },
  ...
  assert.equal(
    f.logs.some(item => item.event === "openlist_operation_failed"),
    false,
  );
  assert.equal(
    JSON.stringify({ texts: f.edits.map(edit => edit.text), logs: f.logs }).includes("private telegram transport"),
    false,
  );
});
...
test("local API abort actively cancels a hanging reader and waits for its cleanup", async t => {
  ...
  let finished = false;
  const running = f.command(["save"]).then(() => {
    finished = true;
  });
  await readReady;
  f.controller.abort();
...
test("save success survives temporary cleanup failure without a false operation error", async t => {
  ...
  const f = await fixture(t, {
    initial: { username: "alice", password: "secret" },
    tempCleanupError: "private temp cleanup",
    reply: { id: 2, raw: { media: {}, file: { name: "sample.bin" } } },
    client: {
      async *iterDownload() {
        yield Buffer.from("media");
    ...
    response: async () =>
      Response.json(++responses === 1 ? { code: 200, data: { token: "token" } } : { code: 200, data: null }),
  });
  ...
  assert.equal(
    JSON.stringify({ texts: f.edits.map(edit => edit.text), logs: f.logs }).includes("private temp cleanup"),
    false,
  );
});

test("install uses a verified staged binary, initializes credentials and never echoes the password", async t =>
  linux(t, async () => {
    const f = await fixture(t, {
      dependencies: {
        async releaseBinary(_ctx, directory) {
          const binary = path.join(directory, "verified-openlist");
      ...
      process(call) {
        if (call.command === "/opt/openlist/openlist")
          return {
            stdout: Buffer.from("username: admin\npassword: top-secret\n"),
            stderr: Buffer.alloc(0),
            exitCode: 0,
          };
        if (call.args[0] === "is-active")
          return { stdout: Buffer.from("inactive\n"), stderr: Buffer.alloc(0), exitCode: 0 };
        if (call.args[0] === "is-enabled")
          return { stdout: Buffer.from("disabled\n"), stderr: Buffer.alloc(0), exitCode: 0 };
      },
    });
    ...
    assert.ok(
      f.processes.some(
        call =>
          call.command === "/usr/bin/install" &&
          call.args.includes("0755") &&
          call.args.at(-1) === "/opt/openlist/openlist",
      ),
    );
    assert.ok(
      f.processes.some(
        call =>
          call.command === "/usr/bin/install" &&
          call.args.includes("0644") &&
          call.args.at(-1) === "/etc/systemd/system/openlist.service",
      ),
    );
    assert.ok(
      f.processes.some(
        call => call.command === "/usr/bin/systemctl" && call.args.join(" ") === "enable --now openlist",
      ),
    );
    assert.ok(
      f.processes.some(
        call => call.command === "/bin/rm" && call.args[0] === "-rf" && call.args[1].includes("/recovery/"),
      ),
    );
  }));

test("install success survives temporary cleanup failure without a false failure receipt", async t =>
  linux(t, async () => {
    const f = await fixture(t, {
      tempCleanupError: "private install temp cleanup",
      dependencies: {
        async releaseBinary(_ctx, directory) {
          const binary = path.join(directory, "verified");
          await fs.writeFile(binary, "verified");
          return binary;
        },
        async ordinary() {
      ...
      process(call) {
        if (call.command === "/opt/openlist/openlist")
          return { stdout: Buffer.from("username: admin\npassword: secret\n"), stderr: Buffer.alloc(0), exitCode: 0 };
        if (call.args[0] === "is-active")
          return { stdout: Buffer.from("inactive\n"), stderr: Buffer.alloc(0), exitCode: 0 };
        if (call.args[0] === "is-enabled")
          return { stdout: Buffer.from("disabled\n"), stderr: Buffer.alloc(0), exitCode: 0 };
    ...
    assert.doesNotMatch(f.edits.at(-1).text, /操作失败/);
    assert.ok(
      f.logs.some(item => item.event === "openlist_temp_cleanup_failed" && item.fields.operation === "install"),
    );
  }));

test("built-in release rejects untrusted bytes before tar extraction", async t =>
  linux(t, async () => {
    const f = await fixture(t, {
      dependencies: {
        async ordinary() {
    ...
    assert.equal(
      f.processes.some(call => call.command === "/usr/bin/tar"),
      false,
    );
  }));

test("release open failure still cancels and unlocks the acquired response reader", async t =>
  linux(t, async () => {
    let response,
      canceled = 0;
    const f = await fixture(t, {
      ...
      beforeTempUse: async directory =>
        fs.writeFile(path.join(directory, "openlist-linux-musl-arm64.tar.gz"), "collision"),
      response: async () =>
        (response = new Response(
          new ReadableStream({
            start(controller) {
              controller.enqueue(Buffer.from("archive"));
            },
            cancel() {
              canceled += 1;
            },
...
test("release download abort actively cancels a hanging reader and awaits cleanup", async t =>
  linux(t, async () => {
    let reading, cancelStarted, releaseCancel;
    const readReady = new Promise(resolve => {
      reading = resolve;
    ...
    let finished = false;
    const running = f.command(["install"]).then(() => {
      finished = true;
    });
    await readReady;
    f.controller.abort();
...
test("failed update restores binary, service and data snapshots with the prior service state", async t =>
  linux(t, async () => {
    const f = await fixture(t, {
      dependencies: {
        async releaseBinary(_ctx, directory) {
        ...
        async ordinary(target) {
          return ["/opt/openlist/openlist", "/etc/systemd/system/openlist.service", "/opt/openlist/data"].includes(
            target,
          );
        },
      },
      process(call) {
        ...
        if (joined === "is-active openlist")
          return { stdout: Buffer.from("active\n"), stderr: Buffer.alloc(0), exitCode: 0 };
        if (joined === "is-enabled openlist")
          return { stdout: Buffer.from("enabled\n"), stderr: Buffer.alloc(0), exitCode: 0 };
    ...
    assert.ok(
      commands.some(value =>
        /install -m 0644 .*openlist\.service\.previous \/etc\/systemd\/system\/openlist\.service$/.test(value),
      ),
    );
...
test("rollback failure retains persistent binary, service and data snapshots", async t =>
  linux(t, async () => {
    ...
    const f = await fixture(t, {
      ...
      async process(call) {
        ...
        if (call.command === "/bin/cp" && call.args[2]?.includes("/recovery/")) {
          if (call.args[1] === "/opt/openlist/data") {
            await fs.mkdir(call.args[2], { recursive: true });
            await fs.writeFile(path.join(call.args[2], "marker"), "old-data");
          } else {
            await fs.mkdir(path.dirname(call.args[2]), { recursive: true });
            await fs.writeFile(call.args[2], call.args[1].includes("service") ? "old-service" : "old-bin");
          }
        }
        if (joined === "is-active openlist")
          return { stdout: Buffer.from("active\n"), stderr: Buffer.alloc(0), exitCode: 0 };
        ...
        if (!failedInstall && call.command === "/usr/bin/install" && call.args[2]?.includes("/recovery/")) {
          failedInstall = true;
          throw new Error("sk-rollback-secret");
        }
      },
    });
    await f.command(["update"]);
...
test("mid-update cancellation retains snapshots and does not bypass SDK cancellation for rollback", async t =>
  linux(t, async () => {
    const f = await fixture(t, {
      dependencies: {
        async releaseBinary(_ctx, directory) {
      ...
      async process(call, _count, controller) {
        ...
        if (call.command === "/bin/cp" && call.args[2]?.includes("/recovery/")) {
          if (call.args[1] === "/opt/openlist/data") {
            ...
            await fs.mkdir(path.dirname(call.args[2]), { recursive: true });
            await fs.writeFile(call.args[2], "snapshot");
          }
        }
        if (joined === "is-active openlist")
          return { stdout: Buffer.from("active\n"), stderr: Buffer.alloc(0), exitCode: 0 };
    ...
    const batches = await fs.readdir(path.join(f.root, "recovery"));
    assert.equal(batches.length, 1);
    assert.deepEqual((await fs.readdir(path.join(f.root, "recovery", batches[0]))).sort(), [
      "data.previous",
      "openlist.previous",
      "openlist.service.previous",
    ]);
    assert.equal(
      f.processes.filter(call => call.args.join(" ") === "stop openlist").length,
      1,
      "rollback process calls are rejected by the cancelled SDK scope",
    );
  }));
...
test("uninstall disables the service and removes only the unit file while preserving data", async t => {
  ...
  assert.ok(
    f.processes.some(call => call.command === "/usr/bin/systemctl" && call.args.join(" ") === "disable --now openlist"),
  );
  assert.ok(
    f.processes.some(
      call => call.command === "/bin/rm" && call.args.join(" ") === "-f /etc/systemd/system/openlist.service",
    ),
  );
  assert.ok(f.processes.some(call => call.command === "/usr/bin/systemctl" && call.args.join(" ") === "daemon-reload"));
  assert.equal(
    f.processes.some(call => call.args.includes("/opt/openlist/data")),
    false,
  );
});
...
test("backup uses a validated millisecond name and removes a partial directory on copy failure", async t => {
  const f = await fixture(t, {
    process(call) {
      if (call.command === "/bin/date")
        return { stdout: Buffer.from("20260913_182233_007\n"), stderr: Buffer.alloc(0), exitCode: 0 };
  ...
  assert.ok(
    f.processes.some(
      call =>
        call.command === "/bin/rm" && call.args.join(" ") === "-rf /opt/openlist_backups/backup_20260913_182233_007",
    ),
  );
});
  ...
  const f = await fixture(t, {
    ...
    async process(call) {
      if (call.command === "/bin/cp") {
        await fs.copyFile(call.args[1], call.args[2]);
        return;
      }
      if (call.command === "/usr/bin/install") {
        await fs.copyFile(call.args.at(-2), call.args.at(-1));
        await fs.chmod(call.args.at(-1), 0o600);
        return;
      }
      if (call.args.join(" ") === "is-active openlist")
        return { stdout: Buffer.from("active\n"), stderr: Buffer.alloc(0), exitCode: 0 };
    },
  });
...
test("setport success survives temporary cleanup failure and keeps the committed port", async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "openlist-port-cleanup-")),
...
test("setport rollback failure retains its persistent config snapshot without committing the port", async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "openlist-port-rollback-")),
    config = path.join(directory, "config.json");
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  ...
  const f = await fixture(t, {
    ...
    async process(call) {
      ...
      if (joined === "is-active openlist")
        return { stdout: Buffer.from("active\n"), stderr: Buffer.alloc(0), exitCode: 0 };
      if (joined === "start openlist" && ++startCalls === 1)
        throw new Error("OpenList 初始账号信息未能读取；sk-port-start-secret");
    },
  });
  await f.command(["setport", "6262"]);
  assert.match(f.edits.at(-1).text, /OpenList 操作失败/);
...
test("setport cancellation after stop retains config snapshot and performs no post-cancel mutation", async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "openlist-port-cancel-")),
    config = path.join(directory, "config.json");
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const original = JSON.stringify({ scheme: { http_port: 5244 }, marker: "old" });
  await fs.writeFile(config, original);
...
test("restore failure atomically puts the old data directory back and restarts prior active service", async t => {
  ...
  const f = await fixture(t, {
    dependencies: {
      async ordinary(target) {
        return (
          target === "/opt/openlist_backups/backup_20260913_182233_007/data" ||
          target === "/opt/openlist/data" ||
          target.includes("/data.mibot-stage-")
        );
      },
    },
    process(call) {
  ...
  assert.match(f.edits.at(-1).text, /OpenList 操作失败/);
  assert.equal(
    JSON.stringify({ texts: f.edits.map(edit => edit.text), logs: f.logs }).includes("private restore failure"),
    false,
  );
  const commands = f.processes.map(call => `${call.command} ${call.args.join(" ")}`);
