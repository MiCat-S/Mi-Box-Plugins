...
const result = JSON.stringify({
  isp: "Example ISP",
  server: { id: 123, name: "Test & Node", location: "Shanghai" },
  interface: { externalIp: "203.0.113.9", name: "test0" },
  ping: { latency: 8.5, jitter: 1.2 },
  download: { bandwidth: 12500000, bytes: 50000000 },
  upload: { bandwidth: 6250000, bytes: 25000000 },
...
test.before(async () => {
  ...
  esbuild.buildSync({
    entryPoints: [path.join(plugins, "speedtest/v2/report.ts")],
    outfile: reportOutput,
    bundle: true,
    packages: "external",
    alias: { "telebox/sdk": path.join(core, "dist/v2/sdk.js") },
    platform: "node",
    format: "cjs",
    target: "node24",
  });
  reportTools = require(reportOutput);
  png = await sharp({ create: { width: 32, height: 20, channels: 4, background: { r: 25, g: 120, b: 220, alpha: 1 } } })
    .png()
    .toBuffer();
});
test.after(async () => {
  if (moduleRoot) await fs.rm(moduleRoot, { recursive: true, force: true });
});
...
async function hostFixture(t, options = {}) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async reply(message, text, messageOptions, signal) {
        signal.throwIfAborted();
...
function fetchForResult(image = png) {
  return async (input, init = {}) => {
    ...
    if ((init.method || "GET") === "HEAD" && url.hostname === "www.speedtest.net")
      return new Response(null, { status: 204 });
    if (url.hostname === "ip-api.com")
      return Response.json({ as: "AS64500 Example", country: "China", countryCode: "CN" });
...
test("loads with the declared process budget and preserves both command aliases across unload/reload", async t => {
  ...
  assert.deepEqual(
    f.host.listCommands().map(value => value.name),
    ["speedtest", "st"],
  );
...
test("legacy migration is lossless, V2-preferred, and idempotent", async t => {
  ...
  const f = await hostFixture(t, {
    setup: async root => {
      const dir = path.join(root, "speedtest");
      await fs.mkdir(dir, { recursive: true });
      await fs.writeFile(
        path.join(dir, "config.json"),
        `{"default_server_id":9,"preferred_type":"photo","legacyBig":${big}}`,
      );
      await fs.writeFile(path.join(dir, "speedtest.json"), '{"default_server_id":10,"commandOnly":"keep"}');
      await fs.writeFile(
        path.join(dir, "v2-config.json"),
        `{"schemaVersion":1,"default_server_id":77,"preferred_type":"txt","legacyImported":false,"v2Big":${big}}`,
      );
    },
  });
...
test("lazy install accepts a bounded response without Content-Length and update truncation preserves the old CLI", async t => {
  ...
  const f = await hostFixture(t, {
    fetch: async (input, init = {}) => {
      ...
      return new Response(archive.subarray(0, Math.max(1, archive.length - 7)), {
        status: 200,
...
test("cancelling a queued serial command releases cleanup and never starts the queued process", async t => {
  ...
  f = await hostFixture(t, {
    setup: async root => {
      ...
      await writeExecutable(
        path.join(root, "speedtest/speedtest"),
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    try {
      if ((await fs.readFile(marker)).length) break;
    } catch {}
...
test("archive download writes every short write before replacing the managed CLI", async t => {
  ...
  t.mock.method(fs, "open", async (...args) => {
    ...
    return new Proxy(handle, {
      get(target, key) {
        if (key === "write")
          return (buffer, offset, length) => target.write(buffer, offset, Math.max(1, Math.floor(length / 2)));
...
test("a zero-progress archive write fails and preserves the old CLI", async t => {
  ...
  const managed = path.join(f.root, "speedtest/speedtest"),
    before = await fs.readFile(managed),
  ...
  t.mock.method(fs, "open", async (...args) => {
    ...
    return new Proxy(handle, {
      get(target, key) {
        if (key === "write") return async () => ({ bytesWritten: 0, buffer: Buffer.alloc(0) });
        const value = Reflect.get(target, key, target);
        return typeof value === "function" ? value.bind(target) : value;
      },
    });
...
test("sticker decoding enforces the external image pixel limit before falling back", async t => {
  const oversized = await sharp({ create: { width: 4097, height: 4097, channels: 3, background: "black" } })
    .png()
    .toBuffer();
  const f = await hostFixture(t, {
    setup: async root => {
      ...
      await fs.writeFile(
        path.join(dir, "v2-config.json"),
        JSON.stringify({ schemaVersion: 1, default_server_id: null, preferred_type: "sticker", legacyImported: true }),
      );
...
test("photo, sticker, file, and txt modes deliver real image artifacts with their documented behavior", async t => {
  for (const type of [null, "photo", "sticker", "file", "txt"]) {
    let deleted = 0;
    const f = await hostFixture(t, {
      fetch: fetchForResult(),
      setup: async root => {
        const dir = path.join(root, "speedtest");
        await fs.mkdir(dir, { recursive: true });
        await fs.writeFile(
          path.join(dir, "v2-config.json"),
          JSON.stringify({ schemaVersion: 1, default_server_id: null, preferred_type: type, legacyImported: true }),
        );
...
test("media failure falls through to text without exposing transport errors", async t => {
  const f = await hostFixture(t, {
    ...
    setup: async root => {
      ...
      await fs.writeFile(
        path.join(dir, "v2-config.json"),
        JSON.stringify({ schemaVersion: 1, default_server_id: null, preferred_type: "photo", legacyImported: true }),
      );
...
for (const externalIp of ["203.0.113.9", "2001:db8::1234"]) {
  test(`report omits client address and interface (${externalIp})`, async t => {
    ...
    const context = {
      signal,
      http: { json: async () => ({}) },
      tasks: { run: async (_name, operation) => operation(signal) },
    };
...
test("report masks IP fields in server and ISP names", async t => {
  const sample = JSON.parse(result);
  sample.server.name = "38.59.246.201";
  sample.isp = "2001:db8::1234";
  const signal = new AbortController().signal;
