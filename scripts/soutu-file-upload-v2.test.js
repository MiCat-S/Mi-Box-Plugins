...
async function fixture(
  t,
  { bytes = Buffer.from("ffd8ffe000104a464946", "hex"), download, upload, media = "photo" } = {},
) {
  ...
  const edits = [],
    ...
    targets = [];
  const raw = {
    ...(media === "photo" ? { photo: {} } : media === "document" ? { document: { mimeType: "image/png" } } : {}),
    async downloadMedia(options) {
      assert.equal(typeof options.outputFile, "string");
      assert.ok(options.signal instanceof AbortSignal);
      targets.push(options.outputFile);
  ...
  const host = new PluginHost({
    ...
    telegram: {
      async edit(_m, text, options) {
        ...
        events.push(text.includes("正在下载") ? "progress" : "result");
      },
      async reply() {},
      async invoke() {
  ...
  return {
    ...
    targets,
    run: (text = ".soutu") =>
      host.dispatchPrimary({ id: 1, chatId: "1", senderId: "1", outgoing: true, replyToId: 2, text }),
    async cleaned() {
      for (const file of targets) await assert.rejects(fs.stat(path.dirname(file)), { code: "ENOENT" });
    },
  };
}
...
for (const [name, hex] of [
  ["photo.jpg", "ffd8ffe000104a464946"],
  ["photo.png", "89504e470d0a1a0a"],
  ["photo.gif", "474946383961"],
  ["photo.webp", "524946460000000057454250"],
  ["photo.jpg", "01"],
]) {
...
test("soutu accepts the 20 MiB boundary using a file-backed multipart body", async t => {
  const f = await fixture(t, {
    download: async ({ outputFile, progressCallback }) => {
      const file = await fs.open(outputFile, "w");
      try {
        await file.truncate(20 * 1024 * 1024);
      } finally {
        await file.close();
      }
...
for (const size of [0, 20 * 1024 * 1024 + 1]) {
  test(`soutu rejects a ${size}-byte file before upload`, async t => {
    ...
    await f.run();
    assert.equal(f.requests.length, 0);
    assert.match(f.edits.at(-1).text, /搜图失败/);
    await f.cleaned();
  });
}
...
test("soutu unload cancels a partial download before upload", async t => {
  ...
  const f = await fixture(t, {
    download: async ({ outputFile, signal }) => {
      await fs.writeFile(outputFile, "partial");
...
test("soutu delivers the returned link once when temporary cleanup fails", async t => {
  ...
  const f = await fixture(t, {
    upload: async () => {
      uploads += 1;
      return new Response("https://0x0.st/clean.jpg");
    },
  });
