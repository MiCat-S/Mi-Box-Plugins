...
function compile(source, mocks = {}) {
  ...
  vm.runInNewContext(
    ...
    }).code,
    {
      ...
      fetch() {
        assert.fail("real fetch is forbidden");
      },
    },
  );
...
const { ScopedHttp } = compile(fs.readFileSync(path.join(core, "src/v2/http.ts"), "utf8"), {
  ...
  undici: require(require.resolve("undici", { paths: [core] })),
});
...
const legacyHelpers = [
  ...
  "aggregateOpenAIResponses",
  "collectResponsesSources",
  "parseResponsesOutputContent",
  "aggregateResponsesApiPayloads",
  "parseOpenAIResponsePayloads",
...
const sse = payloads =>
  payloads.map(value => `data: ${JSON.stringify(value)}\r\n\r\n`).join("") + "data: [DONE]\r\n\r\n";
const defer = () => {
  let resolve;
...
test("provider selection and path normalization match original helpers", async () => {
  for (const url of [
    ...
    "https://fixture.invalid/custom",
    "https://fixture.invalid/x/api/v1/chat/completions?q=private-source",
    "https://fixture.invalid/v1/messages",
    "https://fixture.invalid/v1/images/generations",
  ...
  assert.deepEqual(clean(provider.selectChatProvider(snapshot)), {
...
test("text request bodies, URL/auth/UA and flags match original callAI across providers", async () => {
  const providers = [
    ...
    { url: "http://127.0.0.1:8080/api/v1/messages" },
    ...
    for (const responses of [false, true])
      for (const stream of [false, true]) {
        const snapshot = config(
          { currentChatReasoningEffort: "xhigh", currentChatServiceTier: "fast" },
          { ...p, responses, stream },
        );
        const old = original(
          snapshot,
          provider.resolveProviderType(snapshot.configs.main) === "gemini"
            ? gemini("translated")
...
test("reasoning/service tier fields are never dropped or changed by model heuristics", async () => {
  for (const responses of [false, true])
    for (const reasoning of ["auto", "none", "minimal", "low", "medium", "high", "xhigh"]) {
...
test("translation uses exact legacy independent prompt, same selection and immutable settings", async t => {
  ...
  Object.freeze(snapshot.configs.main);
  Object.freeze(snapshot.configs);
  ...
  for (const target of ["zh-CN", "en"]) {
    ...
    assert.equal(
      await old.translation.translateText("private-source\n\nparagraph", target, signal),
...
test("OpenAI JSON and SSE output aggregation matches original successful text fixtures", async () => {
  const values = [
    ...
    sse([
      ...
      completion("ignored fallback"),
    ]),
    sse([
      { type: "response.created" },
      { type: "response.output_text.delta", delta: "a " },
      { type: "response.output_text.delta", delta: "b" },
      { type: "response.output_text.done", text: "a b" },
      { type: "response.completed", response: responseObject("a b") },
    ]),
...
test("SSE supports arbitrary byte boundaries and does not duplicate completed text", async t => {
  ...
  assert.equal(
    await provider.translateText(config({}, { stream: true, responses: true }), f.http, "source", "en"),
    "你好 world",
  );
});
...
test("malformed/empty provider replies are rejected instead of echoing raw bodies or placeholder translations", async t => {
  const cases = [
    ["", "EMPTY_OUTPUT"],
    [" ", "EMPTY_OUTPUT"],
    ["private-source secret-key", "INVALID_RESPONSE"],
    ['{"secret-key":"private-source"', "INVALID_RESPONSE"],
    ["null", "EMPTY_OUTPUT"],
    ["{}", "EMPTY_OUTPUT"],
    ["[]", "EMPTY_OUTPUT"],
    [JSON.stringify(completion(" ")), "EMPTY_OUTPUT"],
    [JSON.stringify(completion({ text: { secret: "secret-key" } })), "EMPTY_OUTPUT"],
    [JSON.stringify(responseObject("")), "EMPTY_OUTPUT"],
    ["data: [DONE]\n", "EMPTY_OUTPUT"],
  ...
  for (const [raw, code] of cases) {
    ...
    await assert.rejects(provider.translateText(config(), f.http, "private-source", "en"), error =>
      noSecrets(error, code),
    );
  }
  for (const raw of ["null", "{}", '{"candidates":[{"content":{"parts":{}}}]}']) {
    assert.throws(
      () => provider.parseChatText(raw, "gemini"),
      error => noSecrets(error, "EMPTY_OUTPUT"),
    );
  }
});
...
test("provider error envelopes never expose keys/source text even after a partial SSE result", async t => {
  for (const body of [
    ...
    await assert.rejects(provider.translateText(config(), f.http, "private-source", "en"), error =>
      noSecrets(error, "PROVIDER"),
    );
  }
  assert.throws(
    () =>
      provider.parseChatText(JSON.stringify({ data: { error: { message: "secret-key private-source" } } }), "gemini"),
    error => noSecrets(error, "PROVIDER"),
  );
});
    ...
    const f = fixture(
      ...
      async () =>
        new Response(
          new ReadableStream({
            start(controller) {
              controller.enqueue(Buffer.from("secret-key private-source"));
            },
            cancel() {
              cancelled = true;
            },
...
test("injected HTTP and fake fetch errors are scrubbed, with no automatic retries", async t => {
  ...
  await assert.rejects(
    ...
    error => noSecrets(error, "FAILED"),
  );
  const forged = new provider.ProviderError("FAILED");
  forged.code = "secret-key";
  forged.status = "private-source";
  await assert.rejects(
    provider.translateText(
      config(),
      {
...
test("byte limit uses actual UTF-8 chunks, not Content-Length; exact boundary succeeds", async t => {
  ...
  for (const extra of [0, -1]) {
    ...
    const task = provider.translateText(config(), f.http, "source", "en", undefined, {
      maxResponseBytes: Buffer.byteLength(raw) + extra,
    });
...
test("output limit rejects rather than truncates JSON/SSE/Gemini and counts UTF-16 before trimming", () => {
  for (const [raw, format] of [
    ...
    assert.throws(
      () => provider.parseChatText(raw, format, { maxOutputChars: 1 }),
      error => noSecrets(error, "OUTPUT_TOO_LARGE"),
    );
  }
  assert.throws(
    () => provider.parseChatText(JSON.stringify(completion(" a ")), "openai", { maxOutputChars: 1 }),
    error => noSecrets(error, "OUTPUT_TOO_LARGE"),
  );
  const long = "a".repeat(provider.DEFAULT_PROVIDER_LIMITS.maxOutputChars + 1);
  assert.throws(
    () => provider.parseChatText(JSON.stringify(completion(long)), "openai"),
    error => noSecrets(error, "OUTPUT_TOO_LARGE"),
  );
});
...
test("invalid selection/config/limits and pre-aborted calls never reach HTTP", async t => {
  ...
  const snapshots = [
    ...
    config({ currentChatTag: "missing" }),
    config({ currentChatTag: "__proto__" }),
    ...[0, -1, Infinity, NaN, 2147484].map(timeout => config({ timeout })),
    config({}, { url: "secret-key private-source" }),
    config({}, { url: "file:///private-source" }),
  ];
  for (const snapshot of snapshots) {
    await assert.rejects(provider.translateText(snapshot, f.http, "private-source", "en"), error =>
      noSecrets(error, "CONFIG"),
    );
  }
  for (const maxResponseBytes of [-1, Infinity, 0.1, undefined]) {
    await assert.rejects(
      provider.translateText(config(), f.http, "source", "en", undefined, { maxResponseBytes }),
      error => noSecrets(error, "CONFIG"),
    );
  }
  const controller = new AbortController();
  controller.abort(new Error("secret-key private-source"));
  await assert.rejects(provider.translateText(config(), f.http, "source", "en", controller.signal), error =>
    noSecrets(error, "ABORTED"),
  );
...
test("body size overflow awaits cleanup; cleanup failures remain sanitized", async t => {
  ...
  const f = fixture(
    ...
    async () =>
      new Response(
        new ReadableStream({
          start(controller) {
            controller.enqueue(Buffer.from("private-source secret-key"));
          },
          cancel() {
            cancelling.resolve();
            return cleanup.promise;
  ...
  await assert.rejects(provider.translateText(config(), bad.http, "private-source", "en"), error =>
    noSecrets(error, "CLEANUP_FAILED"),
  );
});
...
test("errored stream cleanup follows the SDK error category and releases readers", async t => {
  ...
  const f = fixture(t, async () => {
    body = new ReadableStream({
      pull(controller) {
        controller.error(new Error("secret-key private-source"));
      },
    });
...
test("empty HTTP body, SSE terminal fragments and query-key encoding stay within the text contract", async t => {
  ...
  await assert.rejects(provider.translateText(config(), f.http, "source", "en"), error =>
    noSecrets(error, "EMPTY_OUTPUT"),
  );
...
test("restricted models are rejected before credentials or source text reach HTTP", async t => {
  for (const model of ["gpt-5.6-luna", "gpt-5.6-luna-fast", "GPT-5.6-TERRA/preview"]) {
    ...
    assert.throws(
      () => provider.buildChatRequest(snapshot, "private-source"),
      error => noSecrets(error, "CONFIG"),
    );
...
test("Anthropic chat uses Messages auth, multimodal content and text parsing", async () => {
  const snapshot = config(
    { currentChatModel: "claude-sonnet" },
    { type: "anthropic", url: "https://api.anthropic.com", key: "anthropic-key" },
  );
  const request = provider.buildChatRequest(snapshot, "describe", "system", {
    temperature: 0.3,
    maxOutputTokens: 777,
  ...
  assert.equal(
    provider.parseChatText(JSON.stringify({ content: [{ type: "text", text: "answer" }] }), "anthropic"),
    "answer",
  );
});
