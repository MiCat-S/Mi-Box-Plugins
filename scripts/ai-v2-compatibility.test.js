...
test.before(async () => {
  create = require(path.join(buildPlugin({id: 'ai', packageRoot: path.resolve(__dirname, '../ai'), entry: 'v2.ts', rootDir: core}).artifactDir, 'index.cjs')).default;
  const root = await fs.realpath(await fs.mkdtemp(path.join(core, 'temp/ai-answer-build-')));
  buildSync({entryPoints: [path.resolve(__dirname, '../ai/v2/answer.ts')], outfile: path.join(root, 'answer.cjs'), bundle: true, platform: 'node', packages: 'external'});
  deliverAnswer = require(path.join(root, 'answer.cjs')).deliverAnswer;
});

const CONFIG = {
  configs: {main: {tag: 'main', url: 'https://api.example.com/v1', key: 'secret-key', stream: false, responses: false,
    models: {chat: 'gpt-4o', search: 'gpt-4o', image: 'dall-e-3', video: 'sora'}}},
  currentChatTag: 'main', currentChatModel: 'gpt-4o', currentChatReasoningEffort: 'high', currentChatServiceTier: 'priority',
  currentSearchTag: 'main', currentSearchModel: 'gpt-4o', currentSearchReasoningEffort: 'low', currentSearchServiceTier: 'flex',
  currentImageTag: 'main', currentImageModel: 'dall-e-3', currentVideoTag: 'main', currentVideoModel: 'sora',
  imagePreview: true, videoPreview: false, videoAudio: true, videoDuration: 12, prompt: 'be nice', collapse: false, timeout: 45,
  telegraphToken: 'token-value',
  telegraph: {enabled: true, limit: 3, list: [
...
async function fixture(initial = CONFIG) {
  ...
  const edits = [], httpCalls = [];
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async (...args) => { httpCalls.push(args); throw new Error('unexpected external request'); }},
    telegram: {
      edit: async (message, text, _opts, signal) => { signal.throwIfAborted(); edits.push({id: message.id, text}); },
      reply: async (message, text, _opts, signal) => { signal.throwIfAborted(); edits.push({id: message.id, text}); },
      invoke: async () => ({}),
  ...
  const cleanup = async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); };
...
// ---------------------------------------------------------------------------
const READ_ONLY = [
  ['!p model', [/chat 模型: <code>gpt-4o<\/code>/, /video 模型: <code>sora<\/code>/, /chat 思考强度: <code>high<\/code>/, /search 服务等级: <code>flex<\/code>/]],
  ...
  ['!p telegraph', [/Telegraph 状态/, /当前状态: 开启/, /限制数量: <code>3<\/code>/, /记录数量: <code>2\/3<\/code>/, /https:\/\/telegra\.ph\/a/, /https:\/\/telegra\.ph\/b/]],
];
...
test('AI01 chat request carries reply context and the reply image part', async t => {
  ...
  await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
    configs: {main: {tag: 'main', url: 'https://api.example.com/v1', key: 'k', stream: false, responses: false, models: {chat: 'gpt-4o'}}},
    currentChatTag: 'main', currentChatModel: 'gpt-4o',
  }));
  const edits = [], bodies = [];
  const reply = {id: 2, chatId: 'chat', text: 'CONTEXT-TEXT', raw: {media: {className: 'MessageMediaDocument', document: {className: 'Document', mimeType: 'image/png', size: 3}}}};
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    ...
    telegram: {
      ...
      reply: async (_message, text) => { edits.push(text); },
      invoke: async () => ({}),
...
test('AI04 telegraph del removes one index atomically and rejects out-of-range', async t => {
  ...
  assert.deepEqual(data.telegraph.list.map(item => item.title), ['B']);
...
test('AI02 renders safe markdown, sends a signed new reply and deletes the command', async t => {
  ...
  await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
    configs: {main: {tag: 'main', url: 'https://api.example.com/v1', key: 'k', stream: false, responses: false, models: {chat: 'gpt-4o'}}},
    currentChatTag: 'main', currentChatModel: 'gpt-4o', collapse: false, telegraph: {enabled: false, limit: 5, list: []},
  }));
  const deleted = [], sent = [];
  const client = {
    sendMessage: async (_peer, payload) => { sent.push(payload); return {id: 100 + sent.length}; },
    deleteMessages: async (peer, ids, opts) => { deleted.push({peer, ids, opts}); },
  };
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async () => new Response(JSON.stringify({choices: [{message: {content: '**bold**\n\n```js\ncode();\n```'}}]}), {status: 200, headers: {'content-type': 'application/json'}})},
    telegram: {
      edit: async () => {},
      reply: async () => {},
      invoke: async () => ({}),
  ...
  assert.deepEqual(deleted.map(entry => entry.ids), [[1]], 'the command message is deleted after send');
...
test('AI02 a failed command deletion does not fail the answer', async t => {
  ...
  const logs = [], sent = [];
  const client = {
    sendMessage: async (_peer, payload) => { sent.push(payload); return {id: 7}; },
    deleteMessages: async () => { throw new Error('delete failed'); },
  };
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error: (event) => logs.push(event)},
    http: {fetch: async () => new Response(JSON.stringify({choices: [{message: {content: 'ok'}}]}), {status: 200, headers: {'content-type': 'application/json'}})},
    telegram: {
...
function answerContext(sent, controller = new AbortController()) {
  return {
    ...
    log: {info() {}, error() {}},
    telegram: {
      withClient: async (operation) => operation({
        sendMessage: async (_peer, payload) => { sent.push(payload); return {id: 100 + sent.length}; },
...
test('AI02 long single-line answer paginates, anchors continuations to the first answer and keeps all text', async () => {
  ...
  await deliverAnswer(answerContext(sent), {id: 90, chatId: 'chat', raw: {peerId: 'chat'}},
    {question: 'q', answer, tag: 'main', collapse: false, replyToId: 12}, new AbortController().signal);
...
test('AI02 long question and cross-page code block paginate without splitting or overflowing', async () => {
  ...
  await deliverAnswer(answerContext(sent), {id: 90, chatId: 'chat', raw: {peerId: 'chat'}},
    {question, answer, tag: 'main', collapse: true, replyToId: 12}, new AbortController().signal);
...
test('AI02 escapes HTML in the question and answer across pages', async () => {
  const sent = [];
  await deliverAnswer(answerContext(sent), {id: 90, chatId: 'chat', raw: {peerId: 'chat'}},
    {question: '<script>alert(1)</script>', answer: '<img src=x onerror=alert(1)> '.repeat(400), tag: 'main', collapse: false},
...
test('AI01 album reply collects every grouped image in order', async t => {
  ...
  const reply = {id: 2, chatId: 'chat', text: 'CONTEXT', raw: {groupedId: 99n, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'}};
  const client = {
    async *iterMessages(_peer, _params) { for (const item of album) yield item; },
  ...
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    ...
    telegram: {
      edit: async () => {}, reply: async () => {}, invoke: async () => ({}),
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation(client, signal),
    }});
  await host.load(create());
...
test('AI01 streaming progress over budget aborts using real big-integer sizes', async t => {
  ...
  let providerCalls = 0;
  const edits = [];
  const reply = {id: 2, chatId: 'chat', text: '', raw: {media: {className: 'MessageMediaDocument', document: {className: 'Document', mimeType: 'image/png'}}}};
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async () => { providerCalls += 1; throw new Error('must not call provider'); }},
    telegram: {
      edit: async (_m, text) => { edits.push(text); }, reply: async () => {}, invoke: async () => ({}),
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation({
        downloadMedia: async (_raw, options) => {
          downloads += 1;
          // Teleproto passes bigInt(downloaded), bigInt(total).
          options.progressCallback?.({toString: () => String(25 * 1024 * 1024)}, {toString: () => String(25 * 1024 * 1024)});
...
test('AI01 GIF without thumb yields a PNG first frame while webm/TGS are not faked', async t => {
  ...
  const gif = await sharp({create: {width: 2, height: 2, channels: 4, background: {r: 10, g: 20, b: 30, alpha: 1}}}).gif().toBuffer();
  const runCase = async (mime, bytes) => {
    ...
    const bodies = [];
    const reply = {id: 2, chatId: 'chat', text: '', raw: {media: {className: 'MessageMediaDocument', document: {className: 'Document', mimeType: mime}}}};
    const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
      logger: {info() {}, error() {}},
      http: {fetch: async (_url, init) => {
        bodies.push(String(init.body));
      ...
      telegram: {
        edit: async () => {}, reply: async () => {}, invoke: async () => ({}),
        getReply: async (message) => message.id === 1 ? reply : undefined,
        withClient: async (operation, signal) => operation({downloadMedia: async () => bytes, sendMessage: async () => ({id: 1}), deleteMessages: async () => {}}, signal),
      }});
    await host.load(create());
    t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
    await host.dispatchPrimary({id: 1, chatId: 'chat', senderId: '1', outgoing: true, text: '!p describe'});
...
test('AI01 a 5-image album downloads four and discloses the drop', async t => {
  ...
  let downloads = 0;
  const replies = [];
  const album = Array.from({length: 5}, (_, index) => ({id: 10 + index, groupedId: 5n, media: {className: 'MessageMediaPhoto'}}));
  const reply = {id: 10, chatId: 'chat', text: '', raw: {groupedId: 5n, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'}};
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async () => new Response(JSON.stringify({choices: [{message: {content: 'ok'}}]}), {status: 200, headers: {'content-type': 'application/json'}})},
    telegram: {
      edit: async () => {}, reply: async (_m, text) => { replies.push(text); }, invoke: async () => ({}),
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation({
        async *iterMessages() { for (const item of album) yield item; },
        downloadMedia: async () => { downloads += 1; return Buffer.from([1, 2, 3]); },
        sendMessage: async () => ({id: 1}), deleteMessages: async () => {},
      }, signal),
    }});
  await host.load(create());
  t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  ...
  assert.ok(replies.some(text => text.includes('部分图片')), 'the album drop is disclosed in Chinese');
});
...
test('AI01 a large video thumb is fetched by thumb size, not the whole video', async t => {
  ...
  const reply = {id: 2, chatId: 'chat', text: '', raw: {media: {className: 'MessageMediaDocument',
  ...
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    ...
    telegram: {
      ...
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation({
        downloadMedia: async (_raw, options) => { fetched.push(options?.thumb ? 'thumb' : 'full'); return options?.thumb ? png : Buffer.from('big'); },
        sendMessage: async () => ({id: 1}), deleteMessages: async () => {},
      }, signal),
    }});
  await host.load(create());
...
test('AI01 oversized document is rejected before download', async t => {
  ...
  let downloads = 0;
  const reply = {id: 2, chatId: 'chat', text: 'CONTEXT', raw: {media: {className: 'MessageMediaDocument', document: {className: 'Document', mimeType: 'image/png', size: 30 * 1024 * 1024}}}};
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async () => new Response(JSON.stringify({choices: [{message: {content: 'ok'}}]}), {status: 200, headers: {'content-type': 'application/json'}})},
    telegram: {
      edit: async () => {}, reply: async () => {}, invoke: async () => ({}),
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation({downloadMedia: async () => { downloads += 1; return Buffer.from([1]); }}, signal),
    }});
  await host.load(create());
  t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  await host.dispatchPrimary({id: 1, chatId: 'chat', senderId: '1', outgoing: true, text: '!p describe'});
...
test('AI05 Gemini video posts the original endpoint/body and polls to completion', async t => {
  ...
  await fs.mkdir(path.join(root, 'ai'));
  await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
    configs: {v: {tag: 'v', url: 'https://generativelanguage.googleapis.com', key: 'veo-key', type: 'gemini', stream: false, responses: false, models: {video: 'veo-2.0-generate-001'}}},
    currentVideoTag: 'v', currentVideoModel: 'veo-2.0-generate-001', videoAudio: true, videoDuration: 12, videoPreview: true,
  }));
  const calls = [];
  ...
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    ...
    http: {fetch: async (url, init) => {
      ...
      if (String(url).includes(':generateVideos')) return new Response(JSON.stringify({name: 'operations/abc'}), {status: 200, headers: {'content-type': 'application/json'}});
      if (String(url).includes('operations/abc')) return new Response(JSON.stringify({done: true, response: {generatedVideos: [{video: {videoBytes: bytes}}]}}), {status: 200, headers: {'content-type': 'application/json'}});
      return new Response('{}', {status: 200, headers: {'content-type': 'application/json'}});
    }},
    telegram: {
      edit: async () => {}, reply: async () => {}, invoke: async () => ({}), getReply: async () => undefined,
      withClient: async (operation, signal) => operation({sendFile: async () => ({id: 1}), deleteMessages: async () => {}}, signal),
    }});
  await host.load(create());
  t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  ...
  assert.ok(calls.some(call => call.url.includes('operations/abc')), 'operation poll runs');
});
...
test('AI05 Doubao video posts roles/audio/duration and polls the task', async t => {
  ...
  await fs.mkdir(path.join(root, 'ai'));
  await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
    configs: {d: {tag: 'd', url: 'https://ark.example.com', key: 'ark-key', type: 'doubao', stream: false, responses: false, models: {video: 'doubao-seedance-1-0'}}},
    currentVideoTag: 'd', currentVideoModel: 'doubao-seedance-1-0', videoAudio: false, videoDuration: 8, videoPreview: true,
  }));
  const calls = [];
  const reply = {id: 2, chatId: 'chat', text: '', raw: {groupedId: 7n, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'}};
  const album = [{id: 2, groupedId: 7n, media: {className: 'MessageMediaPhoto'}}, {id: 3, groupedId: 7n, media: {className: 'MessageMediaPhoto'}}];
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}},
    http: {fetch: async (url, init) => {
      calls.push({url: String(url), method: init?.method ?? 'GET', body: init?.body ? String(init.body) : undefined});
      if (String(url).endsWith('/tasks')) return new Response(JSON.stringify({task_id: 'task-1'}), {status: 200, headers: {'content-type': 'application/json'}});
      if (String(url).endsWith('/tasks/task-1')) return new Response(JSON.stringify({status: 'succeeded', content: {video_url: 'https://cdn.example.com/v.mp4'}}), {status: 200, headers: {'content-type': 'application/json'}});
      if (String(url) === 'https://cdn.example.com/v.mp4') return new Response(Buffer.from('video-bytes'), {status: 200, headers: {'content-type': 'video/mp4'}});
      return new Response('{}', {status: 200, headers: {'content-type': 'application/json'}});
    }},
    telegram: {
      edit: async () => {}, reply: async () => {}, invoke: async () => ({}),
      getReply: async (message) => message.id === 1 ? reply : undefined,
      withClient: async (operation, signal) => operation({sendFile: async () => ({id: 1}), deleteMessages: async () => {},
        async *iterMessages() { for (const item of album) yield item; }, downloadMedia: async () => Buffer.from([1, 2, 3])}, signal),
    }});
  await host.load(create());
  t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  await host.dispatchPrimary({id: 1, chatId: 'chat', senderId: '1', outgoing: true, text: '!p video firstlast two frames'});
  ...
  assert.ok(calls.some(call => call.url.endsWith('/tasks/task-1')), 'task poll runs');
});
...
test('AI05 Gemini operation failure surfaces as an error without sending media', async t => {
  ...
  await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
    configs: {v: {tag: 'v', url: 'https://generativelanguage.googleapis.com', key: 'veo-key', type: 'gemini', stream: false, responses: false, models: {video: 'veo-2.0-generate-001'}}},
    currentVideoTag: 'v', currentVideoModel: 'veo-2.0-generate-001', videoAudio: false, videoDuration: 5, videoPreview: true,
  }));
  let sentFiles = 0;
  const edits = [];
  const host = new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    ...
    http: {fetch: async (url) => {
      if (String(url).includes(':generateVideos')) return new Response(JSON.stringify({name: 'operations/bad'}), {status: 200, headers: {'content-type': 'application/json'}});
      return new Response(JSON.stringify({done: true, error: {message: 'boom'}}), {status: 200, headers: {'content-type': 'application/json'}});
    }},
    telegram: {
      edit: async (_m, text) => { edits.push(text); }, reply: async () => {}, invoke: async () => ({}), getReply: async () => undefined,
      withClient: async (operation, signal) => operation({sendFile: async () => { sentFiles += 1; return {id: 1}; }, deleteMessages: async () => {}}, signal),
    }});
  await host.load(create());
  t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  await host.dispatchPrimary({id: 1, chatId: 'chat', senderId: '1', outgoing: true, text: '!p video a cat'});
...
function mediaHost(root, client, fetchImpl, replies) {
  return new PluginHost({storageRoot: root, selfId: '1', prefixes: ['!'], aliases: {p: 'ai'},
    logger: {info() {}, error() {}}, http: {fetch: fetchImpl},
    telegram: {
      edit: async () => {}, reply: async (_m, text) => { replies.push(text); },
      invoke: async () => ({}),
...
async function chatCase({ replyGroup, ownGroup, downloads }) {
  ...
  const client = {
    ...
    sendMessage: async () => ({id: 1}), deleteMessages: async () => {},
    reply: {id: 5, chatId: 'chat', text: '', raw: {groupedId: 1n, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'}},
  };
  const host = mediaHost(root, client, async (_url, init) => {
    bodies.push(String(init.body));
    return new Response(JSON.stringify({choices: [{message: {content: 'ok'}}]}), {status: 200, headers: {'content-type': 'application/json'}});
  }, replies);
  await host.load(create());
  const envelope = {id: 1, chatId: 'chat', senderId: '1', outgoing: true, text: '!p describe',
    raw: ownGroup.length ? {groupedId: ownGroup[0].groupedId, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'} : undefined};
  await host.dispatchPrimary(envelope);
  return {bodies, replies, cleanup: async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); }};
}

test('AI01 reply 3 + own 2 merges to four images with a Chinese disclosure', async t => {
  const replyGroup = [0, 1, 2].map(index => ({id: 10 + index, groupedId: 1n, media: {className: 'MessageMediaPhoto'}}));
  ...
  assert.ok(f.replies.some(text => text.includes('部分图片')), 'the merge drop is disclosed in Chinese');
});
...
test('AI01 merged byte budget over 20MiB is disclosed even when each side is under', async t => {
  ...
  const replyGroup = [0, 1].map(index => ({id: 10 + index, groupedId: 1n, media: {className: 'MessageMediaPhoto'}}));
  const ownGroup = [0, 1].map(index => ({id: 20 + index, groupedId: 2n, media: {className: 'MessageMediaPhoto'}}));
  ...
  assert.ok(f.replies.some(text => text.includes('部分图片')), 'the byte-budget drop is disclosed');
});
...
test('AI05 default video with images uses reference roles; explicit firstlast falls back like the original', async t => {
  ...
  const runVideo = async (text, album) => {
    ...
    await fs.mkdir(path.join(root, 'ai'));
    await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify({
      configs: {d: {tag: 'd', url: 'https://ark.example.com', key: 'ark', type: 'doubao', stream: false, responses: false, models: {video: 'doubao-seedance'}}},
      currentVideoTag: 'd', currentVideoModel: 'doubao-seedance', videoAudio: false, videoDuration: 5, videoPreview: true,
    }));
    const calls = [], replies = [];
    const client = {
      async *iterMessages() { for (const item of album) yield item; },
      ...
      sendMessage: async () => ({id: 1}), deleteMessages: async () => {},
      sendFile: async () => ({}),
      reply: album.length ? {id: 5, chatId: 'chat', text: '', raw: {groupedId: album[0].groupedId, media: {className: 'MessageMediaPhoto'}, chatId: 'chat', peerId: 'chat'}} : undefined,
    };
    const host = mediaHost(root, client, async (url, init) => {
      calls.push({url: String(url), body: init?.body ? String(init.body) : undefined});
      if (String(url).endsWith('/tasks')) return new Response(JSON.stringify({task_id: 't'}), {status: 200, headers: {'content-type': 'application/json'}});
      if (String(url).endsWith('/tasks/t')) return new Response(JSON.stringify({status: 'succeeded', content: {video_url: 'https://cdn/v.mp4'}}), {status: 200, headers: {'content-type': 'application/json'}});
      if (String(url) === 'https://cdn/v.mp4') return new Response(Buffer.from('v'), {status: 200, headers: {'content-type': 'video/mp4'}});
      return new Response('{}', {status: 200, headers: {'content-type': 'application/json'}});
    }, replies);
    await host.load(create());
    t.after(async () => { await host.shutdown(1000); await fs.rm(root, {recursive: true, force: true}); });
  ...
  assert.deepEqual(rolesOf(await runVideo('!p video a clip', two)), ['reference_image', 'reference_image'], 'default auto+2 images => reference');
  assert.deepEqual(rolesOf(await runVideo('!p video firstlast a clip', [photo(10, 1n)])), ['first_frame'], 'firstlast with one image falls back to first');
  assert.equal(rolesOf(await runVideo('!p video firstlast a clip', [])).length, 0, 'firstlast with no image falls back to text-only');
  assert.deepEqual(rolesOf(await runVideo('!p video firstlast a clip', two)), ['first_frame', 'last_frame'], 'explicit firstlast with two images keeps both roles');
});
...
test('AI04 telegraph status paginates every record without dropping history', async t => {
  const list = Array.from({length: 40}, (_, index) => ({
    url: `https://telegra.ph/post-${index + 1}-with-a-long-slug`, title: `R${index + 1}# `.repeat(12).trim(), createdAt: '2026-01-01',
  }));
  ...
  for (const edit of f.edits) assert.ok(edit.text.length > 0 && edit.text.length <= 3500, `bounded page (${edit.text.length})`);
