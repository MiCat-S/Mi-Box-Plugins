...
const { artifactDir } = buildPlugin({
  id: "oxost",
  packageRoot: process.env.OXOST_TEST_SOURCE || path.resolve(__dirname, "../oxost"),
  entry: "v2.ts",
});
...
async function fixture(t, options = {}) {
  ...
  const raw = {
    media: {},
    document: { size: 8, attributes: [{ fileName: "report.pdf" }], ...options.document },
    ...(options.raw ?? {}),
    async downloadMedia(params) {
      downloads.push(params);
      if (options.download) return options.download(params);
      if (params?.outputFile) {
        await fs.writeFile(params.outputFile, "document");
  ...
  return {
    ...
    replies,
    logs,
    run: (text = ".0x0 expires=72 secret", patch = {}) =>
      host.dispatchPrimary({ id: 1, chatId: "1", senderId: "1", outgoing: true, replyToId: 2, text, ...patch }),
  };
}
...
test("oxost uploads a file-backed blob and retains the temporary file through response consumption", async t => {
  ...
  const f = await fixture(t, {
    fetch: async (_url, init, download) => {
...
test("oxost file transfer integrates with the actual Teleproto document downloader", async t => {
  ...
  const remote = {
    ...
    _media: {
      ...
      async getFile(_dc, _location, _offset, _size, signal) {
  ...
  const f = await fixture(t, {
    download: params =>
      downloadMedia(remote, document, params.outputFile, undefined, params.progressCallback, { signal: params.signal }),
...
test("oxost failed HTTP upload closes the file lifetime and keeps diagnostics private", async t => {
  const f = await fixture(t, {
    fetch: async () => {
      throw new Error("private-upload-diagnostic");
    },
  });
  await f.run();
  assert.match(f.edits.at(-1), /上传失败/);
...
test("oxost derives photo file names from a 12-byte header", async t => {
  ...
  for (const [magic, name] of cases) {
    ...
    const f = await fixture(t, {
      ...
      raw: { photo: true },
      download: async params => {
        await fs.writeFile(params.outputFile, header);
        return params.outputFile;
      },
      fetch: async (_url, init) => {
        assert.equal(init.body.get("file").name, name);
        return new Response("https://0x0.st/photo");
      },
    });
    await f.run();
...
test("oxost preserves the original sanitized caption filename fallback", async t => {
  const f = await fixture(t, {
    raw: { document: undefined, message: "caption name?.txt", video: true },
    fetch: async (_url, init) => {
      assert.equal(init.body.get("file").name, "caption_name_.txt");
      return new Response("https://0x0.st/caption");
    },
  });
  await f.run();
  assert.match(f.edits.at(-1), /0x0\.st\/caption/);
});
...
test("oxost unload during upload response consumption cleans the temporary file", async t => {
  ...
  const f = await fixture(t, {
    fetch: async (_url, _init, download) => {
      file = download.outputFile;
      return new Response(
        new ReadableStream({
          start(controller) {
...
test("oxost reports a completed upload when temporary cleanup later fails", async t => {
  const directory = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "oxost-cleanup-")));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const edits = [],
    logs = [];
  const signal = new AbortController().signal;
  await create().commands["0x0"].handle(
    {
      command: "0x0",
      prefix: ".",
      args: [],
      message: { id: 1, chatId: "1", outgoing: true, text: ".0x0", replyToId: 2 },
    },
    {
      signal,
      ...
      telegram: {
        ...
        getReply: async () => ({
          raw: {
            media: {},
            document: { size: 8, attributes: [{ fileName: "a.txt" }] },
            async downloadMedia({ outputFile }) {
              await fs.writeFile(outputFile, "document");
            },
          },
        }),
        withClient: async use => use({}, signal),
      },
      files: {
        withTemp: async use => {
