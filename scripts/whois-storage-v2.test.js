...
const { artifactDir } = buildPlugin({
  id: "whois-records",
  packageRoot: path.resolve(__dirname, "../whois"),
  entry: "v2/records.ts",
});
...
async function fixture(t, initial) {
  ...
  const api = records({
    storage: {
      ...
      sqlite: () => ({
        read: fn => sql.read(fn, controller.signal),
        transaction: fn => sql.transaction(fn, controller.signal),
      }),
    },
  });
  t.after(async () => {
    await Promise.all([json.close(), sql.close()]);
...
test("whois interrupted record import rolls back its marker and retries from intact JSON", async t => {
  ...
  const initial = {
    ...
    legacyImported: true,
    extension: 9007199254740993123n,
  };
  const f = await fixture(t, initial);
  await f.sql.transaction(db =>
    db.exec(`CREATE TABLE cache (domain TEXT PRIMARY KEY, value TEXT NOT NULL);
...
test("whois failed clear leaves both tables intact and cancellation rejects queued writes", async t => {
  ...
  await f.sql.transaction(db =>
    db.exec("CREATE TRIGGER reject_clear BEFORE DELETE ON cache BEGIN SELECT RAISE(ABORT, 'clear failure'); END"),
  );
...
test("whois concurrent writes retain completion order, configured history size and all cache entries", async t => {
  ...
  assert.deepEqual(
    history.rows.map(row => row.domain),
    items
      .slice(-20)
      .reverse()
