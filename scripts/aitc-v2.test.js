...
async function fixture(
  t,
  { ai = aiConfig(), legacy, response = { choices: [{ message: { content: "translated" } }] } } = {},
) {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "mibot-aitc-v2-")));
  if (ai) {
    await fs.mkdir(path.join(root, "ai"));
    await fs.writeFile(path.join(root, "ai", "config.json"), JSON.stringify(ai));
  }
  if (legacy) {
    await fs.mkdir(path.join(root, "aitc"));
    await fs.writeFile(path.join(root, "aitc", "config.json"), JSON.stringify(legacy));
  }
  const edits = [],
    requests = [];
  let reply;
