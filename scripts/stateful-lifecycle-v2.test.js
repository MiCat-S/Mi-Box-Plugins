...
const factories = Object.fromEntries(
  ["autodel", "autodelcmd", "autorepeat"].map(id => {
...
function fixture(t, id, initial, client = {}) {
  ...
  const native = {
    ...
    async deleteMessages(chat, ids, options) {
      deleted.push({ chat: String(chat), ids, options });
    },
    async getMe() {
      return { id: 1n };
    },
  ...
  const context = {
    ...
    commands: {
      parse(text) {
        return { command: text.slice(1), args: [] };
      },
    },
    telegram: {
      ...
      async withClient(operation) {
        return operation(native, scope.signal);
      },
    },
    log: { info() {}, error() {} },
  };
...
const deletionState = () => ({
  schemaVersion: 2,
  enabled: true,
  rules: [{ id: "1", command: "ping", delay: 60 }],
  pending: {},
});
const repeatState = () => ({
  schemaVersion: 1,
