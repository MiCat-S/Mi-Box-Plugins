...
test('parsehub ignores same-second old IDs, accepts a progress edit as final media and preserves topic reply',async t=>{
 ...
 await submitted.promise;await nextTurn();for(let i=0;i<12;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await running;
...
test('parsehub welcome timeout sends no business link and never marks initialization successful',async t=>{
 ...
 const running=f.run('.parsehub https://example.com/business');for(let i=0;i<30;i++){await nextTurn();t.mock.timers.tick(500);await nextTurn();}t.mock.timers.reset();await running;
...
test('parsehub paginates text fallback completely and cleanup failure cannot reverse success',async t=>{
 ...
 const running=f.run('.parsehub https://example.com/post');await submitted.promise;await nextTurn();for(let i=0;i<12;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await running;
...
test('parsehub resolves a rawless large chat ID without numeric precision loss',async t=>{
 ...
 const chatId='-1009007199254740993',running=f.run('.parsehub https://example.com/rawless',{chatId,omitRaw:true});await submitted.promise;await nextTurn();for(let i=0;i<10;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await running;assert.equal(inputs.length,1);assert.equal(inputs[0].toString(),chatId);
});
...
test('parsehub reports incomplete when a failed mixed batch falls back text but loses captioned media',async t=>{
 ...
 const running=f.run('.parsehub https://example.com/many');await submitted.promise;await nextTurn();for(let i=0;i<10;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await running;assert.equal(calls,2);const fallback=f.sent.filter(item=>item.peer!=='@ParseHubot').map(item=>item.value.message).join('\n');assert.match(fallback,/text-100/);assert.match(fallback,/video caption/);assert.match(f.replies.at(-1).text,/解析未完成/);assert.match(f.edits.at(-1).text,/0\/1/);
});
...
test('parsehub final receipt failure cannot turn successful forwarding into a business failure',async t=>{
 ...
 const running=f.run('.parsehub https://example.com/receipt');await submitted.promise;await nextTurn();for(let i=0;i<10;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await running;assert.equal(f.forwarded.length,1);assert.equal(f.edits.some(x=>/解析失败/.test(x.text)),false);assert.equal(deletes,1);assert.deepEqual(f.logs.at(-1),{event:'parsehub_receipt_failed',fields:{kind:'internal'}});assert.doesNotMatch(JSON.stringify(f.logs),/SECRET_RECEIPT/);
});

test('parsehub cancellation while the completion receipt is in flight starts no late cleanup',async t=>{
 t.mock.timers.enable({apis:['Date','setTimeout']});const submitted=deferred(),receipt=deferred(),gate=deferred();let reads=0,deletes=0;
 const f=await fixture(t,{onDelete:async()=>{deletes++;},onEdit:async(_message,text)=>{if(/处理完成/.test(text)){receipt.resolve();await gate.promise;}},client:{async getMessages(){return reads++<2?[{id:10,out:false,className:'Message',message:'welcome'}]:[{id:11,out:false,className:'Message',message:'done'}];},async sendMessage(){submitted.resolve();return{id:10,out:true};}}});
 const running=f.run('.parsehub https://example.com/cancel-cleanup');await submitted.promise;await nextTurn();for(let i=0;i<10;i++){t.mock.timers.tick(2000);await nextTurn();}t.mock.timers.reset();await receipt.promise;const unloading=f.host.unload('parsehub',1000);gate.resolve();assert.equal((await unloading).completed,true);await assert.rejects(running,e=>e?.name==='AbortError'||e?.name==='TelegramAbortError');assert.equal(deletes,0);
});
