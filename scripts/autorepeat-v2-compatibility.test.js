...
const live = () => ({raw: {className: 'Message', sender: {className: 'User', bot: false}, date: Math.floor(Date.now() / 1000)}});
const enabledState = (patch = {}) => ({schemaVersion: 1, enabledGroups: [CHAT], dailyHistory: {}, lastDay: shanghaiDay(),
  trigger: {timeWindow: 300, minUsers: 2}, ...patch});
...
async function hostFixture(t, {state, legacy, sendError, getEntity} = {}) {
  ...
  const client = {
    async sendMessage(chat, options) {if (sendError !== undefined) throw new Error(sendError); sent.push({chat: String(chat), text: options.message});},
    async getEntity(target) {return getEntity ? getEntity(target) : {className: 'Channel', megagroup: true, title: `<G ${target}>`};},
    async deleteMessages() {}, async getMe() {return {id: 1n};},
  };
  const host = new PluginHost({storageRoot: root,
    logger: {info() {}, error(event, fields) {logs.push({event, fields});}},
    telegram: {async edit(_m, text) {edits.push(text);}, async reply() {}, async invoke() {}, async getReply() {},
      async withClient(operation, signal) {return operation(client, signal);}}});
  await host.load(create());
  let sequence = 0;
  return {root, host, sent, edits, logs,
    config: async () => JSON.parse(await fs.readFile(path.join(root, 'autorepeat/autorepeat.json'), 'utf8')),
    say: (senderId, text, patch = {}, id = ++sequence) => host.dispatchListeners(
      {id, chatId: CHAT, senderId: String(senderId), outgoing: false, text, ...live(), ...patch}),
    run: (chatId, text) => host.dispatchPrimary({id: ++sequence, chatId, senderId: '1', outgoing: true, text: `.autorepeat ${text}`}),
  };
}

test('AUTOREPEAT-01 migrated legacy daily_history keys still suppress the same content', async t => {
  const f = await hostFixture(t, {legacy: {cache: {autorepeat_settings: [CHAT]},
    daily_history: {[CHAT]: ['old', 'x'.repeat(50) + '60']}, last_day_check: shanghaiDay(),
...
test('AUTOREPEAT-04 the unresolvable-group fallback shows the original full help', async t => {
  const f = await hostFixture(t, {state: enabledState({enabledGroups: []}), getEntity: () => ({className: 'User'})});
...
test('AUTOREPEAT-05 list keeps the original colon and next-page hint', async t => {
  ...
  const f = await hostFixture(t, {state: enabledState({enabledGroups: groups}),
...
test('AUTOREPEAT-07 counts distinct users once per day and ignores self/bots/media/old messages', async t => {
  ...
  await f.host.dispatchListeners({id: 202, chatId: CHAT, senderId: '4', outgoing: false, text: '', ...live(), raw: {sender: {className: 'User', bot: false}, date: Math.floor(Date.now() / 1000)}});
  await f.host.dispatchListeners({id: 203, chatId: CHAT, senderId: '4', outgoing: false, text: 'bot', ...live(), raw: {sender: {className: 'User', bot: true}, date: Math.floor(Date.now() / 1000)}});
  await f.host.dispatchListeners({id: 204, chatId: CHAT, senderId: '4', outgoing: false, text: 'old', ...live(), raw: {sender: {className: 'User', bot: false}, date: Math.floor(Date.now() / 1000) - 3600}});
...
test('AUTOREPEAT-10 command failures never echo unknown error details to chat', async t => {
  ...
  const f = await hostFixture(t, {state: enabledState(), getEntity: () => {throw new Error(secret);}});
  await f.run(CHAT, 'on @private_group');
  assert.equal(f.edits.at(-1), '❌ 操作失败，请稍后重试');
  assert.equal(f.edits.some(text => text.includes('tg-secret') || text.includes('/etc/telebox')), false);
});
