...
async function fixture(t, options = {}) {
  ...
  const host = new PluginHost({
    ...
    http: { fetch },
    telegram: {
      async edit(message, text, settings, signal) {
      ...
      async withClient(operation, signal) {
        return transportScope.run("telegram-boundary", async transportSignal => {
          ...
          clientActive = true;
          try {
            return await operation(client, scopedSignal);
          } finally {
            clientActive = false;
          }
        });
  ...
  return {
    ...
    run: (text, extra = {}) =>
      host.dispatchPrimary({ id: 1, chatId: "7", senderId: "1", outgoing: true, text, raw, ...extra }),
  };
}
...
test("javdb normalizes codes, escapes details, preserves aliases and sends a short report as the cover caption", async t => {
  ...
  assert.deepEqual(
    f.host
      .listCommands()
      .filter(value => value.pluginId === "javdb")
...
test("javdb keeps an expiring deletion inside both resource scopes during unload", async t => {
  ...
  await started;
  let unloaded = false;
  const unloading = f.host.unload("javdb", 1000).then(report => {
    unloaded = true;
    return report;
  });
  await flush();
...
test("javdb actively cancels and releases a hanging image reader on unload", async t => {
  ...
  const f = await fixture(t, {
    imageResponse: () => new Response(stream, { headers: { "content-type": "image/jpeg" } }),
  });
  ...
  const client = {
    ...
    _media: {
      ...
      async savePart(dcId, request, signal) {
        signal.throwIfAborted();
        parts.push({ dcId, request });
      },
    },
  };
  const result = await _fileToMedia(client, {
    file: new CustomFile("cover.jpg", JPEG.length, "", JPEG),
    spoiler: true,
  });
...
test("javdb preserves the first page and stops before cover work when the second page fails", async t => {
  ...
  assert.deepEqual(
    f.logs.map(entry => entry.event),
    ["javdb_report_delivery_failed"],
  );
  assert.deepEqual(
    f.requests.map(entry => entry.url.pathname),
    ["/search", "/v/abc"],
  );
  assert.equal(f.sends.length, 0);
  assert.equal(f.commandDeletes(), 0);
