...
test.before(async () => {
  ...
  buildSync({entryPoints: [path.resolve(__dirname, '../aban/v2/runtime.ts')],
    outfile: path.join(root, 'runtime.cjs'), bundle: true, platform: 'node', packages: 'external'});
...
const user = (id = 2) => new Api.User({id: integer(id), accessHash: integer(30), firstName: 'Target', lastName: 'User', username: 'target'});
const input = (id = 2) => new Api.InputPeerUser({userId: integer(id), accessHash: integer(30)});
const message = (text = '.ban 2') => ({id: 9, message: text, isGroup: true, isChannel: true,
  peerId: new Api.PeerChannel({channelId: integer(100)}), className: 'Message'});

// Runtime seam context: captures edits, logs, managed tasks and native deletes.
function runtimeEnv(options = {}) {
  const controller = new AbortController();
  const edits = [], logs = [], deletes = [], tasks = [], invokes = [];
  const client = {
    getMe: async () => user(1),
    ...
    invoke: async request => {
      ...
      if (request instanceof Api.messages.GetFullChat) {
        return {fullChat: {participants: {participants: [new Api.ChatParticipantCreator({userId: integer(1)})]}}, users: [user()]};
      }
      return {offset: 0};
    },
    deleteMessages: async (...args) => { deletes.push(args); },
  };
  const ctx = {
...
test('unloading before the managed cleanup elapses does not delete the help message', async t => {
  ...
  const edits = [], deletes = [];
  const client = {deleteMessages: async (...args) => { deletes.push(args); }};
  ...
  assert.equal(await host.dispatchPrimary({id: 9, chatId: '-100100', senderId: '1', text: '.aban', outgoing: true}), true);
...
test('a failed status edit still runs the business action', async () => {
  ...
  assert.ok(env.invokes.some(request => request instanceof Api.channels.EditBanned), 'kick RPC must still run');
...
test('a failed result edit does not change the completed business outcome', async () => {
  const failure = Object.assign(new Error('message is not modified'), {name: 'RpcError'});
  let calls = 0;
  const env = runtimeEnv({onEdit: () => { calls += 1; if (calls === 2) throw failure; }});
...
test('cancellation during the status edit stops before the next RPC', async () => {
  const plugin = factory(), env = runtimeEnv({onEdit: (_text, controller) => controller.abort()});
  const invocation = {message: {id: 9, chatId: '-100100', text: '.ban 2', outgoing: true},
    command: 'ban', prefix: '.', args: ['2']};
