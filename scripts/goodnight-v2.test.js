...
const { artifactDir } = buildPlugin({
  id: "goodnight",
  packageRoot: path.resolve(__dirname, "../goodnight"),
  entry: "v2.ts",
});
...
async function fixture(t, initial, options = {}) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async reply(message, text, replyOptions) {
        replies.push({ message, text, options: replyOptions });
        await options.reply?.();
      },
      async invoke() {},
      async getReply() {},
  ...
  return {
    ...
    host,
    replies,
    read: async () => JSON.parse(await fs.readFile(path.join(root, "goodnight/data.json"), "utf8")),
    command: (text, chatId = "chat") =>
      host.dispatchPrimary({ id: 1, chatId, senderId: "owner", outgoing: true, text }),
    message: (text, senderId, extra = {}) =>
      host.dispatchListeners({ id: Date.now(), chatId: "chat", senderId, outgoing: false, text, ...extra }),
  };
}
...
test("goodnight resets previous days and retains independent concurrent group writes", async t => {
  const f = await fixture(t, {
    groups: { chat: { enabled: true, timezone: 8, date: "2000-01-01", sleepUsers: ["old"], wakeUsers: ["old"] } },
  });
...
test("goodnight keeps ranking data when reply fails and logs only a fixed event", async t => {
  const f = await fixture(t, undefined, {
    reply: async () => {
      throw Object.assign(new Error("private message"), { code: "PRIVATE" });
    },
  });
  await f.command(".goodnight on");
