...
async function fixture(t, fetchImpl, editImpl) {
  ...
  return {
    ...
    requests,
    send: args =>
      host.dispatchPrimary({ id: 1, chatId: "chat", senderId: "owner", outgoing: true, text: `.bin ${args}` }),
  };
}
...
const rates = {
  ...
  rates: { USD: 1, CNY: 7, TWD: 32 },
};
function successFetch(url) {
  const value = String(url);
  if (value.includes("bincheck.io"))
    return new Response(
      '<meta property="og:description" content="valid BIN number VISA issued by Taiwan Company Limited in Taiwan">',
    );
  ...
  return new Response(
    JSON.stringify({
      ...
      country: { name: "Taiwan, Province of China", alpha2: "TW", currency: "TWD" },
    }),
  );
}
...
for (const [name, status, expected] of [
  ["not found", 404, "❌ 未找到: <code>415042</code>"],
  ["rate limited", 429, "⏳ 频率受限，请稍后重试"],
]) {
...
test("BIN-COMPAT-06 releases the Binlist response body after a normal read", async t => {
  ...
  const f = await fixture(t, url => {
    ...
    const response = successFetch(url);
    body = response.body;
    return response;
  });
  await f.send("415042");
