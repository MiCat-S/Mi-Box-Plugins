...
async function fixture(t, chartResponse) {
  ...
  await fs.writeFile(
    path.join(root, "nezha/config-v2.json"),
    JSON.stringify({
      schemaVersion: 1,
  ...
  const host = new PluginHost({
    ...
    http: {
      fetch: async (url, init) => {
        ...
        requests.push({ url: target.href, init });
        if (target.hostname === "panel.invalid")
          return Response.json({
            success: true,
            data: target.pathname.endsWith("/server") ? [{ id: 7, name: "Node A" }] : monitor,
          });
  ...
  return {
    ...
    async cleaned() {
      let jobs;
      try {
        jobs = await fs.readdir(path.join(root, "temp/nezha"));
      } catch (e) {
...
test("nezha writes chart chunks before the response ends and preserves bytes and chart options", async t => {
  ...
  assert.equal(f.sent[0].peer, "123");
  assert.equal(f.sent[0].options.caption, "Node A 服务延迟");
  assert.equal(f.sent[0].options.replyTo, 9);
  assert.deepEqual(
    f.requests.map(x => new URL(x.url).pathname),
    ["/base/api/v1/server", "/base/api/v1/service/7", "/chart"],
  );
  ...
  assert.equal(body.backgroundColor, "black");
  assert.equal(body.format, "png");
  assert.equal(body.chart.type, "line");
  assert.equal(body.chart.data.datasets[0].label, "HTTPS");
  assert.deepEqual(body.chart.data.datasets[0].data, [12, 34]);
  await f.cleaned();
});
