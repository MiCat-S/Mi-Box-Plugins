...
async function fixture(t, options = {}) {
  ...
  const sourceCalls = [];
  let nativeCalls = 0;
  const values = () => typeof options.participants === 'function' ? options.participants() : options.participants ?? [];
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async withClient(operation, signal) { nativeCalls += 1; return operation(client, signal); },
    },
  });
...
test('rejects real private and broadcast messages before opening the native client', async t => {
  ...
  await f.dispatch(envelope('.atall', {
...
test('includes administrators, filters bot/deleted/nameless users and preserves topic replies', async t => {
  ...
  admin.participant = new Api.ChannelParticipantAdmin({userId: admin.id, date: 1, adminRights: new Api.ChatAdminRights({})});
  ...
  assert.equal(f.sent[0].parameters.message,
...
test('distinguishes an empty group from members filtered out of the mention list', async t => {
  ...
  const filtered = await fixture(t, {participants: [
...
test('stops at 250 participants and 10 pages and treats receipt cleanup as non-fatal', async t => {
  ...
  await f.dispatch(envelope('.atall', {
    replyTo: new Api.MessageReplyHeader({forumTopic: true, replyToMsgId: 52, replyToTopId: 40}),
    deleteReceipt: async () => { throw new Error('delete denied'); },
  }));
  assert.equal(f.sent.length, 10);
  assert.equal(f.sent.reduce((total, page) => total + (page.parameters.message.match(/tg:\/\/user\?id=/g) ?? []).length, 0), 250);
  assert.equal(f.sent.every(page => page.parameters.message.length <= 3300), true);
  assert.equal(f.sent.every(page => page.parameters.topMsgId === 40), true);
  assert.deepEqual(f.sent.map(page => page.parameters.replyTo), [7, ...Array(9).fill(undefined)]);
  assert.match(f.sent.at(-1).parameters.message, /已达到单次 250 人 \/ 10 页上限/);
  assert.equal(f.logs.some(entry => entry.event === 'atall_receipt_cleanup_failed' && entry.fields === undefined), true);
  assert.equal(f.edits.some(entry => /提醒失败/.test(entry.text)), false);
});
...
test('rejects a concurrent run without opening a second native operation', async t => {
  ...
  const f = await fixture(t, {
    ...
    async beforeParticipants() { enter(); await gate; },
  });
  const first = f.dispatch(envelope('.atall', {id: 10, deleteReceipt: undefined}));
  await entered;
  const second = f.dispatch(envelope('.atall', {
    id: 11,
...
test('unload cancels inter-page delay before more sends or receipt deletion', async t => {
  let deleted = 0;
  let firstSend;
  const sent = new Promise(resolve => { firstSend = resolve; });
  const f = await fixture(t, {
    participants: Array.from({length: 26}, (_, index) => participant(index + 1)),
    async onSendMessage() { firstSend(); },
  });
  const running = f.dispatch(envelope('.atall', {deleteReceipt: async () => { deleted += 1; }}));
  await sent;
  await new Promise(resolve => setTimeout(resolve, 20));
