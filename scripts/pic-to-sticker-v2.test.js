...
async function fixture(t, source, options = {}) {
  ...
  const context = {signal, log: {error(event) {errors.push(event);}}, storage: {json: () => ({async read() {return structuredClone(config);}, async update(fn) {config = await fn(structuredClone(config)); return config;}})},
    ...
    files: {async withTemp(operation) {const dir = await fs.mkdtemp(path.join(root, 'job-')); let result; try {result=await operation(dir, signal);} finally {await fs.rm(dir, {recursive: true, force: true});} if(options.tempCleanupFails)throw new Error('private cleanup'); return result;}}};
...
test('pic_to_sticker counts sent media when temporary cleanup fails for single and batch conversion',async t=>{const image=await sharp({create:{width:16,height:16,channels:4,background:'#abcdef'}}).png().toBuffer();const single=await fixture(t,{media:{},photo:{},document:{size:image.length},chunks:[image]},{tempCleanupFails:true});await single.run();assert.equal(single.outputs.length,1);assert.ok(single.errors.includes('pic_to_sticker_temp_cleanup_failed'));assert.ok(!single.errors.includes('pic_to_sticker_failed'));assert.match(single.edits.at(-1),/贴纸已发送/);const groupedId=8n,source={id:10,groupedId,media:{},photo:{},document:{size:image.length},chunks:[image]},newer=[{id:11,groupedId,media:{},photo:{},document:{size:image.length},chunks:[image]}],batch=await fixture(t,source,{tempCleanupFails:true,newer});await batch.run(['batch']);assert.equal(batch.outputs.length,2);assert.equal(batch.errors.filter(value=>value==='pic_to_sticker_temp_cleanup_failed').length,2);assert.match(batch.edits.at(-1),/成功: 2 张/);assert.match(batch.edits.at(-1),/失败: 0 张/);});
