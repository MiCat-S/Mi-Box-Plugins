...
async function fixture(t, source, options = {}) {
  ...
  const client = {
    ...
    async sendFile(_peer, sendOptions) {
      const data = await fs.readFile(sendOptions.file);
      outputs.push({ data, options: sendOptions, metadata: await sharp(data).metadata() });
    },
  ...
  const context = {
    ...
    storage: {
      json: () => ({
        ...
        async update(fn) {
          config = await fn(structuredClone(config));
          return config;
        },
      }),
    },
    ...
    files: {
      async withTemp(operation) {
        ...
        let result;
        try {
          result = await operation(dir, signal);
        } finally {
          await fs.rm(dir, { recursive: true, force: true });
        }
        if (options.tempCleanupFails) throw new Error("private cleanup");
        return result;
      },
    },
  };
  ...
  const run = (args = []) =>
    create().commands.pts.handle(
      {
        command: "pts",
        prefix: ".",
        args,
        message: { id: 3, chatId: "1", replyToId: 2, text: `.pts ${args.join(" ")}`, outgoing: true, raw },
      },
      context,
    );
...
test("pic_to_sticker streams a bounded input and emits a static Telegram WebP", async t => {
  const gif = await sharp({ create: { width: 64, height: 64, channels: 4, background: "#ff0000" } })
    .gif()
    .toBuffer();
...
test("pic_to_sticker rejects declared oversized media before downloading", async t => {
  const f = await fixture(t, {
    media: {},
    document: { size: 50 * 1024 * 1024 + 1 },
    chunks: [Buffer.from("must-not-download")],
  });
  await f.run();
...
test("pic_to_sticker rejects a small compressed image above the decode pixel budget", async t => {
  const image = await sharp({ create: { width: 4600, height: 4600, channels: 3, background: "#ffffff" } })
    .png()
    .toBuffer();
...
test("pic_to_sticker scans both sides of an arbitrary album reply and keeps cleanup failure separate", async t => {
  const image = await sharp({ create: { width: 32, height: 32, channels: 4, background: "#0088ff" } })
    .png()
    .toBuffer();
...
test("pic_to_sticker counts sent media when temporary cleanup fails for single and batch conversion", async t => {
  const image = await sharp({ create: { width: 16, height: 16, channels: 4, background: "#abcdef" } })
    .png()
    .toBuffer();
  const single = await fixture(
    t,
    { media: {}, photo: {}, document: { size: image.length }, chunks: [image] },
    { tempCleanupFails: true },
  );
  await single.run();
  assert.equal(single.outputs.length, 1);
  assert.ok(single.errors.includes("pic_to_sticker_temp_cleanup_failed"));
  assert.ok(!single.errors.includes("pic_to_sticker_failed"));
  assert.match(single.edits.at(-1), /贴纸已发送/);
  const groupedId = 8n,
    source = { id: 10, groupedId, media: {}, photo: {}, document: { size: image.length }, chunks: [image] },
    newer = [{ id: 11, groupedId, media: {}, photo: {}, document: { size: image.length }, chunks: [image] }],
    batch = await fixture(t, source, { tempCleanupFails: true, newer });
  await batch.run(["batch"]);
  assert.equal(batch.outputs.length, 2);
  assert.equal(batch.errors.filter(value => value === "pic_to_sticker_temp_cleanup_failed").length, 2);
  assert.match(batch.edits.at(-1), /成功: 2 张/);
  assert.match(batch.edits.at(-1), /失败: 0 张/);
});
