...
async function fixture(t, options = {}) {
  ...
  const input =
    options.input ?? Buffer.from("windowBackgroundWhite=#ffffff\nwindowTextBlack=#000000\nactionBarDefault=#112233\n");
  const edits = [],
    sends = [],
    invokes = [],
    logs = [];
  const client = {
    ...
    async downloadMedia(_source, downloadOptions) {
      await fs.writeFile(downloadOptions.outputFile, input);
      return downloadOptions.outputFile;
    },
    async downloadFile(_source, downloadOptions) {
      const bytes = options.wallpaper ?? input;
      await fs.writeFile(downloadOptions.outputFile, bytes);
      return downloadOptions.outputFile;
    },
    async uploadFile(value) {
      if (options.uploadFile) return options.uploadFile(value, controller);
      return new Api.InputFile({ id: 1n, parts: 1, name: "theme.attheme", md5Checksum: "" });
    },
        ...
        return {
          ...
          raw: {
            media: {
              document: {
                size: input.length,
                attributes: [{ className: "DocumentAttributeFilename", fileName: "source.attheme" }],
              },
            },
          },
        };
    ...
    files: {
      async withTemp(operation) {
        ...
        try {
          ...
          if (options.cleanupError && sends.length) throw new Error("PRIVATE_CLEANUP");
        }
      },
    },
  };
  ...
  const invocation = {
    ...
    prefix: ".",
    args: [],
    message: { id: 1, chatId: "1", outgoing: true, replyToId: 2, text: ".theme", raw: { peerId: "peer" } },
  };
...
test("theme is explicit-command only and renders all four supported client formats with scoped files", async t => {
  ...
  assert.deepEqual(Object.keys(f.plugin.commands.theme.subcommands).sort(), [
    "android",
    "clients",
    "cloud",
    "cloud-settings",
  ...
  for (const target of ["android", "desktop", "tgx", "ios"]) {
    await f.plugin.commands.theme.subcommands[target].handle(
      { ...f.invocation, args: [], subcommand: target, subcommands: [target] },
      f.context,
    );
  }
...
test("theme preserves mixed-case slugs and shares only fetch data across concurrent callers", async t => {
  ...
  const f = await fixture(t, {
    async invoke(request) {
      if (request instanceof Api.account.GetTheme) {
        themeCalls += 1;
        if (themeCalls === 1) await gate;
        return {
          settings: {
            baseTheme: new Api.BaseThemeDay(),
            accentColor: 0x2481cc,
            outboxAccentColor: 0x2481cc,
  ...
  const first = link.handle(
    { ...f.invocation, args: ["https://t.me/addtheme/MixedCaseSlug"], subcommand: "link", subcommands: ["link"] },
    f.context,
  );
  while (!themeCalls) await new Promise(resolve => setImmediate(resolve));
  const second = link.handle(
    {
      ...f.invocation,
      args: ["https://t.me/addtheme/MixedCaseSlug"],
      subcommand: "link",
      subcommands: ["link"],
      message: { ...f.invocation.message, id: 3 },
    },
    f.context,
  );
  release();
  ...
  await link.handle(
    {
      ...
      subcommand: "link",
      subcommands: ["link"],
      message: { ...f.invocation.message, id: 4 },
    },
    f.context,
  );
...
test("theme resolves slug-only wallpaper once and embeds it in Android output", async t => {
  const wallpaper = await sharp({ create: { width: 8, height: 8, channels: 3, background: "#123456" } })
    .png()
    .toBuffer();
...
test("theme cloud document bridge produces resolvable UploadMedia and CreateTheme requests", async t => {
  const f = await fixture(t, {
    async invoke(request) {
      if (request instanceof Api.messages.UploadMedia)
        return { document: { id: 3n, accessHash: 4n, fileReference: Buffer.from("ref") } };
      if (request instanceof Api.account.CreateTheme) return { slug: "document_theme" };
      return {};
    },
  });
  await f.plugin.commands.theme.subcommands.cloud.handle(
    { ...f.invocation, args: [], subcommand: "cloud", subcommands: ["cloud"] },
    f.context,
  );
  ...
  await upload.resolve(f.client, Utils);
  await createRequest.resolve(f.client, Utils);
  assert.ok(upload.getBytes().length > 0);
  assert.ok(createRequest.getBytes().length > 0);
});

test("theme cloud-settings builds a real resolvable and serializable CreateTheme request", async t => {
  const f = await fixture(t);
  await f.plugin.commands.theme.subcommands["cloud-settings"].handle(
    { ...f.invocation, args: [], subcommand: "cloud-settings", subcommands: ["cloud-settings"] },
    f.context,
  );
...
test("theme rejects archives above the 64-entry expansion budget", async t => {
  ...
  t.after(() => fs.rm(root, { recursive: true, force: true }));
  esbuild.buildSync({
    entryPoints: [path.resolve(__dirname, "../theme/v2/converter.ts")],
    bundle: true,
    platform: "node",
    format: "cjs",
    outfile: output,
...
test("theme keeps sent output when completion receipt or temp cleanup fails", async t => {
  for (const options of [{ editError: text => text.includes("已完成") }, { cleanupError: true }]) {
    const f = await fixture(t, options);
    await f.plugin.commands.theme.subcommands.android.handle(
      { ...f.invocation, args: [], subcommand: "android", subcommands: ["android"] },
      f.context,
    );
    assert.equal(f.sends.length, 1);
    assert.equal(
      f.edits.some(x => x.text.includes("主题转换失败")),
      false,
    );
    assert.ok(f.logs.some(x => /theme_(?:completion_receipt|temp_cleanup)_failed/.test(x.event)));
    assert.doesNotMatch(JSON.stringify(f.logs), /PRIVATE/);
  }
});

test("theme cancellation after send propagates without a late completion edit", async t => {
  const f = await fixture(t, {
    onSent() {
      f.controller.abort(new Error("stop"));
    },
  });
  await f.plugin.commands.theme.subcommands.android.handle(
    { ...f.invocation, args: [], subcommand: "android", subcommands: ["android"] },
  ...
  assert.equal(f.sends.length, 1);
  assert.equal(
    f.edits.some(x => x.text.includes("已完成")),
    false,
  );
  assert.equal(
    f.edits.some(x => x.text.includes("失败")),
    false,
  );
});

test("theme hides arbitrary conversion diagnostics behind fixed feedback and logs", async t => {
  const secret = "PRIVATE_THEME_SECRET",
    f = await fixture(t, { input: Buffer.from(secret) });
  await f.plugin.commands.theme.subcommands.android.handle(
    { ...f.invocation, args: [], subcommand: "android", subcommands: ["android"] },
    f.context,
  );
  assert.deepEqual(f.logs, [{ event: "theme_convert_failed", fields: undefined }]);
  assert.match(f.edits.at(-1).text, /主题转换失败，请检查文件格式后重试/);
  assert.doesNotMatch(JSON.stringify({ edits: f.edits, logs: f.logs }), new RegExp(secret));
});

test("theme upload cancellation starts no UploadMedia or CreateTheme side effect", async t => {
  let started, release;
  const ready = new Promise(r => (started = r)),
    gate = new Promise(r => (release = r));
  const f = await fixture(t, {
    async uploadFile(_value, controller) {
      started();
      await gate;
      return new Api.InputFile({ id: 1n, parts: 1, name: "x", md5Checksum: "" });
    },
  });
  const running = f.plugin.commands.theme.subcommands.cloud.handle(
    { ...f.invocation, args: [], subcommand: "cloud", subcommands: ["cloud"] },
    f.context,
  );
  await ready;
  ...
  assert.equal(
    f.invokes.some(
      x =>
        x instanceof Api.messages.UploadMedia ||
        x instanceof Api.account.CreateTheme ||
        x instanceof Api.account.UploadWallPaper,
    ),
    false,
  );
  assert.equal(
...
test("theme publishes inflight before concurrent initial status edits settle", async t => {
  let waiting = 0,
    release;
  const gate = new Promise(r => (release = r));
  const f = await fixture(t, {
    editWait: async text => {
      if (/正在(?:读取|复用)/.test(text)) {
        waiting++;
        await gate;
      }
    },
    invoke: async request =>
      request instanceof Api.account.GetTheme
        ? { settings: { baseTheme: new Api.BaseThemeDay(), accentColor: 1 } }
        : {},
  });
  const link = f.plugin.commands.theme.subcommands.link,
    a = link.handle(
      { ...f.invocation, args: ["https://t.me/addtheme/RaceSlug"], subcommand: "link", subcommands: ["link"] },
      f.context,
    ),
    b = link.handle(
      {
        ...f.invocation,
        args: ["https://t.me/addtheme/RaceSlug"],
        subcommand: "link",
        subcommands: ["link"],
        message: { ...f.invocation.message, id: 8 },
      },
      f.context,
    );
  ...
  assert.equal(f.invokes.filter(x => x instanceof Api.account.GetTheme).length, 4);
});

test("theme reports partial link delivery without replacing sent formats", async t => {
  const f = await fixture(t, {
    sendError: index => index === 1,
    invoke: async request =>
      request instanceof Api.account.GetTheme
        ? { settings: { baseTheme: new Api.BaseThemeDay(), accentColor: 1 } }
        : {},
  });
  await f.plugin.commands.theme.subcommands.link.handle(
    { ...f.invocation, args: ["https://t.me/addtheme/Partial"], subcommand: "link", subcommands: ["link"] },
    f.context,
  );
  assert.equal(f.sends.length, 1);
  assert.deepEqual(f.logs, [{ event: "theme_link_partial_delivery_failed", fields: undefined }]);
  assert.match(f.edits.at(-1).text, /已发送 1 个格式/);
});

test("cloud-settings backup failure preserves the created link", async t => {
  const f = await fixture(t, { sendError: () => true });
  await f.plugin.commands.theme.subcommands["cloud-settings"].handle(
    { ...f.invocation, args: [], subcommand: "cloud-settings", subcommands: ["cloud-settings"] },
    f.context,
  );
  assert.ok(f.invokes.some(x => x instanceof Api.account.CreateTheme));
  assert.deepEqual(f.logs, [{ event: "theme_cloud_settings_backup_failed", fields: undefined }]);
  assert.match(f.edits.at(-1).text, /云端配色主题已创建[\s\S]*配色备份发送失败/);
  assert.equal(
    f.edits.some(x => x.text === "云端配色创建失败，请稍后重试"),
    false,
  );
});

test("theme observes fetch rejection while initial edit is pending and permits retry", async t => {
  let release,
    firstEdit = true,
    failFetch = true;
  const gate = new Promise(r => (release = r)),
  ...
  const f = await fixture(t, {
      editWait: async text => {
        if (firstEdit && /正在读取/.test(text)) {
          await gate;
          firstEdit = false;
          throw new Error("PRIVATE_EDIT");
        }
      },
      invoke: async request => {
        if (request instanceof Api.account.GetTheme) {
          if (failFetch) throw new Error("PRIVATE_FETCH");
          return { settings: { baseTheme: new Api.BaseThemeDay(), accentColor: 1 } };
        }
        return {};
      },
    }),
    link = f.plugin.commands.theme.subcommands.link;
  const first = link.handle(
    { ...f.invocation, args: ["https://t.me/addtheme/RetrySlug"], subcommand: "link", subcommands: ["link"] },
    f.context,
  );
  ...
  await link.handle(
    {
      ...f.invocation,
      args: ["https://t.me/addtheme/RetrySlug"],
      subcommand: "link",
      subcommands: ["link"],
      message: { ...f.invocation.message, id: 11 },
    },
    f.context,
  );
  assert.equal(f.invokes.filter(x => x instanceof Api.account.GetTheme).length, 8);
  assert.equal(f.sends.length, 4);
});
