...
let create;
test.before(() => {
  create = require(path.join(buildPlugin({id: 'aff', packageRoot: path.resolve(__dirname, '../aff'), entry: 'v2.ts', rootDir: core}).artifactDir, 'index.cjs')).default;
});

async function hostOn(root, options = {}) {
  const edits = [], replies = new Map();
  const host = new PluginHost({
    storageRoot: root, selfId: '1', prefixes: options.prefixes ?? ['.'], aliases: options.aliases ?? {},
    logger: {info() {}, error() {}},
    telegram: {
      edit: async (message, text, opts, signal) => { signal.throwIfAborted(); edits.push({id: message.id, text, options: opts ?? {}}); },
      reply: async () => {},
      invoke: async () => {},
...
async function fixture(options = {}) {
  ...
  const hosts = [];
  const start = async () => { const current = await hostOn(root, options); hosts.push(current); return current; };
  ...
  const reload = async () => { await current.host.shutdown(1000); current = await start(); return current; };
  return {
    root, read, cleanup, reload,
    get edits() { return current.edits; },
    get send() { return current.send; },
    get replies() { return current.replies; },
  };
}
...
test('AFF01 new saves store format html and resend as HTML after reload', async t => {
  ...
  t.after(() => f.cleanup());
  f.replies.set(2, {id: 2, chatId: 'chat', text: '<b>Aff</b> https://example.com', raw: {message: '<b>Aff</b> https://example.com'}});
...
test('AFF01 legacy web_page entries send HTML while V2 webPage entries stay literal', async t => {
  const f = await fixture({initial: {affs: [
...
test('AFF02 a full store rejects the save with zero writes', async t => {
  const f = await fixture({initial: {affs: Array.from({length: 32}, (_, index) => ({text: `entry-${index}`, webPage: false}))}});
  t.after(() => f.cleanup());
...
test('AFF03 list keeps 10-per-page pagination and escaped Unicode previews', async t => {
  const f = await fixture({prefixes: ['!'], aliases: {a: 'aff'}, initial: {affs: Array.from({length: 12}, (_, index) => ({
    text: `entry${index + 1} <&😀`, webPage: false,
  }))}});
  t.after(() => f.cleanup());
