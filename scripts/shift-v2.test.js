...
const { artifactDir } = buildPlugin({
  id: "shift",
  packageRoot: process.env.SHIFT_TEST_SOURCE || path.resolve(__dirname, "../shift"),
  entry: "v2.ts",
});
...
function fixture(plugin = create(), options = {}) {
  ...
  const client = {
    async getEntity(value) {
      return entities[String(value)] || { className: "User", id: String(value), firstName: String(value) };
    },
    async forwardMessages(target, parameters) {
      await options.forward?.();
      forwards.push({ target, options: parameters });
      return [];
    },
    async getMessages() {
      return [];
  ...
  const storage = {
    json(file) {
      return {
        ...
        async update(change) {
          if (file !== "shift_v2.json" && beforeUpdate) {
            const operation = beforeUpdate;
    ...
    sqlite(file, settings) {
      assert.equal(file, "shift.db");
      assert.equal(settings.mustExist, true);
      return {
        async read(operation, signal) {
          signal?.throwIfAborted();
          const db = new Database(options.sqliteFile, { fileMustExist: true, readonly: true });
          try {
            return operation(db);
          } finally {
            db.close();
  ...
  const context = {
    ...
    storage,
    regexp: {
      async test(pattern, input, settings) {
        return options.regexp
          ? options.regexp(pattern, input, settings)
          : { matched: new RegExp(pattern, settings?.flags).test(input), timedOut: false };
      },
    },
    telegram: {
    ...
    files: {
      ...
      async withTemp(operation) {
        return operation("/tmp", controller.signal);
      },
    },
    tasks: {
      run(label, operation) {
  ...
  const message = (chatId, id, text, raw = {}) => ({
    id,
    chatId,
    senderId: "1",
    outgoing: false,
    text,
    raw: { peerId: chatId, ...raw },
  });
  const run = (args, text = `.shift ${args.join(" ")}`, chatId = "-10050071992547409931234") =>
    plugin.commands.shift.handle(
      { command: "shift", prefix: ".", args, message: { ...message(chatId, 1, text), outgoing: true } },
      context,
    );
  return {
    plugin,
...
test("shift groups albums in a tracked task and migrates legacy string IDs once", async () => {
  ...
  await f.plugin.listeners[0].handle(
    f.message(source, 9, "album caption", { groupedId: "922337203685477000", photo: {} }),
    f.context,
  );
  await f.plugin.listeners[0].handle(
    f.message(source, 8, "", { groupedId: "922337203685477000", photo: {} }),
    f.context,
  );
  ...
  migrated.setLegacy({
    rules: {
      "-10090071992547409931234": {
        target_id: "-10080071992547409931234",
        options: ["all", "replyTo:73", "send-as=70071992547409931234"],
        paused: false,
        filters: [],
      },
    },
...
test("shift migration honors an explicitly empty V2 rule list and preserves extension fields", async t => {
  const f = fixture();
  t.after(() => fs.rm(f.stateFile, { force: true }));
  const current = { schemaVersion: 2, rules: [], backups: {}, future: { owner: "v2" } };
  f.setState(structuredClone(current));
  await fs.writeFile(f.stateFile, JSON.stringify(current));
  f.setLegacy({
    rules: { 1: { target_id: "2", options: ["all"] } },
    stats: { legacy: true },
    future: { owner: "legacy" },
  });
  await f.plugin.setup(f.context);
  assert.deepEqual(f.state().rules, []);
  assert.equal(f.state().legacyImported, true);
  assert.deepEqual(f.state().stats, { legacy: true });
  assert.deepEqual(f.state().future, { owner: "v2" });
});

test("shift migrates actual SQLite rules and statistics with exact 64-bit IDs", async t => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "shift-sqlite-")),
    file = path.join(directory, "shift.db");
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const db = new Database(file);
  db.exec(
    "CREATE TABLE shift_rules(source_id INTEGER PRIMARY KEY,target_id INTEGER NOT NULL,options TEXT NOT NULL,target_type TEXT NOT NULL,paused INTEGER DEFAULT 0,created_at TEXT NOT NULL,filters TEXT NOT NULL);CREATE TABLE shift_stats(stats_key TEXT PRIMARY KEY,stats_data TEXT NOT NULL)",
  );
  db.prepare("INSERT INTO shift_rules VALUES(?,?,?,?,?,?,?)").run(
    9007199254740993123n,
    8007199254740993123n,
    '["all","replyTo:73"]',
    "channel",
    0,
  ...
  db.prepare("INSERT INTO shift_stats VALUES(?,?)").run(
    "shift.stats.9007199254740993123.2025-02-03",
    JSON.stringify({ total: 17, text: 12, photo: 5 }),
  );
  db.close();
  const f = fixture(create(), { sqliteFile: file });
  await f.plugin.setup(f.context);
  const rule = f.state().rules[0];
  assert.equal(rule.source, "9007199254740993123");
  assert.equal(rule.target, "8007199254740993123");
  assert.equal(rule.topicId, 73);
  assert.equal(rule.stats.forwarded, 17);
  assert.equal(rule.stats.lastForwardedAt, Date.parse("2025-02-03"));
  assert.equal(f.state().legacyImported, true);
});
...
test("shift maps recoverable Lowdb backup and historical statistics into the current model", async () => {
  const f = fixture();
  f.setLegacy({
    rules: { 101: { target_id: "202", options: ["all"], created_at: "2025-01-01T00:00:00Z" } },
    stats: { "2025-03-04": { 101: { total: 9, text: 9 } } },
    backups: {
      legacy1: {
        sourceId: 101,
        targetId: 202,
  ...
  await f.plugin.setup(f.context);
  const task = f.state().backups.legacy1;
  assert.equal(f.state().rules[0].stats.forwarded, 9);
  assert.equal(task.source, "101");
  assert.equal(task.target, "202");
  assert.equal(task.order, "asc");
  assert.equal(task.processed, 12);
  assert.equal(task.failed, 2);
  assert.equal(task.cursor, 77);
  assert.equal(task.status, "paused");
});

test("shift list uses complete SDK pagination", async () => {
  const f = fixture(),
    rules = Array.from({ length: 200 }, (_, index) => ({
      source: String(index + 1),
      target: String(index + 1001),
      options: ["all"],
      paused: false,
      filters: [],
      ...
      targetDisplay: "T",
      createdAt: 1,
      stats: { forwarded: index, failed: 0 },
    }));
  f.setState({ schemaVersion: 2, legacyImported: true, rules, backups: {} });
  await f.run(["list"]);
...
test("shift cancellation after native forwarding does not record success or failure", async () => {
  let f;
  f = fixture(create(), { forward: async () => f.controller.abort() });
  await f.run(["set", "@source", "@target", "all"]);
  const source = f.state().rules[0].source;
...
test("shift accepts real Teleproto BigInteger entity IDs and produces serializable forwarding peers", async () => {
  const sourceId = helpers.returnBigInt("90071992547409931234"),
    targetId = helpers.returnBigInt("80071992547409931234");
  let wire;
  const f = fixture(create(), {
    entities: {
      "@source": new Api.Channel({ id: sourceId, accessHash: helpers.returnBigInt(1), title: "Source" }),
      "@target": new Api.Channel({ id: targetId, accessHash: helpers.returnBigInt(2), title: "Target" }),
    },
    forward: async () => {
      const request = new Api.messages.ForwardMessages({
        fromPeer: new Api.InputPeerChannel({ channelId: sourceId, accessHash: helpers.returnBigInt(1) }),
        id: [77],
        randomId: [helpers.returnBigInt(3)],
        toPeer: new Api.InputPeerChannel({ channelId: targetId, accessHash: helpers.returnBigInt(2) }),
        silent: true,
      });
      await request.resolve({ getInputEntity: async value => value }, utils);
      wire = request.getBytes();
    },
  });
  await f.run(["set", "@source", "@target", "all", "silent"]);
  const rule = f.state().rules[0];
  assert.equal(rule.source, "-10090071992547409931234");
  assert.equal(rule.target, "-10080071992547409931234");
  await f.plugin.listeners[0].handle(f.message(rule.source, 77, "hello"), f.context);
...
test("shift executes complex and catastrophic whitelist patterns only through the SDK worker", async () => {
  const calls = [],
    f = fixture(create(), {
      regexp: async (pattern, input, settings) => {
        calls.push({ pattern, input, settings });
  ...
  await f.run(["whitelist", "1", "enable"]);
  const source = f.state().rules[0].source;
  await f.plugin.listeners[0].handle(f.message(source, 9, "a".repeat(4000) + "!"), f.context);
  assert.equal(f.forwards.length, 0);
  assert.deepEqual(
    calls.map(x => x.pattern),
    ["(a+)+$"],
  );
  assert.deepEqual(calls[0].settings, { flags: "i" });
});
test("shift whitelist budget preempts an actually catastrophic expression", async () => {
  const regexp = new ScopedSafeRegExp({ concurrency: 1, queueCapacity: 0 }),
    signal = new AbortController().signal,
    result = await regexp.test("(a+)+$", "a".repeat(4095) + "!", { flags: "i" }, signal);
...
test("shift backup status paginates every task instead of silently keeping ten", async () => {
  ...
  f.setState({ schemaVersion: 2, legacyImported: true, rules: [], backups });
  await f.run(["backup", "status"]);
  assert.ok(f.replies.length > 0);
  const output = [f.edits.at(-1), ...f.replies].join("\n");
  assert.match(output, /task-0:/);
  assert.match(output, /task-149:/);
});
