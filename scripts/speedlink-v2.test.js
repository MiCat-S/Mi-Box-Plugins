...
test('speedlink remote runner pins the scanned host key and uses fixed non-interactive SSH arguments', async t => {
  ...
  const cleanupLogs=[];const context = {signal: new AbortController().signal,log:{error(event){cleanupLogs.push(event);}},files: {async withTemp(operation) {return operation(temporary, context.signal);}},
  ...
  assert.deepEqual(calls[2].args.slice(-8), ['-p', '2222', '--', 'ubuntu@203.0.113.2', 'speedtest', '--accept-license', '--accept-gdpr', '--format=json']);
  ...
  const cleanupDirectory=await fs.mkdtemp(path.join(buildRoot,'cleanup-'));context.files.withTemp=async operation=>{const value=await operation(cleanupDirectory,context.signal);throw new Error('private cleanup');};
  const cleanupResult=await runRemote(context,{name:'tokyo',username:'ubuntu',host:'203.0.113.2',port:2222,fingerprint},45);
  assert.equal(cleanupResult.server.name,'fixed');assert.ok(cleanupLogs.includes('speedlink_remote_cleanup_failed'));
