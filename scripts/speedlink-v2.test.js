...
const { artifactDir } = buildPlugin({
  id: "speedlink",
  packageRoot: path.resolve(__dirname, "../speedlink"),
  entry: "v2.ts",
});
...
test("speedlink loads without starting a process and stores only pinned agent configuration", async t => {
  ...
  assert.deepEqual(stored.servers, [
    {
      name: "tokyo",
      username: "ubuntu",
      host: "203.0.113.2",
...
test("speedlink remote runner pins the scanned host key and uses fixed non-interactive SSH arguments", async t => {
  ...
  esbuild.buildSync({
    entryPoints: [path.resolve(__dirname, "../speedlink/v2/runner.ts")],
    bundle: true,
    platform: "node",
    format: "cjs",
    outfile: output,
  ...
  const resultJson = JSON.stringify({
    server: { id: "1", name: "fixed", location: "remote" },
    isp: "ISP",
    ping: { latency: 10 },
  ...
  const context = {
    ...
    log: {
      error(event) {
        cleanupLogs.push(event);
      },
    },
    files: {
      async withTemp(operation) {
        return operation(temporary, context.signal);
      },
    },
    processes: {
      async run(file, args, options) {
        calls.push({ file, args, options });
        if (file === "/usr/bin/ssh-keyscan")
          return { stdout: Buffer.from("[203.0.113.2]:2222 ssh-ed25519 AAAATEST\n") };
  ...
  const result = await runRemote(
    context,
    { name: "tokyo", username: "ubuntu", host: "203.0.113.2", port: 2222, fingerprint },
    45,
  );
  ...
  assert.deepEqual(calls[2].args.slice(-8), [
    "-p",
    "2222",
    "--",
    "ubuntu@203.0.113.2",
  ...
  const cleanupDirectory = await fs.mkdtemp(path.join(buildRoot, "cleanup-"));
  context.files.withTemp = async operation => {
    const value = await operation(cleanupDirectory, context.signal);
  ...
  assert.equal(cleanupResult.server.name, "fixed");
  assert.ok(cleanupLogs.includes("speedlink_remote_cleanup_failed"));
  const before = calls.length;
  await assert.rejects(
    runRemote(context, { name: "tokyo", username: "ubuntu", host: "203.0.113.2", port: 2222, fingerprint }, 181),
    /10 到 180/,
  );
  ...
  await command.handle(
    {
      ...
      subcommand: "list",
      subcommands: ["list"],
      message: { id: 1, chatId: "1", outgoing: true, text: ".speedlink list" },
    },
    context,
  );
...
test("speedlink all stops the active SSH process at the 30 minute total deadline", async t => {
  ...
  const state = {
    schemaVersion: 1,
    timeoutSeconds: 180,
    servers: [{ name: "slow", username: "ubuntu", host: "203.0.113.2", port: 22, fingerprint }],
    legacyDatabaseDetected: false,
    legacyNoticeShown: false,
  };
  let start;
  ...
  const context = {
    ...
    storage: {
      json() {
        return {
          ...
          async update(change) {
            return change(structuredClone(state));
          },
        };
      },
    },
    files: {
      async withTemp(operation) {
        return operation(root, context.signal);
      },
    },
    processes: {
  ...
  const running = command.handle(
    {
      ...
      subcommand: "all",
      subcommands: ["all"],
      message: { id: 1, chatId: "1", outgoing: true, text: ".speedlink all" },
    },
    context,
  );
  await started;
