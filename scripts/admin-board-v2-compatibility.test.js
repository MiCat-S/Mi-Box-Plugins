...
test.before(() => {
  const built = buildPlugin({
    id: "admin_board",
    packageRoot: path.resolve(__dirname, "../admin_board"),
    entry: "v2.ts",
    rootDir: core,
  });
...
const channel = (id, title, username) =>
  new Api.Channel({ id: bi(id), accessHash: bi(7), title, megagroup: true, ...(username ? { username } : {}) });
...
function fixture(options = {}) {
  ...
  const client = {
    async getEntity(value) {
      if (
        value &&
        typeof value === "object" &&
        (value === target || value.className === "Channel" || value.className === "Chat")
      ) {
        calls.push({ method: "getEntity", value: "<target>" });
        return target;
      }
    ...
    async getParticipants(entity, params = {}) {
      ...
      if (options.onGetParticipants) {
        const result = await options.onGetParticipants(entity, params);
        if (result !== undefined) return result;
      }
      if (params.search)
        return users.filter(candidate => (candidate.username || "").toLowerCase() === params.search.toLowerCase());
      return users;
    },
    async getInputEntity(value) {
      ...
      if (value && value.className === "Channel")
        return new Api.InputPeerChannel({ channelId: value.id, accessHash: bi(7) });
    ...
    async invoke(request) {
        ...
        await request.resolve(
          {
            getInputEntity: async value => {
              if (value && value.className === "Channel")
                return new Api.InputPeerChannel({ channelId: value.id, accessHash: bi(7) });
              if (value && value.className === "Chat") return new Api.InputPeerChat({ chatId: value.id });
              if (value && value.className === "User")
                return new Api.InputPeerUser({ userId: value.id, accessHash: bi(1) });
              return value;
            },
          },
          utils,
      ...
      if (options.onInvoke) {
        const result = await options.onInvoke(request, calls.length);
        if (result !== undefined) return result;
      }
      ...
      if (request instanceof Api.channels.GetParticipants)
        return options.channelParticipants ?? { users, participants: users.map(() => ({})) };
      return {};
    },
  };
  const ctx = {
      ...
      json(name, defaults) {
        ...
        return {
          ...
          update: async mutator => {
            const next = await mutator(clone(state.get(name)));
  ...
  const run = async text => {
    const args = text
      .replace(/^\.admin_board\b\s*/, "")
      .trim()
      .split(/\s+/)
      .filter(Boolean);
    await definition.commands.admin_board.handle(
      {
        message: { id: 1, chatId: String(target.id), text, outgoing: true, raw: { peerId: target } },
        command: "admin_board",
        prefix: ".",
        args,
      },
...
test("ABO-01 rm selects the lowest non-creator, never the creator", async () => {
  ...
  assert.ok(
    !f.calls.some(
      call => call.method === "invoke" && call.className === "channels.EditAdmin" && String(call.value) === "1",
    ),
  );
...
test("ABO-02 explicit trailing target is consumed without eating the last username", async () => {
  ...
  assert.ok(
    f.calls.some(call => call.method === "getEntity" && call.value === "@team"),
    "target resolved",
  );
});
...
test("ABO-02 channel getEntity failure falls back to participant search", async () => {
  ...
  const f = fixture({
    users: [bobby],
    getEntityFails: ["@bobby"],
    channelParticipants: { users: [bobby], participants: [{}] },
  });
...
test("ABO-02 long numeric IDs survive the channel pagination fallback exactly", async () => {
  ...
  const f = fixture({
    users: [target],
    getEntityFails: [bigId],
    channelParticipants: { users: [target], participants: [{}] },
  });
...
test("ABO-03 rm reports progress and the demoted admins", async () => {
  ...
  await f.run(".admin_board rm 1");
  assert.ok(
    f.edits.some(text => /进度: <code>1\/1<\/code>/.test(text)),
    "progress feedback",
  );
...
test("ABO-05 cancellation during the first demote performs no further RPC", async () => {
  ...
  const f = fixture({
    ...
    cache: {
      "77:1": { updatedAt: Date.now(), avgPerDay: 0 },
      "77:2": { updatedAt: Date.now(), avgPerDay: 7 },
      "77:3": { updatedAt: Date.now(), avgPerDay: 6 },
    },
    onGetInputEntity: async () => {
      inputCalls += 1;
      if (inputCalls === 1) f.controller.abort(new DOMException("cancelled", "AbortError"));
    },
  });
...
test("ABO-05 cancellation during the first stats query stops later queries", async () => {
  ...
  assert.ok(
    !f.invokes.some(
      request => request instanceof Api.channels.EditAdmin || request instanceof Api.messages.EditChatAdmin,
    ),
  );
});
...
test("pagination keeps every admin, marks continuation pages and bounds each page", async () => {
  const users = Array.from({ length: 80 }, (_, index) =>
    user({
      id: String(index + 1),
      firstName: `Admin${String(index + 1).padStart(2, "0")}`,
      participant: adminParticipant("A"),
    }),
  );
  ...
  assert.ok(
    f.replies.length >= 1 && f.replies.every(page => continuation.test(page)),
    "every reply is a marked continuation",
  );
...
test("pagination preserves long escaped text without emitting raw markup", async () => {
  ...
  assert.ok(
    f.replies.every(page => continuation.test(page)),
    "continuations are marked",
  );
...
async function hostFixture(options = {}) {
  ...
  const target = options.target ?? channel(77, "Team", "team");
  const users = options.users ?? [
    user({ id: "1001", firstName: "Alice", username: "alice", participant: adminParticipant("A") }),
  ];
  const edits = [],
    replies = [],
    invokes = [],
  ...
  const client = {
    ...
    async invoke(request) {
      ...
      if (request instanceof Api.messages.Search)
        return { count: options.searchCounts?.[String(request.fromId?.userId ?? "")] ?? 0 };
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      reply: async (_message, text, _options, signal) => {
        signal.throwIfAborted();
        replies.push(text);
      },
      invoke: async () => ({}),
  ...
  const savedLocks = async () => {
    try {
      return JSON.parse(await fs.readFile(path.join(root, "admin_board/seat_locks.json"), "utf8")).lockedSeats;
    } catch {
      return {};
    }
  };
...
test("real host paginates an alias list with continuation markers", async t => {
  ...
  assert.ok(
    h.replies.every(page => continuation.test(page)),
    "continuations marked through the host",
  );
  const all = pages.join("\n");
