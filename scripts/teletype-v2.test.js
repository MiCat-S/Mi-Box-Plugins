...
const { artifactDir } = buildPlugin({
  id: "teletype",
  packageRoot: path.resolve(__dirname, "../teletype"),
  entry: "v2.ts",
});
...
async function fixture(t, prefixes = ["."], options = {}) {
  ...
  return {
    ...
    message,
    run(text, extra) {
      return host.dispatchPrimary(message(text, extra));
    },
  };
}
...
test("MESSAGE_NOT_MODIFIED in an intermediate batch still reaches the complete final text", async t => {
  ...
  const f = await fixture(t, ["."], {
    editFailure() {
      if (++calls === 4)
        throw Object.assign(new Error("MESSAGE_NOT_MODIFIED"), { errorMessage: "MESSAGE_NOT_MODIFIED" });
    },
  });
...
test("non-numeric historical maxEdits does not skip progressive body edits", async t => {
  const f = await fixture(t, ["."], {
    initialConfig: { schemaVersion: 1, autoMode: false, enabledUsers: [], maxEdits: "invalid" },
  });
  await f.run(".teletype 甲乙");
