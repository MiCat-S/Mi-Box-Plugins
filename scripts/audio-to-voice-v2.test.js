...
function audioMessage({id = 50, mimeType = 'application/octet-stream', duration = 7, voice, size = waveSample().length} = {}) {
  const attribute = new Api.DocumentAttributeAudio({duration, ...(voice === undefined ? {} : {voice})});
  const document = new Api.Document({id: integer(id), accessHash: integer(id + 1), fileReference: Buffer.alloc(0),
    date: 0, mimeType, size: integer(size), thumbs: [], videoThumbs: [], dcId: 1, attributes: [attribute]});
  return new Api.Message({id, peerId: new Api.PeerChannel({channelId: integer('9007199254740993')}), date: 0,
    message: '.audio_to_voice', media: new Api.MessageMediaDocument({document})});
}
...
async function fixture(t, options = {}) {
  ...
  const host = new PluginHost({storageRoot: root, tempRoot: path.join(root, 'temp'),
    ...
    logger: {info(event) { logs.push(event); }, error(event) { logs.push(event); }}, telegram: {
      async edit(message, text, settings) { edits.push({message, text, settings}); },
      async reply() { assert.fail('unexpected reply'); },
      async invoke() { assert.fail('unexpected generic invoke'); },
      async getReply() { replyReads++; return options.replyRaw ? {raw: options.replyRaw} : undefined; },
      async withClient(operation, signal) { return operation(client, signal); },
    }});
  const definition = artifact.default({resolveFfmpeg: async () => { resolverCalls++; return converter; }});
  await host.load(definition);
  t.after(async () => {
    assert.equal((await host.shutdown(2000)).completed, true);
    await fs.rm(root, {recursive: true, force: true});
  });
  const run = (message = {}) => host.dispatchPrimary({id: 80, chatId: '-1009007199254740993', senderId: '1',
    outgoing: true, text: '.audio_to_voice', raw: commandRaw, ...message});
  return {root, host, edits, sends, logs, commandRaw, run, replyReads: () => replyReads,
    deletes: () => deletes, downloads: () => downloads, resolverCalls: () => resolverCalls,
...
test('audio_to_voice treats command deletion as best-effort after a successful upload', async t => {
  ...
  assert.equal(f.edits.some(({text}) => /音频转换失败/.test(text)), false);
});
...
test('audio_to_voice unload during download starts no process or late Telegram send', async t => {
  ...
  assert.equal(f.sends.length, 0);
  assert.deepEqual(f.edits.map(({text}) => text), ['正在转换音频…']);
});
...
test('audio_to_voice streams a real PCM sample exactly and enforces the input byte ceiling', async t => {
  ...
  const client = {async *iterDownload() { yield sample.subarray(0, 44); yield sample.subarray(44); }};
...
test('audio_to_voice cancellation during a write prevents the next simulated GetFile request', async t => {
  ...
  const client = {iterDownload(_media, params) {
...
test('audio_to_voice retains the hardened FFmpeg argv', () => {
  assert.deepEqual(artifact.ffmpegArguments('/tmp/input', '/tmp/output.ogg'), [
    '-nostdin', '-y', '-i', '/tmp/input', '-vn', '-acodec', 'libopus', '-b:a', '64k', '-ar', '48000', '-ac', '1', '/tmp/output.ogg',
  ]);
});
