...
function memoryStorage(seed = {}) {
  ...
  return {
    ...
    json(file, defaults) {
      ...
      return {
        async read() {
          return structuredClone(documents.get(file));
        },
        async update(mutator) {
          const value = await mutator(structuredClone(documents.get(file)));
          documents.set(file, structuredClone(value));
...
test("botmzt serially retrieves a new bot response and sends spoiler media", async () => {
  ...
  const client = {
    ...
    async getMessages() {
      reads += 1;
      return reads === 1 ? [{ id: 10, out: false }] : [{ id: 11, out: false, media: { photo: true } }];
    },
...
test("bs forwards the replied message to the first channel it can post in", async () => {
  ...
  const client = {
    ...
    async invoke(request) {
      calls.push(["invoke", request.className]);
      return { updates: [{ message: { className: "Message", id: 5 } }] };
    },
  };
  const f = baseContext();
  f.context.telegram.withClient = operation => operation(client, f.context.signal);
  ...
  assert.deepEqual(calls, [
    ["getEntity", "@ObservingHumanActivity"],
    ["invoke", "messages.ForwardMessages"],
  ]);
...
test("cosplay accepts only bounded same-domain media and streams temp files to Telegram", async () => {
  ...
  f.context.http = {
    async withResponse(url, init, consume, options) {
      ...
      assert.deepEqual(options.redirects, {
        allowedHosts: [isImage ? "img.cosplaytele.com" : "cosplaytele.com"],
        maxRedirects: 3,
      });
  ...
  f.context.files = {
    async withTemp(use) {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "cos-v2-"));
      try {
        return await use(directory, f.context.signal);
      } finally {
        await fs.rm(directory, { recursive: true, force: true });
      }
    },
  };
  f.context.telegram.withClient = operation =>
    operation(
      {
        async sendFile(peer, value) {
          files.push({ peer, value });
  ...
  assert.equal(files[0].value.spoiler, true);
  assert.match(files[0].value.caption, /^套图链接: https:\/\/cosplaytele\.com\//);
});
...
test("cosplay rejects invalid counts before network access", async () => {
  ...
  assert.match(f.edits[0].text, /1 到 10/);
});
...
test("cosplay honors cancellation without a success placeholder", async () => {
  ...
  f.context.files = {
    async withTemp() {
      throw new Error("must not create files");
    },
  };
  f.context.telegram.withClient = async () => {
    files += 1;
  };
  await plugin("cosplay").commands.cos.handle(invocation("cos", ["1"]), f.context);
...
test("git_PR restricts token setup while allowing authenticated group reads", async () => {
  ...
  f.context.http = {
    async withResponse(url, init, consume, options) {
      ...
      return consume(
        new Response(JSON.stringify([{ full_name: "o/r", permissions: { push: true } }]), { status: 200 }),
        f.context.signal,
      );
    },
  };
  const definition = plugin("git_PR");
  await definition.setup(f.context);
  ...
  assert.match(f.edits.at(-1).text, /o\/r/);
...
test("git_PR mergeall reports partial success and authentication failures without leaking token", async () => {
  ...
  f.context.http = {
    async withResponse(url, init, consume) {
      ...
      if (init.method === "GET" && /\/pulls\/[12]$/.test(target)) response = Response.json({ mergeable: true });
        ...
        response = new Response(JSON.stringify([{ number: 2 }, { number: 1 }]), { status: 200 });
  ...
  assert.match(f.edits.at(-1).text, /失败：1/);
  assert.ok(f.edits.every(value => !value.text.includes("secret")));
});
...
test("git_PR returns a stable authentication error and never exposes provider details", async () => {
  ...
  await plugin("git_PR").commands.git.handle(
    invocation("git", ["repos"], { saved: false, chatId: "-10099" }),
    f.context,
  );
