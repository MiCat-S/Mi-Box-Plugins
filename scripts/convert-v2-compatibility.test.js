...
function artifact() {
  ...
  const entry = path.join(artifactDir, 'index.cjs'); delete require.cache[require.resolve(entry)]; return require(entry).default;
}
...
test('convert runs mocked helpers through the real host and uses an exact native peer', async t => {
  ...
  const client = {
    async downloadMedia(_media, options) {assert.ok(options.signal instanceof AbortSignal);options.progressCallback(1024n, 1024n);await fs.writeFile(options.outputFile, Buffer.alloc(1024));},
    async sendFile(peer, options) {sent.push({peer, options});},
    async deleteMessages(peer, ids, options) {deleted.push({peer, ids, options});},
  };
  const host = new PluginHost({storageRoot: root, processes: {concurrency: 1, queueCapacity: 2, timeoutMs: 180000, maxOutputBytes: 256 * 1024}, logger: {info() {}, error() {}}, telegram: {
    async edit(_message, text) {edits.push(text);}, async reply() {}, async invoke() {},
    async getReply() {return {id: 9, raw: source};}, async withClient(operation, signal) {return operation(client, signal);},
  }});
  ...
  await host.dispatchPrimary({id: 10, chatId: '-1009007199254740993', senderId: '7', outgoing: true,
    replyToId: 9, text: '.convert 新名称', raw: {}});
...
test('convert aborts an unknown-size download when cumulative progress exceeds the limit', async t => {
  ...
  let native = 0;
  const host = new PluginHost({storageRoot: root, processes: {concurrency: 1, queueCapacity: 2, timeoutMs: 180000, maxOutputBytes: 256 * 1024}, logger: {info() {}, error() {}}, telegram: {
    async edit(_message, text) {assert.doesNotMatch(text, /Input too large/);}, async reply() {}, async invoke() {},
    async getReply() {return {id: 9, raw: {media: {}, document: {}}};},
    async withClient(operation, signal) {native += 1; return operation({async downloadMedia(_media, options) {
      options.progressCallback(2147483649n, 0n); throw new Error('must stop at progress');}}, signal);},
  }});
  await host.load(artifact()());
  t.after(async () => {await host.shutdown(2000); await fs.rm(root, {recursive: true, force: true});});
  await host.dispatchPrimary({id: 10, chatId: '7', senderId: '7', outgoing: true, replyToId: 9, text: '.convert', raw: {}});
...
test('convert rejects oversized helper output even when a helper ignores ffmpeg -fs', async t => {
  ...
  await fs.chmod(helper, 0o700);
  let sends = 0;
  const host = new PluginHost({storageRoot: root, processes: {concurrency: 1, queueCapacity: 2, timeoutMs: 180000, maxOutputBytes: 256 * 1024},
    logger: {info() {}, error() {}}, telegram: {async edit(_message, text) {assert.doesNotMatch(text, /Invalid output/);}, async reply() {}, async invoke() {},
      async getReply() {return {id: 9, raw: {media: {}, document: {size: 5n}}};}, async withClient(operation, signal) {return operation({
        async downloadMedia(_media, options) {options.progressCallback(5n, 5n); await fs.writeFile(options.outputFile, 'video');},
        async sendFile() {sends += 1;}}, signal);}}});
  await host.load(artifact()({ffmpeg: [helper], ffprobe: [helper]}));
  t.after(async () => {await host.shutdown(2000); await fs.rm(root, {recursive: true, force: true});});
  await host.dispatchPrimary({id: 10, chatId: '7', senderId: '7', outgoing: true, replyToId: 9, text: '.convert', raw: {}});
...
test('convert unload during ffprobe does not continue to Telegram upload', async t => {
  ...
  let sends = 0, deletions = 0;
  const client = {async downloadMedia(_media, options) {await fs.writeFile(options.outputFile, 'video');},
    async sendFile() {sends += 1;}, async deleteMessages() {deletions += 1;}};
  const edits = [];
  const host = new PluginHost({storageRoot: root, processes: {concurrency: 1, queueCapacity: 2, timeoutMs: 180000, maxOutputBytes: 256 * 1024},
    logger: {info() {}, error() {}}, telegram: {async edit(_message, text) {edits.push(text);}, async reply() {}, async invoke() {},
      async getReply() {return {id: 9, raw: {media: {}, document: {size: 5n}}};},
      async withClient(operation, signal) {return operation(client, signal);}}});
  await host.load(artifact()({ffmpeg: [ffmpeg], ffprobe: [ffprobe]}));
  t.after(async () => {await host.shutdown(2000); await fs.rm(root, {recursive: true, force: true});});
  const dispatch = host.dispatchPrimary({id: 10, chatId: '7', senderId: '7', outgoing: true, replyToId: 9, text: '.convert', raw: {}});
  for (;;) {try {await fs.access(marker); break;} catch {await new Promise(setImmediate);}}
