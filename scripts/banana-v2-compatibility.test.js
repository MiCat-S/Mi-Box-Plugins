...
test('banana preserves output media type and treats command deletion as best effort', async t => {
  ...
  const client = {
    async *iterDownload() { yield Buffer.from('input-image'); },
    async sendFile(target, options) { files.push({target, options}); },
  ...
  const ai = definePlugin({
    ...
    commands: {},
    services: {
      image: {description: 'image', async handle() {
        return [{data: Buffer.from('jpeg-output'), mimeType: 'image/jpeg', revisedPrompt: '更亮'}];
      }},
      selection: {description: 'selection', async handle() { return {image: {tag: 'test', model: 'image'}}; }},
    },
  });
  await host.load(ai);
...
test('banana stops native sends and skips deletion when cancelled during the first upload', async t => {
  ...
  const client = {
    ...
    async deleteMessages(...args) { deletions.push(args); },
  };
  const host = new PluginHost({
    storageRoot,
    logger: {info() {}, error() {}},
    ...
    telegram: {
      ...
      async reply() {},
      async invoke() {},
      async getReply() { return {id: 77, raw: {media: {photo: {}}}}; },
      async withClient(operation, signal) { return operation(client, signal); },
    },
  });
  await host.load(definePlugin({
    ...
    description: 'test image provider',
    commands: {},
    services: {image: {description: 'image', async handle() {
      return [
