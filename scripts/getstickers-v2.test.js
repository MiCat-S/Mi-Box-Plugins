...
function document(size = 4, {id = 1n, accessHash = 2n, mimeType = 'image/webp', stickerSet} = {}) {
  return Object.assign(Object.create(Api.Document.prototype), {
    id, accessHash, fileReference: Buffer.from('ref'), mimeType, size,
    attributes: [Object.assign(Object.create(Api.DocumentAttributeSticker.prototype), {
      alt: '😀', stickerset: stickerSet || new Api.InputStickerSetShortName({shortName: 'local_pack'}),
    })],
  });
}
...
async function fixture(t, options = {}) {
  ...
  const controller = new AbortController(), edits = [], logs = [], sent = [], conversions = [], requests = [], locations = [], peers = [];
  const doc = options.document || document();
  const documents = options.documents || [doc];
  let downloads = 0;
  const client = {
    async invoke(request) {requests.push(request);await request.resolve(client,utils);assert.ok(request.getBytes().length>0);if(options.invoke)return options.invoke(request,controller);return options.result||{set: {shortName: 'local_pack'}, documents, packs: [{emoticon: '😀', documents: documents.map(item=>item.id)}]}; },
    async downloadFile(location, settings) {
      downloads += 1;locations.push(location);assert.ok(location.getBytes().length>0);
    ...
    async sendFile(peer, settings) {peers.push(peer);if(options.send)return options.send(settings);sent.push(await fs.readFile(settings.file));},
  };
  const context = {
  ...
  let deleted=0;
  return {root, controller, edits, logs, sent, conversions, requests, locations, peers, get downloads() { return downloads; },get deleted(){return deleted;},
    run() { return createGetStickers().commands.getstickers.handle({command: 'getstickers', prefix: '.', args: [],
      message: {id: 1, chatId: '1', text: '.getstickers', outgoing: true, replyToId: 3, raw: {peerId: options.rawPeer===undefined?'peer':options.rawPeer,async delete(){deleted++;if(options.deleteFailure)throw new Error('delete failed');}}}}, context); },
  };
}
...
test('large sticker-set and document ids retain precision and serialize through Teleproto', async t => {
  const setId = '9007199254741999', setHash = '-9007199254742999';
  const docId = '9007199254743999', docHash = '-9007199254744999';
...
test('cancellation while GetStickerSet is in flight prevents every download and failure notice', async t => {
  ...
  const f=await fixture(t,{invoke:async()=>{entered.resolve();await release.promise;return{set:{shortName:'local_pack'},documents:[document()],packs:[]};}});
  const running=f.run();
...
test('upload owns the archive until settlement and only successful delivery deletes the command', async t => {
  let archive;
  const success = await fixture(t, {send: async settings => {archive = settings.file;assert.equal((await fs.stat(settings.file)).isFile(), true);}});
  await success.run();
  assert.equal(success.deleted, 1);
  await assert.rejects(fs.stat(archive), {code: 'ENOENT'});
  const failed = await fixture(t, {send: async settings => {assert.equal((await fs.stat(settings.file)).isFile(), true);throw new Error('upload failed');}});
  await failed.run();
...
test('upload cancellation prevents command deletion and emits no failure notice',async t=>{
  const entered=deferred(),release=deferred();
  const f=await fixture(t,{send:async()=>{entered.resolve();await release.promise;}});
  const running=f.run();
  await entered.promise;
...
test('cancelling an active converter waits for process settlement before temp cleanup', async t => {
  let started, release;
  const ready = new Promise(resolve => {started = resolve;});
  const f = await fixture(t, {convert: async () => {
    started();
