...
async function fixture(t, id, fetch, client) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async withClient(operation, signal) {
        if (!client) throw new Error("unexpected client");
  ...
  return {
    ...
    replies,
    requests,
    run: text => host.dispatchPrimary({ id: 1, chatId: "100", senderId: "1", outgoing: true, text }),
  };
}
...
test("duckduckgo validates arguments and parses bounded HTML results", async t => {
  ...
  const f = await fixture(t, "duckduckgo", async url =>
    url.hostname === "html.duckduckgo.com" ? new Response(html) : Response.json({ data: { web: [] } }),
  );
...
test("deepwiki persists projects and performs a bounded MCP exchange", async t => {
  ...
  const f = await fixture(t, "deepwiki", async (_url, init) => {
    ...
    return new Response(
      JSON.stringify({ jsonrpc: "2.0", id: 2, result: { content: [{ type: "text", text: "Answer **safe**" }] } }),
      { headers: { "content-type": "application/json", "mcp-session-id": "session-1" } },
    );
  });
  await f.run(".deepwiki add core https://github.com/owner/repo");
  await f.run(".deepwiki lst");
  assert.match(f.edits.at(-1).text, /owner\/repo/);
  await f.run(".deepwiki question");
  assert.match(f.edits.at(-1).text, /Answer \*\*safe\*\*/);
  assert.equal(calls, 3);
  assert.equal(
    f.requests.every(x => x.init.redirect === "manual"),
    true,
  );
});
...
test("deepwiki paginates long Unicode answers without splitting HTML entities", async t => {
  ...
  const f = await fixture(t, "deepwiki", async (_url, init) => {
    const body = JSON.parse(init.body);
    if (body.method === "initialize") return new Response("{}", { headers: { "mcp-session-id": "session-1" } });
    if (body.method === "notifications/initialized") return new Response("", { status: 202 });
...
test("bgp DNS uses /24 first and filters noisy root domains", async t => {
  const f = await fixture(
    ...
    async () =>
      new Response(
        `<p>1.1.1.1 one.example.com 1.1.1.2 two.example.com 1.1.1.3 three.example.com 1.0.0.1 one.test.net</p>`,
      ),
  );
...
test("ntp reads a server date through controlled HTTP and rejects unknown mode", async t => {
  const f = await fixture(
    t,
    "ntp",
    async () => new Response(null, { status: 200, headers: { date: new Date().toUTCString() } }),
  );
  await f.run(".ntp");
  assert.match(f.edits.at(-1).text, /时间查询完成/);
  assert.equal(f.requests[0].init.method, "HEAD");
  assert.equal(f.requests[0].init.redirect, "manual");
  const before = f.requests.length;
  await f.run(".ntp nope");
  assert.equal(f.requests.length, before);
  assert.match(f.edits.at(-1).text, /NTP 对时/);
});

test("network plugin errors are redacted", async t => {
  const f = await fixture(t, "duckduckgo", async () => {
...
test("fixed-host redirects are rejected before contacting another origin", async t => {
  ...
  const f = await fixture(t, "duckduckgo", async () => {
    calls++;
    return new Response("", { status: 302, headers: { location: "https://attacker.invalid/secret" } });
  });
  await f.run(".ddg query");
  assert.equal(calls, 1);
  assert.equal(
    f.requests.every(request => request.url.hostname !== "attacker.invalid"),
    true,
  );
...
test("BGP SVG masks decoded addresses and removes IP hyperlink targets before rasterizing", () => {
  ...
  const svg = privateGraph(
    '<svg xmlns="http://www.w3.org/2000/svg"><a href="https://38.59.246.201"><text>38&#46;59.246.201</text></a><text>2001:db8::1</text></svg>',
  );
