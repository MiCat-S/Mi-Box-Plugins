...
async function fixture(t, fetch, client) {
  ...
  return {
    edits, replies, logs,
    run: (text, extra = {}) => host.dispatchPrimary({id: 7, chatId: '-1009007199254740993', senderId: '1', outgoing: true, text, ...extra}),
  };
}
...
test('successful graph transport deletes the command before sending the PNG and preserves the chat ID', async t => {
  ...
  const f = await fixture(t, async () => new Response(svg), {
    async deleteMessages(peer, ids, options) { calls.push(['delete', peer.toString(), ids, options]); },
    async sendFile(peer, options) { calls.push(['file', peer.toString(), await fs.readFile(options.file), options.caption]); },
  });
  await f.run('.bgp 1.1.1.1');
  assert.deepEqual(calls.map(call => call[0]), ['delete', 'file']);
...
test('DNS preserves every escaped record across bounded SDK rich-text pages', async t => {
  const records = Array.from({length: 600}, (_, index) => `1.1.${Math.floor(index / 250)}.${index % 250} node${index}.root${index}.test`).join(' &amp; ');
...
test('missing raw peer fields fall back to precise chat ID and deletion failure is logged', async t => {
  let sentPeer;
  const f = await fixture(t, async () => new Response(svg), {
    async deleteMessages() { throw new Error('expected delete failure'); },
    async sendFile(peer) { sentPeer = peer.toString(); },
  });
  await f.run('.bgp 1.1.1.1', {raw: {className: 'Message'}});
  assert.equal(sentPeer, '-1009007199254740993');
  assert.deepEqual(f.logs.filter(item => item.event === 'bgp_command_delete_failed'), [
...
test('a send failure after command deletion uses a new Telegram message for the error', async t => {
  ...
  const f = await fixture(t, async () => new Response(svg), {
    ...
    async sendFile() { throw new Error('transport-secret'); },
    async sendMessage(peer, options) { sent.push({peer: peer.toString(), options}); },
  });
  await f.run('.bgp 1.1.1.1');
...
test('placeholder graph keeps the legacy DFZ diagnosis and direct prefix link', async t => {
  const f = await fixture(t, async () => new Response('<svg>Not_Visible in_DFZ</svg>'), {
    async deleteMessages() { throw new Error('must not delete'); },
    async sendFile() { throw new Error('must not send'); },
  });
  await f.run('.bgp 1.1.1.1');
