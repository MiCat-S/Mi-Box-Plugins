...
async function fixture(t, options = {}) {
  ...
  const client = {
    async getEntity() {
      return (
        options.entity ??
        new Api.Channel({ id: 55n, accessHash: 7n, title: "<群😀>", photo: new Api.ChatPhotoEmpty(), date: 0 })
      );
    },
    async getInputEntity(value) {
      return String(value).includes("9")
        ? new Api.InputPeerUser({ userId: 9n, accessHash: 3n })
        : new Api.InputPeerChannel({ channelId: 55n, accessHash: 7n });
    },
    async *iterDownload() {
    ...
    async deleteMessages(target, ids, settings) {
      deletes.push({ target, ids, settings });
    },
    ...options.client,
  };
...
test("first Host load imports every legacy field when the new config file is absent", async t => {
  const legacy = {
    enabled: false,
    monitoredChats: ["-1007"],
    bannedMD5s: { ["a".repeat(32)]: "ban" },
    bannedStickerIds: { 77: "delete" },
    defaultAction: "ban",
    legacyOnly: "<kept>",
  };
  const f = await fixture(t, {
    setupRoot: async root => {
      const directory = path.join(root, "im");
...
test("explicit V2 fields win, unknown data survives, and completed migration never rereads legacy", async t => {
  const f = await fixture(t, {
    setupRoot: async root => {
      ...
      await fs.mkdir(directory, { recursive: true });
      await fs.writeFile(
        path.join(directory, "image_monitor_config.json"),
        JSON.stringify({ enabled: false, monitoredChats: ["legacy"], oldUnknown: 1 }),
      );
      await fs.writeFile(
        path.join(directory, "config.json"),
        JSON.stringify({ schemaVersion: 1, importedLegacy: false, enabled: true, monitoredChats: [], v2Unknown: 2 }),
      );
    },
  });
  ...
  await fs.writeFile(
    path.join(f.root, "im", "image_monitor_config.json"),
    JSON.stringify({ enabled: false, monitoredChats: ["changed"] }),
  );
...
test("a corrupt legacy file or pre-cancelled migration cannot write a success marker", async t => {
  ...
  const context = {
    ...
    files: { dataPath: name => path.join(directory, name) },
    storage: {
      json: () => ({
        read: async () => ({ ...base, importedLegacy: false, schemaVersion: 1 }),
        update: async () => {
          updates++;
        },
      }),
...
test("reply shortcuts preserve sticker IDs and stream media MD5 through the managed client", async t => {
  const sticker = new Api.Document({
    id: 888n,
    accessHash: 1n,
  ...
  const f = await fixture(t, {
    reply: {
      ...
      id: 2,
      replyToId: undefined,
      raw: { media: new Api.MessageMediaDocument({ document: sticker }) },
    },
  });
  ...
  f.setReply({
    ...
    raw: {
      media: new Api.MessageMediaPhoto({
        photo: new Api.Photo({ id: 4n, accessHash: 1n, fileReference: Buffer.alloc(0), date: 0, sizes: [], dcId: 1 }),
      }),
    },
  });
...
test("cancellation while resolving the channel prevents ban and delete", async t => {
  ...
  f.client.getInputEntity = async () => {
    enteredResolve();
    await gate;
    return new Api.InputPeerChannel({ channelId: 55n, accessHash: 7n });
  };
...
test("unload stops a hanging media iterator, waits for cleanup, and performs no moderation", async t => {
  ...
  const returnStarted = new Promise(resolve => {
    returnStartedResolve = resolve;
  });
  const returnGate = new Promise(resolve => {
  ...
  const iterator = {
    ...
    return() {
      ...
      returnStartedResolve();
      return returnGate.then(() => {
  ...
  let finished = false;
  const unloading = f.host.unload("im", 1000).then(report => {
    finished = true;
    return report;
  });
  await returnStarted;
