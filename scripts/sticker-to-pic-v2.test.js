...
function fixture(options = {}) {
  ...
  const context = {
    ...
    files: {
      async withTemp(use) {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mibot-stp-"));
        let value;
        try {
          value = await use(dir, context.signal);
          context.signal.throwIfAborted();
        } finally {
          await fs.rm(dir, { recursive: true, force: true });
        }
        if (options.cleanupError) throw new Error("private cleanup");
        return value;
      },
    },
    processes: {
      async run(command, args, processOptions) {
        ...
        if (options.process) return options.process(command, args);
        await fs.writeFile(args.at(-1), "picture");
        return { stdout: Buffer.from("ImageMagick 7") };
      },
    },
    telegram: {
      ...
      async getReply() {
        return { raw: options.sticker ?? sticker() };
      },
      async withClient(operation) {
        return operation(
          {
            async *iterDownload(_media, downloadOptions) {
              assert.equal(downloadOptions.signal, controller.signal);
              for (const chunk of options.chunks ?? [webp]) yield chunk;
            },
            async sendFile(peer, value) {
              sends.push({ peer, value });
              if (options.sendFile) return options.sendFile(peer, value);
            },
  ...
  return {
    ...
    controller,
    run: (text, message = {}) =>
      create().commands.stp.handle(
        {
          command: "stp",
          prefix: ".",
          args: text.split(/\s+/).slice(1),
          message: { id: 1, chatId: "5", outgoing: true, text, raw, replyToId: 9, ...message },
        },
        context,
      ),
  };
...
test("sticker_to_pic converts transparent PNG in a scoped temporary directory", async () => {
  ...
  assert.deepEqual(
    f.edits.slice(0, 3).map(item => item.text),
    ["📥 正在下载贴纸...", "🔄 正在转换为PNG格式...", "📤 正在发送图片..."],
  );
});
...
test("sticker_to_pic declares its conversion process budget", () => {
  assert.deepEqual(create().resources.processes, {
    concurrency: 1,
    queueCapacity: 2,
    timeoutMs: 60000,
