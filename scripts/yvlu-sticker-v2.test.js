...
const { artifactDir } = buildPlugin({
  id: "yvlu-sticker-test",
  packageRoot: path.resolve(__dirname, "../yvlu"),
  entry: "v2/stickers.ts",
});
...
function document(attributes = []) {
  return new Api.Document({
    ...
    accessHash: returnBigInt("9007199254740995"),
    fileReference: Buffer.from([1, 2]),
    date: 1,
    mimeType: "image/webp",
    size: returnBigInt(4),
    dcId: 2,
...
function existingSet() {
  return new Api.messages.StickerSet({
    set: new Api.StickerSet({ id: 1n, accessHash: 2n, title: "set", shortName: "set", count: 1, hash: 0 }),
    packs: [],
    keywords: [],
    documents: [],
  });
...
function contextFor(raw, invoke, uploadFile) {
  ...
  return {
    ...
    telegram: {
      async getReply() {
        return { id: 2, chatId: "1", text: "", outgoing: false, raw };
      },
        ...
        return operation(
          {
            ...
            uploadFile,
            async getMe() {
              return new Api.User({ id: 3n, firstName: "Me" });
            },
            async downloadMedia(_target, options) {
              return options.testImage;
            },
          },
          signal,
        );
...
test("existing sticker reuses its exact InputDocument and serializes AddStickerToSet", async () => {
  ...
  const calls = [],
    ctx = contextFor({ media: new Api.MessageMediaDocument({ document: doc }) }, async request => {
      calls.push(request);
      request.getBytes();
      if (request instanceof Api.messages.GetStickerSet) return existingSet();
      return true;
    });
  assert.equal(await saveSticker(ctx, { id: 1, chatId: "1", text: "", outgoing: true }, "set"), false);
  const add = calls.find(value => value instanceof Api.stickers.AddStickerToSet);
  assert.ok(add);
  add.getBytes();
...
test("photo uploads to a document then serializes CreateStickerSet when the set is absent", async () => {
  const png = await sharp({ create: { width: 8, height: 6, channels: 4, background: "#abc" } })
    .png()
    .toBuffer();
  const uploaded = new Api.InputFile({ id: 4n, parts: 1, name: "sticker.png", md5Checksum: "x" }),
  ...
  ctx.telegram.withClient = async operation =>
    operation(
      {
        invoke: ctxInvoke,
        ...
        async downloadMedia(_target, options) {
          options.progressCallback(returnBigInt(png.length), returnBigInt(png.length));
          return png;
        },
      },
      ctx.signal,
    );
  async function ctxInvoke(request) {
    calls.push(request);
    request.getBytes();
    if (request instanceof Api.messages.GetStickerSet) throw { errorMessage: "STICKERSET_INVALID" };
    if (request instanceof Api.messages.UploadMedia) return new Api.MessageMediaDocument({ document: uploadedDoc });
  ...
  const create = calls.find(value => value instanceof Api.stickers.CreateStickerSet);
  assert.ok(create);
  create.getBytes();
