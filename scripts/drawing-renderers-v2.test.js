...
function load(name, source, entry) {
  ...
  esbuild.buildSync({
    ...(entry ? { entryPoints: [entry] } : { stdin: { contents: source, loader: "ts", resolveDir: core } }),
    outfile,
    bundle: true,
    platform: "node",
    format: "cjs",
...
const chartReference = load(
  "chart-reference",
  between(originalChart, "function generateChartConfig(", "async function downloadChart(") +
    "\nexport {generateChartConfig};",
);
const originalCaptcha = fs.readFileSync(path.join(root, "pmcaptcha/pmcaptcha.ts"), "utf8");
const captchaReference = load(
  "captcha-reference",
  `async function tryGetCanvas(){return require('canvas');}
...
test("word cloud preserves original word selection, spiral placement, font sizes and PNG output", async () => {
  ...
  for (let repeat = 0; repeat < 5; repeat++)
    for (const message of messages) {
      cloud.collectWords(message, actual);
      cloudReference.collectWords(message, expected);
    }
  assert.deepEqual(actual, expected);
  const items = cloud.buildWordItems(actual),
    referenceItems = cloudReference.buildWordItems(expected);
...
test("Nezha command sends the original chart configuration to the existing provider", async () => {
  ...
  const ctx = {
    signal,
    storage: { json: () => ({ read: async () => ({ url: "https://nezha.example", secret: "fixture-secret" }) }) },
    http: {
      withResponse: async (url, options, read) => {
        const location = new URL(url);
        if (location.hostname === "quickchart.io") {
          chartBody = JSON.parse(options.body);
          return read(new Response(Buffer.from("png")), signal);
        }
        return read(
          Response.json({
            success: true,
            data: location.pathname.endsWith("/server") ? [{ id: 1, name: "Test Server" }] : data,
          }),
          signal,
        );
      },
  ...
  await plugin.commands.nezha.handle(
    { args: ["chart", "1"], prefix: ".", message: { id: 1, chatId: "1", text: ".nezha chart 1" } },
    ctx,
  );
  assert.equal(sent, 1);
  assert.deepEqual(chartBody, {
    chart: chartReference.generateChartConfig(data, "Test Server"),
    width: 800,
    height: 400,
