...
test("setup prunes expired and duplicate messages, caps history and preserves active groups and unknown fields", async t => {
  ...
  cache[peer(1)].msgs = [
    cached(9999, now, { custom: "message" }),
    cached(9999, now - 1),
    ...Array.from({ length: 3001 }, (_, i) => cached(i, now - i - 10)),
    cached(9000, now - 31 * 86400),
  ];
...
test("a burst across public groups settles durable bounded writes before immediate reload", async t => {
  ...
  assert.equal(Object.keys(saved.cache).length, 10);
  assert.equal(
    Object.values(saved.cache).reduce((total, chat) => total + chat.msgs.length, 0),
    10,
  );
  await f.host.load(create());
  await f.run(".fbi cache");
...
test("rebuild retains concurrent messages and uses the latest configured group limit", async t => {
  ...
  const f = await fixture(t, {
    cacheLimit: 12,
    cache: { [peer(1)]: group(1, now - 5) },
    client: {
      async getDialogs() {
        started();
        await gate;
        return Array.from({ length: 11 }, (_, i) => ({ id: peer(i + 1), isGroup: true }));
      },
...
test("a listener resolving a new group after rebuild keeps the rebuilt history", async t => {
  ...
  await running;
  assert.deepEqual(
    (await f.read()).cache[peer(1)].msgs.map(m => m.id),
    [100, 1],
  );
});
...
test("unload drains a cache write already submitted for atomic commit", async t => {
  ...
  t.mock.method(fs, "rename", async (source, destination) => {
    if (destination === path.join(f.root, "fbi", "cache.json")) {
      submitted();
      await gate;
    }
...
test("internal rebuild failures are logged without exposing transport details", async t => {
  const f = await fixture(t, {
    client: {
      async getDialogs() {
        const error = new Error("SECRET_RPC_TOKEN=do-not-render");
...
test("rebuild cancellation after getEntity settlement never starts history iteration", async t => {
  ...
  const f = await fixture(t, {
    client: {
      ...
      async getEntity() {
        entered();
        await gate;
        return { className: "Channel", username: "publicgroup" };
      },
      async *iterMessages() {
        iterations++;
        yield cached(1);
  ...
  assert.equal(
    f.edits.some(edit => /缓存重建完成/.test(edit.text)),
    false,
  );
});
