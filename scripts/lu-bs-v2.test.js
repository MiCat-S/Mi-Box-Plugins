...
const { artifactDir, manifest } = buildPlugin({
  id: "lu_bs",
  packageRoot: path.resolve(__dirname, "../lu_bs"),
  entry: "v2.ts",
});
...
function fixture(options = {}) {
  ...
  const client = {
    ...
    async deleteMessages(chat, ids) {
      deleted.push({ chat, ids });
      if (options.deleteMessages) return options.deleteMessages(chat, ids, controller);
    },
  ...
  const jsonStore = {
    read() {
      return tail.then(() => structuredClone(state));
    },
    update(mutator) {
  ...
  const context = {
    ...
    telegram: {
      async edit(target, text, settings) {
        edits.push({ target, text, settings });
      },
      async withClient(operation) {
        return operation(client, context.signal);
      },
    },
  };
  const plugin = create();
  return {
    ...
    run: (text, target = message()) =>
      plugin.commands.lu_bs.handle(
        {
          command: "lu_bs",
          prefix: ".",
...
test("setup migrates legacy state idempotently and preserves unknown fields", async () => {
  const f = fixture({
    state: {
      ...
      lastMessages: { 5: 12n, broken: "secret" },
      future: { kept: true },
    },
  });
  await f.setup();
...
test("Chinese aliases and unknown commands preserve the original command matrix", async () => {
  const f = fixture();
  await f.run(".lu_bs 订阅");
  assert.deepEqual(f.state().subscriptions, ["5"]);
  await f.run(".lu_bs 列表");
  assert.match(f.edits.at(-1).text, /已订阅/);
  await f.run(".lu_bs 退订");
  assert.deepEqual(f.state().subscriptions, []);
  await f.run(".lu_bs 帮助");
  assert.match(f.edits.at(-1).text, /鲁小迅整点报时/);
  await f.run(".lu_bs unknown");
  assert.match(f.edits.at(-1).text, /鲁小迅整点报时/);
});

test("reload aliases refresh the sticker set with fixed success and failure feedback", async () => {
  const ok = fixture();
  await ok.run(".lu_bs 重载");
  assert.match(ok.edits.at(-1).text, /重新加载成功/);
  assert.equal(ok.invokes.length, 1);
  const failed = fixture({
    stickerError: Object.assign(new Error("token=/private/path"), { name: "PrivateRpcError" }),
  });
  await failed.run(".lu_bs reload");
  assert.match(failed.edits.at(-1).text, /贴纸包加载失败/);
  assert.doesNotMatch(
    JSON.stringify({ edits: failed.edits, logs: failed.logs }),
    /PrivateRpcError|token=|private\/path/,
  );
});
...
test("hourly fan-out is bounded and one invalid chat does not affect others", async () => {
  ...
  const f = fixture({
    ...
    sendFile: async chat => {
      ...
      active -= 1;
      if (String(chat) === "-3")
        throw Object.assign(new Error("request failed"), { errorMessage: "CHAT_WRITE_FORBIDDEN" });
      return { id: 20 };
    },
  });
  await f.job();
  ...
  assert.ok(
    f.logs.some(
      entry => entry.event === "lu_bs_subscription_removed" && entry.fields.reason === "CHAT_WRITE_FORBIDDEN",
    ),
  );
...
test("cancellation while loading the sticker set starts no media send", async () => {
  ...
  const f = fixture({
    state: { schemaVersion: 1, subscriptions: ["5"], lastMessages: {} },
    invoke: async () => {
      await blocked;
  ...
  const running = f.job();
  while (!f.invokes.length) await new Promise(setImmediate);
  f.controller.abort(new Error("cancel load"));
  release();
  await assert.rejects(running, /cancel load/);
...
test("cancellation after deleting the previous post prevents replacement send", async () => {
  const f = fixture({
    state: { schemaVersion: 1, subscriptions: ["5"], lastMessages: { 5: 11 } },
    deleteMessages: async (_chat, _ids, controller) => controller.abort(new Error("cancel delete")),
  });
  await assert.rejects(f.job(), /cancel delete/);
  assert.equal(f.deleted.length, 1);
  assert.equal(f.sent.length, 0);
  assert.deepEqual(f.state().lastMessages, { 5: 11 });
});

test("cancellation after a successful media send prevents state persistence", async () => {
  const f = fixture({
    state: { schemaVersion: 1, subscriptions: ["-1009007199254740993"], lastMessages: {} },
    sendFile: async (_chat, _value) => {
      f.controller.abort(new Error("cancel send"));
      return { id: 22 };
    },
  });
  await assert.rejects(f.job(), /cancel send/);
  assert.equal(f.sent.length, 1);
  assert.deepEqual(f.state().lastMessages, {});
  assert.equal(
    f.logs.some(entry => JSON.stringify(entry).includes("cancel send")),
    false,
  );
});
