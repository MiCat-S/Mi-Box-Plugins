...
function fixture(initial) {
  ...
  const client = {
    async sendMessage(chat, value) {
      sent.push({ chat: String(chat), value });
    },
    async getEntity() {
      return { className: "Channel", megagroup: true, id: 7, title: "<group>" };
    },
    async *iterDialogs() {
      yield { isGroup: true, id: "-7" };
    },
  ...
  return {
    ...
    run: text =>
      plugin.commands.autorepeat.handle(
        {
          command: "autorepeat",
          prefix: ".",
          args: text.trim().split(/\s+/).filter(Boolean),
          message: { ...base, text: `.autorepeat ${text}` },
        },
        context,
      ),
...
test("migrates legacy lowdb shape and preserves exact large chat ids", async () => {
  const f = fixture({
    cache: { autorepeat_settings: [-1009007199254740993n, "-7"] },
    daily_history: { "-7": ["old"] },
    last_day_check: 2,
    trigger_config: { timeWindow: 12, minUsers: 3 },
  });
  await f.setup();
...
test("requires distinct users, repeats once per Shanghai day, and ignores loops/edits/bots", async () => {
  const f = fixture();
  await f.setup();
  await f.listen({ senderId: "1" });
  await f.listen({ senderId: "1" });
  assert.equal(f.sent.length, 0);
  await f.listen({ senderId: "2" });
  assert.equal(f.sent.length, 1);
  await f.listen({ senderId: "3" });
  assert.equal(f.sent.length, 1);
...
test("configuration validates bounds and group toggles are idempotent", async () => {
  ...
  await f.run("off");
  await f.run("off");
  assert.deepEqual(f.state().enabledGroups, []);
  await f.run("on");
  await f.run("on");
  assert.deepEqual(f.state().enabledGroups, ["-7"]);
  await f.run("alloff");
  assert.deepEqual(f.state().enabledGroups, []);
});
...
test("compiled listener metadata filters edited messages and unloads cleanly", async t => {
  ...
  await host.dispatchListeners({
    ...
    edited: true,
    text: "same",
    raw: { sender: { className: "User", bot: false } },
  });
