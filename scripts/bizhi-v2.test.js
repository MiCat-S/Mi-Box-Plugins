...
async function fixture(t, fetch, options = {}) {
  ...
  const edits = [], files = [], requests = [], errors = [];
  const host = new PluginHost({storageRoot: root, logger: {info() {}, error(event) { errors.push(event); }}, http: {fetch: async (url, init) => {
    ...
    async withClient(operation, signal) { return operation({async sendFile(peer, value) {
  ...
  return {edits, errors, files, host, requests, run: (text, message = {}) => host.dispatchPrimary({
    id: 1, chatId: '1', senderId: '1', outgoing: true, text, raw: {peerId: {}}, ...message,
  })};
}
...
test('bizhi downloads and sends a qualified Wallhaven image', async t => {
  ...
  assert.equal(tags.every(tag => ['anime', 'illustration', 'digital painting', 'Studio Ghibli', 'anime screenshot'].includes(tag)), true);
  if (search.searchParams.has('page')) assert.ok(Number(search.searchParams.get('page')) >= 1 && Number(search.searchParams.get('page')) <= 3);
...
test('bizhi retries a low-quality Wallhaven result at 2560x1440', async t => {
  ...
  const f = await fixture(t, async url => {
    if (url.hostname === 'wallhaven.cc') {
      searches++;
      if (searches === 1) return Response.json({data: [{
        id: 'low', path: 'https://w.wallhaven.cc/full/lo/wallhaven-low.jpg',
        dimension_x: 1920, dimension_y: 1080, file_size: 1024, file_type: 'image/jpeg',
      }]});
      assert.equal(url.searchParams.get('atleast'), '2560x1440');
      return Response.json({data: [{
        id: 'high', path: 'https://w.wallhaven.cc/full/hi/wallhaven-high.jpg',
        dimension_x: 3840, dimension_y: 2160, file_size: 5 * 1024 * 1024, file_type: 'image/jpeg',
      }]});
    }
    return new Response(Buffer.from([0xff, 0xd8, 0xff]), {headers: {'content-type': 'image/jpeg'}});
  });
...
test('bizhi treats command-message deletion as best effort after a successful send', async t => {
  ...
  await f.run('.bizhi', {raw: {peerId: {}, async delete() { throw new Error('private path'); }}});
  assert.equal(f.files.length, 1);
  assert.deepEqual(f.edits.map(value => value.text), ['正在获取高品质壁纸...']);
...
test('bizhi does not delete or send late feedback when unloaded during upload', async t => {
  ...
  const f = await fixture(t, async url => {
    ...
    return new Response(Buffer.from([0xff, 0xd8, 0xff]), {headers: {'content-type': 'image/jpeg'}});
  }, {sendFile: async () => { uploadStarted.resolve(); await uploadFinished.promise; }});
  let deleted = 0;
  const raw = {peerId: {}, async delete() { deleted++; }};
