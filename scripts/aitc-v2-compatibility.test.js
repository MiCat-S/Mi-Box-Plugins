...
async function fixture(t, options = {}) {
  ...
  if (options.ai !== null && options.ai !== false) { await fs.mkdir(path.join(root, 'ai')); await fs.writeFile(path.join(root, 'ai/config.json'), JSON.stringify(options.ai ?? aiConfig())); }
  if (options.legacyJson) { await fs.mkdir(path.join(root, 'aitc'), {recursive: true}); await fs.writeFile(path.join(root, 'aitc/config.json'), JSON.stringify(options.legacyJson)); }
  if (options.sqlite) {
    await fs.mkdir(path.join(root, 'aitc'), {recursive: true});
    const db = new Database(path.join(root, 'aitc/aitc_config.db'));
    db.exec('CREATE TABLE config (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)');
  ...
  let reply, response = options.response ?? {choices: [{message: {content: 'translated'}}]};
  const host = new PluginHost({storageRoot: root, logger: {info() {}, error() {}}, http: {fetch: async (url, init) => {
    ...
    async edit(message, text, opts) { edits.push({text, opts}); },
    async reply(message, text) { replies.push({text, message}); },
    async invoke() {}, async getReply() { return reply; },
  ...
  return {root, host, edits, replies, requests, read,
    setReply(value) { reply = value; }, setResponse(value) { response = value; },
...
test('AITC01 migrates the legacy SQLite prompt/presets/temp and provider, erasing the key exactly once', async t => {
  const f = await fixture(t, {ai: false, sqlite: {
    aitc_api_key: 'legacy-secret', aitc_api_url: 'https://legacy.example.test', aitc_model: 'legacy-model',
    aitc_prompt: 'legacy long prompt', aitc_prompts: JSON.stringify({casual: 'Use a casual voice'}), aitc_temperature: '0.7',
  }});
  ...
  assert.equal(inspect.prepare("SELECT value FROM config WHERE key = 'aitc_api_key'").get(), undefined, 'legacy key row erased');
  inspect.close();
...
test('AITC01 an existing V2 aiMigrated flag never skips an unprocessed SQLite key', async t => {
  const f = await fixture(t, {legacyJson: {prompt: 'v2 prompt', prompts: {}, temperature: 0.2, apiKey: '', apiUrl: '', model: '', aiMigrated: true},
...
test('AITC01 a failed SQLite erase keeps the secret and retries without duplicating the provider', async t => {
  const f = await fixture(t, {sqlite: {aitc_api_key: 'legacy-secret', aitc_api_url: 'https://legacy.example.test', aitc_model: 'legacy-model'},
    beforeAitc: async root => { await fs.chmod(path.join(root, 'aitc/aitc_config.db'), 0o444); }});
  ...
  assert.equal(before.prepare("SELECT value FROM config WHERE key = 'aitc_api_key'").get().value, 'legacy-secret', 'key retained for retry');
  before.close();
  ...
  const central = await f.read('ai/config.json');
  assert.deepEqual(Object.keys(central.configs).sort(), ['aitc', 'main'], 'the retry imports the provider without duplication');
});
...
test('AITC01 keeps the secret for retry when ai is unavailable and migrates non-secret settings', async t => {
  const f = await fixture(t, {ai: null, sqlite: {
    aitc_api_key: 'legacy-secret', aitc_api_url: 'https://legacy.example.test', aitc_model: 'legacy-model',
    aitc_prompt: 'legacy prompt', aitc_temperature: '0.9',
  }});
  ...
  assert.equal(retained.prepare("SELECT value FROM config WHERE key = 'aitc_api_key'").get().value, 'legacy-secret', 'the SQLite secret is retained until the central import succeeds');
  retained.close();
...
test('AITC01 a corrupt legacy DB is a retryable error, not a completed migration', async t => {
  ...
  const host = new PluginHost({storageRoot: root, logger: {info() {}, error() {}}, telegram: {
    async edit() {}, async reply() {}, async invoke() {}, async getReply() {}, async withClient() {},
  }});
...
test('AITC01 a completed migration never reads the legacy DB again', async t => {
  const f = await fixture(t, {ai: false, sqlite: {aitc_api_key: 'legacy-secret', aitc_api_url: 'https://legacy.example.test', aitc_model: 'legacy-model'}});
...
test('AITC01 a concurrent settings write during migration is preserved over the legacy value', async t => {
  ...
  const config = {apiKey: '', apiUrl: '', model: '', prompt: 'default prompt', prompts: {}, temperature: 0.9,
    aiMigrated: false, sqliteMigrated: false, providerMigrated: false};
  let latched = false;
  const context = {
    ...
    storage: {
      json: () => ({
        ...
        update: async mutator => {
          ...
          if (!latched) { latched = true; config.prompt = 'user concurrent prompt'; }
          Object.assign(config, await mutator({...config}));
          return {...config};
        },
      }),
      sqlite: () => ({read: async callback => callback({prepare: () => ({all: () => [{key: 'aitc_prompt', value: 'legacy prompt'}]})})}),
    },
...
test('AITC01 differing JSON and SQLite providers are both preserved in central ai', async t => {
  const f = await fixture(t, {legacyJson: {apiKey: 'json-key', apiUrl: 'https://json.example.test', model: 'json-model',
    prompt: 'json prompt', prompts: {}, temperature: 0.2, aiMigrated: false},
    sqlite: {aitc_api_key: 'sqlite-key', aitc_api_url: 'https://sqlite.example.test', aitc_model: 'sqlite-model'}});
  assert.equal((await f.read('aitc/config.json')).providerMigrated, true);
  const central = await f.read('ai/config.json');
  const byKey = Object.values(central.configs).filter(provider => provider.key === 'json-key' || provider.key === 'sqlite-key');
...
test('AITC01 does not create an empty legacy DB and prefers existing user settings', async t => {
  const f = await fixture(t, {legacyJson: {apiKey: '', apiUrl: '', model: '', prompt: 'my custom prompt', prompts: {mine: 'keep me'}, temperature: 0.4, aiMigrated: false}});
  assert.equal(await fs.stat(path.join(f.root, 'aitc/aitc_config.db')).then(() => true, () => false), false, 'no legacy DB is created');
  const local = await f.read('aitc/config.json');
...
test('AITC01 cancellation during the central import aborts and keeps the secret for retry', {timeout: 10_000}, async t => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'aitc-cancel-')));
  t.after(async () => { await fs.rm(root, {recursive: true, force: true}); });
  await fs.mkdir(path.join(root, 'aitc'), {recursive: true});
  await fs.writeFile(path.join(root, 'aitc/aitc_config.db'), 'legacy');
  const config = {apiKey: 'legacy-secret', apiUrl: 'https://legacy.example.test', model: 'legacy-model',
  ...
  const context = {
    ...
    files: {dataPath: (name = "aitc_config.db") => path.join(root, "aitc", name)},
    storage: {
      json: () => ({read: async () => config, update: async mutator => { Object.assign(config, await mutator({...config})); return config; }}),
      sqlite: () => ({read: async callback => callback({prepare: () => ({all: () => [
    ...
    services: {
      available: () => true,
      call: () => new Promise((_resolve, reject) => {
        enteredResolve();
...
test('AITC03 decodes entities, strips control characters and stays literal', async t => {
  const f = await fixture(t, {response: () => ({choices: [{message: {content: '&lt;b&gt;A&lt;/b&gt; &#65; &#x1F600;\r\n\u0007tail &#1114112; &#99999999999999999999;'}}]})});
  await f.run('.aitc hi');
...
test('AITC04 uses the original long default prompt and refuses reserved preset names', async t => {
  ...
  const after = await f.read('aitc/config.json');
  for (const name of ['_set_prompt', 'help', 'info', 'key', 'spn', 'aitc']) assert.equal(after.prompts[name], undefined);
...
test('AITC04 explicit JSON fields (even default-looking values) win over the legacy SQLite values', async t => {
  const f = await fixture(t, {legacyJson: {prompt: 'user prompt', prompts: {}, temperature: 0.2, apiKey: '', apiUrl: '', model: '', aiMigrated: false},
...
test('AITC04 legacy values apply only for fields absent from the JSON', async t => {
  const f = await fixture(t, {legacyJson: {prompts: {}, apiKey: '', apiUrl: '', model: '', aiMigrated: false},
...
test('AITC05 info paginates long prompts/presets and never shows secrets', async t => {
  ...
  const f = await fixture(t, {legacyJson: {prompt: 'P'.repeat(20_000), prompts, temperature: 0.2, apiKey: 'legacy-secret', apiUrl: '', model: '', aiMigrated: true}});
  ...
  for (let index = 0; index < 60; index++) assert.ok(all.includes(`preset${String(index).padStart(2, '0')}`), `preset${index} visible`);
