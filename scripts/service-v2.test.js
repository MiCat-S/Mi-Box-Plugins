...
const create = require(
  path.join(
    buildPlugin({ id: "service", packageRoot: path.resolve(__dirname, "../service"), entry: "v2.ts" }).artifactDir,
    "index.cjs",
  ),
).default;
...
function fixture(runProcess) {
  ...
  const context = {
    ...
    telegram: {
      edit: async (_m, text, options) => edits.push({ text, options }),
      reply: async (_m, text, options) => replies.push({ text, options }),
    },
    log: {
      error(event, fields) {
    ...
    processes: {
      async run(file, args, options) {
        ...
        return runProcess
          ...
          : result("Active: active (running) since Mon\n Main PID: 12\n Memory: 2M");
      },
    },
  };
  return {
    ...
    controller,
    run: args =>
      create().commands.service.handle(
        { message: { id: 1, chatId: "1", text: ".service", outgoing: true }, args, command: "service", prefix: "." },
        context,
      ),
  };
}
...
test("service rejects unsafe names and passes a valid unit only as an argument", async () => {
  const f = fixture();
  await f.run(["x;reboot"]);
  assert.equal(f.runs.length, 0);
  assert.match(f.edits[0].text, /非法字符/);
  await f.run(["--help"]);
  await f.run(["-H"]);
  assert.equal(f.runs.length, 0);
  await f.run(["demo@blue.service"]);
  assert.deepEqual(f.runs[0].args, ["--no-pager", "status", "--", "demo@blue.service"]);
  assert.equal(f.runs[0].file, "/usr/bin/systemctl");
});
...
test("service discovers the current mibot.service before legacy candidates", async () => {
  const f = fixture(async (file, args) => {
    ...
    if (args[0] === "status")
      throw Object.assign(new Error("pid not managed"), { stdout: Buffer.alloc(0), stderr: Buffer.alloc(0) });
    if (args[0] === "is-active") return result(args[1] === "mibot.service" ? "active\n" : "inactive\n");
    return result("Active: active (running)\n Main PID: 7");
  });
  await f.run([]);
  assert.deepEqual(
    f.runs.slice(0, 3).map(call => call.args),
    [
      ["-o", "unit=", "-p", String(process.pid)],
      ["status", String(process.pid)],
      ["is-active", "mibot.service"],
    ],
  );
...
test("service retains the PID status discovery path and translated result", async () => {
  ...
  await f.run([]);
  assert.deepEqual(f.runs.at(-1).args, ["--no-pager", "status", "--", "my.bot"]);
  assert.match(f.edits.at(-1).text, /my\.bot 服务详情/);
  assert.match(f.edits.at(-1).text, /活跃 \(运行中\)/);
...
test("service interprets inactive and missing units from failed process output", async () => {
  for (const [text, expected] of [
    ...
    ["Unit demo.service could not be found", "未找到"],
  ]) {
    const failure = Object.assign(new Error("exit"), { stdout: Buffer.from(text), stderr: Buffer.alloc(0) }),
      f = fixture(async () => {
        throw failure;
      });
    await f.run(["demo"]);
    assert.match(f.edits.at(-1).text, new RegExp(expected));
  }
});

test("service does not expose arbitrary process errors in output or logs", async () => {
  const failure = Object.assign(new Error("SECRET_MESSAGE"), { name: "SECRET_NAME", code: "SECRET_CODE" }),
    f = fixture(async () => {
      throw failure;
    });
  await f.run(["demo"]);
  assert.equal(f.edits.at(-1).text, "❌ 获取服务详情时发生错误，请稍后重试");
  assert.deepEqual(f.logs, [{ event: "service_status_failed", fields: { kind: "internal" } }]);
...
test("service paginates complete long status output within the SDK budget", async () => {
  ...
  for (let i = 0; i < 240; i++) assert.match(pages.join("\n"), new RegExp(`内存: ${i}M`));
});
...
test("service cancellation after an in-flight detection result starts no fallback process", async () => {
  ...
  const pending = f.run([]);
  await ready;
  f.controller.abort(new DOMException("stop", "AbortError"));
  release();
...
test("service incoming commands are rejected by the real Host before any process can start", async t => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "mibot-service-v2-"))),
    edits = [];
  ...
  assert.equal(
    await host.dispatchPrimary({ id: 1, chatId: "-1001", senderId: "77", outgoing: false, text: ".service demo" }),
    false,
  );
