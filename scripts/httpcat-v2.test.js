...
async function fixture(t, options = {}) {
  ...
  const edits = [], errors = [], sent = [], requests = [];
  const fetcher = options.fetch || (async () => new Response(new Uint8Array([1, 2, 3]), {status: 200}));
  const client = {sendFile: async (...args) => { sent.push(args); await options.sendFile?.(); }};
  const host = new PluginHost({storageRoot: root, prefixes: options.prefixes, logger: {info() {}, error(event) { errors.push(event); }}, http: {fetch: async (url, init) => {
  ...
  return {edits, errors, host, sent, requests, run: (text, extra = {}) => host.dispatchPrimary({...envelope, text, ...extra})};
}
test('httpcat validates code and sends bounded image through borrowed client', async t => {
  let deleted = 0;
  const f = await fixture(t);
  await f.run('.httpcat 404', {raw: {peerId: {id: 'peer'}, delete: async () => { deleted += 1; }}});
...
test('httpcat accepts short stream chunks and rejects empty or oversized media', async t => {
  ...
  const short = await fixture(t, {fetch: async () => new Response(new ReadableStream({pull(controller) {const chunk = chunks.shift(); if (chunk) controller.enqueue(chunk); else controller.close();}}))});
...
test('httpcat actively cancels and releases a hanging response reader on unload', async t => {
  ...
  assert.deepEqual(f.edits.map(item => item.text), ['正在获取 HTTP 200 猫猫图片...']);
});

test('httpcat treats deletion failure as best effort after a successful send', async t => {
  const f = await fixture(t);
  await f.run('.httpcat 418', {raw: {peerId: {}, delete: async () => {throw Object.assign(new Error('private'), {code: 'SECRET'});}}});
  assert.equal(f.sent.length, 1);
  assert.deepEqual(f.errors, ['httpcat.delete_failed']);
  assert.equal(f.edits.some(item => /失败/.test(item.text)), false);
});
...
test('httpcat cancellation during upload never deletes or emits late failure feedback', async t => {
  ...
  let deleted = 0;
  const f = await fixture(t, {sendFile: async () => {started.resolve(); await release.promise;}});
  const running = f.run('.httpcat 200', {raw: {peerId: {}, delete: async () => {deleted += 1;}}});
