...
test("portball applies bounded mute rights to the replied user", async t => {
  ...
  const client = {
    async getEntity(value) {
      return String(value) === "20" ? target : chat;
    },
    async getMe() {
      return me;
    },
    async getInputEntity() {
      return new Api.InputPeerUser({ userId: target.id, accessHash: 1 });
    },
    async invoke(request) {
        ...
        return {
          participant:
            request.participant instanceof Api.InputPeerSelf
              ? new Api.ChannelParticipantCreator({ userId: me.id })
              : new Api.ChannelParticipant({ userId: target.id, date: 0 }),
        };
      return {};
    },
  ...
  const f = await fixture(t, "portball", {
    client,
    reply: { senderId: "20" },
    message: {
      replyToId: 7,
...
test("isalive renders escaped identity, online state and last message", async t => {
  const user = new Api.User({
    ...
    username: "alice",
    premium: true,
    status: new Api.UserStatusOnline({ expires: 0 }),
  });
  const client = {
    async getEntity() {
      return user;
