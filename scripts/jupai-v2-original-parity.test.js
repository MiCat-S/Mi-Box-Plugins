...
const PNG = sharp({ create: { width: 2, height: 2, channels: 4, background: "#ff0000ff" } })
  .png()
  .toBuffer();
...
function context(options = {}) {
  ...
  const raw = {
    ...
    async delete() {
      options.deletes?.();
      if (options.deleteError) throw new Error("private delete failure");
    },
  };
  const ctx = {
    ...
    http: {
      withResponse: async (url, _init, consume, requestOptions) => {
    ...
    telegram: {
      ...
      async withClient(operation) {
        return operation(
          {
            async sendFile(peer, upload) {
              sends.push({ peer, upload });
  ...
  const run = (args, extra = {}) =>
    create().commands.jupai.handle(
      {
        command: "jupai",
        prefix: "!",
        args,
        message: { id: 7, chatId: "9007199254740993", outgoing: true, text: `!jupai ${args.join(" ")}`, raw, ...extra },
      },
      ctx,
    );
...
test("preserves validated JPEG bytes and resolves large fallback chat IDs to real TL peers", async () => {
  const jpeg = await sharp({ create: { width: 3, height: 2, channels: 3, background: "#00ff00" } })
    .jpeg({ quality: 83 })
    .toBuffer();
