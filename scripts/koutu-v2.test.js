...
const built = buildPlugin({
  id: "koutu",
  packageRoot: process.env.KOUTU_TEST_SOURCE || path.resolve(__dirname, "../koutu"),
  entry: "v2.ts",
});
...
async function fixture(t, options = {}) {
  ...
  const client = {
    async *iterDownload(_source, params) {
      downloads++;
      if (options.iterDownload) yield* options.iterDownload(params);
      else yield options.input ?? jpeg;
    },
    async getInputEntity(target) {
      return options.getInputEntity ? options.getInputEntity(target) : target;
    },
  ...
  const host = new PluginHost({
    ...
    http: {
      fetch: async (url, init) => {
        ...
        const response = options.respond
          ? await options.respond(requests.length, webp, init)
    ...
    telegram: {
      ...
      async withClient(use, signal) {
        return use(client, signal);
      },
    },
  });
  ...
  const run = (text, extra = {}) =>
    host.dispatchPrimary({
      ...
      raw: {
        ...
        async delete() {
          deletes++;
          if (options.cleanupFails) throw new Error("private failure");
        },
      },
      topicId: 9,
      ...extra,
...
test("koutu restricts key commands, masks settings and routes help without API calls", async t => {
  ...
  await f.run(".koutu set key private-key");
  assert.match(f.edits.at(-1), /收藏夹/);
  await f.configure();
  assert.doesNotMatch(f.edits.at(-1), /test-key/);
  ...
  assert.equal(
    JSON.parse(await fs.readFile(path.join(f.root, "assets/koutu/config.json"), "utf8")).apiKey,
    "replacement-key",
  );
  await f.run(".koutu help");
  assert.match(f.edits.at(-1), /picupapi/);
  await f.run(".koutu typo");
...
test("koutu sends transparent WebP in the reply topic and cleanup failure is not a business failure", async t => {
  ...
  assert.equal(f.requests[0].init.headers.apikey, "test-key");
  assert.match(f.requests[0].url, /^https:\/\/picupapi.tukeli.net\/api\/v1\/matting\?/);
  assert.equal(f.sent.length, 1);
  assert.equal(f.sent[0].replyTo, 5);
  assert.equal(f.sent[0].topMsgId, 9);
  ...
  assert.ok(!f.logs.includes("koutu_failed"));
  assert.ok(f.responses.every(value => !value.body.locked));
...
test("koutu retries type rejection exactly once as PNG and hides provider errors", async t => {
  ...
  const retry = await fixture(t, {
    respond: () => new Response('{"code":5013}', { headers: { "content-type": "application/json" } }),
  });
  await retry.configure();
  await retry.run(".koutu");
...
test("koutu rejects TGS, oversized media and oversized pixels before HTTP", async t => {
  for (const scenario of ["tgs", "size", "pixels"]) {
    const f = await fixture(t, {
      input:
        scenario === "pixels"
          ? Buffer.from('<svg width="5000" height="5000" xmlns="http://www.w3.org/2000/svg"/>')
          : undefined,
    });
    if (scenario !== "pixels")
      f.raw.document = {
        size: scenario === "size" ? 20971521n : 1n,
...
test("koutu uses self avatar without a reply and sender input entity for text replies", async t => {
  const f = await fixture(t, { reply: false });
  await f.configure();
  await f.run(".koutu");
  assert.equal(f.targets[0].className, "InputPeerSelf");
  assert.equal(f.sent.length, 1);
  const g = await fixture(t);
  delete g.raw.photo;
  delete g.raw.media;
  g.raw.senderId = 9007199254740995n;
  g.raw.getInputSender = async () => "exact-sender-peer";
  await g.configure();
  await g.run(".koutu");
  assert.equal(g.targets[0], "exact-sender-peer");
  ...
  empty.raw.senderId = 9007199254740995n;
  empty.raw.getInputSender = async () => "empty-media-sender";
  await empty.configure();
  await empty.run(".koutu");
  assert.equal(empty.targets[0], "empty-media-sender");
  const avatar = empty.requests[0].init.body.get("file");
  assert.equal(avatar.name, "avatar.jpg");
...
test("koutu response consumer releases the reader on cancellation and byte overflow", async () => {
  ...
  const pending = responseBytes(response, controller.signal);
  controller.abort();
    ...
    videoFrame(
      {
        processes: {
          async run() {
            count++;
            return { stdout: Buffer.from('{"streams":[{"width":10000,"height":10000}]}') };
          },
        },
      },
      "input",
...
test("koutu uploads PNG and WebP documents with their original names, MIME and bytes", async t => {
  const original = await sharp({ create: { width: 4, height: 4, channels: 4, background: "#ffffff80" } })
    .png()
    .toBuffer();
  for (const format of ["png", "webp"]) {
    ...
    f.raw.document = {
      ...
      attributes: [{ fileName: `原图.${format}` }],
    };
    await f.configure();
    await f.run(".koutu");
    assert.equal(f.sent.length, 1);
    const file = f.requests[0].init.body.get("file");
    assert.equal(file.name, `原图.${format}`);
...
test("koutu GIF, MP4 and WebM selection produces an original-size PNG first frame", async t => {
  ...
  const png = await sharp({ create: { width: 3000, height: 2, channels: 4, background: "#abcdef80" } })
    .png()
    .toBuffer();
  for (const extension of ["gif", "mp4", "webm"]) {
          ...
          return {
            raw: {
              document: {
                ...
                mimeType: "application/octet-stream",
                attributes: [{ fileName: `动画.${extension}` }],
              },
            },
          };
        },
    ...
    assert.equal(selected.mime, "image/png");
    assert.equal(selected.filename, "frame.png");
...
test("koutu unload cancels HTTP, prevents late sends and drains the temporary directory", async t => {
  ...
  const f = await fixture(t, {
    respond: async (_index, _output, init) => {
      ready();
  ...
  await f.configure();
  const pending = f.run(".koutu");
  await started;
  await f.run(".koutu", { chatId: "2" });
  assert.match(f.edits.at(-1), /已有抠图任务/);
  const report = await f.host.unload("koutu", 2000);
  await pending;
...
test("koutu unload aborts a hanging media download through its combined signal", async t => {
  ...
  const pending = f.run(".koutu");
  await ready;
  assert.equal((await f.host.unload("koutu", 2000)).completed, true);
  await pending;
  assert.equal(f.requests.length, 0);
  assert.equal(f.sent.length, 0);
...
test("koutu cancellation during avatar target resolution prevents profile download", async t => {
  ...
  await f.configure();
  const pending = f.run(".koutu", { raw: { peerId: "peer", fromId: { className: "PeerUser" } } });
  await ready;
  const unloading = f.host.unload("koutu", 2000);
  release();
...
test("koutu cancellation during upload prevents receipt deletion and late feedback", async t => {
  ...
  const before = f.edits.length,
    unloading = f.host.unload("koutu", 2000);
  release();
  assert.equal((await unloading).completed, true);
  await pending;
  assert.equal(f.deletes, 0);
