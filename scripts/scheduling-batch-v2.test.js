...
async function fixture(t, id, { reply, initial, file, now } = {}) {
  ...
  if (initial) {
    ...
    await fs.writeFile(path.join(root, id, file), JSON.stringify(initial));
  }
  const edits = [],
    sends = [],
    invokes = [];
  const client = {
    async getMe() {
      return { id: 7, firstName: "Alice 12:30", lastName: "User" };
    },
    async getEntity(value) {
      return new Api.User({ id: returnBigInt(value === "me" ? 7 : value), firstName: "Target" });
    },
  ...
  const run = (text, extra = {}) =>
    host.dispatchPrimary({ id: 1, chatId: "7", senderId: "7", outgoing: true, text, ...extra });
  return {
    root,
    host,
    edits,
    ...
    invokes,
    run,
    read: async name => JSON.parse(await fs.readFile(path.join(root, id, name), "utf8")),
  };
}
...
test("acron imports legacy tasks and manages a dynamically registered send task", async t => {
  ...
  const f = await fixture(t, "acron", {
    initial,
    file: "acron_config.json",
    reply: { id: 4, chatId: "7", text: "scheduled" },
  });
  assert.equal(f.host.snapshot().jobs.jobs, 0);
