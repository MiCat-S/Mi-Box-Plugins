...
test.before(async () => {
  const built = buildPlugin({
    id: "copy_sticker_set",
    packageRoot: path.resolve(__dirname, "../copy_sticker_set"),
    entry: "v2.ts",
  });
...
async function fixture(t, options = {}) {
  ...
  const createResult =
    options.createResult === undefined
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async invoke() {
        assert.fail("unexpected direct invoke");
      },
      async getReply() {
  ...
  const run = text =>
    host.dispatchPrimary({ id: 1, chatId: "123", senderId: "9007199254740993", outgoing: true, text });
...
test("copy_sticker_set keeps known create errors understandable and unknown details private", async t => {
  ...
  for (const [code, expected] of cases) {
    ...
    assert.equal(
      f.logs.every(entry => entry.fields === undefined),
      true,
    );
  }
});
...
test("copy_sticker_set warns at sixty seconds without settling the in-flight RPC", async t => {
  ...
  let settled = false;
  const running = f.run(".css source").finally(() => {
    settled = true;
  });
  await started;
...
test("copy_sticker_set waits for an in-flight timeout edit before final success", async t => {
  ...
  let releaseNotice;
  const noticeGate = new Promise(resolve => {
  ...
  const finalOrder = f.completedEdits
    .filter(entry => /操作仍在等待服务器确认|复制完成/.test(entry.text))
...
test("copy_sticker_set absorbs a failed timeout notification before final success", async t => {
  ...
  assert.match(f.edits.at(-1).text, /复制完成/);
  assert.deepEqual(
    f.logs.filter(entry => entry.level === "error"),
...
test("copy_sticker_set clears timeout notice after timely success and failure", async t => {
  ...
  await Promise.resolve();
  assert.equal(
    success.edits.some(entry => /操作仍在等待服务器确认/.test(entry.text)),
    false,
  );
  assert.equal(
    failure.edits.some(entry => /操作仍在等待服务器确认/.test(entry.text)),
    false,
  );
});
...
test("copy_sticker_set waits for an in-flight RPC and suppresses success after unload", async t => {
  ...
  const unloading = f.host.unload("copy_sticker_set", 120_000).finally(() => {
    unloadSettled = true;
  });
  await new Promise(resolve => setImmediate(resolve));
  assert.equal(
    f.signals.some(signal => signal.aborted),
    true,
  );
  ...
  await Promise.resolve();
  assert.equal(
    f.edits.some(entry => /操作仍在等待服务器确认/.test(entry.text)),
    false,
  );
  assert.equal(unloadSettled, false);
  release();
  ...
  assert.equal(
    f.edits.some(entry => /复制完成/.test(entry.text)),
    false,
  );
});
