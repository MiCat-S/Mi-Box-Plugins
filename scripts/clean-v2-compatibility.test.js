...
const create = require(
  path.join(
    buildPlugin({ id: "clean", packageRoot: path.resolve(__dirname, "../clean"), entry: "v2.ts" }).artifactDir,
    "index.cjs",
  ),
).default;
...
function fixture(client) {
  ...
  const run = args =>
    create().commands.clean.handle(
      { message: message(`.clean ${args.join(" ")}`), args, prefix: ".", command: "clean" },
      ctx,
    );
...
async function realHostFixture(t, client) {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "clean-cancel-"))),
    edits = [];
...
test("CLEAN-COMPAT-00 command help is complete and unknown top-level input is identified", async () => {
  const f = fixture({});
  await f.run([]);
  for (const section of ["功能概述", "命令列表", "智能清理模式", "数据统计", "权限要求"])
    assert.match(f.edits.at(-1), new RegExp(section));
  await f.run(["unknown<&"]);
...
test("CLEAN-COMPAT-01 a primary dialog scan failure is reported without leaking details", async () => {
  ...
  const f = fixture({
    async *iterDialogs({ folder }) {
      if (folder === 0) throw new Error(secret);
    },
  });
  await f.run(["deleted", "pm"]);
  assert.equal(f.edits.at(-1), "❌ <b>操作失败:</b> 未知错误");
  assert.deepEqual(f.logs, [
    { level: "error", event: "clean:command_failed", fields: { chatId: "-1009007199254740993", messageId: 19 } },
  ]);
...
test("CLEAN-COMPAT-02 an archive scan failure remains non-fatal and sanitized", async () => {
  ...
  const f = fixture({
    async *iterDialogs({ folder }) {
      if (folder === 1) throw new Error("private archive token");
  ...
  assert.deepEqual(f.logs, [
    { level: "error", event: "clean:archive_scan_failed", fields: { chatId: "-1009007199254740993", messageId: 19 } },
  ]);
...
function blockedClient({ targets = [], deleteError } = {}) {
  ...
  return {
    ...
    calls,
    async getEntity() {
      return { className: "Channel", id: 7n, megagroup: true };
    },
    ...
    async invoke(request) {
      ...
      calls.push(name);
      if (name.includes("GetParticipants"))
        return {
          users: targets.map(item => item.user).filter(Boolean),
          participants: targets.map(item => item.participant),
        };
...
test("CLEAN-COMPAT-03 an empty blocked-member result keeps the original managed three-second cleanup", async t => {
  ...
  assert.deepEqual(
    client.deleted.map(item => ({ ids: item.ids, options: item.options })),
    [{ ids: [19], options: { revoke: true } }],
  );
});
...
test("CLEAN-COMPAT-04 a completed unblock keeps the original managed five-second cleanup", async t => {
  ...
  const client = blockedClient({ targets }),
    f = fixture(client);
  await f.run(["blocked", "member"]);
...
test("CLEAN-COMPAT-05 receipt cleanup failures expose only fixed event and safe ids", async t => {
  ...
  const client = blockedClient({ deleteError: "private-delete-secret" }),
    f = fixture(client);
  await f.run(["blocked", "member"]);
  await f.tasks[0].fn(f.ctx.signal);
  assert.deepEqual(f.logs, [
    {
      level: "error",
      event: "clean:receipt_cleanup_failed",
...
test("CLEAN-COMPAT-06 smart blocked-PM cleanup skips risky accounts and reports totals", async t => {
  ...
  const client = {
    async invoke(request) {
      ...
      if (name.includes("Unblock")) {
        unblocked.push(String(request.id.id));
        return {};
      }
...
test("CLEAN-COMPAT-07 unloading cancels a pending receipt cleanup before deletion", async t => {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "clean-compat-"))),
    deleted = [];
  const client = blockedClient();
  client.deleteMessages = async (...args) => {
    deleted.push(args);
  };
  const host = new PluginHost({
    storageRoot: root,
...
test("CLEAN-CANCEL-01 cancellation after the first deleted-member ban performs no unban", async t => {
  ...
  const client = {
    ...
    async *iterParticipants() {
      yield { className: "User", deleted: true, id: 2n };
    },
    async invoke(request) {
      const name = request.className ?? request.constructor.name;
      if (name.includes("GetParticipant")) return { participant: { className: "ChannelParticipantCreator" } };
      if (name.includes("EditBanned")) {
        edits++;
        if (edits === 1) {
          entered.resolve();
  ...
  const f = await realHostFixture(t, client),
    running = f.dispatch(".clean deleted member rm");
  await entered.promise;
  const unloading = f.host.unload("clean", 1000);
  release.resolve();
  await running;
...
test("CLEAN-CANCEL-02 cancellation after a blocked-PM page performs no next-page request", async t => {
    ...
    async invoke(request) {
      ...
      if (name.includes("GetBlocked")) {
        ...
        return {
          ...
          count: 200,
          users: Array.from({ length: 100 }, (_, id) => ({ id: BigInt(id + 1) })),
        };
      }
      if (name.includes("Unblock")) {
        unblocks++;
  ...
  const f = await realHostFixture(t, client),
    running = f.dispatch(".clean blocked pm");
  await entered.promise;
  const unloading = f.host.unload("clean", 1000);
  release.resolve();
  await running;
...
test("CLEAN-CANCEL-03 cancellation after a blocked-member page performs no next-page request", async t => {
  ...
  const client = {
    ...
    async invoke(request) {
      ...
      if (name.includes("GetParticipants")) {
        ...
        entered.resolve();
        await release.promise;
        return { users: [], participants: Array.from({ length: 200 }, (_, id) => participant(id + 1)) };
      }
      if (name.includes("GetParticipant")) return { participant: { className: "ChannelParticipantCreator" } };
      throw new Error(`unexpected ${name}`);
    },
  ...
  const f = await realHostFixture(t, client),
    running = f.dispatch(".clean blocked member");
  await entered.promise;
  const unloading = f.host.unload("clean", 1000);
  release.resolve();
  await running;
...
test("CLEAN-CANCEL-04 archive cancellation performs no deleted-dialog removal", async t => {
  ...
  const client = {
    async *iterDialogs({ folder }) {
      if (folder === 0) {
        yield { isUser: true, entity: { className: "User", deleted: true, id: 2n }, inputEntity: "u" };
        return;
      }
      entered.resolve();
      await release.promise;
  ...
  const f = await realHostFixture(t, client),
    running = f.dispatch(".clean deleted pm rm");
  await entered.promise;
  const unloading = f.host.unload("clean", 1000);
  release.resolve();
  await running;
