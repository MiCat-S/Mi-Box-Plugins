...
function game(overrides = {}) {
  return {
    title: 'Control <Ultimate>', description: 'desc & details',
    categories: [{path: 'freegames'}], offerMappings: [{pageSlug: 'control'}],
    price: {totalPrice: {discountPrice: 0, fmtPrice: {originalPrice: '¥198'}}},
    promotions: {promotionalOffers: [{promotionalOffers: [{startDate: '2026-09-10T03:00:00Z', endDate: '2026-09-17T03:00:00Z'}]}]},
    ...overrides,
  };
}
...
async function fixture(t, {data = payload([game()]), fetch, prefixes = ['.'], onReply} = {}) {
  ...
  const host = new PluginHost({
    storageRoot: root, prefixes,
    logger: {info(event, fields) { logs.push({level: 'info', event, fields}); }, error(event, fields) { logs.push({level: 'error', event, fields}); }},
    http: {fetch: fetch ?? (async () => new Response(JSON.stringify(data), {status: 200, headers: {'content-type': 'application/json'}}))},
    telegram: {
      async edit(message, text, options) { edits.push({message, text, options}); },
      async reply(message, text, options) { replies.push({message, text, options}); await onReply?.(replies.length, text); },
      async invoke() { assert.fail('unexpected invoke'); }, async getReply() {}, async withClient() { assert.fail('unexpected native client'); },
    },
  });
  await host.load(createEpic());
  let closed = false;
  t.after(async () => { if (!closed) assert.equal((await host.shutdown(1000)).completed, true); await fs.rm(root, {recursive: true, force: true}); });
  return {host, edits, replies, logs, close: async timeout => { const result = await host.shutdown(timeout); closed = result.completed; return result; },
...
test('epic preserves original current-free-game selection, fallbacks and presentation', async t => {
  ...
  const current = game({description: long, offerMappings: [], catalogNs: {mappings: [{pageSlug: 'fallback-game'}]}});
  const upcoming = game({title: 'Upcoming', price: {totalPrice: {discountPrice: 1999}}, promotions: {upcomingPromotionalOffers: [{promotionalOffers: [{startDate: '2026-10-01', endDate: '2026-10-08', discountSetting: {discountPercentage: 0}}]}]}});
...
test('epic retains every game using complete valid HTML pagination', async t => {
  const items = Array.from({length: 45}, (_, index) => game({title: `game-${index}`, description: `<&😀-${index}-`.repeat(20), offerMappings: [{pageSlug: `game-${index}`}] }));
  ...
  for (const page of pages) {
    ...
    for (const tag of ['a', 'b', 'code']) assert.equal((page.match(new RegExp(`<${tag}(?:\\s[^>]+)?>`, 'g')) || []).length, (page.match(new RegExp(`</${tag}>`, 'g')) || []).length);
  }
});
...
test('epic reports partial pagination safely and does not overwrite published pages', async t => {
  ...
  const items = Array.from({length: 45}, (_, index) => game({title: `game-${index}`, description: 'z'.repeat(100)}));
  const f = await fixture(t, {data: payload(items), onReply(count) { if (count === 1) throw new Error(secret); }});
  await f.run('.epic');
...
test('epic cancellation aborts managed HTTP and emits no failure receipt', async t => {
  ...
  const f = await fixture(t, {fetch: async (_url, init) => {
    requested = true;
    return new Promise((_, reject) => init.signal.addEventListener('abort', () => { aborted = true; reject(init.signal.reason); }, {once: true}));
  }});
...
test('epic consumes a real Host-scoped HTTP response and keeps errors private', async t => {
  ...
  const f = await fixture(t, {fetch: async (url, init) => {
    assert.equal(url.toString(), 'https://store-site-backend-static-ipv4.ak.epicgames.com/freeGamesPromotions?locale=zh-CN&country=CN&allowCountries=CN');
    assert.equal(init.method, 'GET');
    assert.equal(init.credentials, 'omit');
    assert.equal(init.redirect, 'manual');
    return new Response(new ReadableStream({pull(controller) { pulls += 1; controller.enqueue(new TextEncoder().encode(body)); controller.close(); }}), {status: 200});
  }});
  await f.run('.epic');
  assert.ok(pulls > 0);
  const secret = 'https://private.example/?token=secret /Users/private/config';
  const failed = await fixture(t, {fetch: async () => { throw new Error(secret); }});
...
test('epic cancellation stops a hanging response body read', async t => {
  let requested = false, cancelStarted = false, releaseCancel;
  const cancelGate = new Promise(resolve => { releaseCancel = resolve; });
  const f = await fixture(t, {fetch: async () => {
    requested = true;
    return new Response(new ReadableStream({pull() {}, async cancel() { cancelStarted = true; await cancelGate; }}), {status: 200});
  }});
  const running = f.run('.epic');
  while (!requested) await new Promise(resolve => setImmediate(resolve));
  let shutdownFinished = false;
  const shutdown = f.close(1000).then(result => { shutdownFinished = true; return result; });
