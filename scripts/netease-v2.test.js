...
const { artifactDir } = buildPlugin({
  id: "netease",
  packageRoot: path.resolve(__dirname, "../netease"),
  entry: "v2.ts",
});
...
async function fixture(t, { client: patch = {}, deleteCommand, httpFetch } = {}) {
  ...
  const client = {
    ...
    async getInputEntity() {
      return new Api.InputPeerUser({ userId: returnBigInt(163), accessHash: returnBigInt(1) });
    },
    ...
    async getMessages() {
      return messageReads++
        ? [{ id: 9, out: false, date: Math.floor(Date.now() / 1000), media, message: "Song via @Music163bot" }]
        : [];
    },
  ...
  const host = new PluginHost({
    ...
    http: { fetch: httpFetch || (async () => assert.fail("netease has no HTTP path")) },
    telegram: {
...
test("netease ignores same-second media at or below the pre-request message ID watermark", async t => {
  let reads = 0;
  const staleMedia = { kind: "stale" },
    freshMedia = { kind: "fresh" };
  ...
  const f = await fixture(t, {
    client: {
      async getMessages() {
        return reads++ < 2 ? [stale] : [fresh, stale];
      },
    },
  });
...
test("netease serializes concurrent chats so each query receives its own bot media", async t => {
  ...
  const f = await fixture(t, {
    client: {
      async sendMessage(peer, value) {
        if (/^\/(?:search|music) /.test(value.message)) active = value.message.split(" ").slice(1).join(" ");
        return { id: ++nextId, out: true };
      },
      async getMessages() {
        if (active) {
          history.unshift({ id: ++nextId, out: false, date: 1, media: { query: active }, message: `Song ${active}` });
          active = undefined;
        }
  ...
  assert.deepEqual(
    f.files.map(item => item.value.file.query),
    ["alpha", "beta"],
  );
  assert.deepEqual(
    f.files.map(item => item.value.caption),
...
test("netease follows start and command fallbacks and clicks a search result", async t => {
  ...
  const f = await fixture(t, {
    client: {
      async invoke(request) {
        if (request instanceof Api.messages.StartBot) throw new Error("not started");
        return {};
      },
      async sendMessage(peer, value) {
      ...
      async getMessages() {
        if (++reads === 1) return [];
        return clicks ? [{ id: 9, out: false, date: button.date, media, message: "" }] : [button];
      },
    },
  });
  await f.run(".netease query");
...
test("netease keeps a successful upload successful when command cleanup fails", async t => {
  const f = await fixture(t, {
    deleteCommand: async () => {
      throw Object.assign(new Error("SECRET_DELETE"), { name: "SECRET_NAME" });
    },
  });
  await f.run(".netease 123");
  assert.equal(f.files.length, 1);
  assert.equal(
    f.edits.some(item => /获取失败/.test(item.text)),
    false,
  );
