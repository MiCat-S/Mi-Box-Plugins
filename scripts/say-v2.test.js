...
test('say cancels a hung response reader and waits for underlying cleanup', async t => {
  let ready, release, cancelled = 0; const started = new Promise(r => {ready = r;}); const gate = new Promise(r => {release = r;});
  const f = await fixture(t, () => new Response(new ReadableStream({start() {ready();}, cancel() {cancelled++; return gate;}})));
  await f.run('.say key fish token', {saved: true}); const running = f.run('.say pending'); await started;
  const unloading = f.host.unload('say', 1000); await new Promise(r => setTimeout(r, 0)); assert.equal(cancelled, 1); release();
  assert.equal((await unloading).completed, true); await running; assert.equal(f.sent.length, 0); assert.equal(f.deleted.length, 0);
});

test('say never exposes arbitrary provider diagnostics', async t => {
  const secret = 'PRIVATE_PROVIDER_SECRET'; const f = await fixture(t, () => {throw Object.assign(new Error(secret), {name: secret});});
  await f.run('.say key fish token', {saved: true}); await f.run('.say hello');
  assert.match(f.edits.at(-1).text, /语音操作失败，请检查配置或稍后重试/);
  assert.doesNotMatch(JSON.stringify(f.edits), new RegExp(secret));
});

test('say captions stay within Telegram HTML bounds with closed entities',async t=>{const f=await fixture(t);await f.run('.say key volc token',{saved:true});await f.run(`.say ${'<&🙂'.repeat(260)}`);const value=f.sent[0].options.caption;assert.ok(value.length<=1024);assert.doesNotThrow(()=>HTMLParser.parse(value));assert.equal((value.match(/<blockquote>/g)||[]).length,(value.match(/<\/blockquote>/g)||[]).length);});
...
test('say converts MiMo and Fish audio through managed FFmpeg while the scoped file exists', async t => {
  ...
  for (const provider of ['mimo', 'fish']) {
    ...
    const conversion=calls.find(value=>value.args.includes('libopus'));assert.equal(conversion.options.cwd,scoped);assert.equal(conversion.args[conversion.args.indexOf('-protocol_whitelist')+1],'file');assert.deepEqual(conversion.args.slice(-3,-1),['-fs',String(32*1024*1024)]);
    await assert.rejects(fs.stat(scoped), {code: 'ENOENT'});
  }
});

test('say delivers complete short and long translations outside the bounded caption',async t=>{const root=await fs.mkdtemp(path.join(core,'temp/say-translation-'));t.after(()=>fs.rm(root,{recursive:true,force:true}));const output=path.join(root,'media.cjs');esbuild.buildSync({entryPoints:[path.resolve(__dirname,'../say/v2/media.ts')],outfile:output,bundle:true,platform:'node',format:'cjs',packages:'external'});const {sendVoice}=require(output),controller=new AbortController(),replies=[],sent=[],long=`LONG_${'译<&🙂'.repeat(900)}_END`;const cfg={schemaVersion:1,primary:'volc',speed:1,style:'',translate:true,chats:{},providers:{mimo:{apiKey:'',voice:'',endpoint:'standard'},volc:{apiKey:'k',resourceId:'r',voice:'v'},fish:{apiKey:'',voice:''}}};const ctx={signal:controller.signal,log:{error(){}},http:{withResponse:async(_u,_i,consume)=>consume(new Response(JSON.stringify({code:0,data:Buffer.from('OggS').toString('base64')})),controller.signal)},services:{available:()=>true,call:async(_p,_s,input)=>input.target==='en'?'short translation':long},files:{withTemp:async use=>use(root,controller.signal)},telegram:{withClient:async op=>op({sendFile:async(_p,v)=>{sent.push(v);return{};}},controller.signal),reply:async(_m,text)=>replies.push(text)}};await sendVoice(ctx,{message:{raw:{peerId:'p'},text:'短原文',chatId:'1',id:1,outgoing:true}},'短原文',cfg,async()=>{});assert.ok(sent[0].caption.length<=1024);const joined=replies.join('');assert.match(joined,/short translation/);assert.match(joined,/LONG_/);assert.match(joined,/_END/);assert.equal((joined.match(/译/g)||[]).length,2700);});

test('say propagates upload cancellation but contains cleanup failure after a sent voice',async t=>{const root=await fs.mkdtemp(path.join(core,'temp/say-gates-'));t.after(()=>fs.rm(root,{recursive:true,force:true}));const output=path.join(root,'media.cjs');esbuild.buildSync({entryPoints:[path.resolve(__dirname,'../say/v2/media.ts')],outfile:output,bundle:true,platform:'node',format:'cjs',packages:'external'});const {sendVoice}=require(output),cfg={schemaVersion:1,primary:'volc',speed:1,style:'',translate:false,chats:{},providers:{mimo:{apiKey:'',voice:'',endpoint:'standard'},volc:{apiKey:'k',resourceId:'r',voice:'v'},fish:{apiKey:'',voice:''}}},make=(controller,cleanup=false)=>{const logs=[];return[{signal:controller.signal,log:{error:e=>logs.push(e)},http:{withResponse:async(_u,_i,c)=>c(new Response(JSON.stringify({code:0,data:Buffer.from('OggS').toString('base64')})),controller.signal)},services:{available:()=>false},files:{withTemp:async use=>{const value=await use(root,controller.signal);if(cleanup)throw new Error('cleanup');return value;}},telegram:{withClient:async op=>op({sendFile:async()=>{if(!cleanup)controller.abort();return{};}},controller.signal)}},logs];};let c=new AbortController(),[ctx]=make(c);await assert.rejects(sendVoice(ctx,{message:{raw:{peerId:'p'}}},'text',cfg,async()=>{}));await fs.rm(path.join(root,'voice.ogg'),{force:true});c=new AbortController();const pair=make(c,true);assert.ok(await sendVoice(pair[0],{message:{raw:{peerId:'p'}}},'text',cfg,async()=>{}));assert.deepEqual(pair[1],['say_temp_cleanup_failed']);});
