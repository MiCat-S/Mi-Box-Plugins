...
function memoryStorage(initial = {}) {
  ...
  return {
    json(file, defaults) {
      if (!values.has(file)) values.set(file, structuredClone(defaults));
      return {
        async read() {
          return structuredClone(values.get(file));
        },
        async update(change) {
          const next = change(structuredClone(values.get(file)));
          values.set(file, structuredClone(next));
    ...
    sqlite() {
      return {
        async read() {
          throw new Error("missing legacy database");
        },
      };
    },
    values,
...
function tempFiles() {
  return {
    dataPath(name = "") {
      return path.join(os.tmpdir(), "mibot-test-data", name);
    },
    async dataFile(name) {
      const target = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "mibot-data-")), name);
      await fs.mkdir(path.dirname(target), { recursive: true });
      return target;
    },
    async withTemp(use) {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "mibot-media-"));
      const signal = new AbortController().signal;
      try {
        return await use(directory, signal);
      } finally {
...
function base(overrides = {}) {
  ...
  const context = {
    ...
    storage,
    files: overrides.files ?? tempFiles(),
    log: { info() {}, error() {} },
    http: overrides.http ?? {
      async json() {
...
function invoke(factory, command, text, fixture, message = {}) {
  return factory().commands[command].handle(
    {
      command,
      prefix: ".",
...
test("kkp serializes bot requests and associates each new video with one caller", async () => {
  ...
  const client = {
      ...
      return [
        {
          id: sequence * 10 + 1,
          out: false,
          media: { document: sequence },
          document: { mimeType: "video/mp4" },
          message: `video ${sequence}`,
        },
      ];
    },
    async sendMessage(peer, value) {
      if (value.message === "随机色色") {
        active += 1;
    ...
    async sendFile(peer, value) {
      files.push(value.file.document);
      active -= 1;
    },
    async markAsRead() {},
  };
  const f = base({ client });
  const plugin = factories.kkp();
  const run = id =>
    plugin.commands.kkp.handle(
      {
        command: "kkp",
        prefix: ".",
        args: [],
        message: { id, chatId: String(id), outgoing: true, text: ".kkp", raw: { peerId: id, async delete() {} } },
      },
      f.context,
    );
...
test("kkp cancels pending bot polling during unload", async () => {
  ...
  const pending = factories.kkp().commands.kkp.handle(
    {
      command: "kkp",
      prefix: ".",
      args: [],
      message: { id: 1, chatId: "1", outgoing: true, text: ".kkp", raw: { peerId: 1 } },
    },
    f.context,
  );
...
test("convert streams Telegram media through bounded FFmpeg and sends MP3", async () => {
  ...
  const processes = {
    async run(command, args, options) {
      ...
      await fs.writeFile(args.at(-1), Buffer.from("mp3"));
  ...
  assert.equal(calls[0].options.timeoutMs <= 180000, true);
  assert.equal(sent.length, 1);
  assert.equal(sent[0].options.replyTo, 9);
  assert.equal(sent[0].options.file.endsWith(".mp3"), true);
});
...
test("convert AI mode uses the unified search service for metadata", async () => {
  ...
  const services = {
    available(plugin, service) {
      return plugin === "ai" && service === "search";
    },
    async call(plugin, service, input) {
      serviceCalls.push({ plugin, service, input });
      return { text: "歌曲名：稻香\n歌手：周杰伦\n专辑：我很忙" };
    },
  };
  const f = base({
  ...
  assert.deepEqual(
    { plugin: serviceCalls[0].plugin, service: serviceCalls[0].service },
    { plugin: "ai", service: "search" },
  );
...
test("convert delegates AI provider settings and does not retry helper exit failures", async () => {
  ...
  assert.equal(
    f.edits.some(entry => entry.text.includes("private-key")),
    false,
  );
...
test("t migrates existing JSON roles, restricts keys, and streams Fish output into a scoped voice conversion", async () => {
  ...
  await invoke(factories.t, "tk", ".tk secret", f);
  assert.match(f.edits.at(-1).text, /收藏夹/);
  await invoke(factories.t, "tk", ".tk secret", f, { saved: true });
  const stored = storage.values.get("tts_data.json");
  assert.equal(stored.roles.旧角色, "legacy-id");
  assert.equal(stored.schemaVersion, 1);
  const calls = [],
    sent = [];
  f.context.http.withResponse = async (url, init, consume) =>
    consume(new Response(Buffer.from("mp3"), { status: 200 }), f.context.signal);
  f.context.processes.run = async (command, args, options) => {
    calls.push({ command, args, options });
    await fs.writeFile(args.at(-1), "ogg");
    return { stdout: Buffer.alloc(0) };
  };
  f.context.telegram.withClient = async operation =>
    operation(
      {
        async sendFile(peer, value) {
  ...
  await factories.t().commands.t.handle(
    {
      command: "t",
      prefix: ".",
      args: ["你好", "👋"],
  ...
  assert.equal(calls[0].command.startsWith("/"), true);
  assert.equal(sent[0].value.voiceNote, true);
});
...
test("eatgif validates and renders the remote catalog without loading native media dependencies", async () => {
  ...
  assert.match(f.edits.at(-1).text, /&lt;挥手&gt;/);
  assert.doesNotMatch(f.edits.at(-1).text, /<挥手>/);
  assert.deepEqual(factories.eatgif().resources.processes, {
    concurrency: 1,
    queueCapacity: 1,
    timeoutMs: 180000,
...
test("media plugins declare budgets within the production host cap", () => {
  for (const id of ["convert", "t", "eatgif"]) {
    ...
    assert.equal(limits.timeoutMs <= 180000, true);
    assert.equal(limits.maxOutputBytes <= 2 * 1024 * 1024, true);
  }
});
