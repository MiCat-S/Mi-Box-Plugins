...
const create = require(path.join(buildPlugin({id: 'crazy4', packageRoot: path.resolve(__dirname, '../crazy4'), entry: 'v2.ts'}).artifactDir, 'index.cjs')).default;
const CHAT = '9007199254740993';
const deferred = () => {let resolve; const promise = new Promise(done => {resolve = done;}); return {promise, resolve};};

async function fixture(t, client) {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), 'crazy4-compat-'))), edits = [], logs = [];
  const host = new PluginHost({storageRoot: root,
    logger: {info() {}, error(event, fields) {logs.push({event, fields});}},
    telegram: {async edit(_message, text) {edits.push(text);}, async reply() {}, async invoke() {}, async getReply() {},
  ...
  const run = (text = '.crazy4', extra = {}) => host.dispatchPrimary({id: 41, chatId: CHAT, senderId: CHAT, outgoing: true, text, ...extra});
...
test('CRAZY4-COMPAT-01 a raw receipt deletion failure never changes a successful send into failure', async t => {
  const sent = [];
  const f = await fixture(t, {async sendMessage(peer, value) {sent.push({peer, value});}});
  await f.run('.crazy4', {replyToId: 12, raw: {peerId: 'raw-peer', async delete() {throw new Error('private-delete-secret');}}});
...
test('CRAZY4-COMPAT-02 missing raw peer uses the exact chat id and native receipt cleanup', async t => {
  const sent = [], deleted = [];
  const f = await fixture(t, {async sendMessage(peer, value) {sent.push({peer: String(peer), value});},
    async deleteMessages(peer, ids, options) {deleted.push({peer: String(peer), ids, options});}});
  await f.run();
...
test('CRAZY4-COMPAT-03 cancellation after send performs no receipt deletion or failure feedback', async t => {
  ...
  const client = {async sendMessage() {entered.resolve(); await release.promise;}, async deleteMessages() {deletes++;}};
  const f = await fixture(t, client), running = f.run();
...
test('CRAZY4-COMPAT-04 send failures use fixed feedback and safe identifiers', async t => {
  const f = await fixture(t, {async sendMessage() {throw new Error('tg-secret /etc/telebox/session');}});
...
test('CRAZY4-COMPAT-05 help arguments match exactly instead of legacy substring matching', async t => {
  const sent = [];
  const f = await fixture(t, {async sendMessage(_peer, value) {sent.push(value);}, async deleteMessages() {}});
