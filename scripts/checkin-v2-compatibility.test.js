...
function loadCheckin() {
  const { artifactDir } = buildPlugin({
    id: "checkin",
    packageRoot: path.resolve(__dirname, "../checkin"),
    entry: "v2.ts",
  });
  ...
  const client = {
    ...
    async getMessages(peer) {
      return options.onGet
        ? options.onGet(peer)
        : [{ id: 11, date: Math.floor(Date.now() / 1000), out: false, message: "signed" }];
    },
    async invoke(request) {
      if (options.onInvoke) return options.onInvoke(request);
      return {};
    },
  };
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async withClient(operation, signal) {
        return operation(client, options.clientSignal ?? signal);
      },
    },
  });
      ...
      return [
        {
          ...
          replyMarkup: {
            rows: [
              { buttons: [{ text: "Sign", type: { className: "InlineButtonTypeCallback", data: Buffer.from("go") } }] },
            ],
          },
        },
      ];
...
test("long commands and names paginate list and summary without exceeding Telegram bounds", async t => {
  ...
  for (let index = 0; index < 3; index++) {
    const id = `long-${index}`,
      name = `${index}-${"名".repeat(1600)}`;
    await f.run(`.checkin add ${id} ${name} @bot${index}`);
    await f.listen(`/${"x".repeat(1800)}`, { id: 10 + index, replyToId: 91 });
  }
  const beforeListEdits = f.edits.length,
    beforeListReplies = f.replies.length;
  await f.run(".checkin list");
  ...
  await f.run(".checkin");
  const summaryPages = f.sent
    .filter(item => item.peer === "100" && /签到汇总报告|\d+\/\d+/.test(item.value.message))
