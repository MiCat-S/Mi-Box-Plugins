...
test('openlist installation saves initial credentials and updates preserve them',async t=>{
  ...
  let state={schemaVersion:1,username:'',password:'',defaultPath:'',legacyImported:true},installed=false;const calls=[],edits=[];const signal=new AbortController().signal;
  const ctx={signal,storage:{json:()=>({read:async()=>structuredClone(state),update:async fn=>(state=await fn(state))})},telegram:{edit:async(_m,text)=>edits.push(text)},http:{withResponse:async()=>{throw new Error('fixture must not download a release');}},files:{withTemp:async fn=>{const dir=fs.mkdtempSync(path.join(require('node:os').tmpdir(),'mibot-openlist-test-'));try{return await fn(dir,signal);}finally{fs.rmSync(dir,{recursive:true,force:true});}}},processes:{run:async(executable,args)=>{calls.push([executable,...args]);if(executable==='/usr/bin/install'&&args.at(-1)==='/opt/openlist/openlist')installed=true;return{stdout:Buffer.from(args[0]==='admin'?'username: admin\n':''),stderr:Buffer.from(args[0]==='admin'?'password: fixture-password\n':'')};}}};
  const plugin=create({releaseBinary:async(_context,directory)=>{const binary=path.join(directory,'openlist-fixture');await fs.promises.writeFile(binary,'fixture',{mode:0o700});return binary;},ordinary:async(target,kind)=>kind==='file'&&target==='/opt/openlist/openlist'&&installed});
  const run=action=>plugin.commands.op.handle({message:{id:1,chatId:'7',outgoing:true,saved:false},args:[action],prefix:'.'},ctx);
