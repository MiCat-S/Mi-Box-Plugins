...
function listenerFixture(patch = {}) {
  let data = {
    ...
    whitelist: [],
    blacklist: [],
    stats: { formattedMessages: 0, lastFormatted: 0 },
    ...patch,
  };
  const edits = [];
  const logs = [];
  const ctx = {
    ...
    telegram: {
      ...
      async reply() {
        assert.fail("automatic formatting must edit, never reply");
      },
    },
    storage: {
      json: () => ({
  ...
  return {
    ...
    listener,
    run: extra =>
      listener.handle({ id: 1, chatId: "1", senderId: "owner", text: "中文ABC", outgoing: true, ...extra }, ctx),
  };
}
...
test("pangu status and reset preserve other chats and restore global inheritance", async () => {
  ...
  const command = async args =>
    create().commands.pangu.handle(
      {
        args,
        command: "pangu",
        prefix: ".",
        message: { id: 1, chatId: "1", outgoing: true, text: ".pangu " + args.join(" ") },
      },
      f.ctx,
    );
  await command([]);
...
async function migrationFixture(t, legacy, current) {
  ...
  return {
    ctx: { signal: new AbortController().signal, storage },
    db: storage.json("data.json", {}),
    legacy: storage.json("config.json", {}),
  };
}
...
test("pangu preserves explicit V2 values and merges legacy chat and statistics metadata", async t => {
  const f = await migrationFixture(
    ...
    { chats: { 1: false }, globalMode: false, whitelist: [], stats: { formattedMessages: 50 }, custom: "retained" },
  );
  await create().setup(f.ctx);
...
test("pangu setup without legacy data allows later import and formatting preserves metadata", async t => {
  ...
  await f.legacy.update(() => ({
    globalMode: true,
    stats: { formattedMessages: 9, lastFormatted: null, enabledChats: 3 },
  }));
  await create().setup(f.ctx);
