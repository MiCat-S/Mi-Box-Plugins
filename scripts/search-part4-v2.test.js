...
test("search atomically deduplicates concurrent channel additions", async () => {
  ...
  const store = {
    ...
    update(change) {
      const operation = tail.then(async () => {
        data = await change(structuredClone(data));
        return structuredClone(data);
      });
      tail = operation.then(
        () => undefined,
  ...
  const client = {
    async getEntity(value) {
      return { className: "Channel", title: String(value), megagroup: true };
    },
  };
  const context = {
    signal,
  ...
  const invoke = () =>
    create().commands.so.handle(
      {
        command: "so",
        prefix: ".",
        args: ["add", "@same"],
        message: { id: 1, chatId: "1", text: ".so add @same", outgoing: true },
      },
      context,
    );
  await Promise.all([invoke(), invoke()]);
  assert.deepEqual(
    data.channelList.map(item => item.handle),
    ["@same"],
  );
  ...
  await create().commands.so.handle(
    {
      ...
      prefix: ".",
      args: ["default", "@a"],
      message: { id: 1, chatId: "1", text: ".so default @a", outgoing: true },
    },
    context,
  );
