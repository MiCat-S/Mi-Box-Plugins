...
function fixture(reply, options = {}) {
  ...
  const context = {
    ...
    files: {
      async withTemp(use) {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mibot-rev-"));
        try {
          return await use(dir, context.signal);
        } finally {
  ...
  return {
    ...
    invokes,
    run: (text, message = {}) =>
      create().commands.rev.handle(
        {
          command: "rev",
          prefix: ".",
          args: text.split(/\s+/).slice(1),
          message: { id: 1, chatId: "4", outgoing: true, text, raw, ...message },
        },
        context,
      ),
  };
...
test("rev remaps cross-line entities without emitting newline-only ranges", { timeout: 180_000 }, async t => {
  ...
  assert.deepEqual(
    request.entities.map(entity => [entity.className, entity.offset, entity.length]),
    [
      ["MessageEntityItalic", 0, 3],
      ["MessageEntityBold", 0, 2],
      ["MessageEntityBold", 5, 2],
    ],
  );
  ...
  for (const original of originals)
    assert.deepEqual(
      {
        offset: original.entity.offset,
        length: original.entity.length,
        url: original.entity.url,
        userId: original.entity.userId,
      },
...
test(
  "rev drops adjacent semantic half-graphemes instead of creating overlapping entities",
  { timeout: 180_000 },
  async t => {
    const text = "👍🏽";
...
test(
  "rev expands partial formatting but requires complete graphemes for semantic entities",
  { timeout: 180_000 },
  async t => {
    const text = "A👨‍👩‍👧‍👦👍🏽e\u0301Z";
    ...
    assert.deepEqual(
      request.entities.map(entity => [entity.className, entity.offset, entity.length]),
      [
        ["MessageEntityItalic", 3, 4],
        ["MessageEntityCustomEmoji", 3, 4],
        ["MessageEntityBold", 7, 11],
      ],
    );
...
test("rev resolves illegal semantic overlap by retaining the first source entity", { timeout: 180_000 }, async t => {
  ...
  assert.deepEqual(
    linkFirst.invokes[0].entities.map(entity => entity.className),
    ["MessageEntityTextUrl"],
  );
  assert.equal(linkFirst.invokes[0].entities[0].url, "https://example.test/path");
  assert.deepEqual(
    mentionFirst.invokes[0].entities.map(entity => entity.className),
...
test("rev rejects code nesting while retaining unrelated formatting", { timeout: 180_000 }, async t => {
  ...
  assert.deepEqual(
    f.invokes[0].entities.map(entity => [entity.className, entity.offset, entity.length]),
    [
      ["MessageEntityUnderline", 0, 1],
      ["MessageEntityCode", 1, 3],
    ],
  );
});
...
test("rev preserves CRLF boundaries across consecutive empty lines", { timeout: 180_000 }, async t => {
  ...
  assert.equal(f.invokes[0].message, "ba\r\n\r\n\r\ndc");
  assert.deepEqual(
    f.invokes[0].entities.map(entity => [entity.offset, entity.length]),
    [[0, 10]],
  );
});

test(
  "rev ignores zero-length and invalid entities while retaining valid ASCII mapping",
  { timeout: 180_000 },
  async t => {
...
test("rev sends fixed ffmpeg filters for replied media", async () => {
  ...
  assert.deepEqual(f.calls[0].args.slice(0, 5), ["-nostdin", "-y", "-protocol_whitelist", "file", "-i"]);
  assert.equal(f.calls[0].args[f.calls[0].args.indexOf("-fs") + 1], String(50 * 1024 * 1024));
  assert.equal(f.calls[0].options.cwd, path.dirname(f.calls[0].args.at(-1)));
...
test("rev shows local help for missing input", async () => {
  ...
  assert.match(f.edits.at(-1).text, /反转插件/);
});
