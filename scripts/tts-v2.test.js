        ...
        return operation(
          {
            async sendFile(peer, options) {
              sent.push({
                peer,
                options,
                bytes: Buffer.isBuffer(options.file) ? options.file.length : (await fs.stat(options.file)).size,
                content: Buffer.isBuffer(options.file) ? options.file.toString() : undefined,
              });
            },
...
test("tts imports legacy JSON, restricts secrets to Saved Messages and redacts settings", async t => {
  ...
  const directory = path.join(f.root, "tts");
  ...
  await f.run(".tts list");
  assert.match(f.edits.at(-1).text, /eastasia/);
  assert.doesNotMatch(f.edits.at(-1).text, /legacy-key/);
  await f.run(".tts config new-secret eastus");
  assert.match(f.edits.at(-1).text, /仅在收藏夹/);
  await f.run(".tts config new-secret eastus", { saved: true });
  assert.doesNotMatch(f.edits.at(-1).text, /new-secret/);
  const settings = await f.host.readSettings("tts");
  assert.equal(settings.secretSet.key, true);
  assert.equal(settings.values.key, undefined);
  const stored = JSON.parse(await fs.readFile(path.join(directory, "config.json"), "utf8"));
  assert.equal(stored.schemaVersion, 1);
  assert.equal(stored.key, "new-secret");
});
...
test("tts synthesizes replied text by streaming to a scoped file with a locked provider host", async t => {
  const f = await fixture(t, (url, init) => {
    assert.equal(url.hostname, "eastus.tts.speech.microsoft.com");
    assert.equal(url.pathname, "/cognitiveservices/v1");
    assert.equal(init.headers["Ocp-Apim-Subscription-Key"], "test-key");
    assert.match(init.body, /回复文本/);
  ...
  assert.equal(f.sent[0].bytes, 11);
  assert.equal(f.sent[0].options.voiceNote, true);
  assert.equal(f.deleted.length, 1);
  assert.deepEqual(f.deleted[0].ids, [1]);
  assert.deepEqual(await fs.readdir(path.join(f.root, ".temp", "tts")), []);
});
...
test("tts validates regions before network access and lists bounded voice data safely", async t => {
  ...
  const f = await fixture(t, url => {
    assert.equal(url.pathname, "/cognitiveservices/voices/list");
    return Response.json(voices);
  });
  await f.run(".tts config test-key not-a-region", { saved: true });
  assert.match(f.edits.at(-1).text, /Region/);
  assert.equal(f.requests.length, 0);
  await f.run(".tts config test-key eastus", { saved: true });
  await f.run(".tts voices zh-CN");
  assert.equal(f.requests.length, 1);
  assert.match(f.edits.at(-1).text, /XiaoxiaoNeural/);
  assert.match(f.edits.at(-1).text, /&lt;晓晓&gt;/);
});
...
test("tts unload actively cancels and unlocks a hanging Azure audio body with no upload or delete", async t => {
  ...
  assert.equal(lockedAtCancel, true);
...
test("tts voices export preserves entries beyond the former 500-item cutoff", async t => {
  const voices = Array.from({ length: 550 }, (_, index) => ({
    ShortName: `en-US-Voice${index}Neural`,
    LocalName: `Voice ${index}`,
    Locale: "en-US",
    Gender: "Female",
  }));
