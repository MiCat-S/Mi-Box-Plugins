...
function tables(source) {
  ...
  return JSON.parse(
    vm.runInNewContext(code + "\nJSON.stringify({FIAT_CURRENCIES, CRYPTO_CURRENCIES})", Object.create(null), {
      timeout: 1000,
    }),
  );
}
...
test.before(async () => {
  ...
  const built = buildPlugin({
    id: "rate",
    packageRoot: path.resolve(__dirname, "../rate"),
    entry: "v2.ts",
    rootDir: buildRoot,
  });
...
async function fixture(t, { fetch: fetcher = successFetch, edit, hostOptions = {} } = {}) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async reply() {
        assert.fail("rate edits its command message");
      },
      async getReply() {
        assert.fail("rate does not read replies");
      },
      async invoke() {
        assert.fail("native RPC forbidden");
      },
      async withClient() {
        assert.fail("raw Telegram access forbidden");
      },
    },
    ...hostOptions,
  });
  ...
  for (const [name, info] of Object.entries(oldTables.FIAT_CURRENCIES)) {
    for (const token of [name, ...(info.aliases || []), ...(name === "myr" ? ["rm"] : [])]) {
      ...
      if (requests.length > before)
        assert.equal(requests.at(-1).url.searchParams.get("base"), info.symbol.toLowerCase());
...
test("all built-in crypto codes and aliases keep Binance symbols and need no discovery", async t => {
  const { run, requests, edits } = await fixture(t, {
    fetch: async url => (url.hostname === "api.binance.com" ? Response.json({ price: "2" }) : successFetch(url)),
  });
  for (const [name, info] of Object.entries(oldTables.CRYPTO_CURRENCIES)) {
    for (const token of [name, ...(info.aliases || [])]) {
      ...
      assert.equal(
        requests
          .slice(before)
          .find(({ url }) => url.hostname === "api.binance.com")
...
for (const [price, expected] of [
  ...
  test(`price formatting preserves precision tier ${price}`, async t => {
    const { run, edits } = await fixture(t, {
      fetch: async url => (url.hostname === "api.binance.com" ? Response.json({ price }) : successFetch(url)),
    });
    await run(".rate BTC");
    ...
    const { run, requests, edits } = await fixture(t, {
      fetch: async url => {
        if (url.pathname === listPath || ["/symbols", "/currencies"].includes(url.pathname)) {
          ...
          return Response.json(
            index === 0 ? ["ZZZ"] : index === 1 ? { symbols: { ZZZ: { description: "test" } } } : { ZZZ: "Test" },
          );
        }
...
test("built-in crypto takes priority over CoinGecko quote-currency list", async t => {
  const { run, requests, edits } = await fixture(t, {
    fetch: async url => (url.pathname === listPath ? Response.json(["btc", "eth", "zzz"]) : successFetch(url)),
  });
...
test("direct and inverse crypto pairs retain order", async t => {
  ...
  assert.deepEqual(
    requests.slice(0, 2).map(({ url }) => url.searchParams.get("symbol")),
    ["BTCETH", "ETHBTC"],
  );
...
test("unavailable benchmark prices retain conversion and legacy zero placeholders", async t => {
  const { run, edits } = await fixture(t, {
    fetch: async url =>
      url.searchParams.get("symbol") === "BTCETH"
...
test("fiat baskets expire after five minutes, are bounded to 16 bases and isolated by factory", async t => {
  ...
  mock.mock.restore();
  const names = Object.keys(oldTables.FIAT_CURRENCIES)
...
test("unknown currencies stay dynamic and do not grow a per-query currency cache", async t => {
  const { run, requests, edits } = await fixture(t, {
    fetch: async url => (url.hostname === "api.binance.com" ? Response.json({ price: 1 }) : successFetch(url)),
  });
...
test("invalid inputs, credentials, HTML and UTF-16 boundaries are rejected without echo or requests", async t => {
  ...
  const cases = [
    "https://alice:private-token@example.com/?token=private-token",
    "USD?token=private-token",
    "<b>private-token</b>",
    "BTC&USD",
    'BTC"USD',
    "BTC'USD",
  ...
  assert.equal(requests.length, 0);
  assert.doesNotMatch(
    JSON.stringify({ text: edits.map(edit => edit.text), logs }),
    /private-token|example\.com|<b>private/,
  );
});
...
test("malformed JSON, schema, business errors and nonpositive rates fall back and never display invalid math", async t => {
  const bodies = [
    ...
    '"private-token"',
    "{}",
    '{"success":false,"rates":{"USD":1}}',
    '{"result":"error","rates":{"USD":1}}',
    ...[
      null,
      [],
      "text",
      ...
      { USD: {} },
      { "<secret>": 1 },
    ].map(rates => JSON.stringify({ rates })),
...
test("mixed invalid fiat fields do not poison valid rates or object prototypes", async t => {
  const { run, edits } = await fixture(t, {
    fetch: async () =>
      new Response('{"rates":{"EUR":"0.9","USD":1,"CNY":null,"bad":true,"__proto__":42,"constructor":0}}'),
  });
  await run(".rate USD EUR");
  assert.match(edits.at(-1).text, /0\.900000 EUR/);
...
test("reciprocal and multiplication overflow remain explicit finite-number failures", async t => {
  const { run, edits } = await fixture(t, {
    fetch: async url =>
      url.hostname === "api.binance.com" ? Response.json({ price: "1e308" }) : Response.json({ rates: { USD: 1 } }),
  });
  await run(".rate BTC USD 1e308");
  assert.match(edits.at(-1).text, /换算结果超出有限数值范围/);
  const tiny = await fixture(t, {
    fetch: async url =>
      url.hostname === "api.binance.com" ? Response.json({ price: "5e-324" }) : Response.json({ rates: { USD: 1 } }),
  });
...
test("non-2xx and redirect responses stay within original providers and disclose no response secrets", async t => {
  for (const status of [204, 301, 302, 400, 401, 403, 429, 500, 503]) {
    const { run, requests, edits, logs } = await fixture(t, {
      fetch: async () =>
        new Response(status === 204 ? null : "<secret>private-token</secret>", {
          status,
          headers: { Location: "https://alice:private-token@private.invalid/" },
        }),
    });
    await run(".rate USD EUR");
    assert.equal(requests.length, 5);
...
test("native transport errors are classified safely without upstream messages or credentials", async t => {
  for (const errorCode of ["ENOTFOUND", "EAI_AGAIN", "ECONNREFUSED", "ECONNRESET"]) {
    const { run, edits, logs } = await fixture(t, {
      fetch: async () => {
        throw new TypeError("private-token", {
          cause: Object.assign(new Error("https://alice:private-token@secret.invalid"), { code: errorCode }),
        });
      },
    });
    await run(".rate USD EUR");
...
test("stream errors and invalid UTF-8 are safe and allow subsequent commands", async t => {
  for (const body of [
    () =>
      new ReadableStream({
        start(controller) {
          controller.error(new Error("private-stream-secret"));
        },
      }),
    ...
    let fail = true;
    const { run, edits, logs } = await fixture(t, {
      fetch: async url => (fail ? new Response(body()) : successFetch(url)),
    });
    await run(".rate USD EUR");
    assert.match(edits.at(-1).text, /获取价格失败/);
    fail = false;
    ...
    const { run, edits, requests } = await fixture(t, {
      fetch: async (url, init) => {
        ...
        return new Promise((_, reject) =>
          init.signal.addEventListener("abort", () => reject(new Error("private-timeout-token")), { once: true }),
        );
      },
    });
...
test("edit permission failures cause no API calls and no secret disclosure", async t => {
  const { run, edits, requests, logs } = await fixture(t, {
    edit: async () => {
      throw new Error("MESSAGE_EDIT_FORBIDDEN private-token");
    },
  });
  await run(".rate BTC");
  assert.equal(requests.length, 0);
  ...
  assert.doesNotMatch(
    JSON.stringify({ text: edits.map(edit => edit.text), logs }),
    /private-token|MESSAGE_EDIT_FORBIDDEN/,
  );
});

test("owner admission, saved messages, edited policy and prefix aliases are enforced by host", async t => {
  const { host, requests, edits, run } = await fixture(t, {
    hostOptions: { prefixes: ["!!", "."], aliases: { fx: "rate" } },
  });
...
test("unload waits for ignored fetch cancellation, suppresses fallback and blocks reload until settlement", async t => {
  for (const command of [".rate BTC ETH", ".rate ZZZ USD", ".rate USD EUR"]) {
    for (const failure of [false, true]) {
      const started = deferred(),
        release = deferred();
      const { run, host, requests, edits } = await fixture(t, {
        fetch: async url => {
          started.resolve();
...
test("unload during response cleanup waits for real settlement and sends no result", async t => {
  const started = deferred(),
    release = deferred();
  const { run, host, edits, requests } = await fixture(t, {
    fetch: async () =>
      new Response(
        new ReadableStream({
          start(controller) {
  ...
  await started.promise;
  try {
    assert.equal((await host.unload("rate", 5)).completed, false);
  } finally {
    release.resolve();
  }
  await running;
...
test("four concurrent queries bound HTTP admission and busy edit errors stay private", async t => {
  ...
  const running = Array.from({ length: 4 }, (_, index) =>
    host.dispatchPrimary({ ...envelope, chatId: String(index + 1), text: ".rate USD EUR" }),
  );
  await started.promise;
  try {
    await run(".rate USD EUR");
...
test("HTTP timeout does not race an uncooperative fetch or start fallback before settlement", async t => {
  ...
  t.mock.timers.enable({ apis: ["setTimeout"] });
  const running = run(".rate USD EUR").then(() => {
    completed = true;
  });
  await started.promise;
  try {
    ...
    fetch: async () =>
      new Response(
        new ReadableStream({
          ...
          async cancel() {
            canceled++;
            throw new Error("private-cleanup-token");
          },
        }),
      ),
  });
...
test("pre-aborted handler rejects before transport admission and ignores hostile abort reasons", async () => {
  ...
  await assert.rejects(
    createRate().commands.rate.handle(
      { message: envelope, args: ["BTC"], prefix: ".", command: "rate" },
      {
        signal,
        telegram: {
          async edit() {
