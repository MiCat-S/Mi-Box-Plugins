...
test.before(async () => {
  ...
  const built = buildPlugin({
    id: "botmzt",
    packageRoot: path.resolve(__dirname, "../botmzt"),
    entry: "v2.ts",
    rootDir: core,
  });
...
async function run(text, client, options = {}) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      edit: async (_message, value, settings, signal) => {
        signal.throwIfAborted();
        edits.push({ text: value, options: settings });
      },
      reply: async () => {},
  ...
  options.onHost?.(host);
  try {
    const handled = await host.dispatchPrimary({
      id: 9,
      chatId: "-1001",
      senderId: "1",
      text,
      ...
      raw: {
        id: 9,
        className: "Message",
        peerId: { className: "PeerChannel", channelId: 1001n },
...
test("first image request starts a new bot conversation before requesting the image", async t => {
  ...
  const client = {
    ...
    async sendMessage(peer, value) {
      sent.push({ peer, message: value.message });
    },
    async getMessages() {
      reads += 1;
      return reads === 1
  ...
  const result = await run(".rand", client);
...
test("image command preserves the bot text error instead of timing out generically", async t => {
  ...
  const client = {
    ...
    async getMessages() {
      reads += 1;
      return reads === 1 ? [{ id: 3, out: false }] : [{ id: 4, out: false, message: "错误：今日额度不足" }];
    },
  };
  try {
    const result = await run(".rand", client);
...
test("botmzt settings retain details and managed thirty-second deletion", async t => {
  ...
  try {
    const result = await run(".botmzt", client);
    t.after(() => result.host.shutdown(1000));
...
test("known Telegram access failures retain actionable feedback", async t => {
  ...
  const result = await run(".qd", client);
  t.after(() => result.host.shutdown(1000));
...
test("cancellation after initial history read sends no subsequent RPC", async () => {
  ...
  await factory().commands.rand.handle(
    { message: { id: 9, chatId: "-1001", raw: {} }, command: "rand", prefix: ".", args: [] },
    context,
  );
...
test("cancellation during sendFile prevents mark-as-read and command deletion", async () => {
  ...
  const client = {
    ...
    async getMessages() {
      reads += 1;
      return reads === 1 ? [{ id: 4, out: false }] : [{ id: 5, out: false, media: { photo: true } }];
    },
    async sendFile() {
      controller.abort();
    },
...
test("command deletion failure does not replace a successfully sent image", async t => {
  ...
  const client = {
    ...
    async getMessages() {
      reads += 1;
      return reads === 1 ? [{ id: 6, out: false }] : [{ id: 7, out: false, media: { photo: true } }];
    },
  };
  const result = await run(".rand", client, {
    rawDelete: async () => {
      throw new Error("delete secret");
    },
  });
  t.after(() => result.host.shutdown(1000));
