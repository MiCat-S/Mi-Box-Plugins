...
const create = require(
  path.join(
    buildPlugin({ id: "deepwiki", packageRoot: path.resolve(__dirname, "../deepwiki"), entry: "v2.ts" }).artifactDir,
    "index.cjs",
  ),
).default;
const deferred = () => {
...
async function fixture(t, fetch, { reply, initial, failEditAt, failReplyAt } = {}) {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "deepwiki-compat-"))),
    edits = [],
    replies = [],
    requests = [],
    logs = [];
  let editCalls = 0,
    replyCalls = 0;
  if (initial) {
    await fs.mkdir(path.join(root, "deepwiki"), { recursive: true });
    await fs.writeFile(path.join(root, "deepwiki/data.json"), JSON.stringify(initial));
  }
  const host = new PluginHost({
    ...
    telegram: {
      async edit(_message, text) {
        editCalls++;
        if (editCalls === failEditAt) throw new Error("edit failed");
        edits.push(text);
      },
      async reply(_message, text) {
        replyCalls++;
        if (replyCalls === failReplyAt) throw new Error("reply failed");
        replies.push(text);
      },
      async invoke() {},
      async getReply() {
  ...
  const run = (text, extra = {}) =>
    host.dispatchPrimary({ id: ++id, chatId: "100", senderId: "1", outgoing: true, text, ...extra });
...
test("DEEPWIKI-COMPAT-01 a replied question can select a tag without extra inline text", async t => {
  ...
  const f = await fixture(
    t,
    mcp("answer", body => calls.push(body)),
    { reply: { text: "reply question" } },
  );
...
test("DEEPWIKI-COMPAT-02 legacy context is bounded to 50 turns before building the MCP request", async t => {
  ...
  const initial = {
    ...
    chats: {
      100: {
        currentTag: "core",
        repos: { core: { tag: "core", repo: "owner/repo", url: "", addedAt: "" } },
        contextEnabled: true,
        turns: { core: turns },
      },
    },
  ...
  const f = await fixture(
    t,
    mcp("answer", body => {
      if (body.method === "tools/call") question = body.params.arguments.question;
    }),
    { initial },
  );
...
test("DEEPWIKI-COMPAT-06 later page failure preserves the result and reports a stable interruption", async t => {
  ...
  assert.deepEqual(
    { ...f.logs.at(-1).fields, total: undefined },
    { plugin: "deepwiki", published: 1, total: undefined, category: "Error" },
  );
...
test("DEEPWIKI-COMPAT-07 first page failure follows the normal command failure path", async t => {
  ...
  assert.deepEqual(f.logs.at(-1), {
    event: "pagination_delivery_interrupted",
    fields: { plugin: "deepwiki", published: 0, total: 1, category: "Error" },
  });
});
...
test("DEEPWIKI-COMPAT-08 project and context inventories paginate without losing rows", async t => {
  ...
  const initial = {
    schemaVersion: 1,
    legacyImported: true,
    chats: { 100: { currentTag: "project-000", repos, contextEnabled: true, turns: {} } },
  };
