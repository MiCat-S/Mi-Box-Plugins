...
test.before(async () => {
  ...
  const built = buildPlugin({
    id: "ip",
    packageRoot: path.resolve(__dirname, "../ip"),
    entry: "v2.ts",
    rootDir: buildRoot,
  });
...
async function fixture(
  t,
  { fetch: fetcher = async () => Response.json(success), reply, edit, sendReply, hostOptions = {} } = {},
) {
...
test("result preserves field order, proxy/hosting warnings, timezone and AS preview", async t => {
  const { run, edits } = await fixture(t, {
    fetch: async () => Response.json({ ...success, proxy: true, hosting: true }),
  });
...
test("missing optional fields retain N/A fallbacks and omit conditional output", async t => {
  const { run, edits } = await fixture(t, {
    fetch: async () => Response.json({ status: "success", country: "", regionName: null }),
  });
...
test("blank replies and reply-read failures preserve help without exposing transport errors", async t => {
  for (const reply of [
    ...
    " \n\t ",
    async () => {
      throw new Error("secret reply credentials");
    },
  ]) {
...
test("invalid addresses and credential URLs fail locally with bounded escaped output", async t => {
  ...
  const invalid = [
    ...
    "[::1]",
    "8.8.8.8/24",
    "http://user:top-secret@example.com/private",
    "https://example.com",
    "//example.com",
    "user:top-secret@example.com",
    "example.com:443",
...
test("business API failures are displayed verbatim and have no retry or fallback provider", async t => {
  for (const message of [
    ...
    const { run, requests, edits } = await fixture(t, {
      fetch: async () => Response.json({ status: "fail", message }),
    });
...
test("non-200 statuses including redirects and quota failures are explicit without following Location", async t => {
  for (const status of [201, 204, 301, 302, 403, 429, 500, 503]) {
    const { run, edits, requests } = await fixture(t, {
      fetch: async () =>
        new Response(status === 204 ? null : "not JSON", {
          status,
          headers: { Location: "https://user:secret@other.invalid/private" },
        }),
    });
    await run(".ip 8.8.8.8");
...
test("malformed provider JSON and schema failures produce the original parsing-error presentation", async t => {
  const cases = [
    ...
    '"text"',
    "{}",
    JSON.stringify({ status: "other" }),
    JSON.stringify({ ...success, as: 123 }),
    JSON.stringify({ ...success, country: { injected: true } }),
    JSON.stringify({ ...success, proxy: "true" }),
    JSON.stringify({ status: "fail", message: { bad: true } }),
  ];
  ...
  const { run, edits } = await fixture(t, {
    fetch: async () =>
      new Response(
        new ReadableStream({
          start(controller) {
            controller.enqueue(new Uint8Array(65537).fill(65));
          },
          cancel() {
            canceled++;
          },
...
test("native DNS and connection-refused errors preserve localized messages without leaking details", async t => {
  for (const code of ["ENOTFOUND", "EAI_AGAIN", "ECONNREFUSED"]) {
    for (const wrapped of [false, true]) {
      ...
      const { run, edits, logs, requests } = await fixture(t, {
        fetch: async () => {
          throw upstream;
        },
      });
...
• 稍后重试查询`,
      ...
      assert.doesNotMatch(
        JSON.stringify({ edits, logs }),
        /secret-host|private-token|ENOTFOUND|EAI_AGAIN|ECONNREFUSED/,
      );
      assert.equal(requests.length, 1);
...
test("unclassified network failures stay generic without leaking upstream details", async t => {
  ...
  const cases = [
    ...[
      "getaddrinfo ENOTFOUND secret-host",
      "EAI_AGAIN secret-host",
      "ECONNREFUSED secret-host",
      "upstream private-token",
    ].map(reason => new Error(reason)),
...
test("the original 15-second request deadline displays timeout while keeping the plugin active", async t => {
  ...
  const { run, edits, requests, host } = await fixture(t, {
    fetch: (_url, init) =>
...
test("cancellation during reply lookup suppresses help, requests and late output", async t => {
  const started = deferred(),
    release = deferred();
  const { run, host, edits, requests, replyReads } = await fixture(t, {
    reply: async () => {
      started.resolve();
      await release.promise;
      return "8.8.8.8";
  ...
  await started.promise;
  try {
    assert.equal((await host.unload("ip", 5)).completed, false);
  } finally {
    release.resolve();
  }
  await running;
...
test("cancellation during response streaming releases the reader and emits no late result", async t => {
  ...
  let canceled = 0,
    pulls = 0;
  const { run, host, edits } = await fixture(t, {
    fetch: async () =>
      new Response(
        new ReadableStream({
          pull(controller) {
...
test("owner admission, saved messages, edited-message policy and prefix aliases are enforced by Core", async t => {
  const { host, requests, edits, run } = await fixture(t, {
    hostOptions: { prefixes: ["!!", "."], aliases: { lookup: "ip" } },
  });
