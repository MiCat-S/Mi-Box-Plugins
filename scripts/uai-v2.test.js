...
function aiConfig(empty = false) {
  return {
    ...
    currentChatTag: empty ? "" : "main",
    currentChatModel: empty ? "" : "central-model",
...
async function fixture(
  ...
  await fs.writeFile(path.join(root, "ai", "config.json"), JSON.stringify(aiConfig(Boolean(legacy))));
  if (legacy !== undefined || v2 !== undefined) await fs.mkdir(path.join(root, "uai"));
  if (legacy !== undefined)
    await fs.writeFile(
      path.join(root, "uai", "config.json"),
  ...
  let loadError;
  try {
    await host.load(create("uai"));
  } catch (error) {
    loadError = error;
    if (!expectLoadFailure) throw error;
  }
  t.after(async () => {
    if (host.pluginState("uai")) await host.shutdown(1000);
    else await host.shutdown(1000);
...
test("uai migrates every legacy provider into ai, remaps the default, and scrubs local keys", async t => {
  const legacy = {
    providers: {
      old: {
        name: "old",
        base_url: "https://legacy.example.test/base",
  ...
  assert.equal(ai.currentChatModel, "legacy-model");
  assert.equal(ai.configs["uai-1"].key, "legacy-secret");
  assert.equal(ai.configs["uai-2"].key, "gemini-secret");
  assert.deepEqual(local.providers, {});
  assert.equal(local.defaultProvider, null);
  assert.equal(local.collapse, false);
  assert.equal(JSON.stringify(local).includes("secret"), false);
  assert.equal((await f.host.unload("uai", 1000)).completed, true);
...
test("uai routes provider management to ai and exposes only prompts and display settings", async t => {
  ...
  for (const command of ["add p https://other.invalid key openai", "set p", "model p other", "del p", "list"]) {
    await f.run(`.uai ${command}`, { saved: true });
...
test("uai analyzes referenced history through the current central chat provider", async t => {
  ...
  f.setReply({
    ...
    outgoing: false,
    text: "第一条",
    raw: { senderId: 7n, sender: { firstName: "甲" } },
  });
  ...
  assert.match(body.messages.at(-1).content, /第一条/);
  assert.match(f.edits.at(-1).text, /&lt;安全&gt;/);
...
test("corrupt legacy state fails setup without marking migration and succeeds on retry", async t => {
  ...
  await assert.rejects(
    fs.readFile(path.join(f.root, "uai", "v2-config.json"), "utf8"),
    error => error.code === "ENOENT",
  );
  await fs.writeFile(
    path.join(f.root, "uai", "config.json"),
    JSON.stringify({ prompts: { fixed: "已修复" }, collapse: false }),
  );
  await f.host.load(create("uai"));
...
test("legacy defaults do not overwrite explicit V2 prompts or collapse choice", async t => {
  const f = await fixture(t, {
    legacy: { providers: {}, prompts: { same: "旧值", legacy: "保留" }, collapse: false },
    v2: {
      schemaVersion: 2,
      providers: {},
...
test("a stale completed marker cannot skip newly recovered legacy providers", async t => {
  const legacy = {
    providers: {
      recovered: {
        name: "recovered",
        base_url: "https://recovered.example.test",
        api_key: "recovered-secret",
        model: "recovered-model",
        type: "openai",
      },
    },
    default_provider: "recovered",
  };
  const v2 = {
    schemaVersion: 2,
    providers: {},
...
test("legacy-key scrub failure leaves AI migration retryable", async t => {
  ...
  const mocked = t.mock.method(fs, "rename", async (source, destination) => {
    if (String(destination).endsWith("/uai/config.json"))
      throw Object.assign(new Error("scrub denied"), { code: "EACCES" });
