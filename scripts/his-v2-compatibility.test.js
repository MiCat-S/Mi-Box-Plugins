...
const create = require(
  path.join(
    buildPlugin({ id: "his", packageRoot: path.resolve(__dirname, "../his"), entry: "v2.ts" }).artifactDir,
    "index.cjs",
  ),
).default;
const deferred = () => {
...
async function fixture(t, options = {}) {
  const root = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "his-compat-"))),
    edits = [],
    replies = [],
    logs = [];
  let editCalls = 0,
  ...
  const client = options.client || {
    async getEntity(value) {
      return String(value) === "7" ? { id: 7n, title: "Group", megagroup: true } : { firstName: "Alice" };
    },
    async *iterMessages() {
      yield { id: 3, text: "hello" };
    },
  };
  const host = new PluginHost({
    ...
    telegram: {
      async edit(_message, text, settings) {
        editCalls++;
        if (editCalls === options.failEditAt) throw new Error("edit failed token=private-secret /private/path");
        edits.push({ text, settings });
      },
      async reply(_message, text, settings) {
        replyCalls++;
        if (replyCalls === options.failReplyAt) throw new Error("reply failed");
        replies.push({ text, settings });
      },
      async invoke() {},
      async getReply() {
        if (options.getReplyFailure) throw new Error("reply token=private-secret /private/path");
        return options.reply ?? { senderId: "42" };
      },
  ...
  const run = (text = ".his 9007199254741999 1") =>
...
test("HIS-COMPAT-01 numeric targets retain precision in the Teleproto boundary", async t => {
  ...
  const client = {
    ...
    async *iterMessages(_chat, options) {
      target = options.fromUser;
      yield { id: 1, text: "ok" };
    },
  };
...
test("HIS-COMPAT-03 later page failure preserves the published result and reports interruption", async t => {
  const client = {
    ...
    async *iterMessages() {
      for (let id = 1; id <= 100; id++) yield { id, text: "&".repeat(50) };
    },
  };
...
test("HIS-COMPAT-04 reply/count argument matrix keeps defaults, cap, and validation", async t => {
  ...
  const client = {
    ...
    async *iterMessages(chat, options) {
      calls.push({ chat, options });
      yield { id: 1, text: "ok" };
    },
  };
  for (const [command, expectedTarget, expectedLimit] of [
    [".his", "42", 30],
    [".his 999", "42", 100],
    [".his @alice", "@alice", 30],
    [".his @alice 7", "@alice", 7],
    ...
    assert.equal(String(call.options.fromUser), expectedTarget);
    assert.equal(call.options.limit, expectedLimit);
  }
  const invalid = await fixture(t, { client });
  await invalid.run(".his @alice 0");
  assert.equal(invalid.edits.at(-1).text, "❌ 无效的数量参数");
  const excessive = await fixture(t, { client });
  await excessive.run(".his a 1 extra");
  assert.match(excessive.edits.at(-1).text, /参数过多/);
});
...
test("HIS-COMPAT-05 media and service message matrix is labeled and escaped", async t => {
  ...
  const items = [
    ...
    ...[
      ...
      ["MessageMediaDice", "骰子"],
      ["MessageMediaGame", "游戏"],
    ].map(([className, label]) => ({ media: { className }, label })),
  ];
  const client = {
    async getEntity(value) {
      return String(value) === "7" ? { username: "group" } : { firstName: "Alice<&" };
    },
    async *iterMessages() {
      let id = 0;
      for (const item of items) yield { id: ++id, text: "<&", media: item.media };
      yield {
        id: ++id,
        className: "MessageService",
        action: { className: "MessageActionChatEditTitle", title: "New<&" },
      };
    },
  };
  const f = await fixture(t, { client });
  await f.run(".his @alice 100");
  const output = [...f.edits, ...f.replies].map(item => item.text).join("\n");
  for (const item of items) assert.match(output, new RegExp(`\\[${item.label}\\]`));
  assert.match(output, /\[修改群名\] New&lt;&amp;/);
...
test("HIS-COMPAT-06 empty, flood, long-message, and private errors keep dedicated safe feedback", async t => {
  ...
  await empty.run(".his @none");
  assert.match(empty.edits.at(-1).text, /未找到/);
  const flood = await fixture(t, {
    client: {
      async getEntity() {
        return {};
      },
      async *iterMessages() {
        throw new Error("FLOOD_WAIT_37 private");
      },
    },
  });
  await flood.run(".his @flood");
  assert.match(flood.edits.at(-1).text, /等待 37 秒/);
  assert.doesNotMatch(flood.edits.at(-1).text, /private/);
  const long = await fixture(t, {
    client: {
      async getEntity() {
        return {};
      },
      async *iterMessages() {
        throw new Error("MESSAGE_TOO_LONG secret");
      },
    },
  });
  await long.run(".his @long");
  assert.match(long.edits.at(-1).text, /消息过长/);
  assert.doesNotMatch(long.edits.at(-1).text, /secret/);
  ...
  await safe.run(".his @safe");
  assert.equal(safe.edits.at(-1).text, "❌ <b>操作失败</b>，请稍后重试");
});

test("HIS-COMPAT-07 reply lookup and first result transport errors never disclose private details", async t => {
  const replyFailure = await fixture(t, { getReplyFailure: true });
  await replyFailure.run(".his");
  assert.equal(replyFailure.edits.at(-1).text, "❌ <b>操作失败</b>，请稍后重试");
  assert.doesNotMatch(
    JSON.stringify({ edits: replyFailure.edits, logs: replyFailure.logs }),
    /private-secret|private\/path/,
  );
  const finalFailure = await fixture(t, { failEditAt: 2 });
  await finalFailure.run(".his @alice 1");
  assert.equal(finalFailure.edits.at(-1).text, "❌ <b>操作失败</b>，请稍后重试");
  assert.doesNotMatch(
    JSON.stringify({ edits: finalFailure.edits, logs: finalFailure.logs }),
    /private-secret|private\/path/,
  );
});
...
test("HIS-COMPAT-09 cancellation during iteration publishes no partial history", async t => {
  ...
  const f = await fixture(t, { client });
  const running = f.run(".his @alice 2");
  await entered.promise;
  const unloading = f.host.unload("his", 1000);
  release.resolve();
  assert.equal((await unloading).completed, true);
