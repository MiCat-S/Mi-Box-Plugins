...
test("moyu rejects HTTP failure and never deletes on upload failure", async () => {
  for (const f of [
    fixture(new Response("error", { status: 503 })),
...
test("real Host unload waits for reader cancellation cleanup before completing", async t => {
  ...
  await readReady;
  let finished = false;
  const unloading = host.unload("moyu", 1000).then(value => {
    finished = true;
    return value;
  });
  await cancelReady;
...
test("real Host cancellation during upload waits and performs no subsequent delete", async t => {
  ...
  const host = new PluginHost({
    storageRoot: root,
    logger: { info() {}, error() {} },
    http: { fetch: async () => new Response(image) },
    telegram: {
      async edit(_message, text) {
        edits.push(text);
      },
