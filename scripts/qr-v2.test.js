...
function fixture(options = {}) {
  ...
  const context = {
    ...
    files: {
      async withTemp(use) {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mibot-qr-"));
        try {
    ...
    processes: {
      async run(command, args, runOptions) {
        ...
        if (options.processFail) throw new Error("private");
        return { stdout: Buffer.from(options.stdout ?? "PNG") };
      },
    },
    telegram: {
      async edit(message, text, editOptions) {
        edits.push({ message, text, editOptions });
      },
      async reply() {},
      async invoke() {},
      async getReply() {
  ...
  return {
    ...
    deleted,
    run: (text, message = {}) =>
      create().commands.qr.handle(
        {
          command: "qr",
          prefix: ".",
          args: text.split(/\s+/).slice(1),
          message: { id: 1, chatId: "9", outgoing: true, text, raw, ...message },
        },
        context,
      ),
  };
