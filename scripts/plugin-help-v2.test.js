...
const ids = fs
  .readdirSync(root)
  .filter(id => fs.existsSync(path.join(root, id, "v2.ts")))
  .sort();
...
test.before(() => {
  ...
  test.mock.method(require("node:net").Socket.prototype, "connect", () =>
    assert.fail("Help must not open a network connection"),
  );
});
    ...
    const parser = new Parser(
      {
        onclosetag(_name, implied) {
          assert.equal(implied, false, "tags close within each page");
        },
      },
      { xmlMode: true },
    );
...
for (const id of ids)
  test(`${id}: complete artifact help survives both command and catalog rendering`, async t => {
    ...
    const commands = Object.fromEntries(
      Object.entries(definition.commands).map(([name, entry]) => [name, { ...entry, handle: forbidden }]),
    );
    ...
    for (const [name, command] of Object.entries(commands)) {
      assert.ok(
        expected.includes(`${prefix}${name}`),
        `help documents registered command ${name} with the active prefix`,
      );
      for (const args of [
        ["--help"],
        ...(command.helpArgs || []).map(arg => [arg]),
        ...(command.helpOnEmpty ? [[]] : []),
      ]) {
        output.length = 0;
    ...
    if (id === "autochangename") {
      for (const section of ["基础操作", "时区管理", "外观设置", "文案管理", "天气显示", "查看配置"])
        assert.ok(expected.includes(section));
      for (const example of [
        ...
        assert.ok(expected.includes(`${prefix}${example}`));
    }
  });
