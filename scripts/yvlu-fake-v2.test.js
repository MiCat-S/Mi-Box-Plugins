...
async function fixture(text, { deleteFails = false, rawText = text, aliases = false } = {}) {
  ...
  const replied = {
    ...
    entities: [],
    media: /^\.yvlu fr?\s/.test(text) ? { photo: {} } : undefined,
    sender: { id: 1n, firstName: "Original" },
    isReply: true,
    replyTo: { replyToMsgId: 6, quote: true, quoteText: "partial parent", quoteEntities: [] },
  };
  const fakeSender = {
    ...
    lastName: "Sender",
    username: "fake",
    emojiStatus: { documentId: 8n },
  };
  const client = {
    async getEntity(value) {
      return value === "@fake" || value === fakeSender ? fakeSender : value;
    },
    async sendFile(peer, options) {
      sent.push({ peer, options });
    },
    async deleteMessages() {
      if (deleteFails) throw new Error("private cleanup detail");
    },
  };
  const context = {
    ...
    commands: aliases
      ? {
          parse(source) {
            return source.startsWith(".forge ")
              ? { prefix: ".", command: "yvlu", args: ["f", source.slice(7)] }
              : undefined;
          },
        }
      : undefined,
    http: {
      async withResponse(_url, init, operation) {
        payloads.push(JSON.parse(init.body));
        const png = await sharp({ create: { width: 2, height: 2, channels: 4, background: "#000" } })
          .png()
          .toBuffer();
        return operation(
  ...
  await create().commands.yvlu.handle(
    {
      command: "yvlu",
      prefix: ".",
      ...
      message: {
        ...
        raw: {
          peerId: "peer",
          message: rawText,
          entities: rawText.includes("styled")
            ? [{ className: "MessageEntityBold", offset: rawText.indexOf("styled"), length: 6 }]
            : [],
        },
      },
    },
