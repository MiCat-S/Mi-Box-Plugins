...
function fixture(initial = {enabled: false, configVersion: 1, customRules: [{command: 'ping', delay: 10}]}, options = {}) {
  let state = structuredClone(initial), updateTail = Promise.resolve(), readBarrier; const edits = [], tasks = [], errors = [];
  const json = {
    async read() {
      if (readBarrier) {
    ...
    update(fn) {
      const update = async () => {state = await fn(structuredClone(state)); return structuredClone(state);};
  ...
  const client = options.client ?? {async getMessages() {return [{id: 11, out: true}, {id: 10, out: true}];}, async deleteMessages() {}};
  const context = {signal: new AbortController().signal, storage: {json() {return json;}}, tasks: {run(label, fn) {tasks.push({label, fn}); return Promise.resolve();}},
    ...
    telegram: {async edit(_m, text) {edits.push(text);}, async withClient(op) {return op(client, context.signal);}}, log: {info() {}, error(...args) {errors.push(args);}}};
  const plugin = create(), message = {id: 10, chatId: '9007199254740993', senderId: '1', outgoing: true, saved: true, text: ''};
  return {plugin, context, edits, tasks, errors, state: () => state, setup: () => plugin.setup(context),
    gateReads(count) {let release; const promise = new Promise(resolve => {release = resolve;}); readBarrier = {arrivals: 0, count, promise, release};},
...
test('version-zero migration adds missing defaults and assigns collision-free rule ids', async () => {
  const f = fixture({enabled: true, customRules: [{id: '2', command: 'custom', delay: 15}, {command: 'ping', delay: 30}]});
  await f.setup();
  assert.equal(f.state().configVersion, 1);
  assert.equal(f.state().rules.some(rule => rule.command === 'lang' && rule.delay === 10), true);
...
test('requires an explicit bounded integer delay and preserves case-sensitive parameters', async () => {
  const f = fixture({schemaVersion: 2, enabled: false, rules: [], pending: {}}); await f.setup();
  ...
  assert.deepEqual(f.state().rules, [{id: '1', command: 'ping', delay: 12, parameters: ['Install'], deleteResponse: true}]);
});

test('preserves the reachable legacy merge boundary for generic and parameter rules', async () => {
  const same = fixture({schemaVersion: 2, enabled: false, rules: [{id: '1', command: 'ping', delay: 10}], pending: {}});
  await same.setup(); await same.run('add ping 10 install');
  assert.deepEqual(same.state().rules, [{id: '1', command: 'ping', delay: 10, parameters: ['install']}]);

  const different = fixture({schemaVersion: 2, enabled: false, rules: [{id: '1', command: 'ping', delay: 10}], pending: {}});
  await different.setup(); await different.run('add ping 20 install');
...
test('prefers exact no-argument rules regardless of list order', async () => {
  ...
  const f = fixture({schemaVersion: 2, enabled: true, rules: [
...
test('reset preserves already pending deletion work while disabling future matches', async () => {
  ...
  await f.setup(); await f.run('reset');
  assert.equal(f.state().enabled, false);
...
test('response selection uses the exact chat id, filters direction and future ids, and caps at three', async () => {
  let peer, options;
  const client = {async getMessages(value, params) {
    peer = value; options = params;
    return [{id: 15, out: false}, {id: 14, out: true}, {id: 13, out: true}, {id: 12, out: true}, {id: 11, out: true}, {id: 9, out: true}];
  }};
  const f = fixture({schemaVersion: 2, enabled: true,
    rules: [{id: '1', command: 'help', delay: 60, deleteResponse: true}], pending: {}}, {client});
  await f.plugin.listeners[0].handle({id: 10, chatId: '9007199254740993', senderId: '1', outgoing: true, saved: false, text: '.help'}, f.context);
  assert.equal(peer.value, 9007199254740993n);
  assert.deepEqual(options, {limit: 100});
  assert.deepEqual(f.tasks.map(task => task.label), [
    'autodelcmd:9007199254740993:14', 'autodelcmd:9007199254740993:13',
    'autodelcmd:9007199254740993:12', 'autodelcmd:9007199254740993:10',
  ]);
});
...
test('compiled plugin restores pending work and cancels it on unload', async t => {
  ...
  let deletes = 0;
  const host = new PluginHost({storageRoot: root, logger: {info() {}, error() {}}, telegram: {async edit() {}, async reply() {}, async invoke() {}, async getReply() {}, async withClient(op, signal) {return op({deleteMessages: async () => {deletes++;}}, signal);}}});
...
test('failed native deletion is sanitized and removed from persisted pending work', async () => {
  ...
  let peer;
  const client = {async deleteMessages(value) {
    peer = value;
    const error = new Error(secret); error.name = secretName;
    throw error;
  }};
...
test('real host postprocess can schedule the autodelcmd command itself when configured', async t => {
  ...
  const dir = path.join(root, 'autodelcmd'); await fs.mkdir(dir, {recursive: true});
  await fs.writeFile(path.join(dir, 'config.json'), JSON.stringify({schemaVersion: 2, enabled: true,
    rules: [{id: '1', command: 'autodelcmd', delay: 60}], pending: {}}));
