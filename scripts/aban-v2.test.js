...
const channel = (id = 100, rights = { banUsers: true }) =>
  new Api.Channel({
    id: integer(id),
    accessHash: integer(20),
    title: "Group " + id,
    megagroup: true,
    adminRights: new Api.ChatAdminRights(rights),
...
const normalize = value =>
  value === undefined
    ? undefined
    : JSON.parse(JSON.stringify(value, (_key, item) => (typeof item === "bigint" ? String(item) : item)));
const message = (args = ["2"], basic = false) => ({
  id: 9,
  message: ".ban " + args.join(" "),
  isGroup: true,
  isChannel: !basic,
...
return {UserResolver, GroupManager, BanManager, PermissionManager, CommandHandlers, parseTimeString};
    ...
    .replaceAll(
      /const lc[12] = getCurrentGenerationContext\(\);[\s\S]*?\n        }/g,
      "await MessageManager.smartEdit(status, result, 30);",
    )
    .replaceAll("backgroundProcess().catch(() => {});", "await backgroundProcess();");
  buildSync({
    stdin: { contents: code, loader: "ts", resolveDir: core },
    outfile: path.join(root, "oracle.cjs"),
    bundle: true,
    platform: "node",
    packages: "external",
  });
...
function environment(options = {}) {
  ...
  const base = {
    ...
    getEntity: async value => {
      if (options.missing) throw new Error("CACHE_MISS");
      return user(Number(value) || 2);
    },
    getInputEntity: async value => {
      if (options.missing && !(value instanceof Api.User)) throw new Error("CACHE_MISS");
      return input(Number(value.id ?? value));
    },
    getDialogs: async () =>
      (options.groups ?? [channel()]).map(entity => ({
        id: (entity instanceof Api.Channel ? "-100" : "-") + entity.id,
        entity,
        title: entity.title,
        isGroup: true,
    ...
    invoke: async request => {
      ...
      if (options.invoke) {
        const value = await options.invoke(request);
      ...
      if (request instanceof Api.channels.GetParticipant) {
        ...
        return {
          participant:
            self || options.admin
              ? new Api.ChannelParticipantCreator({ userId: integer(self ? 1 : 2) })
      ...
      if (request instanceof Api.messages.GetFullChat)
        return {
          fullChat: {
            participants: {
              participants: [
                new Api.ChatParticipantCreator({ userId: integer(1) }),
                new Api.ChatParticipant({ userId: integer(2) }),
              ],
            },
          },
          users: [user()],
  ...
  const client = new Proxy(base, {
    get(target, key) {
      return async (...args) => {
        ...
        if (
          key === "invoke" &&
          args[0] instanceof Api.channels.GetParticipant &&
          (args[0].participant instanceof Api.InputPeerSelf || args[0].participant?.toString() === "1")
        )
          recorded[0].participant = "self";
  ...
  const ctx = {
    ...
    storage: {
      json: () => ({
        read: async () => ({ cache: saved }),
        update: async fn => {
          const next = fn({ cache: saved });
          const copy = { ...next.cache };
          await cache.clear();
          Object.assign(saved, copy);
        },
      }),
    },
    telegram: {
      ...
      getReply: async () => reply && { senderId: String(reply.senderId), raw: reply },
    },
  };
  return { calls, edits, cache, client, ctx, reply, controller };
}
async function compare(options, run, { unordered = false } = {}) {
  const reference = environment(options),
    migrated = environment(options);
  const original = await oracle(reference),
    current = await createRuntime(migrated.ctx, { message: {}, args: [], command: "ban" });
  const expected = await run(original, reference),
    actual = await run(current, migrated);
...
test("reply resolves a reusable user input entity", async () => {
  const e = await compare({ reply: { senderId: integer(2), sender: user() } }, (r, e) =>
    r.UserResolver.resolveTarget(e.client, message([]), []),
  );
  ...
  await compare(
    {
      missing: true,
      invoke: async req => {
        if (req instanceof Api.channels.GetParticipant && req.participant instanceof Api.InputPeerUser)
          return { users: [] };
        if (req instanceof Api.channels.GetParticipants)
          return { participants: [{ userId: integer(2) }], users: [user()] };
      },
    },
...
test("numeric unresolved channel target uses original pre-ban participant", async () => {
  const e = await compare({ missing: true, lookupFails: true }, (r, e) =>
    r.UserResolver.resolveTarget(e.client, message(), ["2"]),
  );
...
test("batch partial failures and history cleanup match original", async () => {
  await compare(
    {
      groups: [channel(), channel(200)],
      invoke: async req => {
        if (req instanceof Api.channels.EditBanned && String(req.channel.channelId) === "200")
          throw new Error("PARTICIPANT_ID_INVALID");
      },
    },
...
test("a flooded group yields its lane and still gets banned on retry", async t => {
  ...
  let flooded = 0;
  const e = environment({
    groups: [channel(100), channel(200), channel(300)],
    invoke: async req => {
  ...
  const result = await r.BanManager.batchBanUser(
    e.client,
    [
...
test("unsb result and unsupported basic groups match original", async () => {
  await compare({ groups: [channel(), new Api.Chat({ id: integer(300), title: "Basic", creator: true })] }, (r, e) =>
    r.CommandHandlers.handleSuperUnban(e.client, message()),
  );
});
const deferred = () => {
  let resolve;
...
async function hostFixture(t, options = {}) {
  const e = environment(options),
    dir = await fs.mkdtemp(path.join(root, "host-"));
  const host = new PluginHost({
    storageRoot: dir,
    logger: { info() {}, error() {} },
    telegram: {
      edit: async (_msg, text, _opts, signal) => {
        signal.throwIfAborted();
        e.edits.push(text);
        await options.onEdit?.(_msg, text);
      },
      reply: async () => assert.fail("unexpected reply"),
      getReply: async () => undefined,
      invoke: async () => assert.fail("unexpected direct RPC"),
      withClient: async (fn, signal) => fn(e.client, signal),
    },
  });
  ...
  const send = (text, id = 9, extra = {}) =>
    host.dispatchPrimary({ id, chatId: "-100100", senderId: "1", text, outgoing: true, ...extra });
  return { ...e, host, send, dir };
}
test("factory commands, help and edited-message protection", async t => {
  assert.deepEqual(Object.keys(factory().commands).sort(), [
    "aban",
    "ban",
    "kick",
    "mute",
...
test("unload drains in-flight batch and stops queued native calls", async t => {
  ...
  const e = await hostFixture(t, {
    groups: Array.from({ length: 9 }, (_, i) => channel(100 + i)),
    invoke: async req => {
...
test("managed groups exclude migrated, deactivated and left dialogs", async () => {
  const e = environment({
    groups: [
      ...
      new Api.Chat({
        ...
        title: "Migrated",
        creator: true,
        migratedTo: new Api.InputChannel({ channelId: integer(100), accessHash: integer(20) }),
      }),
  ...
  const r = await createRuntime(e.ctx, {});
  assert.deepEqual(
    (await r.GroupManager.getManagedGroups(e.client)).map(g => g.id),
    [100, 304],
  );
});
test("mixed group cache persists through the real host JSON store", async t => {
  const entered = deferred();
  const e = await hostFixture(t, {
    groups: [channel(), new Api.Chat({ id: integer(300), title: "Basic", creator: true })],
    onEdit: async (_msg, text) => {
      if (text.startsWith("✅ 在")) entered.resolve();
    },
  });
  await e.send(".sb 2");
  await entered.promise;
...
test("stale-while-revalidate serves old groups without blocking on a refresh", async () => {
  ...
  await e.cache.set("managed_groups_v7", {
...
test("RPC errorMessage appears separately for batch bans and current history cleanup", async () => {
  const e = environment({
      groups: [channel(), new Api.Chat({ id: integer(300), title: "Basic", creator: true })],
      invoke: async req => {
        if (req instanceof Api.messages.DeleteChatUser)
          throw Object.assign(new Error("The provided chat id is invalid."), { errorMessage: "CHAT_ID_INVALID" });
        if (req instanceof Api.channels.DeleteParticipantHistory)
          throw Object.assign(new Error("Admin rights required."), { errorMessage: "CHAT_ADMIN_REQUIRED" });
      },
    }),
    r = await createRuntime(e.ctx, {});
  await r.CommandHandlers.handleSuperBan(e.client, message());
...
test("missing delete permission is reported without a history mutation", async () => {
  ...
  assert.deepEqual(await r.BanManager.deleteHistoryInCurrentChat(e.client, message().peerId, 2, input()), {
    success: false,
    reason: "DELETE_MESSAGES_PERMISSION_REQUIRED",
  });
  assert.ok(!e.calls.some(c => c.args[0]?.className === "channels.DeleteParticipantHistory"));
});
test("history cleanup stops between pages when unloaded", async () => {
  const e = environment({
    ...
    r = await createRuntime(e.ctx, {});
  await assert.rejects(r.BanManager.deleteHistoryInCurrentChat(e.client, message().peerId, 2, input()), {
    name: "AbortError",
  });
