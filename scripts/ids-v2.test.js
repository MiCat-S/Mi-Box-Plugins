...
function user(fields = {}) {
  return new Api.User({
    ...
    firstName: "Alice",
    username: "alice",
    photo: new Api.UserProfilePhoto({ photoId: integer(1), dcId: 4 }),
    ...fields,
  });
}
...
async function fixture(t, { native = {}, reply, hostOptions = {}, onEdit, onReply } = {}) {
  ...
  const handlers = {
    ...
    getEntity: async value =>
      typeof value === "string" && value.startsWith("@") ? user() : user({ id: integer(value.toString()) }),
    getInputEntity: async value => {
      const text = value?.channelId?.toString?.() ?? value?.toString?.() ?? "";
      if (value?.channelId !== undefined || text.startsWith("-100"))
        return new Api.InputPeerChannel({
          channelId: value.channelId ?? integer(text.slice(4)),
          accessHash: integer(456),
        });
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async invoke() {
        assert.fail("native requests must use withClient");
      },
    },
    ...hostOptions,
  });
...
test("ids exact decimal/hex IDs preserve parseInt-compatible prefixes without rounding", async t => {
  for (const [input, id] of [
    ...
    ["-1009007199254740993", "-1009007199254740993"],
    ["123tail", "123"],
    ["+123", "123"],
...
test("ids rejects a mismatched numeric entity without borrowing its access hash or full profile", async t => {
  ...
  assert.equal(
    f.calls.some(call => call.method === "invoke"),
    false,
  );
});
test("ids deleted or uncached reply sender never falls back to owner", async t => {
  for (const raw of [undefined, { sender: new Api.UserEmpty({ id: integer("9007199254740993") }) }]) {
    const f = await fixture(t, {
      reply: { ...envelope, senderId: "9007199254740993", raw },
      native: {
        invoke: async () => {
          throw new Error("USER_ID_INVALID secret");
        },
        getMe: async () => assert.fail("must not query owner"),
      },
    });
...
test("ids missing self/auth key keeps progress, unexpected auth errors are sanitized", async t => {
  for (const getMe of [
    async () => undefined,
    async () => new Api.UserEmpty({ id: integer(123) }),
    async () => {
      throw new Error("AUTH_KEY_UNREGISTERED private-token");
    },
  ]) {
  ...
  const f = await fixture(t, {
    native: {
      getMe: async () => {
        throw new Error("proxy://secret:private-token@host");
      },
    },
  });
  await f.run();
...
test("ids photo-empty, photo-absent and empty full result retain separate DC fallbacks", async t => {
  for (const [response, expected] of [
    [full(user({ photo: new Api.UserProfilePhotoEmpty() })), "无头像"],
    [full(user({ photo: undefined })), "未知"],
    [{ users: [], fullUser: undefined }, "未知"],
  ]) {
              ...
              getInputEntity: async value => {
                if (value instanceof Api.InputChannel || value instanceof Api.PeerChannel)
                  return new Api.InputPeerChannel({
                    ...
                    accessHash: value.accessHash ?? integer(456),
                  });
                if (value instanceof Api.InputUser)
                  return new Api.InputPeerUser({ userId: value.userId, accessHash: value.accessHash });
...
test("ids generated HTML pages preserve entities, UTF-16, closing tags and oversized single href", async t => {
  for (const username of ["alice", "u".repeat(3600), "&".repeat(5000)]) {
    ...
    const f = await fixture(t, {
      native: { getMe: async () => profile, invoke: async () => full(profile, "a".repeat(199) + "😀tail") },
    });
    await f.run();
...
test("ids preserves the first result page when a later reply fails", async t => {
  ...
  const f = await fixture(t, {
    native: { getMe: async () => profile, invoke: async () => full(profile) },
    onReply: async () => {
  ...
  assert.deepEqual(f.logs.at(-1), {
    event: "ids_delivery_interrupted",
    fields: { published: 1, total: 4, category: "Error" },
  });
});
for (const method of ["getMe", "getEntity", "invoke"]) {
  for (const rejects of [false, true])
    test("ids cancellation waits for " + method + " settlement (" + rejects + ")", async t => {
      ...
      let settled = false;
      const running = f.run(method === "getEntity" ? ".ids @alice" : ".ids").finally(() => {
        settled = true;
      });
