...
test("whois preserves all raw characters in independently valid rich-text pages", () => {
  ...
  const restored = pages
    .slice(1)
    .map(page => page.replace(/^<blockquote expandable>|<\/blockquote>$/g, ""))
    .join("");
...
test("whois expiry handles invalid dates and alternate expiry field", () => {
  ...
  assert.match(
    report("example.com", "Registrar Registration Expiration Date: 2020-01-01", Date.parse("2026-09-06"))[0],
    /已过期/,
  );
});
