...
function centralConfig() {
  return {
    configs: {
      main: {
        tag: "main",
        url: "https://central.example.test/v1",
        key: "central-secret",
      ...
      "sum-legacy": {
        tag: "sum-legacy",
        url: "https://occupied.example.test/v1",
        key: "occupied-secret",
        type: "openai-compatible",
        stream: false,
        responses: false,
        models: { chat: "occupied-model" },
      },
    },
    currentChatTag: "main",
...
test("sum migrates legacy providers into ai, remaps task tags, and calls the unified chat service", async t => {
  ...
  await fs.writeFile(
    path.join(root, "sum", "database.json"),
    JSON.stringify({
      seq: "1",
      tasks: [
        {
  ...
  const client = {
    async getEntity() {
      return { title: "测试群", username: "testgroup" };
    },
    async *iterMessages() {
      yield {
        id: 11,
        ...
        media: {
          ...
          document: { attributes: [{ className: "DocumentAttributeFilename", fileName: "sticker.webp" }] },
        },
        sender: { firstName: "丙" },
        senderId: 3,
      };
...
test("sum preserves the legacy Responses-to-Chat fallback after provider migration", async t => {
  ...
  const client = {
    ...
    async *iterMessages() {
      yield { id: 9, message: "待总结消息", sender: { firstName: "甲" }, senderId: 1 };
    },
    async sendMessage(peer, value) {
      sent.push({ peer, value });
    },
    ...
    http: {
      fetch: async url => {
        ...
        if (target.pathname === "/v1/responses")
          return Response.json({ error: { message: "unsupported endpoint" } }, { status: responsesStatus });
...
test("sum loads when legacy default providers have empty keys and scrubs unusable entries", async t => {
  ...
  assert.deepEqual(
    events.filter(item => item.event === "sum:legacy-provider-skipped"),
