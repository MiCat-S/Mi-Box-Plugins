...
test('subinfo keeps legacy cha alias, batch URLs and extended regions', async () => {
  const edits = [], replies = [], fetched = [];
  const context = {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume){fetched.push(url);return consume(new Response(`ss://x#${url.endsWith('/a')?'HKG':'Malaysia'}`),new AbortController().signal);}},telegram:{async edit(_,text){edits.push(text);},async reply(_,text){replies.push(text);},async getReply(){}}};
  await create().commands.cha.handle({args:['https://example.com/a','https://example.com/b'],message:{id:1,chatId:'1',text:'',outgoing:true}},context);
  assert.deepEqual(fetched.filter(url=>/\/(?:a|b)$/.test(url)),['https://example.com/a','https://example.com/b']);
  assert.match(edits.at(-1),/节点总数/);
  assert.match(edits.at(-1),/HKG/);
  assert.match(replies.join(''),/Malaysia/);
  const detailed=[];
  await create().commands.subinfo.handle({args:['https://example.com/a'],message:{id:2,chatId:'1',text:'',outgoing:true}}, {...context,telegram:{...context.telegram,async edit(_,text){detailed.push(text);},async reply(_,text){detailed.push(text);}}});
  assert.match(detailed.join(''),/香港: 1/);
});

test('subinfo preserves the first page when a continuation fails', async () => {
  const edits=[], replies=[], logs=[];
  const names=Array.from({length:80},(_,i)=>`node-${i}-${'x'.repeat(100)}`);
  await create().commands.subinfo.handle({args:['https://example.com/sub'],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(name,data){logs.push({name,data});}},http:{async withResponse(url,init,consume){return consume(new Response(names.map(n=>`ss://x#${n}`).join('\n')),new AbortController().signal);}},telegram:{async edit(_,text){edits.push(text);},async reply(_,text){replies.push(text);if(replies.length===1)throw new Error('SECRET continuation');},async getReply(){}}});
...
test('subinfo txt uploads a report to the exact rawless peer before deleting command', async () => {
  const calls=[];
  const client={async sendFile(peer,options){calls.push(['send',peer.toString(),options]);},async deleteMessages(peer,ids){calls.push(['delete',peer.toString(),ids]);}};
  await create().commands.cha.handle({args:['txt','https://example.com/sub'],message:{id:7,chatId:'9007199254740997',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume){return consume(new Response('ss://x#A%26B%2F%3Cx%3E'),new AbortController().signal);}},telegram:{async edit(){},async reply(){},async getReply(){},async withClient(use){return use(client,new AbortController().signal);}}});
  assert.deepEqual(calls.map(c=>c.slice(0,2)),[['send','9007199254740997'],['delete','9007199254740997']]);
...
test('subinfo cancellation actively cancels and unlocks a hanging response body', async () => {
  ...
  let cancelled=false,lockedDuringCancel=false;
  const body=new ReadableStream({pull(){return new Promise(()=>{});},cancel(){cancelled=true;lockedDuringCancel=body.locked;}});
  const work=create().commands.subinfo.handle({args:['https://example.com/sub'],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:controller.signal,log:{error(){}},http:{async withResponse(url,init,consume){return consume(new Response(body),controller.signal);}},telegram:{async edit(){},async reply(){},async getReply(){}}});
...
test('subinfo batch keeps successful reports when one subscription fails', async () => {
  const pages=[];
  await create().commands.subinfo.handle({args:['https://example.com/bad','https://example.com/good'],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume){if(url.endsWith('/bad'))throw new Error('SECRET upstream');return consume(new Response('ss://x#Tokyo'),new AbortController().signal);}},telegram:{async edit(_,text){pages.push(text);},async reply(_,text){pages.push(text);},async getReply(){}}});
...
test('subinfo restores bounded mapping and same-origin website discovery without forwarding subscription paths', async () => {
  ...
  await create().commands.subinfo.handle({args:[subscription],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume,options){calls.push({url:String(url),init,options});let response;if(String(url).includes('raw.githubusercontent.com'))response=new Response('panel.example=Mapped Airport');else if(String(url)==='https://panel.example/auth/login')response=new Response('missing',{status:404});else if(String(url)==='https://panel.example/')response=new Response('<title>登录 — Site Airport</title>');else response=new Response('ss://x#Tokyo',{headers:{'content-disposition':'attachment; filename=Header Airport'}});return consume(response,new AbortController().signal);}},telegram:{async edit(_,text){pages.push(text);},async reply(_,text){pages.push(text);},async getReply(){}}});
  assert.match(pages.at(-1),/Mapped Airport/);
  assert.deepEqual(calls.slice(0,3).map(call=>call.url),[
    'https://raw.githubusercontent.com/Hyy800/Quantumult-X/refs/heads/Nana/ymys.txt','https://panel.example/auth/login','https://panel.example/'
  ]);
...
test('subinfo paginates long identity fields even when the subscription has no nodes', async () => {
  ...
  await create().commands.subinfo.handle({args:['https://panel.example/sub'],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume){let response;if(String(url).includes('raw.githubusercontent.com'))response=new Response(`panel.example=${longName}`);else if(String(url)==='https://panel.example/auth/login')response=new Response(`<title>${'T'.repeat(5000)}</title>`);else response=new Response('plain text with no nodes');return consume(response,new AbortController().signal);}},telegram:{async edit(_,text){pages.push(text);},async reply(_,text){pages.push(text);},async getReply(){}}});
  assert.ok(pages.length>1);
...
test('subinfo no-URL help uses the invocation prefix and documents every mode', async () => {
  const pages=[];
  await create().commands.subinfo.handle({args:[],prefix:'!!',message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},telegram:{async edit(_,text){pages.push(text);},async reply(_,text){pages.push(text);},async getReply(){}}});
...
test('subinfo paginates a heavily escaped URL in a batch failure', async () => {
  ...
  await create().commands.subinfo.handle({args:[long,'https://example.com/bad'],message:{id:1,chatId:'1',text:'',outgoing:true}},
    {signal:new AbortController().signal,log:{error(){}},http:{async withResponse(url,init,consume){if(String(url).includes('raw.githubusercontent.com'))return consume(new Response(''),new AbortController().signal);if(String(url).endsWith('/auth/login'))return consume(new Response('<title>Panel</title>'),new AbortController().signal);throw new Error('fixed failure');}},telegram:{async edit(_,text){pages.push(text);},async reply(_,text){pages.push(text);},async getReply(){}}});
  assert.ok(pages.length>1);
  assert.ok(pages.every(page=>page.length<=4096));
