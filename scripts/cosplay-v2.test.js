...
async function fixture(t, options = {}) {
  ...
  const client = {
    async getInputEntity(value) {
      ...
      if (value instanceof Api.PeerUser)
        return new Api.InputPeerUser({ userId: value.userId, accessHash: integer(77) });
      return value;
    },
    ...
    async invoke(request) {
      ...
      if (request instanceof Api.messages.UploadMedia) {
        ...
        return new Api.MessageMediaPhoto({
          photo: new Api.Photo({
            id: integer(100 + index),
            accessHash: integer(200 + index),
  ...
  const raw = {
    ...
    async delete() {
      if (options.deleteFailure) {
        const error = new Error("BOT_TOKEN=delete-secret");
        error.name = "sk-live-delete-secret/etc/passwd";
        throw error;
      }
    },
  };
  return {
    ...
    run(count = "2") {
      ...
      return host.dispatchPrimary({ id: 9, chatId: "42", senderId: "42", outgoing: true, text: `🙂cos${suffix}`, raw });
    },
  };
}
...
test("cosplay sends multiple images as one spoiler album with complete progress", async t => {
  ...
  assert.deepEqual(
    albums[0].request.multiMedia.map(item => item.message),
    ["套图链接: https://cosplaytele.com/set-one/", ""],
  );
...
test("cosplay falls back to spoiler singles only after an album failure", async t => {
  ...
  await f.run("2");
  assert.equal(
    f.wire.some(item => item.request instanceof Api.messages.SendMultiMedia),
    true,
  );
...
test("real host unload actively cancels and releases a hung HTTP reader", async t => {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async getReply() {},
      async withClient() {
        assert.fail("no Telegram send expected");
      },
    },
  });
  t.after(async () => {
