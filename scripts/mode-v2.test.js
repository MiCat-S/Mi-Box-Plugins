...
const {artifactDir} = buildPlugin({id: 'mode', packageRoot: process.env.MODE_TEST_SOURCE || path.resolve(__dirname, '../mode'), entry: 'v2.ts'});
...
async function fixture(t, options = {}) {
  ...
  const edits = [], replies = [], parseModes = [], logs = [];
  const host = new PluginHost({storageRoot: root, logger: {info(event) {logs.push(event);}, error(event) {logs.push(event);}}, telegram: {
    async edit(message, text, settings) {if (options.editError) throw new Error('private edit failure'); edits.push(text); parseModes.push(settings?.parseMode);},
    async reply(_message, text) {if(options.replyError)throw new Error('private page failure');replies.push(text);},
    async withClient() {assert.fail('unexpected client');},
  }});
  let loadError; try {await host.load(create());} catch (error) {loadError=error; if(!options.loadError) throw error;}
  t.after(async () => {
    assert.equal((await host.shutdown(1000)).completed, true);
    await fs.rm(root, {recursive: true, force: true});
  });
  return {edits, replies, parseModes, logs, root, host, loadError, listen: patch => host.dispatchListeners({
...
test('mode migrates legacy fields while explicit V2 fields, including off and empty lists, win', async t => {
  const f = await fixture(t, {legacy: {chats: {'1':'bold'}, whitelist:['legacy'], blacklist:['blocked'], globalMode:'italic',legacyExtra:{keep:1}},
...
test('mode emits a real spoiler entity and logs only a fixed event when editing fails', async t => {
  ...
  await fs.writeFile(path.join(broken.root, 'mode/state-v2.json'), JSON.stringify({schemaVersion:1,legacyImported:true,chats:{'1':'bold'},whitelist:[],blacklist:[],globalMode:'off'}));
  await broken.listen({text:'plain'});
...
test('mode paginates complete long lists and rejects non-object migration files', async t => {
  ...
  assert.match(output,new RegExp(values[0]));assert.match(output,new RegExp(values.at(-1)));
  const interrupted=await fixture(t,{legacy:{chats:{},whitelist:values,blacklist:[],globalMode:'off'},replyError:true});
