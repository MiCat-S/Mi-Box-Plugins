...
test.before(async () => {
  ...
  const built = buildPlugin({id: 'acron', packageRoot: path.resolve(__dirname, '../acron'), entry: 'v2.ts', rootDir: core});
  factory = require(path.join(built.artifactDir, built.manifest.entry)).default;
  buildSync({entryPoints: [path.resolve(__dirname, '../acron/v2/tasks.ts')],
    outfile: path.join(root, 'tasks.cjs'), bundle: true, platform: 'node', packages: 'external'});
...
const replyRaw = (entities, extra = {}) => new Api.Message({
  id: 5, peerId: new Api.PeerChannel({channelId: returnBigInt(100)}), message: 'bold text', entities, ...extra,
});
...
function createHarness(initial) {
  ...
  const makeEnv = (options = {}) => {
    ...
    const edits = [], replies = [], logs = [], sends = [], deletes = [], pins = [], unpins = [], forwards = [], reads = [], dispatches = [];
    const jobs = new Map();
    const safeRegexp = new ScopedSafeRegExp();
    const store = {
      read: async () => clone(state.data),
      update: async mutator => { state.data = clone(await mutator(clone(state.data))); return clone(state.data); },
    };
    const client = {
      sendMessage: async (target, payload) => { sends.push({target, payload}); return {id: 1}; },
      getMessages: async (target, params) => { reads.push({target, params}); return options.messages ?? []; },
      deleteMessages: async (target, ids, opts) => { deletes.push({target, ids, opts}); },
      pinMessage: async (target, id, opts) => { pins.push({target, id, opts}); },
      unpinMessage: async (target, id) => { unpins.push({target, id}); },
      invoke: async request => { forwards.push(request); return {}; },
    ...
    const ctx = {
      ...
      storage: {json: () => store},
      jobs: {register: async (id, spec, handler) => { jobs.set(id, {spec, handler}); return async () => { jobs.delete(id); }; }},
      commands: {
        parse: () => undefined,
        dispatch: async sent => {
      ...
      regexp: {test: (pattern, input, opts) => options.regexp
...
const invocation = (text, chatId = CHAT) => ({
  ...
  command: 'acron', prefix: '.',
  args: text.replace(/^\.acron\b\s*/, '').trim().split(/\s+/).filter(Boolean),
});
...
test('send stores TL entities and resends them after reload with intact long fields', async () => {
  ...
  const created = harness.makeEnv({reply: {id: 5, chatId: CHAT, text: 'bold text',
    raw: replyRaw([new Api.MessageEntityBold({offset: 0, length: 4}),
...
test('send rejects media and replyMarkup instead of silently sending plain text', async () => {
  const harness = createHarness();
  const env = harness.makeEnv({reply: {id: 5, chatId: CHAT, text: 'caption',
    raw: replyRaw(undefined, {media: new Api.MessageMediaPhoto({})})}});
  const definition = factory();
  ...
  const buttons = harness.makeEnv({reply: {id: 5, chatId: CHAT, text: 'buttons',
...
test('copy re-sends the source message with its media and entities', async () => {
  const source = new Api.Message({id: 7, peerId: new Api.PeerChannel({channelId: returnBigInt(900)}),
    message: 'caption', media: new Api.MessageMediaPhoto({}),
...
test('forward passes topic topMsgId and precise peers', async () => {
  const source = new Api.Message({id: 7, peerId: new Api.PeerChannel({channelId: returnBigInt(900)}), message: 'x'});
  const harness = createHarness();
...
test('list accepts lowercase all/type arguments and orders enabled before disabled', async () => {
  const harness = createHarness({schemaVersion: 1, seq: '3', tasks: [
    {id: '1', type: 'send', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', disabled: true},
    {id: '2', type: 'del', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', msgId: '11'},
    {id: '3', type: 'del', cron: '0 0 2 * * *', chat: '-100900', chatId: '-100900', resolvedPeer: true, createdAt: '0', msgId: '12'},
  ]});
  const env = harness.makeEnv();
  const definition = factory();
  await definition.setup(env.ctx);
...
test('del_re rejects missing regex and out-of-range limits with no task and no deletion', async () => {
  ...
  for (const bad of [`${CHAT} 0 /x/`, `${CHAT} -1 /x/`, `${CHAT} 0.5 /x/`, `${CHAT} 1001 /x/`, `${CHAT} x /x/`, `${CHAT} 10 not-a-regex(`, `${CHAT} 10 /x/ii`]) {
  ...
  assert.ok(env.edits.some(text => text.includes('1-1000')), 'fixed range prompt');
...
test('del_re accepts only in-range integer limits and stores them exactly', async () => {
  ...
  assert.deepEqual(harness.state.data.tasks.map(task => task.limit), ['1', '1000']);
});

test('old out-of-range del_re rows are rejected without clamping or deleting', async () => {
  const dirty = {schemaVersion: 1, seq: '1', tasks: [{id: '1', type: 'del_re', cron: '0 0 2 * * *',
    chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', limit: '1001', regex: '/x/', delivery: 'pending'}]};
...
test('old dirty del_re rows are re-validated at execution with zero deletion', async () => {
  const dirty = {schemaVersion: 1, seq: '1', tasks: [{id: '1', type: 'del_re', cron: '0 0 2 * * *',
    chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', limit: '10', regex: '', delivery: 'pending'}]};
  const harness = createHarness(dirty);
  const env = harness.makeEnv({messages: [{id: 8, message: 'unrelated text'}]});
  const definition = factory();
  await definition.setup(env.ctx);
...
test('del_re goes through the managed regexp worker and aborts with zero deletion on timeout', async () => {
  const good = {schemaVersion: 1, seq: '1', tasks: [{id: '1', type: 'del_re', cron: '0 0 2 * * *',
    chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', limit: '5', regex: '/secret/i', delivery: 'pending'}]};
  const harness = createHarness(good);
  const env = harness.makeEnv({messages: [{id: 1, message: 'SECRET'}, {id: 2, message: 'nope'}]});
  const definition = factory();
  await definition.setup(env.ctx);
  await runJob(env, '1');
  ...
  const slow = timeout.makeEnv({messages: [{id: 8, message: 'SECRET'}], regexp: () => ({matched: false, timedOut: true})});
...
test('sticky del_re patterns differ from unanchored matches', async () => {
  const task = regex => ({schemaVersion: 1, seq: '1', tasks: [{id: '1', type: 'del_re', cron: '0 0 2 * * *',
    chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', limit: '5', regex, delivery: 'pending'}]});
...
test('d and v flags run through the managed worker', async () => {
  for (const regex of ['/abc/d', '/abc/v']) {
    const harness = createHarness({schemaVersion: 1, seq: '1', tasks: [{id: '1', type: 'del_re', cron: '0 0 2 * * *',
      chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', limit: '5', regex, delivery: 'pending'}]});
...
test('list renders target, links, next/last times, results and rebuild commands', async () => {
  const tasks = [{id: '1', type: 'send', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true,
    createdAt: '0', replyTo: '44', remark: 'note', lastRunAt: '1700000000000', lastResult: '已发送 1 条消息'}];
...
test('enable with an invalid cron leaves the task disabled and unregistered', async () => {
  const harness = createHarness({schemaVersion: 1, seq: '1', tasks: [
    {id: '1', type: 'send', cron: 'bad cron', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', disabled: true}]});
  const env = harness.makeEnv();
  const definition = factory();
  await definition.setup(env.ctx);
  await dispatch(definition, env, '.acron enable 1');
...
test('enable keeps the task disabled when registration fails', async () => {
  const harness = createHarness({schemaVersion: 1, seq: '1', tasks: [
    {id: '1', type: 'send', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', disabled: true}]});
  const env = harness.makeEnv();
  env.ctx.jobs.register = async () => { throw new Error('Invalid scheduled job'); };
  const definition = factory();
  await definition.setup(env.ctx);
  await dispatch(definition, env, '.acron enable 1');
  assert.equal(harness.state.data.tasks[0].disabled, true);
...
test('long task lists paginate instead of truncating', async () => {
  const tasks = Array.from({length: 40}, (_, index) => ({id: String(index + 1), type: 'del',
    cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', msgId: String(1000 + index)}));
...
test('setup skips invalid cron rows without blocking valid tasks or deleting data', async () => {
  const harness = createHarness({schemaVersion: 1, seq: '3', tasks: [
    ...
    {id: '2', type: 'send', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', disabled: true},
    {id: '3', type: 'del', cron: '0 0 2 * * *', chat: CHAT, chatId: CHAT, resolvedPeer: true, createdAt: '0', msgId: '5'},
  ]});
  const env = harness.makeEnv();
  const definition = factory();
  await definition.setup(env.ctx);
...
test('setup continues when jobs.register rejects one task', async () => {
  const harness = createHarness({schemaVersion: 1, seq: '2', tasks: [
...
test('a cron cmd task routes the sent command into the real host and runs the target handler', async t => {
  ...
  const client = {
    ...
    sendMessage: async (_target, payload) => new Api.Message({id: 42, out: true, message: payload.message,
      peerId: new Api.PeerChannel({channelId: returnBigInt(100)})}),
    getMessages: async () => [], deleteMessages: async () => {}, pinMessage: async () => {},
    unpinMessage: async () => {}, invoke: async () => ({}),
  };
  const host = new PluginHost({
    storageRoot: root, selfId: '1', prefixes: ['.'], logger: {info() {}, error() {}},
    envelope: message => {
    ...
    telegram: {
      ...
      withClient: async (operation, signal) => { signal.throwIfAborted(); return operation(client, signal); },
    },
  });
  t.after(async () => {
  ...
  await host.load(definePlugin({apiVersion: 1, id: 'target', description: 'target',
    commands: {ping: {description: 'ping', handle() { pingRuns += 1; }}}}));
...
test('next run uses Asia/Shanghai regardless of the process timezone', async () => {
  ...
  const hour = Number(new Intl.DateTimeFormat('en-GB', {timeZone: 'Asia/Shanghai', hour: '2-digit', hour12: false}).format(next));
