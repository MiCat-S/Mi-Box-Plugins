...
function create(options) {
  ...
  return require(entry).default(options);
}
const plugin = () => create();
...
test("da admin flow serializes permission RPC, reports completion, and removes completed state", async t => {
  ...
  const deletions = [],
    ...
    requests = [];
  const input = value =>
    value instanceof Api.InputPeerSelf
      ? value
  ...
  const client = {
    ...
    async invoke(request) {
      ...
      if (request instanceof Api.channels.GetParticipant)
        return { participant: new Api.ChannelParticipantCreator({ userId: me.id }) };
    ...
    async sendMessage(target, options) {
      saved.push({ target, options });
      return { id: 500 };
    },
    async editMessage(target, options) {
      edited.push({ target, options });
    },
  };
  ...
  await host.dispatchPrimary({
    ...
    outgoing: true,
    text: ".da true",
    raw: { peerId: peer, isPrivate: false },
  });
  await waitFor(async () => {
    try {
      return (
        JSON.parse(await fs.readFile(path.join(root, "da", "database.json"), "utf8")).tasks.length === 0 &&
        edited.length > 0
      );
    } catch {
      return false;
...
test("da stop cancels an in-flight iterator before deletion and persists a paused task", async t => {
  ...
  const client = {
    ...
    async *iterMessages() {
      iteratorStarted();
      await release;
      yield new Api.Message({ id: 21, message: "late" });
    },
  ...
  await host.dispatchPrimary({
    ...
    outgoing: true,
    text: ".da true",
    raw: { peerId: chat, isPrivate: false },
  });
  await started;
  await host.dispatchPrimary({
    id: 11,
...
async function floodFixture(t, error, pause) {
  ...
  const client = {
    ...
    async *iterMessages() {
      yield { className: "Message", id: 21 };
    },
    async deleteMessages(_peer, ids) {
      if (ids.includes(21)) {
        batchCalls += 1;
        if (batchCalls === 1) throw error;
      }
    },
    async sendMessage() {
      return { id: 500 };
...
test("da preserves a normal short FLOOD_WAIT before one retry", async t => {
  ...
  const fixture = await floodFixture(
    t,
    Object.assign(new Error("private"), { errorMessage: "FLOOD_WAIT_1" }),
    async (ms, signal) => {
...
test("da segments a FLOOD_WAIT beyond the native timer range and stop prevents retry", async t => {
  ...
  const fixture = await floodFixture(
    t,
    Object.assign(new Error("private"), { errorMessage: "FLOOD_WAIT_2147484" }),
    (ms, signal) =>
  ...
  await waitFor(
    async () =>
      JSON.parse(await fs.readFile(path.join(fixture.root, "da", "database.json"), "utf8")).tasks[0]?.isPaused === true,
  );
...
test("da does not retry an unsafe FLOOD_WAIT value", async t => {
  ...
  const fixture = await floodFixture(
    t,
    Object.assign(new Error("private"), { errorMessage: "FLOOD_WAIT_999999999999999999999" }),
    async ms => {
      waits.push(ms);
    },
  );
  ...
  await waitFor(
    async () =>
      JSON.parse(await fs.readFile(path.join(fixture.root, "da", "database.json"), "utf8")).tasks[0]?.isRunning ===
      false,
  );
