...
test('real Host routes dynamic-prefix help without starting a toss',async t=>{const root=await fs.promises.mkdtemp(path.join(require('node:os').tmpdir(),'zhijiao-host-')),edits=[];const host=new PluginHost({storageRoot:root,prefixes:['!!'],logger:{info(){},error(){}},telegram:{async edit(_m,text){edits.push(text);},async reply(){},async invoke(){},async getReply(){},async withClient(){}}});await host.load(create());t.after(async()=>{await host.shutdown(1000);await fs.promises.rm(root,{recursive:true,force:true});});await host.dispatchPrimary({id:1,chatId:'1',senderId:'1',outgoing:true,text:'!!zhijiao help',raw:{}});assert.equal(edits.length,1);assert.match(edits[0],/!!zhijiao/);});
