...
const {artifactDir} = buildPlugin({id: 'keep_online', packageRoot: path.resolve(__dirname, '../keep_online'), entry: 'v2.ts'});
...
async function fixture(t, {getMe = async () => ({id: 9007199254740993n}), prefixes = ['.'], root} = {}) {
  ...
  const host = new PluginHost({storageRoot: root, prefixes, logger: {
    ...
    async withClient(operation, signal) { calls.push({signal}); return operation({getMe}, signal); },
  }});
  ...
  return {
    root, host, edits, logs, calls,
    close: async timeout => { const result = await host.shutdown(timeout); closed = result.completed; return result; },
    run: text => host.dispatchPrimary({id: 1, chatId: '9007199254740993', senderId: '9007199254740993', outgoing: true, text}),
    fire: async () => {
...
test('failed probes neither advance status nor expose client failures', async t => {
  ...
  const f = await fixture(t, {getMe: async () => { throw new Error(secret); }});
  await f.fire();
  await f.run('.keep_online');
  assert.match(f.edits.at(-1).text, /等待首次探测/);
...
test('unload waits for an active probe, cancels it and prevents a late timestamp', async t => {
  ...
  const f = await fixture(t, {getMe: async () => { started.resolve(); await release.promise; return {id: 1n}; }});
  const firing = f.fire();
