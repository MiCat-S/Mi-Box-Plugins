...
test.before(async () => {
  ...
  esbuild.buildSync({
    ...
    outfile: path.join(buildRoot, "host.cjs"),
  });
  metadata = esbuild.buildSync({
    ...options,
    entryPoints: [path.resolve(__dirname, "../nodeseek/v2.ts")],
    outfile: path.join(buildRoot, "nodeseek.cjs"),
    alias: { "telebox/sdk": path.join(core, "src/v2/sdk.ts") },
    metafile: true,
  }).metafile;
...
async function fixture(
  t,
  { data, config, fetch = successful, process: runProcess, wait, edit, send, factory, hostOptions } = {},
) {
  ...
  t.mock.method(fs, "access", async (file, ...args) => {
    if (!String(file).startsWith(root + path.sep))
      throw Object.assign(new Error("fixture executable absent"), { code: "ENOENT" });
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async getReply() {
        assert.fail("unexpected reply read");
      },
      async invoke() {
  ...
  return {
    ...
    run: text =>
      host.dispatchPrimary({
        ...envelope,
        text,
        ...(text.toLowerCase().startsWith(".nodeseek set ") ? { saved: true } : {}),
      }),
...
test("pure synchronous factory and lazy load use only candidate/helper/SDK source", async t => {
  const first = createNodeSeek(),
    second = createNodeSeek();
  ...
  assert.equal(typeof first.setup, "function");
  assert.equal(first.cleanup, undefined);
  assert.deepEqual(first.resources.processes, {
    concurrency: 1,
    queueCapacity: 1,
    timeoutMs: 30000,
  ...
  await fs.writeFile(
    ...
    JSON.stringify({
      compilerOptions: {
        ...
        types: ["node"],
        typeRoots: [path.join(core, "node_modules/@types")],
...
test("cookie writes are Saved Messages only and legacy panel cookie migrates into signing state", async t => {
  const denied = await fixture(t);
  await denied.host.dispatchPrimary({ ...envelope, text: ".nodeseek set session=not-safe-outside-saved" });
  assert.match(denied.edits.at(-1).text, /仅在收藏夹/);
  assert.deepEqual(await fs.readdir(denied.root), []);
  const migrated = await fixture(t, { config: { cookie, interval: 5 }, fetch: successful });
  assert.equal((await migrated.read()).cookie, cookie);
  assert.equal((await migrated.read("config.json")).cookie, undefined);
  await migrated.run(".nodeseek now");
  assert.equal(migrated.requests[0].headers.get("Cookie"), cookie);
  assert.match(migrated.edits.at(-1).text, /签到成功/);
});
...
test("malformed and non-object JSON, redirects and non-200 transport responses stay retryable", async t => {
  for (const [body, status] of [
    ...
    const f = await fixture(t, {
      data: base,
      fetch: async () => new Response(body, { status, headers: { Location: "https://private.invalid" } }),
    });
    await f.run(".nodeseek now");
    assert.equal(f.requests.length, 3);
    assert.equal((await f.read()).lastDoneDate, "");
...
test("provider output is bounded, HTML escaped and configured cookie values are redacted before persistence", async t => {
  const f = await fixture(t, {
    data: base,
    fetch: async () => Response.json({ success: true, message: "<b>&\"' " + cookie + " " + "x".repeat(1000) }),
  });
  await f.run(".nodeseek now");
...
test("status repairs malformed legacy result text and cron/fallback notifications remain UTF-16 valid", async t => {
  ...
  const f = await fixture(t, {
    data: base,
    config: { pythonPath: "/fake/python" },
    fetch: challenge,
    process: async () =>
      processResult({
        status: 200,
        server: "",
        text: JSON.stringify({ success: true, message: "a".repeat(499) + "🍗" }),
      }),
  });
  await f.tick();
...
test("settings migrate legacy config cookie into operational state and keep it write-only", async t => {
  const f = await fixture(t, {
    data: base,
    config: {
      cookie: "panel-private-secret",
  ...
  await f.host.patchSettings("nodeseek", { autoEnabled: false });
  assert.equal((await f.read("config.json")).cookie, undefined);
  assert.equal((await f.read("config.json")).chatId, "9007199254740999");
  assert.equal((await f.read("config.json")).interval, 5);
  assert.equal((await f.read("config.json")).maxItems, 5);
  assert.deepEqual((await f.read("config.json")).unknown, { keep: true });
  assert.equal((await f.read()).autoEnabled, false);
  assert.deepEqual((await f.read()).unknown, base.unknown);
  ...
  assert.equal((await f.read()).cookie, "");
...
test("invalid settings patches reject atomically and schema defaults do not rewrite saved fields", async t => {
  ...
  for (const patch of [
    ...
    { cookie: true },
    { unknown: 9 },
    { pythonPath: "relative/python" },
    JSON.parse('{"__proto__":{}}'),
  ]) {
    await assert.rejects(f.host.patchSettings("nodeseek", patch));
    assert.deepEqual(await f.read("config.json"), migratedConfig);
    assert.equal((await f.read()).cookie, "private-panel-cookie");
  }
  await f.host.patchSettings("nodeseek", {});
  assert.deepEqual((await f.host.readSettings("nodeseek")).values, { autoEnabled: false });
...
test("concurrent settings patches preserve operational and config edits with unknown unsafe integer literals", async t => {
  ...
  await Promise.all([
    f.host.patchSettings("nodeseek", { autoEnabled: true }),
    f.host.patchSettings("nodeseek", { pythonPath: "/fixture/python" }),
  ]);
  const bytes = await fs.readFile(path.join(f.dir, "config.json"), "utf8");
  assert.match(bytes, /9007199254740993123/);
  assert.equal(JSON.parse(bytes).pythonPath, "/fixture/python");
  assert.equal((await f.read()).autoEnabled, true);
});

for (const operation of ["read", "patch"])
  test("settings " + operation + " cancellation tracks file read until it settles", async t => {
    ...
    const running =
      operation === "read"
        ? f.host.readSettings("nodeseek")
        : f.host.patchSettings("nodeseek", { pythonPath: "/new/python" });
...
test("WAF challenge uses configured Python and stdin cookie then accepts fallback HTTP 500 business JSON", async t => {
  const f = await fixture(t, {
    data: base,
    factory: { signRandom: false },
    config: { pythonPath: "/configured/venv/bin/python" },
    fetch: challenge,
...
test("HTTP 200 WAF body triggers fallback but ordinary Cloudflare success does not", async t => {
  for (const bodyChallenge of [true, false]) {
    const f = await fixture(t, {
      ...
      config: { pythonPath: "/fake/python" },
      fetch: async () =>
        new Response(bodyChallenge ? "Checking your browser" : '{"success":true}', {
...
test("legacy venv precedes PATH, explicit Python precedes venv, and both probe and fallback use that choice", async t => {
  for (const explicit of [false, true]) {
    const f = await fixture(t, {
      data: base,
      config: explicit ? { pythonPath: "/configured/python" } : {},
      fetch: challenge,
    });
    ...
    await fs.mkdir(bin);
    await fs.writeFile(path.join(bin, process.platform === "win32" ? "python3.exe" : "python3"), "fixture only", {
      mode: 0o700,
    });
    const previous = process.env.PATH;
    process.env.PATH = bin;
...
for (const state of ["available", "unavailable", "failed"])
  test("status dependency probe reports " + state + " with no cookies or network", async t => {
    const f = await fixture(t, {
      data: base,
      config: { pythonPath: "/fake/python" },
      process: async () => processResult({ state }),
    });
    await f.run(".nodeseek status");
    assert.equal(f.processes.length, 1);
    assert.equal(f.processes[0].probe, true);
...
test("status probe errors and invalid output stay bounded, observable and do not expose helper diagnostics", async t => {
  const cases = [
    async () => {
      throw Object.assign(new Error("private-probe-secret"), { code: "SPAWN_FAILED" });
    },
    async () => {
      throw Object.assign(new Error("private-probe-secret"), { code: "TIMED_OUT" });
    },
    async () => {
      throw Object.assign(new Error("private-probe-secret"), { code: "OUTPUT_LIMIT" });
    },
...
test("unload retains a noncooperative status probe until actual settlement and suppresses late status", async t => {
  ...
  const f = await fixture(t, {
    ...
    process: async () => {
      started.resolve();
      await release.promise;
      return processResult({ state: "available" });
    },
  });
...
for (const code of ["SPAWN_FAILED", "EXIT_FAILED", "TIMED_OUT", "OUTPUT_LIMIT", "IO_FAILED", "CONTROL_FAILED"]) {
  test("fallback process error is bounded and observable: " + code, async t => {
    const f = await fixture(t, {
      ...
      fetch: challenge,
      process: async () => {
        throw Object.assign(new Error("private-helper-credential"), { code, stderr: Buffer.from(cookie) });
      },
    });
    await f.run(".nodeseek now");
    ...
    assert.equal(f.logs.filter(item => item.event === "nodeseek.fallback.failed").length, 3);
    assert.doesNotMatch(
      JSON.stringify({ edits: f.edits, logs: f.logs }),
      /private-helper|fictional-secret|second-secret/,
    );
    assert.equal((await f.read()).lastDoneDate, "");
  });
}
...
test("malformed or oversized fallback output rejects while preserving primary WAF classification", async t => {
  for (const output of [
    ...
    processResult(null),
    processResult({ status: 999, server: "", text: "{}" }),
    processResult({ status: 200, server: 5, text: "{}" }),
    processResult({ status: 200, server: "", text: "x".repeat(65537) }),
    { ...processResult(), stderr: Buffer.alloc(1024 * 1024) },
...
test("daily cron skips disabled, missing-cookie and already-processed state", async t => {
  for (const data of [
    { ...base, autoEnabled: false },
    { ...base, cookie: "" },
    { ...base, lastDoneDate: date() },
  ]) {
...
test("daily delay rereads disabled, cookie-cleared, manual-complete and replacement-cookie state", async t => {
  for (const change of [
    ...
    { cookie: "" },
    { lastDoneDate: date() },
    { cookie: "changed=fictional-new-cookie" },
  ]) {
...
test("cron notification errors remain observable after committed result", async t => {
  const f = await fixture(t, {
    data: base,
    send: async () => {
      throw new Error("private-Telegram-error");
    },
  });
  await f.tick();
  assert.equal((await f.read()).lastDoneDate, date());
...
test("replacing cookie in flight preserves the replacement completion reset and unknown fields", async t => {
  ...
  await f.host.dispatchPrimary({
    ...
    chatId: "other",
    saved: true,
    text: ".nodeseek set session=fictional-new-cookie",
  });
  release.resolve();
  await running;
...
test("bounded HTTP parser accepts 64 KiB with split UTF-8 bytes", async t => {
  ...
  const bytes = Buffer.from(
    JSON.stringify({ success: true, message: "获得鸡腿", padding: "x".repeat(65536 - Buffer.byteLength(raw)) }),
  );
  const f = await fixture(t, {
    data: base,
    fetch: async () =>
      new Response(
        new ReadableStream({
          start(controller) {
            const split = bytes.indexOf(Buffer.from("鸡腿")) + 1;
...
test("64 KiB one-byte HTTP chunks are copied into fixed storage and decoded once", async t => {
  ...
  const bytes = Buffer.from(
    JSON.stringify({ success: true, message: "获得🍗", padding: "x".repeat(65536 - Buffer.byteLength(raw)) }),
  );
  let offset = 0,
    calls = 0;
...
for (const phase of ["http", "process", "telegram"])
  test("unload keeps noncooperative " + phase + " pending until real settlement", async t => {
    const started = deferred(),
      release = deferred();
    ...
    if (phase === "process") {
      options.fetch = challenge;
      options.process = async () => {
        started.resolve();
        await release.promise;
...
for (const phase of ["random delay", "retry delay"])
  test("unload cancels " + phase + " and removes its native timer", async t => {
...
test("storage malformed JSON and symlink failures do not overwrite files or expose filesystem details", async t => {
  ...
  await assert.rejects(f.host.patchSettings("nodeseek", { autoEnabled: false }), /Settings adapter failed/);
...
test("Telegram edit failures are logged and failed final reporting rejects the command", async t => {
  const f = await fixture(t, {
    data: base,
    edit: async () => {
      throw new Error("private-telegram");
    },
  });
...
test("Core enforces owner/edited admission, saved messages and aliases", async t => {
  ...
  assert.equal(
    await f.host.dispatchPrimary({ ...envelope, outgoing: false, saved: true, senderId: "1", text: ".nodeseek help" }),
    true,
  );
  await f.run("!!ns status");
  assert.match(f.edits.at(-1).text, /Cookie：未设置/);
  await f.host.dispatchPrimary({ ...envelope, saved: true, text: "!!ns set tiny" });
  assert.match(f.edits.at(-1).text, /!!nodeseek set ns_xxx/);
  await f.host.dispatchPrimary({ ...envelope, text: "!!ns auto bogus" });
  assert.match(f.edits.at(-1).text, /!!nodeseek auto on/);
  await f.host.dispatchPrimary({ ...envelope, text: "!!ns now" });
  assert.match(f.edits.at(-1).text, /!!nodeseek set/);
});

test("50 reload cycles preserve state and fully release cron, settings, tasks and timers", async t => {
  const baseline = process.getActiveResourcesInfo().filter(name => name === "Timeout").length;
  const f = await fixture(t, { data: base, config: { custom: { keep: true } } });
  for (let cycle = 0; cycle < 50; cycle++) {
    await f.run(".nodeseek now");
    await f.host.patchSettings("nodeseek", { autoEnabled: cycle % 2 === 0 });
    ...
    assert.deepEqual((await f.read()).unknown, base.unknown);
    assert.deepEqual((await f.read("config.json")).custom, { keep: true });
