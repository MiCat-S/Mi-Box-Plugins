...
async function fixture(t, {data, config, fetch = successful, process: runProcess, wait, edit, send, factory, hostOptions} = {}) {
  ...
  return {host, root, dir, cron, requests, edits, sends, logs, processes, waits,
    run: text => host.dispatchPrimary({...envelope, text, ...(text.toLowerCase().startsWith('.nodeseek set ') ? {saved: true} : {})}),
...
test('pure synchronous factory and lazy load use only candidate/helper/SDK source', async t => {
  ...
  assert.deepEqual(first.resources.processes, {concurrency: 1, queueCapacity: 1, timeoutMs: 30000, maxOutputBytes: 1024 * 1024});
...
test('settings migrate legacy config cookie into operational state and keep it write-only', async t => {
  ...
  assert.deepEqual(schema.map(field => field.key), ['cookie', 'autoEnabled', 'pythonPath']);
...
test('concurrent settings patches preserve operational and config edits with unknown unsafe integer literals', async t => {
  ...
  await Promise.all([f.host.patchSettings('nodeseek', {autoEnabled: true}), f.host.patchSettings('nodeseek', {pythonPath: '/fixture/python'})]);
...
for (const operation of ['read', 'patch']) test('settings ' + operation + ' cancellation tracks file read until it settles', async t => {
  ...
  const running = (operation === 'read' ? f.host.readSettings('nodeseek') : f.host.patchSettings('nodeseek', {pythonPath: '/new/python'}));
...
test('replacing cookie in flight preserves the replacement completion reset and unknown fields', async t => {
  ...
  await started.promise;
  await f.host.dispatchPrimary({...envelope, chatId: 'other', saved: true, text: '.nodeseek set session=fictional-new-cookie'});
  release.resolve();
  await running;
