...
async function fixture(t, responder, options = {}) {
  ...
  let replyCalls = 0;
  const host = new PluginHost({storageRoot: root, logger: {info(event, fields) {logs.push({event, fields});}, error() {}},
    http: {async fetch(url, init) {
      requests.push({url: String(url), init});
      return responder(String(url), init, requests.length);
    }}, telegram: {
      async edit(message, text, messageOptions) {if (options.failSuccessReceipt && /成功合并/.test(text)) throw options.failSuccessReceipt; edits.push({message, text, options: messageOptions});},
      async reply(message, text, messageOptions) {replyCalls++; if (options.failReplyOnce && replyCalls === 1) throw new Error('send detail'); replies.push({message, text, options: messageOptions});},
      async invoke() {assert.fail('unexpected invoke');}, async getReply() {}, async withClient() {assert.fail('unexpected client');},
    }});
...
test('repos uses bounded managed HTTP and preserves escaped paginated output after a partial send', async t => {
  const repositories = Array.from({length: 100}, (_, i) => ({full_name: `owner/repo-${i}-${'<&'.repeat(45)}`, permissions: {push: true}}));
...
test('prs restores mergeability details and hides failed detail diagnostics', async t => {
  const f = await fixture(t, async url => {
    if (url.endsWith('pulls?state=open&per_page=50')) return Response.json([
      {number: 1, title: '<ready>', user: {login: 'alice'}}, {number: 2, title: 'blocked', user: {login: 'bob'}}, {number: 3, title: 'unknown', user: {login: 'cat'}},
    ]);
...
test('mergeall merges only mergeable PRs in number order and reports partial success safely', async t => {
  ...
  const f = await fixture(t, async (url, init) => {
    if (url.endsWith('pulls?state=open&per_page=100')) return Response.json([{number: 3}, {number: 1}, {number: 2}]);
    if (init.method === 'GET') return Response.json({mergeable: !url.endsWith('/2')});
    puts.push(url); return url.includes('/1/merge') ? Response.json({merged: true}) : Response.json({message: 'top-secret provider'}, {status: 422});
  });
  await f.run('.git mergeall owner/repo');
  assert.deepEqual(puts.map(url => Number(url.match(/pulls\/(\d+)\/merge/)[1])), [1, 3]);
...
test('oversized responses cancel their body and cancellation stops later HTTP work', async t => {
  let cancelled = false;
  const f = await fixture(t, async () => new Response(new ReadableStream({start(controller) {controller.enqueue(new Uint8Array(2 * 1024 * 1024 + 1));}, cancel() {cancelled = true;}})));
...
test('unload cancels a hanging response reader, waits for cleanup, and starts no merge', async t => {
  ...
  const f = await fixture(t, async (url, init) => {
    ...
    return new Response(new ReadableStream({
      pull() {readStartedResolve(); return new Promise(() => {});},
      cancel() {cancelStartedResolve(); return cleanup;},
    }));
  });
  const dispatch = f.run('.git mergeall owner/repo');
  await readStarted;
  let unloaded = false;
  const unloading = f.host.unload('git_PR', 1000).then(report => {unloaded = true; return report;});
  await cancelStarted;
...
test('untrusted transport errors never expose message, name, code, token, path, or URL', async t => {
  const malicious = Object.assign(new Error('top-secret /Users/cat/private https://internal.invalid'), {name: 'TokenError', code: 'TOKEN_top-secret'});
  const f = await fixture(t, async () => {throw malicious;});
  await f.run('.git repos', {saved: false, chatId: '-100'});
