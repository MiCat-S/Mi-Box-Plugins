...
test.before(async () => {
  ...
  const built = buildPlugin({id: 'codex_image', packageRoot: path.resolve(__dirname, '../codex_image'), entry: 'v2.ts', rootDir: core});
...
async function fixture(t, options = {}) {
  const calls = [], edits = [], replies = [], sent = [], logs = [];
  const client = {async sendFile(peer, value) { sent.push({peer, value}); options.onSendFile?.(); },
    async *iterDownload(media, params) { if (options.iterDownload) { yield* options.iterDownload(media, params); return; } yield Buffer.from('reference'); }};
  const host = new PluginHost({storageRoot: await fs.mkdtemp(path.join(root, 'host-')), prefixes: options.prefixes ?? ['.'], aliases: options.aliases,
    logger: {info() {}, error(event, fields) { logs.push({event, fields}); }}, telegram: {
      async edit(_message, text, settings, signal) { signal.throwIfAborted(); options.onEdit?.(text); edits.push({text, settings}); },
      async reply(_message, text, settings, signal) { signal.throwIfAborted(); replies.push({text, settings}); }, async invoke() {}, async getReply() { return options.reply; },
      async withClient(operation, signal) { return operation(client, signal); },
    }});
  await host.load(definePlugin({id: 'ai', apiVersion: 1, description: 'fixture ai', commands: {}, services: {image: {description: 'fixture',
    async handle(input) { calls.push(input); return options.service ? options.service(input)
      : [{data: Buffer.from('generated'), mimeType: 'image/png', revisedPrompt: 'refined'}]; }}}}));
  await host.load(factory());
  t.after(async () => { await host.shutdown(1000); });
  return {host, calls, edits, replies, sent, logs, run: (text, message = {}) => {
    const raw = message.raw === null ? undefined
      : {message: text, peerId: {}, async delete() { options.onDelete?.(); }, ...message.raw};
...
test('long generation retains the original periodic progress receipt', async t => {
  ...
  try {
    const f = await fixture(t, {service: () => new Promise(resolve => { finish = () => resolve([{data: Buffer.from('generated')}]); }),
      onEdit(text) { if (text.includes('正在等待 AI 返回结果')) finish(); }});
...
test('long prompt and revision use a bounded caption and complete paginated receipts', async t => {
  ...
  for (const marker of ['PROMPT_START_', '_PROMPT_END', 'REVISION_START_', '_REVISION_END']) assert.match(pages, new RegExp(marker));
...
test('missing raw peer falls back to the envelope chat id', async t => {
  ...
  assert.equal(f.logs.some(value => value.event === 'codex_image_failed'), false);
});

test('reference download passes the active signal and cancels a hung iterator', async t => {
  let started, calls = 0;
  const ready = new Promise(resolve => { started = resolve; });
  const f = await fixture(t, {reply: {id: 8, raw: {media: {photo: {}}}}, iterDownload: async function* (_media, params) {
    assert.equal(params.requestSize, 64 * 1024);
    assert.ok(params.signal instanceof AbortSignal);
    started();
    await new Promise((_resolve, reject) => params.signal.addEventListener('abort', () => reject(params.signal.reason), {once: true}));
...
test('reference download preserves short chunks through the exact 20 MiB boundary', async t => {
  ...
  const f = await fixture(t, {reply: {id: 8, raw: {media: {photo: {}}}}, iterDownload: async function* () {
...
test('reference download rejects the first byte beyond 20 MiB before AI dispatch', async t => {
  const f = await fixture(t, {reply: {id: 8, raw: {media: {photo: {}}}}, iterDownload: async function* () {
    yield Buffer.alloc(20 * 1024 * 1024); yield Buffer.from([1]);
  }});
  await f.run('.cximg edit', {replyToId: 8});
...
test('command deletion failure does not replace a successful generated image', async t => {
  const f = await fixture(t, {onDelete() { throw new Error('private delete failure'); }});
...
test('completion receipt failure after a sent image stays a fixed cleanup error', async t => {
  const f = await fixture(t, {onDelete() { throw new Error('private delete failure'); },
    onEdit(text) { if (text.includes('图片生成完成')) throw new Error('private receipt failure'); }});
  await f.run('.cximg draw');
  assert.equal(f.sent.length, 1);
...
test('cancellation during sendFile prevents command deletion', async () => {
  const controller = new AbortController(); let deleted = 0;
  const message = {id: 10, chatId: '-1001', text: '.cximg draw', raw: {peerId: {}, async delete() { deleted += 1; }}};
  const context = {signal: controller.signal, log: {error() {}}, storage: {json: () => ({async read() { return {maxWaitMs: 600000, importedLegacy: true, aiMigrated: true}; }})},
    services: {available: () => true, async call() { return [{data: Buffer.from('generated')}]; }},
    telegram: {async edit() {}, async getReply() {}, async withClient(operation) { return operation({async sendFile() { controller.abort(); }}, controller.signal); }}};
