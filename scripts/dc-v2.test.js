...
test.before(async () => {
  ...
  const built = buildPlugin({
    id: "dc",
    packageRoot: path.resolve(__dirname, "../dc"),
    entry: "v2.ts",
    rootDir: buildRoot,
  });
...
test("dc never invokes a raw message bound to a different native client", async t => {
  ...
  await f.run(".dc", {
    raw: {
      client: {},
      async getChat() {
        assert.fail("foreign client access");
      },
    },
  });
  assert.deepEqual(
...
test("dc explicit usernames and exact numeric IDs resolve before full-user RPC", async t => {
  for (const target of ["@alice", "9007199254740993"]) {
    ...
    assert.deepEqual(
      f.calls.map(call => call.method),
      ["getEntity", "getInputEntity", "invoke"],
    );
...
test("successful lookup does not report query failure when its result edit fails", async t => {
  ...
  assert.equal(
    f.edits.filter(item => item.text.includes("查询失败") || item.text.includes("获取用户信息失败")).length,
    0,
  );
...
test("dc mention-name and phone entities preserve peer-ID interpretation", async t => {
  const f = await fixture(t);
  await f.run(".dc mention", {
    raw: {
      entities: [new Api.MessageEntityMentionName({ offset: 4, length: 7, userId: integer("9007199254740993") })],
    },
  });
...
test("dc absent reply and sender produce distinct notices without querying another user", async t => {
  for (const [reply, text] of [
    [undefined, "❌ 无法获取回复的消息"],
    [{ ...envelope, senderId: undefined }, "❌ 无法获取回复消息的发送者"],
  ]) {
...
test("dc photo-free reply does not fall back to the chat", async t => {
  const f = await fixture(t, {
    reply: { ...envelope, senderId: "123" },
    native: {
      invoke: async () => full(user({ photo: undefined })),
      getEntity: async () => assert.fail("must not fall back"),
    },
  });
...
test("dc failed reply-user RPC falls back to the reply chat without leaking errors", async t => {
  const f = await fixture(t, {
    reply: { ...envelope, senderId: "-100123", chatId: "-100456" },
    native: {
      invoke: async () => {
        throw new Error("private-token");
      },
      getEntity: async peer => {
        assert.equal(peer.toString(), "-100456");
        return { title: "Reply group", photo: { dcId: 2 } };
      },
    },
  });
  await f.run(".dc @ignored", { replyToId: 7 });
...
test("dc reply-chat fallback distinguishes empty photos and inaccessible chats", async t => {
  for (const [getEntity, text] of [
    [async () => ({}), "❌ 回复的消息所在对话需要先设置头像"],
    [
      async () => {
        throw new Error("private-token");
      },
...
test("dc target without a photo keeps its target-specific notice", async t => {
  const f = await fixture(t, {
    native: { invoke: async () => full(user({ photo: new Api.UserProfilePhotoEmpty() })) },
  });
  await f.run(".dc @alice");
...
for (const method of ["getEntity", "getInputEntity", "invoke"]) {
  for (const rejects of [false, true])
    test("dc cancellation retains " + method + " until actual settlement (" + rejects + ")", async t => {
      ...
      let settled = false;
      const running = f.run(".dc @alice").finally(() => {
        settled = true;
      });
