...
async function callChat(cfg, input) {
  ...
  const context = {
    ...
    http: {
      withResponse: async (url, init, consume) => {
...
test("ai chat service applies provider, model and reasoning overrides", async () => {
  const { result, requests } = await callChat(config(), {
    text: "hi",
    systemPrompt: "sys",
    tag: "alt",
    model: "model-alt",
...
test("ai chat service defaults a provider without a protocol override to Responses", async () => {
  const cfg = config();
  delete cfg.configs.main.responses;
  const { requests } = await callChat(cfg, { text: "hi" });
...
test("ai chat service uses the requested provider model and accepts bounded multimodal options", async () => {
  ...
  const { requests } = await callChat(cfg, {
    text: "describe",
    tag: "alt",
    temperature: 0.4,
    maxOutputTokens: 321,
...
test("ai selection service exposes current chat/search choices without secrets", async () => {
  ...
  assert.deepEqual(selection.search, {
    tag: "alt",
    model: "model-search",
  ...
  assert.deepEqual(
    selection.providers.map(item => item.tag),
    ["alt", "main"],
  );
  ...
  const context = {
    ...
    http: {
      async withResponse(url, init, consume) {
        requests.push({ url: String(url), init });
        return consume(await responder(new URL(url), init), controller.signal);
      },
    },
...
test("ai import service resolves tag collisions, is idempotent, and selects the actual imported tag once", async () => {
  ...
  const fixture = mutableContext(cfg);
  const service = createAi().services.import_provider;
  const input = {
    tag: "banana",
    url: "https://generativelanguage.googleapis.com/v1beta",
    key: "legacy",
    type: "gemini",
    models: { image: "gemini-image" },
    select: ["image"],
  };
...
test("ai models service normalizes official provider roots and keeps credentials out of output", async () => {
  ...
  const fixture = mutableContext(cfg, target =>
    Response.json(
      target.hostname.includes("google")
...
test("ai image service returns generated bytes through the current unified image selection", async () => {
  ...
  const cfg = config({
    ...
    currentImageModel: "gemini-image",
  });
  const fixture = mutableContext(cfg, () =>
    Response.json({
      candidates: [
...
test("ai image service parses Codex CRLF SSE and a terminal frame without a blank line", async () => {
  ...
  const cfg = config({
    ...
    currentImageTag: "codex",
    currentImageModel: "gpt-5.4",
  });
  const fixture = mutableContext(
    cfg,
    () =>
      new Response(
