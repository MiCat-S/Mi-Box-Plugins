...
const { artifactDir } = buildPlugin({
  id: "annualreport",
  packageRoot: path.resolve(__dirname, "../annualreport"),
  entry: "v2.ts",
});
...
async function fixture(t, options = {}) {
  ...
  const client = {
    async getDialogs() {
      ...
      return [
        { id: "1", isUser: true, entity: {} },
        { id: "2", isUser: true, entity: { bot: true } },
        { id: "3", isGroup: true },
        { id: "4", isChannel: true },
      ];
    },
    ...
    http: {
      fetch: async () =>
        options.httpFailure
          ...
          : Response.json({ hitokoto: "<hello>", from_who: "author" }),
    },
    telegram: {
  ...
  return {
    ...
    edits,
    calls,
    run: (text = ".annualreport") => host.dispatchPrimary({ id: 1, chatId: "1", senderId: "1", outgoing: true, text }),
  };
}
...
test("annualreport reports live ready plugin count and preserves historical stats", async t => {
  ...
  assert.match(
    f.edits.at(-1).text,
    new RegExp(`#${new Date().getMonth() === 0 ? new Date().getFullYear() - 1 : new Date().getFullYear()}年度报告`),
  );
...
test("annualreport stops before the next Telegram RPC when unload cancels collection", async t => {
  let releaseDialogs;
  const dialogGate = new Promise(resolve => {
