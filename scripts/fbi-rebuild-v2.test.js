...
async function fixture(t, { cache = {}, cacheLimit = 10, surveillance = {}, client: clientPatch = {} } = {}) {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ schemaVersion: 1, importedLegacy: true, cacheLimit, surveillance, extension: { keep: "config" } }),
  );
  await fs.writeFile(
    path.join(dir, "cache.json"),
    JSON.stringify({ schemaVersion: 1, importedLegacy: true, cache, extension: { keep: "cache" } }),
  );
  const edits = [],
    sent = [];
  const client = {
    async getEntity(value) {
      return {
        className: "Channel",
        id: String(value).replace(/^-100/, ""),
        username: `group${String(value).replace(/\D/g, "")}`,
        title: "Public group",
      };
    },
...
test("fbi rebuild merges live increments over fetched history instead of evicting it", async t => {
  ...
  const cache = {
    [peer(42)]: {
      username: "group42",
      msgs: Array.from({ length: 3000 }, (_, index) => cached(3000 - index, now - 10000 - index)),
    },
  };
  const f = await fixture(t, {
    ...
    client: {
      async getDialogs() {
        return [{ id: peer(42), isGroup: true }];
      },
      async *iterMessages() {
        entered();
        await gate;
        for (let id = 6000; id >= 3001; id--) yield cached(id, now - 1);
      },
    },
  });
  ...
  assert.equal(
    msgs.filter(message => message.id >= 3001 && message.id <= 6000).length,
    2999,
    "fetched history must survive",
  );
  ...
  for (let index = 1; index < msgs.length; index++) {
    assert.ok(
      msgs[index - 1].date > msgs[index].date ||
        (msgs[index - 1].date === msgs[index].date && msgs[index - 1].id > msgs[index].id),
      "newest-first order",
    );
  }
  ...
  const f = await fixture(t, {
    ...
    client: {
      ...
      async *iterMessages() {
        entered();
        await gate;
        yield cached(5000, now - 1, { text: "fetched-5000" });
        yield cached(4999, now - 1);
      },
    },
  });
  const rebuild = f.run(".fbi cache rebuild");
...
test("fbi rebuild keeps a touched chat without its stale history and drops untouched chats", async t => {
  ...
  assert.deepEqual(
    saved[peer(42)].msgs.map(message => message.id),
    [6000],
  );
  assert.deepEqual(
    saved[peer(43)].msgs.map(message => message.id),
    [7000],
  );
});
  ...
  const f = await fixture(t, {
    ...
    client: {
      async getDialogs() {
        entered();
        await gate;
        return Array.from({ length: 11 }, (_, index) => ({ id: peer(index + 1), isGroup: true }));
      },
      async *iterMessages(chat) {
        const id = Number(String(chat).slice(4));
        yield cached(id, id === 1 ? now - 10000 : now - 100);
      },
    },
  });
  const rebuild = f.run(".fbi cache rebuild");
  ...
  const f = await fixture(t, {
    ...
    client: {
      async getDialogs() {
        ...
        await gate;
        return [
          { id: peer(1), isGroup: true },
          { id: peer(2), isGroup: true },
        ];
      },
      async *iterMessages(chat) {
        yield cached(Number(String(chat).slice(4)), now - 100);
      },
    },
  });
  const rebuild = f.run(".fbi cache rebuild");
  ...
  const f = await fixture(t, {
    ...
    client: {
      async getDialogs() {
        entered();
        await gate;
        return [{ id: peer(1), isGroup: true }];
      },
      async *iterMessages() {
        if (attempt++ === 0) throw new Error("history failed");
        yield cached(1, now - 100);
      },
    },
  });
...
test("fbi rebuild preserves metadata for a fetched group with no live increment", async t => {
  ...
  const f = await fixture(t, {
    ...
    client: {
      ...
      async *iterMessages() {
        yield cached(1, now - 10000, { text: "fresh text" });
      },
    },
  });
  await f.run(".fbi cache rebuild");
  ...
  assert.deepEqual(
    saved.msgs.map(message => message.id),
    [1],
    "old history must not re-enter",
  );
});
