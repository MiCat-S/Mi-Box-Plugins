...
async function fixture(t, id, options = {}) {
  ...
  const telegram = {
    ...
    async invoke(request) {
      ...
      return options.invoke ? options.invoke(request) : {};
    },
    async getReply() {
...
test("listusernames renders escaped public chats and channel statistics", async t => {
  const f = await fixture(t, "listusernames", {
    invoke: async () => ({
      chats: [
    ...
    reply: {
      raw: {
        ...
        async downloadMedia({ outputFile }) {
          await fs.writeFile(outputFile, photo);
          return outputFile;
        },
      },
    },
          ...
          Catalog: {
            searchStore: {
              elements: [
                {
                  title: "Game <One>",
                  description: "Fun & free",
                  categories: [{ path: "freegames" }],
                  price: { totalPrice: { discountPrice: 0, fmtPrice: { originalPrice: "¥68" } } },
                  promotions: {
                    promotionalOffers: [
                      { promotionalOffers: [{ startDate: "2026-09-01T00:00:00Z", endDate: "2026-09-08T00:00:00Z" }] },
                    ],
                  },
