...
const { artifactDir } = buildPlugin({
  id: "hitokoto",
  packageRoot: path.resolve(__dirname, "../hitokoto"),
  entry: "v2.ts",
});
...
async function fixture(
  t,
  {
    body = { hitokoto: "你好 <世界>", from: "来源", from_who: "作者", type: "a" },
    fetch: fetcher,
    onEdit,
    onReply,
  } = {},
  ...
  const host = new PluginHost({
    ...
    telegram: {
      async edit(message, text, options) {
        await onEdit?.(message, text, options);
        edits.push({ message, text, options });
      },
      async reply(message, text, options) {
        await onReply?.(message, text, options);
  ...
  return {
    ...
    unload: timeout => host.unload("hitokoto", timeout),
    ...
    fetch: async () =>
      new Response(
        new ReadableStream({
          ...
          cancel() {
            throw new Error("SECRET_BODY_CLEANUP");
          },
        }),
      ),
  });
