...
async function fixture(t) {
  ...
  const ai = definePlugin({
    ...
    commands: { fixture: { description: "fixture", async handle() {} } },
    services: {
      selection: {
        description: "fixture selection",
        async handle() {
          calls.push({ service: "selection" });
          return {
            providers: [
              { tag: "main", type: "openai", models: { chat: "gpt" } },
              { tag: "backup", type: "openai" },
            ],
          };
        },
      },
      models: {
        description: "fixture models",
        async handle(input) {
      ...
      diagnostics: {
        ...
        async handle(input) {
          calls.push({ service: "diagnostics", input });
          if (input.action === "benchmark")
            return {
              provider: { tag: input.tag, type: "openai", displayName: "OpenAI" },
              balance: { status: "unsupported", fields: [] },
              benchmarks: [
                { model: "gpt-4.1-mini", ok: true, elapsedMs: 1000, usage: { total: 25 } },
                { model: "gpt-4o-mini", ok: false, elapsedMs: 10, error: "limited" },
              ],
            };
          return {
            provider: { tag: input.tag, type: "openai", displayName: `API ${input.tag}` },
            balance: { status: "ok", fields: [{ label: "套餐", value: "pro" }] },
            chat: { ok: true, text: "ok", model: "gpt", elapsedMs: 20, usage: { prompt: 1, completion: 2, total: 3 } },
            models: { ok: true, names: ["m1", "m2"] },
          };
        },
      },
    },
  ...
  return {
    ...
    edits,
    calls,
    send: text => host.dispatchPrimary({ id: 1, chatId: "chat", senderId: "1", outgoing: true, text }),
  };
}

test("CHECKAPI-COMPAT-01 delete remains an alias of credential management", async t => {
  const f = await fixture(t);
  await f.send("!checkapi delete old");
...
test("CHECKAPI-COMPAT-02 check without a tag checks every configured provider", async t => {
  const f = await fixture(t);
  await f.send("!checkapi check");
  assert.match(f.edits.at(-1).text, /main.*✅ 2 个模型/s);
  assert.match(f.edits.at(-1).text, /backup.*✅ 1 个模型/s);
  assert.deepEqual(
    f.calls.filter(call => call.service === "models").map(call => call.input.tag),
    ["main", "backup"],
  );
});
...
test("CHECKAPI-COMPAT-03 speed renders every benchmark with token throughput and isolated errors", async t => {
  const f = await fixture(t);
  await f.send("!checkapi speed main");
  const output = f.edits.at(-1).text;
  assert.match(output, /gpt-4\.1-mini.*1000ms \(25\.0 tok\/s\)/s);
  assert.match(output, /gpt-4o-mini.*limited/s);
...
test("CHECKAPI-COMPAT-04 compare renders full balance, chat metadata, usage, and model lists for both tags", async t => {
  const f = await fixture(t);
  await f.send("!checkapi compare main backup");
