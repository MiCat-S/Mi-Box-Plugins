...
async function fixture(t, client) {
  ...
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async withClient(operation, signal) { return operation(client, client.operationSignal ?? signal); },
    },
  });
...
test('reports the legacy capped-count detail in progress and completion', async t => {
  ...
  const client = {
    ...
    async *iterMessages() {
      yield new Api.Message({id: 10, peerId: new Api.PeerChannel({channelId: 1}), fromId: new Api.PeerUser({userId: 5}), date: 1788700000, message: 'old'});
    },
  ...
  const client = {
    ...
    async *iterMessages(_peer, options) {
      ...
      yield {
        ...
        async getSender() { senderLookups++; return {username: 'Alice'}; },
      };
    },
    async sendMessage(peer, value) { sent.push({peer, value}); },
  };
...
test('cancellation during selector retry does not enter the deep scan', async t => {
  ...
  const client = {
    ...
    async *iterMessages(_peer, options) { limits.push(options.limit); },
    async sendMessage() { sent++; },
  };
  const f = await fixture(t, client);
