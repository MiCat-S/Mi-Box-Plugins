...
async function fixture(t, { data, rates, ratesStatus = 200, checked = "", fetchRates } = {}) {
    ...
    http: {
      fetch: async (url, init) => {
        ...
        if (String(url).includes("open.er-api.com"))
          return fetchRates ? fetchRates(init) : new Response(JSON.stringify(rates ?? {}), { status: ratesStatus });
  ...
  return {
    ...
    requests,
    run: (input = "41705691") =>
      host.dispatchPrimary({ id: 1, chatId: "chat", senderId: "owner", outgoing: true, text: `.bin ${input}` }),
  };
}
...
test("bin card renders country metadata and dated cross rates in one safe message and reuses rates", async t => {
  const f = await fixture(t, {
    data: {
      ...
      prepaid: false,
      commercial: false,
      bank: { name: "COSMOTE PAYMENTS <SERVICES> & Co." },
      country: { name: "Germany", alpha2: "DE", currency: "EUR" },
      number: { length: 16, luhn: true },
    },
    rates: {
      result: "success",
      base_code: "USD",
      time_last_update_unix: 1788912000,
      rates: { USD: 1, EUR: 0.875, CNY: 7 },
    },
  });
  await f.run();
...
test("bin validates before network, keeps country metadata coherent and deduplicates USD quotes", async t => {
  const f = await fixture(t, {
    data: { brand: "Business", scheme: "visa", country: { name: "United States", alpha2: "US", currency: "USD" } },
...
test("bin validates input and formats provider fields safely", async t => {
  ...
  const host = new PluginHost({
    ...
    http: {
      fetch: async url =>
        new Response(
          String(url).includes("bincheck.io")
            ...
            : JSON.stringify({
                ...
                country: { name: "Taiwan", alpha2: "TW", currency: "TWD" },
              }),
          { status: 200 },
        ),
    },
