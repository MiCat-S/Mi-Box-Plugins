...
const { artifactDir } = buildPlugin({
  id: "autodel",
  packageRoot: path.resolve(__dirname, "../autodel"),
  entry: "v2.ts",
});
...
function fixture(initial = { schemaVersion: 1, settings: {}, importedLegacy: true }) {
  ...
  const plugin = create(),
    message = { id: 1, chatId: CHAT, senderId: "9", outgoing: true, text: "" };
  return {
    plugin,
    context,
    edits,
    ...
    state: () => state,
    run: text =>
      plugin.commands.autodel.handle(
        {
          command: "autodel",
          prefix: ".",
          args: text.trim().split(/\s+/).filter(Boolean),
          message: { ...message, text: `.autodel ${text}` },
        },
        context,
      ),
...
async function hostFixture(t, { settings = {}, legacy = null, prefixes, selfId = "9", deleteError, logs, getMe } = {}) {
  ...
  if (settings)
    await fs.writeFile(
      path.join(root, "autodel/config.json"),
    ...
    telegram: {
      ...
      async withClient(operation, signal) {
        return operation(
          {
            async getMe() {
              return getMe ? getMe() : { id: BigInt(selfId) };
            },
...
const flush = async (turns = 6) => {
  for (let index = 0; index < turns; index++) await new Promise(setImmediate);
};
...
test("AUTODEL-03 channel-attributed posts are never scheduled, user messages are", async t => {
  ...
  const post = messageEnvelope(
    new Api.Message({ id: 11, peerId: channel, out: true, post: true, fromId: channel, message: "channel post" }),
    { selfId: "9" },
  );
  ...
  const mine = messageEnvelope(
    new Api.Message({ id: 12, peerId: channel, out: true, fromId: self, message: "temporary" }),
    { selfId: "9" },
  );
...
test("AUTODEL-04 parses every original time unit and rejects invalid ones", async () => {
  ...
  for (const input of ["5", "abc", "-5s", "1.5h", "9007199254740991d", ""]) {
    ...
    assert.match(
      f.edits.at(-1).text,
      /时间格式错误|定时删除消息|设置自动删除/,
      `${input || "(empty)"} reports help or error`,
    );
  }
});
...
test("AUTODEL-07 deletes only messages authored by the exact cached account id", async t => {
  ...
  await send(
    messageEnvelope(new Api.Message({ id: 51, peerId: channel, out: true, fromId: myself, message: "mine" }), {
      selfId,
    }),
  );
  ...
  await send(
    messageEnvelope(
      new Api.Message({
        ...
        out: true,
        fromId: myself,
        fwdFrom: { className: "MessageFwdHeader", date: 1, fromId: myself },
        message: "own forward",
      }),
      { selfId },
    ),
  ...
  await send(
    messageEnvelope(new Api.Message({ id: 53, peerId: channel, out: true, fromId: other, message: "other" }), {
      selfId,
    }),
  );
  await send(
    messageEnvelope(new Api.Message({ id: 54, peerId: channel, out: true, fromId: sendAs, message: "send-as" }), {
      selfId,
    }),
  );
