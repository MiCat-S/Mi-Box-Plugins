...
const { artifactDir } = buildPlugin({
  id: "music_bot",
  packageRoot: path.resolve(__dirname, "../music_bot"),
  entry: "v2.ts",
});
      ...
      if (value.message !== "/start" && value.message !== "1") {
        ...
        const choice = {
          ...
          async click(selection) {
            ...
            history(bot).unshift({ id: nextId(bot), out: false, date: 1_900_000_000, media: { request, bot } });
          },
        };
...
async function hostFixture(t, client, options = {}) {
  ...
  const edits = [],
    errors = [],
    infos = [],
    operationSignals = options.operationSignals ?? [];
  let nativeCalls = 0,
    messageId = 0;
  const host = new PluginHost({
    ...
    telegram: {
      ...
      async invoke() {
        assert.fail("unexpected transport invoke");
      },
      async getReply() {
...
test("music_bot preserves every command, source mapping, button selection, and ym caption behavior", async t => {
  ...
  for (const [command, bot, request, caption] of cases) {
    ...
    assert.deepEqual(
      f.sent.findLast(value => value.message !== "/start"),
      { bot, message: request },
    );
...
test("search progress safely renders while preserving the bot query", async t => {
  const f = responsiveClient();
  const runtime = await hostFixture(t, f.client);
  await runtime.run(".mbvk A&B");
  assert.equal(runtime.edits[0].text, "🔎 搜索中：<code>A%26B</code>");
  assert.deepEqual(
    f.sent.find(value => value.message !== "/start"),
    { bot: "@vkmusic_bot", message: "A&B" },
  );
});

test("setup RPCs use resolvable native TL requests", async t => {
  const requests = [];
  const f = responsiveClient();
  f.client.getInputEntity = async () =>
    new Api.InputPeerUser({ userId: returnBigInt("9007199254741999"), accessHash: returnBigInt("-9007199254742999") });
  f.client._getInputNotify = async value => value;
  ...
  const runtime = await hostFixture(t, f.client);
  await runtime.run(".mbvk protocol");
  assert.ok(requests[0] instanceof Api.contacts.Unblock);
  assert.ok(requests[1] instanceof Api.account.UpdateNotifySettings);
  assert.ok(requests[1].peer instanceof Api.InputNotifyPeer);
});

test("successful media delivery is not reported failed when command cleanup fails", async t => {
  const f = responsiveClient();
  const runtime = await hostFixture(t, f.client);
  await runtime.run(".mbvk cleanup", {
    async delete() {
      throw Object.assign(new Error("token=/private/path"), { name: "PrivateDeleteError" });
    },
  });
  assert.equal(f.files.length, 1);
  assert.equal(runtime.errors.includes("music_bot_failed"), false);
  assert.equal(runtime.infos.includes("music_bot_command_cleanup_failed"), true);
  assert.doesNotMatch(JSON.stringify(runtime.edits), /PrivateDeleteError|token=|private\/path/);
});

test("transport failures disclose neither error names nor messages", async t => {
  const error = Object.assign(new Error("token=/private/path"), { name: "PrivateTransportError" });
  const client = {
    async invoke() {
      throw error;
    },
  ...
  await runtime.run(".mbvk secret");
  assert.deepEqual(runtime.errors, ["music_bot_failed"]);
  assert.match(runtime.edits.at(-1).text, /音乐搜索失败/);
  assert.doesNotMatch(
    JSON.stringify({ edits: runtime.edits, errors: runtime.errors, infos: runtime.infos }),
    /PrivateTransportError|token=|private\/path/,
  );
});
...
test("watermarks preserve ids above the safe integer range", async t => {
  const files = [];
  const history = [
    {
      id: "9007199254741999",
  ...
  const choice = {
    ...
    buttonCount: 1,
    async click() {
      history.unshift({ id: "9007199254742002", out: false, media: { request: "fresh-large" } });
    },
  };
  const client = {
    async invoke() {},
    ...
    async sendMessage() {
      ...
      return { id: "9007199254742000", out: true };
    },
    async sendFile(_peer, value) {
      files.push(value.file.request);
    },
  ...
  const runtime = await hostFixture(t, client);
  await runtime.run(".mbvk large-id");
  assert.deepEqual(files, ["fresh-large"]);
});
...
test("a failed request sends Start and retries without real-time sleeps", async t => {
  ...
  assert.deepEqual(
    f.sent.map(value => value.message),
    ["retry", "/start", "retry"],
  );
...
test("same-bot requests serialize across chats while another bot proceeds independently", async t => {
  ...
  const runtime = await hostFixture(t, f.client, {
    onNative(index) {
      if (index === 1) secondNative.resolve();
    },
  });
...
test("an explicitly cancelled middle waiter never sends and cannot let its successor pass the predecessor", async t => {
  ...
  const runtime = await hostFixture(t, f.client, {
    ...
    onNative(index) {
      if (index === 2) allNative.resolve();
    },
  });
  const first = runtime.run(".mbvk first");
  await firstStarted.promise;
  let middleSettled = false;
  const middle = runtime.run(".mbvk middle").then(() => {
    middleSettled = true;
  });
...
test("cancellation while forwarding media performs no command deletion or failure feedback", async t => {
  ...
  const runtime = await hostFixture(t, f.client);
  const running = runtime.run(".mbvk cancel-file", {
    async delete() {
      deleted++;
    },
  });
  await entered.promise;
  const unloading = runtime.host.unload("music_bot", 2000);
  release.resolve();
  assert.equal((await unloading).completed, true);
  await running;
  assert.equal(deleted, 0);
  assert.equal(runtime.errors.includes("music_bot_failed"), false);
  assert.doesNotMatch(runtime.edits.map(x => x.text).join("\n"), /失败/);
});

test("missing raw peer uses the precise envelope chat id", async t => {
  const f = responsiveClient();
  const runtime = await hostFixture(t, f.client);
  await runtime.run(".mbvk peer", { peerId: undefined, async delete() {} });
  assert.equal(String(f.files[0].peer), "10001");
...
test("plugin instances isolate queues across hosts and after unload/reload", async t => {
  const releaseOld = deferred(),
    oldStarted = deferred();
  const oldClient = responsiveClient({
    beforeSend: async (_bot, request) => {
      if (request === "old") {
        oldStarted.resolve();
  ...
  const independent = independentRuntime.run(".mbvk independent").then(() => {
  ...
  assert.deepEqual(
    independentClient.sent.map(value => value.message),
    ["independent"],
  );
  assert.deepEqual(
    oldClient.sent.map(value => value.message),
    ["old", "reloaded"],
  );
});
