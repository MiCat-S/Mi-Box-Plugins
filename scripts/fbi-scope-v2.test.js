...
async function fixture(t, { cache = {}, surveillance, entity = group, editHook } = {}) {
  ...
  await fs.writeFile(
    path.join(root, "fbi/cache.json"),
    JSON.stringify({ schemaVersion: 1, importedLegacy: true, cache }),
  );
  if (surveillance)
    await fs.writeFile(
      path.join(root, "fbi/db.json"),
      JSON.stringify({ schemaVersion: 1, importedLegacy: true, cacheLimit: 300, surveillance }),
    );
  const edits = [],
    editMessages = [],
    sent = [];
  const client = {
    async getEntity(value) {
      return ["42", "fixtureuser"].includes(String(value)) ? user : entity;
    },
...
test("fbi consumes one persisted watch exactly once across concurrent chat lanes", async t => {
  ...
  await f.run(".fbi sur 42");
  await Promise.all([f.listen("-10010", "first concurrent hit", 10), f.listen("-10011", "second concurrent hit", 11)]);
