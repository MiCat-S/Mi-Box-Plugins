...
async function rootFixture(t, patternColor, emojiColor) {
  ...
  await fs.writeFile(
    path.join(root, "emoji", "emoji-apple-image.json"),
    JSON.stringify({ "😀": png(emojiColor).toString("base64") }),
  );
  return root;
}
...
async function render(root) {
  ...
  return withAssetRoot(root, () =>
    withCanvasBudget(controller.signal, () =>
      generateQuote({
        assetRoot: root,
...
test("clearResources drops pattern and emoji caches for one root", async t => {
  ...
  await fs.writeFile(
    path.join(root, "emoji", "emoji-apple-image.json"),
    JSON.stringify({ "😀": png("#ffff00").toString("base64") }),
  );
  assert.deepEqual(await render(root), before);
  assert.equal(
    withAssetRoot(root, () => loadBrand("apple")["😀"]),
    emojiBefore,
  );
  clearResources(root);
