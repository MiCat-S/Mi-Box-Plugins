...
const {artifactDir} = buildPlugin({id: 'listusernames', packageRoot: path.resolve(__dirname, '../listusernames'), entry: 'v2.ts'});
const create = require(path.join(artifactDir, 'index.cjs')).default;
const envelope = {id: 2147483640, chatId: '9007199254740993', senderId: '123', outgoing: true, text: '.listusernames', raw: {peerId: new Api.PeerUser({userId: 9007199254740993n})}};
...
async function fixture(t, {chats = [], invoke, sendMessage, prefixes = ['.']} = {}) {
  ...
  const edits = [], replies = [], sends = [], invokes = [], logs = [];
  const client = {async sendMessage(peer, value) { sends.push({peer, value}); return sendMessage?.(sends.length, peer, value); }};
  const host = new PluginHost({storageRoot: root, prefixes, logger: {
    info(event, fields) { logs.push({level: 'info', event, fields}); }, error(event, fields) { logs.push({level: 'error', event, fields}); },
  }, telegram: {
  ...
  return {host, edits, replies, sends, invokes, logs, close: async timeout => { const value = await host.shutdown(timeout); closed = value.completed; return value; },
...
test('query preserves original progress, fields, counts, escaping and precise IDs', async t => {
  ...
  assert.equal(f.edits[1].text, `📋 <b>属于我的公开群组/频道</b>
...
test('all entries survive bounded valid HTML pagination with native reply metadata', async t => {
  const chats = Array.from({length: 80}, (_, index) => ({id: BigInt('9007199254740' + String(index).padStart(3, '0')), title: `<title-${index}>&😀`.repeat(8), username: `name_${index}`, broadcast: index % 3 === 0}));
  const f = await fixture(t, {chats});
  await f.run('.listusernames');
  ...
  for (const page of pages) for (const tag of ['b', 'code']) {
    assert.equal((page.match(new RegExp(`<${tag}>`, 'g')) || []).length, (page.match(new RegExp(`</${tag}>`, 'g')) || []).length);
  }
...
test('later-page failure preserves published results, logs safely and never repeats the query', async t => {
  ...
  const chats = Array.from({length: 80}, (_, index) => ({id: BigInt(index + 1), title: `title-${index}-` + 'x'.repeat(100), username: `name_${index}`}));
  const f = await fixture(t, {chats, sendMessage(count) { if (count === 1) throw new Error(secret); }});
  await f.run('.listusernames');
  assert.equal(f.invokes.length, 1);
  ...
  assert.equal(JSON.stringify({texts: f.edits.map(item => item.text), replies: f.replies.map(item => item.text), logs: f.logs}).includes(secret), false);
});

test('unload waits for a pending native page send and suppresses later pages and errors', async t => {
  const started = deferred(), release = deferred();
  const chats = Array.from({length: 80}, (_, index) => ({id: BigInt(index + 1), title: `title-${index}-` + 'x'.repeat(100), username: `name_${index}`}));
  const f = await fixture(t, {chats, sendMessage: async count => { if (count === 1) { started.resolve(); await release.promise; } }});
...
test('query failures are generic and never expose Telegram errors', async t => {
  ...
  const f = await fixture(t, {invoke: async () => { throw new Error(secret); }});
  await f.run('.listusernames');
