...
const bikoCommand: CommandDefinition = {
  description: "跨对话整理指定用户的近期消息",
  helpArgs: ["help"],
  helpOnEmpty: true,
  args: "来源对话 来源用户 最大消息数 目标对话",
  arguments: [
    {name: "来源对话", required: true, description: "对话 ID 或 @用户名"},
    {name: "来源用户", required: true, description: "用户 ID、@用户名或可解析的实体"},
    {name: "最大消息数", required: true, description: "大于 0 的整数，上限 200"},
    {name: "目标对话", required: true, description: "对话 ID 或 @用户名"},
  ],
  examples: [{args: "-1001234567890 123456789 20 @targetchat"}, {args: "@sourcechat @alice 30 -1009876543210"}],
  help: [
    {
      heading: "说明：",
    ...
    {
      heading: "定时任务：",
      body: "若想实现定时任务，可安装并使用 <code>{prefix}tpm i acron</code>：\n" +
        "每天 2 点从来源对话 <code>@group</code> 获取 <code>@user</code> 的 20 条消息并发送到收藏夹（Saved Messages）。\n" +
        "<pre>{prefix}acron cmd 0 0 2 * * * me 尾行\n{prefix}biko @group @user 20 me</pre>",
    },
  ],
  async handle(invocation, context) {
    if (invocation.args[0]?.toLowerCase() === "help" || invocation.args.length !== 4) {
      await context.telegram.edit(invocation.message, renderCommandHelp("biko", bikoCommand, {prefix: invocation.prefix}), {parseMode: "html", linkPreview: false});
      return;
    }
...
export default function createBiko() {
  return definePlugin({apiVersion: STRUCTURED_PLUGIN_API_VERSION, id: "biko", description: "整理指定用户在来源对话中的消息并发送到目标对话",
    renderHelp: prefix => renderCommandHelp("biko", bikoCommand, {prefix, title: "📦 Biko - 批量获取整理发送指定对话中指定用户的消息"}),
    commands: {biko: bikoCommand}});
}
