...
function display(entity: any, fallback: string): string {
  return (
    [entity?.title, entity?.firstName, entity?.lastName, entity?.username && `@${entity.username}`, entity?.id]
...
function mediaText(message: any): string {
  ...
  if (message?.className === "MessageService") {
    const action = String(message?.action?.className ?? "")
  ...
  const documentMarker = attributes.some((attribute: any) => attribute?.className === "DocumentAttributeSticker")
    ? "[贴纸]"
    : attributes.some((attribute: any) => attribute?.className === "DocumentAttributeAudio" && attribute?.voice)
      ? "[语音]"
      : attributes.some((attribute: any) => attribute?.className === "DocumentAttributeAudio")
        ? "[音频]"
        : attributes.some((attribute: any) => attribute?.className === "DocumentAttributeVideo")
          ? "[视频]"
          : attributes.some((attribute: any) => attribute?.className === "DocumentAttributeAnimated")
            ? "[动图]"
            : "[文档]";
  const marker = message?.photo
    ? "[图片]"
    : message?.video
      ? "[视频]"
      : message?.voice
          ...
          : message?.sticker
            ...
            : message?.document
              ...
              : message?.poll
                ...
                : message?.contact
                  ? "[联系人]"
                  : message?.location || message?.venue
                    ? "[位置]"
                    : message?.media
                      ? "[媒体消息]"
                      : "";
...
async function matches(message: any, selector: Selector, signal: AbortSignal): Promise<boolean> {
  ...
  const direct = String(message?.sender?.username ?? "")
    .replace(/^@/, "")
    .toLowerCase();
...
export default function createBiko() {
  return definePlugin({
    ...
    apiVersion: 1,
    id: "biko",
    description: "整理指定用户在来源对话中的消息并发送到目标对话",
    commands: {
      biko: {
        helpArgs: ["help"],
        helpOnEmpty: true,
        description: "跨对话整理指定用户的近期消息",
            ...
            await context.telegram.withClient(async (client, signal) => {
              ...
              try {
                ...
                selector = /^-?\d+$/.test(userRaw!)
                  ...
                  : {
                      username: userRaw!.replace(/^@/, "").toLowerCase(),
                      display: `@${userRaw!.replace(/^@/, "").toLowerCase()}`,
                    };
              }
              ...
              const collect = async (active: Selector, limit: number): Promise<RecordItem[]> => {
                ...
                for await (const value of client.iterMessages(source, {
                  limit,
                  ...(active.entity ? { fromUser: active.entity } : {}),
                })) {
                  signal.throwIfAborted();
              ...
              let records = await collect(
                selector,
                selector.entity ? count : Math.min(Math.max(count * 20, count + 50), MAX_SCAN),
              );
              signal.throwIfAborted();
