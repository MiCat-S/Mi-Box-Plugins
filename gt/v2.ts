...
class OutputDeliveryError extends Error {
  constructor() { super("GT_OUTPUT_DELIVERY_FAILED"); }
}
...
function* chunks(text: string): Generator<string> {
  ...
  for (const character of text) {
    ...
    if (chunk.length + escaped.length > 3000) { yield chunk; chunk = ""; }
    chunk += escaped;
  }
      ...
      gt: {helpArgs: ["help","h"], description: "AI 翻译", async handle({message, prefix}, context) {
        try {
          ...
          if (!context.services.available("ai", "translate")) {
            await context.telegram.edit(message, "❌ 请先安装或更新配套 ai 插件，并配置 ai model chat", {parseMode: "html"});
            return;
          }
          ...
          output[0] = `🌐 <b>AI 翻译结果</b> (→ ${target === "en" ? "英文" : "中文"})\n\n` +
            `<b>原文:</b>\n<code>${escape(preview)}${preview.length < text.length ? "..." : ""}</code>\n\n` +
            `<b>译文:</b>\n${output[0]}`;
          const delivery = await ui.deliverPages(output, context.signal, (page, index) => index
          ...
          if (delivery.interrupted) {
            context.log.info("gt_delivery_interrupted", {published: delivery.published, total: delivery.total, category: ui.deliveryErrorCategory(delivery.error)});
          ...
          if (!context.signal.aborted) await context.telegram.edit(message,
            "❌ AI 翻译失败，请检查 ai 聊天配置、API 可用性及超时设置后重试", {parseMode: "html"});
        }
      }},
    },
  });
