  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "httpcat", description: "发送 HTTP 状态码对应的图片",
    commands: {httpcat: {description: "发送 HTTP 状态码图片", async handle(invocation, ctx) {
      ...
      try {
        ...
        const data = await ctx.http.withResponse(`https://http.cat/${code}`, {
          method: "GET", redirect: "manual", credentials: "omit",
          headers: {"Accept": "image/jpeg", "User-Agent": "Mi-Box-Httpcat/1.0"},
        }, (response, signal) => readImage(response, signal), {timeoutMs: 60_000, signal: ctx.signal, redirects:{allowedHosts:["http.cat"],maxRedirects:2}});
        ...
        await ctx.telegram.withClient(async (client, signal) => {
          ...
          signal.throwIfAborted();
          const raw = invocation.message.raw as {peerId?: unknown; delete?: (options?: {revoke?: boolean}) => Promise<unknown>} | undefined;
          ...
          if (typeof raw.delete === "function") {
            ...
            catch { ctx.log.error("httpcat.delete_failed"); }
          }
        });
      } catch {
