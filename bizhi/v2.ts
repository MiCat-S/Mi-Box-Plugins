...
async function image(
  ...
  url: URL,
  headers: Readonly<Record<string, string>> = { Accept: "image/*" },
): Promise<Buffer> {
...
function wallpapers(response: unknown): Wallpaper[] {
  ...
  return entries.filter(value => {
    ...
    return (
      ...
      Number.isFinite(item.dimension_x) &&
      Number.isFinite(item.dimension_y) &&
...
async function searchWallhaven(context: PluginContext, params: URLSearchParams): Promise<Wallpaper[]> {
  const response = await context.http.json<unknown>(
    `https://wallhaven.cc/api/v1/search?${params}`,
    {
      method: "GET",
      redirect: "manual",
      credentials: "omit",
      headers: { Accept: "application/json", "User-Agent": "TeleBox-Bot/1.0" },
    },
...
async function wallhaven(context: PluginContext, category: string): Promise<Download> {
  ...
  const qualified = valid.filter(
    item => item.dimension_x >= 1920 && item.dimension_y >= 1080 && item.file_size >= 3 * 1024 * 1024,
  );
  let selected = (qualified.length ? qualified : valid)[
    Math.floor(Math.random() * (qualified.length ? qualified.length : valid.length))
  ]!;
  if (selected.dimension_x < 1920 || selected.dimension_y < 1080 || selected.file_size < 3 * 1024 * 1024) {
    ...
    if (retried.length) {
      const retryQualified = retried.filter(
        item => item.dimension_x >= 2560 && item.dimension_y >= 1440 && item.file_size >= 3 * 1024 * 1024,
      );
...
async function fallback(context: PluginContext, category: string): Promise<Download> {
  ...
  const response = await context.http.json<unknown>(
    ...
    { timeoutMs: 60_000, signal: context.signal, redirects: { allowedHosts: ["api.btstu.cn"], maxRedirects: 2 } },
  );
  ...
  return {
    data: await image(context, target),
    filename: `bizhi_${category || "suiji"}.jpg`,
    source: `${target.href}\n📊 来源: btstu.cn`,
  };
}
      ...
      bizhi: {
        ...
        async handle(invocation, context) {
          ...
          try {
            let result: Download;
            try {
              result = await wallhaven(context, category);
            } catch {
              context.signal.throwIfAborted();
              result = await fallback(context, category);
            }
            ...
            await context.telegram.withClient(async (client, signal) => {
              ...
              const peer =
                raw?.peerId ?? (await import("teleproto/Helpers.js")).returnBigInt(invocation.message.chatId);
              const file = new CustomFile(result.filename, result.data.length, "", result.data);
              await client.sendFile(peer, {
                file,
                replyTo: raw?.replyTo?.replyToTopId ?? invocation.message.id,
                caption: `${sendAsFile ? "📁 源文件" : "📸 来源"}: ${result.source}`,
                forceDocument: sendAsFile,
              });
              signal.throwIfAborted();
              if (typeof raw?.delete === "function") {
                try {
                  ...
                  context.log.error("bizhi_delete_failed");
                }
                signal.throwIfAborted();
              }
            });
