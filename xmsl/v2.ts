...
function normalize(value: Partial<State>): State {
  ...
  try {
    if (baseUrl) baseUrl = apiRoot(baseUrl).toString().replace(/\/$/, "");
  } catch {
    /* A malformed legacy URL must not prevent the plugin from loading. */
  }
...
async function migrate(context: PluginContext): Promise<void> {
  ...
  await context.services.call(
    ...
    "import_provider",
    {
      ...
      url: current.baseUrl,
      key: current.apiKey,
      type: current.apiMode === "gemini" ? "gemini" : "openai-compatible",
      models: { chat: current.model },
      select: ["chat"],
    },
    context.signal,
  );
...
export async function download(
  ...
  await context.telegram.withClient(async (client: any, clientSignal) => {
    const signal = AbortSignal.any([context.signal, callerSignal, clientSignal]);
    signal.throwIfAborted();
  ...
  const value = await readFile(target, { signal: callerSignal });
  callerSignal.throwIfAborted();
  return value;
}
...
function resultPages(value: string): string[] {
  ...
  for (const character of value) {
    ...
    if (current && current.length + encoded.length > 3500) {
      chunks.push(current);
      current = "";
    }
    current += encoded;
...
async function deliverResult(invocation: any, context: PluginContext, value: string): Promise<void> {
  const delivery = await ui.deliverPages(resultPages(value), context.signal, (page, index) =>
    index === 0
  ...
  if (delivery.published) {
    ...
    try {
      ...
      context.signal.throwIfAborted();
      context.log.error("xmsl_delivery_notice_failed");
    }
    return;
  }
  context.log.error("xmsl_delivery_failed");
  try {
    await context.telegram.edit(invocation.message, "XMSL 结果发送失败，请重新执行");
  } catch {
    context.signal.throwIfAborted();
    context.log.error("xmsl_delivery_notice_failed");
  }
...
async function mediaImage(context: PluginContext, message: MessageEnvelope): Promise<Image | undefined> {
  ...
  try {
    return await context.files.withTemp(async (directory, signal) => {
      ...
      const sticker = Boolean(
        raw.media?.document?.attributes?.some(
          (item: any) =>
            item?.className === "DocumentAttributeSticker" || item?.constructor?.name === "DocumentAttributeSticker",
        ),
      );
      if (!mime.startsWith("image/") && mime !== "video/webm" && mime !== "application/x-tgsticker" && !sticker) return;
      const input = path.join(
        directory,
        mime === "video/webm" ? "input.webm" : mime === "application/x-tgsticker" ? "input.tgs" : "input.bin",
      );
      ...
      if (mime === "video/webm") {
        ...
        await helper(context, directory, active, FFMPEG, [
          "-nostdin",
          "-y",
          "-protocol_whitelist",
          "file",
        ...
        const gif = path.join(directory, "animation.gif"),
          output = path.join(directory, "frame.png");
    ...
    if (produced) {
      context.log.error("xmsl_temp_cleanup_failed");
      return produced;
    }
    throw error;
  }
...
async function showState(
  invocation: Parameters<CommandDefinition["handle"]>[0],
  context: PluginContext,
): Promise<void> {
  await migrate(context);
  context.signal.throwIfAborted();
  if (!context.services.available("ai", "selection")) {
    await context.telegram.edit(invocation.message, "请先安装并配置 ai 插件");
    return;
  }
  const selection = await context.services.call<{ chat?: { tag?: string; model?: string } }>(
    "ai",
    "selection",
    null,
    context.signal,
  ...
  await context.telegram.edit(
    invocation.message,
    `<b>XMSL 状态</b>\nAI：<code>${escape(selection.chat?.tag || "-")} / ${escape(selection.chat?.model || "-")}</code>`,
    { parseMode: "html" },
  );
}
...
async function handle(invocation: any, context: PluginContext): Promise<void> {
  try {
    ...
    if (!invocation.args.length && invocation.message.replyToId === undefined) {
      await showState(invocation, context);
      return;
    }
    await migrate(context);
    context.signal.throwIfAborted();
    ...
    if (!text && invocation.message.replyToId !== undefined) {
      ...
      if (reply) {
        ...
        try {
          image = await mediaImage(context, reply);
        } catch {
          context.signal.throwIfAborted();
    ...
    if (text.length > MAX_INPUT) {
      await context.telegram.edit(invocation.message, "输入内容过长");
      return;
    }
    await context.telegram.edit(invocation.message, image ? "正在识别图片…" : "处理中…");
    const answer = await context.services.call<string>(
      "ai",
      "chat",
      {
        text: text || "请识别这张图片/贴纸的内容",
    ...
    context.signal.throwIfAborted();
    const withoutThink = answer.includes("</think>")
      ? answer.slice(answer.lastIndexOf("</think>") + 8).trim()
      : answer.trim();
...
const command: CommandDefinition = {
  ...
  subcommands: {
    ...
    show: {
      description: "查看模式、地址、模型及密钥是否配置",
      args: "",
      async handle(i, context) {
        try {
          await showState(i, context);
        } catch {
          if (!context.signal.aborted) {
            context.log.error("xmsl_failed");
            await context.telegram.edit(i.message, "XMSL 调用失败，请检查配置、媒体依赖和网络");
          }
        }
      },
    },
  ...
  help: [
    {
      heading: "图片和贴纸：",
      body: "支持 JPEG/PNG/GIF/WebP、WebM 视频贴纸和 TGS 动态贴纸。WebM 需要 FFmpeg；TGS 需要 Python、rlottie-python 和 FFmpeg，读取首帧。媒体最多 20 MiB，文本最多 50000 字符。",
    },
    {
      heading: "AI 配置：",
      body: "<code>{prefix}xm</code> 与 <code>{prefix}xmsl</code> 使用相同参数。供应商、密钥与聊天模型由 ai 插件统一管理。",
    },
  ],
  handle,
};
