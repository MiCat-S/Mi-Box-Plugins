...
async function helper(
  ...
  for (const command of candidates) {
    try {
      return await context.processes.run(command, args, { cwd, timeoutMs, maxOutputBytes: 256 * 1024 });
    } catch (error) {
...
function stickerSet(
  document: any,
  Api: any,
  toLong: typeof import("teleproto/Helpers.js").returnBigInt,
): any | undefined {
  ...
  if (set instanceof Api.InputStickerSetShortName && set.shortName)
    return new Api.InputStickerSetShortName({ shortName: set.shortName });
  const id = set?.id ?? set?._id;
  const accessHash = set?.accessHash ?? set?.access_hash;
  if (id !== undefined && accessHash !== undefined)
    return new Api.InputStickerSetID({ id: toLong(id), accessHash: toLong(accessHash) });
}
...
function stickerDocument(source: any, Api: any): any | undefined {
  ...
  const document =
    source?.sticker ?? source?.document ?? media?.document ?? (media instanceof Api.Document ? media : undefined);
...
async function convert(
  ...
  try {
    if (kind === "tgs") {
      ...
      await helper(context, PYTHON, [script, input, output], 120_000);
      return true;
    }
    const filter =
      kind === "mp4"
    ...
    await helper(
      context,
      FFMPEG,
      ["-nostdin", "-y", "-i", input, "-vf", filter, ...(kind === "webp" ? ["-loop", "0"] : []), output],
      120_000,
    );
    return true;
  } catch (error) {
    ...
    context.log.info("getstickers_conversion_skipped", { kind });
    return false;
  }
}
...
async function createArchive(source: string, target: string, signal: AbortSignal): Promise<void> {
  ...
  const archiveStopped = new Promise<void>(resolve => {
    resolveArchiveStopped = resolve;
  });
  const outputStopped = new Promise<void>(resolve => {
    resolveOutputStopped = resolve;
  });
  const complete = () => {
    if (!settled && archiveFinalized && outputClosed) {
      settled = true;
      resolveTerminal();
    }
  };
...
function help(prefix: string): string {
  return `回复贴纸后发送 <code>${prefix.replace(/[&<>]/g, "")}getstickers</code>，将整包转换并打包为 ZIP`;
}
    ...
    commands: {
      getstickers: {
        ...
        async handle(invocation, context) {
          try {
            const reply =
              invocation.message.replyToId === undefined
                ? undefined
                : await context.telegram.getReply(invocation.message);
            context.signal.throwIfAborted();
            ...
            await context.telegram.withClient(async (client, clientSignal) => {
              ...
              await context.files.withTemp(async (directory, tempSignal) => {
                ...
                for (const item of result?.packs ?? [])
                  for (const id of item?.documents ?? []) emojiById.set(String(id), String(item?.emoticon ?? ""));
                for (let index = 0; index < documents.length; index++) {
                  signal.throwIfAborted();
                  const item: any = documents[index];
                  const kind = extension(item, Api);
                  const declaredSize = byteSize(item.size);
                  if (declaredSize !== undefined && (declaredSize <= 0n || declaredSize > BigInt(MAX_ITEM)))
                    throw new Error("Invalid sticker size");
                  const stem = String(index).padStart(3, "0");
                  const sourceFile = path.join(pack, `${stem}.${kind}`);
                  await client.downloadFile(
                    new Api.InputDocumentFileLocation({
                      id: returnBigInt(item.id),
                      accessHash: returnBigInt(item.accessHash),
                      fileReference: item.fileReference ?? Buffer.alloc(0),
                      thumbSize: "",
                    }),
                    {
                      ...
                      progressCallback(downloaded) {
                        signal.throwIfAborted();
                        const received = byteSize(downloaded);
                        if (received === undefined || received > BigInt(MAX_ITEM)) throw new Error("Sticker too large");
                      },
                    },
                  );
                  signal.throwIfAborted();
                  ...
                  if (
                    !sourceInfo.isFile() ||
                    !sourceInfo.size ||
                    sourceInfo.size > MAX_ITEM ||
                    (declaredSize !== undefined && BigInt(sourceInfo.size) !== declaredSize)
                  )
                    throw new Error("Invalid sticker download");
                  signal.throwIfAborted();
                  const converted = path.join(pack, `${stem}.gif`);
                  const ok = await convert(context, sourceFile, converted, kind, directory);
                  signal.throwIfAborted();
                  if (ok) {
                    const convertedInfo = await stat(converted);
                    signal.throwIfAborted();
                    if (!convertedInfo.isFile() || !convertedInfo.size || convertedInfo.size > MAX_ITEM)
                      throw new Error("Converted sticker too large");
                  ...
                  await appendFile(
                    packFile,
                    JSON.stringify({ image_file: finalName, emojis: emojiById.get(String(item.id)) ?? "" }) + "\n",
                    { encoding: "utf8", mode: 0o600 },
                  );
                  signal.throwIfAborted();
                  if (index === 0 || (index + 1) % 10 === 0 || index + 1 === documents.length) {
                    await context.telegram.edit(
                      invocation.message,
                      `正在下载 ${documents.length} 张贴纸… ${index + 1}/${documents.length}`,
                    );
                    signal.throwIfAborted();
                  }
                }
                ...
                if (!info.isFile() || !info.size || info.size > MAX_ARCHIVE) throw new Error("Invalid archive");
                signal.throwIfAborted();
                ...
                if (typeof raw?.delete === "function") {
                  try {
                    ...
                    context.log.info("getstickers_command_cleanup_failed");
                  }
                }
              });
            });
