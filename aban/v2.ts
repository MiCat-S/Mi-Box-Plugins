...
export default function createAban() {
    ...
    const run = async () => {
      ...
      await ctx.telegram.withClient(async (native, signal) => {
        ...
        const peerId =
          raw?.peerId ??
          (chat.startsWith("-100")
            ? new Api.PeerChannel({ channelId: integer(chat.slice(4)) })
            : chat.startsWith("-")
              ? new Api.PeerChat({ chatId: integer(chat.slice(1)) })
              : new Api.PeerUser({ userId: integer(chat) }));
        const message = Object.create(raw ?? null);
        Object.defineProperties(message, {
          id: { value: inv.message.id },
          peerId: { value: peerId },
        ...
        if (inv.command === "refresh") {
          ...
          await runtime.CommandHandlers.handleBasicCommand(
            client,
            message,
            inv.command as "ban" | "kick" | "unban" | "mute" | "unmute",
          );
        signal.throwIfAborted();
      });
    };
  ...
  return definePlugin({
    ...
    commands: Object.fromEntries(
      Object.entries(commands).map(([name, description]) => [name, { description, ignoreEdited: true, handle }]),
    ),
  });
}
