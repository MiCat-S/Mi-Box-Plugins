...
const regions: [string, string[]][] = [
  ["香港", ["香港", "hong kong", "hongkong", "hkg", "hk"]],
  ["台湾", ["台湾", "taiwan", "taipei", "tpe", "tw"]],
  ["日本", ["日本", "japan", "osaka", "jap", "jp", "tokyo"]],
  ["新加坡", ["新加坡", "singapore", "sgp", "sg"]],
  ["韩国", ["韩国", "korea", "seoul", "kor", "kr"]],
  ["印度", ["印度", "india"]],
  ["马来西亚", ["马来西亚", "malaysia"]],
  ["泰国", ["泰国", "thailand"]],
  ["越南", ["越南", "vietnam"]],
  ...
  ["加拿大", ["加拿大", "canada", "ca"]],
  ["英国", ["英国", "united kingdom", "uk", "london"]],
  ["德国", ["德国", "germany", "de"]],
  ["法国", ["法国", "france"]],
  ["荷兰", ["荷兰", "netherlands"]],
  ["瑞士", ["瑞士", "switzerland"]],
...
function nodeName(line: string, type: string, index: number): string {
  ...
  if (hash >= 0 && line.slice(hash + 1)) {
    try {
      return decodeURIComponent(line.slice(hash + 1));
    } catch {
      return line.slice(hash + 1);
    }
  }
  if (type === "vmess") {
    try {
      const data: unknown = JSON.parse(Buffer.from(line.slice(line.indexOf("://") + 3), "base64url").toString("utf8"));
      if (data && typeof data === "object" && "ps" in data && typeof data.ps === "string" && data.ps.trim())
        return data.ps;
    } catch {
      /* Invalid node metadata does not invalidate other subscription entries. */
    }
  }
  if (type === "ssr") {
...
function parse(raw: string) {
  ...
  if (/^(?:proxies\s*:|\{)/m.test(text)) {
    ...
    if (!config || typeof config !== "object" || !("proxies" in config) || !Array.isArray(config.proxies))
      throw new Error("无效的 Clash 节点列表");
    for (const proxy of config.proxies) {
      if (!proxy || typeof proxy !== "object" || typeof proxy.type !== "string" || !proxy.type.trim())
        throw new Error("无效的 Clash 节点");
      ...
      nodes.push({
        ...
        name:
          typeof proxy.name === "string" && proxy.name.trim()
            ? proxy.name
            : `${type.toUpperCase()} ${nodes.length + 1}`,
      });
    }
  } else {
  ...
  for (const { type, name } of nodes) {
    ...
    const found = regions.find(([, keys]) =>
      keys.some(key =>
        /^[a-z]+$/.test(key) ? new RegExp(`(?:^|[^a-z])${key}(?:$|[^a-z])`).test(lower) : lower.includes(key),
      ),
    );
    if (found) regionCounts[found[0]] = (regionCounts[found[0]] ?? 0) + 1;
  }
  if (nodes.length - Object.values(regionCounts).reduce((a, b) => a + b, 0) > 0)
    regionCounts["其他"] = nodes.length - Object.values(regionCounts).reduce((a, b) => a + b, 0);
...
async function mappings(ctx: PluginContext): Promise<Record<string, string>> {
  try {
    const { response, text } = await fetchBoundedText(ctx, MAPPINGS_URL, 256 * 1024, 10_000, [
    ...
    for (const raw of text.split(/\r?\n/)) {
      const line = raw.trim(),
        at = line.indexOf("=");
      if (line && !line.startsWith("#") && at > 0) result[line.slice(0, at).trim()] = line.slice(at + 1).trim();
    }
    return result;
  } catch {
...
function mappedName(url: string, map: Record<string, string>) {
  for (const [key, value] of Object.entries(map)) if (key && url.includes(key)) return value;
  return null;
}
function headerName(value: string | null) {
...
async function websiteInfo(ctx: PluginContext, url: string): Promise<SiteInfo> {
  ...
  for (const candidate of [`${origin}/auth/login`, `${origin}/`])
    try {
      const { response, text } = await fetchBoundedText(ctx, candidate, 512 * 1024, 5_000, [target.hostname], headers);
      if (response.ok) {
        const title = titleOf(text);
        return {
          website: origin,
          name:
            title?.includes("Cloudflare") || title?.includes("Just a moment")
              ? "Cloudflare防御"
              : title?.includes("Access denied") || title?.includes("404 Not Found")
                ? "非机场面板域名"
                : title,
        };
      }
...
async function pagesFor(
  ...
  const types =
    Object.entries(result.counts)
      ...
      .join("\n") || "未识别常见节点协议";
  const regionText = Object.entries(result.regionCounts)
    .map(([name, count]) => `${name}: ${count}`)
    .join(" · ");
  const config = mappedName(url, mapping) ?? headerName(subscription.contentDisposition) ?? site.name ?? "未知";
  const profile =
    subscription.profileUrl && /^https?:\/\//i.test(subscription.profileUrl) ? subscription.profileUrl : site.website;
...
async function deliver(ctx: PluginContext, message: MessageEnvelope, pages: readonly string[]): Promise<void> {
  ...
  const result = await ui.deliverPages(labeled, ctx.signal, (page, index) =>
    index
      ? ctx.telegram.reply(message, page, { parseMode: "html" })
      : ctx.telegram.edit(message, page, { parseMode: "html" }),
  );
  if (!result.interrupted) return;
  ctx.log.error("subinfo_page_delivery_failed", {
...
function command(concise: boolean): CommandDefinition {
  return {
    helpArgs: ["help", "h"],
    description: concise ? "简洁查询订阅信息" : "查看订阅基础信息",
      ...
      try {
        ...
        if (!input.values.length) {
          await deliver(
            ctx,
            invocation.message,
            await ui.renderRichText(renderPluginHelp(invocation.prefix), ui.PAGE_LABEL_RESERVE),
          );
          return;
        }
        ...
        for (const url of input.values) {
          ctx.signal.throwIfAborted();
          try {
            const site = await websiteInfo(ctx, url),
              subscription = await fetchSubscription(ctx, url);
            all.push(...(await pagesFor(url, subscription, concise, site, mapping)));
          } catch {
            ctx.signal.throwIfAborted();
            const failure =
              input.values.length === 1
                ? "订阅读取或解析失败，请稍后重试"
                : `<b>订阅链接</b>：<code>${esc(url)}</code>\n订阅读取或解析失败，请稍后重试`;
            all.push(...(await ui.renderRichText(failure, ui.PAGE_LABEL_RESERVE)));
          }
        }
        if (input.txt) {
          ...
          await ctx.telegram.withClient(async (client, signal) => {
            ...
            await client.sendFile(peer, {
              ...
              caption: `✅ 订阅查询报告（共 ${input.values.length} 个链接）`,
          ...
          try {
            ...
            ctx.signal.throwIfAborted();
            ctx.log.error("subinfo_command_cleanup_failed", { category: "DELETE_FAILED" });
          }
        } else await deliver(ctx, invocation.message, all);
      } catch {
        if (!ctx.signal.aborted) await ctx.telegram.edit(invocation.message, "订阅读取或解析失败，请稍后重试");
      }
    },
  };
}
export default function createSubinfo() {
  return definePlugin({
    ...
    id: "subinfo",
    description: "查看订阅基础信息",
    commands: { subinfo: command(false), cha: command(true) },
  });
}
