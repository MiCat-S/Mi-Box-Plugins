...
由 `scripts/update-readme.js` 根据 `plugins.json` 自动生成，只列出有 V2 入口、可以通过 TPM 安装的插件。
请勿手动编辑下面两个标记之间的内容。
...
- `bulk_delete` - 批量删除消息
- `calc` - 计算器
- `checkapi` - 检测 ai 插件中的 API、模型与对话
- `checkin` - 自动签到插件
- `clean` - 账号清理工具 Pro
- `clean_member` - 群组成员清理
- `clear_sticker` - 批量删除群组内贴纸
- `codex_image` - 使用统一 AI 图片服务生成或编辑图片
- `convert` - 视频转音频
...
- `news` - 每日新闻
- `nezha` - 哪吒监控
- `nodeseek` - NodeSeek 签到、Cookie 与自动签到设置
- `ntp` - NTP 时间同步
- `openlist` - openlist管理
...
2. 包含完整的功能说明
3. 添加适当的错误处理
4. 在 plugins.json 登记插件描述（TPM 搜索显示的就是它），并运行 `node scripts/update-readme.js`
   同步 README 插件列表；PR 会检查列表是否已同步
