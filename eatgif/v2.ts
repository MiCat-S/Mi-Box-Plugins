...
const escape = (value: unknown): string => String(value ?? "").replace(/[&<>"']/g, character =>
  ({"&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#x27;"})[character]!);
const renderList = (catalog: Catalog, prefix: string): string => `<b>头像动图表情</b>\n<code>${escape(prefix)}eatgif 名称</code>（需回复目标）\n\n` +
  Object.entries(catalog).map(([name, value]) => `• <code>${escape(name)}</code> - ${escape(value.desc)}`).join("\n");
async function deliverList(context: PluginContext, message: MessageEnvelope, catalog: Catalog, prefix: string, missing?: string): Promise<void> {
  const source = `${missing ? `未找到：<code>${escape(missing)}</code>\n\n` : ""}${renderList(catalog, prefix)}`;
  const pages = (await ui.renderRichText(source, ui.PAGE_LABEL_RESERVE)).map((page, index, all) => page + ui.pageLabel(index, all.length));
  const delivery = await ui.deliverPages(pages, context.signal, (page, index) =>
    index ? context.telegram.reply(message, page, {parseMode: "html"}) : context.telegram.edit(message, page, {parseMode: "html"}));
  if (!delivery.interrupted) return;
  context.log.info("eatgif_pagination_interrupted", {published: delivery.published, total: delivery.total,
...
export async function responseBytes(context: PluginContext, url: URL, maximum: number): Promise<Buffer> {
  return context.http.withResponse(url, {credentials: "omit"}, async (response, signal) => {
    ...
    try { while (true) { signal.throwIfAborted(); const item = await reader.read(); signal.throwIfAborted(); if (item.done) break;
...
async function json<T>(context: PluginContext, relative: string): Promise<T> {
  const file = safeRelative(relative); const data = await responseBytes(context, new URL(file, ROOT), 1024 * 1024);
...
async function ffmpeg(context: PluginContext, cwd: string, args: readonly string[]) {
  for (const command of FFMPEG) {
    try { return await context.processes.run(command, args, {cwd, timeoutMs: 180_000, maxOutputBytes: 256 * 1024}); }
...
export function validateDetail(detail: Detail): Detail {
  if (!Number.isInteger(detail?.width) || !Number.isInteger(detail?.height) || detail.width < 1 || detail.height < 1 ||
      detail.width > MAX_DIMENSION || detail.height > MAX_DIMENSION || !Array.isArray(detail.res) ||
      detail.res.length < 1 || detail.res.length > MAX_FRAMES ||
      detail.width * detail.height * detail.res.length > MAX_ANIMATION_PIXELS) throw new Error("Invalid animation");
  for (const entry of detail.res) {
    safeRelative(entry?.url);
    if (entry.delay !== undefined && (!Number.isFinite(entry.delay) || entry.delay < 1 || entry.delay > 5000)) throw new Error("Invalid animation");
    for (const role of [entry.me, entry.you]) {
      if (!role) continue;
      safeRelative(role.mask);
      if (!Number.isInteger(role.x) || !Number.isInteger(role.y) ||
          (role.rotate !== undefined && (!Number.isFinite(role.rotate) || role.rotate < -360 || role.rotate > 360)) ||
          (role.brightness !== undefined && (!Number.isFinite(role.brightness) || role.brightness < 0.1 || role.brightness > 2))) {
...
export async function downloadAvatarDirect(client: any, entityLike: EntityLike, signal: AbortSignal): Promise<Buffer | undefined> {
  signal.throwIfAborted();
  const entity = entityLike instanceof Api.User || entityLike instanceof Api.Chat || entityLike instanceof Api.Channel
    ? entityLike
  ...
  const result = await client.invoke(new Api.upload.GetFile({
    location: new Api.InputPeerPhotoFileLocation({peer: utils.getInputPeer(entity), photoId: photo.photoId, big: false}),
    offset: helpers.returnBigInt(0), limit: 512 * 1024, precise: true,
  }), photo.dcId);
  signal.throwIfAborted();
  if (!(result instanceof Api.upload.File) || !Buffer.isBuffer(result.bytes) || !result.bytes.length || result.bytes.length > MAX_AVATAR_BYTES) return;
  return result.bytes;
}
async function masked(sharp: typeof import("sharp"), loadAsset: (context: PluginContext, relative: string) => Promise<Buffer>,
  context: PluginContext, role: Role, face: Buffer): Promise<OverlayOptions> {
  const mask = await loadAsset(context, role.mask); const metadata = await sharp(mask, {limitInputPixels: MAX_ANIMATION_PIXELS}).metadata();
...
export async function uploadResult(context: PluginContext, raw: ApiTypes.Message, upload: string, webm: string,
  replyToId: number | undefined, tempSignal: AbortSignal): Promise<void> {
  await context.telegram.withClient(async (client, clientSignal) => {
    ...
    signal.throwIfAborted(); const {Api} = await import("teleproto"); signal.throwIfAborted();
    await client.sendFile(raw.peerId, {file: upload, replyTo: replyToId,
      ...(upload === webm ? {attributes: [new Api.DocumentAttributeSticker({alt: "✨", stickerset: new Api.InputStickerSetEmpty()})]} : {})});
    signal.throwIfAborted();
    if (typeof raw.delete === "function") { try { await raw.delete({revoke: true}); } catch { context.log.info("eatgif_receipt_cleanup_failed"); } }
    signal.throwIfAborted();
  });
}
...
export default function createEatGif() {
  ...
  const getCatalog = async (context: PluginContext): Promise<Catalog> => {
    ...
    const promise = json<Catalog>(context, "config.json").then(value => {
      if (generation === current) catalog = value;
      return value;
    }).finally(() => { if (catalogLoad?.promise === promise) catalogLoad = undefined; });
  ...
  const loadAsset = async (context: PluginContext, relative: string): Promise<Buffer> => {
    ...
    const promise = (async () => {
      ...
      try {
        const info = await stat(target); context.signal.throwIfAborted();
      ...
      await mutateCache(async () => {
        ...
        try { if (generation === current) await rename(staging, target); }
        finally { await unlink(staging).catch(() => undefined); }
      });
      return data;
    })().finally(() => { if (assetLoads.get(key)?.promise === promise) assetLoads.delete(key); });
  ...
  return definePlugin({renderHelp: renderPluginHelp, apiVersion: 1, id: "eatgif", description: "将双方头像合成为动画贴纸",
    ...
    commands: {eatgif: {helpArgs: ["help","h"], description: "生成头像融合动画", async handle(invocation, context) {
      ...
      try {
        if (sub === "clear") { await mutateCache(async () => { generation += 1; catalog = undefined; catalogLoad = undefined; assetLoads.clear();
        ...
        if (!sub || sub === "list" || sub === "ls" || sub === "help" || sub === "h") {
          await deliverList(context, invocation.message, list, invocation.prefix); return;
        }
        const selected = list[sub]; if (!selected) { await deliverList(context, invocation.message, list, invocation.prefix, sub); return; }
        ...
        await context.files.withTemp(async (directory, signal) => {
          ...
          const faces = await context.telegram.withClient(async (client, clientSignal) => {
            ...
            operationSignal.throwIfAborted(); const meEntity = await client.getMe(); operationSignal.throwIfAborted();
            const me = await downloadAvatarDirect(client, meEntity, operationSignal); operationSignal.throwIfAborted();
            const you = await downloadAvatarDirect(client, target, operationSignal); operationSignal.throwIfAborted();
            return {me, you};
          });
          if (!faces.me) { await context.telegram.edit(invocation.message, "无法获取自己的头像"); return; }
          if (!faces.you) { await context.telegram.edit(invocation.message, "无法获取对方的头像"); return; }
          ...
          for (const entry of detail.res) {
            ...
            if (metadata.width !== detail.width || metadata.height !== detail.height) throw new Error("Invalid frame");
          ...
          const handle = await open(gif, "wx", 0o600); try { await handle.writeFile(gifData); } finally { await handle.close(); }
          let upload = gif;
          try {
            await ffmpeg(context, directory, ["-nostdin", "-y", "-protocol_whitelist", "file", "-i", "output.gif", "-c:v", "libvpx-vp9", "-b:v", "0", "-crf", "41", "-pix_fmt", "yuva420p", "-auto-alt-ref", "0", "-fs", String(MAX_OUTPUT_BYTES), "output.webm"]);
    ...
    }}}, cleanup() { generation += 1; catalog = undefined; catalogLoad = undefined; assetLoads.clear(); },
  });
}
