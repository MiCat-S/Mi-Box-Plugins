...
async function bytes(response: Response, signal: AbortSignal, maximum: number): Promise<Buffer> {
  ...
  try {
    for (;;) {
      ...
      if (total > maximum) throw new Error("服务响应过大");
...
function decoded(value: unknown): Buffer {
  if (
    typeof value !== "string" ||
    !value ||
    value.length > Math.ceil((MAX_AUDIO_BYTES * 4) / 3) + 8 ||
  ...
  const data = await ctx.http.withResponse(
    `https://${host}/v1/chat/completions`,
    {
      ...
      headers: {
        ...
        "User-Agent": "TeleBox-Say/2",
      },
      body: JSON.stringify({
        model: "mimo-v2.5-tts",
        messages,
        audio: { format: "wav", voice: current.voice || "mimo_default" },
      }),
    },
...
async function volc(ctx: PluginContext, value: string, config: SayConfig): Promise<Omit<SynthResult, "provider">> {
  const current = config.providers.volc;
  const host = "openspeech.bytedance.com";
  const body = await ctx.http.withResponse(
    `https://${host}/api/v3/tts/unidirectional`,
    {
      method: "POST",
      credentials: "omit",
      headers: {
        "X-Api-Key": current.apiKey,
        "X-Api-Resource-Id": current.resourceId || "seed-tts-2.0",
      ...
      body: JSON.stringify({
        req_params: {
          ...
          audio_params: { format: "ogg_opus", sample_rate: 48_000, speech_rate: Math.round((config.speed - 1) * 100) },
        },
      }),
    },
  ...
  for (const chunk of chunks)
    if (typeof chunk?.code === "number" && chunk.code !== 0 && chunk.code !== 20_000_000) {
...
async function fish(ctx: PluginContext, value: string, config: SayConfig): Promise<Omit<SynthResult, "provider">> {
  const current = config.providers.fish;
  const host = "api.fish.audio";
  const output = await ctx.http.withResponse(
    `https://${host}/v1/tts`,
    {
      method: "POST",
      credentials: "omit",
...
export async function synthesize(
  ...
  value: string,
  config: SayConfig,
  progress?: (provider: ProviderName, attempt: number) => Promise<void>,
): Promise<SynthResult> {
  const order = providerOrder(config);
  if (!order.length) throw new Error("未配置任何语音服务商");
  for (let index = 0; index < order.length; index++) {
    const provider = order[index]!;
    await progress?.(provider, index + 1);
    try {
      const result =
        provider === "mimo"
          ? await mimo(ctx, value, config)
          : provider === "volc"
            ? await volc(ctx, value, config)
            : await fish(ctx, value, config);
