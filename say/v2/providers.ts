...
async function bytes(response: Response, signal: AbortSignal, maximum: number): Promise<Buffer> {
  ...
  const reader = response.body.getReader(); const chunks: Buffer[] = []; let total = 0, done = false, cancelPromise: Promise<void> | undefined;
  const cancel = () => cancelPromise ??= reader.cancel().catch(() => undefined); const onAbort = () => { void cancel(); };
  signal.addEventListener("abort", onAbort, {once: true});
  try {
    for (;;) {
      signal.throwIfAborted(); const part = await reader.read(); signal.throwIfAborted(); if (part.done) {done = true; break;}
