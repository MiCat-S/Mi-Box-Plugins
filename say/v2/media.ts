...
function caption(text: string): string {
  let body="";for(const character of text){const next=escape(character);if(body.length+next.length>900){body+="…";break;}body+=next;}
  return `<blockquote>🇺🇳 ${body}</blockquote>`;
}

async function opus(ctx: PluginContext, directory: string, source: Buffer, extension: string, signal:AbortSignal): Promise<string> {
  ...
  if (extension === "ogg" || extension === "opus") {
    await writeFile(output, source, {mode: 0o600, flag: "wx",signal}); return output;
  }
  const ffmpeg = await findFfmpeg(ctx);
  if (!ffmpeg) throw new Error("MiMo/Fish 输出需要 FFmpeg；请由系统管理员安装后重试");
  await writeFile(input, source, {mode: 0o600, flag: "wx",signal});signal.throwIfAborted();
  await ctx.processes.run(ffmpeg.path, ["-nostdin", "-v", "error", "-y", "-protocol_whitelist", "file", "-i", input, "-vn", "-c:a", "libopus", "-b:a", "64k", "-ar", "48000", "-ac", "1","-fs",String(32*1024*1024), output],
    {cwd:directory,signal,timeoutMs: 180_000, maxOutputBytes: 256 * 1024});
  signal.throwIfAborted();const info = await stat(output);signal.throwIfAborted(); if (!info.isFile() || !info.size || info.size > 32 * 1024 * 1024) throw new Error("FFmpeg 未生成有效语音");
  return output;
}
...
export async function sendVoice(ctx: PluginContext, invocation: {message: MessageEnvelope}, text: string, config: SayConfig,
  ...
  let published:any;try{return await ctx.files.withTemp(async (directory, signal) => {
    ...
    const sent = await ctx.telegram.withClient(async client => {
      ...
      signal.throwIfAborted();return client.sendFile(raw.peerId, {file: output, voiceNote: true, forceDocument: false,
    ...
    published=sent;signal.throwIfAborted();
    try {const lines=config.translate?await translate(ctx,text):[];signal.throwIfAborted();const pages=await ui.renderDocument({title:"Say 文本",sections:[ui.section("原文",[ui.text(text)]),...lines.map(x=>ui.section(x.label,[ui.text(x.value)]))]});if(lines.length||text.length>700){const delivery=await ui.deliverPages(pages,signal,page=>ctx.telegram.reply(invocation.message,page,{parseMode:"html"}));signal.throwIfAborted();if(delivery.interrupted)ctx.log.error("say_translation_receipt_failed");}}catch{signal.throwIfAborted();ctx.log.error("say_translation_receipt_failed");}
    return sent;
  });}catch(error){if(ctx.signal.aborted)throw ctx.signal.reason;if(published){ctx.log.error("say_temp_cleanup_failed");return published;}throw error;}
}

export async function deleteSource(ctx:PluginContext,message: MessageEnvelope): Promise<void> {
  const raw = message.raw as any;
  ctx.signal.throwIfAborted();if (typeof raw?.delete === "function")try{await raw.delete({revoke:true});ctx.signal.throwIfAborted();}catch{ctx.signal.throwIfAborted();ctx.log.error("say_source_delete_failed");}
}
