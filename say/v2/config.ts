...
export function normalizeConfig(value: any): SayConfig {
  ...
  return {
    ...
    providers: {
      mimo: {
        apiKey: text(providers.mimo?.apiKey),
        voice: text(providers.mimo?.voice, "冰糖").slice(0, 80),
        endpoint: providers.mimo?.endpoint === "tokenplan" ? "tokenplan" : "standard",
      },
      volc: {
        apiKey: text(providers.volc?.apiKey ?? providers.volc?.token),
        resourceId: text(providers.volc?.resourceId, "seed-tts-2.0").slice(0, 80),
        voice: text(providers.volc?.voice).slice(0, 120),
      },
      fish: {
        apiKey: text(providers.fish?.apiKey),
        voice: text(providers.fish?.voice, FISH_ROLES["雷军"]).slice(0, 128),
      },
    },
    primary: source.primary === "volc" || source.primary === "fish" ? source.primary : "mimo",
    speed: Number.isFinite(speed) && speed >= 0.5 && speed <= 2 ? speed : 1,
    style: text(source.style).slice(0, 500),
    translate: typeof source.translate === "boolean" ? source.translate : true,
    chats:
      source.chats && typeof source.chats === "object" && !Array.isArray(source.chats)
        ? Object.fromEntries(
            Object.entries(source.chats).filter(([key, enabled]) => /^-?[0-9]+$/.test(key) && enabled === true),
          )
        : {},
  };
}
...
export function providerOrder(config: SayConfig): ProviderName[] {
  return [
    config.primary,
    ...(["volc", "mimo", "fish"] as ProviderName[]).filter(value => value !== config.primary),
