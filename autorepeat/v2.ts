...
const defaults: State = {
  ...
  dailyHistory: {},
  lastDay: 0,
  trigger: { timeWindow: 300, minUsers: 5 },
};
...
function normalize(value: unknown): State {
  ...
  const enabledGroups = (
    Array.isArray(source.enabledGroups) ? source.enabledGroups : Array.isArray(legacyGroups) ? legacyGroups : []
  )
    .map(String)
    .filter(id => /^-?\d+$/.test(id));
  ...
  if (sourceHistory && typeof sourceHistory === "object")
    for (const [id, values] of Object.entries(sourceHistory)) {
      if (/^-?\d+$/.test(id) && Array.isArray(values))
        dailyHistory[id] = [...new Set(values.map(String))].slice(0, 5000);
    }
  const rawTrigger = source.trigger ?? source.trigger_config ?? {};
  const timeWindow = Number(rawTrigger.timeWindow),
    minUsers = Number(rawTrigger.minUsers);
  return {
    ...source,
    schemaVersion: 1,
    enabledGroups: [...new Set(enabledGroups)],
    dailyHistory,
    lastDay: Number.isInteger(Number(source.lastDay ?? source.last_day_check))
      ? Number(source.lastDay ?? source.last_day_check)
      : 0,
    trigger: {
      timeWindow: Number.isInteger(timeWindow) && timeWindow > 0 && timeWindow <= 86400 ? timeWindow : 300,
  ...
  void context.tasks.run(`autorepeat:expire:${chatId}:${id}`, async signal => {
    try {
      ...
      await context.telegram.withClient(async client =>
        client.deleteMessages(returnBigInt(chatId), [id], { revoke: true }),
      );
    } catch {
...
async function processMessage(message: MessageEnvelope, context: PluginContext, { recent, serial }: RuntimeState) {
  ...
  const current = previous
    ...
    .then(async () => {
      ...
      const state = await updateState(context, value => {
        ...
        const token = contentKey(message.text),
          legacy = legacyContentKey(message.text),
          history = value.dailyHistory[chatId] ?? [];
      ...
      await context.telegram.withClient(async client =>
        client.sendMessage(returnBigInt(chatId), { message: message.text }),
      );
    });
  ...
  try {
    ...
    if (serial.get(chatId) === current) serial.delete(chatId);
  }
}
...
export default function createPlugin() {
  ...
  return definePlugin({
    ...
    apiVersion: 1,
    id: "autorepeat",
    description: "在群组内达到不同用户人数阈值后自动复读相同文本。",
    commands: {
      autorepeat: {
          ...
          try {
            ...
            if (action === "allon") {
              ...
              await updateState(context, state => ({
                ...state,
                enabledGroups: [...new Set([...state.enabledGroups, ...ids])],
              }));
              await edit(context, message, `✅ 已开启 ${ids.length} 个群组的自动复读`);
              return;
            }
            if (action === "alloff") {
              await updateState(context, state => ({ ...state, enabledGroups: [] }));
              await edit(context, message, "✅ 已关闭所有群组的自动复读");
              return;
            }
            if (action === "set") {
              const timeWindow = Number(args[1]),
                minUsers = Number(args[2]);
              if (
                !Number.isInteger(timeWindow) ||
                timeWindow < 1 ||
                timeWindow > 86400 ||
                !Number.isInteger(minUsers) ||
                minUsers < 2 ||
                minUsers > 1000
              ) {
                await edit(
                  context,
                  message,
                  `❌ 参数错误\n使用格式: <code>${prefix}autorepeat set [1-86400秒] [2-1000人]</code>`,
                  true,
                );
                return;
              }
            ...
            if (action === "list") {
              const state = await store(context).read(),
                ...
                ids = state.enabledGroups,
                pages = Math.max(1, Math.ceil(ids.length / 20));
              if (!ids.length) {
                await edit(context, message, "📝 当前没有开启自动复读的群组");
                return;
              }
              const rows: string[] = [];
              for (const id of ids.slice((page - 1) * 20, page * 20)) {
                try {
                  const title = await context.telegram.withClient(async client =>
                    String(((await client.getEntity(returnBigInt(id))) as any).title ?? id),
                  );
                  rows.push(`• <b>${escape(title)}</b> (<code>${escape(id)}</code>)`);
                } catch {
                  rows.push(`• <code>${escape(id)}</code> (无法获取信息)`);
                }
              }
              await edit(
                context,
                message,
                `📝 <b>已开启自动复读群组 (${ids.length}):</b>\n<b>第 ${Math.min(page, pages)}/${pages} 页</b>\n\n${rows.join("\n")}` +
                  (pages > 1 ? `\n\n使用 <code>${prefix}autorepeat list ${page + 1}</code> 查看下一页` : ""),
                true,
              );
              return;
            }
            if (["on", "off"].includes(action)) {
              ...
              await updateState(context, state => ({
                ...
                enabledGroups:
                  action === "on"
                    ? [...new Set([...state.enabledGroups, group.id])]
                    : state.enabledGroups.filter(id => id !== group.id),
              }));
              await edit(
                context,
                message,
                `${action === "on" ? "✅ 已开启" : "❌ 已关闭"} <b>${escape(group.title)}</b> 的自动复读`,
                true,
                3,
              );
              return;
            ...
            if (!context.signal.aborted) await edit(context, message, "❌ 操作失败，请稍后重试");
          }
        },
      },
    },
    listeners: [
      {
        ...
        async handle(message, context) {
          ...
          const { chatId, id } = message;
          try {
            await processMessage(message, context, runtime);
          } catch {
            if (!context.signal.aborted) context.log.error("autorepeat:listener_failed", { chatId, messageId: id });
          }
        },
      },
    ],
