...
const fmt = (ms: number) =>
  `${ms >= 0 ? "+" : "-"}${Math.abs(ms) >= 1000 ? `${(Math.abs(ms) / 1000).toFixed(3)}s` : `${Math.abs(ms).toFixed(1)}ms`}`;
      ...
      ntp: {
        ...
        async handle(invocation, ctx) {
          ...
          try {
            ...
            if (mode !== "s") {
              await ctx.telegram.edit(invocation.message, header, { parseMode: "html" });
              return;
            }
            const executable =
              process.platform === "linux" ? "/bin/date" : process.platform === "darwin" ? "/bin/date" : "";
            if (!executable) {
              await ctx.telegram.edit(invocation.message, `${header}\n• 当前平台不支持自动设置`, { parseMode: "html" });
              return;
            }
            const args =
              process.platform === "linux"
                ? ["-u", "-s", `@${Math.floor(result.serverTime / 1000)}`]
                : ["-u", new Date(result.serverTime).toISOString().replace(/[-:T]/g, "").slice(4, 16)];
            try {
            ...
            if (!ctx.signal.aborted)
              await ctx.telegram.edit(invocation.message, `❌ ${mode === "s" ? "对时" : "查询"}失败，请稍后重试`);
          }
        },
      },
    },
