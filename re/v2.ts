        ...
        async handle(invocation, ctx) {
          ...
          if (!reply) {
            await ctx.telegram.edit(
              invocation.message,
              `你必须回复一条消息才能够进行复读；使用 ${invocation.prefix}re [数量] [次数]`,
            );
            return;
          }
          if (!replied?.peerId) {
            await ctx.telegram.reply(invocation.message, "无法获取被回复的消息，请重试。");
            return;
          }
          ...
          try {
            await ctx.telegram.withClient(async (client, signal) => {
              ...
              signal.throwIfAborted();
              const source = await replied.getInputChat();
              signal.throwIfAborted();
              const commandRaw = invocation.message.raw as ApiTypes.Message | undefined;
              const target = await commandRaw?.getInputChat();
              signal.throwIfAborted();
              ...
              signal.throwIfAborted();
              if (typeof commandRaw?.delete === "function") {
                await commandRaw.delete({ revoke: true });
                signal.throwIfAborted();
              }
              ...
              if (!copy) return;
              for (let index = forwarded; index < repeat; index++)
                for (const message of messages) {
                  signal.throwIfAborted();
                  const options = {
                    ...(topic ? { replyTo: topic } : {}),
                    ...(message.entities?.length ? { formattingEntities: message.entities } : {}),
                  };
                  if (message.media) {
                    await client.sendFile(target, { ...options, file: message.media, caption: message.message ?? "" });
                    signal.throwIfAborted();
                  } else if (message.message) {
                    await client.sendMessage(target, { ...options, message: message.message });
                    signal.throwIfAborted();
                  }
                }
            });
