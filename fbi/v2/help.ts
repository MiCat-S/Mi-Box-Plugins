...
export function renderHelp(prefix: string): string {
  ...
  return `🕵️ <b>公开群消息查询与蹲守</b>

从当前账号可见、已缓存的公开群消息中查询目标，并在观察到目标的新消息时通知。

<b>查询与任务：</b>
• <code>${p}fbi det [目标]</code> — 查询缓存中目标最新的一条消息
• <code>${p}fbi loc [目标]</code> — 按缓存消息数量寻找目标最活跃的群
• <code>${p}fbi sur [目标]</code> — 蹲守目标下一条可见的公开群消息
• <code>${p}fbi obs [目标]</code> — 在当前群定点蹲守
• <code>${p}fbi obs 群链接 [目标]</code> — 在指定公开群定点蹲守
...
<b>范围与任务行为：</b>
• 结果限于账号可访问且拥有公开用户名的群组，以及当前缓存内容；缓存未命中表示尚未收录相关消息。
• 每群最多保留 3000 条、最近 30 天的消息；重建范围受对话数量上限和账号可见历史限制。
