...
const delay=(ms:number,signal:AbortSignal)=>{signal.throwIfAborted();return new Promise<void>((resolve,reject)=>{const timer=setTimeout(done,ms);function done(){signal.removeEventListener("abort",abort);resolve();}function abort(){clearTimeout(timer);reject(signal.reason);}signal.addEventListener("abort",abort,{once:true});});};
...
async function target(c:PluginContext,message:MessageEnvelope,arg?:string){if(!arg&&message.replyToId){const reply=await c.telegram.getReply(message);if(reply?.senderId)arg=reply.senderId;}if(!arg)return;return c.telegram.withClient(async(client,signal)=>{try{const key:any=/^-?\d+$/.test(arg!)?returnBigInt(arg!):arg!;const e:any=await client.getEntity(key);signal.throwIfAborted();const id=String(e?.id??arg).replace(/^@/,"");const plain=esc([e?.firstName,e?.lastName].filter(Boolean).join(" ")||e?.title||id);return{id,name:e?.username?`<a href="https://t.me/${encodeURIComponent(e.username)}">${plain}</a>`:plain};}catch{signal.throwIfAborted();return{id:arg!.replace(/^@/,""),name:esc(arg!)};}});}
...
export default function createFbi(){let context:PluginContext|undefined,chats=new Map<string,Chat>(),watches=new Map<string,Watch>(),rebuilding=false,cacheLimit=300,activityClock=0,rebuildIncrements=new Map<string,Map<number,Cached>>();
 const sendHtml=async(c:PluginContext,message:MessageEnvelope,html:string)=>{
  ...
  const delivery=await ui.deliverPages(pages,c.signal,(page,index)=>index===0
  ...
  if(delivery.interrupted)c.log.error("fbi_delivery_failed",{published:delivery.published,total:delivery.total});
 };
  ...
  try{
   ...
   await c.telegram.withClient(async(client,signal)=>{
    ...
    for(const d of dialogs.slice(0,limit)){
     ...
     let entity:any;try{entity=await client.getEntity(d.id);signal.throwIfAborted();}catch{signal.throwIfAborted();continue;}
 ...
 return definePlugin({renderHelp: renderPluginHelp, apiVersion:1,id:"fbi",description:"跨群组消息追踪与公开群缓存",commands:{fbi:{helpArgs: ["help", "h"], helpOnEmpty: true, description:"搜索、监视或定位用户",ignoreEdited:true,async handle({message,args,prefix},c){try{const sub=args[0]?.toLowerCase();if(!sub){await sendHtml(c,message,renderPluginHelp(prefix));return;}if(sub==="ssv"){const stopped=[...watches.values()];if(!stopped.length){await c.telegram.edit(message,"当前没有活跃的蹲守任务");return;}for(const watch of stopped)if(watches.get(watch.targetId)===watch)watches.delete(watch.targetId);try{await cfg(c).update(v=>({...v,surveillance:Object.fromEntries(watches)}));}catch(error){for(const watch of stopped)if(!watches.has(watch.targetId))watches.set(watch.targetId,watch);throw error;}for(const watch of stopped){try{await c.telegram.edit({id:watch.triggerMsgId,chatId:watch.triggerPeer,outgoing:true,text:""},`蹲守过程中没有发现 ${watch.targetName} 的行踪。`);}catch{c.signal.throwIfAborted();}}await c.telegram.edit(message,"已终止所有蹲守任务");return;}if(sub==="cache"){if(args[1]==="limit"){const n=Number(args[2]);if(!Number.isInteger(n)||n<10||n>MAX_GROUPS)throw new UserError("缓存上限须为 10 至 1000");await applyLimit(normalizeConfig(await cfg(c).update(v=>normalizeConfig({...v,cacheLimit:n}))));await c.telegram.edit(message,`缓存上限已设置为 ${n}`);return;}if(args[1]==="rebuild"){await c.telegram.edit(message,"正在重建公开群缓存…");await rebuild(c);await c.telegram.edit(message,`缓存重建完成，共 ${chats.size} 个群组`);return;}const state=normalizeConfig(await cfg(c).read());await c.telegram.edit(message,`缓存群组：${chats.size}\n缓存上限：${state.cacheLimit}\n状态：${rebuilding?"重建中":"就绪"}`);return;}if(!TARGET_COMMANDS.has(sub)){await sendHtml(c,message,renderPluginHelp(prefix));return;}let arg=args[1],scope:string|undefined;if(sub==="obs"&&arg&&/^(?:https?:\/\/)?t\.me\//i.test(arg)){const username=arg.replace(/^(?:https?:\/\/)?t\.me\//i,"").split(/[/?#]/)[0]!.toLowerCase();for(const [id,chat] of chats)if(chat.username?.toLowerCase()===username)scope=id;if(!scope)scope=await c.telegram.withClient(async(client,signal)=>{const entity:any=await client.getEntity(username);signal.throwIfAborted();if(!isPublicChat(entity))throw new UserError("请提供公开群组或频道链接");return`${entity.className==="Channel"?"-100":"-"}${entity.id}`;});arg=args[2];}else if(sub==="obs")scope=message.chatId;if(sub==="obs"&&!isGroupPeer(scope??""))throw new UserError("请在群组中使用，或提供公开群组链接");const who=await target(c,message,arg);if(!who)throw new UserError("无法识别目标，请回复消息或提供用户名/ID");if(sub==="sur"||sub==="obs"){const watch:Watch={targetId:who.id,targetName:who.name,triggerPeer:message.chatId,triggerMsgId:message.id,...(scope?{scopePeer:scope}:{})};watches.set(who.id,watch);await cfg(c).update(v=>({...normalizeConfig(v),surveillance:Object.fromEntries(watches)}));await c.telegram.edit(message,`${sub==="obs"?"正在定点监视":"正在蹲守"} ${who.name}`,{parseMode:"html"});return;}if(sub==="det"){let found:{peer:string;chat:Chat;msg:Cached}|undefined;for(const [peer,chat] of chats)for(const msg of chat.msgs)if(msg.senderId===who.id&&(!found||msg.date>found.msg.date))found={peer,chat,msg};await c.telegram.edit(message,found?`发现 ${who.name} 的最新消息：\n${link(found.peer,found.chat,found.msg)}`:`暂未发现 ${who.name} 的公开群消息`,{parseMode:"html",linkPreview:false});return;}if(sub==="loc"){let best:{peer:string;chat:Chat;count:number;msg?:Cached}|undefined;for(const [peer,chat] of chats){const items=chat.msgs.filter(x=>x.senderId===who.id);if(items.length&&(!best||items.length>best.count))best={peer,chat,count:items.length,msg:items[0]};}await c.telegram.edit(message,best?`${who.name} 最活跃的群组：${best.msg?link(best.peer,best.chat,best.msg):esc(best.chat.title??best.peer)}（${best.count} 条）`:`暂未定位到 ${who.name} 的活跃群组`,{parseMode:"html",linkPreview:false});return;}await sendHtml(c,message,renderPluginHelp(prefix));}catch(e){c.signal.throwIfAborted();c.log.error("fbi_command_failed",{kind:e instanceof UserError?"input":"internal"});await c.telegram.edit(message,e instanceof UserError?`操作失败：${esc(e.message)}`:"操作失败，请稍后重试",{parseMode:"html"});}}}},listeners:[{edited:true,ignoreCommands:true,async handle(message,c){const raw:any=message.raw;if(!isGroupPeer(message.chatId))return;const peer=message.chatId;let chat=chats.get(peer);if(!chat){try{const entity:any=await c.telegram.withClient(async(client,signal)=>{const value=await client.getEntity(raw?.peerId??returnBigInt(peer));signal.throwIfAborted();return value;});if(isPublicChat(entity))chat=chats.get(peer)??{username:String(entity.username),title:String(entity.title??entity.username),msgs:[]};}catch{c.signal.throwIfAborted();}if(chat)chats.set(peer,chat);}if(chat){const observed=stripped(message);upsert(chat,observed);touch(peer,chat);if(rebuilding)recordIncrement(peer,observed);await schedule();}const watch=message.senderId?watches.get(message.senderId):undefined;if(!watch||watch.scopePeer&&watch.scopePeer!==peer||watch.triggerPeer===peer&&watch.triggerMsgId===message.id)return;let entity:any;try{entity=await c.telegram.withClient(async(client,signal)=>{const value=await client.getEntity(raw?.peerId??returnBigInt(peer));signal.throwIfAborted();return value;});}catch{c.signal.throwIfAborted();return;}if(!isPublicChat(entity)||watches.get(watch.targetId)!==watch)return;watches.delete(watch.targetId);try{await cfg(c).update(v=>({...normalizeConfig(v),surveillance:Object.fromEntries(watches)}));}catch(error){if(!watches.has(watch.targetId))watches.set(watch.targetId,watch);throw error;}const current:Chat={username:String(entity.username),title:String(entity.title??entity.username),msgs:[stripped(message)]},result=`发现 ${watch.targetName} 最新动向：\n${link(peer,current,stripped(message))}`;await c.telegram.withClient(async(client,signal)=>{signal.throwIfAborted();await client.sendMessage(returnBigInt(watch.triggerPeer),{message:result,parseMode:"html",linkPreview:false});signal.throwIfAborted();await client.sendMessage("me",{message:result,parseMode:"html",linkPreview:false});signal.throwIfAborted();});}}],settings:c=>({id:"fbi",title:"FBI 跨群追踪",description:"跨群公开消息缓存配置",category:"插件配置",icon:"🕵️",getSchema:()=>[{key:"cacheLimit",label:"缓存群组上限",type:"number",min:10,max:1000}],getValues:()=>cfg(c).read(),setValues:async patch=>{await applyLimit(normalizeConfig(await cfg(c).update(v=>normalizeConfig({...v,...patch}))));}}),async setup(c){context=c;await migrate(c);const state=normalizeConfig(await cfg(c).read()),saved=normalizeCache(await cacheStore(c).read());watches=new Map(Object.entries(state.surveillance));cacheLimit=state.cacheLimit;const restored=restore(saved.cache,cacheLimit);chats=restored.chats;activityClock=Math.max(0,...Array.from(chats.values(),activity));if(restored.changed)await persist();}});}
